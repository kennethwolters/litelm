...
from litelm.types.guardrails import ApplyGuardrailRequest, ApplyGuardrailResponse
...
async def test_bedrock_apply_guardrail_endpoint_integration():
    ...
    from litelm.proxy.guardrails.guardrail_endpoints import apply_guardrail
    ...
    with patch(
        "litelm.proxy.guardrails.guardrail_endpoints.GUARDRAIL_REGISTRY"
    ) as mock_registry:
        # Mock the make_bedrock_api_request method
        with patch.object(
            guardrail, "make_bedrock_api_request", new_callable=AsyncMock
...
async def test_bedrock_apply_guardrail_blocked_with_disable_exception_on_block():
    ...
    """
    from litelm.exceptions import GuardrailInterventionNormalStringError
