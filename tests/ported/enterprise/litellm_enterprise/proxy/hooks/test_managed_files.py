...
from litelm_enterprise.proxy.hooks.managed_files import _PROXY_LiteLLMManagedFiles

from litelm.caching import DualCache
from litelm.proxy._types import CallTypes
from litelm.proxy.openai_files_endpoints.common_utils import (
...
async def test_async_pre_call_hook_batch_retrieve():
    ...
    prisma_client.db.litelm_managedobjecttable.find_first.return_value = return_value
    proxy_managed_files = _PROXY_LiteLLMManagedFiles(
...
@pytest.mark.asyncio
async def test_async_pre_call_deployment_hook_resolves_model_id_from_litelm_metadata():
    """
    For batch operations the router stores model_info under
    kwargs["litelm_metadata"]["model_info"] (not top-level kwargs["model_info"]).
    ...
    # model_info is nested under litelm_metadata (batch path)
    kwargs = {
...
async def test_async_pre_call_deployment_hook_prefers_top_level_model_info():
    ...
    should use it without falling back to litelm_metadata.
    """
    proxy_managed_files = _PROXY_LiteLLMManagedFiles(
        DualCache(), prisma_client=MagicMock()
    )
    ...
    assert result["input_file_id"] == top_level_provider_file, (
        "Should prefer top-level model_info over litelm_metadata"
    )
...
async def test_async_pre_call_deployment_hook_no_model_info_leaves_file_id_unchanged():
    """
    When model_info is absent from both top-level and litelm_metadata,
...
async def test_async_post_call_success_hook_for_unified_finetuning_job():
    from litelm.types.utils import LiteLLMFineTuningJob
...
async def test_can_user_call_unified_file_id(call_type):
    ...
    return_value = MagicMock()
    return_value.created_by = "123"
    prisma_client.db.litelm_managedfiletable.find_first.return_value = return_value
    proxy_managed_files = _PROXY_LiteLLMManagedFiles(
...
async def test_router_acreate_batch_only_selects_from_file_id_mapping(monkeypatch):
    ...
    with patch.object(
        litelm, "acreate_batch", return_value=AsyncMock()
...
async def test_output_file_id_for_batch_retrieve():
    ...
    from litelm.types.utils import LiteLLMBatch
    ...
    batch._hidden_params = {
        "litelm_call_id": "dcd789e0-c0ad-4244-9564-4e611448d650",
        "api_base": "https://api.openai.com",
        "model_id": "12345679",
        "response_cost": 0.0,
        "additional_headers": {},
        "litelm_model_name": "gpt-4o",
        "unified_batch_id": "litelm_proxy;model_id:12345679;llm_batch_id:batch_685c5e5d63988190b85bdb2147ba131d",
    }
    proxy_managed_files = _PROXY_LiteLLMManagedFiles(
        DualCache(), prisma_client=AsyncMock()
    )
...
async def test_error_file_id_for_failed_batch():
    ...
    batch._hidden_params = {
        ...
        "additional_headers": {},
        "litelm_model_name": "gpt-4o",
        "unified_batch_id": "litelm_proxy;model_id:test-model-id;llm_batch_id:batch_abc123",
    }
    ...
    with patch("litelm.afile_retrieve", new_callable=AsyncMock) as mock_retrieve:
...
def test_update_responses_input_with_unified_file_id():
    ...
    """
    from litelm.litelm_core_utils.prompt_templates.common_utils import (
        update_responses_input_with_model_file_ids,
    )

    # Create a base64-encoded unified file ID
    # This decodes to: litelm_proxy:application/pdf;unified_id,6c0b5890-8914-48e0-b8f4-0ae5ed3c14a5;target_model_names,gpt-4o;llm_output_file_id,file-ECBPW7ML9g7XHdwGgUPZaM;llm_output_file_model_id,e26453f9e76e7993680d0068d98c1f4cc205bbad0967a33c664893568ca743c2
...
def test_update_responses_input_with_model_file_id_mapping():
    ...
    managed_file_id = "litelm_proxy_file_123"
...
def test_update_responses_tools_with_model_file_id_mapping():
    ...
    # Managed file IDs
    managed_file_id_1 = "litelm_proxy_file_123"
    managed_file_id_2 = "litelm_proxy_file_456"
...
async def test_completion_with_file_access_check():
    ...
    prisma_client = AsyncMock()
    prisma_client.db.litelm_managedfiletable.find_first = AsyncMock(return_value=None)
...
async def test_store_unified_file_id_with_none_file_object():
    ...
    prisma_client = AsyncMock()
    prisma_client.db.litelm_managedfiletable.create = AsyncMock(return_value=MagicMock())
    internal_usage_cache = MagicMock()
    ...
    prisma_client.db.litelm_managedfiletable.create.assert_called_once()
    call_args = prisma_client.db.litelm_managedfiletable.create.call_args
...
async def test_afile_delete_returns_provider_response_when_stored_file_object_none():
    ...
    prisma_client.db.litelm_managedfiletable.find_first = AsyncMock(return_value=db_record)
    prisma_client.db.litelm_managedfiletable.delete = AsyncMock()
...
async def test_afile_retrieve_fetches_from_provider_when_file_object_none():
    ...
    with patch("litelm.afile_retrieve", new_callable=AsyncMock) as mock_afile_retrieve:
...
async def test_list_batches_from_managed_objects_table():
    ...
    prisma_client.db.litelm_managedobjecttable.find_many.return_value = [
        batch_record_1,
        batch_record_2,
    ]
    ...
    prisma_client.db.litelm_managedobjecttable.find_many.assert_called_once_with(
...
def _create_unified_batch_id(model_id: str, batch_id: str) -> str:
    import base64
    unified_str = f"litelm_proxy;model_id:{model_id};llm_batch_id:{batch_id}"
...
async def test_list_batches_from_managed_objects_table_provider_filter_raises_exception():
    ...
    prisma_client.db.litelm_managedobjecttable.find_many.assert_not_called()
...
async def test_list_batches_from_managed_objects_table_filters_by_created_by():
    ...
    prisma_client.db.litelm_managedobjecttable.find_many.return_value = [batch_user1]
    ...
    prisma_client.db.litelm_managedobjecttable.find_many.assert_called_with(
    ...
    prisma_client.db.litelm_managedobjecttable.find_many.return_value = [batch_user2]
...
# Reference: https://github.com/BerriAI/litelm/pull/17401/files
...
async def test_user_b_cannot_retrieve_user_a_batch():
    ...
    prisma_client.db.litelm_managedobjecttable.find_first.return_value = batch_record
...
async def test_user_b_cannot_retrieve_user_a_file():
    ...
    prisma_client.db.litelm_managedfiletable.find_first.return_value = file_record
...
async def test_list_batches_only_returns_user_own_batches():
    ...
    prisma_client.db.litelm_managedobjecttable.find_many.return_value = [batch_user_a]
