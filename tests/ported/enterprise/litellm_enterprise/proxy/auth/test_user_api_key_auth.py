...
from litelm_enterprise.proxy.auth.user_api_key_auth import enterprise_custom_auth
...
async def test_enterprise_custom_auth_mode_on():
    ...
    with patch(
        "litelm_enterprise.proxy.proxy_server.custom_auth_settings", {"mode": "on"}
    ):
...
async def test_enterprise_custom_auth_mode_auto_with_error():
    ...
    with patch(
        "litelm_enterprise.proxy.proxy_server.custom_auth_settings", {"mode": "auto"}
    ):
        result = await enterprise_custom_auth(request, "test-api-key", mock_user_auth)
        assert result is None
        mock_user_auth.assert_called_once_with(request, "test-api-key")
...
async def test_enterprise_custom_auth_returns_string():
    from litelm.proxy._types import hash_token
    ...
    with patch(
        "litelm.proxy.auth.user_api_key_auth.enterprise_custom_auth", mock_user_auth
    ), patch("litelm.proxy.proxy_server.master_key", "sk-1234"), patch(
