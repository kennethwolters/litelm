...
from litelm_enterprise.proxy.management_endpoints.internal_user_endpoints import router
...
def mock_user_api_key_auth():
    ...
    with patch(
        "enterprise.litelm_enterprise.proxy.management_endpoints.internal_user_endpoints.user_api_key_auth"
    ) as mock_auth:
...
class TestAvailableEnterpriseUsers:
    ...
    async def test_available_users_with_max_users_set(
        ...
        with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma, patch(
            ...
            # Mock database count
            mock_prisma.db.litelm_usertable.count = AsyncMock(return_value=5)
            mock_prisma.db.litelm_teamtable.count = AsyncMock(return_value=2)
    ...
    async def test_available_users_without_max_users_set(
        ...
        with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma, patch(
            ...
            # Mock database count
            mock_prisma.db.litelm_usertable.count = AsyncMock(return_value=3)
            mock_prisma.db.litelm_teamtable.count = AsyncMock(return_value=1)
    ...
    async def test_available_users_negative_remaining_bug(
        ...
        with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma, patch(
            ...
            mock_prisma.db.litelm_usertable.count = AsyncMock(return_value=8)
            mock_prisma.db.litelm_teamtable.count = AsyncMock(return_value=3)
