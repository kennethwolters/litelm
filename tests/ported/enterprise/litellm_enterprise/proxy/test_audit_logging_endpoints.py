...
from litelm_enterprise.proxy.audit_logging_endpoints import router as audit_router
from litelm_enterprise.types.proxy.audit_logging_endpoints import AuditLogResponse
...
def mock_prisma_client():
    with patch("litelm.proxy.proxy_server.prisma_client") as mock:
        mock.db.litelm_auditlog.find_many = AsyncMock()
        mock.db.litelm_auditlog.find_unique = AsyncMock()
        mock.db.litelm_auditlog.count = AsyncMock()
        yield mock
...
async def test_get_audit_logs(mock_prisma_client):
    ...
    mock_prisma_client.db.litelm_auditlog.find_many.return_value = [
        AuditLogResponse(**MOCK_AUDIT_LOG)
    ]
    mock_prisma_client.db.litelm_auditlog.count.return_value = 1

    # Mock the auth dependency
    with patch("litelm.proxy.auth.user_api_key_auth.user_api_key_auth") as mock_auth:
...
async def test_get_audit_log_by_id(mock_prisma_client):
    ...
    mock_prisma_client.db.litelm_auditlog.find_unique.return_value = AuditLogResponse(
        **MOCK_AUDIT_LOG
    )
...
async def test_get_audit_log_by_id_not_found(mock_prisma_client):
    ...
    mock_prisma_client.db.litelm_auditlog.find_unique.return_value = None
