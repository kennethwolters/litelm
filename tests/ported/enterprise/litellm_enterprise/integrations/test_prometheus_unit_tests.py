...
from litelm.types.utils import BudgetConfig, GenericBudgetConfigType
...
async def test_async_prometheus_success_logging_with_callbacks(prometheus_logger):
    ...
    diff = await op()
    await asyncio.sleep(2)
    assert diff["litelm_requests_metric_total"] == 1.0
...
async def test_async_prometheus_budget_logging_with_callbacks(prometheus_logger):
    ...
    # TODO: Should implement `litelm_provider_remaining_budget_metric` in prometheus.py
    assert diff.get("litelm_provider_remaining_budget_metric", 50.0) == 50.0
...
async def test_prometheus_metric_tracking():
    ...
    litelm._async_success_callback = [mock_prometheus]
...
class CustomPrometheusLogger(PrometheusLogger):
    ...
    def set_deployment_complete_outage(
        self,
        litelm_model_name: str,
        model_id: str,
        api_base: str,
        api_provider: str,
    ):
        self.deployment_complete_outages.append(
            [litelm_model_name, model_id, api_base, api_provider]
        )
    ...
    def increment_deployment_cooled_down(
        ...
        self.deployment_cooled_downs.append(
            [litelm_model_name, model_id, api_base, api_provider, exception_status]
        )
