...
import litelm
from litelm.constants import PROMETHEUS_BUDGET_METRICS_REFRESH_INTERVAL_MINUTES
...
def test_initialize_budget_metrics_cron_job():
    ...
    litelm.callbacks = [prometheus_logger]
...
def test_end_user_not_tracked_for_all_prometheus_metrics():
    ...
    original_setting = getattr(
        litelm, "enable_end_user_cost_tracking_prometheus_only", None
    )
    litelm.enable_end_user_cost_tracking_prometheus_only = None  # Default behavior
    ...
    try:
        ...
        litelm.enable_end_user_cost_tracking_prometheus_only = True
        ...
        litelm.enable_end_user_cost_tracking_prometheus_only = original_setting
...
def test_prometheus_config_parsing():
    ...
    test_config = [
        {
            ...
            "metrics": [
                "litelm_deployment_failure_responses",
                "litelm_deployment_total_requests",
                "litelm_proxy_failed_requests_metric",
                "litelm_proxy_total_requests_metric",
            ],
            "include_labels": [
                "requested_model",
                "team",
    ...
    # Set configuration
    litelm.prometheus_metrics_config = test_config
...
def test_get_metric_labels():
    ...
    test_config = [
        {
            "group": "service_metrics",
            "metrics": ["litelm_deployment_failure_responses"],
            "include_labels": ["litelm_model_name", "api_provider"],
        }
    ]
    ...
    # Get filtered labels
    labels = logger.get_labels_for_metric("litelm_deployment_failure_responses")

    # Verify only configured labels are returned
    assert "litelm_model_name" in labels
    assert "api_provider" in labels
    # These should be filtered out even if they're in the default labels
    assert (
        len([l for l in labels if l not in ["litelm_model_name", "api_provider"]]) == 0
    )
...
def test_no_prometheus_config():
    ...
    litelm.prometheus_metrics_config = None
...
def test_prometheus_metrics_config_type():
    ...
    valid_config = PrometheusMetricsConfig(
        group="service_metrics",
        metrics=["litelm_deployment_failure_responses"],
        include_labels=["litelm_model_name"],
    )

    assert valid_config.group == "service_metrics"
    assert valid_config.metrics == ["litelm_deployment_failure_responses"]
    assert valid_config.include_labels == ["litelm_model_name"]
...
def test_basic_functionality():
    ...
    assert litelm.prometheus_metrics_config is not None
    assert len(litelm.prometheus_metrics_config) == 1
    assert litelm.prometheus_metrics_config[0]["group"] == "service_metrics"
    assert (
        "litelm_deployment_failure_responses"
        in litelm.prometheus_metrics_config[0]["metrics"]
    )
    ...
    test_config = [
        {
            ...
            "metrics": [
                ...
                "litelm_deployment_total_requests",  # valid metric
            ],
            "include_labels": ["litelm_model_name"],
        }
    ]
...
def test_invalid_labels_validation():
    ...
    test_config = [
        {
            "group": "service_metrics",
            "metrics": ["litelm_deployment_total_requests"],
            "include_labels": [
                "litelm_model_name",  # valid label
...
def test_valid_configuration_passes_validation():
    ...
    try:
        ...
        assert "litelm_deployment_total_requests" in logger.enabled_metrics
        assert "litelm_deployment_failure_responses" in logger.enabled_metrics
...
def mock_prometheus_logger():
    ...
    with patch("litelm.proxy.proxy_server.premium_user", True):
        ...
        logger.litelm_proxy_total_requests_metric = MockCounter(
            "litelm_proxy_total_requests_metric"
        )
        logger.litelm_tokens_metric = MockCounter("litelm_total_tokens")
        logger.litelm_input_tokens_metric = MockCounter("litelm_input_tokens")
        logger.litelm_output_tokens_metric = MockCounter("litelm_output_tokens")
        logger.litelm_spend_metric = MockCounter("litelm_spend_metric")
        logger.litelm_requests_metric = MockCounter("litelm_requests_metric")
...
async def test_request_counter_semantic_validation(mock_prometheus_logger):
    ...
    This test specifically catches the bug where litelm_proxy_total_requests_metric
    ...
    total_requests_metric = mock_prometheus_logger.litelm_proxy_total_requests_metric
    assert (
    ...
    tokens_metric = mock_prometheus_logger.litelm_tokens_metric
    assert (
...
async def test_multiple_requests_counter_semantics(mock_prometheus_logger):
    ...
    total_request_increments = sum(
        mock_prometheus_logger.litelm_proxy_total_requests_metric.inc_calls
    )
    total_token_increments = sum(mock_prometheus_logger.litelm_tokens_metric.inc_calls)
...
async def test_streaming_request_counter_semantics(mock_prometheus_logger):
    ...
    for (
        inc_value
    ) in mock_prometheus_logger.litelm_proxy_total_requests_metric.inc_calls:
        assert (
            inc_value == 1
...
async def test_spend_counter_semantics(mock_prometheus_logger):
    ...
    spend_metric = mock_prometheus_logger.litelm_spend_metric
...
def test_callback_failure_metric_increments(prometheus_logger):
    ...
    This tests the litelm_callback_logging_failures_metric counter.
    """
    # Get initial value
    initial_value = 0
    try:
        initial_value = (
            prometheus_logger.litelm_callback_logging_failures_metric.labels(
    ...
    current_value = prometheus_logger.litelm_callback_logging_failures_metric.labels(
        callback_name="S3Logger"
    )._value.get()
    ...
    langfuse_value = prometheus_logger.litelm_callback_logging_failures_metric.labels(
    ...
    s3_value = prometheus_logger.litelm_callback_logging_failures_metric.labels(
        callback_name="S3Logger"
    )._value.get()
...
def test_callback_failure_metric_different_callbacks(prometheus_logger):
    ...
    for callback_name in callbacks_to_test:
        ...
        initial = 0
        try:
            initial = prometheus_logger.litelm_callback_logging_failures_metric.labels(
        ...
        # Verify incremented
        current = prometheus_logger.litelm_callback_logging_failures_metric.labels(
            callback_name=callback_name
        )._value.get()
...
async def test_langfuse_callback_failure_metric(prometheus_logger):
    ...
    litelm_callback_logging_failures_metric is incremented with callback_name="langfuse".
    """
    ...
    initial_value = 0
    try:
        initial_value = prometheus_logger.litelm_callback_logging_failures_metric.labels(
    ...
    with patch("litelm.integrations.langfuse.langfuse_prompt_management.langfuse_client_init"):
        langfuse_logger = LangfusePromptManagement()
    
    # Mock the log_event_on_langfuse to raise an exception
    with patch(
        "litelm.integrations.langfuse.langfuse_prompt_management.LangFuseHandler.get_langfuse_logger_for_request"
    ) as mock_get_logger:
...
async def test_langfuse_otel_callback_failure_metric(prometheus_logger):
    ...
    litelm_callback_logging_failures_metric is incremented with callback_name="langfuse_otel".
    """
    from unittest.mock import MagicMock, patch
    ...
    with patch("litelm.integrations.opentelemetry.OpenTelemetry.__init__", return_value=None):
