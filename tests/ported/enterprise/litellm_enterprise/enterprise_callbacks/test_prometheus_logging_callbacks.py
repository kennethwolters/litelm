...
try:
    from litelm.integrations.prometheus import (
        PrometheusLogger,
...
async def test_async_log_success_event(prometheus_logger):
    ...
    prometheus_logger.litelm_requests_metric = MagicMock()
    prometheus_logger.litelm_spend_metric = MagicMock()

    # Token Metrics
    prometheus_logger.litelm_tokens_metric = MagicMock()
    prometheus_logger.litelm_input_tokens_metric = MagicMock()
    prometheus_logger.litelm_output_tokens_metric = MagicMock()

    # Remaining Budget Metrics
    prometheus_logger.litelm_remaining_team_budget_metric = MagicMock()
    prometheus_logger.litelm_remaining_api_key_budget_metric = MagicMock()

    # Virtual Key Rate limit Metrics
    prometheus_logger.litelm_remaining_api_key_requests_for_model = MagicMock()
    prometheus_logger.litelm_remaining_api_key_tokens_for_model = MagicMock()

    # Latency Metrics
    prometheus_logger.litelm_llm_api_time_to_first_token_metric = MagicMock()
    prometheus_logger.litelm_llm_api_latency_metric = MagicMock()
    prometheus_logger.litelm_request_total_latency_metric = MagicMock()
    ...
    prometheus_logger.litelm_requests_metric.labels.assert_called()
    prometheus_logger.litelm_spend_metric.labels.assert_called()

    # Token Metrics
    prometheus_logger.litelm_tokens_metric.labels.assert_called()
    prometheus_logger.litelm_input_tokens_metric.labels.assert_called()
    prometheus_logger.litelm_output_tokens_metric.labels.assert_called()

    # Remaining Budget Metrics
    prometheus_logger.litelm_remaining_team_budget_metric.labels.assert_called()
    prometheus_logger.litelm_remaining_api_key_budget_metric.labels.assert_called()

    # Virtual Key Rate limit Metrics
    prometheus_logger.litelm_remaining_api_key_requests_for_model.labels.assert_called()
    prometheus_logger.litelm_remaining_api_key_tokens_for_model.labels.assert_called()

    # Latency Metrics
    prometheus_logger.litelm_llm_api_time_to_first_token_metric.labels.assert_called()
    prometheus_logger.litelm_llm_api_latency_metric.labels.assert_called()
    prometheus_logger.litelm_request_total_latency_metric.labels.assert_called()
...
def test_increment_token_metrics(prometheus_logger):
    ...
    enum_values = UserAPIKeyLabelValues(
        litelm_model_name=standard_logging_payload["model"],
    ...
    prometheus_logger.litelm_tokens_metric.labels.assert_called_once_with(
        end_user=None,
        user=None,
        user_email=None,
    ...
    prometheus_logger.litelm_tokens_metric.labels().inc.assert_called_once_with(100)

    prometheus_logger.litelm_input_tokens_metric.labels.assert_called_once_with(
        end_user=None,
        user=None,
        user_email=None,
        hashed_api_key="test_hash",
    ...
    prometheus_logger.litelm_input_tokens_metric.labels().inc.assert_called_once_with(
        50
    )

    prometheus_logger.litelm_output_tokens_metric.labels.assert_called_once_with(
        end_user=None,
        user=None,
        user_email=None,
        hashed_api_key="test_hash",
    ...
    prometheus_logger.litelm_output_tokens_metric.labels().inc.assert_called_once_with(
        50
    )
...
async def test_increment_remaining_budget_metrics(prometheus_logger):
    ...
    prometheus_logger.litelm_remaining_team_budget_metric = MagicMock()
    prometheus_logger.litelm_remaining_api_key_budget_metric = MagicMock()
    prometheus_logger.litelm_team_max_budget_metric = MagicMock()
    prometheus_logger.litelm_api_key_max_budget_metric = MagicMock()
    prometheus_logger.litelm_team_budget_remaining_hours_metric = MagicMock()
    prometheus_logger.litelm_api_key_budget_remaining_hours_metric = MagicMock()
    ...
    with patch(
        "litelm.proxy.auth.auth_checks.get_team_object"
    ) as mock_get_team, patch(
        "litelm.proxy.auth.auth_checks.get_key_object"
    ) as mock_get_key:
        ...
        await prometheus_logger._increment_remaining_budget_metrics(
            ...
            user_api_key="key1",
            user_api_key_alias="alias1",
            litelm_params=litelm_params,
            response_cost=10,
        )

        # Test remaining budget metrics
        prometheus_logger.litelm_remaining_team_budget_metric.labels.assert_called_once_with(
            team="team1", team_alias="team_alias1"
        )
        prometheus_logger.litelm_remaining_team_budget_metric.labels().set.assert_called_once_with(
            40  # 100 - (50 + 10)
        )

        prometheus_logger.litelm_remaining_api_key_budget_metric.labels.assert_called_once_with(
            hashed_api_key="key1", api_key_alias="alias1"
        )
        prometheus_logger.litelm_remaining_api_key_budget_metric.labels().set.assert_called_once_with(
            40  # 75 - (25 + 10)
        )

        # Test max budget metrics
        prometheus_logger.litelm_team_max_budget_metric.labels.assert_called_once_with(
            team="team1", team_alias="team_alias1"
        )
        prometheus_logger.litelm_team_max_budget_metric.labels().set.assert_called_once_with(
            100
        )

        prometheus_logger.litelm_api_key_max_budget_metric.labels.assert_called_once_with(
            hashed_api_key="key1", api_key_alias="alias1"
        )
        prometheus_logger.litelm_api_key_max_budget_metric.labels().set.assert_called_once_with(
            75
        )

        # Test remaining hours metrics
        prometheus_logger.litelm_team_budget_remaining_hours_metric.labels.assert_called_once_with(
            team="team1", team_alias="team_alias1"
        )
        # The remaining hours should be approximately 10 (with some small difference due to test execution time)
        remaining_hours_call = prometheus_logger.litelm_team_budget_remaining_hours_metric.labels().set.call_args[
            0
        ][
            0
        ]
        ...
        prometheus_logger.litelm_api_key_budget_remaining_hours_metric.labels.assert_called_once_with(
            hashed_api_key="key1", api_key_alias="alias1"
        )
        # The remaining hours should be approximately 10 (with some small difference due to test execution time)
        remaining_hours_call = prometheus_logger.litelm_api_key_budget_remaining_hours_metric.labels().set.call_args[
            0
        ][
            0
        ]
...
def test_set_latency_metrics(prometheus_logger):
    ...
    prometheus_logger.litelm_llm_api_time_to_first_token_metric.labels.assert_called_once_with(
        end_user=None,
        user="test_user",
        hashed_api_key="test_hash",
        api_key_alias="test_alias",
    ...
    prometheus_logger.litelm_llm_api_time_to_first_token_metric.labels().observe.assert_called_once_with(
        0.5
    )

    # end_time - api_call_start_time
    prometheus_logger.litelm_llm_api_latency_metric.labels.assert_called_once_with(
        end_user=None,
        user="test_user",
        hashed_api_key="test_hash",
        api_key_alias="test_alias",
    ...
    prometheus_logger.litelm_llm_api_latency_metric.labels().observe.assert_called_once_with(
        1.5
    )

    # total latency for the request
    prometheus_logger.litelm_request_total_latency_metric.labels.assert_called_once_with(
        end_user=None,
        user="test_user",
        hashed_api_key="test_hash",
        api_key_alias="test_alias",
    ...
    prometheus_logger.litelm_request_total_latency_metric.labels().observe.assert_called_once_with(
        2.0
    )
...
def test_set_latency_metrics_missing_timestamps(prometheus_logger):
    ...
    prometheus_logger.litelm_llm_api_time_to_first_token_metric.labels.assert_not_called()

    # Other metrics should still be called
    prometheus_logger.litelm_llm_api_latency_metric.labels.assert_called_once()
    prometheus_logger.litelm_request_total_latency_metric.labels.assert_called_once()
...
def test_set_latency_metrics_missing_api_call_start(prometheus_logger):
    ...
    prometheus_logger.litelm_llm_api_time_to_first_token_metric.labels.assert_not_called()
    prometheus_logger.litelm_llm_api_latency_metric.labels.assert_not_called()
...
def test_increment_top_level_request_and_spend_metrics(prometheus_logger):
    ...
    - litelm_requests_metric is incremented by 1
    - litelm_spend_metric is incremented by the response cost in the standard logging payload
    """
    standard_logging_payload = create_standard_logging_payload()
    enum_values = UserAPIKeyLabelValues(
        litelm_model_name=standard_logging_payload["model"],
    ...
    prometheus_logger.litelm_requests_metric.labels.assert_called_once_with(
        end_user=None,
        user=None,
        user_email=None,
        hashed_api_key="test_hash",
    ...
    prometheus_logger.litelm_requests_metric.labels().inc.assert_called_once()

    # The spend metric uses keyword arguments (same as requests metric)
    prometheus_logger.litelm_spend_metric.labels.assert_called_once_with(
        end_user=None,
        user=None,
        user_email=None,
        hashed_api_key="test_hash",
    ...
    prometheus_logger.litelm_spend_metric.labels().inc.assert_called_once_with(0.1)
...
async def test_async_log_failure_event(prometheus_logger):
    ...
    # Mock the metrics
    prometheus_logger.litelm_llm_api_failed_requests_metric = MagicMock()
    prometheus_logger.litelm_deployment_failure_responses = MagicMock()
    prometheus_logger.litelm_deployment_total_requests = MagicMock()
    ...
    # litelm_llm_api_failed_requests_metric incremented
    # Labels: end_user, api_key_hash, api_key_alias, model, team, team_alias, user, model_id
    prometheus_logger.litelm_llm_api_failed_requests_metric.labels.assert_called_once_with(
        None,  # end_user_id
        "test_hash",
        "test_alias",
        "gpt-3.5-turbo",
    ...
    prometheus_logger.litelm_llm_api_failed_requests_metric.labels().inc.assert_called_once()

    # deployment should be marked in partial outage
    prometheus_logger.set_deployment_partial_outage.assert_called_once_with(
        litelm_model_name="gpt-3.5-turbo",
        model_id="model-123",
        api_base="https://api.openai.com",
        api_provider="openai",
    )
    ...
    prometheus_logger.litelm_deployment_failure_responses.labels.assert_called_once()
    actual_failure_labels = (
        prometheus_logger.litelm_deployment_failure_responses.labels.call_args.kwargs
    )
    ...
    prometheus_logger.litelm_deployment_failure_responses.labels().inc.assert_called_once()

    # deployment total requests incremented - verify key labels are populated
    prometheus_logger.litelm_deployment_total_requests.labels.assert_called_once()
    actual_total_labels = (
        prometheus_logger.litelm_deployment_total_requests.labels.call_args.kwargs
    )
    ...
    prometheus_logger.litelm_deployment_total_requests.labels().inc.assert_called_once()
...
async def test_async_post_call_failure_hook(prometheus_logger):
    ...
    it should increment the litelm_proxy_failed_requests_metric and litelm_proxy_total_requests_metric
    """
    # Mock the prometheus metrics
    prometheus_logger.litelm_proxy_failed_requests_metric = MagicMock()
    prometheus_logger.litelm_proxy_total_requests_metric = MagicMock()

    # Create test data
    request_data = {"model": "gpt-3.5-turbo"}

    original_exception = litelm.RateLimitError(
    ...
    prometheus_logger.litelm_proxy_failed_requests_metric.labels.assert_called_once_with(
        end_user=None,
        user="test_user",
        user_email=None,
    ...
    prometheus_logger.litelm_proxy_failed_requests_metric.labels().inc.assert_called_once()

    # Assert total requests metric was incremented with correct labels
    prometheus_logger.litelm_proxy_total_requests_metric.labels.assert_called_once_with(
        end_user=None,
        hashed_api_key="test_key",
        api_key_alias="test_alias",
        requested_model="gpt-3.5-turbo",
    ...
    prometheus_logger.litelm_proxy_total_requests_metric.labels().inc.assert_called_once()
...
async def test_async_post_call_success_hook(prometheus_logger):
    ...
    litelm_proxy_total_requests_metric is NOT incremented here to avoid double-counting.
    ...
    prometheus_logger.litelm_proxy_total_requests_metric.labels.assert_not_called()


def test_set_llm_deployment_success_metrics(prometheus_logger):
    # Mock all the metrics used in the method
    prometheus_logger.litelm_remaining_requests_metric = MagicMock()
    prometheus_logger.litelm_remaining_tokens_metric = MagicMock()
    prometheus_logger.litelm_deployment_success_responses = MagicMock()
    prometheus_logger.litelm_deployment_total_requests = MagicMock()
    prometheus_logger.litelm_deployment_latency_per_output_token = MagicMock()
    prometheus_logger.set_deployment_healthy = MagicMock()
    prometheus_logger.litelm_overhead_latency_metric = MagicMock()
    ...
    standard_logging_payload["hidden_params"]["litelm_overhead_time_ms"] = 100
    ...
    prometheus_logger.litelm_remaining_requests_metric.labels.assert_called_once_with(
        ...
        litelm_model_name="gpt-3.5-turbo",  # actual model used - litelm model name
        hashed_api_key=standard_logging_payload["metadata"]["user_api_key_hash"],
        api_key_alias=standard_logging_payload["metadata"]["user_api_key_alias"],
        model_id="model-123",
    )

    prometheus_logger.litelm_remaining_requests_metric.labels().set.assert_called_once_with(
        123
    )

    # Verify remaining tokens metric
    prometheus_logger.litelm_remaining_tokens_metric.labels.assert_called_once_with(
        api_base="https://api.openai.com",
        api_key_alias=standard_logging_payload["metadata"]["user_api_key_alias"],
        api_provider="openai",
        hashed_api_key=standard_logging_payload["metadata"]["user_api_key_hash"],
    ...
    prometheus_logger.litelm_remaining_tokens_metric.labels().set.assert_called_once_with(
        4321
    )
    ...
    prometheus_logger.litelm_deployment_success_responses.labels.assert_called_once_with(
        litelm_model_name="gpt-3.5-turbo",
        model_id="model-123",
        api_base="https://api.openai.com",
        api_provider="openai",
    ...
    prometheus_logger.litelm_deployment_success_responses.labels().inc.assert_called_once()

    # Verify total requests metric
    prometheus_logger.litelm_deployment_total_requests.labels.assert_called_once_with(
        litelm_model_name="gpt-3.5-turbo",
        model_id="model-123",
        api_base="https://api.openai.com",
        api_provider="openai",
    ...
    prometheus_logger.litelm_deployment_latency_per_output_token.labels.assert_called_once_with(
        litelm_model_name="gpt-3.5-turbo",
        model_id="model-123",
        api_base="https://api.openai.com",
        api_provider="openai",
    ...
    prometheus_logger.litelm_overhead_latency_metric.labels.assert_called_once_with(
        api_base="https://api.openai.com",
        api_key_alias=standard_logging_payload["metadata"]["user_api_key_alias"],
        api_provider="openai",
        hashed_api_key=standard_logging_payload["metadata"]["user_api_key_hash"],
    ...
    prometheus_logger.litelm_deployment_latency_per_output_token.labels().observe.assert_called_once_with(
...
async def test_log_success_fallback_event(prometheus_logger):
    prometheus_logger.litelm_deployment_successful_fallbacks = MagicMock()
    ...
    prometheus_logger.litelm_deployment_successful_fallbacks.labels.assert_called_once_with(
    ...
    prometheus_logger.litelm_deployment_successful_fallbacks.labels().inc.assert_called_once()


@pytest.mark.asyncio
async def test_log_failure_fallback_event(prometheus_logger):
    prometheus_logger.litelm_deployment_failed_fallbacks = MagicMock()
    ...
    prometheus_logger.litelm_deployment_failed_fallbacks.labels.assert_called_once_with(
        requested_model=original_model_group,
        fallback_model="gpt-4",
        hashed_api_key="test_hash",
        api_key_alias="test_alias",
    ...
    prometheus_logger.litelm_deployment_failed_fallbacks.labels().inc.assert_called_once()


def test_deployment_state_management(prometheus_logger):
    prometheus_logger.litelm_deployment_state = MagicMock()
    ...
    prometheus_logger.litelm_deployment_state.labels.assert_called_with(
        litelm_model_name=test_params["litelm_model_name"],
        model_id=test_params["model_id"],
        api_base=test_params["api_base"],
        api_provider=test_params["api_provider"],
    )
    prometheus_logger.litelm_deployment_state.labels().set.assert_called_with(0)

    # Test set_deployment_partial_outage (state=1)
    prometheus_logger.set_deployment_partial_outage(**test_params)
    prometheus_logger.litelm_deployment_state.labels().set.assert_called_with(1)

    # Test set_deployment_complete_outage (state=2)
    prometheus_logger.set_deployment_complete_outage(**test_params)
    prometheus_logger.litelm_deployment_state.labels().set.assert_called_with(2)
...
def test_increment_deployment_cooled_down(prometheus_logger):
    ...
    prometheus_logger.litelm_deployment_cooled_down = MagicMock()
    prometheus_logger.litelm_deployment_cooled_down.labels = MagicMock(
    ...
    prometheus_logger.litelm_deployment_cooled_down.labels.assert_called_once_with(
...
def test_prometheus_factory(monkeypatch, enable_end_user_cost_tracking_prometheus_only):
    from litelm.integrations.prometheus import prometheus_label_factory
    from litelm.types.integrations.prometheus import UserAPIKeyLabelValues

    monkeypatch.setattr(
        "litelm.enable_end_user_cost_tracking_prometheus_only",
...
def test_get_custom_labels_from_metadata(monkeypatch):
    monkeypatch.setattr(
        "litelm.custom_prometheus_metadata_labels", ["metadata.foo", "metadata.bar"]
    )
...
def test_get_custom_labels_from_metadata_tags(monkeypatch):
    monkeypatch.setattr("litelm.custom_prometheus_metadata_labels", [])
    metadata = {"foo": "bar", "bar": "baz", "taz": "qux"}
    assert get_custom_labels_from_metadata(metadata) == {}
...
def test_get_custom_labels_from_top_level_metadata(monkeypatch):
    ...
    """
    monkeypatch.setattr(
        "litelm.custom_prometheus_metadata_labels",
...
async def test_async_log_success_event_with_top_level_metadata(
    ...
    monkeypatch.setattr(
        "litelm.custom_prometheus_metadata_labels", ["requester_ip_address"]
    )
    ...
    prometheus_logger.litelm_requests_metric = create_mock_metric()
    prometheus_logger.litelm_spend_metric = create_mock_metric()
    prometheus_logger.litelm_tokens_metric = create_mock_metric()
    prometheus_logger.litelm_input_tokens_metric = create_mock_metric()
    prometheus_logger.litelm_output_tokens_metric = create_mock_metric()
    prometheus_logger.litelm_remaining_team_budget_metric = create_mock_metric()
    prometheus_logger.litelm_remaining_api_key_budget_metric = create_mock_metric()
    prometheus_logger.litelm_remaining_user_budget_metric = create_mock_metric()
    prometheus_logger.litelm_user_max_budget_metric = create_mock_metric()
    prometheus_logger.litelm_user_budget_remaining_hours_metric = create_mock_metric()
    prometheus_logger.litelm_remaining_api_key_requests_for_model = (
        create_mock_metric()
    )
    prometheus_logger.litelm_remaining_api_key_tokens_for_model = create_mock_metric()
    prometheus_logger.litelm_llm_api_time_to_first_token_metric = create_mock_metric()
    prometheus_logger.litelm_llm_api_latency_metric = create_mock_metric()
    prometheus_logger.litelm_request_total_latency_metric = create_mock_metric()
    # Cache metrics
    prometheus_logger.litelm_cache_hits_metric = create_mock_metric()
    prometheus_logger.litelm_cache_misses_metric = create_mock_metric()
    prometheus_logger.litelm_cached_tokens_metric = create_mock_metric()
    # Deployment metrics
    prometheus_logger.litelm_deployment_state = create_mock_metric()
    prometheus_logger.litelm_deployment_success_responses = create_mock_metric()
    prometheus_logger.litelm_deployment_total_requests = create_mock_metric()
    prometheus_logger.litelm_deployment_latency_per_output_token = create_mock_metric()
    prometheus_logger.litelm_remaining_requests_metric = create_mock_metric()
    prometheus_logger.litelm_remaining_tokens_metric = create_mock_metric()
    prometheus_logger.litelm_overhead_latency_metric = create_mock_metric()
    prometheus_logger.litelm_proxy_total_requests_metric = create_mock_metric()
    ...
    assert prometheus_logger.litelm_requests_metric.labels.called
    assert prometheus_logger.litelm_spend_metric.labels.called
    ...
    call_args = prometheus_logger.litelm_requests_metric.labels.call_args
...
def test_get_custom_labels_from_tags(monkeypatch):
    from litelm.integrations.prometheus import get_custom_labels_from_tags

    monkeypatch.setattr(
        "litelm.custom_prometheus_tags", ["prod", "test-env", "batch.job"]
    )
...
def test_get_custom_labels_from_tags_empty_config(monkeypatch):
    ...
    monkeypatch.setattr("litelm.custom_prometheus_tags", [])
    tags = ["prod", "debug"]
    result = get_custom_labels_from_tags(tags)
    assert result == {}
...
def test_get_custom_labels_from_tags_no_tags(monkeypatch):
    ...
    monkeypatch.setattr("litelm.custom_prometheus_tags", ["prod", "test"])
    tags = []
    result = get_custom_labels_from_tags(tags)
    assert result == {
        "tag_prod": "false",
...
def test_tag_matches_wildcard_configured_pattern():
    ...
    from litelm.integrations.prometheus import _tag_matches_wildcard_configured_pattern
...
async def test_initialize_remaining_budget_metrics(prometheus_logger):
    ...
    """
    litelm.prometheus_initialize_budget_metrics = True
    # Mock the prisma client and get_paginated_teams function
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        "litelm.proxy.management_endpoints.team_endpoints.get_paginated_teams"
    ) as mock_get_teams:
        ...
        prometheus_logger.litelm_remaining_team_budget_metric.assert_has_calls(
            expected_budget_calls, any_order=True
        )

        # Get all the calls made to the hours metric
        hours_calls = (
            prometheus_logger.litelm_team_budget_remaining_hours_metric.mock_calls
        )
        ...
        prometheus_logger.litelm_team_budget_remaining_hours_metric.assert_has_calls(
...
async def test_initialize_remaining_budget_metrics_exception_handling(
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        ...
        "litelm.proxy.management_endpoints.key_management_endpoints._list_key_helper"
    ) as mock_list_keys:
        ...
        # The code accesses prisma_client.db.litelm_usertable.find_many and count
        ...
        # Mock litelm_teamtable to raise an exception for team count metrics
        mock_teamtable = MagicMock()
        mock_teamtable.count = MagicMock(side_effect=Exception("Team count error"))

        mock_db = MagicMock()
        mock_db.litelm_usertable = mock_usertable
        mock_db.litelm_teamtable = mock_teamtable
        mock_prisma.db = mock_db

        # Mock the Prometheus metrics
        prometheus_logger.litelm_remaining_team_budget_metric = MagicMock()
        prometheus_logger.litelm_remaining_api_key_budget_metric = MagicMock()
        prometheus_logger.litelm_remaining_user_budget_metric = MagicMock()
        prometheus_logger.litelm_total_users_metric = MagicMock()
        prometheus_logger.litelm_teams_count_metric = MagicMock()

        # Mock the logger to capture the error
        with patch("litelm._logging.verbose_logger.exception") as mock_logger:
            # Call the function
            await prometheus_logger._initialize_remaining_budget_metrics()
        ...
        prometheus_logger.litelm_remaining_team_budget_metric.assert_not_called()
        prometheus_logger.litelm_remaining_api_key_budget_metric.assert_not_called()
        prometheus_logger.litelm_remaining_user_budget_metric.assert_not_called()
        prometheus_logger.litelm_total_users_metric.assert_not_called()
        prometheus_logger.litelm_teams_count_metric.assert_not_called()
...
async def test_initialize_api_key_budget_metrics(prometheus_logger):
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        ...
        prometheus_logger.litelm_remaining_api_key_budget_metric.assert_has_calls(
            expected_budget_calls, any_order=True
        )

        # Get all the calls made to the hours metric
        hours_calls = (
            prometheus_logger.litelm_api_key_budget_remaining_hours_metric.mock_calls
        )
        ...
        prometheus_logger.litelm_api_key_max_budget_metric.assert_has_calls(
...
def test_set_team_budget_metrics_multiple_teams(prometheus_logger):
    ...
    prometheus_logger.litelm_team_max_budget_metric.assert_has_calls(
        expected_max_budget_calls, any_order=True
    )
    ...
    assert (
        prometheus_logger.litelm_team_budget_remaining_hours_metric.labels.call_count
        == 3
    )
    assert (
        prometheus_logger.litelm_team_budget_remaining_hours_metric.labels().set.call_count
        == 3
    )
...
def test_set_team_budget_metrics_null_values(prometheus_logger):
    ...
    prometheus_logger.litelm_team_max_budget_metric.assert_not_called()

    # Verify reset metric is not set when budget_reset_at is None
    prometheus_logger.litelm_team_budget_remaining_hours_metric.assert_not_called()
...
def test_set_team_budget_metrics_with_custom_labels(prometheus_logger, monkeypatch):
    ...
    monkeypatch.setattr("litelm.custom_prometheus_metadata_labels", custom_labels)
...
def test_prometheus_label_factory_with_custom_tags(monkeypatch):
    ...
    monkeypatch.setattr("litelm.custom_prometheus_tags", ["prod", "test-env"])
...
def test_get_exception_class_name(prometheus_logger):
    ...
    rate_limit_error = litelm.RateLimitError(
    ...
    auth_error = litelm.AuthenticationError(
    ...
    context_window_error = litelm.ContextWindowExceededError(
...
def test_set_llm_deployment_success_metrics_with_label_filtering():
    ...
    """
    from litelm.types.integrations.prometheus import PrometheusMetricsConfig
    ...
    config = [
        PrometheusMetricsConfig(
            group="test_group",
            metrics=[
                "litelm_overhead_latency_metric",
                "litelm_remaining_requests_metric",
                "litelm_remaining_tokens_metric",
                "litelm_deployment_success_responses",
                "litelm_deployment_total_requests",
            ],
            include_labels=[
                "litelm_model_name",
                "api_provider",
    ...
    # Mock litelm.prometheus_metrics_config
    with patch("litelm.prometheus_metrics_config", config):
        ...
        prometheus_logger.litelm_overhead_latency_metric.labels.assert_called_once()
        overhead_labels = (
            prometheus_logger.litelm_overhead_latency_metric.labels.call_args[1]
        )
        ...
        prometheus_logger.litelm_remaining_requests_metric.labels.assert_called_once()
        requests_labels = (
            prometheus_logger.litelm_remaining_requests_metric.labels.call_args[1]
        )
        ...
        prometheus_logger.litelm_remaining_tokens_metric.labels.assert_called_once()
        tokens_labels = (
            prometheus_logger.litelm_remaining_tokens_metric.labels.call_args[1]
        )
        ...
        prometheus_logger.litelm_deployment_success_responses.labels.assert_called_once()
        success_labels = (
            prometheus_logger.litelm_deployment_success_responses.labels.call_args[1]
        )
        ...
        prometheus_logger.litelm_deployment_total_requests.labels.assert_called_once()
        total_labels = (
            prometheus_logger.litelm_deployment_total_requests.labels.call_args[1]
        )
        ...
        prometheus_logger.litelm_overhead_latency_metric.labels().observe.assert_called_once()
        prometheus_logger.litelm_remaining_requests_metric.labels().set.assert_called_once_with(
            123
        )
        prometheus_logger.litelm_remaining_tokens_metric.labels().set.assert_called_once_with(
...
async def test_prometheus_token_metrics_with_prometheus_config():
    ...
    - litelm_total_tokens -> litelm_total_tokens_metric
    - litelm_input_tokens -> litelm_input_tokens_metric
    - litelm_output_tokens -> litelm_output_tokens_metric
    ...
    with patch("litelm.prometheus_metrics_config", config):
        ...
        # Expected values
        expected_values = {
            "litelm_total_tokens_metric": 1500.0,
            "litelm_input_tokens_metric": 900.0,
            "litelm_output_tokens_metric": 600.0,
            "litelm_requests_metric": 1.0,
        }
