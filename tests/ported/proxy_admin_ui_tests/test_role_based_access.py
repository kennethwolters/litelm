...
from litelm.proxy.auth.auth_checks import get_user_object
...
async def test_create_new_user_in_organization(prisma_client, user_role):
    ...
    setattr(litelm.proxy.proxy_server, "llm_router", MagicMock())
...
async def test_user_role_permissions(prisma_client, route, user_role, expected_result):
    ...
    try:
        ...
        # Generate key for new user with team_id="litelm-dashboard"
        key_response = await generate_key_fn(
            data=GenerateKeyRequest(user_id=user_id, team_id="litelm-dashboard"),
