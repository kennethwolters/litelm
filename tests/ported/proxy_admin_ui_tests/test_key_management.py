...
from litelm.types.proxy.management_endpoints.ui_sso import LiteLLM_UpperboundKeyGenerateParams
...
async def test_get_users(prisma_client):
    ...
    # Clean up test users
    for user in test_users:
        await prisma_client.db.litelm_usertable.delete(where={"user_id": user.user_id})
...
async def test_get_users_filters_dashboard_keys(prisma_client):
    """
    Tests that /users/list endpoint doesn't return keys with team_id='litelm-dashboard'
    ...
    # Create two keys for the user - one with team_id="litelm-dashboard" and one without
    regular_key = await generate_key_helper_fn(
        user_id=test_user.user_id,
        request_type="key",
        team_id="litelm-dashboard",  # This key should be included in the response
        models=[],
        aliases={},
        config={},
        spend=0,
    ...
    await prisma_client.db.litelm_usertable.delete(
...
async def cleanup_existing_teams(prisma_client):
    all_teams = await prisma_client.db.litelm_teamtable.find_many()
...
def test_is_team_key():
    from litelm.proxy.management_endpoints.key_management_endpoints import _is_team_key
...
def test_team_key_generation_team_member_check():
    ...
    litelm.key_generation_settings = {
...
def test_key_generation_required_params_check(
    ...
    if key_type == "team_key":
        litelm.key_generation_settings = StandardKeyGenerationConfig(
...
async def test_list_key_helper_team_filtering(prisma_client):
    ...
    1. Create keys with different team_ids (None, litelm-dashboard, other)
    2. Verify filtering excludes litelm-dashboard keys
    ...
    # Create 2 keys with team_id=litelm-dashboard
    for i in range(2):
        key = await generate_key_fn(
            data=GenerateKeyRequest(
    ...
    try:
        # Test 1: Get all keys with pagination (exclude litelm-dashboard)
        all_keys = []
        page = 1
        ...
        # Verify no litelm-dashboard keys are present
        dashboard_keys = [k for k in all_keys if k.team_id == "litelm-dashboard"]
        assert len(dashboard_keys) == 0, "Should not include litelm-dashboard keys"
...
@pytest.mark.asyncio
@patch("litelm.proxy.management_endpoints.key_management_endpoints.get_team_object")
async def test_key_generate_always_db_team(mock_get_team_object):
    from litelm.proxy.management_endpoints.key_management_endpoints import (
        generate_key_fn,
    )

    setattr(litelm.proxy.proxy_server, "prisma_client", MagicMock())
