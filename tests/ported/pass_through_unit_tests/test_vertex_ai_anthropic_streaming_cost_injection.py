...
async def test_vertex_ai_anthropic_streaming_cost_injection_enabled():
    ...
    original_value = getattr(litelm, "include_cost_in_streaming_usage", False)
    ...
    try:
        ...
        litelm_logging_obj = MagicMock(spec=LiteLLMLoggingObj)
        litelm_logging_obj.model_call_details = {"model": "claude-sonnet-4@20250514"}
        litelm_logging_obj.async_success_handler = AsyncMock()
        ...
        with patch("litelm.completion_cost", return_value=0.00015):
            received_chunks = []
        ...
        litelm.include_cost_in_streaming_usage = original_value
...
async def test_vertex_ai_anthropic_streaming_model_extraction():
    ...
    try:
        ...
        litelm_logging_obj = MagicMock(spec=LiteLLMLoggingObj)
        litelm_logging_obj.model_call_details = {}
        litelm_logging_obj.async_success_handler = AsyncMock()
        ...
        with patch("litelm.completion_cost") as mock_cost:
