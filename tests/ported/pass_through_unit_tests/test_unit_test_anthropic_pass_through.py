...
async def test_anthropic_passthrough_handler(
    ...
    """
    Unit test - Assert that the anthropic passthrough handler calls the litelm logging object's async_success_handler
    """
    start_time = datetime.now()
    ...
    assert isinstance(result["result"], litelm.ModelResponse)


@pytest.mark.parametrize(
    "metadata_params",
    [{"metadata": {"user_id": "test"}}, {"litelm_metadata": {"user": "test"}}, {}],
)
def test_create_anthropic_response_logging_payload(mock_logging_obj, metadata_params):
    ...
    result = AnthropicPassthroughLoggingHandler._create_anthropic_response_logging_payload(
        litelm_model_response=model_response,
        model="claude-3-opus-20240229",
        kwargs={
            "litelm_params": {
                "metadata": {
            ...
            "litelm_call_id": "5cf924cb-161c-4c1d-a565-31aa71ab50ab",
...
@pytest.mark.parametrize(
    "end_user_id",
    [{"litelm_metadata": {"user": "test"}}, {"metadata": {"user_id": "test"}}],
)
