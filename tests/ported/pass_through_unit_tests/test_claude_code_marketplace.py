...
from litelm.proxy.proxy_server import LitellmUserRoles
from litelm.caching.caching import DualCache
from litelm.types.proxy.claude_code_endpoints import RegisterPluginRequest

# Import the functions we're testing
from litelm.proxy.anthropic_endpoints.claude_code_endpoints.claude_code_marketplace import (
    register_plugin,
    get_marketplace,
)
...
def mock_prisma_client():
    ...
    mock_client.db.litelm_claudecodeplugintable = mock_table
...
async def test_register_plugin(mock_prisma_client):
    ...
    setattr(litelm.proxy.proxy_server, "prisma_client", mock_prisma_client)
    ...
    stored_plugin = await mock_prisma_client.db.litelm_claudecodeplugintable.find_unique(
    ...
    await mock_prisma_client.db.litelm_claudecodeplugintable.delete(
        where={"name": plugin_name}
    )
