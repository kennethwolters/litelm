...
import litelm
from litelm.integrations.websearch_interception import (
    WebSearchInterceptionLogger,
)
from litelm.anthropic_interface import messages
...
async def test_websearch_interception_non_streaming():
    ...
    litelm.callbacks = [websearch_logger]
    ...
    try:
        ...
        print("\n📞 Making litelm.messages.acreate() call...")
        ...
        if has_tool_use:
            ...
            if "litelm" in text_content.lower():
                print("\n" + "="*80)
                print("✅ TEST 1 PASSED!")
                print("="*80)
                print("✅ User made ONE litelm.messages.acreate() call")
...
async def test_websearch_interception_streaming():
    ...
    try:
        ...
        print("\n📞 Making litelm.messages.acreate() call with stream=True...")
        print(f"   Model: bedrock/us.anthropic.claude-3-5-sonnet-20241022-v2:0")
        print(f"   Query: 'What is LiteLLM?'")
        print(f"   Tools: WebSearch")
        ...
        if has_tool_use:
            ...
            if "litelm" in text_content.lower():
                ...
                print("✅ TEST 2 PASSED!")
                print("="*80)
                print("✅ User made ONE litelm.messages.acreate() call with stream=True")
...
async def test_claude_code_native_websearch():
    ...
    try:
        ...
        print(f"   Query: 'Perform a web search for the query: litelm what is it'")
        print(f"   Tools: Native web_search_20250305")
        print(f"   Stream: False")

        response = await messages.acreate(
            model="bedrock/us.anthropic.claude-sonnet-4-5-20250929-v1:0",
            messages=[{"role": "user", "content": "Perform a web search for the query: litelm what is it"}],
            tools=[
                {
...
async def test_litelm_standard_websearch_tool():
    """
    PRIORITY TEST #1: Test with the canonical litelm_web_search tool format.

    This validates that using get_litelm_web_search_tool() directly
    ...
    from litelm.integrations.websearch_interception import get_litelm_web_search_tool
    ...
    try:
        ...
        print(f"   Tool: litelm_web_search (standard format, no conversion needed)")
        print(f"   Stream: False")

        response = await messages.acreate(
            model="bedrock/us.anthropic.claude-sonnet-4-5-20250929-v1:0",
            messages=[{"role": "user", "content": "What is the latest news about AI? Give me a brief overview."}],
            tools=[get_litelm_web_search_tool()],
            max_tokens=1024,
            stream=False,
        )
...
async def test_claude_code_native_websearch_streaming():
    ...
    Validates:
    - Native tool conversion (web_search_20250305 → litelm_web_search)
    ...
    try:
        ...
        if has_tool_use:
            ...
            print("✅ TEST PASSED!")
            print("="*80)
            print("✅ Native tool converted to litelm_web_search")
...
def test_is_web_search_tool_detection():
    ...
    from litelm.integrations.websearch_interception import is_web_search_tool

    test_cases = [
        ({"name": "litelm_web_search"}, True, "LiteLLM standard tool"),
...
async def test_pre_request_hook_modifies_request_body():
    ...
    2. Native web_search_20250305 tool is converted to litelm_web_search
    3. Stream is converted from True to False
    4. Modified parameters reach the API call
    """
    import asyncio
    from unittest.mock import AsyncMock, patch, MagicMock
    from litelm.constants import LITELLM_WEB_SEARCH_TOOL_NAME
    ...
    def mock_anthropic_messages_handler(
        ...
        **kwargs
        ...
        from litelm.types.llms.anthropic_messages.anthropic_response import AnthropicMessagesResponse
    ...
    with patch('litelm.llms.anthropic.experimental_pass_through.messages.handler.anthropic_messages_handler',
