...
from litelm.types.passthrough_endpoints.pass_through_endpoints import EndpointType
from litelm.proxy.pass_through_endpoints.success_handler import (
    PassThroughEndpointLogging,
)
from litelm.proxy.pass_through_endpoints.streaming_handler import (
...
def test_init_kwargs_for_pass_through_endpoint_basic(
    ...
    assert result["litelm_call_id"] == "test-call-id"
    assert result["passthrough_logging_payload"] == passthrough_payload

    #########################################################
    # Check metadata
    #########################################################
    assert result["litelm_params"]["metadata"]["user_api_key"] == "test-key"
    assert result["litelm_params"]["metadata"]["user_api_key_hash"] == "test-key"
    assert result["litelm_params"]["metadata"]["user_api_key_alias"] is None
    assert result["litelm_params"]["metadata"]["user_api_key_user_email"] is None
    assert result["litelm_params"]["metadata"]["user_api_key_user_id"] == "test-user"
    assert result["litelm_params"]["metadata"]["user_api_key_team_id"] == "test-team"
    assert result["litelm_params"]["metadata"]["user_api_key_org_id"] is None
    assert result["litelm_params"]["metadata"]["user_api_key_team_alias"] is None
    assert result["litelm_params"]["metadata"]["user_api_key_end_user_id"] == "test-user"
    assert result["litelm_params"]["metadata"]["user_api_key_request_route"] is None


def test_init_kwargs_with_litelm_metadata(mock_request, mock_user_api_key_dict):
    """
    Expected behavior: litelm_metadata should be merged with default metadata

    see usage example here: https://docs.litelm.ai/docs/pass_through/anthropic_completion#send-litelm_metadata-tags
    """
    request = mock_request()
    parsed_body = {
        "litelm_metadata": {"custom_field": "custom_value", "tags": ["tag1", "tag2"]}
    }
    passthrough_payload = PassthroughStandardLoggingPayload(
        url="https://test.com",
        request_body={},
    ...
    # Check that litelm_metadata was merged with default metadata
    metadata = result["litelm_params"]["metadata"]
...
async def test_pass_through_request_logging_failure(
    ...
    with patch(
        "litelm.proxy.pass_through_endpoints.pass_through_endpoints.PassThroughEndpointLogging.pass_through_async_success_handler",
...
async def test_pass_through_request_logging_failure_with_stream(
    ...
    # Patch both the logging handler and the httpx client
    with patch(
        "litelm.proxy.pass_through_endpoints.streaming_handler.PassThroughStreamingHandler._route_streaming_logging_to_handler",
        new=mock_logging_failure,
    ), patch(
        "httpx.AsyncClient.send",
        return_value=mock_response,
...
def test_init_kwargs_filters_pricing_params(mock_request, mock_user_api_key_dict):
    ...
    # Verify pricing parameters are stored in litelm_params for internal use
    litelm_params = result["litelm_params"]
    assert litelm_params["input_cost_per_token"] == 0.00002
    assert litelm_params["output_cost_per_token"] == 0.00002
...
def test_custom_pricing_used_in_cost_calculation():
    """
    Test that when custom pricing parameters are provided in litelm_params,
    they are actually used for cost calculation.
    
    This ensures that the custom pricing functionality works end-to-end:
    1. Pricing params are stored in litelm_params
    2. These params are used by completion_cost() to calculate costs
    
    Regression test for: LIT-1221
    """
    from litelm import completion_cost, Choices, Message, ModelResponse
