...
from litelm.llms.anthropic.experimental_pass_through.messages.handler import (
    anthropic_messages,
)
...
class TestAnthropicOpenAIAPI(BaseAnthropicMessagesTest):
    ...
    @pytest.mark.asyncio
    async def test_anthropic_messages_litelm_router_streaming_with_logging(self):
        """
...
async def test_anthropic_messages_streaming_with_bad_request():
    ...
    """
    try:
        response = await litelm.anthropic.messages.acreate(
...
@pytest.mark.asyncio
async def test_anthropic_messages_litelm_router_non_streaming():
    """
...
@pytest.mark.asyncio
async def test_anthropic_messages_litelm_router_routing_strategy():
    """
...
@pytest.mark.asyncio
async def test_anthropic_messages_litelm_router_latency_metadata_tracking():
    """
    Test the anthropic_messages with routing strategy and verify that _latency_per_deployment
    field is passed in litelm_metadata when calling litelm.anthropic_messages
    """
    with unittest.mock.patch("litelm.anthropic_messages") as mock_anthropic_messages:
        ...
        # Verify that litelm.anthropic_messages was called
        ...
        # Verify that litelm_metadata was passed and contains _latency_per_deployment
        assert (
            "litelm_metadata" in call_kwargs
        ), "litelm_metadata should be passed to anthropic_messages"

        litelm_metadata = call_kwargs["litelm_metadata"]
        assert litelm_metadata is not None, "litelm_metadata should not be None"
        assert isinstance(
            litelm_metadata, dict
        ), "litelm_metadata should be a dictionary"

        # Verify _latency_per_deployment is present
        assert (
            "_latency_per_deployment" in litelm_metadata
        ), "litelm_metadata should contain _latency_per_deployment field"

        # Verify the structure of _latency_per_deployment
        latency_per_deployment = litelm_metadata["_latency_per_deployment"]
        assert isinstance(
        ...
        # Verify other expected fields in litelm_metadata
        assert "model_group" in litelm_metadata
        assert litelm_metadata["model_group"] == MODEL_GROUP
        assert "deployment" in litelm_metadata
        assert "model_info" in litelm_metadata
        ...
        print(
            "✅ Successfully verified that _latency_per_deployment is passed in litelm_metadata to anthropic_messages"
        )
...
@pytest.mark.asyncio
async def test_anthropic_messages_litelm_router_non_streaming_with_logging():
    """
    Test the anthropic_messages with non-streaming request
...
#     with unittest.mock.patch("litelm.llms.custom_httpx.llm_http_handler.get_async_httpx_client") as mock_get_client:
    ...
    with unittest.mock.patch.object(
        ...
        with unittest.mock.patch("botocore.auth.SigV4Auth.add_auth"):
            ...
            with unittest.mock.patch(
                ...
                await litelm.anthropic.messages.acreate(
...
def test_sync_openai_messages():
    ...
    """
    litelm._turn_on_debug()
    response = litelm.anthropic.messages.create(
        messages=[{"role": "user", "content": "Hello, can you tell me a short joke?"}],
        model="openai/gpt-4o-mini",
        max_tokens=100,
    )
