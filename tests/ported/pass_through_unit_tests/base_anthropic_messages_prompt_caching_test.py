...
Tests that prompt caching works correctly via litelm.anthropic.messages interface
...
class BaseAnthropicMessagesPromptCachingTest(ABC):
    ...
    async def test_prompt_caching_returns_cache_read_tokens_on_second_call(self):
        ...
        response1 = await litelm.anthropic.messages.acreate(
            model=self.get_model(),
            messages=messages,
            max_tokens=100,
        )
        ...
        response2 = await litelm.anthropic.messages.acreate(
            model=self.get_model(),
            messages=messages,
            max_tokens=100,
        )
