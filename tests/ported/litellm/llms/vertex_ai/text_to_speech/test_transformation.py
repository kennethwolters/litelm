...
import litelm
from litelm.llms.vertex_ai.text_to_speech.transformation import (
...
class TestVertexAITextToSpeechConfig:
    ...
    def test_transform_text_to_speech_request_body(
        ...
        # Test with voice dict in litelm_params (as set by dispatch)
...
@patch("litelm.llms.custom_httpx.llm_http_handler.HTTPHandler.post")
@patch.object(VertexAITextToSpeechConfig, "_ensure_access_token")
@patch.object(VertexAITextToSpeechConfig, "_get_token_and_url")
def test_litelm_speech_vertex_ai_chirp(mock_get_token, mock_ensure_token, mock_post):
    """
    Test that litelm.speech(model="vertex_ai/chirp") sends the correct URL and request body
    """
    # Mock authentication
    mock_ensure_token.return_value = ("mock-token", "test-project")
    mock_get_token.return_value = ("mock-token", "mock-url")
