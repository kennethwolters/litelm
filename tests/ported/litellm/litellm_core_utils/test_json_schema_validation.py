...
Ensures the per-request flag overrides the global litelm.enable_json_schema_validation,
...
from litelm.utils import Rules, post_call_processing
...
def _reset_global_flag():
    ...
    original = litelm.enable_json_schema_validation
    litelm.enable_json_schema_validation = False
    yield
    litelm.enable_json_schema_validation = original
...
class TestPerRequestJsonSchemaValidation:
    ...
    def test_per_request_on_overrides_global_off(self):
        ...
        litelm.enable_json_schema_validation = False
        with pytest.raises(litelm.JSONSchemaValidationError):
            post_call_processing(
                _make_response(INVALID_CONTENT),
                "test-model",
                {
