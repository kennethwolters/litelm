"""
Unit tests for batch ID encoding when x-litelm-model header is used.
...
@pytest.mark.asyncio
async def test_create_batch_with_x_litelm_model_encodes_batch_id():
    """
    When x-litelm-model header is provided, create_batch should encode the
    response batch_id with model info so retrieve_batch can route correctly.
    """
    from litelm.proxy.batches_endpoints.endpoints import create_batch

    model_name = "my-vllm-model"
    raw_batch_id = "batch_abc123"

    mock_response = _make_batch_response(batch_id=raw_batch_id)
    mock_request = _make_mock_request(headers={"x-litelm-model": model_name})
    ...
    with (
        patch(
            "litelm.proxy.batches_endpoints.endpoints._read_request_body",
            new=AsyncMock(
                return_value={"input_file_id": "file-input456", "endpoint": "/v1/chat/completions", "completion_window": "24h"}
            ),
        ),
        patch(
            "litelm.proxy.batches_endpoints.endpoints.ProxyBaseLLMRequestProcessing"
        ) as mock_processor_cls,
        patch(
            "litelm.proxy.batches_endpoints.endpoints.get_credentials_for_model",
            return_value=mock_credentials,
        ),
        patch(
            "litelm.proxy.batches_endpoints.endpoints.prepare_data_with_credentials",
        ),
        patch(
            "litelm.acreate_batch",
            new=AsyncMock(return_value=mock_response),
        ),
        patch(
            "litelm.proxy.batches_endpoints.endpoints.is_known_model",
            return_value=False,
        ),
        patch("litelm.proxy.proxy_server.general_settings", {}),
        patch("litelm.proxy.proxy_server.llm_router", MagicMock()),
        patch("litelm.proxy.proxy_server.proxy_config", MagicMock()),
        patch("litelm.proxy.proxy_server.version", "1.0.0"),
        patch(
...
@pytest.mark.asyncio
async def test_create_batch_with_x_litelm_model_encodes_output_and_error_file_ids():
    """
...
@pytest.mark.asyncio
async def test_create_batch_without_x_litelm_model_returns_raw_ids():
    """
    Without x-litelm-model header, create_batch should NOT encode batch IDs
    ...
    with (
        ...
        patch("litelm.proxy.proxy_server.general_settings", {}),
        patch("litelm.proxy.proxy_server.llm_router", None),
        patch("litelm.proxy.proxy_server.proxy_config", MagicMock()),
        patch("litelm.proxy.proxy_server.version", "1.0.0"),
        patch(
            "litelm.proxy.proxy_server.proxy_logging_obj",
    ...
    # Without x-litelm-model, the batch_id should remain raw
    assert response.id == raw_batch_id
