...
async def test_get_agents_blocked_for_internal_user_when_disabled():
    ...
    from litelm.proxy.agent_endpoints.endpoints import get_agents

    user = _make_internal_user()
    gs = {"disable_agents_for_internal_users": True, "allow_agents_for_team_admins": False}

    request_mock = MagicMock()
    with patch.dict("litelm.proxy.proxy_server.general_settings", gs, clear=True):
...
async def test_get_agents_allowed_when_not_disabled():
    ...
    with patch.dict("litelm.proxy.proxy_server.general_settings", {}, clear=True):
        with patch(
...
async def test_get_agent_daily_activity_blocked_when_disabled():
    from litelm.proxy.agent_endpoints.endpoints import get_agent_daily_activity
