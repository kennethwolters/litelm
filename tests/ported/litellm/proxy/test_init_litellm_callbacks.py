"""
Unit tests for ProxyLogging._init_litelm_callbacks.

Validates that string callbacks in litelm.callbacks are replaced in-place
with their initialized instances, preventing duplicate entries (string + instance)
that caused double-counting of metrics like litelm_proxy_total_requests_metric.
"""
...
class TestInitLitellmCallbacks:
    """Tests for ProxyLogging._init_litelm_callbacks."""
    ...
    @patch(
        "litelm.proxy.utils.ProxyLogging._add_proxy_hooks",
        new_callable=lambda: lambda self, *a, **kw: None,
    )
    def test_should_replace_string_callback_with_instance(self, _mock_hooks):
        """
        When litelm.callbacks contains a string callback (e.g. "lago"),
        _init_litelm_callbacks should replace the string with the initialized
        CustomLogger instance, not leave both the string and instance in the list.
        """
        fake_logger = FakeCustomLogger()

        # Start with a string callback in litelm.callbacks
        litelm.callbacks = ["lago"]  # type: ignore

        proxy_logging = self._make_proxy_logging()

        with patch(
            "litelm.litelm_core_utils.litelm_logging._init_custom_logger_compatible_class",
            return_value=fake_logger,
        ):
            proxy_logging._init_litelm_callbacks(llm_router=None)

        # The string "lago" should be replaced by the instance, not appended
        string_entries = [c for c in litelm.callbacks if isinstance(c, str)]
        instance_entries = [
            c for c in litelm.callbacks if isinstance(c, FakeCustomLogger)
        ]
    ...
    def test_should_not_duplicate_existing_instance_callbacks(self, _mock_hooks):
        """
        When litelm.callbacks already contains a CustomLogger instance (not a string),
        _init_litelm_callbacks should not create a duplicate.
        """
        existing_logger = FakeCustomLogger()

        litelm.callbacks = [existing_logger]  # type: ignore

        proxy_logging = self._make_proxy_logging()

        proxy_logging._init_litelm_callbacks(llm_router=None)

        # Count how many FakeCustomLogger instances are in litelm.callbacks
        instance_count = sum(
            1 for c in litelm.callbacks if isinstance(c, FakeCustomLogger)
        )
        assert instance_count == 1, (
            f"Expected exactly 1 FakeCustomLogger instance, found {instance_count}. "
            f"litelm.callbacks = {litelm.callbacks}"
        )
    ...
    def test_should_handle_unrecognized_string_callback(self, _mock_hooks):
        ...
        the string should remain in litelm.callbacks (not crash).
        """
        litelm.callbacks = ["unknown_callback"]  # type: ignore
        ...
        assert "unknown_callback" in litelm.callbacks
    ...
    def test_should_replace_multiple_string_callbacks(self, _mock_hooks):
        """
        When litelm.callbacks contains multiple string callbacks,
        ...
        litelm.callbacks = ["callback_a", "callback_b"]  # type: ignore
