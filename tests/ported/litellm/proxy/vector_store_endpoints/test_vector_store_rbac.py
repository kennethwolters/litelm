...
async def test_list_vector_stores_blocked_when_disabled():
    from litelm.proxy.vector_store_endpoints.management_endpoints import list_vector_stores

    user = _make_internal_user()
    with patch.dict("litelm.proxy.proxy_server.general_settings", _DISABLED_GS, clear=True):
...
async def test_list_vector_stores_allowed_when_not_disabled():
    ...
    mock_prisma.db.litelm_managedvectorstorestable.find_many = AsyncMock(return_value=[])

    with patch.dict("litelm.proxy.proxy_server.general_settings", _ENABLED_GS, clear=True):
        with patch("litelm.proxy.proxy_server.prisma_client", mock_prisma):
            with patch.object(litelm, "vector_store_registry", None):
                with patch(
                    "litelm.proxy.vector_store_endpoints.management_endpoints.VectorStoreRegistry._get_vector_stores_from_db",
...
async def test_new_vector_store_blocked_when_disabled():
    from litelm.proxy.vector_store_endpoints.management_endpoints import new_vector_store
