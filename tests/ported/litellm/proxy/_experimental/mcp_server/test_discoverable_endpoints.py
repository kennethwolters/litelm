...
async def test_authorize_endpoint_includes_response_type():
    ...
    try:
        from litelm.proxy._experimental.mcp_server.discoverable_endpoints import (
            authorize,
        )
    ...
    mock_request.base_url = "https://litelm.example.com/"
    mock_request.headers = {}

    # Mock the encryption functions to avoid needing a signing key
    with patch(
        "litelm.proxy._experimental.mcp_server.discoverable_endpoints.encrypt_value_helper"
    ) as mock_encrypt:
...
async def test_authorize_endpoint_forwards_pkce_parameters():
    ...
    # Mock request
    mock_request = MagicMock(spec=Request)
    mock_request.base_url = "https://litelm-proxy.example.com/"
    mock_request.headers = {}
...
async def test_token_endpoint_forwards_code_verifier():
    ...
    with patch(
        "litelm.proxy._experimental.mcp_server.discoverable_endpoints.get_async_httpx_client"
    ) as mock_get_client:
...
async def test_register_client_without_mcp_server_name_returns_dummy():
    ...
    mock_request = MagicMock(spec=Request)
    mock_request.base_url = "https://proxy.litelm.example/"
    mock_request.headers = {}
    with patch(
        "litelm.proxy._experimental.mcp_server.discoverable_endpoints._read_request_body",
    ...
    assert result == {
        ...
        "client_secret": "dummy",
        "redirect_uris": ["https://proxy.litelm.example/callback"],
    }
...
async def test_register_client_remote_registration_success():
    ...
    assert call_args.kwargs["json"]["redirect_uris"] == [
        "https://proxy.litelm.example/callback"
    ]
...
async def test_authorize_endpoint_respects_x_forwarded_proto():
    ...
    mock_request = MagicMock(spec=Request)
    mock_request.base_url = "http://litelm.example.com/"  # HTTP
    ...
    assert (
        "redirect_uri=https%3A%2F%2Flitelm.example.com%2Fcallback" in location
        or "redirect_uri=https://litelm.example.com/callback" in location
    )
...
async def test_token_endpoint_respects_x_forwarded_proto():
    ...
    # Mock request with http base_url but X-Forwarded-Proto: https
    mock_request = MagicMock(spec=Request)
    mock_request.base_url = "http://litelm-proxy.example.com/"  # HTTP
    mock_request.headers = {"X-Forwarded-Proto": "https"}  # Behind HTTPS proxy
    ...
    assert (
        ...
        == "https://litelm-proxy.example.com/callback"
    )
...
async def test_oauth_protected_resource_standard_pattern():
    ...
    assert response["resource"] == "https://litelm.example.com/mcp/test_server"
    assert response["authorization_servers"][0] == "https://litelm.example.com/test_server"
...
async def test_oauth_protected_resource_legacy_pattern():
    ...
    assert response["resource"] == "https://litelm.example.com/test_server/mcp"
    assert response["authorization_servers"][0] == "https://litelm.example.com/test_server"
    assert response["scopes_supported"] == oauth2_server.scopes
...
async def test_oauth_authorization_server_respects_x_forwarded_proto():
    ...
    # Verify response uses HTTPS URLs
    assert response["authorization_endpoint"].startswith("https://litelm.example.com/")
    assert response["token_endpoint"].startswith("https://litelm.example.com/")
    assert response["registration_endpoint"].startswith("https://litelm.example.com/")
...
async def test_register_client_respects_x_forwarded_proto():
    ...
    # Mock request with http base_url but X-Forwarded-Proto: https
    mock_request = MagicMock(spec=Request)
    mock_request.base_url = "http://proxy.litelm.example/"  # HTTP
    mock_request.headers = {"X-Forwarded-Proto": "https"}  # Behind HTTPS proxy
