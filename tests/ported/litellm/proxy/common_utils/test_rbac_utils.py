"""
Tests for litelm/proxy/common_utils/rbac_utils.py
...
from litelm.proxy.common_utils.rbac_utils import check_feature_access_for_user
...
# general_settings is imported from litelm.proxy.proxy_server inside the
# function, so we patch it via patch.dict on the original dict.
_GS_PATH = "litelm.proxy.proxy_server.general_settings"
...
async def test_agents_disabled_team_admin_allowed():
    ...
    with patch.dict(
        ...
        with patch(
            "litelm.proxy.management_endpoints.common_utils._user_has_admin_privileges",
