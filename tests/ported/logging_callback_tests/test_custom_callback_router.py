...
import litelm
from litelm import Cache, Router
...
class CompletionCustomHandler(
    ...
    def log_pre_api_call(self, model, messages, kwargs):
        try:
            ...
            assert isinstance(kwargs["litelm_params"]["metadata"], dict)
            assert isinstance(kwargs["litelm_params"]["metadata"]["model_group"], str)
            assert isinstance(kwargs["litelm_params"]["metadata"]["deployment"], str)
            assert isinstance(kwargs["litelm_params"]["model_info"], dict)
            assert isinstance(kwargs["litelm_params"]["model_info"]["id"], str)
            assert isinstance(
                kwargs["litelm_params"]["proxy_server_request"], (str, type(None))
            )
            assert isinstance(
                kwargs["litelm_params"]["preset_cache_key"], (str, type(None))
            )
            assert isinstance(kwargs["litelm_params"]["stream_response"], dict)
    ...
    def log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            assert isinstance(end_time, datetime)
            ## RESPONSE OBJECT
            assert isinstance(response_obj, litelm.ModelResponse)
            ## KWARGS
            assert isinstance(kwargs["model"], str)
            assert isinstance(kwargs["messages"], list) and isinstance(
                kwargs["messages"][0], dict
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            ## RESPONSE OBJECT
            assert isinstance(
                response_obj, (litelm.ModelResponse, litelm.EmbeddingResponse)
            )
            ## KWARGS
            assert isinstance(kwargs["model"], str)

            # checking we use base_model for azure cost calculation
            base_model = litelm.utils._get_base_model_from_metadata(
            ...
            if (
                ...
                # this checks response_cost == litelm.cost_per_token(model=base_model)
                ...
                prompt_price, completion_price = litelm.cost_per_token(
                    model=base_model,
...
async def test_async_chat_azure():
    try:
        ...
        litelm.callbacks = [customHandler_completion_azure_router]
        ...
        litelm.callbacks = [customHandler_streaming_azure_router]
...
async def test_async_embedding_azure():
    try:
        ...
        response = await router.aembedding(
            model="azure-embedding-model", input=["hello from litelm!"]
        )
...
async def test_async_chat_azure_with_fallbacks():
    try:
        ...
        litelm.callbacks = [customHandler_fallbacks]
        litelm.set_verbose = True
        # with fallbacks
        model_list = [
            {
        ...
        router = Router(
            ...
            retry_policy=litelm.router.RetryPolicy(
...
async def test_async_completion_azure_caching():
    ...
    litelm.callbacks = [customHandler_caching]
...
async def test_async_completion_azure_caching_streaming():
    ...
    # Use Router instead of direct litelm.acompletion to get router-specific metadata
    model_list = [
        {
...
async def test_async_embedding_azure_caching():
    ...
    response1 = await router.aembedding(
        ...
        input=[f"good morning from litelm1 {unique_time}"],
        caching=True,
    )
...
async def test_rate_limit_error_callback():
    ...
    Relevant issue: https://github.com/BerriAI/litelm/issues/4096
    """
    ...
    litelm_logging_obj = LiteLLMLogging(
        model="my-test-gpt",
    ...
    with patch.object(
        ...
        except (litelm.RateLimitError, ValueError):
            pass
