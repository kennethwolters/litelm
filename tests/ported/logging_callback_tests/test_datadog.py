...
from litelm.integrations.datadog.datadog_handler import (
  get_datadog_source,
  get_datadog_service,
  get_datadog_env, 
...
import litelm.integrations.datadog.datadog as datadog_module
...
from litelm.types.integrations.datadog import DatadogInitParams
...
async def test_create_datadog_logging_payload():
    ...
    assert dd_payload["ddsource"] == os.getenv("DD_SOURCE", "litelm")
    assert dd_payload["service"] == "litelm-server"
...
async def test_datadog_log_redis_failures():
    ...
    try:
        ...
        litelm.callbacks = [dd_logger]
        litelm.service_callback = ["datadog"]
...
async def test_datadog_logging():
    try:
        litelm.success_callback = ["datadog"]
        litelm.set_verbose = True
        response = await litelm.acompletion(
            model="gpt-3.5-turbo",
            messages=[{"role": "user", "content": "what llm are u"}],
...
def test_datadog_static_methods():
    ...
    assert get_datadog_source() == "litelm"
    assert get_datadog_service() == "litelm-server"
    ...
    assert (
        "env:unknown,service:litelm-server,version:unknown,HOSTNAME:"
        in get_datadog_tags()
    )
...
async def test_datadog_message_redaction():
    ...
    from litelm.datadog_params
    """
    try:
        # Test using litelm.datadog_params pattern
        litelm.datadog_params = DatadogInitParams(turn_off_message_logging=True)
        ...
        # Verify that turn_off_message_logging was set correctly from litelm.datadog_params
...
def test_datadog_ignores_ddtrace_agent_host():
    ...
    Regression test for: https://github.com/BerriAI/litelm/issues/16379
    """
    test_env = {
