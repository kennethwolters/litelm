...
async def test_basic_s3_v2_logging(streaming):
    ...
    litelm.callbacks = [s3_v2_logger]
    blockbuster = BlockBuster()
    blockbuster.activate()

    litelm._turn_on_debug()
    litelm.callbacks = ["s3_v2"]
    litelm.s3_callback_params = {
        "s3_bucket_name": "load-testing-oct",
        "s3_aws_secret_access_key": "os.environ/AWS_SECRET_ACCESS_KEY",
        "s3_aws_access_key_id": "os.environ/AWS_ACCESS_KEY_ID",
...
def test_s3_logging():
    ...
    # on circle ci - we only test litelm.acompletion()
    try:
...
def test_s3_logging_r2():
    ...
    try:
        # redirect stdout to log_file
        # litelm.cache = litelm.Cache(
        #     type="s3", s3_bucket_name="litelm-r2-bucket", s3_region_name="us-west-2"
        # )
        litelm.set_verbose = True
        from litelm._logging import verbose_logger
        import logging
        ...
        litelm.success_callback = ["s3"]
        litelm.s3_callback_params = {
            "s3_bucket_name": "litelm-r2-bucket",
