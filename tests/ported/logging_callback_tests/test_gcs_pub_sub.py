...
from litelm.integrations.gcs_pubsub.pub_sub import *
...
async def test_async_gcs_pub_sub():
    ...
    litelm.callbacks = [gcs_pub_sub_logger]
    ...
    assert (
        actual_url
        == "https://pubsub.googleapis.com/v1/projects/reliableKeys/topics/litelmDB:publish"
    )
...
async def test_async_gcs_pub_sub_v1():
    # Create a mock for the async_httpx_client's post method
    litelm.gcs_pub_sub_use_v1 = True
    mock_post = AsyncMock()
    mock_post.return_value.status_code = 202
    mock_post.return_value.text = "Accepted"
