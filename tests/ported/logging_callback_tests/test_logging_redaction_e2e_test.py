...
async def test_global_redaction_on():
    ...
    assert response["choices"][0]["message"]["content"] == "redacted-by-litelm"
    assert standard_logging_payload["messages"][0]["content"] == "redacted-by-litelm"
    print(
...
async def test_global_redaction_with_dynamic_params(turn_off_message_logging):
    ...
    if turn_off_message_logging is True:
        ...
        assert response["choices"][0]["message"]["content"] == "redacted-by-litelm"
        assert (
            standard_logging_payload["messages"][0]["content"] == "redacted-by-litelm"
        )
    else:
        assert (
        ...
        if "content" in output_item and isinstance(output_item["content"], list):
            for content_item in output_item["content"]:
                if "text" in content_item:
                    assert content_item["text"] == "redacted-by-litelm", f"Expected redacted text but got: {content_item['text']}"
    print(
...
async def test_redaction_responses_api_stream():
    ...
    if "choices" in standard_logging_payload["response"]:
        ...
        assert standard_logging_payload["response"]["choices"][0]["message"]["content"] == "redacted-by-litelm"
...
async def test_redaction_responses_api_with_reasoning_summary():
    ...
    from litelm.litelm_core_utils.redact_messages import perform_redaction
    ...
    def patched_isinstance(obj, cls):
        if cls == litelm.ResponsesAPIResponse and obj.__class__.__name__ == 'ResponsesAPIResponse':
            return True
    ...
    try:
        ...
        assert model_call_details["messages"][0]["content"] == "redacted-by-litelm", \
...
async def test_redaction_with_coroutine_objects():
    ...
    assert result == {"text": "redacted-by-litelm"}
...
async def test_disable_redaction_header_responses_api():
    ...
    because Responses API uses 'litelm_metadata' instead of 'metadata'.
    """
    litelm.turn_off_message_logging = True
    test_custom_logger = TestCustomLogger()
    litelm.callbacks = [test_custom_logger]
    ...
    # Pass the header via litelm_metadata (as the proxy does for Responses API)
    response = await litelm.aresponses(
        model="gpt-3.5-turbo",
        input="hi",
        mock_response=mock_response,
        litelm_metadata={
            "headers": {
                "litelm-disable-message-redaction": "true"
            }
        }
    )
    ...
    assert standard_logging_payload["response"] != {"text": "redacted-by-litelm"}
    assert standard_logging_payload["messages"][0]["content"] == "hi"
...
async def test_redaction_with_metadata_completion_api():
    ...
    # based on the global setting (litelm.turn_off_message_logging = True)
    response = await litelm.acompletion(
        model="gpt-3.5-turbo",
        messages=[{"role": "user", "content": "hi"}],
        mock_response="hello",
