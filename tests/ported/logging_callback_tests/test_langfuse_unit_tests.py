...
import pytest
from litelm.integrations.langfuse.langfuse import (
    LangFuseLogger,
)
from litelm.integrations.langfuse.langfuse_handler import LangFuseHandler
from litelm.litelm_core_utils.litelm_logging import DynamicLoggingCache
...
def test_langfuse_e2e_sync(monkeypatch):
    ...
    monkeypatch.setattr(litelm, "success_callback", ["langfuse"])
    ...
    with respx.mock:
        ...
        completion(
            ...
            messages=[{"role": "user", "content": "hello from litelm"}],
            stream=False,
            mock_response="Hello from litelm 2",
        )
...
def test_masking_function_isolated_from_other_loggers():
    ...
    """
    from litelm.litelm_core_utils.litelm_logging import scrub_sensitive_keys_in_metadata

    def my_masking_fn(data):
        return data

    # Simulate litelm_params with masking function in metadata
    litelm_params = {
        "metadata": {
    ...
    result = scrub_sensitive_keys_in_metadata(litelm_params)
