...
from litelm.integrations.langsmith import (
    LangsmithLogger,
...
async def test_is_serializable():
    from litelm.integrations.langsmith import is_serializable
...
async def test_langsmith_key_based_logging():
    """
    In key based logging langsmith_api_key and langsmith_project are passed directly to litelm.acompletion
    """
    try:
        ...
        mock_get_client = patch(
            "litelm.integrations.langsmith.get_async_httpx_client",
            return_value=mock_async_httpx_handler
        )
        mock_get_client.start()
        
        litelm.set_verbose = True
        litelm.DEFAULT_FLUSH_INTERVAL_SECONDS = 1

        litelm.callbacks = [LangsmithLogger()]
...
async def test_langsmith_queue_logging():
    try:
        ...
        litelm.callbacks = [test_langsmith_logger]
