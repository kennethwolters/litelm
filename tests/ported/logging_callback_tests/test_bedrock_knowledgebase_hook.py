...
import asyncio
import litelm
import litelm.vector_stores.main
import gzip
import json
import logging
import time
...
from litelm.integrations.vector_store_integrations.vector_store_pre_call_hook import VectorStorePreCallHook
...
from litelm.types.utils import StandardLoggingPayload, StandardLoggingVectorStoreRequest
...
def setup_vector_store_registry():
    from litelm.vector_stores.vector_store_registry import VectorStoreRegistry, LiteLLM_ManagedVectorStore
    # Init vector store registry
    litelm.vector_store_registry = VectorStoreRegistry(
        vector_stores=[
...
async def test_e2e_bedrock_knowledgebase_retrieval_with_completion(setup_vector_store_registry):
    ...
    print("value of litelm.vector_store_registry:", litelm.vector_store_registry)
    ...
    with patch.object(client, "post") as mock_post:
        ...
        try:
            response = await litelm.acompletion(
                ...
                messages=[{"role": "user", "content": "what is litelm?"}],
                vector_store_ids = [
                    "T37J8R4WTM"
                ],
                client=client
...
async def test_e2e_bedrock_knowledgebase_retrieval_with_llm_api_call_with_tools_and_filters(setup_vector_store_registry):
    ...
    # The logs above show:  litelm.asearch(... filters={'key': 'user_id', 'value': 'fake-user-id', 'operator': 'eq'})
...
async def test_bedrock_kb_request_body_has_transformed_filters(setup_vector_store_registry):
    ...
    async def fake_async_vector_store_search_handler(
        ...
        litelm_params_dict = (
            litelm_params.model_dump(exclude_none=False)
            if hasattr(litelm_params, "model_dump")
            else dict(litelm_params)
        )
        api_base = vector_store_provider_config.get_complete_url(
            api_base=litelm_params_dict.get("api_base"),
            litelm_params=litelm_params_dict,
        )
    ...
    with patch.object(
        litelm.vector_stores.main.base_llm_http_handler,
...
async def test_openai_with_vector_store_ids_in_tool_call_mock_openai(setup_vector_store_registry):
    ...
    with patch.object(
        ...
        # assert that the tool call was not sent to the upstream llm API if it's a litelm vector store
...
#     assert vector_store_request_metadata.get("query") == "what is litelm?"
...
#     assert vector_store_search_response.get("search_query") == "what is litelm?"
...
async def test_e2e_bedrock_knowledgebase_retrieval_without_vector_store_registry(setup_vector_store_registry):
    litelm._turn_on_debug()
    client = AsyncHTTPHandler()
    litelm.vector_store_registry = None
...
async def test_e2e_bedrock_knowledgebase_retrieval_with_vector_store_not_in_registry(setup_vector_store_registry):
    ...
    if litelm.vector_store_registry is not None:
        print("Registry iniitalized:", litelm.vector_store_registry.vector_stores)
    else:
...
async def test_provider_specific_fields_in_proxy_http_response(setup_vector_store_registry):
    ...
    mock_response.usage = litelm.Usage(
        prompt_tokens=10,
    ...
    with mock_patch("litelm.acompletion", new=AsyncMock(return_value=mock_response)):
