...
def test_get_model_cost_information():
    ...
    litelm_info_gpt_3_5_turbo_model_map_value = litelm.get_model_info(
    ...
    assert result["model_map_value"] == litelm_info_gpt_3_5_turbo_model_map_value
    # assert all fields in StandardLoggingModelInformation are present
    assert all(
        field in result for field in StandardLoggingModelInformation.__annotations__
    )
...
def test_get_final_response_obj():
    ...
    # Test redaction when litelm.turn_off_message_logging is True
    litelm.turn_off_message_logging = True
    try:
        model_response = litelm.ModelResponse(
            choices=[
                litelm.Choices(message=litelm.Message(content="sensitive content"))
            ]
        )
        ...
        assert result["choices"][0]["message"]["content"] == "redacted-by-litelm"
        # Verify that redaction occurred in kwargs
        assert kwargs["messages"][0]["content"] == "redacted-by-litelm"
    finally:
        # Reset litelm.turn_off_message_logging to its original value
...
def test_get_standard_logging_payload_trace_id():
    ...
    # Test case 1: When litelm_trace_id is provided in litelm_params
    from unittest.mock import MagicMock
    
    # Create a mock Logging object
    mock_logging_obj = MagicMock()
    mock_logging_obj.litelm_trace_id = "default-trace-id"
    
    # Test when litelm_trace_id is in litelm_params
    litelm_params = {"litelm_trace_id": "dynamic-trace-id"}
    ...
    # Test case 2: When litelm_trace_id is not provided in litelm_params
    litelm_params = {}
    result = StandardLoggingPayloadSetup._get_standard_logging_payload_trace_id(
        logging_obj=mock_logging_obj,
        litelm_params=litelm_params
    ...
    # Test case 3: When litelm_params is None
    result = StandardLoggingPayloadSetup._get_standard_logging_payload_trace_id(
        logging_obj=mock_logging_obj,
        litelm_params={}
    )
    ...
    # Test case 4: When litelm_trace_id in params is not a string
    litelm_params = {"litelm_trace_id": 12345}
    result = StandardLoggingPayloadSetup._get_standard_logging_payload_trace_id(
        logging_obj=mock_logging_obj,
        litelm_params=litelm_params
    )
...
def test_get_error_information():
    ...
    # Test with litelm exception from provider
    litelm_exception = litelm.exceptions.RateLimitError(
        message="Test error",
        llm_provider="openai",
        model="gpt-3.5-turbo",
        response=None,
        litelm_debug_info=None,
        max_retries=None,
        num_retries=None,
    )
    ...
    assert result["error_message"] == "litelm.RateLimitError: Test error"
...
def test_cost_breakdown_in_standard_logging_payload():
    ...
    """
    from litelm.litelm_core_utils.litelm_logging import get_standard_logging_object_payload, Logging
...
def test_usage_dict_roundtrip_in_payload(use_combined_usage_object):
    ...
    logging_obj = Logging(
        ...
        call_type="completion",
        start_time=datetime.now(),
        litelm_call_id="test-usage-roundtrip",
        function_id="test-fn",
    )
...
def test_standard_logging_payload_uses_actual_model_for_azure_router():
    ...
    logging_obj = Logging(
        ...
        call_type="completion",
        start_time=datetime.now(),
        litelm_call_id="test-azure-router-opt-in",
        function_id="test-fn",
    )
...
def test_standard_logging_payload_uses_actual_model_for_azure_router_with_underscore():
    ...
    logging_obj = Logging(
        ...
        call_type="completion",
        start_time=datetime.now(),
        litelm_call_id="test-azure-router-underscore",
        function_id="test-fn",
    )
...
def test_merge_litelm_metadata_basic():
    """
    Test that merge_litelm_metadata correctly merges metadata and litelm_metadata.
    User API key fields (from metadata) should take precedence over model-related fields (from litelm_metadata).
    """
    litelm_params = {
        "metadata": {
    ...
    result = StandardLoggingPayloadSetup.merge_litelm_metadata(litelm_params)
...
def test_merge_litelm_metadata_precedence():
    """
    Test that metadata fields take precedence over litelm_metadata when there are conflicts.
    """
    litelm_params = {
        "metadata": {
        ...
        "litelm_metadata": {
            ...
            "custom_field": "from_litelm_metadata",  # This should NOT overwrite
    ...
    # litelm_metadata values should only be included if not in metadata
    assert result["model_group"] == "gpt-4-group"


def test_merge_litelm_metadata_skip_non_serializable():
    """
...
def test_merge_litelm_metadata_empty_params():
    """
    Test that merge_litelm_metadata handles empty or missing metadata gracefully.
    """
    # Test with empty litelm_params
    result = StandardLoggingPayloadSetup.merge_litelm_metadata({})
    assert result == {}
    ...
    # Test with only litelm_metadata
    litelm_params = {
        "litelm_metadata": {
            "model_group": "gpt-4-group",
        }
...
def test_merge_litelm_metadata_bedrock_passthrough_scenario():
    """
    Test merge_litelm_metadata in a Bedrock passthrough scenario where both 
