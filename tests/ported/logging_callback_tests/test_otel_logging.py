...
from litelm.integrations.arize.arize_phoenix import ArizePhoenixLogger
from litelm.integrations._types.open_inference import (
    OpenInferenceSpanKindValues,
    SpanAttributes as OISpanAttributes,
)
from litelm.integrations.opentelemetry import (
...
from litelm.proxy._types import SpanAttributes

verbose_logger.setLevel(logging.DEBUG)

EXPECTED_SPAN_NAMES = ["litelm_request", "raw_gen_ai_request"]
...
async def test_async_otel_callback(streaming):
    ...
    litelm.callbacks = [OpenTelemetry(config=OpenTelemetryConfig(exporter=exporter))]
    ...
    for span in spans:
        ...
        if span.name == "litelm_request":
            validate_litelm_request(span)
...
def validate_litelm_request(span):
    expected_attributes = [
...
async def test_awesome_otel_with_message_logging_off(streaming, global_redact):
    ...
    tests when litelm.turn_off_message_logging is set to True
...
async def test_arize_phoenix_creates_nested_spans_on_dedicated_provider():
    ...
    ``litelm_proxy_request`` parent span and a ``litelm_request`` child span
    ...
    litelm.callbacks = [phoenix_logger]
    litelm.success_callback = []
    litelm.failure_callback = []
    ...
    # - "litelm_proxy_request" (parent) — created by _get_phoenix_context
    # - "litelm_request" (child)       — the LLM call span
    # - "raw_gen_ai_request"            — raw request sub-span
    assert "litelm_proxy_request" in span_names, f"Expected proxy parent span, got: {span_names}"
