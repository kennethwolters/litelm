...
from litelm.types.integrations.slack_alerting import AlertType
...
from litelm.integrations.SlackAlerting.slack_alerting import (
    DeploymentMetrics,
    SlackAlerting,
)
from litelm.proxy._types import CallInfo, Litellm_EntityType, WebhookEvent
from litelm.proxy.utils import ProxyLogging
from litelm.router import AlertingConfig, Router
from litelm.utils import get_api_base
...
async def test_get_api_base():
    ...
    litelm_params = {
        ...
        "litelm_call_id": "68f46d2d-714d-4ad8-8137-69600ec8755c",
        "model_alias_map": {},
...
async def test_response_taking_too_long_callback(slack_alerting):
    ...
    kwargs = {"model": "test_model", "messages": "test_messages", "litelm_params": {}}
...
async def test_alerting_metadata(slack_alerting):
    ...
    kwargs = {
        ...
        "litelm_params": {"metadata": {"alerting_metadata": {"hello": "world"}}},
    }
    with patch.object(slack_alerting, "send_alert", new=AsyncMock()) as mock_send_alert:
...
async def test_daily_reports_unit_test(slack_alerting):
    with patch.object(slack_alerting, "send_alert", new=AsyncMock()) as mock_send_alert:
        ...
        deployment_metrics = DeploymentMetrics(
            ...
            updated_at=litelm.utils.get_utc_datetime(),
        )
...
async def test_daily_reports_completion(slack_alerting):
    with patch.object(slack_alerting, "send_alert", new=AsyncMock()) as mock_send_alert:
        litelm.callbacks = [slack_alerting]
        ...
        router = litelm.Router(
            model_list=[
                {
                    "model_name": "gpt-5",
                    "litelm_params": {"model": "gpt-3.5-turbo", "api_key": "bad_key"},
                }
            ]
        )
...
async def test_daily_reports_redis_cache_scheduler():
    ...
    from litelm.router import AlertingConfig
...
async def test_send_daily_reports_ignores_zero_values():
    ...
    router.get_model_info.side_effect = lambda x: {"litelm_params": {"model": x}}
...
async def test_outage_alerting_called(
    ...
    if error_code == 400:
        ...
        error_to_raise = litelm.BadRequestError(
        ...
        error_to_raise = litelm.ServiceUnavailableError(
            ...
            response=httpx.Response(
                ...
                request=httpx.Request(
                    method="completion",
                    url="https://github.com/BerriAI/litelm",
                ),
            ),
        )
...
async def test_langfuse_trace_id():
    ...
    from litelm.integrations.SlackAlerting.utils import _add_langfuse_trace_id_to_alert
    ...
    trace_url = await _add_langfuse_trace_id_to_alert(
        request_data={"litelm_logging_obj": litelm_logging_obj}
    )
    ...
    assert returned_trace_id == litelm_logging_obj._get_trace_id(service_name="langfuse")
...
async def test_print_alerting_payload_warning():
    ...
    from litelm.integrations.SlackAlerting.batching_handler import send_to_webhook
    import logging
