...
from litelm.litelm_core_utils.custom_logger_registry import (
...
async def use_callback_in_llm_call(
    ...
    if callback == "dynamic_rate_limiter":
        ...
        litelm.argilla_transformation_object = {}
        ...
        litelm.global_bitbucket_config = {
            "workspace": "test-workspace",
            "repository": "test-repo",
            "access_token": "test-token",
            "branch": "main"
        }
        litelm.global_gitlab_config = {
    ...
    if used_in == "callbacks":
        ...
        litelm.success_callback = [callback]
    ...
    for _ in range(5):
        ...
        if used_in == "callbacks":
            assert isinstance(litelm._async_success_callback[0], expected_class)
            assert isinstance(litelm._async_failure_callback[0], expected_class)
            assert isinstance(litelm.success_callback[0], expected_class)
            assert isinstance(litelm.failure_callback[0], expected_class)

            assert (
                len(litelm._async_success_callback) == 1
            ), f"Got={litelm._async_success_callback}"
            assert len(litelm._async_failure_callback) == 1
            assert len(litelm.success_callback) == 1
            assert len(litelm.failure_callback) == 1
            assert len(litelm.callbacks) == 1
        elif used_in == "success_callback":
            print(f"litelm.success_callback: {litelm.success_callback}")
            print(f"litelm._async_success_callback: {litelm._async_success_callback}")
            assert isinstance(litelm.success_callback[0], expected_class)
            assert len(litelm.success_callback) == 1  # ["lago", LagoLogger]
            assert isinstance(litelm._async_success_callback[0], expected_class)
        ...
        if callback == "bitbucket":
            ...
            if hasattr(litelm, 'global_bitbucket_config'):
                delattr(litelm, 'global_bitbucket_config')
            patch.stopall()
...
def test_dynamic_logging_global_callback():
    ...
    from litelm.types.utils import ModelResponse, Choices, Message, Usage

    cl = CustomLogger()

    litelm_logging = LiteLLMLoggingObj(
    ...
    with patch.object(cl, "log_success_event") as mock_log_success_event:
        ...
        litelm.success_callback = [cl]

        try:
            litelm_logging.success_handler(
