...
from litelm.types.utils import StandardLoggingPayload, Usage, ModelInfoBase
...
async def test_stream_token_counting_gpt_4o():
    ...
    litelm.logging_callback_manager.add_litelm_callback(custom_logger)
...
async def test_stream_token_counting_without_include_usage():
    ...
    by default, litelm passes `include_usage=True` for OpenAI API
    """
    custom_logger = TestCustomLogger()
    litelm.logging_callback_manager.add_litelm_callback(custom_logger)
...
async def test_stream_token_counting_with_redaction():
    """
    When litelm.turn_off_message_logging=True is used, the usage tracked == usage from llm api chunk
    """
...
async def test_stream_token_counting_anthropic_with_include_usage():
    ...
    print("input_tokens_litelm", custom_logger.recorded_usage.prompt_tokens)
    print("output_tokens_litelm", custom_logger.recorded_usage.completion_tokens)
