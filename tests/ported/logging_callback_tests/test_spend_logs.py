...
import litelm
from litelm.proxy.spend_tracking.spend_tracking_utils import get_logging_payload, _sanitize_request_body_for_spend_logs_payload
from litelm.proxy._types import SpendLogsMetadata, SpendLogsPayload
...
def test_spend_logs_payload(model_id: Optional[str]):
    ...
    input_args: dict = {
        "kwargs": {
            ...
            "litelm_params": {
                ...
                "litelm_call_id": "b9929bf6-7b80-4c8c-b486-034e6ac0c8b7",
                "model_alias_map": {},
    ...
    # This is crucial - used in PROD, it should pass, related issue: https://github.com/BerriAI/litelm/issues/4334
    assert (
...
def test_spend_logs_payload_whisper():
    ...
    kwargs: dict = {
        ...
        "litelm_call_id": "05921cf7-33f9-421c-aad9-33310c1e2702",
    ...
    response = litelm.utils.TranscriptionResponse(
...
def test_spend_logs_payload_with_prompts_enabled(monkeypatch):
    ...
    from litelm.proxy.proxy_server import general_settings
        ...
        "response_obj": litelm.ModelResponse(
            ...
            choices=[
                litelm.Choices(
                    finish_reason="stop",
                    index=0,
                    message=litelm.Message(content="Hi there!", role="assistant"),
                )
            ],
            model="gpt-3.5-turbo",
            usage=litelm.Usage(completion_tokens=2, prompt_tokens=1, total_tokens=3),
        ),
        "start_time": datetime.datetime.now(),
        "end_time": datetime.datetime.now(),
    }
    ...
    input_args["kwargs"]["litelm_params"] = litelm_params
...
def test_large_request_no_truncation_threshold():
    ...
    """
    from litelm.constants import MAX_STRING_LENGTH_PROMPT_IN_DB, LITELLM_TRUNCATED_PAYLOAD_FIELD
...
def test_configurable_string_length_env_var(monkeypatch):
    ...
    import importlib
    import litelm.constants
    import litelm.proxy.spend_tracking.spend_tracking_utils
    importlib.reload(litelm.constants)
    importlib.reload(litelm.proxy.spend_tracking.spend_tracking_utils)
    
    from litelm.constants import MAX_STRING_LENGTH_PROMPT_IN_DB, LITELLM_TRUNCATED_PAYLOAD_FIELD
    from litelm.proxy.spend_tracking.spend_tracking_utils import _sanitize_request_body_for_spend_logs_payload
