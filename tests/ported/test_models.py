...
async def get_model_info(session, key, litelm_model_id=None):
    """
    Make sure only models user has access to are returned
    """
    if litelm_model_id:
        url = f"http://0.0.0.0:4000/model/info?litelm_model_id={litelm_model_id}"
    else:
...
async def test_get_specific_model():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for idx, m in enumerate(models):
            assert m == "gpt-4"
            litelm_model_id = response["data"][idx]["model_info"]["id"]
            model_specific_info = response["data"][idx]
        assert litelm_model_id is not None
        response = await get_model_info(
            session=session, key=key, litelm_model_id=litelm_model_id
        )
        assert response["data"][0]["model_info"]["id"] == litelm_model_id
        assert (
