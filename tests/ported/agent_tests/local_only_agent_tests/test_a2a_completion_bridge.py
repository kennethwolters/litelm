...
Tests the SDK-level functions that route A2A requests through litelm.acompletion.
...
async def test_a2a_completion_bridge_non_streaming():
    ...
    response = await asend_message(
        ...
        litelm_params={"custom_llm_provider": "langgraph", "model": "agent"},
    )
...
async def test_vertex_agent_engine_non_streaming():
    """
    Test non-streaming request to Vertex AI Agent Engine via litelm.acompletion.
    ...
    # Call via litelm.acompletion with vertex_ai/agent_engine/ prefix
...
async def test_vertex_agent_engine_streaming():
    """
    Test streaming request to Vertex AI Agent Engine via litelm.acompletion.
    
    Uses the Reasoning Engine resource ID to call a hosted agent with streaming.
    """
    #litelm._turn_on_debug()

    # Call via litelm.acompletion with streaming
    response = await litelm.acompletion(
        model=f"vertex_ai/agent_engine/{VERTEX_AGENT_RESOURCE_NAME}",
        messages=[{"role": "user", "content": "Hello! What can you do?"}],
        stream=True,
