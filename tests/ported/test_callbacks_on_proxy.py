...
async def get_active_callbacks(session):
    ...
    async with session.get(url, headers=headers) as response:
        ...
        all_litelm_callbacks = _json_response["all_litelm_callbacks"]

        print("current number of callbacks: ", _num_callbacks)
        print("current number of alerts: ", _num_alerts)
        return _num_callbacks, _num_alerts, all_litelm_callbacks
...
async def test_check_num_callbacks():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        num_callbacks_1, _, all_litelm_callbacks_1 = await get_active_callbacks(
            session=session
        )
        assert num_callbacks_1 > 0
        await asyncio.sleep(30)

        num_callbacks_2, _, all_litelm_callbacks_2 = await get_active_callbacks(
            session=session
        )

        print("all_litelm_callbacks_1", all_litelm_callbacks_1)

        print(
            "diff in callbacks=",
            set(all_litelm_callbacks_1) - set(all_litelm_callbacks_2),
        )
        ...
        num_callbacks_3, _, all_litelm_callbacks_3 = await get_active_callbacks(
            session=session
        )

        print(
            "diff in callbacks = all_litelm_callbacks3 - all_litelm_callbacks2 ",
            set(all_litelm_callbacks_3) - set(all_litelm_callbacks_2),
        )
...
async def test_check_num_callbacks_on_lowest_latency():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        num_callbacks_1, num_alerts_1, all_litelm_callbacks_1 = (
            await get_active_callbacks(session=session)
        )

        await asyncio.sleep(30)

        num_callbacks_2, num_alerts_2, all_litelm_callbacks_2 = (
            await get_active_callbacks(session=session)
        )

        print(
            "diff in callbacks all_litelm_callbacks_2 - all_litelm_callbacks_1 =",
            set(all_litelm_callbacks_2) - set(all_litelm_callbacks_1),
        )
        ...
        num_callbacks_3, num_alerts_3, all_litelm_callbacks_3 = (
            await get_active_callbacks(session=session)
        )

        print(
            "diff in callbacks all_litelm_callbacks_3 - all_litelm_callbacks_2 =",
            set(all_litelm_callbacks_3) - set(all_litelm_callbacks_2),
        )
