...
from litelm.types.router import (
    AllowedFailsPolicy,
    DeploymentTypedDict,
...
async def test_dynamic_cooldowns():
    """
    Assert kwargs for completion/embedding have 'cooldown_time' as a litelm_param
    """
    # litelm.set_verbose = True
    tmp_mock = MagicMock()

    litelm.failure_callback = [tmp_mock]
    ...
    assert "cooldown_time" in tmp_mock.call_args[0][0]["litelm_params"]
    assert tmp_mock.call_args[0][0]["litelm_params"]["cooldown_time"] == 0
...
def test_single_deployment_no_cooldowns(num_deployments):
    ...
    for i in range(num_deployments):
        model = DeploymentTypedDict(
            model_name="gpt-3.5-turbo",
            litelm_params=LiteLLMParamsTypedDict(
                model="gpt-3.5-turbo",
            ),
        )
...
async def test_router_fallbacks_with_cooldowns_and_dynamic_credentials():
    ...
    """
    from litelm.router_utils.cooldown_handlers import _async_get_cooldown_deployments
