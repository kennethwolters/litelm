...
class MyCustomHandler(CustomLogger):
    ...
    def log_pre_api_call(self, model, messages, kwargs):
        ...
        print(
            f"previous_models: {kwargs['litelm_params']['metadata'].get('previous_models', None)}"
        )
        self.previous_models = len(
            kwargs["litelm_params"]["metadata"].get("previous_models", [])
        )  # {"previous_models": [{"model": litelm_model_name, "exception_type": AuthenticationError, "exception_string": <complete_traceback>}]}
...
async def test_async_fallbacks_max_retries_per_request():
    litelm.set_verbose = False
    litelm.num_retries_per_request = 0
    model_list = [
        {  # list of model deployments
            "model_name": "azure/gpt-3.5-turbo",  # openai model name
            "litelm_params": {  # params for litelm completion/embedding call
        ...
        model_list = [
            {
                ...
                "litelm_params": get_azure_params("chatgpt-v-3"),
                "model_info": {"id": 1},
                "rpm": AZURE_FAST_RPM,
            },
            {
            ...
            },
            {
                ...
                "litelm_params": get_openai_params("gpt-3.5-turbo"),
                "model_info": {"id": 3},
                "rpm": OPENAI_RPM,
            },
            {
                "model_name": "anthropic-claude-3-5-haiku-20241022",
                "litelm_params": get_anthropic_params("claude-3-5-haiku-20241022"),
                "model_info": {"id": 4},
                "rpm": ANTHROPIC_RPM,
            },
        ]
...
@pytest.mark.parametrize("sync_mode", [True, False])
@pytest.mark.parametrize("litelm_module_fallbacks", [True, False])
@pytest.mark.asyncio
async def test_default_model_fallbacks(sync_mode, litelm_module_fallbacks):
    """
    Related issue - https://github.com/BerriAI/litelm/issues/3623

    If model misconfigured, setup a default model for generic fallback
    """
    if litelm_module_fallbacks:
        litelm.default_fallbacks = ["my-good-model"]
    router = Router(
        model_list=[
            {
        ...
        default_fallbacks=(
            ["my-good-model"] if litelm_module_fallbacks is False else None
        ),
    )
...
async def test_router_content_policy_fallbacks(
    ...
    if content_filter_response_exception:
        ...
        mock_response = litelm.ModelResponse(
            choices=[litelm.Choices(finish_reason="content_filter")],
            model="gpt-3.5-turbo",
            usage=litelm.Usage(prompt_tokens=10, completion_tokens=0, total_tokens=10),
        )
    router = Router(
        model_list=[
            {
...
async def test_using_default_fallback(sync_mode):
    ...
    from litelm._logging import verbose_logger, verbose_router_logger

    verbose_logger.setLevel(logging.DEBUG)
    verbose_router_logger.setLevel(logging.DEBUG)
    litelm.default_fallbacks = ["very-bad-model"]
    router = Router(
        model_list=[
            {
                "model_name": "openai/*",
...
async def test_using_default_working_fallback(sync_mode):
    ...
    verbose_logger.setLevel(logging.DEBUG)
    verbose_router_logger.setLevel(logging.DEBUG)
    litelm.default_fallbacks = ["openai/gpt-3.5-turbo"]
    router = Router(
        model_list=[
            {
                "model_name": "openai/*",
...
def test_router_fallbacks_with_custom_model_costs():
    ...
    model_info = litelm.get_model_info(model="claude-sonnet-4-5-20250929")

    print(f"key: {model_info['key']}")

    assert model_info["litelm_provider"] == "anthropic"
...
def test_router_fallbacks_with_model_id():
    router = Router(
        model_list=[
            {
                "model_name": "gpt-3.5-turbo",
                "litelm_params": {"model": "gpt-3.5-turbo", "rpm": 1},
                "model_info": {
                    "id": "123",
                },
            }
...
def test_get_fallback_model_group():
    from litelm.router_utils.fallback_event_handlers import get_fallback_model_group
...
async def test_router_attempted_fallbacks_in_response(expected_attempted_fallbacks):
    ...
    - Test cases: works on first try, `x-litelm-attempted-fallbacks` is 0
    - Works on 1st fallback, `x-litelm-attempted-fallbacks` is 1
    - Works on 3rd fallback, `x-litelm-attempted-fallbacks` is 3
    """
    router = Router(
        model_list=[
            {
    ...
    if expected_attempted_fallbacks == 0:
        ...
        assert (
            resp._hidden_params["additional_headers"]["x-litelm-attempted-fallbacks"]
