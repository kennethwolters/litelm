...
import litelm
from litelm import (  # AuthenticationError,; RateLimitError,; ServiceUnavailableError,; OpenAIError,
    ContextWindowExceededError,
    completion,
    embedding,
)

litelm.vertex_project = "pathrise-convert-1606954137718"
litelm.vertex_location = "us-central1"
litelm.num_retries = 0

# litelm.failure_callback = ["sentry"]
#### What this tests ####
...
async def test_content_policy_exception_azure():
    ...
    except litelm.ContentPolicyViolationError as e:
        print("caught a content policy violation error! Passed")
        print("exception", e)
        assert e.response is not None
        assert e.litelm_debug_info is not None
        assert isinstance(e.litelm_debug_info, str)
        assert len(e.litelm_debug_info) > 0
        pass
...
# for model in litelm.models_by_provider["bedrock"]:
...
def test_anthropic_openai_exception():
    # test if anthropic raises litelm.AuthenticationError
    try:
        litelm.set_verbose = True
        ## Test azure call
...
def test_completion_bedrock_invalid_role_exception():
    """
    Test if litelm raises a BadRequestError for an invalid role on Bedrock
    """
    try:
        litelm.set_verbose = True
        response = completion(
        ...
        #  litelm.BadRequestError: litelm.BadRequestError: Invalid Message passed in {'role': 'very-bad-role', 'content': 'hello'}

        # IMPORTANT ASSERTION
        assert (
            (str(e))
            == "litelm.BadRequestError: Invalid Message passed in {'role': 'very-bad-role', 'content': 'hello'}"
        )
...
def test_litelm_completion_vertex_exception():
    try:
        import litelm
...
def test_litelm_predibase_exception():
    """
...
def test_exception_mapping(provider):
    ...
    litelm.set_verbose = True
    error_map = {
        400: litelm.BadRequestError,
        401: litelm.AuthenticationError,
        404: litelm.NotFoundError,
        408: litelm.Timeout,
        429: litelm.RateLimitError,
        500: litelm.InternalServerError,
        503: litelm.ServiceUnavailableError,
    }
...
def test_fireworks_ai_exception_mapping():
    ...
    Related to: https://github.com/BerriAI/litelm/pull/11455
    Based on Fireworks AI documentation: https://docs.fireworks.ai/tools-sdks/python-client/api-reference
    """
    import litelm
    from litelm.llms.fireworks_ai.common_utils import FireworksAIException
    from litelm.litelm_core_utils.exception_mapping_utils import ExceptionCheckers
    ...
    test_scenarios = [
        {
            ...
            "expected_exception": litelm.RateLimitError,
        },
        {
        ...
        },
        {
            ...
            "expected_exception": litelm.BadRequestError,
        },
    ]
    ...
    for scenario in test_scenarios:
        ...
        try:
            ...
            if scenario["expected_exception"] == litelm.RateLimitError:
...
def test_anthropic_tool_calling_exception():
    """
    Related - https://github.com/BerriAI/litelm/issues/4348
    """
    tools = [
        {
            "type": "function",
...
def _pre_call_utils(
    ...
    if call_type == "embedding":
        ...
        if sync_mode:
            original_function = litelm.embedding
        else:
            original_function = litelm.aembedding
        ...
        if sync_mode:
            original_function = litelm.completion
        else:
            original_function = litelm.acompletion
        ...
        if sync_mode:
            original_function = litelm.text_completion
        else:
            original_function = litelm.atext_completion
...
async def test_exception_with_headers(sync_mode, provider, model, call_type, streaming):
    """
    User feedback: litelm says "No deployments available for selected model, Try again in 60 seconds"
    but Azure says to retry in at most 9s

    ```
    {"message": "litelm.proxy.proxy_server.embeddings(): Exception occured - No deployments available for selected model, Try again in 60 seconds. Passed model=text-embedding-ada-002. pre-call-checks=False, allowed_model_region=n/a, cooldown_list=[('b49cbc9314273db7181fe69b1b19993f04efb88f2c1819947c538bac08097e4c', {'Exception Received': 'litelm.RateLimitError: AzureException RateLimitError - Requests to the Embeddings_Create Operation under Azure OpenAI API version 2023-09-01-preview have exceeded call rate limit of your current OpenAI S0 pricing tier. Please retry after 9 seconds. Please go here: https://aka.ms/oai/quotaincrease if you would like to further increase the default rate limit.', 'Status Code': '429'})]", "level": "ERROR", "timestamp": "2024-08-22T03:25:36.900476"}
    ```
    """
    ...
    if sync_mode:
        if provider == "openai":
            ...
            openai_client = openai.AzureOpenAI(
                api_key="", base_url="", api_version=litelm.AZURE_DEFAULT_API_VERSION
            )
    else:
        if provider == "openai":
    ...
    with patch.object(
        ...
        litelm.utils._get_retry_after_from_exception_header = (
        ...
        try:
            ...
            assert e.litelm_response_headers is not None
            assert int(e.litelm_response_headers["retry-after"]) == cooldown_time
...
def test_openai_gateway_timeout_error():
    ...
    try: 
        with patch.object(
            ...
            litelm.completion(model="openai/gpt-3.5-turbo", messages=[{"role": "user", "content": "Hello world"}], client=openai_client)
...
async def test_exception_with_headers_httpx(
    ...
    with patch.object(
        ...
        try:
            ...
            exception_raised = True
            assert (
                e.litelm_response_headers is not None
            ), "litelm_response_headers is None"
            print("e.litelm_response_headers", e.litelm_response_headers)
            assert int(e.litelm_response_headers["retry-after"]) == cooldown_time
...
async def test_bad_request_error_contains_httpx_response(model):
    ...
    Relevant issue: https://github.com/BerriAI/litelm/issues/6732
    """
    try:
    ...
    except litelm.BadRequestError as e:
...
def test_context_window_exceeded_error_from_litelm_proxy():
    from httpx import Response
    from litelm.litelm_core_utils.exception_mapping_utils import (
        extract_and_raise_litelm_exception,
    )

    args = {
        "response": Response(status_code=400, text="Bad Request"),
        "error_str": "Error code: 400 - {'error': {'message': \"litelm.ContextWindowExceededError: litelm.BadRequestError: this is a mock context window exceeded error\\nmodel=gpt-3.5-turbo. context_window_fallbacks=None. fallbacks=None.\\n\\nSet 'context_window_fallback' - https://docs.litelm.ai/docs/routing#fallbacks\\nReceived Model Group=gpt-3.5-turbo\\nAvailable Model Group Fallbacks=None\", 'type': None, 'param': None, 'code': '400'}}",
        "model": "gpt-3.5-turbo",
        "custom_llm_provider": "litelm_proxy",
    }
    with pytest.raises(litelm.ContextWindowExceededError):
        extract_and_raise_litelm_exception(**args)
...
def test_bad_request_error_with_response_without_request():
    ...
    # Test that extract_and_raise_litelm_exception can handle this
    args = {
        "response": response_without_request,
        "error_str": "Error code: 400 - {'error': {'message': 'litelm.BadRequestError: Invalid request parameters', 'type': None, 'param': None, 'code': '400'}}",
        "model": "gpt-3.5-turbo",
        "custom_llm_provider": "openai",
    }
    
    # This should raise BadRequestError without RuntimeError
    with pytest.raises(litelm.BadRequestError) as exc_info:
        extract_and_raise_litelm_exception(**args)
