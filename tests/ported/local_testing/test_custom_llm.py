...
from litelm.utils import ModelResponseIterator
from litelm.types.utils import ImageResponse, ImageObject, EmbeddingResponse
...
class MyCustomLLM(CustomLLM):
    def completion(
        ...
        client: Optional[litelm.HTTPHandler] = None,
    ) -> ModelResponse:
    ...
    async def acompletion(
        ...
        headers={},
        timeout: Optional[Union[float, openai.Timeout]] = None,
        client: Optional[litelm.AsyncHTTPHandler] = None,
...
def test_get_llm_provider():
    """"""
    from litelm.utils import custom_llm_setup

    my_custom_llm = MyCustomLLM()
    litelm.custom_provider_map = [
...
async def test_simple_image_generation_async():
    ...
    resp = await litelm.aimage_generation(
        model="custom_llm/my-fake-model",
        prompt="Hello world",
    )
...
def test_get_supported_openai_params():
    ...
    from litelm import get_supported_openai_params
...
def test_simple_embedding():
    ...
    resp = litelm.embedding(
        model="custom_llm/my-fake-model",
        input=["good morning from litelm", "good night from litelm"]
    )
