...
import litelm
from litelm import aembedding, completion, embedding
from litelm.caching.caching import Cache
from redis.asyncio import RedisCluster
from litelm.caching.redis_cluster_cache import RedisClusterCache
...
async def test_dual_cache_async_batch_get_cache():
    ...
    """
    from litelm.caching.caching import DualCache, InMemoryCache, RedisCache
...
async def test_batch_get_cache_with_none_keys(sync_mode):
    ...
    """
    from litelm.caching.caching import RedisCache
...
def test_caching_dynamic_args():  # test in memory cache
    try:
        ...
        litelm.cache = None  # disable cache
...
async def test_caching_with_cache_controls(sync_flag):
    try:
        ...
        if sync_flag:
            ...
            ## TTL = 0
            response1 = await litelm.acompletion(
                model="gpt-3.5-turbo",
                messages=messages,
                cache={"ttl": 0},
            ...
            response2 = await litelm.acompletion(
                model="gpt-3.5-turbo",
                messages=messages,
                cache={"s-maxage": 10},
                mock_response="Hello world",
...
def test_caching_with_models_v2():
    messages = [
        {"role": "user", "content": "who is ishaan CTO of litelm from litelm 2023"}
    ]
    litelm.cache = Cache()
...
def c():
    litelm.enable_caching_on_provider_specific_optional_params = True
    messages = [
        {"role": "user", "content": "who is ishaan CTO of litelm from litelm 2023"}
    ]
    litelm.cache = Cache()
    ...
    if (
        ...
        print(f"response2: {response2}")
        pytest.fail(f"Error occurred:")
    litelm.enable_caching_on_provider_specific_optional_params = False
...
def test_embedding_caching_azure():
    ...
    embedding1 = embedding(
        ...
        input=["good morning from litelm", "this is another item"],
        api_key=api_key,
        api_base=api_base,
...
async def test_embedding_caching_redis_ttl():
    ...
    issue: https://github.com/BerriAI/litelm/issues/6010
    """
    litelm.set_verbose = True
    ...
    with patch("redis.asyncio.Redis.pipeline", return_value=mock_pipeline):
        ...
        embedding_val_1 = await litelm.aembedding(
            model="text-embedding-ada-002",
            input=inputs,
            encoding_format="base64",
        )
...
async def test_redis_cache_basic():
    ...
    cache_key = litelm.cache.get_cache_key(
        model="gpt-3.5-turbo",
        messages=messages,
    )
    print(f"cache_key: {cache_key}")
    litelm.cache.add_cache(result=response1, cache_key=cache_key)
    print(f"cache key pre async get: {cache_key}")
    stored_val = await litelm.cache.async_get_cache(
        model="gpt-3.5-turbo",
        messages=messages,
    )
...
async def test_redis_batch_cache_write():
    ...
    response4 = await litelm.acompletion(
        model="gpt-3.5-turbo",
        messages=messages,
    )
...
async def test_redis_cache_cluster_init_unit_test():
    try:
        ...
        resp = litelm.Cache(type="redis", redis_startup_nodes=startup_nodes)
...
async def test_redis_cache_atext_completion():
    try:
        ...
        response1 = await litelm.atext_completion(
        ...
        response2 = await litelm.atext_completion(
            model="gpt-3.5-turbo-instruct", prompt=prompt, max_tokens=40, temperature=1
        )
...
async def test_s3_cache_acompletion_azure():
    ...
    try:
        ...
        litelm.cache = Cache(
            type="s3",
            s3_bucket_name="litelm-my-test-bucket-2",
...
def test_custom_redis_cache_with_key():
    ...
    litelm.cache.get_cache_key = custom_get_cache_key
    ...
    litelm.cache.cache.set_cache = set_cache
    litelm.cache.cache.get_cache = get_cache
...
def test_cache_override():
    # test if we can override the cache, when `caching=False` but litelm.cache = Cache() is set
...
def test_custom_redis_cache_params():
    ...
    try:
        ...
        print(litelm.cache.cache.redis_client)
        litelm.cache = None
        litelm.success_callback = []
        litelm._async_success_callback = []
    except Exception as e:
    ...
    try:
        ...
        cache_key = cache_instance.get_cache_key(
            **{
                ...
                "temperature": 0.2,
                "stream": True,
                "litelm_call_id": "ffe75e7e-8a07-431f-9a74-71a5b9f35f0b",
                "litelm_logging_obj": {},
            }
        )
        ...
        embedding_cache_key_2 = cache_instance.get_cache_key(
            **{
                ...
                "litelm_call_id": "2642e009-b3cd-443d-b5dd-bb7d56123b0e",
                "litelm_logging_obj": "<litelm.utils.Logging object at 0x12f1bddb0>",
            }
        )
...
def test_cache_context_managers():
    ...
    assert litelm.cache == None
    assert "cache" not in litelm.success_callback
    assert "cache" not in litelm._async_success_callback
    ...
    assert litelm.cache != None
    assert litelm.cache.type == "redis"

    print("VARS of litelm.cache", vars(litelm.cache))
...
def test_caching_redis_simple(caplog, capsys):
    """
    Relevant issue - https://github.com/BerriAI/litelm/issues/4511
    """
    litelm.set_verbose = True  ## REQUIRED FOR TEST.
    litelm.cache = Cache(
...
async def test_cache_default_off_acompletion():
    ...
    from litelm.caching.caching import CacheMode
    ...
    response3 = await litelm.acompletion(
        model="gpt-3.5-turbo",
        messages=[
            {
                "role": "user",
...
async def test_redis_sentinel_caching():
    ...
    litelm.cache.add_cache(result=response1, cache_key=cache_key)
    print(f"cache key pre async get: {cache_key}")
    stored_val = litelm.cache.get_cache(
        model="gpt-3.5-turbo",
        messages=messages,
    )
    ...
    stored_val_2 = await litelm.cache.async_get_cache(
        model="gpt-3.5-turbo",
        messages=messages,
    )
...
async def test_redis_proxy_batch_redis_get_cache():
    ...
    from litelm.caching.caching import Cache, DualCache
    from litelm.proxy._types import UserAPIKeyAuth
    from litelm.proxy.hooks.batch_redis_get import _PROXY_BatchRedisRequests
...
def test_basic_caching_import():
    from litelm.caching import Cache
...
async def test_caching_kwargs_input(sync_mode):
    ...
    from litelm.caching.caching_handler import LLMCachingHandler
    ...
    input = {
        ...
        "kwargs": {
            ...
            "caching": True,
            "litelm_call_id": "fae2aa4f-9f75-4f11-8c9c-63ab8d9fae26",
...
async def test_audio_caching(stream):
    litelm.cache = Cache(type="local")

    ## CALL 1 - no cache hit
    completion = await litelm.acompletion(
...
def test_redis_caching_default_ttl():
    ...
    """
    from litelm.caching.redis_cache import RedisCache

    litelm.default_redis_ttl = 120
...
async def test_redis_get_ttl():
    ...
    test that litelm redis caching wrapper handles -1 and -2 values and returns them as None
    """
    try:
        from litelm.caching.redis_cache import RedisCache
...
def test_redis_caching_multiple_namespaces():
    ...
    messages = [{"role": "user", "content": f"what is litelm? {test_uuid}"}]

    # Mock the Redis client creation from the _redis module
    with patch("litelm._redis.get_redis_client") as mock_get_redis_client, patch(
        "litelm._redis.get_redis_connection_pool"
...
def test_caching_with_reasoning_content():
    ...
    try:
        messages = [{"role": "user", "content": f"what is litelm? {uuid.uuid4()}"}]
        litelm.cache = Cache()
...
async def test_cache_key_in_hidden_params_acompletion():
    ...
    Validates fix for missing x-litelm-cache-key header on proxy cache hits.
    """
    litelm.cache = Cache(
        type="redis",
        host=os.environ["REDIS_HOST"],
