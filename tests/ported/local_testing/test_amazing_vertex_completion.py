...
from litelm.llms.vertex_ai.gemini.transformation import (
    _gemini_convert_messages_with_history,
)
from litelm.llms.vertex_ai.vertex_llm_base import VertexBase
...
async def test_get_response():
    ...
    try:
        ...
        pass
    except litelm.UnprocessableEntityError as e:
        pass
...
def test_avertex_ai():
    ...
    load_vertex_ai_credentials()
    test_models = (
        litelm.vertex_chat_models
        | litelm.vertex_code_chat_models
        | litelm.vertex_text_models
        | litelm.vertex_code_text_models
    )
    litelm.set_verbose = False
    vertex_ai_project = "pathrise-convert-1606954137718"

    test_models = random.sample(list(test_models), 1)
    test_models += list(litelm.vertex_language_models)  # always test gemini-pro
    for model in test_models:
        try:
            ...
            assert response.choices[0].finish_reason in litelm._openai_finish_reasons
        except litelm.RateLimitError as e:
            pass
...
async def test_async_vertexai_response_basic():
    ...
    except litelm.NotFoundError as e:
        pass
    except litelm.RateLimitError as e:
        pass
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "us-central1",
        ),  # critical - we had this issue: https://github.com/BerriAI/litelm/issues/13888
        ("vertex_ai/openai/gpt-oss-20b-maas", "us-central1"),
    ],
)
@pytest.mark.parametrize(
...
async def test_gemini_pro_json_schema_args_sent_httpx(
    ...
    with patch.object(client, "post", new=httpx_response) as mock_call:
        ...
        except litelm.JSONSchemaValidationError as e:
...
async def test_anthropic_message_via_anthropic_messages():
    from litelm.llms.custom_httpx.llm_http_handler import AsyncHTTPHandler
    ...
    with patch.object(client, "post", new=httpx_response) as mock_call:
        messages = [{"role": "user", "content": "List 5 cookie recipes"}]
        response = await litelm.anthropic_messages(
    ...
    with patch.object(client, "post", new=httpx_response) as mock_call:
        response_2 = await litelm.acompletion(
            model="vertex_ai/claude-3-5-sonnet@20240620",
            messages=messages,
            max_tokens=100,
            client=client,
...
async def test_gemini_pro_function_calling_streaming(sync_mode):
    ...
    try:
        if sync_mode == True:
            ...
            for chunk in response:
                chunks.append(chunk)
                assert isinstance(chunk, litelm.ModelResponseStream)
        else:
            response = await litelm.acompletion(**data)
            print(f"completion: {response}")

            assert isinstance(response, litelm.CustomStreamWrapper)
        ...
        complete_response = litelm.stream_chunk_builder(chunks=chunks)
        assert (
...
async def test_vertexai_embedding(sync_mode):
    try:
        ...
        input_text = ["good morning from litelm", "this is another item"]
...
async def test_vertexai_multimodal_embedding():
    ...
    with patch(
        "litelm.llms.custom_httpx.http_handler.AsyncHTTPHandler.post",
        return_value=mock_response,
    ) as mock_post:
        # Act: Call the litelm.aembedding function
        response = await litelm.aembedding(
...
async def test_completion_fine_tuned_model():
    ...
    with patch(
        ...
        # Act: Call the litelm.completion function
...
def mock_gemini_list_request(*args, **kwargs):
    from litelm.types.llms.vertex_ai import (
        CachedContent,
...
async def test_gemini_context_caching_disabled_flag(sync_mode):
    ...
    original_flag_value = litelm.disable_anthropic_gemini_context_caching_transform
    
    try:
        # Enable the disable flag
        litelm.disable_anthropic_gemini_context_caching_transform = True
        ...
        litelm.disable_anthropic_gemini_context_caching_transform = original_flag_value
...
def test_gemini_function_call_parameter_in_messages():
    ...
    with patch(
        "litelm.llms.vertex_ai.vertex_llm_base.VertexBase._ensure_access_token",
...
async def test_vertexai_embedding_finetuned(respx_mock: MockRouter):
    ...
    - Response is properly parsed into litelm's embedding response format
    """
    load_vertex_ai_credentials()
    litelm.set_verbose = True
    litelm.disable_aiohttp_transport = (
    ...
    expected_request = {
        "instances": [
            {"inputs": "good morning from litelm"},
...
async def test_vertexai_model_garden_model_completion(
    ...
    """
    Relevant issue: https://github.com/BerriAI/litelm/issues/6480
    ...
    litelm.module_level_aclient = httpx.AsyncClient()
    load_vertex_ai_credentials()
    litelm.set_verbose = True
...
def test_litelm_api_base(monkeypatch, provider, route):
    from litelm.llms.custom_httpx.http_handler import HTTPHandler
    ...
    monkeypatch.setattr(litelm, "api_base", "https://litelm.com")
    ...
    with patch.object(client, "post", new=MagicMock()) as mock_client:
        ...
        mock_client.assert_called()
        assert mock_client.call_args.kwargs["url"].startswith("https://litelm.com")
...
def test_vertex_ai_response_id():
    """Test that litelm preserves the response ID from Vertex AI's API for non-streaming responses"""
    from litelm.llms.custom_httpx.http_handler import HTTPHandler
...
def test_vertex_ai_streaming_response_id():
    """Test that litelm preserves the response ID from Vertex AI's API for streaming responses"""
    from litelm.llms.custom_httpx.http_handler import HTTPHandler
