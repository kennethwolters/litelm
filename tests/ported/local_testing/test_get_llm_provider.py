...
def test_get_llm_provider():
    _, response, _, _ = litelm.get_llm_provider(model="anthropic.claude-v2:1")
...
def test_get_llm_provider_fireworks():  # tests finetuned fireworks models - https://github.com/BerriAI/litelm/issues/4923
    model, custom_llm_provider, _, _ = litelm.get_llm_provider(
...
def test_get_llm_provider_catch_all():
    _, response, _, _ = litelm.get_llm_provider(model="*")
    assert response == "openai"


def test_get_llm_provider_gpt_instruct():
    _, response, _, _ = litelm.get_llm_provider(model="gpt-3.5-turbo-instruct-0914")

    assert response == "text-completion-openai"


def test_get_llm_provider_mistral_custom_api_base():
    model, custom_llm_provider, dynamic_api_key, api_base = litelm.get_llm_provider(
...
def test_default_api_base():
    from litelm.litelm_core_utils.get_llm_provider_logic import (
        _get_openai_compatible_provider_info,
    )

    # Patch environment variable to remove API base if it's set
    with patch.dict(os.environ, {}, clear=True):
        for provider in litelm.openai_compatible_providers:
            ...
            for other_provider in litelm.provider_list:
...
# -------- Tests for force_use_litelm_proxy ---------

def test_get_litelm_proxy_custom_llm_provider():
    """
    Tests force_use_litelm_proxy uses LITELLM_PROXY_API_BASE and LITELLM_PROXY_API_KEY from env.
    """
    ...
    with patch.dict(os.environ, {
        ...
        model, provider, key, base = litelm.LiteLLMProxyChatConfig().litelm_proxy_get_custom_llm_provider_info(model=test_model)

    assert model == test_model
    assert provider == "litelm_proxy"
    assert key == expected_api_key
    assert base == expected_api_base

def test_get_litelm_proxy_with_args_override_env_vars():
    """
    Tests force_use_litelm_proxy uses api_base and api_key args over environment variables.
    """
    test_model = "gpt-4"
    ...
    with patch.dict(os.environ, {
        ...
        model, provider, key, base = litelm.LiteLLMProxyChatConfig().litelm_proxy_get_custom_llm_provider_info(
            model=test_model,
...
def test_get_litelm_proxy_model_prefix_stripping():
    """
    Tests force_use_litelm_proxy strips 'litelm_proxy/' prefix from model name.
    """
    original_model = "litelm_proxy/claude-2"
    ...
    with patch.dict(os.environ, {
        ...
        model, provider, key, base = litelm.LiteLLMProxyChatConfig().litelm_proxy_get_custom_llm_provider_info(model=original_model)
...
# -------- Tests for get_llm_provider triggering use_litelm_proxy ---------

def test_get_llm_provider_LITELLM_PROXY_ALWAYS_true():
    """
    Tests get_llm_provider uses litelm_proxy when USE_LITELLM_PROXY is "True".
    """
    ...
    with patch.dict(os.environ, {
        ...
        model, provider, key, base = litelm.get_llm_provider(model=test_model_input)
...
def test_get_llm_provider_LITELLM_PROXY_ALWAYS_true_model_prefix():
    """
    Tests get_llm_provider with USE_LITELLM_PROXY="True" and model prefix "litelm_proxy/".
    """
    test_model_input = "litelm_proxy/gpt-4-turbo"
...
def test_get_llm_provider_use_proxy_arg_true():
    """
    Tests get_llm_provider uses litelm_proxy when use_proxy=True argument is passed.
    """
    test_model_input = "mistral/mistral-large"
    expected_model_output = "mistral/mistral-large" # force_use_litelm_proxy keep the model name
    ...
    with patch.dict(os.environ, {
        ...
        model, provider, key, base = litelm.get_llm_provider(
            model=test_model_input, 
            litelm_params=LiteLLM_Params(use_litelm_proxy=True, model=test_model_input)
        )
...
def test_get_llm_provider_use_proxy_arg_true_with_direct_args():
    ...
    These args should be passed to force_use_litelm_proxy and override env vars.
    """
