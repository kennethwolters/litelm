...
from litelm.types.guardrails import GuardrailItem
...
from litelm.proxy.guardrails.guardrail_hooks.lakera_ai import lakeraAI_Moderation
from litelm.proxy.proxy_server import embeddings
...
async def test_moderations_on_embeddings():
    try:
        ...
        setattr(litelm.proxy.proxy_server, "llm_router", temp_router)

        api_route = APIRoute(path="/embeddings", endpoint=embeddings)
        litelm.callbacks = [lakeraAI_Moderation()]
        request = Request(
            {
                "type": "http",
                "route": api_route,
...
@pytest.mark.asyncio
@patch("litelm.llms.custom_httpx.http_handler.AsyncHTTPHandler.post")
@patch(
    "litelm.guardrail_name_config_map",
    new=make_config_map(
        {
...
@patch("litelm.add_function_to_prompt", False)
@pytest.mark.skip(reason="lakera deprecated their v1 endpoint.")
...
async def test_callback_specific_param_run_pre_call_check_lakera():
    ...
    import litelm
    from litelm.proxy.guardrails.guardrail_hooks.lakera_ai import lakeraAI_Moderation
    from litelm.proxy.guardrails.init_guardrails import initialize_guardrails
    from litelm.types.guardrails import GuardrailItem, GuardrailItemSpec
    ...
    litelm_settings = {"guardrails": guardrails_config}

    assert len(litelm.guardrail_name_config_map) == 0
    initialize_guardrails(
        guardrails_config=guardrails_config,
        premium_user=True,
        config_file_path="",
        litelm_settings=litelm_settings,
    )

    assert len(litelm.guardrail_name_config_map) == 1

    prompt_injection_obj: Optional[lakeraAI_Moderation] = None
    print("litelm callbacks={}".format(litelm.callbacks))
