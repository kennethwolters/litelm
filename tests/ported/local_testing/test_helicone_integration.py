...
litelm.success_callback = ["helicone"]
...
def test_helicone_removes_otel_span_from_metadata():
    """
    Test that HeliconeLogger removes litelm_parent_otel_span from metadata
    to prevent JSON serialization errors.
    """
    from litelm.integrations.helicone import HeliconeLogger
    ...
    metadata = {
        ...
        "litelm_parent_otel_span": mock_span,  # This would cause JSON serialization error
    ...
    litelm_params = {"proxy_server_request": {"headers": {}}}
    result_metadata = logger.add_metadata_from_header(litelm_params, metadata)
    
    # Verify that litelm_parent_otel_span was removed
    assert "litelm_parent_otel_span" not in result_metadata
    ...
    print("✅ Test passed: litelm_parent_otel_span was successfully removed from metadata")
