...
def test_response_model_none():
    """
    Addresses:https://github.com/BerriAI/litelm/issues/2972
    """
    x = completion(
        model="mymodel",
    ...
    print(f"x: {x}")
    assert isinstance(x, litelm.ModelResponse)
...
def _openai_mock_response(*args, **kwargs) -> litelm.ModelResponse:
...
async def test_completion_predibase():
    try:
        ...
        pass
    except litelm.ServiceUnavailableError as e:
        pass
    except litelm.InternalServerError:
        pass
    except Exception as e:
        pytest.fail(f"Error occurred: {e}")
...
def test_completion_claude_3_empty_response():
    ...
    try:
        response = litelm.completion(model="claude-sonnet-4-5-20250929", messages=messages)
        print(response)
    except litelm.InternalServerError as e:
...
async def test_model_function_invoke(model, sync_mode, api_key, api_base):
    try:
        ...
        if sync_mode:
            response = litelm.completion(**data)
        else:
            response = await litelm.acompletion(**data)
...
async def test_anthropic_no_content_error():
    """
    https://github.com/BerriAI/litelm/discussions/3440#discussioncomment-9323402
    """
    try:
...
def test_parse_xml_params():
    from litelm.litelm_core_utils.prompt_templates.factory import parse_xml_params
...
def test_completion_openai_response_headers():
    ...
    streaming_response = litelm.completion(
        model="gpt-4o-mini",
        messages=messages,
        stream=True,
    )
    ...
    # embedding
    embedding_response = litelm.embedding(
    ...
    litelm.return_response_headers = False
...
async def test_async_completion_openai_response_headers():
    ...
    streaming_response = await litelm.acompletion(
        model="gpt-4o-mini",
        messages=messages,
        stream=True,
    )
    ...
    # embedding
    embedding_response = await litelm.aembedding(
        model="text-embedding-ada-002",
        input="hello",
    )
...
def test_lm_studio_completion(monkeypatch):
    ...
    except litelm.AuthenticationError as e:
        pytest.fail(f"Error occurred: {e}")
    except litelm.APIError as e:
        print(e)
...
# this should throw an exception, to trigger https://logs.litelm.ai/
...
def test_completion_openai():
    try:
        ...
        litelm.api_key = os.environ["OPENAI_API_KEY"]
        response = completion(
            model="gpt-3.5-turbo",
...
def test_completion_openai_pydantic(model, api_version):
    try:
        ...
        litelm.enable_json_schema_validation = True
        for _ in range(3):
            try:
                response = completion(
                    model=model,
                ...
                break
            except litelm.JSONSchemaValidationError:
...
def test_completion_openai_with_optional_params():
    ...
    try:
        litelm.set_verbose = True
        litelm.success_callback = [custom_callback]
        response = completion(
        ...
        print(response)
        litelm.success_callback = []  # unset callbacks
...
def test_completion_logprobs():
    """
    This function is used to test the litelm.completion logprobs functionality.
...
def test_completion_openai_litelm_key():
    try:
        litelm.set_verbose = True
        litelm.num_retries = 0
        litelm.api_key = os.environ["OPENAI_API_KEY"]
        ...
        ###### reset environ key
        os.environ["OPENAI_API_KEY"] = litelm.api_key
...
def test_completion_ollama_function_call(model):
    ...
    try:
        litelm.set_verbose = True
        response = litelm.completion(model=model, messages=messages, tools=tools)
        print(response)
...
async def test_completion_functions_param():
    ...
    try:
        ...
        with patch.object(client, "post", side_effect=gemini_mock_post) as mock_client:
            response: litelm.ModelResponse = await litelm.acompletion(
...
def test_completion_azure_extra_headers():
    ...
    from litelm.llms.custom_httpx.httpx_handler import HTTPHandler

    http_client = Client()

    with patch.object(http_client, "send", new=MagicMock()) as mock_client:
        litelm.client_session = http_client
        try:
            response = completion(
...
def test_azure_openai_ad_token():
    ...
    # we use litelm.input_callbacks for this test
    def tester(
        kwargs,  # kwargs to completion
    ):
        print("inside kwargs")
...
def test_completion_azure3():
    ...
    try:
        print("azure gpt-3.5 test\n\n")
        litelm.set_verbose = True
        litelm.api_base = os.environ["AZURE_API_BASE"]
        litelm.api_key = os.environ["AZURE_API_KEY"]
        litelm.api_version = os.environ["AZURE_API_VERSION"]
        ...
        os.environ["AZURE_API_BASE"] = litelm.api_base
        os.environ["AZURE_API_VERSION"] = litelm.api_version
        os.environ["AZURE_API_KEY"] = litelm.api_key
...
# new azure test for using litelm. vars,
# use the following vars in this test and make an azure_api_call
#  litelm.api_type = self.azure_api_type
#  litelm.api_base = self.azure_api_base
#  litelm.api_version = self.azure_api_version
#  litelm.api_key = self.api_key
def test_completion_azure_with_litelm_key():
    try:
        print("azure gpt-3.5 test\n\n")
        import openai

        #### set litelm vars
        litelm.api_type = "azure"
        litelm.api_base = os.environ["AZURE_API_BASE"]
        litelm.api_version = os.environ["AZURE_API_VERSION"]
        litelm.api_key = os.environ["AZURE_API_KEY"]
        ...
        ######### UNSET litelm vars
        litelm.api_type = None
        litelm.api_base = None
        litelm.api_version = None
        litelm.api_key = None
...
async def test_completion_replicate_llama3(sync_mode):
    ...
    try:
        ...
        # Add any assertions here to check the response
        assert isinstance(response, litelm.ModelResponse)
...
def test_replicate_custom_prompt_dict():
    ...
    litelm.register_prompt_template(
    ...
    try:
        ...
        pass
    except litelm.APIConnectionError as e:
        pass
    except Exception as e:
        pytest.fail(f"An exception occurred - {str(e)}")
    print(f"response: {response}")
    litelm.custom_prompt_dict = {}  # reset
...
def test_bedrock_deepseek_custom_prompt_dict():
    ...
    assert model in litelm.known_tokenizer_config
...
def test_customprompt_together_ai():
    try:
        ...
        print(litelm.success_callback)
        print(litelm._async_success_callback)
        response = completion(
            model="together_ai/mistralai/Mixtral-8x7B-Instruct-v0.1",
            messages=messages,
            roles={
        ...
        print(response)
    except litelm.exceptions.Timeout as e:
...
def response_format_tests(response: litelm.ModelResponse):
    ...
    assert isinstance(choice, litelm.Choices)
    assert isinstance(choice.get("index"), int)

    message = choice.get("message")
    assert isinstance(message, litelm.Message)
    ...
    assert isinstance(response.usage, litelm.Usage)  # type: ignore
...
#     model_name = "qvv0xeq"
#     litelm.api_base = "https://app.baseten.co"
#     try:
...
async def test_acompletion_gemini():
    ...
    messages = [{"role": "user", "content": "Hey, how's it going?"}]
    try:
        response = await litelm.acompletion(model=model_name, messages=messages)
        # Add any assertions here to check the response
        print(f"response: {response}")
    except litelm.Timeout as e:
        pass
...
def test_unified_auth_params(provider, model, project, region_name, token):
    ...
    translated_optional_params = litelm.utils.get_optional_params(**data)

    if provider == "azure":
        special_auth_params = (
            litelm.AzureOpenAIConfig().get_mapped_special_auth_params()
        )
    elif provider == "bedrock":
        special_auth_params = (
            litelm.AmazonBedrockGlobalConfig().get_mapped_special_auth_params()
        )
    elif provider == "vertex_ai":
        special_auth_params = litelm.VertexAIConfig().get_mapped_special_auth_params()
    elif provider == "watsonx":
        special_auth_params = (
            litelm.IBMWatsonXAIConfig().get_mapped_special_auth_params()
        )
...
#     try:
#         litelm.config_path = "../config.json"
#         litelm.set_verbose = True
#         response = litelm.config_completion(messages=messages)
#         # Add any assertions here to check the response
#         print(response)
#         litelm.config_path = None
#     except Exception as e:
#         pytest.fail(f"Error occurred: {e}")
...
def test_completion_together_ai_stream():
    litelm.set_verbose = True
    user_message = "Write 1pg about YC & litelm"
    messages = [{"content": user_message, "role": "user"}]
    try:
        response = completion(
            model="together_ai/mistralai/Mixtral-8x7B-Instruct-v0.1",
...
def test_moderation():
    response = litelm.moderation(input="i'm ishaan cto of litelm")
    print(response)
...
def test_openai_hallucinated_tool_call_util(function_name, expect_modification):
    ...
    2. Iterate over tool_uses array to call functions:
        ...
        - parameters will be JSON object for function arguments
    """
    from litelm.utils import _handle_invalid_parallel_tool_calls
    from litelm.types.utils import ChatCompletionMessageToolCall
...
def test_completion_openai_metadata(monkeypatch, enable_preview_features):
    ...
    monkeypatch.setattr(litelm, "enable_preview_features", enable_preview_features)
    with patch.object(
