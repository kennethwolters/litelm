...
from litelm.router_utils.fallback_event_handlers import (
    run_async_fallback,
...
async def test_run_async_fallback(original_function):
    ...
    original_exception = litelm.exceptions.InternalServerError(
    ...
    if original_function == router._acompletion:
        assert isinstance(result, litelm.ModelResponse)
    elif original_function == router._atext_completion:
        assert isinstance(result, litelm.TextCompletionResponse)
    elif original_function == router._aembedding:
        assert isinstance(result, litelm.EmbeddingResponse)
...
async def test_failed_fallbacks_raise_most_recent_exception(original_function):
    ...
    with pytest.raises(litelm.exceptions.RateLimitError):
