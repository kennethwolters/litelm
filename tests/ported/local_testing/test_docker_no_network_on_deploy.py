...
class TestDockerNoNetworkOnDeploy:
    ...
    TEST_IMAGE_NAME = "litelm-no-network-test"
    TEST_CONTAINER_NAME = "litelm-no-network-test-container"
    ...
    def _build_test_image(self) -> bool:
        ...
        """
        # Check if main litelm image exists
        result = subprocess.run(
            ["docker", "images", "-q", "litelm/litelm"],
            capture_output=True,
            text=True,
            timeout=10,
        )
...
"""
        ...
        config_path = "/tmp/litelm_test_config.yaml"
        with open(config_path, "w") as f:
            f.write(minimal_config)

        image_to_use = "litelm/litelm"
