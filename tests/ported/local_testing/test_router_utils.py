...
def test_update_kwargs_before_fallbacks_unit_test():
    ...
    assert kwargs["litelm_trace_id"] is not None
...
async def test_update_kwargs_before_fallbacks(call_type):
    ...
    if call_type.value.startswith("a"):
        with patch.object(router, "async_function_with_fallbacks") as mock_client:
            ...
            assert mock_client.call_args.kwargs["litelm_trace_id"] is not None
...
async def test_router_model_group_headers():
    os.environ["LITELLM_LOCAL_MODEL_COST_MAP"] = "True"
    litelm.model_cost = litelm.get_model_cost_map(url="")
    from litelm.types.utils import OPENAI_RESPONSE_HEADERS
    ...
    assert (
        resp._hidden_params["additional_headers"]["x-litelm-model-group"]
...
def test_router_redis_cache():
    router = Router(
        model_list=[{"model_name": "gemini/*", "litelm_params": {"model": "gemini/*"}}]
    )
...
def test_router_handle_clientside_credential():
    ...
    assert new_deployment.litelm_params.api_key == "123"
...
def test_router_get_deployment_credentials():
    router = Router(
        model_list=[
            {
                "model_name": "gemini/*",
                "litelm_params": {"model": "gemini/*", "api_key": "123"},
                "model_info": {"id": "1"},
            }
        ]
    )
