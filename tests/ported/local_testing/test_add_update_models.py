...
from litelm.router import (
    Deployment,
    LiteLLM_Params,
)
from litelm.types.router import ModelInfo, updateDeployment, updateLiteLLMParams

from litelm.proxy._types import UserAPIKeyAuth, NewTeamRequest, LiteLLM_TeamTable
...
async def test_add_new_model(prisma_client):
    ...
    setattr(litelm.proxy.proxy_server, "store_model_in_db", True)
    ...
    _new_models = await prisma_client.db.litelm_proxymodeltable.find_many()
...
async def test_add_update_model(prisma_client):
    # test that existing litelm_params are not updated
    ...
    _original_litelm_params = _new_model_in_db.litelm_params
    print("_original_litelm_params: ", _original_litelm_params)
    print("now updating the tpm for model")
    # run update to update "tpm"
    await update_model(
        model_params=updateDeployment(
            litelm_params=updateLiteLLMParams(tpm=123456),
            model_info=ModelInfo(
                id=_new_model_id,
            ),
        ),
    ...
    # assert all other litelm params are identical to _original_litelm_params
    for key, value in _original_litelm_params.items():
        if key == "tpm":
            # assert that tpm actually got updated
            assert _new_model_in_db.litelm_params[key] == 123456
        else:
            assert _new_model_in_db.litelm_params[key] == value
...
async def test_add_team_model_to_db(prisma_client):
    ...
    team = await prisma_client.db.litelm_teamtable.find_first(
        where={
            "team_id": team_id,
        },
        include={"litelm_model_table": True},
    )
    ...
    litelm_model_table = team.litelm_model_table
    print("litelm_model_table=", litelm_model_table.model_dump_json())
    model_aliases = litelm_model_table.model_aliases
