...
async def test_router_retry_policy(error_type):
    from litelm.router import AllowedFailsPolicy, RetryPolicy
...
def test_timeout_for_rate_limit_error_with_healthy_deployments(
    ...
    rate_limit_error = litelm.RateLimitError(
        ...
        response=httpx.Response(
            ...
            request=httpx.Request(method="tpm_rpm_limits", url="https://github.com/BerriAI/litelm"),  # type: ignore
        ),
    )
    model_list = [
        {
...
def test_no_retry_for_unprocessable_entity_error_422():
    ...
    # Act & Assert
    error = litelm.UnprocessableEntityError(
...
internal_server_error = litelm.InternalServerError(
...
service_unavailable_error = litelm.ServiceUnavailableError(
    message="service unavailable error",
    model="gpt-12",
    llm_provider="azure",
)

timeout_error = litelm.Timeout(
