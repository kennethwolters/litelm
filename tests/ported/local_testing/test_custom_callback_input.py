...
import litelm
from litelm import Cache, completion, embedding
from litelm.integrations.custom_logger import CustomLogger
from litelm.types.utils import LiteLLMCommonStrings
...
# Test interfaces
## 1. litelm.completion() + litelm.embeddings()
## refer to test_custom_callback_input_router.py for the router +  proxy tests


class CompletionCustomHandler(
    CustomLogger
):  # https://docs.litelm.ai/docs/observability/custom_callback#callback-class
    """
    ...
    def log_pre_api_call(self, model, messages, kwargs):
        try:
            ...
            assert isinstance(kwargs["litelm_params"], dict)
            assert isinstance(kwargs["start_time"], (datetime, type(None)))
            assert isinstance(kwargs["stream"], bool)
            assert isinstance(kwargs["user"], (str, type(None)))
            ### METADATA
            metadata_value = kwargs["litelm_params"].get("metadata")
            assert metadata_value is None or isinstance(metadata_value, dict)
            if metadata_value is not None:
                if litelm.turn_off_message_logging is True:
                    assert (
                        metadata_value["raw_request"]
                        is LiteLLMCommonStrings.redacted_by_litelm.value
                    )
                else:
        ...
        try:
            ...
            assert (
                isinstance(
                    ...
                    (str, litelm.CustomStreamWrapper, BaseModel),
                )
    ...
    async def async_log_stream_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            ## RESPONSE OBJECT
            assert isinstance(response_obj, litelm.ModelResponseStream)
            ## KWARGS
            assert isinstance(kwargs["model"], str)
            ...
            assert (
                isinstance(
                    kwargs["original_response"], (str, litelm.CustomStreamWrapper)
                )
                or inspect.isasyncgen(kwargs["original_response"])
                or inspect.iscoroutine(kwargs["original_response"])
            )
        ...
        try:
            ...
            assert isinstance(
                ...
                (
                    ...
                    litelm.EmbeddingResponse,
                    litelm.ImageResponse,
                ),
            )
            ## KWARGS
            assert isinstance(kwargs["model"], str)
            ...
            assert isinstance(kwargs["optional_params"], dict)
            assert isinstance(kwargs["litelm_params"], dict)
            assert isinstance(kwargs["litelm_params"]["api_base"], str)
            ...
            assert isinstance(
                kwargs["original_response"],
                (str, litelm.CustomStreamWrapper, BaseModel),
            ), "Original Response={}. Allowed types=[str, litelm.CustomStreamWrapper, BaseModel]".format(
                kwargs["original_response"]
            )
            assert isinstance(kwargs["additional_args"], (dict, type(None)))
            assert isinstance(kwargs["log_event_type"], str)
    ...
    def log_failure_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            assert isinstance(kwargs["optional_params"], dict)
            assert isinstance(kwargs["litelm_params"], dict)
            assert isinstance(kwargs["litelm_params"]["metadata"], Optional[dict])
            assert isinstance(kwargs["start_time"], (datetime, type(None)))
            assert isinstance(kwargs["stream"], bool)
            assert isinstance(kwargs["user"], (str, type(None)))
            assert (
...
async def test_async_text_completion_bedrock():
    try:
        customHandler = CompletionCustomHandler()
        litelm.callbacks = [customHandler]
        response = await litelm.atext_completion(
...
async def test_async_embedding_openai():
    try:
        ...
        litelm.callbacks = [customHandler_success]
        ...
        litelm.callbacks = [customHandler_failure]
        try:
            response = await litelm.aembedding(
                model="text-embedding-ada-002",
                input=["good morning from litelm"],
...
def test_amazing_sync_embedding():
    try:
        ...
        response = litelm.embedding(
            model="azure/text-embedding-ada-002", input=["good morning from litelm"]
        )
        print(f"customHandler_success.errors: {customHandler_success.errors}")
        print(f"customHandler_success.states: {customHandler_success.states}")
        time.sleep(2)
...
def test_image_generation_openai():
    try:
        ...
        pass
    except litelm.ContentPolicyViolationError:
...
def test_standard_logging_payload(model, turn_off_message_logging):
    ...
    litelm.turn_off_message_logging = turn_off_message_logging
    ...
    with patch.object(
        ...
        if turn_off_message_logging:
            print("checks redacted-by-litelm")
            assert "redacted-by-litelm" == slobject["messages"][0]["content"]
            ...
            if "choices" in response:
                ...
                assert response["text"] == "redacted-by-litelm"
...
def test_logging_key_masking_gemini():
    ...
    with patch.object(
        ...
        assert (
            ...
            not in mock_client.call_args.kwargs["kwargs"]["litelm_params"]["api_base"]
        )
        key = mock_client.call_args.kwargs["kwargs"]["litelm_params"]["api_base"]
...
def test_litelm_logging_no_log_param(monkeypatch, disable_no_log_param):
    monkeypatch.setattr(litelm, "global_disable_no_log_param", disable_no_log_param)
    ...
    should_run = litelm_logging_obj.should_run_callback(
        callback="langfuse",
        litelm_params={"no-log": True},
