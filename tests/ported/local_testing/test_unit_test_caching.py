...
from litelm.litelm_core_utils.model_param_helper import ModelParamHelper
...
def test_preset_cache_key():
    """
    Test that the preset cache key is used if it is set in kwargs["litelm_params"]
    """
    cache = Cache()
    kwargs = {
        "model": "gpt-3.5-turbo",
        "messages": [{"role": "user", "content": "Hello, world!"}],
        "temperature": 0.7,
        "litelm_params": {"preset_cache_key": "preset-cache-key"},
    }
