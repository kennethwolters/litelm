...
import litelm
from litelm.proxy.common_utils.encrypt_decrypt_utils import encrypt_value
from litelm.proxy.proxy_server import ProxyConfig
from litelm.proxy.utils import DualCache, ProxyLogging
...
async def test_delete_deployment():
    ...
    encrypted_litelm_params = litelm_params.dict(exclude_none=True)

    master_key = "sk-1234"

    setattr(litelm.proxy.proxy_server, "master_key", master_key)

    for k, v in encrypted_litelm_params.items():
        if isinstance(v, str):
            encrypted_value = encrypt_value(v, master_key)
            encrypted_litelm_params[k] = base64.b64encode(encrypted_value).decode(
                "utf-8"
            )

    deployment = Deployment(model_name="gpt-3.5-turbo", litelm_params=litelm_params)
    deployment_2 = Deployment(
        model_name="gpt-3.5-turbo-2", litelm_params=litelm_params
    )
    ...
    db_model = DBModel(
        ...
        litelm_params=encrypted_litelm_params,
...
async def test_db_error_new_model_check():
    ...
    Relevant issue: https://github.com/BerriAI/litelm/blob/ddfe687b13e9f31db2fb2322887804e3d01dd467/litelm/proxy/proxy_server.py#L2461
    """
    import base64
...
deployment = Deployment(model_name="gpt-3.5-turbo", litelm_params=litelm_params)
deployment_2 = Deployment(model_name="gpt-3.5-turbo-2", litelm_params=litelm_params)
...
def _create_model_list(flag_value: Literal[0, 1], master_key: str):
    ...
    new_litelm_params = LiteLLM_Params(
    ...
    encrypted_litelm_params = new_litelm_params.dict(exclude_none=True)
...
async def test_add_and_delete_deployments(llm_router, model_list_flag_value):
    ...
    llm_router = getattr(litelm.proxy.proxy_server, "llm_router")
...
from litelm import LITELLM_CHAT_PROVIDERS, LlmProviders
from litelm.utils import ProviderConfigManager
from litelm.llms.base_llm.chat.transformation import BaseConfig
...
def _check_provider_config(config: BaseConfig, provider: LlmProviders):
    ...
    if (
        provider != litelm.LlmProviders.OPENAI
        and provider != litelm.LlmProviders.OPENAI_LIKE
        and provider != litelm.LlmProviders.CUSTOM_OPENAI
    ):
        assert (
...
#     from litelm.llms.openai.chat.gpt_transformation import OpenAIGPTConfig
...
def test_litelm_proxy_responses_api_config():
    """Test that litelm_proxy provider returns correct Responses API config"""
    from litelm.llms.litelm_proxy.responses.transformation import (
        LiteLLMProxyResponsesAPIConfig,
    )

    config = ProviderConfigManager.get_provider_responses_api_config(
        model="litelm_proxy/gpt-4",
        provider=LlmProviders.LITELLM_PROXY,
    )
    print(f"config: {config}")
    assert config is not None, "Config should not be None for litelm_proxy provider"
    assert isinstance(
