...
import pytest
import litelm
from litelm.proxy.auth.auth_utils import (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"api_base": "^https://litelm.*direct\.fireworks\.ai/v1$"},
            {"api_base": "https://litelm-dev.direct.fireworks.ai/v1"},
            True,
        ),
        (
            {"api_base": "^https://litelm.*novice\.fireworks\.ai/v1$"},
            {"api_base": "https://litelm-dev.direct.fireworks.ai/v1"},
            False,
        ),
    ],
...
def test_get_end_user_id_from_request_body_always_returns_str():
    from litelm.proxy.auth.auth_utils import get_end_user_id_from_request_body
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Test 9: Multiple fallback sources - litelm_metadata
        (
            {},
            {"user_header_name": "X-User-ID"},
            {"litelm_metadata": {"user": "litelm-user-789"}},
            "litelm-user-789"
        ),
        ...
        (
            ...
            {
                "user": "body-user",
                "litelm_metadata": {"user": "litelm-user"},
...
def test_get_end_user_id_from_request_body_with_user_header_name(
    ...
    with patch('litelm.proxy.proxy_server.general_settings', general_settings_config):
        end_user_id = get_end_user_id_from_request_body(request_body, dict(mock_request.headers))
...
def test_get_end_user_id_from_request_body_backwards_compatibility():
    ...
    # Test with litelm_metadata
    request_body = {"litelm_metadata": {"user": "litelm-user-456"}}
    end_user_id = get_end_user_id_from_request_body(request_body)
    assert end_user_id == "litelm-user-456"
...
def test_get_model_from_request(request_data, expected_model):
    from litelm.proxy.auth.auth_utils import get_model_from_request
...
def test_get_customer_user_header_from_mapping_returns_customer_header():
    from litelm.proxy.auth.auth_utils import get_customer_user_header_from_mapping
...
def test_get_customer_user_header_from_mapping_no_customer_returns_none():
    ...
    single_mapping = {"header_name": "X-Only-Internal", "litelm_user_role": "internal_user"}
...
def test_get_internal_user_header_from_mapping_no_internal_returns_none():
    ...
    single_mapping = {"header_name": "X-Only-Customer", "litelm_user_role": "customer"}
