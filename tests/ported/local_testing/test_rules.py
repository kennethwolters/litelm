...
def test_pre_call_rule():
    try:
        litelm.pre_call_rules = [my_pre_call_rule]
        ### completion
        response = completion(
            model="gpt-3.5-turbo",
...
def test_post_call_rule():
    try:
        litelm.pre_call_rules = []
        litelm.post_call_rules = [my_post_call_rule]
        ### completion
        response = completion(
            model="gpt-3.5-turbo",
...
def test_post_call_rule_streaming():
    try:
        litelm.pre_call_rules = []
        litelm.post_call_rules = [my_post_call_rule_2]
        ### completion
        response = completion(
            model="gpt-3.5-turbo",
            messages=[{"role": "user", "content": "say sorry"}],
