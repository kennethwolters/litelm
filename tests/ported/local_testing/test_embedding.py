...
import litelm
from litelm import completion, completion_cost, embedding
...
def test_openai_embedding():
    try:
        ...
        litelm_response = dict(response)
        litelm_response_keys = set(litelm_response.keys())
        litelm_response_keys.discard("_response_ms")

        print(litelm_response_keys)
        print("LiteLLM Response\n")
        # print(litelm_response)
        ...
        assert (
            litelm_response_keys == openai_response_keys
        )  # ENSURE the Keys in litelm response is exactly what the openai package returns
        assert (
            len(litelm_response["data"]) == 2
        )  # expect two embedding responses from litelm_response since input had two
        print(openai_response_keys)
...
async def test_together_ai_embedding(model, api_base, api_key, sync_mode):
    try:
        ...
        assert set(["usage", "model", "object", "data"]) == set(
            response_keys
        )  # assert litelm response has expected keys from OpenAI embedding response

        request_cost = litelm.completion_cost(
            completion_response=response, call_type="embedding"
        )

        print("Calculated request cost=", request_cost)

        assert isinstance(response.usage, litelm.Usage)
...
async def test_azure_ai_embedding_image(model, api_base, api_key, sync_mode):
    try:
        ...
        request_cost = litelm.completion_cost(completion_response=response)
...
async def test_cohere_embedding(sync_mode, model, api_base):
    try:
        ...
        if sync_mode:
            ...
            response = await litelm.aembedding(**data)
...
async def test_bedrock_embedding_titan(model, sync_mode):
    try:
        ...
        if sync_mode:
            response = embedding(
                model=model,
                input=f"good morning from litelm, attempting to embed data {current_time}",  # input should always be a string in this test
...
def test_bedrock_embedding_cohere():
    try:
        ...
        response = embedding(
            ...
            input=[
                "good morning from litelm, attempting to embed data",
...
def test_demo_tokens_as_input_to_embeddings_fails_for_titan():
    ...
    with pytest.raises(
        litelm.BadRequestError,
        match='litelm.BadRequestError: BedrockException - {"message":"Malformed input request: expected type: String, found: JSONArray, please reformat your input and try again."}',
    ):
        litelm.embedding(model="amazon.titan-embed-text-v1", input=[[1]])

    with pytest.raises(
        litelm.BadRequestError,
        match='litelm.BadRequestError: BedrockException - {"message":"Malformed input request: expected type: String, found: Integer, please reformat your input and try again."}',
    ):
        litelm.embedding(
...
def tgi_mock_post(*args, **kwargs):
    ...
    expected_data = {
        "inputs": {
            "source_sentence": "good morning from litelm",
...
@pytest.mark.asyncio
@patch(
    "litelm.llms.huggingface.embedding.handler.async_get_hf_task_embedding_for_model"
)
@patch("litelm.llms.huggingface.embedding.handler.get_hf_task_embedding_for_model")
@pytest.mark.parametrize("sync_mode", [True, False])
...
@pytest.mark.parametrize("sync_mode", [True, False])
@pytest.mark.parametrize(
    "input", ["good morning from litelm", ["good morning from litelm"]]  #
)
@pytest.mark.asyncio
...
def test_embedding_response_ratelimit_headers(model):
    ...
    # if upstream api returns this header, we need to verify the header is transformed by litelm
    if (
...
def test_jina_ai_img_embeddings(input_data, expected_payload_input):
    ...
    This test verifies that when litelm.embedding is called with a jina_ai model,
    ...
    with patch("litelm.llms.custom_httpx.http_handler.HTTPHandler.post") as mock_post:
        # Configure the mock to return a successful, minimal valid response.
        # This prevents litelm from raising an error when processing the response.
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "object": "list",
        ...
        try:
            ...
            pytest.fail(
                f"litelm.embedding call failed with an unexpected exception: {e}"
            )
...
def test_encoding_format_none_not_omitted_from_openai_sdk():
    ...
    """
    with patch("litelm.llms.openai.openai.OpenAIChatCompletion._get_openai_client") as mock_get_client:
