...
from litelm.caching import InMemoryCache
...
def langfuse_client():
    ...
    _cached_client = litelm.in_memory_llm_clients_cache.get_cache(_langfuse_cache_key)
    if _cached_client:
        ...
        litelm.in_memory_llm_clients_cache.set_cache(
...
def create_async_task(**completion_kwargs):
    """
    Creates an async task for the litelm.acompletion function.
    ...
    Any kwargs passed to this function will be passed to the litelm.acompletion function.
    By default a standard set of arguments are used for the litelm.acompletion function.
    """
    completion_args = {
    ...
    return asyncio.create_task(litelm.acompletion(**completion_args))
...
async def test_langfuse_logging_without_request_response(stream, langfuse_client):
    try:
        ...
        _unique_trace_name = f"litelm-test-{str(uuid.uuid4())}"
        litelm.set_verbose = True
        ...
        assert _trace_data[0].input == {
            "messages": [{"content": "redacted-by-litelm", "role": "user"}]
        }
        assert _trace_data[0].output == {
            "role": "assistant",
            "content": "redacted-by-litelm",
            "function_call": None,
            "tool_calls": None,
        }
...
async def test_langfuse_logging_audio_transcriptions(langfuse_client):
    ...
    assert generations[0].name == "litelm-atranscription"
...
async def test_langfuse_masked_input_output(langfuse_client):
    ...
    for mask_value in [True, False]:
        ...
        print(response)
        expected_input = "redacted-by-litelm" if mask_value else "This is a test"
        expected_output = (
            "redacted-by-litelm" if mask_value else "This is a test response"
        )
        langfuse_client.flush()
...
async def test_aaalangfuse_logging_metadata(langfuse_client):
    ...
    session_id = f"litelm-test-session-{run_id}"
    trace_common_metadata = {
        "session_id": session_id,
        "tags": ["litelm-test-tag1", "litelm-test-tag2"],
        "update_trace_keys": [
            "output",
            "trace_metadata",
        ...
        "trace_release": "litelm-test-release",
        "version": "litelm-test-version",
    }
    for trace_num in range(1, 3):  # Two traces
        metadata = copy.deepcopy(trace_common_metadata)
        trace_id = f"litelm-test-trace{trace_num}-{run_id}"
        ...
        for generation_num in range(
            ...
            metadata["trace_user_id"] = f"litelm-test-user{generation_num}-{run_id}"
            generation_id = (
                f"litelm-test-trace{trace_num}-generation-{generation_num}-{run_id}"
            )
...
def test_aaalangfuse_existing_trace_id():
    ...
    import litelm
    from litelm.integrations.langfuse.langfuse import LangFuseLogger
    ...
    response_obj = litelm.ModelResponse(
        ...
        usage=litelm.Usage(completion_tokens=37, prompt_tokens=14, total_tokens=51),
    )
    ...
    langfuse_args = {
        ...
        "user_id": None,
        "print_verbose": litelm.print_verbose,
        "level": "DEFAULT",
        "status_message": None,
    }
    ...
    new_response_obj = litelm.ModelResponse(
        id="chatcmpl-9K5HUAbVRqFrMZKXL0WoC295xhguY",
        choices=[
            litelm.Choices(
                finish_reason="stop",
    ...
    langfuse_args = {
        ...
        "kwargs": {
            ...
            "litelm_params": {
                ...
                "custom_llm_provider": "openai",
                "api_base": "https://api.openai.com/v1/",
                "litelm_call_id": "508113a1-c6f1-48ce-a3e1-01c6cce9330e",
                "model_alias_map": {},
                "completion_call_id": None,
...
@pytest.mark.asyncio
@pytest.mark.skip(
    reason="local only test, use this to verify if we can send request to litelm proxy server"
)
...
generation_params = {
    ...
    "metadata": {
        ...
        "hidden_params": {
            ...
            "litelm_call_id": "1fa31658-20af-40b5-9ac9-60fd7b5ad98c",
...
def test_langfuse_prompt_type(prompt):
    ...
    from litelm.integrations.langfuse.langfuse import _add_prompt_to_generation_params
...
def test_langfuse_logging_metadata():
    from litelm.integrations.langfuse.langfuse import log_requester_metadata
