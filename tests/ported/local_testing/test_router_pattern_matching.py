...
def test_add_pattern_vertex_ai():
    ...
    deployment = Deployment(
        ...
        litelm_params=LiteLLM_Params(model="vertex_ai/gemini-1.5-flash-latest"),
        model_info=ModelInfo(),
    )
...
async def test_route_with_no_matching_pattern():
    ...
    """
    from litelm.types.router import RouterErrors

    router = Router(
        model_list=[
            {
                "model_name": "*meta.llama3*",
                "litelm_params": {"model": "bedrock/meta.llama3*"},
            }
        ]
    )
    ...
    ## FAILS
    with pytest.raises(litelm.BadRequestError) as e:
    ...
    with pytest.raises(litelm.BadRequestError):
        ...
        model_list=[
            {
                ...
                "litelm_params": {"model": "anthropic/*", "api_key": "test"},
            }
        ]
    )
    ...
    router = Router(
        model_list=[
            ...
            },
            {
                ...
                "litelm_params": {"model": "anthropic/claude-3-5-sonnet"},
            },
        ]
    )
...
def test_sorted_patterns():
    ...
    """
    from litelm.router_utils.pattern_match_deployments import PatternUtils
...
def test_wildcard_priority_over_deployment_names():
    """
    Test that wildcard routes take priority over deployment_names (litelm_params.model) matching.
    ...
    NOT deployment 1 (even though deployment 1's litelm_params.model matches).
    
    Priority order should be:
    1. Exact model_name match
    2. Wildcard model_name match
    3. deployment_names (litelm_params.model) match
    """
    router = Router(
        model_list=[
            {
    ...
    assert deployments[0]['litelm_params']['api_base'] == "http://localhost:8082/openai", \
        f"Expected wildcard deployment (8082), got {deployments[0]['litelm_params']['api_base']}"
    ...
    assert deployments is not None, "No deployments found"
    assert len(deployments) == 1, f"Expected 1 deployment, got {len(deployments)}"
    assert deployments[0]['litelm_params']['api_base'] == "http://localhost:8080/openai", \
        f"Expected exact match deployment (8080), got {deployments[0]['litelm_params']['api_base']}"
    ...
    assert deployments is not None, "No deployments found"
    assert len(deployments) == 1, f"Expected 1 deployment, got {len(deployments)}"
    assert deployments[0]['litelm_params']['api_base'] == "http://localhost:8081/openai", \
        f"Expected '*' wildcard deployment (8081), got {deployments[0]['litelm_params']['api_base']}"
