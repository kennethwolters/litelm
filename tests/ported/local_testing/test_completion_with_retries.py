...
import openai
import litelm
from litelm import completion_with_retries, completion, acompletion_with_retries
from litelm import responses_with_retries, aresponses_with_retries
from litelm.responses.main import responses, aresponses
from litelm import (
    AuthenticationError,
    BadRequestError,
    RateLimitError,
...
async def test_completion_with_retry_policy(sync_mode):
    ...
    from litelm.types.router import RetryPolicy
    ...
    with patch.object(litelm, target_function) as mock_completion_with_retries:
        data = {
...
async def test_completion_with_retries(sync_mode):
    """
    If completion_with_retries is called with num_retries=3, and max_retries=0, then litelm.completion should receive num_retries , max_retries=0
    """
    from unittest.mock import patch, MagicMock, AsyncMock
    ...
    with patch.object(litelm, target_function) as mock_completion:
        if sync_mode:
...
async def test_responses_with_retries(sync_mode):
    ...
    then litelm.responses should receive num_retries=0, max_retries=0
    """
    from unittest.mock import patch, MagicMock, AsyncMock
    ...
    with patch("litelm.responses.main.responses" if sync_mode else "litelm.responses.main.aresponses") as mock_responses:
        if sync_mode:
...
async def test_responses_retry_on_auth_error(sync_mode):
    ...
    if sync_mode:
        with patch.object(litelm, "responses_with_retries") as mock_retry:
        ...
        with patch.object(litelm, "aresponses_with_retries") as mock_retry:
            mock_retry.return_value = None
            try:
                await aresponses(
                    model="gpt-4o",
