...
import litelm
from litelm import APIConnectionError, Router
...
@patch("litelm.secret_managers.get_azure_ad_token_provider.os")
...
async def test_audio_speech_router():
    """
    Test that router uses OpenAI/Azure OpenAI Client initialized during init for litelm.aspeech
    """
    ...
    with patch("litelm.aspeech") as mock_aspeech:
        ...
        print(
            "litelm.aspeech was called with kwargs = ", mock_aspeech.call_args.kwargs
        )
