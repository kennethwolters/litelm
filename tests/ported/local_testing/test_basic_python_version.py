...
def test_using_litelm():
    try:
        import litelm

        print("litelm imported successfully")
    except Exception as e:
        pytest.fail(
            f"Error occurred: {e}. Installing litelm on python3.8 failed please retry"
        )


def test_litelm_proxy_server():
    # Install the local litelm[proxy] package in development mode
    ...
    try:
        ...
        pytest.fail("Failed to import litelm.proxy_server")
...
def test_package_dependencies():
    ...
    try:
        ...
        # Get the litelm package root path
        litelm_path = pathlib.Path(litelm.__file__).parent.parent
        pyproject_path = litelm_path / "pyproject.toml"
...
def test_litelm_proxy_server_config_no_general_settings():
    # Install the local litelm packages in development mode
    server_process = None
    try:
        subprocess.run(["pip", "install", "-e", ".[proxy]"])
