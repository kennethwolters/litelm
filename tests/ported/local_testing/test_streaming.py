...
import litelm.litelm_core_utils
import litelm.litelm_core_utils.litelm_logging
from litelm.utils import ModelResponseListIterator
...
def test_completion_azure_stream_content_filter_no_delta():
    ...
    try:
        ...
        chunk_list = []
        for chunk in chunks:
            new_chunk = litelm.ModelResponseStream(id=chunk["id"])
            if "choices" in chunk and isinstance(chunk["choices"], list):
                new_choices = []
                for choice in chunk["choices"]:
                    if isinstance(choice, litelm.utils.StreamingChoices):
                        _new_choice = choice
                    elif isinstance(choice, dict):
                        _new_choice = litelm.utils.StreamingChoices(**choice)
        ...
        response = litelm.CustomStreamWrapper(
            completion_stream=completion_stream,
            model="gpt-4-0613",
            custom_llm_provider="cached_response",
            logging_obj=litelm.Logging(
                model="gpt-4-0613",
...
def test_completion_azure_stream():
    try:
        ...
        for idx, init_chunk in enumerate(response):
            ...
            if finished:
                assert isinstance(init_chunk.choices[0], litelm.utils.StreamingChoices)
                break
        if complete_response.strip() == "":
            raise Exception("Empty response received")
    except Exception as e:
    ...
    try:
        ...
        if sync_mode:
            ...
            for idx, init_chunk in enumerate(response):
                ...
                if finished:
                    assert isinstance(
                        init_chunk.choices[0], litelm.utils.StreamingChoices
                    )
                    break
            if complete_response.strip() == "":
                raise Exception("Empty response received")
        ...
        pass
    except litelm.ServiceUnavailableError:
        pass
...
async def test_completion_gemini_stream(sync_mode):
    try:
        ...
        complete_response = litelm.stream_chunk_builder(
...
async def test_completion_replicate_llama3_streaming(sync_mode):
    ...
    try:
        if sync_mode:
            final_chunk: Optional[litelm.ModelResponse] = None
            response: litelm.CustomStreamWrapper = completion(  # type: ignore
                model=model_name,
                messages=messages,
            ...
            response: litelm.CustomStreamWrapper = await litelm.acompletion(  # type: ignore
                model=model_name,
                messages=messages,
...
async def test_parallel_streaming_requests(sync_mode, model):
    ...
    try:
        ...
        def sync_test_streaming():
            response: litelm.CustomStreamWrapper = litelm.completion(  # type: ignore
                model=model,
                messages=messages,
                stream=True,
                max_tokens=10,
...
def test_sagemaker_weird_response():
    ...
    try:
        ...
        from litelm.llms.sagemaker.completion.handler import TokenIterator
...
async def test_openai_stream_options_call(model, sync):
    litelm.enable_preview_features = True
    litelm.set_verbose = True
    usage = None
    chunks = []
    if sync:
    ...
    assert isinstance(last_chunk.usage, litelm.Usage)
...
def validate_first_function_call_chunk_structure(item):
    if not (isinstance(item, dict) or isinstance(item, litelm.ModelResponse)):
    ...
    for choice in item["choices"]:
        if not (
            ...
            or isinstance(choice, litelm.utils.StreamingChoices)
        ):
        ...
        if not (
            ...
            or isinstance(choice["delta"], litelm.utils.Delta)
        ):
            raise Exception(
...
def validate_final_function_call_chunk_structure(data):
    if not (isinstance(data, dict) or isinstance(data, litelm.ModelResponse)):
        raise Exception("Incorrect format")
    ...
    for choice in data["choices"]:
        if not (
            isinstance(choice, dict) or isinstance(choice["delta"], litelm.utils.Delta)
        ):
            raise Exception("Incorrect format")
        for key in required_keys_in_choices_array:
            if key not in choice:
...
def test_streaming_and_function_calling(model):
    ...
    try:
        # litelm.set_verbose = True
        response: litelm.CustomStreamWrapper = completion(
            model=model,
            tools=tools,
            messages=messages,
            stream=True,
...
def test_success_callback_streaming():
    ...
    litelm.success_callback = [success_callback]
...
def test_unit_test_custom_stream_wrapper():
    ...
    chunk = litelm.ModelResponseStream(**chunk)
...
@pytest.mark.parametrize(
    "loop_amount",
    [
        litelm.REPEATED_STREAMING_CHUNK_LIMIT + 1,
        litelm.REPEATED_STREAMING_CHUNK_LIMIT - 1,
    ],
)
@pytest.mark.parametrize(
...
def test_unit_test_custom_stream_wrapper_repeating_chunk(
    ...
    if (loop_amount > litelm.REPEATED_STREAMING_CHUNK_LIMIT) and expected_chunk_fail:
        with pytest.raises((litelm.InternalServerError, litelm.exceptions.MidStreamFallbackError)):
            for chunk in response:
                continue
    else:
        for chunk in response:
...
def test_unit_test_custom_stream_wrapper_openai():
    ...
    chunk = {
        ...
        "choices": [
            litelm.utils.StreamingChoices(
                delta=litelm.utils.Delta(
...
def test_aamazing_unit_test_custom_stream_wrapper_n():
    ...
    Relevant issue: https://github.com/BerriAI/litelm/issues/3276
    """
    chunks = [
        {
...
def test_unit_test_custom_stream_wrapper_function_call():
    ...
    """
    from litelm.types.llms.openai import ChatCompletionDeltaChunk
    ...
    response = litelm.CustomStreamWrapper(
        ...
        model="gpt-3.5-turbo",
        custom_llm_provider="cached_response",
        logging_obj=litelm.litelm_core_utils.litelm_logging.Logging(
            model="gpt-3.5-turbo",
            messages=[{"role": "user", "content": "Hey"}],
            stream=True,
            call_type="completion",
    ...
    initial_model_response = litelm.ModelResponse(
...
def test_mock_response_iterator_tool_use():
    """
    Relevant Issue: https://github.com/BerriAI/litelm/issues/7364
    """
    from litelm.llms.bedrock.chat.invoke_handler import MockResponseIterator
    from litelm.types.utils import (
...
def test_streaming_with_cost_calculation():
    ...
    litelm.include_cost_in_streaming_usage = True
