...
def test_weighted_selection_router():
    ...
    # users can pass rpms as a litelm_param
    try:
        ...
        for _ in range(1000):
            ...
            selected_model_id = selected_model["litelm_params"]["model"]
...
def test_usage_based_routing():
    ...
    Then at some point, we exceed the TPM limit (set in the litelm_params)
        ...
        model_list = [
            {
                ...
                "litelm_params": get_azure_params("chatgpt-low-tpm"),
                "tpm": 100,
            },
            {
                "model_name": "azure/gpt-4",
                "litelm_params": get_azure_params("chatgpt-high-tpm"),
                "tpm": 1000,
            },
        ]
...
def test_get_available_deployment_for_pass_through():
    ...
    try:
        ...
        assert selected_model["litelm_params"]["model"] == "gpt-3.5-turbo"
        assert selected_model["litelm_params"]["use_in_pass_through"] is True
...
def test_get_available_deployment_for_pass_through_no_deployments():
    ...
    try:
        ...
        if isinstance(e, litelm.BadRequestError):
            pass  # Expected error
        else:
            traceback.print_exc()
            pytest.fail(f"Error occurred: {e}")
