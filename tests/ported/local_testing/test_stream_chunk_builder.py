...
from litelm.types.utils import StreamingChoices, ChatCompletionAudioResponse
...
from litelm import completion, stream_chunk_builder
...
def test_stream_chunk_builder_litelm_function_call():
    try:
        litelm.set_verbose = False
...
# test_stream_chunk_builder_litelm_function_call()


def test_stream_chunk_builder_litelm_tool_call():
    try:
...
# test_stream_chunk_builder_litelm_tool_call()


def test_stream_chunk_builder_litelm_tool_call_regular_message():
    try:
...
# test_stream_chunk_builder_litelm_tool_call_regular_message()


def test_stream_chunk_builder_litelm_mixed_calls():
...
def test_stream_chunk_builder_litelm_empty_chunks():
    with pytest.raises(litelm.APIError):
...
def test_stream_chunk_builder_multiple_tool_calls():
    ...
    for chunk in init_chunks:
        chunks.append(litelm.ModelResponseStream(**chunk))
    ...
    expected_response = litelm.ModelResponse(**completed_response)
...
def test_stream_chunk_builder_openai_prompt_caching():
    ...
    chunks: List[litelm.ModelResponse] = []
    usage_obj = None
    for chunk in chat_completion:
        chunks.append(litelm.ModelResponseStream(**chunk.model_dump()))

    print(f"chunks: {chunks}")

    usage_obj: litelm.Usage = chunks[-1].usage  # type: ignore
...
def test_stream_chunk_builder_openai_audio_output_usage():
    ...
    usage_obj: Optional[litelm.Usage] = None
...
def test_stream_chunk_builder_empty_initial_chunk():
    from litelm.litelm_core_utils.streaming_chunk_builder_utils import (
        ChunkProcessor,
    )
...
def load_env():
    ...
    tools = [
        {
            "type": "function",
            "function": litelm.utils.function_to_dict(get_current_weather),
        }
    ]
    OPENAI_GPT4oMINI = {
        "messages": messages,
...
def execute_completion(opts: dict):
    ...
    response_gen = litelm.completion(**opts)
    ...
    print("\n\n")
    assembly = litelm.stream_chunk_builder(partial_streaming_chunks)
    ...
    for tool_call in assembly.choices[0].message.tool_calls:
        json.loads(tool_call.function.arguments) # assert valid json - https://github.com/BerriAI/litelm/issues/10034
