...
def test_get_model_info_custom_llm_with_same_name_vllm(monkeypatch):
    ...
    model_info = litelm.get_model_info(model, custom_llm_provider=provider)
    print("model_info", model_info)
    assert model_info["input_cost_per_token"] == 0.0


def test_get_model_info_shows_correct_supports_vision():
    info = litelm.get_model_info("gemini/gemini-1.5-flash")
    print("info", info)
...
def test_get_model_info_shows_assistant_prefill():
    ...
    info = litelm.get_model_info("deepseek/deepseek-chat")
    print("info", info)
...
def test_get_model_info_finetuned_models():
    info = litelm.get_model_info("ft:gpt-3.5-turbo:my-org:custom_suffix:id")
    print("info", info)
    assert info["input_cost_per_token"] == 0.000003


def test_get_model_info_gemini_pro():
    info = litelm.get_model_info("gemini-1.5-pro-002")
    print("info", info)
    assert info["key"] == "gemini-1.5-pro-002"


def test_get_model_info_ollama_chat():
    from litelm.llms.ollama.completion.transformation import OllamaConfig

    with patch.object(
        litelm.module_level_client,
        "post",
...
def test_get_model_info_bedrock_region():
    ...
    litelm.model_cost.pop("us.anthropic.claude-3-5-sonnet-20241022-v2:0", None)
    info = litelm.get_model_info(**args)
    print("info", info)
    assert info["key"] == "anthropic.claude-3-5-sonnet-20241022-v2:0"
    assert info["litelm_provider"] == "bedrock"
...
def test_get_model_info_completion_cost_unit_tests(model):
    info = litelm.get_model_info(model)
    print("info", info)
...
def _enforce_bedrock_converse_models(
    ...
    for model, info in litelm.model_cost.items():
        if (
            info["litelm_provider"] == "bedrock"
            and info["mode"] == "chat"
            and model not in whitelist_models
        ):
            raise AssertionError(
                f"New bedrock chat model detected: {model}. Please set `litelm_provider='bedrock_converse'` for this model."
            )
...
def test_model_info_bedrock_converse(monkeypatch):
    ...
    _enforce_bedrock_converse_models(
        model_cost=litelm.model_cost, whitelist_models=whitelist_models
    )
...
def test_model_info_bedrock_converse_enforcement(monkeypatch):
    ...
    litelm.model_cost["fake.bedrock-chat-model"] = {
...
def test_get_model_info_custom_provider():
    # Custom provider example copied from https://docs.litelm.ai/docs/providers/custom_llm_server:
    import litelm
    from litelm import CustomLLM, completion, get_llm_provider

    class MyCustomLLM(CustomLLM):
        def completion(self, *args, **kwargs) -> litelm.ModelResponse:
            return litelm.completion(
                model="gpt-3.5-turbo",
    ...
    litelm.custom_provider_map = [  # 👈 KEY STEP - REGISTER HANDLER
    ...
    litelm.register_model(model_info)
...
def test_get_model_info_bedrock_models():
    ...
    for k, v in litelm.model_cost.items():
        if v["litelm_provider"] == "bedrock":
            ...
            base_model_key = (
                base_model
                if base_model in litelm.model_cost
                else f"bedrock/{base_model}"
            )
            if base_model_key not in litelm.model_cost:
                continue
            base_model_info = litelm.model_cost[base_model_key]
...
def test_get_model_info_huggingface_models(monkeypatch):
    from litelm import Router
    from litelm.types.router import ModelGroupInfo
    ...
    info = litelm.get_model_info("huggingface/meta-llama/Meta-Llama-3-8B-Instruct")
    print("info", info)
    assert info is not None
...
def test_get_model_info_cost_calculator_bedrock_region_cris_stripped(model, provider):
    ...
    Relevant Issue: https://github.com/BerriAI/litelm/issues/8115
    """
...
def test_get_model_info_case_insensitive_lookup(monkeypatch):
    ...
    info_lower = litelm.get_model_info(
        model="qwen/qwen3-next-80b-a3b-thinking", custom_llm_provider="together_ai"
    )
    ...
    info_mixed = litelm.get_model_info(
        model="QWEN/qwen3-NEXT-80b-a3b-thinking", custom_llm_provider="together_ai"
    )
