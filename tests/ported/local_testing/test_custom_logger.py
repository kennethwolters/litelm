...
async def test_async_chat_openai_stream():
    try:
        ...
        litelm.success_callback = [tmp_function.async_test_logging_fn]
...
def test_async_custom_handler_stream():
    try:
        ...
        litelm.callbacks = [customHandler2]
        litelm.set_verbose = False
        messages = [
            {"role": "system", "content": "You are a helpful assistant."},
            {
                "role": "user",
                "content": "write 1 sentence about litelm being amazing",
            },
        ]
        complete_streaming_response = ""
...
def test_azure_completion_stream():
    ...
    try:
        ...
        messages = [
            ...
            {
                "role": "user",
                "content": f"write 1 sentence about litelm being amazing {time.time()}",
            },
        ]
        complete_streaming_response = ""
...
async def test_async_custom_handler_embedding():
    try:
        ...
        litelm.callbacks = [customHandler_embedding]
        # success
...
async def test_async_custom_handler_embedding_optional_param():
    ...
    litelm.callbacks = [customHandler_optional_params]
    response = await litelm.aembedding(
