...
from litelm.proxy.guardrails.guardrail_hooks.aim.aim import (
    AimGuardrail,
    AimGuardrailMissingSecrets,
)
from litelm.proxy.proxy_server import StreamingCallbackError, UserAPIKeyAuth
from litelm.types.utils import ModelResponseStream, ModelResponse
...
async def test_block_callback(mode: str):
    ...
    aim_guardrails = [
        callback for callback in litelm.callbacks if isinstance(callback, AimGuardrail)
    ]
...
async def test_post_call_stream__all_chunks_are_valid(monkeypatch, length: int):
    ...
    monkeypatch.setattr(
        "litelm.proxy.guardrails.guardrail_hooks.aim.aim.connect", connect_mock
    )
...
async def test_post_call_stream__blocked_chunks(monkeypatch):
    from litelm.proxy.proxy_server import StreamingCallbackError
