...
from litelm.integrations.gcs_bucket.gcs_bucket import (
    GCSBucketLogger,
...
async def test_basic_gcs_logger_failure():
    ...
    try:
        response = await litelm.acompletion(
            ...
            max_tokens=10,
            user="ishaan-2",
            mock_response=litelm.BadRequestError(
                model="gpt-3.5-turbo",
...
async def test_basic_gcs_logging_per_request_with_callback_set():
    ...
    Request 2 - don't pass gcs_bucket_name in kwargs - ensure 'litelm-testing-bucket'
    """
    import logging
    from litelm._logging import verbose_logger
    ...
    GCS_BUCKET_NAME = "example-bucket-1-litelm"
    ...
    current_date = datetime.now().strftime("%Y-%m-%d")
    standard_callback_dynamic_params = StandardCallbackDynamicParams(
        gcs_bucket_name="litelm-testing-bucket"
    )
...
@pytest.mark.skip(reason="This test is flaky")
@pytest.mark.asyncio
async def test_basic_gcs_logging_per_request_with_no_litelm_callback_set():
    """
    Test GCS Bucket logging per request

    key difference: no litelm.callbacks set
...
async def test_basic_gcs_logger_with_folder_in_bucket_name():
    ...
    bucket_name = "litelm-testing-bucket/test-folder-logs"
...
def test_create_file_e2e():
    ...
    from litelm import create_file
