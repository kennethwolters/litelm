...
import litelm
from litelm import aembedding, completion, embedding, aresponses, responses
from litelm.caching.caching import Cache

from unittest.mock import AsyncMock, patch, MagicMock
from litelm.caching.caching_handler import LLMCachingHandler, CachingHandlerResponse
from litelm.caching.caching import LiteLLMCacheType
from litelm.types.utils import CallTypes
from litelm.types.rerank import RerankResponse
...
chat_completion_response = litelm.ModelResponse(
    id=str(uuid.uuid4()),
    choices=[
        litelm.Choices(
...
text_completion_response = litelm.TextCompletionResponse(
    id=str(uuid.uuid4()),
    choices=[litelm.utils.TextChoices(text="Hello, how can I help you today?")],
)
...
async def test_async_set_get_cache(response):
    ...
    logging_obj = LiteLLMLogging(
        litelm_call_id=str(datetime.now()),
    ...
    original_function = (
        litelm.acompletion
        if isinstance(response, litelm.ModelResponse)
        else litelm.atext_completion
    )
    if isinstance(response, litelm.ModelResponse):
