...
import json
import httpx
from litelm.types.utils import PromptTokensDetails
from litelm.litelm_core_utils.litelm_logging import CustomLogger
...
def test_custom_pricing_as_completion_cost_param():
    ...
    from litelm.utils import Usage
...
def test_cost_ft_gpt_35():
    try:
        # this tests if litelm.completion_cost can calculate cost for ft:gpt-3.5-turbo:my-org:custom_suffix:id
        # it needs to lookup  ft:gpt-3.5-turbo in the litelm model_cost map to get the correct cost
        from litelm import Choices, Message, ModelResponse
        from litelm.utils import Usage
...
def test_cost_azure_gpt_35():
    try:
        # this tests if litelm.completion_cost can calculate cost for azure/chatgpt-deployment-2 which maps to azure/gpt-3.5-turbo
...
def test_cost_azure_embedding():
    try:
        ...
        async def _test():
            response = await litelm.aembedding(
                model="azure/text-embedding-ada-002",
                input=["good morning from litelm", "gm"],
            )
        ...
        cost = litelm.completion_cost(completion_response=response)
...
def test_whisper_openai():
    ...
    cost = litelm.completion_cost(model="whisper-1", completion_response=transcription)

    print(f"cost: {cost}")
    print(f"whisper dict: {litelm.model_cost['whisper-1']}")
    expected_cost = round(
        litelm.model_cost["whisper-1"]["output_cost_per_second"]
...
def test_dalle_3_azure_cost_tracking():
    ...
    # response = litelm.image_generation(
    #     model=model,
...
def test_replicate_llama3_cost_tracking():
    ...
    response = litelm.ModelResponse(
        ...
        choices=[
            litelm.utils.Choices(
                finish_reason="stop",
                index=0,
                message=litelm.utils.Message(
    ...
    cost = round(cost, 5)
    expected_cost = round(
        litelm.model_cost["replicate/meta/meta-llama-3-8b-instruct"][
...
def test_groq_response_cost_tracking(is_streaming):
    ...
    response_cost = litelm.response_cost_calculator(
...
from litelm.types.utils import CallTypes


def test_together_ai_qwen_completion_cost():
    input_kwargs = {
        "completion_response": litelm.ModelResponse(
            **{
    ...
    response = litelm.cost_calculator.get_model_params_and_category(
...
def test_gemini_completion_cost(above_128k, provider):
    ...
    model_info = litelm.get_model_info(model=model_name, custom_llm_provider=provider)
...
def test_vertex_ai_completion_cost():
    ...
    model_info = litelm.get_model_info(model="gemini-1.5-flash")
...
def test_vertex_ai_claude_completion_cost():
    ...
    litelm.set_verbose = True
    input_tokens = litelm.token_counter(
        model="vertex_ai/claude-3-sonnet@20240229",
        messages=[{"role": "user", "content": "Hey, how's it going?"}],
    )
    print(f"input_tokens: {input_tokens}")
    output_tokens = litelm.token_counter(
        model="vertex_ai/claude-3-sonnet@20240229",
...
def test_vertex_ai_embedding_completion_cost(caplog):
    """
    Relevant issue - https://github.com/BerriAI/litelm/issues/4630
    """
    os.environ["LITELLM_LOCAL_MODEL_COST_MAP"] = "True"
    litelm.model_cost = litelm.get_model_cost_map(url="")
    ...
    model_info = litelm.get_model_info(model="vertex_ai/textembedding-gecko")
    ...
    for item in captured_logs:
        print("\nitem:{}\n".format(item))
        if (
            "litelm.litelm_core_utils.llm_cost_calc.google.cost_per_character(): Exception occured "
            in item
        ):
...
#     ## CALCULATED COST
#     resp = litelm.embedding(model="textembedding-gecko", input=[text])
...
async def test_completion_cost_hidden_params(sync_mode):
    litelm.return_response_headers = True
    if sync_mode:
        response = litelm.completion(
            model="gpt-3.5-turbo",
            messages=[{"role": "user", "content": "Hey, how's it going?"}],
...
@pytest.mark.parametrize("usage", ["litelm_usage", "openai_usage"])
def test_vertex_ai_mistral_predict_cost(usage):
    from litelm.types.utils import Choices, Message, ModelResponse, Usage

    if usage == "litelm_usage":
...
def test_completion_cost_azure_common_deployment_name():
    ...
    with patch.object(
        litelm.cost_calculator, "completion_cost", new=MagicMock()
    ) as mock_client:
        _ = litelm.response_cost_calculator(
            response_object=response,
            model="gpt-4-0314",
            custom_llm_provider="azure",
            call_type=CallTypes.acompletion.value,
...
def test_completion_cost_prompt_caching(model, custom_llm_provider):
    ...
    from litelm.utils import Choices, Message, ModelResponse, Usage
    ...
    _model_info = litelm.get_model_info(
...
def test_completion_cost_databricks(model):
    ...
    resp = litelm.completion(model=model, messages=messages)  # works fine
...
from litelm.llms.fireworks_ai.cost_calculator import get_base_model_for_pricing
...
def test_cost_azure_openai_prompt_caching():
    ...
    from litelm import get_model_info
...
def test_completion_cost_azure_ai_rerank(model):
    from litelm import RerankResponse, rerank
...
def test_together_ai_embedding_completion_cost():
    from litelm.utils import Choices, EmbeddingResponse, Message, ModelResponse, Usage
...
def test_completion_cost_params():
    """
    Relevant Issue: https://github.com/BerriAI/litelm/issues/6133
    """
    litelm.set_verbose = True
...
def test_completion_cost_params_2():
    ...
    model_info = litelm.get_model_info("gemini-1.5-pro-002")
...
def test_completion_cost_params_gemini_3():
    ...
    from litelm.llms.vertex_ai.cost_calculator import cost_per_character
    ...
    model_info = litelm.get_model_info("gemini-1.5-flash")
...
async def test_test_completion_cost_gpt4o_audio_output_from_model(stream):
    ...
    model_info = litelm.get_model_info("gpt-4o-audio-preview-2024-10-01")
...
def test_completion_cost_model_response_cost(response_model, custom_llm_provider):
    """
    Relevant issue: https://github.com/BerriAI/litelm/issues/6310
    """
...
def test_completion_cost_azure_tts():
    ...
    litelm.response_cost_calculator(**args)


def test_select_model_name_for_cost_calc():
    from litelm.cost_calculator import _select_model_name_for_cost_calc
    from litelm.types.utils import ModelResponse, Choices, Usage, Message
...
def test_moderations():
    from litelm import moderation

    os.environ["LITELLM_LOCAL_MODEL_COST_MAP"] = "True"
    litelm.model_cost = litelm.get_model_cost_map(url="")
    litelm.add_known_models()

    assert "omni-moderation-latest" in litelm.model_cost
    print(
        f"litelm.model_cost['omni-moderation-latest']: {litelm.model_cost['omni-moderation-latest']}"
    )
    assert "omni-moderation-latest" in litelm.open_ai_chat_completion_models
...
def test_cost_calculator_azure_embedding():
    from litelm.cost_calculator import response_cost_calculator
    from litelm.types.utils import EmbeddingResponse, Usage
...
def test_add_known_models():
    litelm.add_known_models()
    assert (
        "bedrock/us-west-1/meta.llama3-70b-instruct-v1:0" not in litelm.bedrock_models
    )
...
def test_bedrock_cost_calc_with_region():
    ...
    hidden_params = {
        ...
        "optional_params": {},
        "litelm_call_id": "cf371a5d-679b-410f-b862-8084676d6d59",
        "model_id": None,
        "api_base": None,
    ...
    bedrock_models = litelm.bedrock_models + litelm.bedrock_converse_models

    for model in bedrock_models:
        if litelm.model_cost[model]["mode"] == "chat":
            response = {
                "id": "cmpl-55db75e0b05344058b0bd8ee4e00bf84",
                "choices": [
                    {
...
@pytest.mark.parametrize("base_model_arg", ["litelm_param", "model_info"])
...
def test_cost_calculator_with_base_model_with_router(base_model_arg):
    ...
    if base_model_arg == "litelm_param":
        model_item["litelm_params"][
            "base_model"
...
def test_cost_calculator_with_base_model_with_router_embedding(base_model_arg):
    ...
    if base_model_arg == "litelm_param":
        model_item["litelm_params"]["base_model"] = "cohere.embed-english-v3"
    elif base_model_arg == "model_info":
        model_item["model_info"] = {
...
async def test_cost_calculator_with_custom_pricing_router(model_item, custom_pricing):
    ...
    if custom_pricing == "litelm_params":
        model_item["litelm_params"]["input_cost_per_token"] = 0.0000008
        model_item["litelm_params"]["output_cost_per_token"] = 0.0000032
...
def test_json_valid_model_cost_map():
    ...
    model_cost = litelm.get_model_cost_map(url="")
