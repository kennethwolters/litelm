#### What this tests ####
# This tests litelm router with batch completion
...
async def test_batch_completion_multiple_models(mode):
    ...
    if mode == "all_responses":
        response = await router.abatch_completion(
            ...
            messages=[
                {"role": "user", "content": "is litelm becoming a better product ?"}
            ],
            max_tokens=15,
        )
...
async def test_batch_completion_multiple_models_multiple_messages():
    ...
    assert isinstance(response[0][0], litelm.ModelResponse)
