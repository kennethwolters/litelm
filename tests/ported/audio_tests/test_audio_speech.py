...
async def test_audio_speech_litelm(sync_mode, model, api_base, api_key):
    litelm._turn_on_debug()
    speech_file_path = Path(__file__).parent / "speech.mp3"

    if sync_mode:
        response = litelm.speech(
            model=model,
            voice="alloy",
...
@pytest.mark.asyncio
@pytest.mark.flaky(retries=3, delay=1)
async def test_audio_speech_litelm_vertex(sync_mode):
...
@pytest.mark.asyncio
async def test_speech_litelm_vertex_async():
    # Mock the response
...
@pytest.mark.asyncio
async def test_speech_litelm_vertex_async_with_voice():
    # Mock the response
    mock_response = AsyncMock()
...
@pytest.mark.asyncio
async def test_speech_litelm_vertex_async_with_voice_ssml():
    # Mock the response
    mock_response = AsyncMock()
...
async def test_azure_ava_tts_with_custom_voice():
    ...
    with patch("litelm.llms.custom_httpx.http_handler.AsyncHTTPHandler.post") as mock_post:
