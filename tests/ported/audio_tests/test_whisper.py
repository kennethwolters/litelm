# What is this?
## Tests `litelm.transcription` endpoint. Outside litelm module b/c of audio file used in testing (it's ~700kb).
...
async def test_transcription(
    ...
    transcript = await litelm.atranscription(
        model=model,
        file=audio_file,
        api_key=api_key,
        api_base=api_base,
...
async def test_transcription_caching():
    ...
    response_1 = await litelm.atranscription(
        model="whisper-1",
        file=audio_file,
    )
    ...
    response_2 = await litelm.atranscription(
        model="whisper-1",
        file=audio_file,
    )
    ...
    response_3 = await litelm.atranscription(
        model="whisper-1",
        file=audio_file2,
    )
...
async def test_gpt_4o_transcribe_model_mapping():
    ...
    response = await litelm.atranscription(
    ...
    response2 = await litelm.atranscription(
    ...
    response3 = await litelm.atranscription(
...
async def test_azure_transcribe_model_mapping():
    ...
    with patch("litelm.llms.azure.audio_transcriptions.AzureAudioTranscription.get_azure_openai_client", return_value=mock_azure_client):
