...
import pytest
import time
from litelm import mock_completion
from unittest.mock import MagicMock, AsyncMock, patch
sys.path.insert(0, os.path.abspath("../.."))
import litelm
from litelm.proxy.guardrails.guardrail_hooks.presidio import _OPTIONAL_PresidioPIIMasking, PresidioPerRequestConfig
from litelm.integrations.custom_logger import CustomLogger
from litelm.types.utils import StandardLoggingPayload, StandardLoggingGuardrailInformation
...
async def test_standard_logging_payload_includes_guardrail_information():
    ...
    # 2. call litelm.acompletion
    response = await litelm.acompletion(**request_data)
...
async def test_langfuse_trace_includes_guardrail_information():
    ...
    from litelm.integrations.langfuse.langfuse_prompt_management import LangfusePromptManagement 
...
async def test_bedrock_guardrail_status_blocked():
    ...
    with patch.object(bedrock_guard.async_handler, "post", AsyncMock(return_value=mock_response)):
        ...
        # Call litelm.acompletion to trigger logging callbacks
...
async def test_bedrock_guardrail_status_success():
    ...
    with patch.object(bedrock_guard.async_handler, "post", AsyncMock(return_value=mock_response)):
        ...
        # Call litelm.acompletion to trigger logging
        response = await litelm.acompletion(**request_data)
        await asyncio.sleep(1)
...
async def test_noma_guardrail_status_blocked():
    ...
    """
    from litelm.proxy.guardrails.guardrail_hooks.noma.noma import NomaGuardrail
    from litelm.proxy._types import UserAPIKeyAuth
    from unittest.mock import AsyncMock, MagicMock, patch
...
def test_guardrail_status_fields_computation():
    ...
    """
    from litelm.litelm_core_utils.litelm_logging import _get_status_fields
