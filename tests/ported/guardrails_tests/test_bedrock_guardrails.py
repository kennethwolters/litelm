...
import litelm
from litelm.proxy.guardrails.guardrail_hooks.bedrock_guardrails import BedrockGuardrail
...
async def test_bedrock_guardrails_with_streaming():
    ...
    with pytest.raises(Exception):  # Assert that this raises an exception
        ...
        litelm.callbacks.append(guardrail)
...
async def test_bedrock_guardrail_blocked_vs_anonymized_actions():
    ...
    from litelm.types.proxy.guardrails.guardrail_hooks.bedrock_guardrails import (
        ...
        chunks = [
            ModelResponseStream(
                ...
                choices=[
                    litelm.utils.StreamingChoices(
                        index=0,
                        delta=litelm.utils.Delta(content="Sure! My email is "),
                        finish_reason=None,
                    )
                ],
                created=1234567890,
            ...
            ModelResponseStream(
                ...
                choices=[
                    litelm.utils.StreamingChoices(
                        index=0,
                        delta=litelm.utils.Delta(content="123-45-6789"),
                        finish_reason="stop",
                    )
                ],
                created=1234567890,
        ...
        chunks = [
            ModelResponseStream(
                ...
                choices=[
                    litelm.utils.StreamingChoices(
                        index=0,
                        delta=litelm.utils.Delta(content="My email is "),
                        finish_reason=None,
                    )
                ],
                created=1234567890,
            ...
            ModelResponseStream(
                ...
                choices=[
                    litelm.utils.StreamingChoices(
                        index=0,
                        delta=litelm.utils.Delta(content="john@example.com"),
                        finish_reason="stop",
                    )
                ],
                created=1234567890,
...
async def test_bedrock_guardrail_blocked_action_shows_output_text():
    ...
    mock_bedrock_response.json.return_value = {
        "action": "GUARDRAIL_INTERVENED",
        "outputs": [{"text": "this violates litelm corporate guardrail policy"}],
        "assessments": [
            {
                "topicPolicy": {
                    "topics": [
    ...
    with patch.object(
        ...
        expected_output_text = "this violates litelm corporate guardrail policy"
        ...
        chunks = [
            ...
            ModelResponseStream(
                ...
                choices=[
                    litelm.utils.StreamingChoices(
                        index=0,
                        delta=litelm.utils.Delta(content="step 1, step 2..."),
                        finish_reason="stop",
                    )
                ],
                created=1234567890,
        ...
        choices=[
            litelm.Choices(
                ...
                message=litelm.Message(
                    ...
                    tool_calls=[
                        litelm.utils.ChatCompletionMessageToolCall(
                            id="tooluse_kZJMlvQmRJ6eAyJE5GIl7Q",
                            function=litelm.utils.Function(
