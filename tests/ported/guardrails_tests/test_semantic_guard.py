...
class TestRouteLoader:
    ...
    def test_load_builtin_prompt_injection_template(self):
        from litelm.proxy.guardrails.guardrail_hooks.semantic_guard.route_loader import (
...
class TestSemanticGuardrailInit:
    ...
    def test_empty_routes_raises(self):
        from litelm.proxy.guardrails.guardrail_hooks.semantic_guard.semantic_guard import (
            SemanticGuardrail,
        )
...
class TestContentFilterSqlInjectionTemplate:
    """Tests for the keyword-based SQL injection policy template (litelm-content-filter)."""

    @pytest.fixture
    def sql_injection_guardrail(self):
        import litelm
        from litelm.proxy.guardrails.guardrail_hooks.litelm_content_filter.content_filter import (
            ContentFilterGuardrail,
        )
        from litelm.types.proxy.guardrails.guardrail_hooks.litelm_content_filter import (
            ContentFilterCategoryConfig,
        )

        content_filter_dir = os.path.join(
            os.path.dirname(__file__),
            "../../litelm/proxy/guardrails/guardrail_hooks/litelm_content_filter",
        )
        ...
        return ContentFilterGuardrail(
            ...
            event_hook=litelm.types.guardrails.GuardrailEventHooks.pre_call,
        )
...
class TestContentFilterPromptInjectionTemplate:
    """Tests for the keyword-based prompt injection policy template (litelm-content-filter)."""
