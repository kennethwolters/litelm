...
- Verifies litelm.completion() still works even with a broken cost map
- Verifies litelm.get_model_info() raises the expected error for unmapped models
...
import litelm
from litelm.litelm_core_utils.get_model_cost_map import (
    GetModelCostMap,
    get_model_cost_map,
)
...
class TestGetModelCostMapFallback:
    ...
    def test_should_fallback_when_fetched_map_shrinks_dramatically(self):
        ...
        tiny_map = {f"model-{i}": {"litelm_provider": "test"} for i in range(11)}
        mock_response = MagicMock()
        mock_response.raise_for_status = MagicMock()
...
class TestBadHostedModelCostMap:
    ...
    def test_should_model_info_pass_after_bad_hosted_map(self):
        ...
        original = litelm.model_cost
        litelm.model_cost = fallback_map
        try:
            # gpt-4o is in every backup — should work fine
            info = litelm.get_model_info("gpt-4o")
            assert info is not None
            assert info["input_cost_per_token"] > 0
        finally:
            litelm.model_cost = original

    def test_should_completion_pass_after_bad_hosted_map(self):
        """
        If the hosted map is bad, litelm.completion() should still work.

        Uses litelm's built-in mock_response param so the real completion
