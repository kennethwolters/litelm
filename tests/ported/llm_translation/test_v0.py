...
from litelm.llms.v0.chat.transformation import V0ChatConfig
...
def test_v0_in_provider_lists():
    ...
    assert "v0" in litelm.openai_compatible_providers
    assert "v0" in litelm.provider_list
    assert "https://api.v0.dev/v1" in litelm.openai_compatible_endpoints
...
def test_v0_models_configuration():
    ...
    for model in v0_models:
        ...
        assert model_info.get("litelm_provider") == "v0", f"{model} should have v0 as provider"
