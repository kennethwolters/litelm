...
from litelm.proxy._types import NewSkillRequest, UserAPIKeyAuth
...
async def test_slack_gif_skill_creates_gif(prisma_client):
    ...
    2. Hook resolves skill, adds litelm_code_execution tool, injects SKILL.md
    ...
    from litelm.llms.litelm_proxy.skills.handler import LiteLLMSkillsHandler
    from litelm.proxy.hooks.litelm_skills import SkillsInjectionHook
    ...
    try:
        ...
        request_data = {
            ...
            "container": {
                "skills": [
                    {"type": "custom", "skill_id": f"litelm:{created_skill.skill_id}"}
                ]
            },
        }
        ...
        assert "litelm_code_execution" in tool_names, "Should have litelm_code_execution tool"
        
        # 4. Make GPT-4o call via messages API (tools already in Anthropic format)
        print("\n--- Making GPT-4o call via messages API ---")
        response = await litelm.anthropic.acreate(
        ...
        if isinstance(response, dict):
            generated_files = response.get("_litelm_generated_files", [])
        else:
            generated_files = getattr(response, "_litelm_generated_files", [])
