...
async def test_deepseek_provider_async_completion(stream):
    ...
    with patch(
        "litelm.llms.custom_httpx.llm_http_handler.AsyncHTTPHandler.post"
    ) as mock_post:
        mock_response_data = litelm.ModelResponse(
            choices=[
                litelm.Choices(
                    message=litelm.Message(content="Hello!"),
                    index=0,
                    finish_reason="stop",
                )
            ]
...
def test_completion_cost_deepseek():
    ...
    try:
        response_1 = litelm.completion(model=model_name, messages=messages_1)
        response_2 = litelm.completion(model=model_name, messages=message_2)
