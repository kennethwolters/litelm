...
import importlib
import litelm.litelm_core_utils.get_model_cost_map
import litelm

# Reload modules to pick up environment variable
importlib.reload(litelm.litelm_core_utils.get_model_cost_map)
importlib.reload(litelm)

from litelm import completion
from litelm.llms.bedrock.common_utils import BedrockModelInfo, AmazonBedrockGlobalConfig
...
class TestBedrockGovCloudSupport:
    ...
    @patch('litelm.llms.bedrock.common_utils.init_bedrock_client')
    def test_govcloud_client_initialization(self, mock_init_client):
        ...
        from litelm.llms.bedrock.common_utils import init_bedrock_client
    ...
    def test_govcloud_model_in_bedrock_models_list(self):
        ...
        assert not any("us-gov-east-1" in model for model in litelm.bedrock_models)
        assert not any("us-gov-west-1" in model for model in litelm.bedrock_models)
    ...
    def test_govcloud_model_cost_properties(self):
        ...
        assert govcloud_model["litelm_provider"] == "bedrock"
    ...
    def test_govcloud_cost_per_token_with_region(self):
        ...
        from litelm import cost_per_token
        from litelm.utils import Usage
