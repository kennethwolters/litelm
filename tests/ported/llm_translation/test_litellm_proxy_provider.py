...
import litelm
from litelm import completion, embedding
import pytest
...
@pytest.mark.asyncio
async def test_litelm_gateway_from_sdk():
    ...
    with patch.object(
        ...
        try:
            completion(
                model="litelm_proxy/my-vllm-model",
                messages=messages,
...
@pytest.mark.asyncio
async def test_litelm_gateway_from_sdk_structured_output():
    ...
    with patch.object(
        ...
        try:
            litelm.completion(
                model="litelm_proxy/openai/gpt-4o",
                messages=[
                ...
                base_url="https://litelm.ml-serving-internal.scale.com",
                client=openai_client,
            )
        except Exception as e:
            print(e)
...
@pytest.mark.asyncio
async def test_litelm_gateway_from_sdk_embedding(is_async):
    litelm.set_verbose = True
...
@pytest.mark.parametrize("is_async", [False, True])
@pytest.mark.asyncio
async def test_litelm_gateway_from_sdk_image_generation(is_async):
    litelm._turn_on_debug()
    ...
    with patch.object(patch_target.__self__, patch_target.__name__, new=mock_method):
        try:
            if is_async:
                response = await litelm.aimage_generation(
                    model="litelm_proxy/dall-e-3",
...
@pytest.mark.parametrize("is_async", [False, True])
@pytest.mark.asyncio
async def test_litelm_gateway_image_generation_direct(is_async):
    """Test image generation using the litelm_proxy provider directly."""
    litelm._turn_on_debug()
    ...
    if is_async:
        ...
        with patch("litelm.llms.openai.openai.AsyncOpenAI", return_value=mock_async_client) as mock_async_constructor:
            response = await litelm.aimage_generation(
                model="litelm_proxy/dall-e-3",
                prompt="A beautiful sunset over mountains",
        ...
        with patch("litelm.llms.openai.openai.OpenAI", return_value=mock_sync_client) as mock_sync_constructor:
            response = litelm.image_generation(
                model="litelm_proxy/dall-e-3",
                prompt="A beautiful sunset over mountains",
                api_base="http://my-proxy",
...
@pytest.mark.parametrize("is_async", [False, True])
@pytest.mark.asyncio
async def test_litelm_gateway_from_sdk_image_edit(is_async):
    litelm._turn_on_debug()
    ...
    if is_async:
        ...
        patch_target = "litelm.llms.custom_httpx.http_handler.AsyncHTTPHandler.post"
    else:
        mock_post = MagicMock(return_value=MockResponse(mock_response, 200))
        patch_target = "litelm.llms.custom_httpx.http_handler.HTTPHandler.post"

    with patch(patch_target, new=mock_post):
        if is_async:
            await litelm.aimage_edit(
                model="litelm_proxy/gpt-image-1",
...
@pytest.mark.parametrize("is_async", [False, True])
@pytest.mark.asyncio
async def test_litelm_gateway_from_sdk_transcription(is_async):
    litelm.set_verbose = True
    litelm._turn_on_debug()
    ...
    with patch.object(patch_target.__self__, patch_target.__name__, new=mock_method):
        try:
            if is_async:
                await litelm.atranscription(
                    model="litelm_proxy/whisper-1",
                    file=b"sample_audio",
                    client=openai_client,
                    api_base="my-custom-api-base",
                )
            else:
                litelm.transcription(
                    model="litelm_proxy/whisper-1",
                    file=b"sample_audio",
                    client=openai_client,
                    api_base="my-custom-api-base",
...
@pytest.mark.parametrize("is_async", [False, True])
@pytest.mark.asyncio
async def test_litelm_gateway_from_sdk_speech(is_async):
    litelm.set_verbose = True
    ...
    with patch.object(patch_target.__self__, patch_target.__name__, new=mock_method):
        try:
            if is_async:
                await litelm.aspeech(
                    model="litelm_proxy/tts-1",
...
@pytest.mark.parametrize("is_async", [False, True])
@pytest.mark.asyncio
async def test_litelm_gateway_from_sdk_rerank(is_async):
    litelm.set_verbose = True
    litelm._turn_on_debug()
    ...
    with patch.object(client, "post", new=mock_method):
        ...
        try:
            if is_async:
                response = await litelm.arerank(
                    model="litelm_proxy/rerank-english-v2.0",
...
def test_litelm_gateway_from_sdk_with_response_cost_in_additional_headers():
    litelm.set_verbose = True
    litelm._turn_on_debug()
    ...
    mock_response = MagicMock()
    mock_response.headers = {"x-litelm-response-cost": "120"}
    mock_response.parse.return_value = litelm.ModelResponse(
        **{
    ...
    with patch.object(
        ...
        assert (
            response._hidden_params["additional_headers"][
                "llm_provider-x-litelm-response-cost"
            ]
            == "120"
        )
...
def test_litelm_gateway_from_sdk_with_thinking_param():
    try:
        response = litelm.completion(
            model="litelm_proxy/anthropic.claude-3-7-sonnet-20250219-v1:0",
            messages=[{"role": "user", "content": "Hello world"}],
            api_base="http://0.0.0.0:4000",
            api_key="sk-PIp1h0RekR",
            # client=openai_client,
