...
from litelm.litelm_core_utils.llm_response_utils.convert_dict_to_response import (
...
def test_convert_to_model_response_object_preserves_provider_specific_fields_from_proxy():
    ...
    Regression test for https://github.com/BerriAI/litelm/issues/21153
    """
    citations = [
        [
            {
...
def test_convert_to_model_response_object_no_provider_specific_fields_still_works():
    ...
    Ensures the fix for https://github.com/BerriAI/litelm/issues/21153
...
def test_convert_to_model_response_object_default_usage_overwritten():
    ...
    (i.e. no extra litelm.Usage() set via setattr beforehand).

    This validates the optimization of removing the redundant
    `setattr(model_response, "usage", litelm.Usage())` in completion().
    """
    mr = ModelResponse()
...
def test_convert_to_model_response_object_with_null_top_logprobs():
    ...
    an array, so litelm should normalize null to [].

    Regression test for https://github.com/BerriAI/litelm/issues/21932
    """
    response_object = {
