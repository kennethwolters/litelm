...
async def test_cohere_request_body_with_allowed_params():
    ...
    with patch("litelm.llms.custom_httpx.http_handler.AsyncHTTPHandler.post", return_value=mock_response) as mock_post:
        try:
...
async def test_cohere_v2_conversation_history():
    ...
    except (litelm.ServiceUnavailableError, litelm.InternalServerError, litelm.Timeout, litelm.APIConnectionError):
