...
from litelm.llms.lambda_ai.chat.transformation import LambdaAIChatConfig
...
def test_get_llm_provider_lambda_ai():
    ...
    from litelm.litelm_core_utils.get_llm_provider_logic import get_llm_provider
...
def test_lambda_ai_in_provider_lists():
    ...
    assert "lambda_ai" in litelm.openai_compatible_providers
    assert "lambda_ai" in litelm.provider_list
    assert "https://api.lambda.ai/v1" in litelm.openai_compatible_endpoints
...
def test_lambda_ai_models_configuration():
    ...
    for model in lambda_ai_models:
        ...
        assert model_info.get("litelm_provider") == "lambda_ai", f"{model} should have lambda_ai as provider"
...
def test_lambda_ai_model_list_populated():
    ...
    assert len(litelm.lambda_ai_models) > 0, "lambda_ai_models list should not be empty"
    
    # Check that all models in the list are Lambda AI models
    for model in litelm.lambda_ai_models:
    ...
    for model in expected_models:
        assert model in litelm.lambda_ai_models, f"{model} should be in lambda_ai_models list"