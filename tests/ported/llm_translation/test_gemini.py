...
from litelm.llms.vertex_ai.context_caching.transformation import (
...
def test_gemini_flash_image_preview_models(model_name: str):
    ...
    from litelm.types.utils import ImageResponse, ImageObject
    ...
    with patch(
        "litelm.llms.custom_httpx.llm_http_handler.HTTPHandler.post"
    ) as mock_post:
...
def test_gemini_thinking():
    litelm._turn_on_debug()
    from litelm.types.utils import Message, CallTypes
...
def test_gemini_with_grounding():
    from litelm import completion, Usage, stream_chunk_builder
...
async def test_claude_tool_use_with_gemini():
    """
    Tests that tool use via litelm.anthropic.messages.acreate with a non-Anthropic model
    ...
    with patch("litelm.acompletion", new_callable=AsyncMock) as mock_acompletion:
...
def test_gemini_with_thinking():
    ...
    if tool_calls:
        ...
        second_response = litelm.completion(
            model=model,
            messages=messages,
            seed=22,
            reasoning_effort="low",
...
def test_gemini_exception_message_format():
    ...
    from litelm.litelm_core_utils.exception_mapping_utils import exception_type
    from litelm import BadRequestError
...
def test_reasoning_effort_none_mapping():
    ...
    Related issue: https://github.com/BerriAI/litelm/issues/16420
    """
...
def test_gemini_function_args_preserve_unicode():
    ...
    https://github.com/BerriAI/litelm/issues/16533

    Before fix: "や" becomes "\u3084"
    After fix: "や" stays as "や"
    """
    from litelm.llms.vertex_ai.gemini.vertex_and_google_ai_studio_gemini import VertexGeminiConfig
...
def test_anthropic_thinking_param_to_gemini_3_thinkingLevel():
    ...
    Related issue: https://github.com/BerriAI/litelm/issues/XXXX
    """
    from litelm.llms.vertex_ai.gemini.vertex_and_google_ai_studio_gemini import (
        VertexGeminiConfig,
    )
...
def test_gemini_image_size_limit_exceeded():
    ...
    with pytest.raises(litelm.ImageFetchError) as excinfo:
        completion(
            model="gemini/gemini-2.5-flash-lite",
            messages=messages
        )
