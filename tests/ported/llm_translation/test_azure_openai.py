...
import pytest
from litelm.llms.azure.common_utils import process_azure_headers
...
def test_azure_extra_headers(input, call_type, header_value):
    from litelm import embedding, image_generation
...
def test_process_azure_endpoint_url(api_base, model, expected_endpoint):
    from litelm.llms.azure.azure import AzureChatCompletion
...
class TestAzureEmbedding(BaseLLMEmbeddingTest):
    ...
    def get_custom_llm_provider(self) -> litelm.LlmProviders:
        return litelm.LlmProviders.AZURE
...
def test_get_azure_ad_token_from_username_password(
    ...
    from litelm.llms.azure.common_utils import (
...
@patch(
    "litelm.main.azure_chat_completions.make_sync_azure_openai_chat_completion_request"
)
...
@pytest.mark.parametrize("max_retries", [0, 4])
@pytest.mark.parametrize("stream", [True, False])
@patch("litelm.main.azure_chat_completions.make_azure_openai_chat_completion_request")
@pytest.mark.asyncio
...
@patch("litelm.llms.azure.common_utils.select_azure_base_url_or_endpoint")
@pytest.mark.asyncio
async def test_azure_instruct(
    mock_select_azure_base_url_or_endpoint, max_retries, stream, sync_mode
):
    import litelm
    from litelm import completion, acompletion
...
async def test_azure_embedding_max_retries_0(
    ...
    import litelm
    from litelm import aembedding, embedding
...
def test_azure_openai_responses_bridge():
    ...
    with patch.object(litelm, "responses") as mock_responses:
        try:
            response = completion(
...
def test_azure_with_content_safety_error():
    ...
    """
    from litelm import completion
    from litelm.exceptions import ContentPolicyViolationError
