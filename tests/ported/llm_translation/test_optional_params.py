...
import litelm
from litelm.litelm_core_utils.prompt_templates.factory import map_system_message_pt
from litelm.types.completion import (
...
### Scenarios: w/ optional params + litelm.drop_params = True


def test_supports_system_message():
    """
    Check if litelm.completion(...,supports_system_message=False)
    """
    messages = [
...
@patch("litelm.main.openai_chat_completions.completion")
def test_requester_metadata_forwarded_to_openai(mock_completion):
    ...
    original_api_key = litelm.api_key
    litelm.api_key = "sk-test"
    original_preview_flag = litelm.enable_preview_features
    ...
    try:
        ...
        litelm.api_key = original_api_key
        litelm.enable_preview_features = original_preview_flag
...
def test_vertex_ai_llama_3_optional_params():
    litelm.vertex_llama3_models = ["meta/llama3-405b-instruct-maas"]
    litelm.drop_params = True
    optional_params = get_optional_params(
...
def test_vertex_ai_mistral_optional_params():
    litelm.vertex_mistral_models = ["mistral-large@2407"]
    litelm.drop_params = True
    optional_params = get_optional_params(
...
def test_azure_gpt_optional_params_gpt_vision():
    ...
    optional_params = litelm.utils.get_optional_params(
        model="",
        user="John",
        custom_llm_provider="azure",
        max_tokens=10,
...
def test_dynamic_drop_params_e2e():
    with patch(
        "litelm.llms.custom_httpx.http_handler.HTTPHandler.post", new=MagicMock()
    ) as mock_response:
        try:
            response = litelm.completion(
                model="command-r",
...
def test_drop_params_parallel_tool_calls(model, provider, should_drop):
    """
    https://github.com/BerriAI/litelm/issues/4584
    """
    response = litelm.utils.get_optional_params(
        model=model,
...
def test_get_optional_params_image_gen():
    response = litelm.utils.get_optional_params_image_gen(
...
def test_get_optional_params_num_retries():
    """
    Relevant issue - https://github.com/BerriAI/litelm/issues/5124
    """
    with patch(
        "litelm.main.get_optional_params",
...
def test_vertex_safety_settings(provider):
    litelm.vertex_ai_safety_settings = [
        {
...
def test_o1_model_temperature_params(provider, temperature, expected_error):
    if expected_error:
        with pytest.raises(litelm.UnsupportedParamsError):
            get_optional_params(
                model="o1-preview",
                custom_llm_provider=provider,
...
def test_drop_nested_params_add_prop_and_strict(provider, model):
    """
    Relevant issue - https://github.com/BerriAI/litelm/issues/5288

    Relevant issue - https://github.com/BerriAI/litelm/issues/6136
    """
    tools = [
        {
            "type": "function",
...
def test_hosted_vllm_tool_param():
    """
    Relevant issue - https://github.com/BerriAI/litelm/issues/6228
    """
    optional_params = get_optional_params(
        model="my-vllm-model",
...
def test_forward_user_param():
    from litelm.utils import get_supported_openai_params, get_optional_params
...
def test_gemini_frequency_penalty():
    from litelm.utils import get_supported_openai_params
...
def test_litelm_proxy_claude_3_5_sonnet():
    tools = [
        {
            "type": "function",
            "function": {
...
def test_is_vertex_anthropic_model():
    assert (
        litelm.VertexAIAnthropicConfig().is_supported_model(
            model="claude-3-5-sonnet", custom_llm_provider="litelm_proxy"
        )
        is False
    )
...
def test_litelm_proxy_thinking_param():
    optional_params = get_optional_params(
        model="gpt-4o",
        custom_llm_provider="litelm_proxy",
        thinking={"type": "enabled", "budget_tokens": 1024},
...
def test_azure_response_format_param():
    optional_params = litelm.get_optional_params(
...
def test_validate_openai_optional_params_disable_stop_sequence_limit():
    ...
    When litelm.disable_stop_sequence_limit is True, stop sequences should not be truncated.
    """
    # Save original value
    original_value = litelm.disable_stop_sequence_limit

    try:
        # Test with disable_stop_sequence_limit = True - should NOT truncate
        litelm.disable_stop_sequence_limit = True
        stop_sequences = ["stop1", "stop2", "stop3", "stop4", "stop5", "stop6"]
        result = validate_openai_optional_params(stop=stop_sequences)
        ...
        litelm.disable_stop_sequence_limit = False
        stop_sequences = ["stop1", "stop2", "stop3", "stop4", "stop5", "stop6"]
        result = validate_openai_optional_params(stop=stop_sequences)
        assert result == ["stop1", "stop2", "stop3", "stop4"]
        assert len(result) == 4
        ...
        litelm.disable_stop_sequence_limit = original_value
...
def test_validate_openai_optional_params_integration():
    ...
    try:
        with patch("litelm.llms.openai.openai.OpenAI") as mock_client:
...
def test_drop_store_param_for_anthropic():
    ...
    Ref: https://github.com/BerriAI/litelm/issues/19700
    """
    optional_params = get_optional_params(
...
def test_store_in_openai_chat_completion_params():
    ...
    Ref: https://github.com/BerriAI/litelm/issues/19700
    """
    from litelm.constants import OPENAI_CHAT_COMPLETION_PARAMS

    assert "store" in OPENAI_CHAT_COMPLETION_PARAMS

    # Verify get_standard_openai_params recognizes store
    from litelm.utils import get_standard_openai_params
