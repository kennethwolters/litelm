...
from litelm.llms.bedrock.chat import BedrockLLM
from litelm.llms.custom_httpx.http_handler import AsyncHTTPHandler, HTTPHandler
from litelm.litelm_core_utils.prompt_templates.factory import _bedrock_tools_pt
...
def bedrock_session_token_creds():
    ...
    if aws_session_token is not None:
        ...
        aws_role_name = (
            "arn:aws:iam::335785316107:role/litelm-github-unit-tests-circleci"
        )
...
def process_stream_response(res, messages):
    ...
    if isinstance(res, litelm.utils.CustomStreamWrapper):
        chunks = []
        for part in res:
            chunks.append(part)
            text = part.choices[0].delta.content or ""
            print(text, end="")
        res = litelm.stream_chunk_builder(chunks, messages=messages)
    else:
...
def test_completion_bedrock_claude_sts_oidc_auth():
    ...
    # aws_role_name = os.environ["AWS_TEMP_ROLE_NAME"]
    # TODO: This is using ai.moda's IAM role, we should use LiteLLM's IAM role eventually
    aws_role_name = "arn:aws:iam::335785316107:role/litelm-github-unit-tests-circleci"
...
def test_bedrock_ptu():
    ...
    Reference: https://github.com/BerriAI/litelm/issues/3805
    """
    client = HTTPHandler()
...
async def test_bedrock_custom_api_base():
    ...
    Reference: https://github.com/BerriAI/litelm/issues/3805, https://github.com/BerriAI/litelm/issues/5389#issuecomment-2313677977
...
async def test_bedrock_extra_headers(model):
    """
    Relevant Issue: https://github.com/BerriAI/litelm/issues/9106
    """
    client = AsyncHTTPHandler()
...
async def test_bedrock_custom_prompt_template():
    ...
    Reference: https://github.com/BerriAI/litelm/issues/4415
    """
    client = AsyncHTTPHandler()
...
def test_bedrock_tool_calling():
    """
    # related issue: https://github.com/BerriAI/litelm/issues/5007
...
def test_bedrock_tools_transformation_valid_params():
    from litelm.types.llms.bedrock import ToolJsonSchemaBlock
...
def test_not_found_error():
    with pytest.raises(litelm.NotFoundError):
        completion(
...
def test_bedrock_get_base_model(model, expected_base_model):
    from litelm.llms.bedrock.common_utils import BedrockModelInfo

    assert BedrockModelInfo.get_base_model(model) == expected_base_model


from litelm.litelm_core_utils.prompt_templates.factory import (
    _bedrock_converse_messages_pt,
)


def test_bedrock_converse_translation_tool_message():
    from litelm.types.utils import ChatCompletionMessageToolCall, Function
...
def test_base_aws_llm_get_credentials():
    ...
    from litelm.llms.bedrock.base_aws_llm import BaseAWSLLM
...
def test_bedrock_completion_test_4(modify_params):
    litelm.set_verbose = True
    litelm.modify_params = modify_params
    ...
    if modify_params:
        ...
        with pytest.raises(Exception) as e:
            litelm.completion(**data)
        assert "litelm.modify_params" in str(e.value)


def test_bedrock_context_window_error():
    with pytest.raises(litelm.ContextWindowExceededError) as e:
        litelm.completion(
...
def test_bedrock_prompt_caching_message(messages, expected_cache_control):
    ...
    transformed_messages = litelm.AmazonConverseConfig()._transform_request(
...
def test_bedrock_supports_tool_call(model, expected_supports_tool_call):
    supported_openai_params = (
        litelm.AmazonConverseConfig().get_supported_openai_params(model=model)
    )
...
class TestBedrockConverseChatCrossRegion(BaseLLMChatTest):
    ...
    def test_completion_cost(self):
        ...
        litelm.model_cost.pop(bedrock_model, None)
...
class TestBedrockRerank(BaseLLMRerankTest):
    def get_custom_llm_provider(self) -> litelm.LlmProviders:
        return litelm.LlmProviders.BEDROCK
...
def test_bedrock_nova_topk(top_k_param):
    ...
    original_transform = litelm.AmazonConverseConfig()._transform_request
    captured_data = None
    ...
    with patch(
        "litelm.AmazonConverseConfig._transform_request", side_effect=mock_transform
    ):
        litelm.completion(**data)
...
def test_nova_optional_params_tool_choice():
    ...
    except litelm.APIConnectionError:
        pass
...
class TestBedrockEmbedding(BaseLLMEmbeddingTest):
    ...
    def test_bedrock_image_embedding_transformation(self):
        from litelm.llms.bedrock.embed.amazon_titan_multimodal_transformation import (
...
async def test_bedrock_image_url_sync_client():
    ...
    import logging
    from litelm import verbose_logger
...
def test_bedrock_error_handling_streaming():
    from litelm.llms.bedrock.chat.invoke_handler import (
...
def test_handle_top_k_value_helper(model, expected_output):
    assert (
        litelm.AmazonConverseConfig()._handle_top_k_value(model, {"topK": 3})
        == expected_output
    )
    assert (
        litelm.AmazonConverseConfig()._handle_top_k_value(model, {"top_k": 3})
        == expected_output
    )
...
def test_bedrock_invoke_provider():
    assert (
        litelm.AmazonInvokeConfig().get_bedrock_invoke_provider(
    ...
    assert (
        litelm.AmazonInvokeConfig().get_bedrock_invoke_provider("amazon.nova-pro-v1:0")
        == "nova"
    )
    assert (
        litelm.AmazonInvokeConfig().get_bedrock_invoke_provider(
...
async def test_bedrock_thinking_in_assistant_message(sync_mode):
    litelm._turn_on_debug()
    from litelm.llms.custom_httpx.http_handler import HTTPHandler, AsyncHTTPHandler
    ...
    with patch.object(client, "post") as mock_post:
        try:
            if sync_mode:
                response = litelm.completion(**params, client=client)
            else:
                response = await litelm.acompletion(**params, client=client)
        except Exception as e:
            print(e)
...
def test_bedrock_meta_llama_function_calling():
    ...
    """
    from litelm.utils import return_raw_request
...
async def test_bedrock_passthrough(sync_mode: bool):
    ...
    if sync_mode:
        response = litelm.llm_passthrough_route(
        ...
        response = await litelm.allm_passthrough_route(
            model="bedrock/us.anthropic.claude-3-5-sonnet-20240620-v1:0",
            method="POST",
            endpoint="/model/us.anthropic.claude-3-5-sonnet-20240620-v1:0/invoke",
            data=data,
...
async def test_bedrock_passthrough_router():
    """
    Test bedrock passthrough using litelm.Router with async mode.
...
async def test_bedrock_converse__streaming_passthrough(monkeypatch):
    ...
    monkeypatch.setattr(litelm, "callbacks", [mock_custom_logger])
...
def test_bedrock_openai_convert_messages_to_prompt():
    ...
    """
    from litelm.llms.bedrock.chat.invoke_handler import BedrockLLM
...
def test_bedrock_openai_request_transformation():
    ...
    """
    from litelm.llms.bedrock.chat.invoke_transformations.base_invoke_transformation import (
        AmazonInvokeConfig,
    )
...
def test_bedrock_openai_parameter_filtering():
    ...
    """
    from litelm.llms.bedrock.chat.invoke_transformations.amazon_openai_transformation import (
...
def test_bedrock_openai_error_handling():
    ...
    from litelm.llms.bedrock.chat.invoke_handler import BedrockLLM
    from litelm import ModelResponse
    from litelm.llms.bedrock.common_utils import BedrockError
    from unittest.mock import Mock
    import json
