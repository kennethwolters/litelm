...
def test_bedrock_embedding_models(model, input_type, embed_response):
    ...
    with patch.object(client, "post") as mock_post:
        ...
        input_data = (
            img_base_64 if input_type == "image" else "Hello world from litelm"
        )
        ...
        try:
            ...
            assert isinstance(response, litelm.EmbeddingResponse)
            print(response.data)
...
def test_e2e_bedrock_embedding():
    ...
    assert isinstance(
        response, litelm.EmbeddingResponse
...
def test_e2e_bedrock_async_invoke_embedding_twelvelabs_marengo():
    ...
    with patch(
        "litelm.llms.bedrock.embed.embedding.BedrockEmbedding._make_sync_call"
    ) as mock_call:
...
async def test_e2e_bedrock_async_invoke_embedding_async_twelvelabs_marengo():
    ...
    with patch(
        "litelm.llms.bedrock.embed.embedding.BedrockEmbedding._make_async_call"
    ) as mock_call:
        mock_call.return_value = {
...
def test_bedrock_embedding_uses_correct_region_when_specified():
    ...
    relevant issue: https://github.com/BerriAI/litelm/issues/16517
    """
