...
from litelm.types.llms.anthropic import AnthropicResponse
from litelm.types.utils import GenericStreamingChunk, ChatCompletionToolCallChunk
from litelm.types.llms.openai import ChatCompletionToolCallFunctionChunk
from litelm.llms.anthropic.common_utils import process_anthropic_headers
from litelm.llms.anthropic.chat.handler import AnthropicChatCompletion
...
def test_anthropic_beta_header(
    ...
    headers = litelm.AnthropicConfig().get_anthropic_headers(
...
def test_anthropic_tool_helper(cache_control_location):
    from litelm.llms.anthropic.chat.transformation import AnthropicConfig
...
def test_anthropic_tool_with_image():
    from litelm.litelm_core_utils.prompt_templates.factory import prompt_factory
    import json
...
def test_anthropic_map_openai_params_tools_and_json_schema():
    ...
    mapped_params = litelm.AnthropicConfig().map_openai_params(
...
from litelm.constants import RESPONSE_FORMAT_TOOL_NAME
...
def test_anthropic_json_mode_and_tool_call_response(
    ...
    result = litelm.AnthropicConfig()._transform_response_for_json_mode(
        json_mode=json_mode,
...
def test_map_stop_sequences(stop_input, expected_output, drop_params):
    ...
    litelm.drop_params = drop_params
    config = AnthropicConfig()
...
async def test_anthropic_structured_output():
    ...
    Relevant Issue: https://github.com/BerriAI/litelm/issues/8291
    """
...
def test_anthropic_thinking_in_assistant_message(model):
    ...
    response = litelm.completion(**params)
