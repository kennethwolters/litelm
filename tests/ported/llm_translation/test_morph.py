...
import litelm
from litelm import MorphChatConfig, get_llm_provider
...
def test_morph_in_provider_lists():
    ...
    import litelm
    from litelm.constants import openai_compatible_providers, openai_compatible_endpoints
    ...
    assert "morph" in litelm.provider_list
    
    # Check models are in model_list after initialization
    assert all(model in litelm.model_list for model in ["morph/morph-v3-large", "morph/morph-v3-fast"])


def test_morph_model_info():
    """Test that morph models have correct configuration."""
    import litelm
    model_info = litelm.get_model_info("morph/morph-v3-large")
    
    assert model_info["litelm_provider"] == "morph"
