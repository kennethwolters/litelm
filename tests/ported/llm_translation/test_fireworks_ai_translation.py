...
import litelm
from litelm import transcription
...
def test_map_response_format():
    ...
    Relevant Issue: https://github.com/BerriAI/litelm/issues/6797
...
class TestFireworksAIAudioTranscription(BaseLLMAudioTranscriptionTest):
    ...
    def get_custom_llm_provider(self) -> litelm.LlmProviders:
        return litelm.LlmProviders.FIREWORKS_AI
...
def test_transform_inline(content, model, expected_url):
    ...
    result = litelm.FireworksAIConfig()._add_transform_inline_image_block(
...
def test_global_disable_flag(model, is_disabled, expected_url):
    ...
    assert result["image_url"] == expected_url
    litelm.disable_add_transform_inline_image_block = False  # Reset for other tests
...
def test_global_disable_flag_with_transform_messages_helper(monkeypatch):
    ...
    monkeypatch.setattr(litelm, "disable_add_transform_inline_image_block", True)
