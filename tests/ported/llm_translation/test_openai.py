...
async def test_openai_prediction_param_with_caching():
    ...
    """
    from litelm.caching.caching import LiteLLMCacheType
    import logging
    ...
    litelm.set_verbose = True
    litelm.cache = litelm.Cache(type=LiteLLMCacheType.LOCAL)
    code = """
    /// <summary>
    /// Represents a user with a first name, last name, and username.
    /// </summary>
    ...
    completion_response_1 = litelm.completion(
        model="gpt-4o-mini",
        messages=[
            {
                "role": "user",
    ...
    # cache hit
    completion_response_2 = litelm.completion(
        model="gpt-4o-mini",
        messages=[
            {
                "role": "user",
    ...
    completion_response_3 = litelm.completion(
        model="gpt-4o-mini",
        messages=[
...
@patch("litelm.main.openai_chat_completions._get_openai_client")
...
def test_openai_chat_completion_streaming_handler_reasoning_content():
    from litelm.llms.openai.chat.gpt_transformation import (
...
def validate_web_search_annotations(annotations: ChatCompletionAnnotation):
    """validates litelm response contains web search annotations"""
...
def test_openai_web_search():
    ...
    print("litelm response: ", response.model_dump_json(indent=4))
...
def test_openai_web_search_streaming():
    ...
    for chunk in response:
        print("litelm response chunk: ", chunk)
        if (
...
class TestOpenAIGPT4OAudioTranscription(BaseLLMAudioTranscriptionTest):
    ...
    def get_custom_llm_provider(self) -> litelm.LlmProviders:
        return litelm.LlmProviders.OPENAI


@pytest.mark.asyncio
@pytest.mark.parametrize("model", ["gpt-4o"])
async def test_openai_pdf_url(model):
    from litelm.utils import return_raw_request, CallTypes
...
async def test_openai_codex_stream(sync_mode):
    from litelm.main import stream_chunk_builder
    ...
    chunks = []
    if sync_mode:
        response = litelm.completion(**kwargs)
        for chunk in response:
            chunks.append(chunk)
    else:
...
async def test_openai_via_gemini_streaming_bridge():
    ...
    """
    from litelm import Router
    from litelm.types.utils import ModelResponseStream
...
def test_openai_tool_calling():
    ...
    response = litelm.completion(**completion_params)
...
def test_openai_gpt_5_codex_reasoning():
    ...
    response = litelm.completion(**completion_kwargs)
    print("response: ", response)
    for chunk in response:
        print("chunk: ", chunk)
...
# Regression test for: https://github.com/BerriAI/litelm/issues/8977
@pytest.mark.parametrize("model", ["gpt-4o"])
@pytest.mark.asyncio
