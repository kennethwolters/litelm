...
ALLOWED_FILE = os.path.normpath("litelm/_uuid.py")
...
def scan_directory_for_fastuuid(base_dir: str) -> List[Dict[str, Any]]:
    ...
    scan_root = os.path.join(base_dir, "litelm")
...
def main() -> None:
    ...
    if violations:
        print(
            "\n🚨 fastuuid must only be imported inside litelm/_uuid.py. Found violations:"
        )
        for v in violations:
            print(f"* {v['module']} ({v['file']}:{v['line']}) -> {v['import']}")
        print("\n")
        raise Exception(
            "Found fastuuid imports outside litelm/_uuid.py. Use litelm._uuid.uuid or litelm._uuid.uuid4 instead."
        )
    else:
