...
def check_for_litelm_module_deletion(base_dir):
    """
    Checks for code patterns that delete litelm modules from sys.modules
    in the test_litelm directory.
    
    Specifically looks for patterns like:
    for module in list(sys.modules.keys()):
        if module.startswith("litelm"):
            del sys.modules[module]
    """
    problematic_files = []
    test_dir = os.path.join(base_dir, "test_litelm")
    ...
    for root, _, files in os.walk(test_dir):
        for file in files:
            if file.endswith(".py"):
                ...
                # Check for litelm module deletion patterns
                if has_litelm_module_deletion(tree):
                    relative_path = os.path.relpath(file_path, base_dir)
                    problematic_files.append(relative_path)
                    print(f"Found litelm module deletion in: {relative_path}")
    
    return problematic_files


def has_litelm_module_deletion(tree):
    """
    Checks if the AST contains patterns that delete litelm modules from sys.modules.
    
    Looks for:
    1. Loops over sys.modules.keys()
    2. Conditions checking if module startswith "litelm"
    3. del sys.modules[module] statements
    """
    class LiteLLMDeletionVisitor(ast.NodeVisitor):
        def __init__(self):
            self.has_sys_modules_loop = False
            self.has_litelm_check = False
        ...
        def visit_If(self, node):
            # Check for conditions like module.startswith("litelm")
            if (isinstance(node.test, ast.Call) and
                ...
                node.test.args[0].value == "litelm"):
                
                self.has_litelm_check = True
    ...
    return (visitor.has_sys_modules_loop and 
            visitor.has_litelm_check and 
            visitor.has_del_sys_modules)


def main():
    """
    Main function to check for litelm module deletion patterns in test files.
    """
    # local dir 
    #tests_dir = "../../tests/"
    ...
    problematic_files = check_for_litelm_module_deletion(tests_dir)
    
    if problematic_files:
        print("\nERROR: Found files that delete litelm modules from sys.modules:")
        for file_path in problematic_files:
            print(f"  - {file_path}")
        
        raise Exception(
            f"Found {len(problematic_files)} file(s) that delete litelm modules from sys.modules. "
            f"This can cause import issues and test failures. Files: {problematic_files}"
        )
    else:
        print("✓ No litelm module deletion patterns found in test_litelm directory.")
