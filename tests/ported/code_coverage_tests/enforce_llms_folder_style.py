...
def get_unique_names_from_llms_dir(base_dir: str):
    """
    Returns a set of unique file and folder names from the root level of litelm/llms directory,
...
def run_lint_check(unique_names):
    _all_litelm_providers = [str(provider.value) for provider in litelm.LlmProviders]
    violations = []
    for name in unique_names:
        if (
            name.lower() not in _all_litelm_providers
    ...
    if len(violations) > 0:
        raise ValueError(
            f"There are {len(violations)} violations in the llms folder. \n\n {violations}. \n\n Valid providers: {_all_litelm_providers}"
        )


def main():
    # llms_dir = "./litelm/llms/"  # Update this path if needed
    llms_dir = "litelm/litelm/llms"  # LOCAL TESTING
