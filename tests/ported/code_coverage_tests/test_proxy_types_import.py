...
def test_proxy_types_not_imported():
    """
    Test that proxy._types is not directly imported in litelm/__init__.py
    by examining the source code using AST parsing.
    """
    # Read the litelm/__init__.py file
    # local_init_file = "../litelm/"
    init_file_path = os.path.join("./litelm", "__init__.py")
    ...
    if found_imports:
        print("❌ BAD, this can import time to import litelm. Found direct imports of proxy._types in litelm/__init__.py:")
        print("=" * 80)
        ...
        raise Exception(
            f"Found {len(found_imports)} direct import(s) of proxy._types in litelm/__init__.py"
        )
    
    print("✓ No direct imports of proxy._types found in litelm/__init__.py")
    return True
...
def main():
    ...
    """
    print("=" * 60)
    print("Testing litelm import performance")
    print("Checking that proxy._types is not directly imported from litelm/__init__.py")
    print("=" * 60)
    
    try:
        test_proxy_types_not_imported()
        print("\n" + "=" * 60)
        print("✓ Test passed! proxy._types is not directly imported from litelm/__init__.py")
        print("=" * 60)
