...
def get_function_names_from_file(file_path: str) -> List[str]:
    """
    Extracts all static method names from litelm_logging.py
    """
...
def main():
    logging_file = "./litelm/litelm_core_utils/litelm_logging.py"
    tests_dir = "./tests/"
    ...
    if untested_functions:
        ...
        raise Exception(
            f"{untested_perc * 100:.2f}% of functions in litelm_logging.py are not tested: {untested_functions}"
        )
    else:
        print("All functions in litelm_logging.py are covered by tests.")
