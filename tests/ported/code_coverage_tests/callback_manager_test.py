...
ALLOWED_FILES = [
    # local files
    "../../litelm/litelm_core_utils/litelm.logging_callback_manager.py",
    "../../litelm/proxy/common_utils/callback_utils.py",
...
def check_for_callback_modifications(file_path):
    """
    Checks if any direct modifications to specific litelm callback lists are made in the given file.
    ...
    for node in ast.walk(tree):
        # Check for attribute calls like litelm.callbacks.append()
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute):
            ...
            # Check if the attribute chain starts with 'litelm' and modifies a protected list
            if (
                len(attr_chain) >= 3
                and attr_chain[0] == "litelm"
                ...
                if (
                    ...
                    violations.append(
                        ...
                        f"Direct modification of 'litelm.{protected_list}' using '{operation}' is not allowed. "
...
def test_no_unauthorized_callback_modifications():
    ...
    """
    base_dir = "./litelm"  # Adjust this path as needed
    # base_dir = "../../litelm"  # LOCAL TESTING
