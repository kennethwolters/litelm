...
def find_set_verbose_assignments(file_path):
    """
    Finds all assignments of litelm.set_verbose = True in a given Python file.
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Assign):
            # Check if this is an assignment to litelm.set_verbose
            for target in node.targets:
                if isinstance(target, ast.Attribute):
                    # Check if it's litelm.set_verbose
                    if (isinstance(target.value, ast.Name) and 
                        target.value.id == "litelm" and 
...
def scan_litelm_files(base_dir):
    """
    Scans all Python files in the litelm directory for set_verbose assignments.
    ...
    for litelm_dir in litelm_dirs:
        dir_path = os.path.join(base_dir, litelm_dir)
...
def test_no_hardcoded_set_verbose():
    """
    Pytest-compatible test function that ensures no hardcoded litelm.set_verbose = True assignments exist.
    """
    base_dir = "./"  # Adjust path as needed for your setup
    
    violations = scan_litelm_files(base_dir)
    ...
    if violations:
        ...
        error_msg = (
            f"Found {total_violations} prohibited litelm.set_verbose = True assignments:\n"
            + "\n".join(violation_details) + 
            "\n\nREASON: litelm.set_verbose = True should not be hardcoded in production code. "
...
def main():
    """
    Main function that scans for litelm.set_verbose = True assignments and fails if any are found.
    """
    base_dir = "./"  # Adjust path as needed for your setup
    
    print("Scanning for litelm.set_verbose = True assignments...")
    violations = scan_litelm_files(base_dir)
    
    if violations:
        print("\n❌ FOUND PROHIBITED litelm.set_verbose = True ASSIGNMENTS:")
        print("=" * 60)
        ...
        print("   - Use litelm.set_verbose = os.getenv('LITELLM_VERBOSE', 'false').lower() == 'true'")
        ...
        print("✅ No prohibited litelm.set_verbose = True assignments found.")
