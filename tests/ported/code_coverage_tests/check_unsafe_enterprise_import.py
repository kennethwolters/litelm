...
class EnterpriseImportFinder(ast.NodeVisitor):
    ...
    def visit_Import(self, node):
        # Check for direct imports of litelm_enterprise
        for name in node.names:
            if "litelm_enterprise" in name.name or "enterprise" in name.name:
    ...
    def visit_ImportFrom(self, node):
        # Check for from litelm_enterprise imports
        if node.module and ("litelm_enterprise" in node.module or "enterprise" in node.module):
            if not self.in_try_block:
                self.unsafe_imports.append({
                    "file": self.current_file,
                    "line": node.lineno,
...
if __name__ == "__main__":
    # Check for unsafe enterprise imports in the litelm directory
    directory_path = "./litelm"
