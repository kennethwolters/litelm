...
1. Every provider folder in litelm/llms/ has a corresponding entry in provider_endpoints_support.json
2. Every provider in litelm/llms/openai_like/providers.json is documented in provider_endpoints_support.json
"""
...
def get_repo_root() -> Path:
    ...
    # Check if litelm directory exists in current working directory
    cwd = Path.cwd()
    if (cwd / "litelm").exists() and (cwd / "litelm").is_dir():
...
def get_llm_provider_folders() -> Set[str]:
    """Get all provider folder names from litelm/llms directory."""
    repo_root = get_repo_root()
    llms_dir = repo_root / "litelm" / "llms"
...
def get_openai_like_providers() -> Set[str]:
    """Get all provider names from litelm/llms/openai_like/providers.json."""
    repo_root = get_repo_root()
    providers_file = repo_root / "litelm" / "llms" / "openai_like" / "providers.json"
...
def main():
    ...
    # Check 1: Provider folders in litelm/llms
    print("\n📁 Checking provider folders in litelm/llms/...")
    ...
    if undocumented_folders:
        ...
        for folder in undocumented_folders:
            error_msg += f"  - litelm/llms/{folder}/\n"
        ...
        for folder in undocumented_folders[:3]:
            ...
            error_msg += (
                f'      "url": "https://docs.litelm.ai/docs/providers/{folder}",\n'
            )
            error_msg += '      "endpoints": {\n'
    ...
    if undocumented_openai_like:
        ...
        for provider in undocumented_openai_like[:3]:
            ...
            error_msg += (
                f'      "url": "https://docs.litelm.ai/docs/providers/{normalized}",\n'
            )
            error_msg += '      "endpoints": {\n'
            error_msg += '        "chat_completions": true,\n'
            error_msg += '        "messages": true,\n'
