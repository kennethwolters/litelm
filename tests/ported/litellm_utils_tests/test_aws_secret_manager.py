...
async def test_write_and_read_simple_secret():
    ...
    test_secret_name = f"litelm_test_{uuid.uuid4().hex[:8]}"
...
async def test_write_and_read_json_secret():
    ...
    secret_manager = AWSSecretsManagerV2()
    test_secret_name = f"litelm_test_{uuid.uuid4().hex[:8]}_json"
    test_secret_value = {
        "api_key": "test_key",
        "model": "gpt-4",
        "temperature": 0.7,
        "metadata": {"team": "ml", "project": "litelm"},
    }
...
async def test_read_nonexistent_secret():
    ...
    secret_manager = AWSSecretsManagerV2()
    nonexistent_secret = f"litelm_nonexistent_{uuid.uuid4().hex}"
...
async def test_primary_secret_functionality():
    ...
    secret_manager = AWSSecretsManagerV2()
    primary_secret_name = f"litelm_test_primary_{uuid.uuid4().hex[:8]}"
...
async def test_write_secret_with_description_and_tags():
    ...
    secret_manager = AWSSecretsManagerV2()
    test_secret_name = f"litelm_test_{uuid.uuid4().hex[:8]}_tags"
...
def test_load_aws_secret_manager_with_settings():
    ...
    try:
        ...
        assert litelm.secret_manager_client is not None
        assert isinstance(litelm.secret_manager_client, AWSSecretsManagerV2)
        
        # Verify settings were passed through
        assert litelm.secret_manager_client.aws_role_name == settings.aws_role_name
        assert litelm.secret_manager_client.aws_region_name == settings.aws_region_name
        assert litelm.secret_manager_client.aws_session_name == settings.aws_session_name
    finally:
        # Cleanup
...
async def test_end_to_end_iam_role_secret_write():
    ...
    test_secret_name = f"litelm_test_iam_{uuid.uuid4().hex[:8]}"
