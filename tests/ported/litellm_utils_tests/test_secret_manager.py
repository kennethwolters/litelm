...
import pytest
import litelm
from litelm.llms.azure.azure import get_azure_ad_token_from_oidc
from litelm.llms.bedrock.chat import BedrockConverseLLM, BedrockLLM
from litelm.secret_managers.aws_secret_manager_v2 import AWSSecretsManagerV2
from litelm.secret_managers.main import (
    get_secret,
...
def test_aws_secret_manager():
    ...
    secret_val = get_secret("litelm_master_key")
    ...
    assert secret_val["litelm_master_key"] == "sk-1234"
...
def test_oidc_circle_v1_with_amazon():
    ...
    aws_role_name = "arn:aws:iam::335785316107:role/litelm-github-unit-tests-circleci-v1-assume-only"
...
def test_google_secret_manager():
    ...
    from litelm.secret_managers.google_secret_manager import GoogleSecretManager
...
def test_should_read_secret_from_secret_manager():
    ...
    """
    from litelm.types.secret_managers.main import KeyManagementSettings

    # Test when secret manager client is None
    litelm.secret_manager_client = None
    ...
    litelm.secret_manager_client = "dummy_client"
    litelm._key_management_settings = KeyManagementSettings(access_mode="read_only")
    ...
    litelm._key_management_settings = KeyManagementSettings(access_mode="write_only")
    assert _should_read_secret_from_secret_manager() is False
...
def test_key_management_settings_defaults():
    ...
    assert settings.prefix_for_stored_virtual_keys == "litelm/"
...
def test_key_management_settings_custom_values():
    ...
    settings = KeyManagementSettings(
        ...
        prefix_for_stored_virtual_keys="litelm/custom/",
        access_mode="read_and_write",
        primary_secret_name="primary/litelm/keys",
        description=custom_description,
        tags=custom_tags,
    )

    assert settings.store_virtual_keys is True
    assert settings.prefix_for_stored_virtual_keys == "litelm/custom/"
    assert settings.access_mode == "read_and_write"
    assert settings.primary_secret_name == "primary/litelm/keys"
...
async def test_async_write_secret_receives_description_and_tags(monkeypatch):
    ...
    """
    from litelm import litelm
    from litelm.secret_managers.aws_secret_manager_v2 import AWSSecretsManagerV2
    from litelm.types.secret_managers.main import KeyManagementSettings

    # Mock out AWS network calls
    mock_async_write = AsyncMock(return_value={"Name": "litelm/test_secret"})
    ...
    litelm.secret_manager_client = AWSSecretsManagerV2()
...
def test_key_management_settings_serialization_roundtrip():
    ...
    original = KeyManagementSettings(
        store_virtual_keys=True,
        prefix_for_stored_virtual_keys="litelm/dev/",
        access_mode="read_and_write",
    ...
    assert reloaded.prefix_for_stored_virtual_keys == "litelm/dev/"
