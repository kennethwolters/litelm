...
from litelm.litelm_core_utils.logging_callback_manager import LoggingCallbackManager
...
def test_add_string_callback():
    """
    Test adding a string callback to litelm.callbacks - only 1 instance of the string callback should be added
    """
    manager = LoggingCallbackManager()
    test_callback = "test_callback"

    # Add string callback
    manager.add_litelm_callback(test_callback)
    assert test_callback in litelm.callbacks

    # Test duplicate prevention
    manager.add_litelm_callback(test_callback)
    assert litelm.callbacks.count(test_callback) == 1
...
def test_duplicate_langfuse_logger_test():
    ...
    for _ in range(10):
        ...
        manager.add_litelm_success_callback(langfuse_logger)
    print("litelm.success_callback: ", litelm.success_callback)
...
def test_duplicate_multiple_loggers_test():
    ...
    for _ in range(10):
        ...
        manager.add_litelm_success_callback(langfuse_logger)
        manager.add_litelm_success_callback(otel_logger)
    print("litelm.success_callback: ", litelm.success_callback)
    assert len(litelm.success_callback) == 2

    # Check exactly one instance of each logger type
    langfuse_count = sum(
        1
        for callback in litelm.success_callback
...
def test_add_function_callback():
    ...
    manager.add_litelm_callback(test_func)
    assert test_func in litelm.callbacks

    # Test duplicate prevention
    manager.add_litelm_callback(test_func)
    assert litelm.callbacks.count(test_func) == 1
...
def test_add_custom_logger(mock_custom_logger):
    ...
    # Add custom logger
    manager.add_litelm_callback(mock_custom_logger)
    assert mock_custom_logger in litelm.callbacks
...
def test_add_multiple_callback_types(mock_custom_logger):
    ...
    manager.add_litelm_callback(string_callback)
    manager.add_litelm_callback(test_func)
    manager.add_litelm_callback(mock_custom_logger)

    assert string_callback in litelm.callbacks
    assert test_func in litelm.callbacks
    assert mock_custom_logger in litelm.callbacks
    assert len(litelm.callbacks) == 3
...
def test_success_failure_callbacks():
    ...
    # Add callbacks
    manager.add_litelm_success_callback(success_callback)
    manager.add_litelm_failure_callback(failure_callback)

    assert success_callback in litelm.success_callback
    assert failure_callback in litelm.failure_callback
...
def test_async_callbacks():
    ...
    # Add async callbacks
    manager.add_litelm_async_success_callback(async_success)
    manager.add_litelm_async_failure_callback(async_failure)

    assert async_success in litelm._async_success_callback
    assert async_failure in litelm._async_failure_callback
...
def test_remove_callback_from_list_by_object():
    ...
    def TestObject():
        def __init__(self):
            manager.add_litelm_callback(self.callback)
            manager.add_litelm_success_callback(self.callback)
            manager.add_litelm_failure_callback(self.callback)
            manager.add_litelm_async_success_callback(self.callback)
            manager.add_litelm_async_failure_callback(self.callback)
    ...
    manager.remove_callback_from_list_by_object(litelm.callbacks, obj)
    manager.remove_callback_from_list_by_object(litelm.success_callback, obj)
    manager.remove_callback_from_list_by_object(litelm.failure_callback, obj)
    manager.remove_callback_from_list_by_object(litelm._async_success_callback, obj)
    manager.remove_callback_from_list_by_object(litelm._async_failure_callback, obj)
...
def test_reset_callbacks(callback_manager):
    ...
    callback_manager.add_litelm_callback("test")
    callback_manager.add_litelm_success_callback("success")
    callback_manager.add_litelm_failure_callback("failure")
    callback_manager.add_litelm_async_success_callback("async_success")
    callback_manager.add_litelm_async_failure_callback("async_failure")
...
async def test_slack_alerting_callback_registration(callback_manager):
    """
    Test that litelm callbacks are correctly registered for slack alerting
    ...
    with patch('litelm.integrations.SlackAlerting.slack_alerting.get_async_httpx_client') as mock_http:
        ...
        assert len(litelm.callbacks) == 1
        assert isinstance(litelm.callbacks[0], SlackAlerting)
        ...
        assert len(litelm.callbacks) == 1  # Regular callback for outage alerts
        assert isinstance(litelm.callbacks[0], SlackAlerting)
        ...
        assert len(litelm._async_success_callback) == 1
        assert litelm._async_success_callback[0] == response_taking_too_long_callback
...
async def test_generic_api_compatible_callbacks_json():
    ...
    """
    from litelm.integrations.generic_api.generic_api_callback import GenericAPILogger
