...
async def test_reset_budget_keys_partial_failure():
    ...
    with patch.object(
        ...
        await job.reset_budget_for_litelm_keys()
...
async def test_reset_budget_users_partial_failure():
    ...
    with patch.object(
        ...
        await job.reset_budget_for_litelm_users()
        await asyncio.sleep(0.1)
...
async def test_reset_budget_endusers_partial_failure():
    ...
    prisma_client.update_data = AsyncMock()
    # Mock db.litelm_verificationtoken.update_many (used by reset_budget_for_keys_linked_to_budgets)
    prisma_client.db.litelm_verificationtoken.update_many = AsyncMock(
    ...
    with patch.object(
        ...
        ResetBudgetJob,
        "reset_budget_for_litelm_team_members",
        side_effect=fake_reset_team_members,
    ) as mock_reset_team_members:
        await job.reset_budget_for_litelm_budget_table()
        await asyncio.sleep(0.1)
...
async def test_reset_budget_teams_partial_failure():
    ...
    with patch.object(
        ...
        await job.reset_budget_for_litelm_teams()
        await asyncio.sleep(0.1)
...
async def test_service_logger_keys_success():
    ...
    with patch.object(
        ...
        with patch(
            "litelm.proxy.common_utils.reset_budget_job.verbose_proxy_logger.exception"
        ) as mock_verbose_exc:
            await job.reset_budget_for_litelm_keys()
...
@pytest.mark.asyncio
async def test_reset_budget_for_litelm_team_members_called():
    """
    Test that when reset_budget_for_litelm_budget_table is called,
    team members' budgets are also reset via reset_budget_for_litelm_team_members
    """
    # Arrange
    budget1 = LiteLLM_BudgetTableFull(
        **{
    ...
    # Mock the db.litelm_teammembership.update_many call
    prisma_client.db = MagicMock()
    prisma_client.db.litelm_teammembership = MagicMock()
    prisma_client.db.litelm_teammembership.update_many = AsyncMock(
    ...
    prisma_client.db.litelm_teammembership.update_many.assert_called_once()

    # Verify the call was made with correct parameters
    call_args = prisma_client.db.litelm_teammembership.update_many.call_args
