...
async def test_azure_health_check():
    response = await litelm.ahealth_check(
        model_params={
...
def test_update_litelm_params_for_health_check():
    """
    Test if _update_litelm_params_for_health_check correctly:
    ...
    """
    from litelm.proxy.health_check import _update_litelm_params_for_health_check
    ...
    updated_params = _update_litelm_params_for_health_check(model_info, litelm_params)
    ...
    for prefix in cris_prefixes:
        ...
        updated_params = _update_litelm_params_for_health_check(
            model_info, litelm_params
        )
        assert (
...
async def test_perform_health_check_filters_by_model_id():
    ...
    """
    from litelm.proxy.health_check import perform_health_check
    ...
    model_list = [
        ...
        },
        {
            ...
            "litelm_params": {"model": "gpt-4", "api_key": "fake-key-2"},
        },
    ]
    ...
    async def mock_perform_health_check(m_list, details=True, **kwargs):
        ...
        return [{"model": "gpt-4", "api_key": m_list[0]["litelm_params"]["api_key"]}], []

    with patch(
        "litelm.proxy.health_check._perform_health_check",
...
async def test_perform_health_check_with_health_check_model():
    ...
    """
    from litelm.proxy.health_check import _perform_health_check

    # Mock model list with health_check_model specified
    model_list = [
        {
            "litelm_params": {"model": "openai/*", "api_key": "fake-key"},
            "model_info": {
                "mode": "chat",
    ...
    async def mock_health_check(litelm_params, **kwargs):
        health_check_calls.append(litelm_params["model"])
        return {"status": "healthy"}

    with patch("litelm.ahealth_check", side_effect=mock_health_check):
...
async def test_health_check_bad_model():
    ...
    with patch(
        "litelm.ahealth_check", side_effect=mock_health_check
...
async def test_health_check_respects_concurrency_limit():
    ...
    model_list = [
        {"litelm_params": {"model": f"openai/gpt-4o-mini-{i}", "api_key": "fake-key"}}
        for i in range(6)
    ]
...
async def test_health_check_creates_only_bounded_initial_tasks():
    ...
    with patch("litelm.ahealth_check", side_effect=mock_health_check), patch(
        "litelm.proxy.health_check.asyncio.create_task", side_effect=tracked_create_task
    ):
...
async def test_timeout_does_not_cancel_other_health_checks():
    ...
    model_list = [
        {
            "litelm_params": {"model": "openai/slow-model", "api_key": "fake-key"},
            "model_info": {"health_check_timeout": 0.05},
        },
        {
            "litelm_params": {"model": "openai/fast-model", "api_key": "fake-key"},
            "model_info": {"health_check_timeout": 1},
        },
    ]

    async def mock_health_check(litelm_params, **kwargs):
        if litelm_params["model"] == "openai/slow-model":
...
async def test_image_generation_health_check_prompt(monkeypatch):
    ...
    import importlib
    import litelm.constants as litelm_constants
    import litelm.proxy.health_check as health_check

    def reload_modules():
        reloaded_constants = importlib.reload(litelm_constants)
    ...
    async def run_health_check(health_check_module):
        ...
        async def mock_health_check(litelm_params, mode=None, prompt=None, input=None):
            health_check_calls.append(
                {
                    "mode": mode,
                    "prompt": prompt,
                    "model": litelm_params.get("model"),
                }
            )
            return {"status": "healthy"}
        ...
        model_list = [
            {
                "litelm_params": {"model": "dall-e-3", "api_key": "fake-key"},
                "model_info": {
                    "mode": "image_generation",
                },
            }
        ...
        with patch(
            "litelm.proxy.health_check.litelm.ahealth_check",
    ...
    litelm_constants, health_check = reload_modules()
    health_check_calls = await run_health_check(health_check)

    assert len(health_check_calls) == 1
    assert (
        health_check_calls[0]["prompt"] == litelm_constants.DEFAULT_HEALTH_CHECK_PROMPT
    )
...
async def test_health_check_with_custom_llm_provider():
    ...
    with patch("litelm.acompletion", return_value=mock_response):
