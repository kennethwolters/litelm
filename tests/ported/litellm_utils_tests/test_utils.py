...
from litelm.types.utils import StandardCallbackDynamicParams
...
import litelm
from litelm.llms.custom_httpx.http_handler import AsyncHTTPHandler, headers
from litelm.litelm_core_utils.duration_parser import duration_in_seconds
from litelm.litelm_core_utils.duration_parser import (
...
def test_basic_trimming_no_max_tokens_specified():
    ...
    ) <= litelm.model_cost["gpt-4"]["max_tokens"]
...
def test_trimming_with_system_message_within_max_tokens():
    ...
    messages = [
        ...
        {
            "role": "user",
            "content": "This is a medium normal message, let's say litelm is awesome.",
        },
    ]
    trimmed_messages = trim_messages(
...
def test_trimming_with_tool_calls():
    from litelm.types.utils import ChatCompletionMessageToolCall, Function, Message
...
def test_trimming_with_model_cost_max_input_tokens(model):
    ...
    assert (
        ...
        < litelm.model_cost[model]["max_input_tokens"]
    )
...
def test_aget_valid_models():
    ...
    # list of openai supported llms on litelm
    expected_models = (
        litelm.open_ai_chat_completion_models | litelm.open_ai_text_completion_models
    )
    ...
    # GEMINI
    expected_models = litelm.gemini_models
    old_environ = os.environ
...
def test_function_to_dict():
    ...
    function_json = litelm.utils.function_to_dict(get_current_weather)
    print(function_json)
...
def test_supports_function_calling(model, expected_bool):
    try:
        assert litelm.supports_function_calling(model=model) == expected_bool
...
def test_supports_web_search(model, expected_bool):
    try:
        assert litelm.supports_web_search(model=model) == expected_bool
    except Exception as e:
        pytest.fail(f"Error occurred: {e}")
...
def test_supports_reasoning(model, expected_bool):
    ...
    try:
        assert litelm.supports_reasoning(model=model) == expected_bool
    except Exception as e:
        pytest.fail(f"Error occurred: {e}")
...
def test_get_chat_completion_prompt():
    ...
    """
    from litelm.litelm_core_utils.litelm_logging import Logging

    litelm_logging_obj = Logging(
        model="gpt-3.5-turbo",
    ...
    litelm_logging_obj.get_chat_completion_prompt(
        model="gpt-3.5-turbo",
    ...
    assert litelm_logging_obj.messages == [
...
def test_redact_msgs_from_logs():
    ...
    """
    from litelm.litelm_core_utils.litelm_logging import Logging
    from litelm.litelm_core_utils.redact_messages import (
        redact_message_input_output_from_logging,
    )

    litelm.turn_off_message_logging = True

    response_obj = litelm.ModelResponse(
        choices=[
            {
                "finish_reason": "stop",
                "index": 0,
    ...
    _redacted_response_obj = redact_message_input_output_from_logging(
        result=response_obj,
        model_call_details=litelm_logging_obj.model_call_details,
    )
...
def test_redact_embedding_response():
    ...
    response_obj = litelm.EmbeddingResponse(
    ...
    assert isinstance(_redacted_response_obj, litelm.EmbeddingResponse)
...
def test_redact_msgs_from_logs_with_dynamic_params():
    ...
    In all tests litelm.turn_off_message_logging is True
    ...
    3. standard_callback_dynamic_params.turn_off_message_logging not set, litelm.turn_off_message_logging is True: Redaction should occur.
    """
    from litelm.litelm_core_utils.litelm_logging import Logging
    from litelm.litelm_core_utils.redact_messages import (
        redact_message_input_output_from_logging,
    ...
    litelm_logging_obj.model_call_details["standard_callback_dynamic_params"] = (
    ...
    assert _redacted_response_obj.choices[0].message.content == "redacted-by-litelm"

    # Test Case 3: standard_callback_dynamic_params does not override litelm.turn_off_message_logging
    # since litelm.turn_off_message_logging is True redaction should occur
    standard_callback_dynamic_params = StandardCallbackDynamicParams()
    litelm_logging_obj.model_call_details["standard_callback_dynamic_params"] = (
        standard_callback_dynamic_params
    )
...
def test_logging_trace_id(langfuse_trace_id, langfuse_existing_trace_id):
    ...
    litelm_call_id = "my-unique-call-id"
    litelm_logging_obj = Logging(
        model="gpt-3.5-turbo",
        messages=[{"role": "user", "content": "hi"}],
        stream=False,
        call_type="acompletion",
        litelm_call_id=litelm_call_id,
        start_time=datetime.now(),
        function_id="1234",
    )
    ...
    litelm.completion(
        ...
        mock_response="Hey!",
        litelm_logging_obj=litelm_logging_obj,
        metadata=metadata,
    )

    time.sleep(3)
    assert litelm_logging_obj._get_trace_id(service_name="langfuse") is not None

    ## if existing_trace_id exists
    if langfuse_existing_trace_id is not None:
        assert (
            litelm_logging_obj._get_trace_id(service_name="langfuse")
    ...
    ## if no trace_id or existing_trace_id is provided, use litelm_trace_id
    else:
        assert (
            litelm_logging_obj._get_trace_id(service_name="langfuse")
            == litelm_logging_obj.litelm_trace_id
        )
...
def test_convert_model_response_object():
    ...
    args = {
        ...
        "model_response_object": litelm.ModelResponse(
            id="chatcmpl-b88ce43a-7bfc-437c-b8cc-e90d59372cfb",
            choices=[
                litelm.Choices(
                    finish_reason="stop",
                    index=0,
                    message=litelm.Message(content="default", role="assistant"),
                )
            ],
            created=1719376241,
    ...
    try:
        litelm.convert_to_model_response_object(**args)
...
def test_parse_content_for_reasoning(content, expected_reasoning, expected_content):
    assert litelm.utils._parse_content_for_reasoning(content) == (
        expected_reasoning,
        expected_content,
    )
...
def test_supports_response_schema(model, expected_bool):
    ...
    from litelm.utils import supports_response_schema
...
def test_supports_vision(model, expected_bool):
    ...
    """
    from litelm.utils import supports_vision
...
def test_usage_object_null_tokens():
    ...
    Fixes https://github.com/BerriAI/litelm/issues/5096
    """
    usage_obj = litelm.Usage(prompt_tokens=2, completion_tokens=None, total_tokens=2)
...
def test_is_base64_encoded():
    ...
    from litelm.utils import is_base64_encoded
...
def test_async_http_handler_force_ipv4(mock_async_client):
    """
    Test AsyncHTTPHandler when litelm.force_ipv4 is True

    This is prod test - we need to ensure that httpx always uses ipv4 when litelm.force_ipv4 is True
    """
    import httpx
    import ssl
    ...
    litelm.force_ipv4 = True
    litelm.disable_aiohttp_transport = True
...
def test_supports_audio_input(model, expected_bool):
    ...
    from litelm.utils import supports_audio_input, supports_audio_output
...
def test_validate_chat_completion_user_messages(messages, expected_bool):
    from litelm.utils import validate_chat_completion_user_messages
...
def test_validate_chat_completion_tool_choice(tool_choice, expected_bool):
    from litelm.utils import validate_chat_completion_tool_choice
...
def test_models_by_provider():
    ...
    from litelm import models_by_provider

    providers = set()
    for k, v in litelm.model_cost.items():
        if "_" in v["litelm_provider"] and "-" in v["litelm_provider"]:
            continue
        elif k == "sample_spec":
            continue
        elif (
            v["litelm_provider"] == "sagemaker"
            or v["litelm_provider"] == "bedrock_converse"
        ):
            continue
            ...
            providers.add(v["litelm_provider"])
...
@pytest.mark.parametrize(
    "litelm_params, disable_end_user_cost_tracking, expected_end_user_id",
    [
        ({}, False, None),
...
def test_get_end_user_id_for_cost_tracking(
    ...
    from litelm.utils import get_end_user_id_for_cost_tracking

    litelm.disable_end_user_cost_tracking = disable_end_user_cost_tracking
    assert (
        get_end_user_id_for_cost_tracking(litelm_params=litelm_params)
        == expected_end_user_id
    )


@pytest.mark.parametrize(
    "litelm_params, enable_end_user_cost_tracking_prometheus_only, expected_end_user_id",
    [
        ({}, True, None),
...
def test_get_end_user_id_for_cost_tracking_prometheus_only(
    ...
    assert (
        get_end_user_id_for_cost_tracking(
            litelm_params=litelm_params, service_type="prometheus"
        )
        == expected_end_user_id
    )
...
@pytest.mark.parametrize(
    "litelm_params, expected_end_user_id",
    [
        # Test with only metadata field (old behavior)
        ({"metadata": {"user_api_key_end_user_id": "user_from_metadata"}}, "user_from_metadata"),
        # Test with only litelm_metadata field (new behavior)
        ({"litelm_metadata": {"user_api_key_end_user_id": "user_from_litelm_metadata"}}, "user_from_litelm_metadata"),
        ...
        ({"metadata": {"user_api_key_end_user_id": "user_from_metadata"}, 
          "litelm_metadata": {"user_api_key_end_user_id": "user_from_litelm_metadata"}}, 
         "user_from_metadata"),
        # Test with user_api_key_end_user_id in litelm_params (should take precedence over metadata)
        ({"user_api_key_end_user_id": "user_from_params", 
          "metadata": {"user_api_key_end_user_id": "user_from_metadata"}}, 
         "user_from_params"),
        # Test with empty metadata but valid litelm_metadata
        ({"metadata": {}, "litelm_metadata": {"user_api_key_end_user_id": "user_from_litelm_metadata"}}, 
         "user_from_litelm_metadata"),
...
def test_get_end_user_id_for_cost_tracking_metadata_handling(
    ...
    """
    Test that get_end_user_id_for_cost_tracking correctly handles both metadata and litelm_metadata
    fields using the get_litelm_metadata_from_kwargs helper function.
    """
    from litelm.utils import get_end_user_id_for_cost_tracking
    
    # Ensure cost tracking is enabled for this test
    litelm.disable_end_user_cost_tracking = False
    
    result = get_end_user_id_for_cost_tracking(litelm_params=litelm_params)
...
def test_is_prompt_caching_enabled_return_default_image_dimensions():
    ...
    """
    with patch("litelm.utils.token_counter") as mock_token_counter:
        litelm.utils.is_prompt_caching_valid_prompt(
            messages=[
                {
                    "role": "user",
                    "content": [
...
def test_token_counter_with_image_url_with_detail_high():
    ...
    """
    from litelm.constants import DEFAULT_IMAGE_TOKEN_COUNT
    from litelm._logging import verbose_logger
    import logging

    verbose_logger.setLevel(logging.DEBUG)

    _tokens = litelm.utils.token_counter(
        messages=[
            {
                "role": "user",
                "content": [
...
def test_fireworks_ai_document_inlining():
    ...
    - supports_vision
    """
    from litelm.utils import supports_pdf_input, supports_vision
...
def test_logprobs_type():
    from litelm.types.utils import Logprobs
...
def test_get_valid_models_openai_proxy(monkeypatch):
    from litelm.utils import get_valid_models
    import litelm

    litelm._turn_on_debug()

    monkeypatch.setenv("LITELLM_PROXY_API_KEY", "sk-1234")
    monkeypatch.setenv("LITELLM_PROXY_API_BASE", "https://litelm-api.up.railway.app/")
    ...
    with patch.object(
        ...
        assert "litelm_proxy/gpt-4o" in valid_models
...
def test_get_valid_models_fireworks_ai(monkeypatch):
    ...
    monkeypatch.setattr(litelm, "provider_list", ["fireworks_ai"])
...
def test_pick_cheapest_chat_model_from_llm_provider():
    from litelm.litelm_core_utils.llm_request_utils import (
...
def test_get_num_retries(num_retries):
    from litelm.utils import _get_wrapper_num_retries
...
def test_add_custom_logger_callback_to_specific_event(monkeypatch):
    from litelm.utils import _add_custom_logger_callback_to_specific_event

    monkeypatch.setattr(litelm, "success_callback", [])
    monkeypatch.setattr(litelm, "failure_callback", [])

    _add_custom_logger_callback_to_specific_event("langfuse", "success")

    assert len(litelm.success_callback) == 1
    assert len(litelm.failure_callback) == 0
...
def test_add_custom_logger_callback_to_specific_event_e2e(monkeypatch):
    ...
    monkeypatch.setattr(litelm, "success_callback", [])
    monkeypatch.setattr(litelm, "failure_callback", [])
    monkeypatch.setattr(litelm, "callbacks", [])

    litelm.success_callback = ["humanloop"]

    curr_len_success_callback = len(litelm.success_callback)
    curr_len_failure_callback = len(litelm.failure_callback)
    ...
    assert len(litelm.success_callback) == curr_len_success_callback
    assert len(litelm.failure_callback) == curr_len_failure_callback
...
def test_custom_logger_exists_in_callbacks_individual_functions(monkeypatch):
    ...
    for list_name in [
        ...
        monkeypatch.setattr(litelm, list_name, [])
    ...
    litelm.success_callback.append(mock_logger)
    ...
    litelm._async_success_callback.append(mock_logger)
    assert _custom_logger_class_exists_in_success_callbacks(mock_logger) == True
    assert _custom_logger_class_exists_in_failure_callbacks(mock_logger) == False
    ...
    litelm.failure_callback.append(mock_logger)
    assert _custom_logger_class_exists_in_success_callbacks(mock_logger) == False
    ...
    litelm._async_failure_callback.append(mock_logger)
    assert _custom_logger_class_exists_in_success_callbacks(mock_logger) == False
    assert _custom_logger_class_exists_in_failure_callbacks(mock_logger) == True
...
async def test_add_custom_logger_callback_to_specific_event_with_duplicates(
    ...
    """
    from litelm.integrations.langfuse.langfuse_prompt_management import (
        LangfusePromptManagement,
    )

    # Reset all callback lists
    monkeypatch.setattr(litelm, "callbacks", [])
    monkeypatch.setattr(litelm, "_async_success_callback", [])
    monkeypatch.setattr(litelm, "_async_failure_callback", [])
    monkeypatch.setattr(litelm, "success_callback", [])
    monkeypatch.setattr(litelm, "failure_callback", [])

    # Add logger to both success_callback and _async_success_callback
    langfuse_logger = LangfusePromptManagement()
    litelm.success_callback.append(langfuse_logger)
    litelm._async_success_callback.append(langfuse_logger)

    # Get initial lengths
    initial_success_callback_len = len(litelm.success_callback)
    initial_async_success_callback_len = len(litelm._async_success_callback)
    ...
    assert len(litelm.success_callback) == initial_success_callback_len
    assert len(litelm._async_success_callback) == initial_async_success_callback_len
...
async def test_add_custom_logger_callback_to_specific_event_with_duplicates_callbacks(
    ...
    # Add logger to both success_callback and _async_success_callback
    langfuse_logger = LangfusePromptManagement()
    litelm.callbacks.append(langfuse_logger)
    ...
    # Assert no new callbacks were added
    initial_callbacks_len = len(litelm.callbacks)
    initial_async_success_callback_len = len(litelm._async_success_callback)
    initial_success_callback_len = len(litelm.success_callback)
    print(
        f"Num callbacks before: litelm.callbacks: {len(litelm.callbacks)}, litelm._async_success_callback: {len(litelm._async_success_callback)}, litelm.success_callback: {len(litelm.success_callback)}"
    )
    ...
    assert len(litelm.callbacks) == initial_callbacks_len
    assert len(litelm._async_success_callback) == initial_async_success_callback_len
    assert len(litelm.success_callback) == initial_success_callback_len

    print(
        f"Num callbacks after 10 mock calls: litelm.callbacks: {len(litelm.callbacks)}, litelm._async_success_callback: {len(litelm._async_success_callback)}, litelm.success_callback: {len(litelm.success_callback)}"
    )


def test_add_custom_logger_callback_to_specific_event_e2e_failure(monkeypatch):
    from litelm.integrations.openmeter import OpenMeterLogger
    ...
    litelm.failure_callback = ["openmeter"]
    ...
    assert any(
        isinstance(callback, OpenMeterLogger) for callback in litelm.failure_callback
    )


@pytest.mark.asyncio
async def test_wrapper_kwargs_passthrough():
    from litelm.utils import client
    from litelm.litelm_core_utils.litelm_logging import (
    ...
    # get litelm logging object
    litelm_logging_obj: LiteLLMLoggingObject = mock_original.call_args.kwargs.get(
        "litelm_logging_obj"
    )
    assert litelm_logging_obj is not None

    print(
        f"litelm_logging_obj.model_call_details: {litelm_logging_obj.model_call_details}"
    )

    # get base model
    assert (
        litelm_logging_obj.model_call_details["litelm_params"]["base_model"]
        == "gpt-4o-mini"
    )


def test_dict_to_response_format_helper():
    from litelm.llms.base_llm.base_utils import _dict_to_response_format_helper
...
from litelm.integrations.custom_guardrail import CustomGuardrail
from litelm.utils import get_applied_guardrails
...
def test_get_applied_guardrails(test_case):
    ...
    # Setup
    litelm.callbacks = test_case["callbacks"]
...
def test_should_use_cohere_v1_client(endpoint, params, expected_bool):
    assert litelm.utils.should_use_cohere_v1_client(endpoint, params) == expected_bool


def test_add_openai_metadata():
    from litelm.utils import add_openai_metadata

    metadata = {
        "user_api_key_end_user_id": "123",
        "hidden_params": {"api_key": "123"},
        "litelm_parent_otel_span": MagicMock(),
        "none-val": None,
        "int-val": 1,
        "dict-val": {"a": 1, "b": 2},
    }
...
def test_message_object():
    from litelm.types.utils import Message
...
def test_delta_object():
    from litelm.types.utils import Delta
...
def test_claude_3_7_sonnet_supports_pdf_input(model, expected_bool):
    from litelm.utils import supports_pdf_input
...
def test_get_valid_models_from_provider_cache_invalidation(monkeypatch):
    ...
    _model_cache.set_cached_model_info(
        "openai", litelm_params=None, available_models=["gpt-4o-mini"]
    )
...
def test_get_valid_models_from_dynamic_api_key():
    ...
    """
    from litelm.utils import get_valid_models
    from litelm.types.router import CredentialLiteLLMParams
...
def test_get_whitelisted_models():
    ...
    Create whitelist to prevent naming regressions for older litelm versions.
    """
    whitelisted_models = []
    for model, info in litelm.model_cost.items():
        if info.get("litelm_provider") == "bedrock" and info.get("mode") == "chat":
...
def test_completion_with_no_model():
    ...
    with pytest.raises(TypeError):
        response = litelm.completion(messages=[{"role": "user", "content": "Hello, how are you?"}])


def test_get_base_model_from_metadata():
    """
    Test _get_base_model_from_metadata function with both metadata and litelm_metadata.
    This ensures cost tracking works for both Chat Completions API and Responses API.

    Related issue: https://github.com/BerriAI/litelm/issues/16772
    """
    from litelm.utils import _get_base_model_from_metadata
    ...
    # Test 2: base_model in litelm_metadata (Responses API and generic API calls pattern)
    model_call_details_with_litelm_metadata = {
        "litelm_params": {
            "litelm_metadata": {
                "model_info": {
    ...
    result = _get_base_model_from_metadata(model_call_details_with_litelm_metadata)
    assert result == "azure/gpt-5-mini", f"Expected 'azure/gpt-5-mini', got {result}"

    # Test 3: base_model in litelm_params (direct base_model)
    ...
    # Test 4: metadata takes precedence over litelm_metadata
    model_call_details_with_both = {
        "litelm_params": {
            ...
            "litelm_metadata": {
                "model_info": {
                    "base_model": "azure/gpt-4-from-litelm-metadata"
                }
            }
        }
    }
