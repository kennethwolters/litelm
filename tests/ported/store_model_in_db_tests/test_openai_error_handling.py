...
def test_missing_model_parameter_curl(curl_command):
    ...
    assert "litelm.BadRequestError" in response["error"]["message"]
...
async def test_chat_completion_bad_model_with_spend_logs():
    ...
    with httpx.Client() as client:
        ...
        # Extract the litelm call ID from headers
        litelm_call_id = response.headers.get("x-litelm-call-id")
        print(f"Status code: {response.status_code}")
        print(f"Headers: {dict(response.headers)}")
        print(f"LiteLLM Call ID: {litelm_call_id}")
    ...
    url = "http://0.0.0.0:4000/spend/logs?request_id=" + litelm_call_id
    ...
    with httpx.Client() as client:
        ...
        log_entry = spend_logs[0]  # Should be the specific log for our litelm_call_id

        # Verify the structure of the log entry
        assert log_entry["request_id"] == litelm_call_id
        ...
        assert "litelm.BadRequestError" in error_info["error_message"]
