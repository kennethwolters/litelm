...
from litelm.proxy.proxy_server import app, ProxyLogging
...
async def test_master_key_not_inserted(test_client):
    ...
    result = await prisma_client.db.litelm_verificationtoken.find_many()
    print(result)

    # The expectation is that no token (or unintended record) is added on startup.
    assert len(result) == 0, (
        "SECURITY ALERT SECURITY ALERT SECURITY ALERT: Expected no record in the litelm_verificationtoken table. On startup - the master key should NOT be Inserted into the DB."
