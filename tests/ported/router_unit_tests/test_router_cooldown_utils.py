...
from litelm.router_utils.cooldown_callbacks import router_cooldown_event_callback
...
from litelm.router_utils.router_callbacks.track_deployment_metrics import (
...
from litelm import Router
from litelm.router_utils.cooldown_handlers import _should_cooldown_deployment
...
async def test_router_cooldown_event_callback_no_deployment():
    ...
    await router_cooldown_event_callback(
        litelm_router_instance=mock_router,
    ...
    return Router(
        model_list=[
            ...
            },
            {
                ...
                "litelm_params": {"model": "openai/test_deployment"},
                "model_id": "test_deployment_2",
            },
            {
                "model_name": "test_deployment",
                "litelm_params": {"model": "openai/test_deployment-2"},
...
def test_should_run_cooldown_logic(testing_litelm_router):
    testing_litelm_router.disable_cooldowns = True
    # don't run cooldown logic if disable_cooldowns is True
    assert (
        _should_run_cooldown_logic(
            testing_litelm_router, "test_deployment", 500, Exception("Test")
        )
        is False
    )

    # don't cooldown if deployment is None
    testing_litelm_router.disable_cooldowns = False
    assert (
        _should_run_cooldown_logic(testing_litelm_router, None, 500, Exception("Test"))
        is False
    )

    # don't cooldown if it's a provider default deployment
    testing_litelm_router.provider_default_deployment_ids = ["test_deployment"]
    assert (
        _should_run_cooldown_logic(
            testing_litelm_router, "test_deployment", 500, Exception("Test")
        )
...
def test_should_cooldown_deployment_rate_limit_error(testing_litelm_router):
    """
    ...
    _exception = litelm.exceptions.RateLimitError(
        "Rate limit", "openai", "gpt-3.5-turbo"
    )
    assert (
        _should_cooldown_deployment(
            testing_litelm_router, "test_deployment", 429, _exception
        )
        is True
    )


def test_should_cooldown_deployment_auth_limit_error(testing_litelm_router):
    """
    ...
    _exception = litelm.exceptions.AuthenticationError(
        "Unauthorized", "openai", "gpt-3.5-turbo"
    )
    assert (
        _should_cooldown_deployment(
            testing_litelm_router, "test_deployment", 401, _exception
        )
        is True
    )
...
@pytest.mark.asyncio
async def test_should_cooldown_deployment(testing_litelm_router):
    """
    Cooldown a deployment if it fails 60% of requests in 1 minute - DEFAULT threshold is 50%
    """
    from litelm._logging import verbose_router_logger
    import logging
    ...
    available_deployment = testing_litelm_router.get_available_deployment(
    ...
    for _ in range(40):
        increment_deployment_successes_for_current_minute(
            litelm_router_instance=testing_litelm_router, deployment_id=deployment_id
        )
    ...
    for _ in range(41):
        tasks.append(
            testing_litelm_router.acompletion(
                model=deployment_id,
    ...
    assert (
        _should_cooldown_deployment(
            testing_litelm_router, deployment_id, 500, Exception("Test")
        )
        is True
    )
...
def test_increment_deployment_successes_for_current_minute_does_not_write_to_redis(
    ...
    from litelm.caching.in_memory_cache import InMemoryCache
    from litelm.router_utils.router_callbacks.track_deployment_metrics import (
        increment_deployment_successes_for_current_minute,
    )
    ...
    testing_litelm_router.cache = DualCache(
        redis_cache=mock_redis_cache, in_memory_cache=InMemoryCache()
    )

    # Call the function we're testing
    increment_deployment_successes_for_current_minute(
        litelm_router_instance=testing_litelm_router, deployment_id="test_deployment"
    )
    ...
    print(
        ...
        testing_litelm_router.cache.in_memory_cache.cache_dict,
    )
    assert (
        testing_litelm_router.cache.in_memory_cache.get_cache(
...
@patch(
    "litelm.router_utils.cooldown_handlers.get_deployment_successes_for_current_minute"
)
@patch(
    "litelm.router_utils.cooldown_handlers.get_deployment_failures_for_current_minute"
)
def test_should_cooldown_high_traffic_all_fails(mock_failures, mock_successes, router):
    # Simulate 10 failures, 0 successes
    from litelm.constants import SINGLE_DEPLOYMENT_TRAFFIC_FAILURE_THRESHOLD
...
def test_is_cooldown_required_empty_string_exception_status(testing_litelm_router):
    """
    Test that _is_cooldown_required returns False when exception_status is an empty string
    """
    result = _is_cooldown_required(
        litelm_router_instance=testing_litelm_router,
        model_id="test_deployment",
        exception_status="",
    )
...
def test_should_cooldown_deployment_minimum_request_threshold(testing_litelm_router):
    """
    ...
    """
    from litelm.constants import DEFAULT_FAILURE_THRESHOLD_MINIMUM_REQUESTS
    ...
    _exception = litelm.exceptions.InternalServerError(
        "Internal error", "openai", "gpt-3.5-turbo"
    )

    # With only 1 request, should NOT cooldown (below minimum threshold)
    should_cooldown = _should_cooldown_deployment(
        testing_litelm_router, deployment_id, 500, _exception
    )
    assert (
        should_cooldown is False
