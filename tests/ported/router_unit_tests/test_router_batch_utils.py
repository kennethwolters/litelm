...
from litelm.router_utils.batch_utils import (
...
def test_router_metadata_variable_name():
    ...
    assert (
        _get_router_metadata_variable_name(function_name="batch") == "litelm_metadata"
    )
    assert (
        _get_router_metadata_variable_name(function_name="acreate_file") == "litelm_metadata"
    )
    assert (
        _get_router_metadata_variable_name(function_name="aget_file") == "litelm_metadata"
    )


def test_non_json_input():
    """Test that replace_model_in_jsonl returns original content for non-JSON input"""
    from litelm.router_utils.batch_utils import replace_model_in_jsonl
...
def test_should_replace_model_in_jsonl():
    ...
    from litelm.router_utils.batch_utils import should_replace_model_in_jsonl
