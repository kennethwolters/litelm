...
def test_default_deployment_isolation():
    ...
    2. litelm_params dict
    
    Deeper nested objects are intentionally shared for performance (safe because
    the router only modifies the 'model' field at litelm_params level).
    
    Critical behavior verified:
    1. Each deployment gets independent model value
    2. Original default_deployment unchanged for litelm_params fields
    3. Shared fields (api_key) accessible in all copies
    4. Adding new litelm_params fields is isolated per deployment
    ...
    assert deployment1["litelm_params"]["model"] == "custom-model-1"  # type: ignore
    assert deployment2["litelm_params"]["model"] == "custom-model-2"  # type: ignore
    
    # Assert: Original default_deployment must remain unchanged (not mutated by requests)
    assert router.default_deployment["litelm_params"]["model"] == "gpt-3.5-turbo"  # type: ignore
    
    # Assert: Shared fields should still be accessible in all copies
    assert deployment1["litelm_params"]["api_key"] == "test-key"  # type: ignore
    assert deployment2["litelm_params"]["api_key"] == "test-key"  # type: ignore
    
    # Assert: Modifying litelm_params in one deployment doesn't affect others
    # This tests the shallow copy properly isolated the litelm_params dict level
    deployment1["litelm_params"]["temperature"] = 0.9  # type: ignore
    assert "temperature" not in deployment2["litelm_params"]  # type: ignore
    assert "temperature" not in router.default_deployment["litelm_params"]  # type: ignore
    
    # Assert: Deep nested objects ARE shared (intentional trade-off for 100x perf gain)
    # Safe because router only modifies top-level litelm_params fields
    deployment1["litelm_params"]["custom_config"]["nested_setting"] = "modified"  # type: ignore
    assert deployment2["litelm_params"]["custom_config"]["nested_setting"] == "modified"  # type: ignore
    assert router.default_deployment["litelm_params"]["custom_config"]["nested_setting"] == "modified"  # type: ignore
