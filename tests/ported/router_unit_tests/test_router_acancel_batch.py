...
async def test_router_acancel_batch(router):
    """Test that router.acancel_batch() calls litelm.acancel_batch with correct params"""
    mock_response = MagicMock()
    mock_response.id = "batch_123"
    mock_response.status = "cancelled"
    
    with patch.object(litelm, "acancel_batch", new_callable=AsyncMock) as mock_cancel:
