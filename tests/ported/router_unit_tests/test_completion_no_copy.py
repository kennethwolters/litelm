...
Verifies that spreading deployment["litelm_params"] directly (without copy)
...
async def test_acompletion_deployment_not_mutated():
    ...
    Optimization: Remove deployment["litelm_params"].copy() in _acompletion
    ...
    original_params = deployment_before.litelm_params.model_dump()
    ...
    assert deployment_after.litelm_params.model_dump() == original_params
...
def test_completion_deployment_not_mutated():
    ...
    Optimization: Remove deployment["litelm_params"].copy() in _completion
    since data is only read and spread into input_kwargs dict.
    """
    router = Router(
        model_list=[
    ...
    with patch("litelm.completion", new_callable=Mock) as mock_completion:
        from litelm import ModelResponse
