...
def test_initialize_deployment_for_pass_through_success(reusable_credentials):
    ...
    """
    from litelm.litelm_core_utils.credential_accessor import CredentialAccessor
    ...
    if not reusable_credentials:
        ...
        litelm_params = LiteLLM_Params(
            model="vertex_ai/test-model",
            litelm_credential_name="vertex_credentials",
            use_in_pass_through=True,
        )
...
def test_initialize_deployment_for_pass_through_missing_params():
    ...
    with pytest.raises(
        ValueError,
        match="vertex_project, and vertex_location must be set in litelm_params for pass-through endpoints",
    ):
        router._initialize_deployment_for_pass_through(
            deployment=deployment,
            custom_llm_provider="vertex_ai",
