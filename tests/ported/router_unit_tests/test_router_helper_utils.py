...
from litelm.types.router import Deployment, LiteLLM_Params
...
def test_routing_strategy_init(model_list):
    ...
    from litelm.types.router import RoutingStrategy
...
def test_routing_strategy_init_invalid_strategy(model_list):
    ...
    See: https://github.com/BerriAI/litelm/issues/11330
...
def test_print_deployment(model_list):
    ...
    assert 10 * "*" in printed_deployment["litelm_params"]["api_key"]
...
def test_print_deployment_with_redact_enabled(model_list):
    ...
    original_setting = litelm.redact_user_api_key_info
    try:
        litelm.redact_user_api_key_info = True
        printed_deployment = router.print_deployment(deployment)

        assert "*" in printed_deployment["litelm_params"]["aws_access_key_id"]
        assert "*" in printed_deployment["litelm_params"]["aws_secret_access_key"]
        assert "us-west-2" == printed_deployment["litelm_params"]["aws_region_name"]
    finally:
        litelm.redact_user_api_key_info = original_setting
...
async def test_image_generation(model_list, sync_mode):
    ...
    from litelm.types.utils import ImageResponse
...
async def test_router_schedule_atext_completion(model_list):
    ...
    from litelm.types.utils import TextCompletionResponse
...
def test_update_kwargs_with_default_litelm_params(model_list):
    """Test if the '_update_kwargs_with_default_litelm_params' function is working correctly"""
    router = Router(
        model_list=model_list,
        default_litelm_params={"api_key": "test", "metadata": {"key": "value"}},
    )
    kwargs: dict = {"metadata": {"key2": "value2"}}
    router._update_kwargs_with_default_litelm_params(kwargs=kwargs)
...
@pytest.mark.parametrize(
    ...
    [
        ("mock_testing_fallbacks", litelm.InternalServerError),
        ("mock_testing_context_fallbacks", litelm.ContextWindowExceededError),
        ("mock_testing_content_policy_fallbacks", litelm.ContentPolicyViolationError),
    ],
)
...
def test_deployment_callback_respects_cooldown_time(model_list):
    ...
    with patch("litelm.router._set_cooldown_deployments") as mock_set:
...
async def test_routing_strategy_pre_call_checks(model_list, sync_mode):
    ...
    if sync_mode:
        ...
        with patch.object(
            ...
            AsyncMock(
                side_effect=litelm.RateLimitError(
            ...
            try:
                ...
                assert isinstance(e, litelm.RateLimitError)
...
def test_upsert_deployment(model_list):
    ...
    deployment.litelm_params.model = "gpt-4o"
...
def test_track_deployment_metrics(model_list):
    ...
    from litelm.types.utils import ModelResponse
...
@pytest.mark.parametrize(
    ...
    [
        (litelm.exceptions.BadRequestError, "BadRequestError", 3),
        (litelm.exceptions.AuthenticationError, "AuthenticationError", 4),
        (litelm.exceptions.RateLimitError, "RateLimitError", 6),
        (
            litelm.exceptions.ContentPolicyViolationError,
...
def test_get_num_retries_from_retry_policy(
    ...
    from litelm.router import RetryPolicy
...
def test_get_allowed_fails_from_policy(
    ...
    from litelm.types.router import AllowedFailsPolicy
...
def test_initialize_alerting(model_list):
    ...
    from litelm.types.router import AlertingConfig
    from litelm.integrations.SlackAlerting.slack_alerting import SlackAlerting
    ...
    for callback in litelm.callbacks:
...
def test_discard(model_list):
    ...
    assert len(litelm.callbacks) == 0
    assert len(litelm.success_callback) == 0
    assert len(litelm.failure_callback) == 0
    assert len(litelm._async_success_callback) == 0
    assert len(litelm._async_failure_callback) == 0
    assert len(litelm.input_callback) == 0
    assert len(litelm.service_callback) == 0
...
def test_pass_through_assistants_endpoint_factory(model_list):
    ...
    router._pass_through_assistants_endpoint_factory(
        original_function=litelm.acreate_assistants,
        custom_llm_provider="openai",
        client=None,
        **{},
    )
...
def test_factory_function(model_list):
    ...
    router = Router(model_list=model_list)
    router.factory_function(litelm.acreate_assistants)
...
def test_get_deployment_by_litelm_model(model_list):
    """Test if the 'get_deployment_by_litelm_model' function is working correctly"""
    router = Router(model_list=model_list)
    deployment = router._get_deployment_by_litelm_model(model="gpt-3.5-turbo")
    assert deployment is not None
...
#     from litelm.router_utils.pattern_match_deployments import PatternMatchRouter
#     import re
...
#         matched_pattern=match, litelm_deployment_litelm_model="openai/*"
#     )
#     assert updated_model == "openai/fo::hi:static::hello"


@pytest.mark.parametrize(
    "user_request_model, model_name, litelm_model, expected_model",
    [
...
def test_pattern_match_deployment_set_model_name(
    ...
    updated_model = pattern_router.set_deployment_model_name(match, litelm_model)
        ...
        deployments=[
            {
                ...
                "litelm_params": {"model": litelm_model},
            }
        ],
    )

    for model in updated_models:
        assert model["litelm_params"]["model"] == expected_model
...
async def test_pass_through_moderation_endpoint_factory(model_list):
    ...
    response = await router._pass_through_moderation_endpoint_factory(
        original_function=litelm.amoderation,
...
async def test_async_callback_filter_deployments(model_list):
    from litelm.router_strategy.budget_limiter import RouterBudgetLimiting
...
def test_mock_router_testing_params_str_to_bool_conversion(
    ...
    from litelm.types.router import MockRouterTestingParams
...
def test_is_auto_router_deployment(model_list):
    ...
    litelm_params_auto = LiteLLM_Params(model="auto_router/my-auto-router")
    assert router._is_auto_router_deployment(litelm_params_auto) is True

    # Test case 2: Model doesn't start with "auto_router/" - should return False
    litelm_params_regular = LiteLLM_Params(model="gpt-3.5-turbo")
    assert router._is_auto_router_deployment(litelm_params_regular) is False

    # Test case 3: Model is empty string - should return False
    litelm_params_empty = LiteLLM_Params(model="")
    assert router._is_auto_router_deployment(litelm_params_empty) is False

    # Test case 4: Model contains "auto_router/" but doesn't start with it - should return False
    litelm_params_contains = LiteLLM_Params(model="prefix_auto_router/something")
    assert router._is_auto_router_deployment(litelm_params_contains) is False


@patch("litelm.router_strategy.auto_router.auto_router.AutoRouter")
...
def test_generate_model_id_with_deployment_model_name(model_list):
    ...
    # Test case 1: Normal case with valid model_group and litelm_params
    model_group = "gpt-4.1"
    litelm_params = {
        "model": "gpt-4.1",
        "api_key": "test_key",
    ...
    try:
        result = router._generate_model_id(
            model_group=model_group, litelm_params=litelm_params
        )
    ...
    try:
        result = router._generate_model_id(
            model_group=None, litelm_params=litelm_params
        )
        pytest.fail(
    ...
    # Test case 3: Edge case with None key in litelm_params
    litelm_params_with_none_key = {
        "model": "gpt-4.1",
        "api_key": "test_key",
    ...
    try:
        result = router._generate_model_id(
            model_group=model_group, litelm_params=litelm_params_with_none_key
        )
        assert isinstance(result, str)
        assert len(result) > 0
        print(f"✓ Success with None key in litelm_params: {result}")
    except Exception as e:
        pytest.fail(f"Failed with None key in litelm_params: {e}")

    # Test case 4: Edge case with empty litelm_params
    try:
        result = router._generate_model_id(model_group=model_group, litelm_params={})
        assert isinstance(result, str)
        assert len(result) > 0
        print(f"✓ Success with empty litelm_params: {result}")
    except Exception as e:
        pytest.fail(f"Failed with empty litelm_params: {e}")
...
def test_handle_clientside_credential_with_deployment_model_name(model_list):
    ...
    deployment = {
        "model_name": "gpt-4.1",
        "litelm_params": {"model": "gpt-4.1", "api_key": "test_key"},
    }
    ...
    # Mock dynamic_litelm_params that would be returned by get_dynamic_litelm_params
    dynamic_litelm_params = {
        "api_key": "client_side_key",
        "api_base": "https://api.openai.com/v1",
    }
    ...
    try:
        ...
        result = router._generate_model_id(
            model_group=model_group, litelm_params=dynamic_litelm_params
        )
        assert isinstance(result, str)
        assert len(result) > 0
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("_ageneric_api_call_with_fallbacks", "litelm_metadata"),
        ("batch", "litelm_metadata"),
        ("completion", "metadata"),
        ("acreate_file", "litelm_metadata"),
        ("aget_file", "litelm_metadata"),
    ],
)
def test_handle_clientside_credential_metadata_loading(
    ...
    # Verify the litelm_params contain the clientside credentials
    assert result_deployment.litelm_params.api_key == "client_side_key"
    assert result_deployment.litelm_params.api_base == "https://api.openai.com/v1"
    ...
    # For _ageneric_api_call_with_fallbacks/batch, it should use "litelm_metadata"
    if function_name == "acompletion":
        assert "metadata" in kwargs
        assert "litelm_metadata" not in kwargs
        ...
        assert "litelm_metadata" in kwargs
        # Note: acompletion would not have litelm_metadata, but other functions might have both
...
def test_handle_clientside_credential_metadata_variable_name(
    ...
    from litelm.router_utils.batch_utils import _get_router_metadata_variable_name
...
def test_handle_clientside_credential_with_responses_function(model_list):
    ...
    # Mock kwargs with clientside credentials and litelm_metadata (for responses function)
    kwargs = {
        "api_key": "client_side_key",
        "api_base": "https://api.openai.com/v1",
        "litelm_metadata": {
    ...
    print(
        "✓ Success with _ageneric_api_call_with_fallbacks function name and litelm_metadata"
    )
...
def test_get_metadata_variable_name_from_kwargs(model_list):
    ...
    # Test case 1: kwargs contains litelm_metadata - should return "litelm_metadata"
    kwargs_with_litelm_metadata = {
        "litelm_metadata": {"user": "test"},
        "metadata": {"other": "data"}
    }
    result = router._get_metadata_variable_name_from_kwargs(kwargs_with_litelm_metadata)
    assert result == "litelm_metadata"
...
async def test_asearch_with_fallbacks(search_tools):
    ...
    """
    from litelm.llms.base_llm.search.transformation import SearchResponse, SearchResult
...
async def test_asearch_with_fallbacks_helper_missing_search_provider():
    ...
    is found but doesn't have search_provider in its litelm_params.
    """
    ...
    with pytest.raises(ValueError, match="search_provider not found in litelm_params"):
        await router._asearch_with_fallbacks_helper(
            model="bad-tool",
            original_generic_function=mock_original_function,
            query="test query"
...
def test_get_first_default_fallback():
    ...
    model_list = [
        {
            "model_name": "gpt-3.5-turbo",
            "litelm_params": {"model": "gpt-3.5-turbo", "api_key": "fake-key"},
        }
    ]
...
def test_resolve_model_name_from_model_id():
    ...
    # Test case 3: model_id matches litelm_params.model exactly
    model_list = [
        {
...
def test_get_router_model_info_with_deployment_object():
    ...
    router = Router(
        model_list=[
            {
                "model_name": "gpt-4",
                "litelm_params": {"model": "gpt-4", "api_key": "test-key"},
                "model_info": {"id": "test-id"},
            }
        ]
    )
    ...
    assert isinstance(deployment.litelm_params, LiteLLM_Params)
