...
from litelm import Router, CustomLogger
...
async def test_audio_speech_router(mode):
    ...
    from litelm.llms.openai.openai import HttpxBinaryResponseContent
...
async def test_rerank_endpoint(model_list):
    from litelm.types.utils import RerankResponse
...
async def test_moderation_endpoint_with_api_base():
    ...
    with patch("litelm.main.openai_chat_completions._get_openai_client") as mock_get_client:
...
async def test_router_with_empty_choices(model_list):
    """
    https://github.com/BerriAI/litelm/issues/8306
    """
    router = Router(model_list=model_list)
...
async def test__aadapter_completion():
    ...
    """
    # Create a mock response for litelm.aadapter_completion
    mock_response = {
        "id": "adapter_resp_123",
        "object": "adapter.completion",
        "created": 1677858242,
    ...
    # Create a router with a mocked litelm.aadapter_completion
    with patch(
        "litelm.aadapter_completion", new_callable=AsyncMock
        ...
        # Verify litelm.aadapter_completion was called with the right parameters
