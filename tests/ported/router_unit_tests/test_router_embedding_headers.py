...
default_litelm_params (including headers) to be propagated.
"""
import os
import sys
...
class TestRouterEmbeddingHeaders:
    ...
    def test_embedding_calls_update_kwargs_before_fallbacks(self):
        ...
        with patch.object(
            ...
            with patch("litelm.embedding") as mock_litelm_embedding:
                mock_litelm_embedding.return_value = MagicMock(
    ...
    async def test_aembedding_calls_update_kwargs_before_fallbacks(self):
        ...
        with patch.object(
            ...
            with patch(
                "litelm.aembedding", new_callable=AsyncMock
            ) as mock_litelm_aembedding:
                mock_litelm_aembedding.return_value = MagicMock(
                    data=[{"embedding": [0.1, 0.2, 0.3]}]
                )
    ...
    def test_embedding_propagates_default_litelm_params(self):
        """
        Test that embedding calls properly propagate default_litelm_params including headers.

        This is the main fix - ensuring that headers set in default_litelm_params
        ...
        # Create router with default_litelm_params containing headers
        router = Router(
            model_list=model_list,
            default_litelm_params={
        ...
        with patch("litelm.embedding") as mock_litelm_embedding:
            ...
            # Verify that litelm.embedding was called with the headers
            mock_litelm_embedding.assert_called_once()
            call_kwargs = mock_litelm_embedding.call_args[1]
    ...
    @pytest.mark.asyncio
    async def test_aembedding_propagates_default_litelm_params(self):
        """
        Test that async embedding calls properly propagate default_litelm_params including headers.
        """
        custom_headers = {"X-Custom-Header": "test-value", "X-API-Version": "v2"}
        ...
        with patch(
            ...
            # Verify that litelm.aembedding was called with the headers
            mock_litelm_aembedding.assert_called_once()
            call_kwargs = mock_litelm_aembedding.call_args[1]
    ...
    def test_embedding_sets_litelm_trace_id(self):
        """
        Test that embedding calls include a litelm_trace_id.
        ...
        with patch("litelm.embedding") as mock_litelm_embedding:
            ...
            # Verify litelm_trace_id was set
            assert "litelm_trace_id" in call_kwargs
            assert isinstance(call_kwargs["litelm_trace_id"], str)
            assert len(call_kwargs["litelm_trace_id"]) > 0
    ...
    def test_embedding_consistency_with_completion(self):
        ...
        router = Router(
            model_list=model_list, default_litelm_params={"headers": custom_headers}
        )

        # Test completion
        with patch("litelm.completion") as mock_completion:
        ...
        # Test embedding
        with patch("litelm.embedding") as mock_embedding:
        ...
        # Both should have headers from default_litelm_params
        ...
        # Both should have litelm_trace_id
        assert "litelm_trace_id" in completion_kwargs
        assert "litelm_trace_id" in embedding_kwargs
...
if __name__ == "__main__":
    ...
    test.test_embedding_propagates_default_litelm_params()
    test.test_embedding_metadata_includes_model_group()
    test.test_embedding_sets_litelm_trace_id()
