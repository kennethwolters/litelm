...
from litelm.llms.bedrock.vector_stores.transformation import BedrockVectorStoreConfig
...
class TestBedrockVectorStore(BaseVectorStoreTest):
    ...
    def test_get_filename_from_metadata(self):
        ...
        metadata_with_path = {
            "x-amz-bedrock-kb-source-uri": "https://docs.litelm.ai/tutorial/setup.html"
        }
...
async def test_bedrock_search_with_credentials_managed_registry():
    ...
    original_registry = getattr(litelm, "vector_store_registry", None)
    original_credential_list = getattr(litelm, "credential_list", [])
    
    try:
        # Set up test AWS credentials in the credential system
        test_credentials = CredentialItem(
            credential_name="bedrock-litelm-website-knowledgebase",
            credential_info={
                "provider": "aws",
        ...
        litelm.credential_list = [test_credentials]
        ...
        vector_store = LiteLLM_ManagedVectorStore(
            ...
            litelm_credential_name="bedrock-litelm-website-knowledgebase",
        )
        
        # Set up registry
        registry = VectorStoreRegistry([vector_store])
        litelm.vector_store_registry = registry
        ...
        with patch.object(registry, 'get_credentials_for_vector_store', wraps=registry.get_credentials_for_vector_store) as mock_get_creds:
            ...
            with patch('litelm.vector_stores.main.base_llm_http_handler.vector_store_search_handler') as mock_handler:
                ...
                litelm_params = call_kwargs.get("litelm_params", {})
                ...
                assert hasattr(litelm_params, 'aws_access_key_id'), "aws_access_key_id should be in litelm_params"
                assert hasattr(litelm_params, 'aws_secret_access_key'), "aws_secret_access_key should be in litelm_params"
                assert hasattr(litelm_params, 'aws_region_name'), "aws_region_name should be in litelm_params"
        ...
        litelm.vector_store_registry = original_registry
        litelm.credential_list = original_credential_list