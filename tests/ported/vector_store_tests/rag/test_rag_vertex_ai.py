...
import litelm
from litelm.types.rag import RAGIngestOptions
...
class TestRAGVertexAI(BaseRAGTest):
    ...
    async def query_vector_store(
        ...
        try:
            ...
            search_response = await litelm.vector_stores.asearch(
    ...
    async def test_create_corpus_and_ingest(self):
        ...
        try:
            response = await litelm.rag.aingest(
            ...
            pytest.skip(f"Skipping test due to litelm.InternalServerError: {e}")
        except Exception as e:
