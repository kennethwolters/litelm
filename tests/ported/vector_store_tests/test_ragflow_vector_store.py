...
from litelm.llms.ragflow.vector_stores.transformation import RAGFlowVectorStoreConfig
from litelm.litelm_core_utils.litelm_logging import Logging as LiteLLMLoggingObj
from litelm.types.vector_stores import VectorStoreCreateOptionalRequestParams
...
class TestRAGFlowVectorStore(BaseVectorStoreTest):
    ...
    def test_get_auth_credentials(self):
        ...
        litelm_params = {"api_key": "test-api-key-123"}
        credentials = config.get_auth_credentials(litelm_params)
    ...
    def test_get_complete_url(self):
        ...
        litelm_params = {"api_base": "http://custom-host:9999"}
        url = config.get_complete_url(api_base=None, litelm_params=litelm_params)
        assert url == "http://custom-host:9999/api/v1/datasets"
        
        # Test with api_base parameter
        url = config.get_complete_url(api_base="http://test-host:8888", litelm_params={})
    ...
    def test_validate_environment(self):
        ...
        # Test with api_key in litelm_params
        litelm_params = GenericLiteLLMParams(api_key="test-key")
    ...
    async def test_basic_create_vector_store(self, sync_mode):
        ...
        try: 
            if sync_mode:
                response = litelm.vector_stores.create(
                    name=f"test-ragflow-{int(__import__('time').time())}",
                    **create_args
                )
            else:
                response = await litelm.vector_stores.acreate(
                    name=f"test-ragflow-{int(__import__('time').time())}",
                    **create_args
                )
        ...
        print("litelm create response=", json.dumps(response, indent=4, default=str))
