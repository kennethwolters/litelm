...
async def update_proxy_budget(session):
    ...
    data = {"user_id": "litelm-proxy-budget", "spend": 0}
...
async def chat_completion_streaming(session, key, model="gpt-4"):
    ...
    prompt_tokens = litelm.token_counter(model="gpt-35-turbo", messages=messages)
    data = {
        "model": model,
        "messages": messages,
        "stream": True,
    ...
    completion_tokens = litelm.token_counter(
...
async def test_key_info_spend_values():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        prompt_cost, completion_cost = litelm.cost_per_token(
            model="gpt-35-turbo",
...
async def test_key_model_list(model_access, model_access_level, model_endpoint):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        team_id = "litelm_dashboard_{}".format(uuid.uuid4())
