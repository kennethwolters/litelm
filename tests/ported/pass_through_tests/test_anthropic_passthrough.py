...
async def test_anthropic_basic_completion_with_headers():
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            ...
            litelm_call_id = response_headers.get("x-litelm-call-id")
            ...
            for attempt in range(max_retries):
                ...
                async with session.get(
                    f"http://0.0.0.0:4000/spend/logs?request_id={litelm_call_id}",
            ...
            assert log_entry["request_id"] == litelm_call_id, "Request ID should match"
            assert (
