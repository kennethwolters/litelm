...
async def test_llm_guard_triggered_safe_request():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        assert "x-litelm-applied-guardrails" in headers

        assert (
            headers["x-litelm-applied-guardrails"]
...
async def test_no_llm_guard_triggered():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        assert "x-litelm-applied-guardrails" not in headers
...
async def test_guardrails_with_api_key_controls():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        assert "x-litelm-applied-guardrails" in headers
        assert headers["x-litelm-applied-guardrails"] == "bedrock-pre-guard"
    ...
    async with aiohttp.ClientSession() as session:
        try:
            response, headers = await chat_completion(
                ...
                "sk-1234",
                model="fake-openai-endpoint",
                messages=[{"role": "user", "content": f"Hello do you like litelm?"}],
                guardrails=["custom-during-guard"],
            )
            pytest.fail("Should have thrown an exception")
        except Exception as e:
            print(e)
            assert "Guardrail failed words - `litelm` detected" in str(e)
...
async def test_guardrail_load_balancing():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for i in range(num_requests):
            ...
            assert "x-litelm-applied-guardrails" in headers
            assert headers["x-litelm-applied-guardrails"] == "lb-test-guard"
