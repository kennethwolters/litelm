...
async def test_mock_basic_google_ai_studio_responses_api_with_tools():
    """
    - Ensure that this is the request that litelm.completion gets when we pass web search options 

    litelm.acompletion(messages=[{'role': 'user', 'content': 'what is the latest version of supabase python package and when was it released?'}], model='gemini-2.5-flash', tools=[], web_search_options={'search_context_size': 'low', 'user_location': None})
    """
    ...
    with patch('litelm.acompletion', new_callable=AsyncMock) as mock_acompletion:
        ...
        print("call kwargs to litelm.completion=", json.dumps(call_kwargs, indent=4, default=str))
...
async def test_gemini_3_responses_api_streaming_with_thought_signatures():
    ...
    response_stream = await litelm.aresponses(
        model=request_model,
        input="What is the weather in Mumbai?",
        tools=tools,
        stream=True,
