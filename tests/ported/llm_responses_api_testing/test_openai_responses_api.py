...
import httpx
from litelm.llms.openai.responses.transformation import OpenAIResponsesAPIConfig
...
def validate_standard_logging_payload(
    ...
    Args:
        ...
        response (dict): The litelm response to compare against
...
def test_basic_openai_responses_api_streaming_with_logging():
    ...
    litelm.callbacks = [test_custom_logger]
    ...
    for event in response:
        ...
        print("litelm response=", json.dumps(event, indent=4, default=str))
...
def validate_responses_match(slp_response, litelm_response):
    """Validate that the standard logging payload OpenAI response matches the litelm response"""
    # Validate core fields
    assert slp_response["id"] == litelm_response["id"], "ID mismatch"
    assert slp_response["model"] == litelm_response["model"], "Model mismatch"
    assert (
        slp_response["created_at"] == litelm_response["created_at"]
    ), "Created at mismatch"

    # Validate usage
    assert (
        slp_response["usage"]["prompt_tokens"]
        == litelm_response["usage"]["input_tokens"]
    ), "Input tokens mismatch"
    assert (
        slp_response["usage"]["completion_tokens"]
        == litelm_response["usage"]["output_tokens"]
    ), "Output tokens mismatch"
    assert (
        slp_response["usage"]["total_tokens"]
        == litelm_response["usage"]["total_tokens"]
    ), "Total tokens mismatch"

    # Validate output/messages
    assert len(slp_response["output"]) == len(
        litelm_response["output"]
    ), "Output length mismatch"
    for slp_msg, litelm_msg in zip(slp_response["output"], litelm_response["output"]):
        assert slp_msg["role"] == litelm_msg.role, "Message role mismatch"
        # Access the content's text field for the litelm response
        litelm_content = litelm_msg.content[0].text if litelm_msg.content else ""
        assert (
            slp_msg["content"][0]["text"] == litelm_content
        ), f"Message content mismatch. Expected {litelm_content}, Got {slp_msg['content']}"
        assert slp_msg["status"] == litelm_msg.status, "Message status mismatch"
...
async def test_basic_openai_responses_api_non_streaming_with_logging():
    ...
    print("litelm response=", json.dumps(response, indent=4, default=str))
...
def validate_stream_event(event):
    """
    Validate that a streaming event from litelm.responses() or litelm.aresponses()
...
@pytest.mark.parametrize("sync_mode", [True, False])
@pytest.mark.asyncio
async def test_openai_responses_litelm_router(sync_mode):
    """
...
@pytest.mark.parametrize("sync_mode", [True, False])
@pytest.mark.asyncio
async def test_openai_responses_litelm_router_streaming(sync_mode):
    """
...
@pytest.mark.asyncio
async def test_openai_responses_litelm_router_no_metadata():
    """
...
@pytest.mark.asyncio
async def test_openai_responses_litelm_router_with_metadata():
    """
...
@pytest.mark.asyncio
async def test_openai_responses_litelm_router_with_prompt():
    """Test that prompt object is passed through the Router for responses API"""

    prompt_obj = {
        "id": "pmpt_abc123",
        "version": "2",
        "variables": {"random_variable": "ishaan_from_litelm"},
    }
...
def test_bad_request_bad_param_error():
    ...
    try:
        litelm.responses(model="gpt-4o", input="This should fail", temperature=2000)
...
async def test_openai_o1_pro_response_api_streaming(sync_mode):
    ...
    with patch(
        ...
        if sync_mode:
            ...
            with patch(
                "litelm.llms.custom_httpx.http_handler.HTTPHandler.post",
                ...
                for event in response:
                    print(
                        f"Sync litelm response #{event_count}:",
            ...
            async for event in response:
                print(
                    f"Async litelm response #{event_count}:",
                    json.dumps(event, indent=4, default=str),
                )
                event_count += 1
...
def test_basic_computer_use_preview_tool_call():
    ...
    linux is an unsupported environment for the computer_use_preview tool, but litelm users should still be able to pass it to openai
    """
...
def test_mcp_tools_with_responses_api():
    ...
    try:
        response = litelm.responses(model=MODEL, tools=MCP_TOOLS, input=USER_QUERY)
        print(response)
        ...
        if mcp_approval_id:
            response_with_mcp_call = litelm.responses(
                model=MODEL,
                tools=MCP_TOOLS,
                input=[
                    {
...
async def test_openai_responses_api_field_types():
    ...
    response_without_store = await litelm.aresponses(model="gpt-4o", input="hi")
...
async def test_openai_responses_api_token_limit_error():
    """
    Relevant issue: https://github.com/BerriAI/litelm/issues/15785
...
async def test_openai_compact_responses_api(sync_mode):
    ...
    try:
        if sync_mode:
            response = litelm.compact_responses(
                model="openai/gpt-4o",
                input=input_messages,
                instructions="Be helpful and concise",
            )
        else:
            response = await litelm.acompact_responses(
                model="openai/gpt-4o",
                input=input_messages,
                instructions="Be helpful and concise",
            )
