...
from litelm.responses.litelm_completion_transformation.handler import LiteLLMCompletionTransformationHandler
from litelm.responses.litelm_completion_transformation.transformation import LiteLLMCompletionResponsesConfig
...
def test_multiturn_tool_calls():
    ...
    follow_up_response = litelm.responses(
        model='anthropic/claude-4-sonnet-20250514',
...
async def test_async_response_api_handler_merges_trace_id_without_error():
    ...
    async def fake_session_handler(previous_response_id, litelm_completion_request):
        litelm_completion_request["litelm_trace_id"] = "session-trace"
        return litelm_completion_request
    ...
    with patch.object(
        ...
        with patch("litelm.acompletion", new_callable=AsyncMock) as mock_acompletion:
            ...
            await handler.async_response_api_handler(
                litelm_completion_request={"model": "test"},
                request_input="hi",
                responses_api_request={"previous_response_id": "123"},
                litelm_trace_id="original-trace",
            )
            # ensure acompletion called once with merged trace_id
            assert mock_acompletion.call_count == 1
            assert (
                mock_acompletion.call_args.kwargs["litelm_trace_id"] == "session-trace"
            )