...
async def test_async_file_and_batch():
    ...
    # retrieve batch
    retrieve_batch_response = await litelm.aretrieve_batch(
...
async def test_mock_bedrock_file_url_mapping():
    ...
    async def mock_async_create_file(transformed_request, **kwargs):
        ...
        from litelm.files.main import base_llm_http_handler
        return await base_llm_http_handler.__class__.async_create_file(
            base_llm_http_handler, transformed_request, **kwargs
        )
    
    with patch('litelm.files.main.base_llm_http_handler.async_create_file', side_effect=mock_async_create_file):
        file_obj = await litelm.acreate_file(
        ...
        # Verify mapping
        from litelm.llms.bedrock.files.transformation import BedrockFilesConfig
...
async def test_bedrock_retrieve_batch():
    ...
    with patch("litelm.llms.custom_httpx.http_handler.AsyncHTTPHandler.get") as mock_get:
        mock_response.raise_for_status.return_value = None
        mock_get.return_value = mock_response
        
        # Test retrieve batch
        batch_response = await litelm.aretrieve_batch(
...
def test_bedrock_batch_with_encryption_key_in_post_request():
    ...
    with patch("litelm.llms.custom_httpx.http_handler.HTTPHandler.post", side_effect=mock_post):
        response = litelm.create_batch(
            completion_window="24h",
            endpoint="/v1/chat/completions",
