...
import litelm
from litelm import create_batch, create_file
...
async def test_create_batch(provider):
    ...
    file_obj = await litelm.acreate_file(
    ...
    create_batch_response = await litelm.acreate_batch(
    ...
    print("response from litelm.create_batch=", create_batch_response)
    ...
    retrieved_batch = await litelm.aretrieve_batch(
    ...
    # list all batches
    list_batches = await litelm.alist_batches(custom_llm_provider=provider, limit=2)
    print("list_batches=", list_batches)

    file_content = await litelm.afile_content(
    ...
    try:
        cancel_batch_response = await litelm.acancel_batch(
...
def cleanup_azure_files():
    ...
    for _file in azure_files:
        ...
        delete_file_response = litelm.file_delete(
            file_id=_file.id,
            custom_llm_provider="azure",
            api_key=os.getenv("AZURE_FT_API_KEY"),
            api_base=os.getenv("AZURE_FT_API_BASE"),
...
async def test_async_create_batch(provider):
    ...
    litelm.callbacks = [custom_logger, "datadog"]
    ...
    create_batch_response = await litelm.acreate_batch(
        ...
        custom_llm_provider=provider,
        metadata={"key1": "value1", "key2": "value2"},
        # litelm specific param - used for logging metadata on logging callback
        litelm_metadata=extra_metadata_field,
    )
    ...
    # file obj
    file_obj = await litelm.afile_retrieve(
        file_id=batch_input_file_id, custom_llm_provider=provider
    )
    ...
    # delete file
    delete_file_response = await litelm.afile_delete(
        file_id=batch_input_file_id, custom_llm_provider=provider
    )
    ...
    all_files_list = await litelm.afile_list(
        custom_llm_provider=provider,
    )
...
mock_file_response = {
    "kind": "storage#object",
    "id": "litelm-local/litelm-vertex-files/publishers/google/models/gemini-1.5-flash-001/5f7b99ad-9203-4430-98bf-3b45451af4cb/1739598666670574",
    "selfLink": "https://www.googleapis.com/storage/v1/b/litelm-local/o/litelm-vertex-files%2Fpublishers%2Fgoogle%2Fmodels%2Fgemini-1.5-flash-001%2F5f7b99ad-9203-4430-98bf-3b45451af4cb",
    "mediaLink": "https://storage.googleapis.com/download/storage/v1/b/litelm-local/o/litelm-vertex-files%2Fpublishers%2Fgoogle%2Fmodels%2Fgemini-1.5-flash-001%2F5f7b99ad-9203-4430-98bf-3b45451af4cb?generation=1739598666670574&alt=media",
    "name": "litelm-vertex-files/publishers/google/models/gemini-1.5-flash-001/5f7b99ad-9203-4430-98bf-3b45451af4cb",
    "bucket": "litelm-local",
...
mock_vertex_batch_response = {
    ...
    "displayName": "litelm_batch_job",
    "model": "projects/123456789/locations/us-central1/models/gemini-1.5-flash-001",
    "modelVersionId": "v1",
    "inputConfig": {
        "gcsSource": {
            "uris": [
                "gs://litelm-local/litelm-vertex-files/publishers/google/models/gemini-1.5-flash-001/5f7b99ad-9203-4430-98bf-3b45451af4cb"
            ]
        }
    },
    "outputConfig": {
        "gcsDestination": {"outputUriPrefix": "gs://litelm-local/batch-outputs/"}
    },
    "dedicatedResources": {
        "machineSpec": {
...
async def test_avertex_batch_prediction(monkeypatch):
    monkeypatch.setenv("GCS_BUCKET_NAME", "litelm-local")
    ...
    with patch.object(
        ...
        with patch(
            "litelm.llms.custom_httpx.http_handler.AsyncHTTPHandler.get"
        ) as mock_get:
...
async def test_vertex_list_batches(monkeypatch):
    monkeypatch.setenv("GCS_BUCKET_NAME", "litelm-local")
    monkeypatch.setenv("VERTEXAI_PROJECT", "litelm-test-project")
    monkeypatch.setenv("VERTEXAI_LOCATION", "us-central1")

    monkeypatch.setattr(
        "litelm.llms.vertex_ai.batches.handler.VertexAIBatchPrediction._ensure_access_token",
        lambda self, credentials, project_id, custom_llm_provider: ("mock-token", "litelm-test-project"),
    )
    ...
    with patch(
        ...
        list_response = await litelm.alist_batches(
            custom_llm_provider="vertex_ai",
            limit=2,
        )
...
async def test_vertex_async_create_batch_logs_error_body_on_http_error():
    ...
    """
    from litelm.llms.vertex_ai.batches.handler import VertexAIBatchPrediction
...
async def test_delete_batch_output_file():
    ...
    if retrieved_batch.output_file_id:
        ...
        delete_output_file_response = await litelm.afile_delete(
    ...
    delete_input_file_response = await litelm.afile_delete(
