...
import litelm
from litelm import DualCache
from litelm.proxy._types import UserAPIKeyAuth
from litelm.proxy.hooks.batch_rate_limiter import (
    BatchFileUsage,
    _PROXY_BatchRateLimiter,
)
from litelm.proxy.hooks.parallel_request_limiter_v3 import (
    _PROXY_MaxParallelRequestsHandler_v3,
)
from litelm.proxy.utils import InternalUsageCache
...
def get_expected_batch_file_usage(file_path: str) -> tuple[int, int]:
    ...
    for item in file_contents:
        ...
        if messages:
            item_tokens = litelm.token_counter(model=model, messages=messages)
...
{"custom_id": "request-3", "method": "POST", "url": "/v1/chat/completions", "body": {"model": "gpt-3.5-turbo", "messages": [{"role": "user", "content": "Hey"}]}}"""
    ...
    try:
        ...
        file_obj_small = await litelm.acreate_file(
    ...
    try:
        # Upload file to OpenAI
        file_obj_large = await litelm.acreate_file(
...
async def test_batch_rate_limit_multiple_requests():
    ...
    try:
        # Upload file to OpenAI
        file_obj_1 = await litelm.acreate_file(
    ...
    try:
        # Upload file to OpenAI
        file_obj_2 = await litelm.acreate_file(
...
async def test_batch_rate_limiter_with_managed_files():
    ...
    try:
        ...
        original_afile_content = litelm.afile_content
        ...
        with patch('litelm.afile_content', side_effect=mock_afile_content):
            data = {
                "model": "gpt-3.5-turbo",
...
async def test_batch_rate_limiter_managed_files_regression():
    ...
    from litelm.types.llms.openai import HttpxBinaryResponseContent
    import httpx
    ...
    with patch.dict('sys.modules', {
        'litelm.proxy.proxy_server': MagicMock(
    ...
    with patch('litelm.afile_content') as mock_afile_content:
        mock_response = httpx.Response(
            status_code=200,
            content=batch_content,
            headers={"content-type": "application/octet-stream"},
        ...
        # Verify litelm.afile_content was called
        assert mock_afile_content.called, \
            "REGRESSION: litelm.afile_content not called for non-managed files"
    ...
    with patch.object(batch_limiter, '_fetch_managed_file_content') as mock_fetch:
        ...
        try:
            with patch('litelm.afile_content') as mock_afile_content:
                # If litelm.afile_content is called for managed files, bug exists
                ...
                # This should use _fetch_managed_file_content, not litelm.afile_content
                usage = await batch_limiter.count_input_file_usage(
                    file_id=managed_file_id,
                    custom_llm_provider="openai",
                    user_api_key_dict=user_api_key_dict,
...
async def test_batch_logging_azure_credentials_regression():
    ...
    Fix: Pass litelm_params (containing credentials) from the logging object
    through to the file content retrieval functions.
    """
    from unittest.mock import AsyncMock, MagicMock, patch
    from litelm.batches.batch_utils import (
        _extract_file_access_credentials,
        _get_batch_output_file_content_as_dictionary,
        _handle_completed_batch,
    )
    from litelm.types.llms.openai import Batch, HttpxBinaryResponseContent
    import httpx
    ...
    # Setup: Azure credentials (as they would be in litelm_params)
    azure_credentials = {
    ...
    with patch('litelm.files.main.afile_content', side_effect=mock_afile_content_tracker):
        result = await _get_batch_output_file_content_as_dictionary(
            batch=mock_batch,
            custom_llm_provider="azure",
            litelm_params=azure_credentials,
        )
    ...
    with patch('litelm.files.main.afile_content') as mock_afile_content_fail:
    ...
    with patch('litelm.files.main.afile_content') as mock_afile_content:
        mock_response = httpx.Response(
            status_code=200,
            content=batch_output,
            headers={"content-type": "application/octet-stream"},
        ...
        # Call without litelm_params (should still work for OpenAI)
        result = await _get_batch_output_file_content_as_dictionary(
            batch=mock_batch,
            custom_llm_provider="openai",
            litelm_params=None,
        ...
        print("   ✓ Works without litelm_params for OpenAI")
