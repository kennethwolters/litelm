...
from litelm import create_fine_tuning_job
from litelm._logging import verbose_logger
from litelm.llms.vertex_ai.fine_tuning.handler import (
    FineTuningJobCreate,
    VertexFineTuningAPI,
)
from litelm.types.llms.openai import Hyperparameters
...
async def test_create_fine_tune_jobs_async():
    try:
        ...
        litelm.callbacks = ["datadog", custom_logger]
        ...
        create_fine_tuning_response = await litelm.acreate_fine_tuning_job(
            model="gpt-3.5-turbo-0125",
            training_file=file_obj.id,
        )

        print(
            "response from litelm.create_fine_tuning_job=", create_fine_tuning_response
        )
        ...
        ft_jobs = await litelm.alist_fine_tuning_jobs(limit=2)
        print("response from litelm.list_fine_tuning_jobs=", ft_jobs)
        assert len(list(ft_jobs)) > 0

        # retrieve fine tuning job
        response = await litelm.aretrieve_fine_tuning_job(
            fine_tuning_job_id=create_fine_tuning_response.id,
        )
        print("response from litelm.retrieve_fine_tuning_job=", response)

        # delete file

        await litelm.afile_delete(
            file_id=file_obj.id,
        )

        # cancel ft job
        response = await litelm.acancel_fine_tuning_job(
            fine_tuning_job_id=create_fine_tuning_response.id,
        )

        print("response from litelm.cancel_fine_tuning_job=", response)
...
async def test_azure_create_fine_tune_jobs_async():
    try:
        ...
        # list fine tuning jobs
        print("listing ft jobs")
        ft_jobs = await litelm.alist_fine_tuning_jobs(
            limit=2,
            custom_llm_provider="azure",
            api_base="https://exampleopenaiendpoint-production.up.railway.app",
        )
...
async def test_create_vertex_fine_tune_jobs_mocked():
    ...
    original_success_callback = litelm.success_callback
    original_async_success_callback = litelm._async_success_callback
    ...
    try:
        ...
        litelm.success_callback = original_success_callback
        litelm._async_success_callback = original_async_success_callback
...
async def test_mock_openai_create_fine_tune_job():
    ...
    with patch.object(client.fine_tuning.jobs, "create") as mock_create:
        ...
        response = await litelm.acreate_fine_tuning_job(
            model="gpt-3.5-turbo-0125",
            training_file="file-123",
...
async def test_mock_openai_list_fine_tune_jobs():
    ...
    with patch.object(
        ...
        await litelm.alist_fine_tuning_jobs(limit=2, after="ft-000", client=client)
...
async def test_mock_openai_cancel_fine_tune_job():
    ...
    with patch.object(client.fine_tuning.jobs, "cancel") as mock_cancel:
        try:
            await litelm.acancel_fine_tuning_job(
...
async def test_mock_azure_create_fine_tune_job_with_azure_specific_params():
    ...
    with patch("litelm.llms.azure.fine_tuning.handler.AzureOpenAIFineTuningAPI.create_fine_tuning_job") as mock_create:
