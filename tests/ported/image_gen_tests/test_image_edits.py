...
class BaseLLMImageEditTest(ABC):
    ...
    def image_edit_function(self):
        return litelm.image_edit

    @property
    def async_image_edit_function(self):
        return litelm.aimage_edit
    ...
    @pytest.mark.asyncio
    async def test_openai_image_edit_litelm_sdk(self, sync_mode):
        """
...
TEST_IMAGES = [
    ...
    open(os.path.join(pwd, "litelm_site.png"), "rb"),
]
...
def get_test_images_as_bytesio():
    ...
    bytesio_images = []
    for image_path in ["ishaan_github.png", "litelm_site.png"]:
...
class TestAzureAIFlux2ImageEdit(BaseLLMImageEditTest):
    ...
    def get_base_image_edit_call_args(self) -> dict:
        ...
        return {
            ...
            "image": SINGLE_TEST_IMAGE,
            "api_base": "https://litelm-ci-cd-prod.services.ai.azure.com",
...
@pytest.mark.flaky(retries=3, delay=2)
@pytest.mark.asyncio
async def test_openai_image_edit_litelm_router():
    litelm._turn_on_debug()
    try:
        prompt = """
        Create a studio ghibli style image that combines all the reference images. Make sure the person looks like a CTO.
...
async def test_openai_image_edit_with_bytesio():
    ...
    from litelm import image_edit, aimage_edit
    litelm._turn_on_debug()
    try:
        prompt = """
        Create a studio ghibli style image that combines all the reference images. Make sure the person looks like a CTO.
...
@pytest.mark.asyncio
async def test_azure_image_edit_litelm_sdk():
...
async def test_recraft_image_edit_api():
    from litelm import aimage_edit
    import requests
    litelm._turn_on_debug()
    global TEST_IMAGES
    try:
...
def test_recraft_image_edit_config():
    ...
    """
    from litelm.llms.recraft.image_edit.transformation import RecraftImageEditConfig
