...
from litelm.proxy.management_endpoints.jwt_key_mapping_endpoints import (
    _to_response,
...
async def test_jwt_to_virtual_key_mapping_resolution():
    ...
    prisma_client.db.litelm_jwtkeymapping.find_first = AsyncMock()

    # Mock finding a mapping
    mock_mapping = MagicMock()
    mock_mapping.token = "sk-1234"
    mock_mapping.is_active = True
    prisma_client.db.litelm_jwtkeymapping.find_first.return_value = mock_mapping
    ...
    with patch(
        "litelm.proxy.auth.user_api_key_auth.get_key_object", new_callable=AsyncMock
    ) as mock_get_key:
        ...
        prisma_client.db.litelm_jwtkeymapping.find_first.assert_called_once()

        # Test Cache hit
        prisma_client.db.litelm_jwtkeymapping.find_first.reset_mock()
        ...
        prisma_client.db.litelm_jwtkeymapping.find_first.assert_not_called()
...
async def test_jwt_to_virtual_key_mapping_no_mapping():
    ...
    """
    jwt_handler = JWTHandler()
    jwt_handler.litelm_jwtauth = LiteLLM_JWTAuth(virtual_key_claim_field="email")
    jwt_claims = {"email": "unknown@example.com"}

    prisma_client = MagicMock()
    prisma_client.db.litelm_jwtkeymapping.find_first = AsyncMock()
    prisma_client.db.litelm_jwtkeymapping.find_first.return_value = None
...
def _mock_prisma():
    prisma = MagicMock()
    prisma.db.litelm_jwtkeymapping.create = AsyncMock()
    prisma.db.litelm_jwtkeymapping.find_unique = AsyncMock()
    prisma.db.litelm_jwtkeymapping.find_many = AsyncMock()
    prisma.db.litelm_jwtkeymapping.update = AsyncMock()
    prisma.db.litelm_jwtkeymapping.delete = AsyncMock()
    prisma.db.litelm_jwtkeymapping.count = AsyncMock(return_value=0)
    return prisma
...
async def test_create_returns_409_on_unique_violation():
    ...
    from litelm.proxy._types import CreateJWTKeyMappingRequest

    mock_prisma = _mock_prisma()
    mock_prisma.db.litelm_jwtkeymapping.create.side_effect = Exception(
...
async def test_delete_returns_404_when_not_found():
    ...
    from litelm.proxy._types import DeleteJWTKeyMappingRequest

    mock_prisma = _mock_prisma()
    mock_prisma.db.litelm_jwtkeymapping.find_unique.return_value = None
    mock_cache = AsyncMock()
...
async def test_update_returns_404_when_not_found():
    ...
    from litelm.proxy._types import UpdateJWTKeyMappingRequest
...
async def test_create_success_returns_response_without_token():
    ...
    mock_prisma = _mock_prisma()
    mock_prisma.db.litelm_jwtkeymapping.create.return_value = _mock_mapping()
    mock_cache = AsyncMock()
