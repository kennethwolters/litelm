...
def test_aaaasschema_migration_check(schema_setup, monkeypatch):
    ...
    deploy_dir = Path("./litelm-proxy-extras/litelm_proxy_extras")
    ...
    temp_base = Path(tempfile.mkdtemp(prefix="litelm_schema_migration_"))
    ...
    try:
        ...
        if not temp_migrations_dir.exists() or not any(temp_migrations_dir.iterdir()):
            ...
            pytest.fail(
                "No existing migrations found - first migration needed. Run `litelm/ci_cd/baseline_db.py` to create new migration -E.g. `python litelm/ci_cd/baseline_db_migration.py`."
            )
        ...
        if diff_result.returncode == 2:
            ...
            pytest.fail(
                "Schema changes detected - new migration required. Run `litelm/ci_cd/run_migration.py` to create new migration -E.g. `python litelm/ci_cd/run_migration.py <migration_name>`."
            )
        else:
