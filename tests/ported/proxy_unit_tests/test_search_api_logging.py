...
from litelm.proxy.spend_tracking.spend_management_endpoints import view_spend_logs
from litelm.proxy.utils import ProxyLogging, hash_token, update_spend
...
def prisma_client():
    ...
    proxy_server.litelm_proxy_budget_name = (
...
async def test_search_api_logging_and_cost_tracking(prisma_client):
    ...
    from litelm.proxy.management_endpoints.key_management_endpoints import generate_key_fn
    ...
    with patch("litelm.search.main.asearch", new_callable=AsyncMock) as mock_asearch:
        ...
        kwargs = {
            ...
            "litelm_call_id": request_id,  # Set request_id in kwargs
            "litelm_params": {
                "metadata": {
