"""Tests for the /public/litelm_blog_posts endpoint."""
...
def client():
    ...
    from litelm.proxy.public_endpoints.public_endpoints import router
...
def test_get_blog_posts_returns_response_shape(client):
    with patch(
        "litelm.proxy.public_endpoints.public_endpoints.get_blog_posts",
        return_value=SAMPLE_POSTS,
    ):
        response = client.get("/public/litelm_blog_posts")
    ...
    assert post["url"] == "https://www.litelm.ai/blog/test"
    ...
    many_posts = [
        {
            ...
            "description": "desc",
            "date": "2026-01-01",
            "url": f"https://www.litelm.ai/blog/{i}",
        }
        for i in range(10)
    ]
