...
from litelm.proxy.auth.auth_checks import get_key_object
...
from litelm.proxy.image_endpoints import image_generation
from litelm.proxy.management_endpoints.customer_endpoints import (
    new_end_user,
)
...
def test_call_with_user_over_budget(prisma_client):
    ...
    try:
        ...
        async def test():
            ...
            from litelm.proxy.proxy_server import _ProxyDBLogger
...
def test_call_with_end_user_over_budget(prisma_client):
    ...
    # we only check this when litelm.max_end_user_budget is set
    import random

    setattr(litelm.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(litelm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(litelm, "max_end_user_budget", 0.00001)
    try:
...
def test_call_with_proxy_over_budget(prisma_client):
    ...
    setattr(litelm.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(litelm.proxy.proxy_server, "master_key", "sk-1234")
    litelm_proxy_budget_name = f"litelm-proxy-budget-{time.time()}"
    setattr(
        litelm.proxy.proxy_server,
        "litelm_proxy_admin_name",
        litelm_proxy_budget_name,
    )
    setattr(litelm, "max_budget", 0.00001)
    from litelm.proxy.proxy_server import user_api_key_cache

    user_api_key_cache.set_cache(
        key="{}:spend".format(litelm_proxy_budget_name), value=0
    )
...
def test_call_with_proxy_over_budget_stream(prisma_client):
    ...
    try:
        ...
        async def test():
            ...
            # request = NewUserRequest(
            #     max_budget=0.00001, user_id=litelm_proxy_budget_name
            # )
            request = NewUserRequest()
            key = await new_user(
                data=request,
...
def test_delete_key(prisma_client):
    ...
    setattr(litelm.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(litelm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(litelm.proxy.proxy_server, "user_custom_auth", None)
    try:
...
def test_generate_and_call_key_info(prisma_client):
    ...
    try:
        ...
        async def test():
            await litelm.proxy.proxy_server.prisma_client.connect()
            request = NewUserRequest(
                metadata={"team": "litelm-team3", "project": "litelm-project3"}
            )
            key = await new_user(
                data=request,
                user_api_key_dict=UserAPIKeyAuth(
...
def test_generate_and_update_key(prisma_client):
    ...
    try:
        ...
        async def test():
            ...
            # create team "litelm-core-infra@gmail.com""
            print("creating team litelm-core-infra@gmail.com")
            _team_1 = "litelm-core-infra@gmail.com_{}".format(uuid.uuid4())
            await new_team(
                NewTeamRequest(
                    team_id=_team_1,
                ),
            ...
            request = NewUserRequest(
                metadata={"project": "litelm-project3"},
                team_id=_team_1,
            )
...
def test_key_generate_with_custom_auth(prisma_client):
    ...
    async def custom_generate_key_fn(data: GenerateKeyRequest) -> dict:
        ...
        if team_id is not None and team_id == "litelm-core-infra@gmail.com":
            # only team_id="litelm-core-infra@gmail.com" can make keys
            return {
                "decision": True,
            }
        else:
    ...
    setattr(litelm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(
        litelm.proxy.proxy_server, "user_custom_key_generate", custom_generate_key_fn
    )
    try:
        ...
        async def test():
            ...
            request_2 = GenerateKeyRequest(
                team_id="litelm-core-infra@gmail.com",
            )
...
def test_call_with_key_over_budget(prisma_client):
    ...
    try:
        ...
        async def test():
            ...
            # update spend using track_cost callback, make 2nd request, it should fail
            from litelm import Choices, Message, ModelResponse, Usage
            from litelm.caching.caching import Cache
            from litelm.proxy.proxy_server import _ProxyDBLogger
...
def test_call_with_key_over_budget_no_cache(prisma_client):
    ...
    # Related to this: https://github.com/BerriAI/litelm/issues/3920
    setattr(litelm.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(litelm.proxy.proxy_server, "master_key", "sk-1234")
    try:
        ...
        async def test():
            ...
            setattr(litelm.proxy.proxy_server, "proxy_batch_write_at", 1)
...
async def test_aasync_call_with_key_over_model_budget(
    ...
    setattr(litelm.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(litelm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(litelm.proxy.proxy_server, "premium_user", True)
    await litelm.proxy.proxy_server.prisma_client.connect()
    verbose_proxy_logger.setLevel(logging.DEBUG)
    ...
    from litelm.proxy.proxy_server import model_max_budget_limiter
...
async def test_key_name_set(prisma_client):
    ...
    setattr(litelm.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(litelm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(litelm.proxy.proxy_server, "general_settings", {"allow_user_auth": True})
    await litelm.proxy.proxy_server.prisma_client.connect()
    try:
        request = GenerateKeyRequest()
        key = await generate_key_fn(
...
async def test_default_key_params(prisma_client):
    ...
    setattr(litelm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(litelm.proxy.proxy_server, "general_settings", {"allow_user_auth": True})
    litelm.default_key_generate_params = {"max_budget": 0.000122}
    await litelm.proxy.proxy_server.prisma_client.connect()
    try:
        request = GenerateKeyRequest()
        key = await generate_key_fn(
...
async def test_upperbound_key_param_larger_budget(prisma_client):
    ...
    setattr(litelm.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(litelm.proxy.proxy_server, "master_key", "sk-1234")
    litelm.upperbound_key_generate_params = LiteLLM_UpperboundKeyGenerateParams(
...
def test_get_bearer_token():
    from litelm.proxy.auth.user_api_key_auth import _get_bearer_token
...
async def test_update_logs_with_spend_logs_url(prisma_client):
    ...
    """
    from litelm.proxy.db.db_spend_update_writer import DBSpendUpdateWriter
...
async def test_user_api_key_auth(prisma_client):
    from litelm.proxy.proxy_server import ProxyException
...
async def test_user_api_key_auth_without_master_key(prisma_client):
    ...
    try:
        ...
        setattr(litelm.proxy.proxy_server, "prisma_client", prisma_client)
        setattr(litelm.proxy.proxy_server, "master_key", None)
        setattr(
            litelm.proxy.proxy_server, "general_settings", {"allow_user_auth": True}
        )
        await litelm.proxy.proxy_server.prisma_client.connect()
...
async def test_key_with_no_permissions(prisma_client):
    ...
    setattr(litelm.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(litelm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(litelm.proxy.proxy_server, "general_settings", {"allow_user_auth": False})
    await litelm.proxy.proxy_server.prisma_client.connect()
    try:
        response = await generate_key_helper_fn(
            request_type="key",
            **{"duration": "1hr", "key_max_budget": 0, "models": [], "aliases": {}, "config": {}, "spend": 0, "user_id": "ishaan", "team_id": "litelm-dashboard"},  # type: ignore
        )
...
async def test_master_key_hashing(prisma_client):
    try:
        ...
        setattr(litelm.proxy.proxy_server, "prisma_client", prisma_client)
        setattr(litelm.proxy.proxy_server, "master_key", master_key)
...
async def test_custom_api_key_header_name(prisma_client):
    ...
    setattr(
        litelm.proxy.proxy_server,
        "general_settings",
        {"litelm_key_header_name": "x-litelm-key"},
    )
    await litelm.proxy.proxy_server.prisma_client.connect()
    ...
    request = Request(
        {
            ...
            "path": api_route.path,
            "headers": [
                (b"x-litelm-key", b"Bearer sk-1234"),
            ],
        }
    )

    # this should pass because we pass the master key as X-Litellm-Key and litelm_key_header_name="X-Litellm-Key" in general settings
...
async def test_team_access_groups(prisma_client):
    ...
    setattr(litelm.proxy.proxy_server, "llm_router", litelm_router)
...
async def test_list_keys(prisma_client):
    ...
    from litelm.proxy.proxy_server import hash_token
...
async def test_user_api_key_auth_db_unavailable():
    ...
    setattr(litelm.proxy.proxy_server, "prisma_client", MockPrismaClient())
    setattr(litelm.proxy.proxy_server, "user_api_key_cache", MockDualCache())
    setattr(litelm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(
        litelm.proxy.proxy_server,
        "general_settings",
    ...
    assert result.user_id == litelm.proxy.proxy_server.litelm_proxy_admin_name
...
async def test_user_api_key_auth_db_unavailable_not_allowed():
    ...
    setattr(litelm.proxy.proxy_server, "prisma_client", MockPrismaClient())
    setattr(litelm.proxy.proxy_server, "user_api_key_cache", MockDualCache())
    setattr(litelm.proxy.proxy_server, "general_settings", {})
    setattr(litelm.proxy.proxy_server, "master_key", "sk-1234")
    ...
    # Run test with a sample API key
    with pytest.raises(litelm.proxy._types.ProxyException):
        await user_api_key_auth(
            request=request,
            api_key="Bearer sk-123456789",
        )
...
@pytest.mark.skip(reason="Requires reliable external DB connection (prisma).")
@pytest.mark.asyncio
@mock.patch("litelm.secret_managers.aws_secret_manager_v2.AWSSecretsManagerV2.async_write_secret")
@mock.patch("litelm.secret_managers.aws_secret_manager_v2.AWSSecretsManagerV2.async_read_secret")
@mock.patch("litelm.secret_managers.aws_secret_manager_v2.AWSSecretsManagerV2.async_delete_secret")
async def test_key_generate_with_secret_manager_call(
    ...
    """
    from litelm.secret_managers.aws_secret_manager_v2 import AWSSecretsManagerV2
    from litelm.types.secret_managers.main import (
        KeyManagementSystem,
        KeyManagementSettings,
    )

    from litelm.proxy.hooks.key_management_event_hooks import (
    ...
    litelm.secret_manager_client = aws_secret_manager_client
    litelm._key_management_system = KeyManagementSystem.AWS_SECRET_MANAGER
    litelm._key_management_settings = KeyManagementSettings(
    ...
    setattr(litelm.proxy.proxy_server, "general_settings", general_settings)
    setattr(litelm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(litelm.proxy.proxy_server, "prisma_client", prisma_client)
    await litelm.proxy.proxy_server.prisma_client.connect()
    ############################################################################
    ...
    result = await aws_secret_manager_client.async_read_secret(
        secret_name=f"{litelm._key_management_settings.prefix_for_stored_virtual_keys}{key_alias}"
    )
...
def test_should_track_cost_callback():
    ...
    """
    from litelm.proxy.hooks.proxy_track_cost_callback import (
...
async def test_get_paginated_teams(prisma_client):
    ...
    """
    from litelm.proxy.management_endpoints.team_endpoints import get_paginated_teams
...
async def test_reset_budget_job(prisma_client, entity_type):
    ...
    from litelm.proxy.common_utils.reset_budget_job import ResetBudgetJob
    ...
    if entity_type == "key":
        ...
        updated = await prisma_client.db.litelm_verificationtoken.update_many(
        ...
        await prisma_client.db.litelm_usertable.update_many(
        ...
        await prisma_client.db.litelm_teamtable.update(
    ...
    if entity_type == "key":
        entity_before = await prisma_client.db.litelm_verificationtoken.find_unique(
            where={"token": entity_id}
        )
    elif entity_type == "user":
        entity_before = await prisma_client.db.litelm_usertable.find_unique(
            where={"user_id": entity_id}
        )
    elif entity_type == "team":
        entity_before = await prisma_client.db.litelm_teamtable.find_unique(
    ...
    if entity_type == "key":
        entity_after = await prisma_client.db.litelm_verificationtoken.find_unique(
            where={"token": entity_id}
        )
    elif entity_type == "user":
        entity_after = await prisma_client.db.litelm_usertable.find_unique(
            where={"user_id": entity_id}
        )
    elif entity_type == "team":
        entity_after = await prisma_client.db.litelm_teamtable.find_unique(
            where={"team_id": entity_id}
        )
...
def test_delete_nonexistent_key_returns_404(prisma_client):
    ...
    try:
        ...
        async def test():
            ...
            result = await litelm.proxy.proxy_server.user_api_key_auth(
                request=request, api_key=bearer_token
            )
            result.user_role = LitellmUserRoles.PROXY_ADMIN
            try:
