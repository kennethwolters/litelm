...
async def test_read_config_file_with_os_environ_vars():
    ...
    # Add assertions
    assert (
        config["litelm_settings"]["default_internal_user_params"]["user_role"]
        == "admin"
    )
    assert (
        config["litelm_settings"]["s3_callback_params"]["s3_aws_access_key_id"]
        == "1234567890"
    )
    assert (
        config["litelm_settings"]["s3_callback_params"]["s3_aws_secret_access_key"]
        == "1234567890"
    )

    for model in config["model_list"]:
        if "azure" in model["litelm_params"]["model"]:
            assert model["litelm_params"]["api_key"] == "1234567890"
        elif "fireworks" in model["litelm_params"]["model"]:
            assert model["litelm_params"]["api_key"] == "1234567890"
...
async def test_basic_include_directive():
    ...
    assert config["litelm_settings"]["callbacks"] == ["prometheus"]
...
def test_add_callbacks_from_db_config():
    ...
    # Reset litelm callbacks before test
    ...
    # 1 instance of LangfusePromptManagement should exist in litelm.success_callback
    ...
    litelm.success_callback = []
    litelm.failure_callback = []
    litelm._known_custom_logger_compatible_callbacks = []
...
def test_add_callbacks_invalid_input():
    ...
    # Test Case 2: Missing litelm_settings
    config_data = {}
    proxy_config._add_callbacks_from_db_config(config_data)
...
async def test_json_logs_calls_turn_on_json():
    """
    Test that json_logs: true in litelm_settings calls litelm._turn_on_json()
        ...
        "model_list": [
            {
                ...
                "litelm_params": {"model": "openai/gpt-4", "api_key": "test-key"},
            }
        ],
        "litelm_settings": {"json_logs": True},
    }
    ...
    try:
        ...
        with mock.patch("litelm._turn_on_json") as mock_turn_on_json:
        ...
        assert litelm.json_logs is True

    finally:
        # Cleanup
        os.unlink(temp_file_path)
        litelm.json_logs = False
