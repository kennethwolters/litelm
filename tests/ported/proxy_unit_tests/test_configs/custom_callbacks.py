...
class testCustomCallbackProxy(CustomLogger):
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        # Access litelm_params passed to litelm.completion(), example access `metadata`
        litelm_params = kwargs.get("litelm_params", {})
        metadata = litelm_params.get(
            "metadata", {}
        )  # headers passed to LiteLLM proxy, can be found here

        # Calculate cost using  litelm.completion_cost()
        cost = litelm.completion_cost(completion_response=response_obj)
