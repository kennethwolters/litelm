...
from litelm.proxy.utils import _get_docs_url, _get_redoc_url
...
from litelm.proxy.auth.auth_utils import is_request_body_safe
...
def mock_request(monkeypatch):
    ...
    monkeypatch.setattr(
        "litelm.proxy.litelm_pre_call_utils.add_litelm_data_to_request", mock_request
    )
    return mock_request
...
@pytest.mark.asyncio
async def test_add_litelm_data_to_request_thread_endpoint(endpoint, mock_request):
    ...
    assert "litelm_metadata" in data
...
@pytest.mark.asyncio
async def test_add_litelm_data_to_request_non_thread_endpoint(endpoint, mock_request):
    mock_request.url.path = endpoint
    user_api_key_dict = UserAPIKeyAuth(
        api_key="test_api_key", user_id="test_user_id", org_id="test_org_id"
    )
    ...
    assert "litelm_metadata" not in data
...
async def test_traceparent_not_added_by_default(endpoint, mock_request):
    ...
    setattr(litelm.proxy.proxy_server, "open_telemetry_logger", otel_logger)
    ...
    setattr(litelm.proxy.proxy_server, "open_telemetry_logger", None)
...
async def test_add_key_or_team_level_spend_logs_metadata_to_request(
    ...
    print(data)
    new_data = await add_litelm_data_to_request(
        data, mock_request, user_api_key_dict, proxy_config
    )
...
def test_add_litelm_data_for_backend_llm_call(
    headers, general_settings, expected_data
):
    import json
...
def test_foward_litelm_user_info_to_backend_llm_call():
    import json

    litelm.add_user_information_to_llm_headers = True
    ...
    expected_data = {
        "x-litelm-user_api_key_user_id": "test_user_id",
        "x-litelm-user_api_key_org_id": "test_org_id",
        "x-litelm-user_api_key_hash": "test_api_key",
        "x-litelm-user_api_key_spend": 0.0,
        "x-litelm-user_api_key_auth_metadata": {},
    }
...
def test_update_internal_user_params():
    from litelm.proxy._types import NewUserRequest
    ...
    assert updated_data_json["models"] == litelm.default_internal_user_params["models"]
    assert (
        updated_data_json["max_budget"]
        == litelm.default_internal_user_params["max_budget"]
    )
    assert (
        updated_data_json["budget_duration"]
        == litelm.default_internal_user_params["budget_duration"]
    )
...
async def test_add_litelm_data_to_request_duplicate_tags(
    key_tags, request_tags, expected_tags
):
    """
...
def test_enforced_params_check(
    ...
    from litelm.proxy.litelm_pre_call_utils import _enforced_params_check
...
def test_get_key_models():
    ...
    from litelm.proxy.auth.model_checks import get_key_models
...
def test_get_team_models():
    ...
    from litelm.proxy.auth.model_checks import get_team_models
...
def test_update_config_fields():
    ...
    all_team_config = updated_config["litelm_settings"]["default_team_settings"]
...
def test_update_config_fields_default_internal_user_params(monkeypatch):
    ...
    monkeypatch.setattr(litelm, "default_internal_user_params", None)
    ...
    assert litelm.default_internal_user_params == {
        "user_role": "proxy_admin",
        "max_budget": 1000,
        "budget_duration": "1mo",
    }
...
@pytest.mark.parametrize(
    ...
    [
        (
            ["openai/*"],
            [{"model_name": "openai/*", "litelm_params": {"model": "openai/*"}}],
            "openai",
        ),
        (
            ["bedrock/*"],
            [{"model_name": "bedrock/*", "litelm_params": {"model": "bedrock/*"}}],
            "bedrock",
        ),
        (
            ["anthropic/*"],
            [{"model_name": "anthropic/*", "litelm_params": {"model": "anthropic/*"}}],
            "anthropic",
        ),
        (
            ["cohere/*"],
            [{"model_name": "cohere/*", "litelm_params": {"model": "cohere/*"}}],
            "cohere",
        ),
    ],
)
def test_get_complete_model_list(proxy_model_list, model_list, provider):
    ...
    """
    from litelm import Router
    from litelm.proxy.auth.model_checks import get_complete_model_list
...
def test_team_callback_metadata_all_none_values():
    from litelm.proxy._types import TeamCallbackMetadata
...
def test_proxy_config_state_post_init_callback_call(monkeypatch):
    ...
    Addresses issue: https://github.com/BerriAI/litelm/issues/6787
    ...
    assert config["litelm_settings"]["default_team_settings"][0]["team_id"] == "test"
...
@pytest.mark.parametrize(
    ...
    [
        (
            {
                "litelm_budget_table_max_budget": None,
                "litelm_budget_table_tpm_limit": None,
                "litelm_budget_table_rpm_limit": 1,
                "litelm_budget_table_model_max_budget": None,
            },
            "rpm_limit",
            1,
        ),
        ...
        (
            {
                "litelm_budget_table_max_budget": None,
                "litelm_budget_table_tpm_limit": None,
                "litelm_budget_table_rpm_limit": None,
                "litelm_budget_table_model_max_budget": {"gpt-4o": 100},
            },
            "model_max_budget",
            {"gpt-4o": 100},
        ),
...
def test_litelm_verification_token_view_response_with_budget_table(
    associated_budget_table,
    expected_user_api_key_auth_key,
    expected_user_api_key_auth_value,
):
    from litelm.proxy._types import LiteLLM_VerificationTokenView
...
def test_litelm_verification_token_view_budget_does_not_override_key_model_max_budget():
    """
    ...
    args = {
        ...
        "team_model_aliases": None,
        "team_member": None,
        "litelm_budget_table_model_max_budget": {
...
def test_get_model_group_info():
    from litelm import Router
    from litelm.proxy.proxy_server import _get_model_group_info
...
class MockDb:
    ...
    async def query_raw(self, query: str, *args):
        ...
        filtered_keys = [
            ...
            if k["team_id"] != "litelm-dashboard" or k["team_id"] is None
        ]
...
async def test_get_user_info_for_proxy_admin(mock_team_data, mock_key_data):
    ...
    from litelm.proxy._types import UserAPIKeyAuth, UserInfoResponse
...
def test_custom_openid_response():
    ...
    from litelm.proxy._types import LiteLLM_JWTAuth
    ...
    jwt_handler.update_environment(
        ...
        litelm_jwtauth=LiteLLM_JWTAuth(
...
def test_get_temp_budget_increase():
    ...
    from litelm.proxy._types import UserAPIKeyAuth
    from litelm.proxy.auth.user_api_key_auth import _get_temp_budget_increase
...
def test_update_key_budget_with_temp_budget_increase():
    ...
    from litelm.proxy._types import UserAPIKeyAuth
    from litelm.proxy.auth.user_api_key_auth import (
...
async def test_health_check_not_called_when_disabled(monkeypatch):
    ...
    monkeypatch.setattr(
        "litelm.proxy.proxy_server.PrismaClient", lambda **kwargs: mock_prisma
    )
...
@patch(
    "litelm.proxy.proxy_server.get_openapi_schema",
    return_value={
        "paths": {
...
def test_custom_openapi(mock_get_openapi_schema):
    from litelm.proxy.proxy_server import custom_openapi

    openapi_schema = custom_openapi()
    assert openapi_schema is not None


from litelm.proxy.utils import ProxyUpdateSpend
...
async def test_spend_logs_cleanup_after_error():
    ...
    mock_client.db.litelm_spendlogs.create_many = AsyncMock(
...
#     from litelm.types.utils import ProviderSpecificHeader
#     from litelm.llms.custom_httpx.http_handler import HTTPHandler
...
@pytest.mark.parametrize(
    "wildcard_model, litelm_params, expected_models",
    [
        (
            "anthropic/*",
            {"model": "anthropic/*"},
...
def test_get_known_models_from_wildcard(
    ...
    from litelm.proxy.auth.model_checks import get_known_models_from_wildcard
    from litelm.types.router import LiteLLM_Params

    wildcard_models = get_known_models_from_wildcard(
        wildcard_model=wildcard_model, litelm_params=LiteLLM_Params(**litelm_params)
    )
...
def test_get_known_models_from_wildcard_without_litelm_params():
    """
    Test wildcard expansion without litelm_params (BYOK case - team has openai/*
    but no deployment in router config).
    """
    from litelm.proxy.auth.model_checks import get_known_models_from_wildcard

    wildcard_models = get_known_models_from_wildcard(
        wildcard_model="openai/*", litelm_params=None
    )
...
def test_update_model_if_team_alias_exists(data, user_api_key_dict, expected_model):
    from litelm.proxy.litelm_pre_call_utils import _update_model_if_team_alias_exists
...
def mock_prisma_client():
    client = MagicMock()
    client.db = MagicMock()
    client.db.litelm_teamtable = AsyncMock()
    return client
...
async def test_get_admin_team_ids(
    ...
    # Setup
    mock_prisma_client.db.litelm_teamtable.find_many.return_value = mock_teams
    user_api_key_dict = UserAPIKeyAuth(
    ...
    if should_query_db:
        mock_prisma_client.db.litelm_teamtable.find_many.assert_called_once_with(
            where={"team_id": {"in": user_info.teams}}
        )
    else:
        mock_prisma_client.db.litelm_teamtable.find_many.assert_not_called()
...
async def test_post_call_failure_hook_auth_error_key_info_route():
    ...
    from litelm.proxy._types import ProxyErrorTypes
    ...
    with patch.object(
        ...
        request_data = {
            ...
            "litelm_call_id": "test_call_id_123",
        }
...
async def test_during_call_hook_parallel_execution():
    ...
    try:
        litelm.callbacks = [TestGuardrail(f"g{i}") for i in range(3)]
...
async def test_during_call_hook_parallel_execution_with_error():
    ...
    try:
        litelm.callbacks = [FailingGuardrail()]
