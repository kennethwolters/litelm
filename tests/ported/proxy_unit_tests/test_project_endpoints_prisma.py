...
from litelm.proxy.management_endpoints.project_endpoints import (
    new_project,
    update_project,
    delete_project,
    project_info,
...
async def test_new_project(prisma_client):
    ...
    try:
        ...
        assert response.litelm_budget_table is not None
        assert response.litelm_budget_table.max_budget == 100.0
...
async def test_update_project(prisma_client):
    ...
    try:
        ...
        assert update_response.litelm_budget_table is not None
        assert update_response.litelm_budget_table.max_budget == 200.0
...
async def test_project_info(prisma_client):
    ...
    try:
        ...
        assert info_response.litelm_budget_table is not None
        assert info_response.litelm_budget_table.max_budget == 150.0
...
def test_premium_user_gate():
    ...
    setattr(litelm.proxy.proxy_server, "premium_user", False)

    # Verify that CommonProxyErrors.not_premium_user exists
    from litelm.proxy._types import CommonProxyErrors
...
async def test_project_max_budget_check_fires_alert():
    ...
    """
    from litelm.proxy.auth.auth_checks import _project_max_budget_check
    from litelm.proxy._types import (
        LiteLLM_BudgetTable,
        LiteLLM_ProjectTableCachedObj,
    )
    ...
    project = LiteLLM_ProjectTableCachedObj(
        ...
        created_by="admin",
        updated_by="admin",
        litelm_budget_table=LiteLLM_BudgetTable(max_budget=100.0),
    )
    ...
    with pytest.raises(litelm.BudgetExceededError) as exc_info:
...
async def test_project_soft_budget_check():
    ...
    """
    from litelm.proxy.auth.auth_checks import _project_soft_budget_check
    from litelm.proxy._types import (
        LiteLLM_BudgetTable,
        LiteLLM_ProjectTableCachedObj,
    )
    ...
    project = LiteLLM_ProjectTableCachedObj(
        ...
        created_by="admin",
        updated_by="admin",
        litelm_budget_table=LiteLLM_BudgetTable(soft_budget=75.0),
    )
...
def test_litelm_entity_type_has_project():
    """
...
async def test_list_projects_returns_timestamps():
    ...
    from litelm.proxy.management_endpoints.project_endpoints import list_projects
    from litelm.proxy._types import LiteLLM_ProjectTable
    ...
    mock_prisma.db.litelm_projecttable.find_many = AsyncMock(
...
def test_litelm_project_table_has_timestamp_fields():
    """
