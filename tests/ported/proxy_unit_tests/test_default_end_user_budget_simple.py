...
Tests the core scenarios where litelm.max_end_user_budget_id applies
...
import litelm
from litelm.proxy._types import LiteLLM_BudgetTable, LiteLLM_EndUserTable
...
async def test_default_budget_applied_to_end_user_without_budget():
    ...
    litelm.max_end_user_budget_id = default_budget_id
    ...
    mock_prisma_client.db.litelm_endusertable.find_unique = AsyncMock(
    ...
    assert result.litelm_budget_table is not None
    assert result.litelm_budget_table.budget_id == default_budget_id
    assert result.litelm_budget_table.max_budget == 10.0
    assert result.litelm_budget_table.rpm_limit == 2
    assert result.litelm_budget_table.tpm_limit == 10
    
    litelm.max_end_user_budget_id = None
...
async def test_explicit_budget_not_overridden_by_default():
    ...
    mock_end_user_data = {
        "user_id": end_user_id,
        "spend": 10.0,
        "litelm_budget_table": explicit_budget.dict(),
        "alias": None,
        "allowed_model_region": None,
        "default_model": None,
        "blocked": False,
    ...
    assert result is not None
    assert result.litelm_budget_table.budget_id == explicit_budget_id
    assert result.litelm_budget_table.max_budget == 100.0
    assert result.litelm_budget_table.rpm_limit == 50
...
async def test_system_works_without_default_budget_configured():
    ...
    """
    end_user_id = f"test_user_{uuid.uuid4().hex}"
    litelm.max_end_user_budget_id = None  # Not configured
    ...
    assert result.litelm_budget_table is None  # No budget applied
