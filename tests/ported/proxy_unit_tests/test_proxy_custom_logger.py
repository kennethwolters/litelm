...
import pytest, time
import litelm
from litelm import embedding, completion, completion_cost, Timeout
from litelm import RateLimitError
...
def test_embedding(client):
    try:
        ...
        from litelm.proxy.types_utils.utils import get_instance_fn
        ...
        litelm.callbacks = [my_custom_logger]
        # Your test data
        print("initialized proxy")
        # import the initialized custom logger
        print(litelm.callbacks)

        # assert len(litelm.callbacks) == 1 # assert litelm is initialized with 1 callback
        ...
        litelm_params = kwargs.get("litelm_params")
        
        # Test 1: Verify metadata is populated correctly
        metadata = litelm_params.get("metadata", None)
        print("\n\n Metadata in custom logger kwargs", litelm_params.get("metadata"))
        assert metadata is not None, "metadata should be present in litelm_params"
        ...
        proxy_server_request = litelm_params.get("proxy_server_request")
        ...
        model_info = litelm_params.get("model_info")
...
def test_chat_completion(client):
    try:
        ...
        print("LiteLLM Callbacks", litelm.callbacks)
        print("my_custom_logger", my_custom_logger)
        ...
        test_data = {
            ...
            "messages": [
                {"role": "user", "content": "write a litelm poem"},
            ],
            "max_tokens": 10,
        }
        ...
        litelm_params = my_custom_logger.async_completion_kwargs.get("litelm_params")
        ...
        config_model_info = litelm_params.get("model_info")
        ...
        proxy_server_request_object = litelm_params.get("proxy_server_request")
        ...
        assert body.get("messages") == [{"role": "user", "content": "write a litelm poem"}], "messages should match"
