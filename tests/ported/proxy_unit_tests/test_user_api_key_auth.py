...
def test_check_valid_ip(
    ...
    from litelm.proxy.auth.auth_utils import _check_valid_ip
...
async def test_team_object_has_object_permission_id():
    ...
    setattr(litelm.proxy.proxy_server, "user_api_key_cache", user_api_key_cache)
    setattr(litelm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(litelm.proxy.proxy_server, "prisma_client", "test-client")

    request = Request(scope={"type": "http"})
    request._url = URL(url="/chat/completions")

    with patch(
        "litelm.proxy.auth.user_api_key_auth.common_checks", new_callable=AsyncMock
...
async def test_returned_user_api_key_auth(user_role, expected_role):
    from litelm.proxy._types import LiteLLM_UserTable, LitellmUserRoles
    from litelm.proxy.auth.user_api_key_auth import _return_user_api_key_auth_obj
...
async def test_aaauser_personal_budgets(key_ownership):
    ...
    test_user_cache = getattr(litelm.proxy.proxy_server, "user_api_key_cache")
...
def test_is_ui_route_allowed(route, user_role, expected_result):
    from litelm.proxy.auth.auth_checks import _is_ui_route
...
def test_is_api_route_allowed(route, user_role, expected_result):
    from litelm.proxy.auth.auth_checks import _is_api_route_allowed
    from litelm.proxy._types import LiteLLM_UserTable
...
async def test_auth_not_connected_to_db():
    ...
    setattr(litelm.proxy.proxy_server, "user_api_key_cache", user_api_key_cache)
    setattr(litelm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(litelm.proxy.proxy_server, "prisma_client", None)
    setattr(
...
def test_get_api_key_from_custom_header(headers, custom_header_name, expected_api_key):
    ...
    api_key = get_api_key_from_custom_header(
        request=request, custom_litelm_key_header_name=custom_header_name
    )
...
def test_allowed_route_inside_route(
    ...
    from litelm.proxy.auth.auth_checks import allowed_route_check_inside_route
    from litelm.proxy._types import UserAPIKeyAuth, LitellmUserRoles
...
def test_read_request_body():
    from litelm.proxy.common_utils.http_parsing_utils import _read_request_body
    from fastapi import Request
...
async def test_soft_budget_alert():
    ...
    setattr(litelm.proxy.proxy_server, "user_api_key_cache", user_api_key_cache)
    setattr(litelm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(litelm.proxy.proxy_server, "prisma_client", AsyncMock())
    ...
    alert_called = False
    original_budget_alerts = litelm.proxy.proxy_server.proxy_logging_obj.budget_alerts
...
def test_is_allowed_route():
    from litelm.proxy.auth.auth_checks import _is_allowed_route
    from litelm.proxy._types import UserAPIKeyAuth
    import datetime
...
def test_is_user_proxy_admin(user_obj, expected_result):
    from litelm.proxy.auth.auth_checks import _is_user_proxy_admin
...
def test_get_user_role(user_obj, expected_role):
    from litelm.proxy.auth.user_api_key_auth import _get_user_role
...
async def test_user_api_key_auth_websocket():
    from litelm.proxy.auth.user_api_key_auth import user_api_key_auth_websocket
    ...
    with patch(
        "litelm.proxy.auth.user_api_key_auth.user_api_key_auth", autospec=True
...
async def test_jwt_user_api_key_auth_builder_enforce_rbac(enforce_rbac, monkeypatch):
    from litelm.proxy.auth.handle_jwt import JWTHandler, JWTAuthManager
    ...
    local_cache.set_cache(
        key="litelm_jwt_auth_keys_my-fake-url",
        value=keys,
    )

    litelm_jwtauth = LiteLLM_JWTAuth(
        **{
            "admin_jwt_scope": "litelm_proxy_endpoints_access",
...
def test_can_rbac_role_call_model():
    from litelm.proxy.auth.handle_jwt import JWTAuthManager
    from litelm.proxy._types import RoleBasedPermissions
...
def test_get_model_from_request(route, request_data, expected_model):
    from litelm.proxy.auth.user_api_key_auth import get_model_from_request
...
async def test_jwt_non_admin_team_route_access(monkeypatch):
    ...
    monkeypatch.setattr(
        litelm.proxy.proxy_server, "general_settings", {"enable_jwt_auth": True}
    )
    ...
    # "JWTHandler has no attribute 'litelm_jwtauth'"
    from litelm.proxy._types import LiteLLM_JWTAuth
    from litelm.caching.dual_cache import DualCache

    litelm.proxy.proxy_server.jwt_handler.update_environment(
        prisma_client=None,
    ...
    with patch(
        ...
        "litelm.proxy.auth.handle_jwt.JWTAuthManager.auth_builder",
...
@pytest.mark.asyncio
async def test_x_litelm_api_key():
    """
    Check if auth can pick up x-litelm-api-key header, even if Bearer token is provided
    """
    from fastapi import Request
    from starlette.datastructures import URL
    ...
    valid_token = await user_api_key_auth(request=request, api_key="Bearer " + ignored_key, custom_litelm_key_header=master_key)
