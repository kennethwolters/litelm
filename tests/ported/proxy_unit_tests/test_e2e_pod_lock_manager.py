...
import pytest
from litelm.proxy.db.db_transaction_queue.pod_lock_manager import PodLockManager
import litelm
...
async def test_e2e_size_of_redis_buffer():
    ...
    from litelm.proxy.db.db_transaction_queue.base_update_queue import BaseUpdateQueue
    from litelm.caching import RedisCache
    from litelm._uuid import uuid
    ...
    setattr(litelm.proxy.proxy_server, "use_redis_transaction_buffer", True)
    ...
    with patch("litelm.proxy.db.db_spend_update_writer.PodLockManager.acquire_lock", return_value=True):
