...
from litelm.llms.bedrock.count_tokens.bedrock_token_counter import BedrockTokenCounter
from litelm.llms.bedrock.count_tokens.handler import BedrockCountTokensHandler
from litelm.proxy._types import ProxyException, TokenCountRequest
from litelm.proxy.anthropic_endpoints.endpoints import (
    count_tokens as anthropic_count_tokens,
)
from litelm.proxy.proxy_server import token_counter
from litelm.types.utils import TokenCountResponse
...
async def test_anthropic_messages_count_tokens_endpoint():
    ...
    """
    from litelm.proxy.anthropic_endpoints.endpoints import count_tokens
    ...
    import litelm.proxy.anthropic_endpoints.endpoints as anthropic_endpoints
...
async def test_factory_anthropic_endpoint_calls_anthropic_counter():
    ...
    with patch(
        "litelm.llms.anthropic.count_tokens.token_counter.anthropic_count_tokens_handler",
        mock_handler
    ):
        # Mock router to return Anthropic deployment
        with patch("litelm.proxy.proxy_server.llm_router") as mock_router:
            mock_router.model_list = [
                {
                    "model_name": "claude-3-5-sonnet",
                    "litelm_params": {"model": "anthropic/claude-3-5-sonnet-20241022"},
                    "model_info": {},
                }
            ]
...
async def test_factory_gpt4_endpoint_does_not_call_anthropic_counter():
    ...
    with patch(
        ...
        # Mock litelm token counter
        with patch("litelm.token_counter") as mock_litelm_counter:
            mock_litelm_counter.return_value = 50
            ...
            with patch("litelm.proxy.proxy_server.llm_router") as mock_router:
                mock_router.model_list = [
                    {
                        "model_name": "gpt-4",
                        "litelm_params": {"model": "openai/gpt-4"},
                        "model_info": {},
                    }
                ]
...
async def test_factory_normal_token_counter_endpoint_does_not_call_anthropic():
    ...
    with patch(
        ...
        # Mock litelm token counter
        with patch("litelm.token_counter") as mock_litelm_counter:
            mock_litelm_counter.return_value = 35
...
async def test_factory_registration():
    ...
    from litelm.llms.anthropic.common_utils import AnthropicModelInfo
    ...
    non_anthropic_deployment = {"litelm_params": {"model": "openai/gpt-4"}}
...
async def test_bedrock_count_tokens_endpoint():
    ...
    async def mock_count_tokens_handler(request_data, litelm_params, resolved_model):
...
async def test_vertex_ai_anthropic_token_counting():
    ...
    with patch(
        "litelm.llms.vertex_ai.vertex_ai_partner_models.count_tokens.handler.VertexAIPartnerModelsTokenCounter.handle_count_tokens_request"
...
def test_vertex_ai_partner_models_token_counting_endpoint(vertex_location):
    ...
    """
    from litelm.llms.vertex_ai.vertex_ai_partner_models.count_tokens.handler import (
...
async def test_bedrock_token_counter_error_propagation_bedrock_error():
    ...
    with patch.object(
        ...
        with patch(
            "litelm.llms.bedrock.count_tokens.bedrock_token_counter.BedrockCountTokensHandler"
        ) as MockHandler:
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    with patch.object(handler, "_sign_request", return_value=({}, "{}")):
                        with patch(
                            "litelm.llms.bedrock.count_tokens.handler.get_async_httpx_client"
                        ) as mock_client:
...
async def test_proxy_token_counter_error_raises_exception_when_disabled():
    ...
    setattr(litelm.proxy.proxy_server, "llm_router", mock_router)

    # Save original value and function
    original_disable = litelm.disable_token_counter
    original_get_provider_token_counter = litelm.proxy.proxy_server._get_provider_token_counter

    try:
        litelm.disable_token_counter = True
        ...
        litelm.proxy.proxy_server._get_provider_token_counter = mock_get_provider_token_counter
        ...
        litelm.disable_token_counter = original_disable
        litelm.proxy.proxy_server._get_provider_token_counter = original_get_provider_token_counter
...
async def test_proxy_token_counter_error_falls_back_when_enabled():
    ...
    try:
        litelm.disable_token_counter = False
