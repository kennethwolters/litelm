...
def test_load_config_with_custom_role_names():
    config = {
        "general_settings": {
            "litelm_proxy_roles": {"admin_jwt_scope": "litelm-proxy-admin"}
        }
    }
    proxy_roles = LiteLLM_JWTAuth(
        **config.get("general_settings", {}).get("litelm_proxy_roles", {})
    )

    print(f"proxy_roles: {proxy_roles}")

    assert proxy_roles.admin_jwt_scope == "litelm-proxy-admin"
...
async def test_token_single_public_key(monkeypatch):
    ...
    await cache.async_set_cache(
        key="litelm_jwt_auth_keys_https://example.com/public-key",
...
@pytest.mark.parametrize("audience", [None, "litelm-proxy"])
@pytest.mark.asyncio
async def test_valid_invalid_token(audience, monkeypatch):
    ...
    await cache.async_set_cache(
        key="litelm_jwt_auth_keys_https://example.com/public-key", value=[public_jwk]
    )
...
def prisma_client():
    ...
    from litelm.proxy.utils import PrismaClient, ProxyLogging
...
async def test_team_token_output(prisma_client, audience, monkeypatch):
    ...
    jwt_handler.litelm_jwtauth = LiteLLM_JWTAuth(team_id_jwt_field="client_id")
    ...
    setattr(litelm.proxy.proxy_server, "jwt_handler", jwt_handler)
    try:
...
async def aaaatest_user_token_output(
    ...
    import litelm
    from litelm.proxy._types import NewTeamRequest, NewUserRequest, UserAPIKeyAuth
    ...
    jwt_handler.litelm_jwtauth.user_id_jwt_field = "sub"
    jwt_handler.litelm_jwtauth.team_id_default = default_team_id
    jwt_handler.litelm_jwtauth.user_id_upsert = user_id_upsert

    if team_id_set:
        jwt_handler.litelm_jwtauth.team_id_jwt_field = "client_id"
    ...
    ## 1. INITIAL TEAM CALL - should fail
    # use generated key to auth in
    setattr(litelm.proxy.proxy_server, "general_settings", {"enable_jwt_auth": True})
    setattr(litelm.proxy.proxy_server, "jwt_handler", jwt_handler)
    try:
        result = await user_api_key_auth(request=request, api_key=bearer_token)
        pytest.fail("Team doesn't exist. This should fail")
...
async def test_allowed_routes_admin(
    ...
    pseudo_routes = jwt_handler.litelm_jwtauth.admin_allowed_routes
...
async def test_allow_access_by_email(
    ...
    Relevant issue: https://github.com/BerriAI/litelm/issues/5605
    """
    import jwt
    from starlette.datastructures import URL
    ...
    setattr(litelm.proxy.proxy_server, "jwt_handler", jwt_handler)
    setattr(litelm.proxy.proxy_server, "prisma_client", {})
...
async def test_end_user_jwt_auth(monkeypatch):
    ...
    from litelm.types.router import RouterGeneralSettings
    ...
    setattr(litelm.proxy.proxy_server, "prisma_client", {})
    setattr(litelm.proxy.proxy_server, "jwt_handler", jwt_handler)
    from litelm.proxy.proxy_server import cost_tracking
    ...
    mock_response = litelm.ModelResponse(
        id="chatcmpl-mock",
        choices=[
            litelm.Choices(
                finish_reason="stop",
                index=0,
                message=litelm.Message(
    ...
    with patch("litelm.acompletion", new=AsyncMock(return_value=mock_response)) as mock_completion:
...
def test_check_scope_based_access(requested_model, should_work):
    from litelm.proxy.auth.handle_jwt import JWTAuthManager
    from litelm.proxy._types import ScopeMapping
    ...
    args = {
        "scope_mappings": [
            ScopeMapping(
                ...
                scope="litelm.api.consumer",
            ),
            ScopeMapping(
                models=["gpt-3.5-turbo-testing"],
                routes=None,
                scope="litelm.api.gpt_3_5_turbo",
            ),
        ],
        "scopes": [
            "profile",
...
async def test_custom_validate_called():
    ...
    jwt_handler.litelm_jwtauth = MagicMock(
...
async def test_auth_jwt_es256_jwk_path(monkeypatch):
    ...
    now = int(time.time())
    token = jwt.encode(
        {"sub": "alice", "aud": "litelm-proxy", "iss": "http://example", "iat": now, "exp": now + 300},
        ec_priv_pem,
        algorithm="ES256",
        headers={"kid": "ec1"},
    )
...
async def test_auth_jwt_rs256_regression(monkeypatch):
    ...
    now = int(time.time())
    token = jwt.encode(
        {"sub": "bob", "aud": "litelm-proxy", "iss": "http://example", "iat": now, "exp": now + 300},
        rsa_priv_pem,
        algorithm="RS256",
        headers={"kid": "rsa1"},
    )
...
async def test_auth_jwt_mismatched_key_fails(monkeypatch):
    ...
    now = int(time.time())
    token = jwt.encode(
        {"sub": "mallory", "aud": "litelm-proxy", "iss": "http://example", "iat": now, "exp": now + 300},
        ec_priv_pem,
        algorithm="ES256",
        headers={"kid": "ec1"},
    )
