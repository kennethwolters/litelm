...
import litelm.proxy
import litelm.proxy.proxy_server
...
# this file is to test litelm/proxy
...
from litelm.proxy.proxy_server import (  # Replace with the actual module where your FastAPI router is defined
    app,
    initialize,
    save_worker_config,
)
...
def mock_patch_acompletion():
    return mock.patch(
        "litelm.proxy.proxy_server.llm_router.acompletion",
...
def mock_patch_aimage_generation():
    return mock.patch(
        "litelm.proxy.proxy_server.llm_router.aimage_generation",
...
def test_chat_completion(mock_acompletion, client_no_auth):
    ...
    try:
        ...
        mock_acompletion.assert_called_once_with(
            ...
            max_tokens=10,
            litelm_call_id=mock.ANY,
            litelm_logging_obj=mock.ANY,
...
def test_get_settings_request_timeout(client_no_auth):
    """
    When no timeout is set, it should use the litelm.request_timeout value
    """
    # Set a known value for litelm.request_timeout
    import litelm
    ...
    assert settings["litelm.request_timeout"] == litelm.request_timeout


@pytest.mark.parametrize(
    "litelm_key_header_name",
    ["x-litelm-key", None],
)
def test_add_headers_to_request(litelm_key_header_name):
    from fastapi import Request
    from starlette.datastructures import URL
    import json
    from litelm.proxy.litelm_pre_call_utils import (
        clean_headers,
    ...
    request_headers = clean_headers(headers, litelm_key_header_name)
...
def test_chat_completion_forward_headers(
    mock_acompletion, client_no_auth, litelm_key_header_name, forward_headers
):
    global headers
    try:
        if forward_headers:
            gs = getattr(litelm.proxy.proxy_server, "general_settings")
            gs["forward_client_headers_to_llm_api"] = True
            setattr(litelm.proxy.proxy_server, "general_settings", gs)
        if litelm_key_header_name is not None:
            gs = getattr(litelm.proxy.proxy_server, "general_settings")
            gs["litelm_key_header_name"] = litelm_key_header_name
            setattr(litelm.proxy.proxy_server, "general_settings", gs)
        # Your test data
        test_data = {
            "model": "gpt-3.5-turbo",
        ...
        if litelm_key_header_name is not None:
            headers_to_not_forward = {litelm_key_header_name: "Bearer 1234"}
        else:
...
async def test_team_disable_guardrails(mock_acompletion, client_no_auth):
    ...
    from litelm.proxy.proxy_server import hash_token, user_api_key_cache
    ...
    setattr(litelm.proxy.proxy_server, "user_api_key_cache", user_api_key_cache)
    setattr(litelm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(litelm.proxy.proxy_server, "prisma_client", "hello-world")
...
def test_custom_logger_failure_handler(mock_acompletion, client_no_auth):
    ...
    litelm.callbacks = [mock_logger, mock_logger_unit_tests]
    proxy_logging_obj._init_litelm_callbacks(llm_router=None)

    setattr(litelm.proxy.proxy_server, "user_api_key_cache", user_api_key_cache)
    setattr(litelm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(litelm.proxy.proxy_server, "prisma_client", "FAKE-VAR")
    setattr(litelm.proxy.proxy_server, "proxy_logging_obj", proxy_logging_obj)
...
def test_embedding(mock_aembedding, client_no_auth):
    global headers
    from litelm.proxy.proxy_server import user_custom_auth
...
def test_chat_completion_optional_params(mock_acompletion, client_no_auth):
    ...
    # This tests if all the /chat/completion params are passed to litelm
    try:
        # Your test data
        ...
        litelm.callbacks = [customHandler]
...
@mock.patch("litelm.proxy.proxy_server.litelm.Cache")
def test_load_router_config(mock_cache, fake_env_vars):
    ...
    try:
        ...
        # tests for litelm.cache set from config
        ...
        assert litelm.cache is not None
        assert "redis_client" in vars(
            litelm.cache.cache
        )  # it should default to redis on proxy
        assert litelm.cache.supported_call_types == [
            "completion",
            "acompletion",
            "embedding",
            "aembedding",
        ...
        assert litelm.cache is not None
        print(litelm.cache)
        print(litelm.cache.supported_call_types)
        print(vars(litelm.cache.cache))
        assert "redis_client" in vars(
            litelm.cache.cache
        )  # it should default to redis on proxy
        assert litelm.cache.supported_call_types == [
...
async def test_team_update_redis():
    ...
    """
    from litelm.caching.caching import DualCache, RedisCache
    from litelm.proxy._types import LiteLLM_TeamTableCachedObj
    from litelm.proxy.auth.auth_checks import _cache_team_object
...
async def test_get_team_redis(client_no_auth):
    ...
    """
    from litelm.caching.caching import DualCache, RedisCache
    from litelm.proxy.auth.auth_checks import get_team_object
...
from litelm.proxy.management_endpoints.internal_user_endpoints import new_user
from litelm.proxy.management_endpoints.team_endpoints import team_member_add
...
async def test_create_user_default_budget(prisma_client, user_role):
    ...
    setattr(litelm.proxy.proxy_server, "master_key", "sk-1234")
    setattr(litelm, "max_internal_user_budget", 10)
    setattr(litelm, "internal_user_budget_duration", "5m")
    ...
    with patch.object(
        litelm.proxy.proxy_server.prisma_client, "insert_data", new=AsyncMock()
    ) as mock_client:
        await new_user(
            request,
        )
        ...
        if user_role == LitellmUserRoles.INTERNAL_USER.value:
            assert (
                ...
                == litelm.max_internal_user_budget
            )
            assert (
                mock_client.call_args.kwargs["data"]["budget_duration"]
                == litelm.internal_user_budget_duration
            )
...
async def test_create_team_member_add(prisma_client, new_member_method):
    ...
    from litelm.proxy._types import LiteLLM_TeamTableCachedObj, LiteLLM_UserTable
    from litelm.proxy.proxy_server import hash_token, user_api_key_cache
    ...
    await litelm.proxy.proxy_server.prisma_client.connect()
    user = f"ishaan {uuid.uuid4().hex}"
    _team_id = "litelm-test-client-id-new"
    team_obj = LiteLLM_TeamTableCachedObj(
        team_id=_team_id,
        blocked=False,
        last_refreshed_at=time.time(),
    ...
    with patch(
        "litelm.proxy.proxy_server.prisma_client.db.litelm_usertable",
        new_callable=AsyncMock,
    ) as mock_litelm_usertable, patch(
        "litelm.proxy.auth.auth_checks._get_team_object_from_user_api_key_cache",
        new=AsyncMock(return_value=team_obj),
    ) as mock_team_obj, patch(
        "litelm.proxy.proxy_server.prisma_client.get_data",
        ...
        mock_litelm_usertable.upsert = mock_client
        mock_litelm_usertable.find_many = AsyncMock(return_value=None)
        # Mock find_first for user_email validation (returns None for new users)
        mock_litelm_usertable.find_first = AsyncMock(return_value=None)
        # Mock find_unique for user_id validation (returns None for new users)
        mock_litelm_usertable.find_unique = AsyncMock(return_value=None)
        team_mock_client = AsyncMock()
        original_val = getattr(
            litelm.proxy.proxy_server.prisma_client.db, "litelm_teamtable"
        )
        litelm.proxy.proxy_server.prisma_client.db.litelm_teamtable = team_mock_client
        ...
        litelm.proxy.proxy_server.prisma_client.db.litelm_teamtable = original_val
...
async def test_create_team_member_add_team_admin_user_api_key_auth(
    ...
    from litelm.proxy._types import LiteLLM_TeamTableCachedObj, Member
...
async def test_create_team_member_add_team_admin(
    ...
    """
    Relevant issue - https://github.com/BerriAI/litelm/issues/5300
...
async def test_user_info_team_list(prisma_client):
    ...
    from litelm.proxy.management_endpoints.internal_user_endpoints import user_info

    with patch(
        "litelm.proxy.management_endpoints.team_endpoints.list_team",
        new_callable=AsyncMock,
    ) as mock_client:
...
async def test_add_callback_via_key(prisma_client):
    ...
    try:
        ...
        with patch.object(
            litelm.litelm_core_utils.litelm_logging,
            "LangFuseLogger",
            new=MagicMock(),
        ) as mock_client:
            ...
            assert "user_api_key_metadata" in kwargs["litelm_params"]["metadata"]
            assert (
                "logging"
                in kwargs["litelm_params"]["metadata"]["user_api_key_metadata"]
            )
            checked_keys = False
            for item in kwargs["litelm_params"]["metadata"]["user_api_key_metadata"][
                "logging"
            ]:
...
async def test_add_callback_via_key_litelm_pre_call_utils(
    ...
    from litelm.proxy.litelm_pre_call_utils import add_litelm_data_to_request

    setattr(litelm.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(litelm.proxy.proxy_server, "master_key", "sk-1234")
    await litelm.proxy.proxy_server.prisma_client.connect()

    proxy_config = getattr(litelm.proxy.proxy_server, "proxy_config")
    ...
    new_data = await add_litelm_data_to_request(**data)
...
async def test_disable_fallbacks_by_key(disable_fallbacks_set):
    from litelm.proxy.litelm_pre_call_utils import LiteLLMProxyRequestSetup
...
async def test_add_callback_via_key_litelm_pre_call_utils_gcs_bucket(
    prisma_client, callback_type, expected_success_callbacks, expected_failure_callbacks
):
    import json
...
async def test_add_callback_via_key_litelm_pre_call_utils_langsmith(
    prisma_client, callback_type, expected_success_callbacks, expected_failure_callbacks
):
    import json
...
async def test_gemini_pass_through_endpoint():
    ...
    from litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints import (
        Request,
        Response,
        gemini_proxy_route,
    )
...
async def test_proxy_model_group_alias_checks(prisma_client, hidden):
    ...
    from litelm.proxy.proxy_server import model_group_info, model_info_v1, model_list
    ...
    _model_list = [
        {
            ...
            "litelm_params": {"model": "gpt-3.5-turbo"},
        }
    ]
    model_alias = "gpt-4"
    ...
    setattr(litelm.proxy.proxy_server, "llm_router", router)
    setattr(litelm.proxy.proxy_server, "llm_model_list", _model_list)
    ...
    _model_list = [
        {
            ...
            "litelm_params": {"model": "cohere/rerank-english-v3.0"},
            "model_info": {
                "mode": "rerank",
            },
        }
    ]
    router = litelm.Router(model_list=_model_list)
    setattr(litelm.proxy.proxy_server, "llm_router", router)
    setattr(litelm.proxy.proxy_server, "llm_model_list", _model_list)
...
#         new_team,
#     )
#     from litelm.proxy._types import TeamMemberAddRequest, Member, NewTeamRequest
...
async def test_proxy_server_prisma_setup():
    from litelm.proxy.proxy_server import ProxyStartupEvent, proxy_state
    from litelm.proxy.utils import ProxyLogging
    from litelm.caching import DualCache

    user_api_key_cache = DualCache()

    with patch.object(
        litelm.proxy.proxy_server, "PrismaClient", new=MagicMock()
...
async def test_get_ui_settings_spend_logs_threshold():
    ...
    from litelm.proxy.proxy_server import proxy_state
    from fastapi import Request
    from litelm.constants import MAX_SPENDLOG_ROWS_TO_QUERY
...
async def test_run_background_health_check_reflects_llm_model_list(monkeypatch):
    ...
    """
    import litelm.proxy.proxy_server as proxy_server
    import copy
...
def test_non_root_ui_path_logic(monkeypatch, tmp_path, ui_exists, ui_has_content):
    ...
    # Create a temporary directory to act as /tmp/litelm_ui
    test_ui_path = tmp_path / "litelm_ui"
...
async def test_update_config_success_callback_normalization():
    ...
    """
    import litelm.proxy.proxy_server as proxy_server
    from litelm.proxy._types import ConfigYAML
    ...
    class MockPrisma:
        def __init__(self):
            self.db = MagicMock()
            self.db.litelm_config = MagicMock()
            self.db.litelm_config.upsert = AsyncMock()
    ...
    class MockProxyConfig:
        ...
        async def get_config(self):
            ...
            return {"litelm_settings": {"success_callback": ["langfuse"]}}
    ...
    config_update = ConfigYAML(litelm_settings={"success_callback": ["SQS", "sQs"]})
    ...
    callbacks = saved["litelm_settings"]["success_callback"]
    ...
    [
        {
            "model": {
                ...
                "litelm_params": {"model": "azure/gpt-4.1-mini"},
        ...
        },
        {
            "model": {
                ...
                "litelm_params": {"model": "openai/gpt-4.1-mini"},
            },
        ...
        },
        {
            "model": {
                ...
                "litelm_params": {"model": "anthropic/claude-sonnet-4-5@20250929"},
        ...
        },
        {
            "model": {
                ...
                "litelm_params": {"model": "gemini/gemini-2.5-flash@001"},
...
def test_get_litelm_model_info(data):
    from litelm.proxy.proxy_server import get_litelm_model_info

    model = data["model"]
    get_info_mock = MagicMock()

    with mock.patch(
        "litelm.get_model_info",
        new=get_info_mock,
    ):
        get_litelm_model_info(model=model)
