...
import pytest, litelm
import httpx
from litelm.proxy._types import UserAPIKeyAuth
from litelm.proxy.auth.auth_checks import get_end_user_object
...
from litelm.proxy.utils import PrismaClient
from litelm.proxy.auth.auth_checks import (
    can_team_access_model,
    _virtual_key_soft_budget_check,
    _team_soft_budget_check,
)
from litelm.proxy.utils import ProxyLogging
from litelm.proxy.utils import CallInfo
...
async def test_get_end_user_object(customer_spend, customer_budget):
    ...
    try:
        ...
        if (
            isinstance(e, litelm.BudgetExceededError)
...
async def test_can_key_call_model(model, expect_to_work):
    ...
    """
    from litelm.proxy.auth.auth_checks import can_key_call_model
    ...
    router = litelm.Router(model_list=llm_model_list)
    args = {
        "model": model,
...
async def test_can_team_call_model(model, expect_to_work):
    from litelm.proxy.auth.auth_checks import model_in_access_group
    from fastapi import HTTPException
...
async def test_wildcard_access_after_cost_map_reload(key_models, model, expect_to_work):
    ...
    add_known_models(model_cost_map=new_map) must update litelm.anthropic_models
    ...
    litelm.model_cost but never re-ran add_known_models(), so the provider sets
    stayed stale and wildcard matching failed for new models.

    Fix: each reload now calls litelm.add_known_models(model_cost_map=new_map)
    ...
    new_cost_map = dict(litelm.model_cost)
    new_cost_map[model] = {
        "litelm_provider": "anthropic",
        "max_tokens": 8192,
        "input_cost_per_token": 0.000003,
        "output_cost_per_token": 0.000015,
    }

    original_model_cost = litelm.model_cost
    litelm.model_cost = new_cost_map

    # Confirm the model is NOT yet in the provider set before reload propagation.
    assert model not in litelm.anthropic_models

    # Simulate what proxy_server.py now does after every reload.
    litelm.add_known_models(model_cost_map=new_cost_map)

    # After add_known_models(), the model must be in the set.
    assert model in litelm.anthropic_models

    llm_model_list = [
        {
            "model_name": "anthropic/*",
            "litelm_params": {"model": "anthropic/*", "api_key": "test-api-key"},
            "model_info": {"id": "test-id-anthropic-wildcard", "db_model": False},
        },
        {
            "model_name": "openai/*",
            "litelm_params": {"model": "openai/*", "api_key": "test-api-key"},
    ...
    try:
        ...
        litelm.anthropic_models.discard(model)
...
async def test_add_known_models_explicit_map_updates_provider_sets():
    ...
    litelm.add_known_models(model_cost_map=new_model_cost_map) instead of
    ...
    assert fake_new_model not in litelm.anthropic_models
    ...
    try:
        assert fake_new_model in litelm.anthropic_models, (
            "add_known_models(model_cost_map=...) did not add the new model to "
            "litelm.anthropic_models — wildcard access checks would fail."
        )
    finally:
        # Clean up: restore original state.
        litelm.model_cost = original_model_cost
        litelm.anthropic_models.discard(fake_new_model)


@pytest.mark.asyncio
async def test_is_valid_fallback_model():
    from litelm.proxy.auth.auth_checks import is_valid_fallback_model
        ...
        model_list=[
            {
                ...
                "litelm_params": {"model": "openai/gpt-3.5-turbo"},
            }
        ]
    )
...
async def test_virtual_key_max_budget_check(
    ...
    """
    from litelm.proxy.auth.auth_checks import _virtual_key_max_budget_check
    from litelm.proxy.utils import ProxyLogging
    ...
    except litelm.BudgetExceededError as e:
...
async def test_can_user_call_model():
    from litelm.proxy.auth.auth_checks import can_user_call_model
        ...
        model_list=[
            {
                ...
                "litelm_params": {"model": "anthropic/anthropic-claude"},
            },
            {
                "model_name": "gpt-3.5-turbo",
                "litelm_params": {"model": "gpt-3.5-turbo", "api_key": "test-api-key"},
            },
        ]
    )
...
async def test_can_user_call_model_with_no_default_models():
    from litelm.proxy.auth.auth_checks import can_user_call_model
    from litelm.proxy._types import ProxyException, SpecialModelNames
...
async def test_get_fuzzy_user_object():
    from litelm.proxy.auth.auth_checks import _get_fuzzy_user_object
    from litelm.proxy.utils import PrismaClient
    ...
    mock_prisma.db.litelm_usertable.find_unique = AsyncMock(return_value=test_user)
    result = await _get_fuzzy_user_object(
        prisma_client=mock_prisma, sso_user_id="sso_123", user_email="test@example.com"
    )
    assert result == test_user
    mock_prisma.db.litelm_usertable.find_unique.assert_called_with(
        where={"sso_user_id": "sso_123"}, include={"organization_memberships": True}
    )

    # Test 2: SSO ID not found, find by email
    mock_prisma.db.litelm_usertable.find_unique = AsyncMock(return_value=None)
    mock_prisma.db.litelm_usertable.find_first = AsyncMock(return_value=test_user)
    ...
    assert result == test_user
    mock_prisma.db.litelm_usertable.find_first.assert_called_with(
    ...
    mock_prisma.db.litelm_usertable.update.assert_called_with(
        where={"user_id": "test_123"}, data={"sso_user_id": "new_sso_456"}
    )

    # Test 4: User not found by either method
    mock_prisma.db.litelm_usertable.find_unique = AsyncMock(return_value=None)
    mock_prisma.db.litelm_usertable.find_first = AsyncMock(return_value=None)
...
async def test_cache_access_object():
    ...
    from litelm.proxy.auth.auth_checks import _cache_access_object
    from litelm.proxy._types import LiteLLM_AccessGroupTable
...
async def test_delete_cache_access_object():
    ...
    from litelm.proxy.auth.auth_checks import _delete_cache_access_object
    from litelm.proxy._types import LiteLLM_AccessGroupTable
...
async def test_get_resources_from_access_groups(resource_field, access_group_data, expected):
    ...
    with patch(
        "litelm.proxy.auth.auth_checks.get_access_object",
...
async def test_get_models_from_access_groups_empty_ids():
    ...
    from litelm.proxy.auth.auth_checks import _get_models_from_access_groups
...
async def test_can_team_access_model_via_access_group_ids():
    ...
    from litelm.proxy.auth.auth_checks import can_team_access_model
    ...
    with patch(
        "litelm.proxy.auth.auth_checks._get_models_from_access_groups",
        new_callable=AsyncMock,
        return_value=["gpt-4"],
    ):
        result = await can_team_access_model(
...
async def test_can_key_call_model_via_access_group_ids():
    ...
    router = litelm.Router(
        model_list=[
            {
                "model_name": "gpt-4",
                "litelm_params": {"model": "openai/gpt-4", "api_key": "test"},
            }
        ]
    )
