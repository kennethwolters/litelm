...
def find_litelm_type_hints(directory: str) -> List[Tuple[str, int, str]]:
    """
    Recursively search for Python files in the given directory
    and find type hints containing 'litelm.'.
    ...
    def is_litelm_type_hint(node):
        """
        Recursively check if a type annotation contains 'litelm.'

        Handles more complex type hints like:
        - Optional[litelm.Type]
        - Union[litelm.Type1, litelm.Type2]
        - Nested type hints
        """
        try:
            ...
            # Direct check for litelm in type string
            if "litelm." in type_str:
                return True
            ...
            if isinstance(node, ast.Subscript):
                ...
                if isinstance(node.value, ast.Name) and node.value.id in [
                    ...
                    if isinstance(node.slice, ast.Tuple):
                        return any(is_litelm_type_hint(elt) for elt in node.slice.elts)
                    else:
                        return is_litelm_type_hint(node.slice)

                # Recursive check for subscripted types
                return is_litelm_type_hint(node.value) or is_litelm_type_hint(
                    node.slice
                )

            # Recursive check for attribute types
            if isinstance(node, ast.Attribute):
                return "litelm." in ast.unparse(node)

            # Recursive check for name types
            if isinstance(node, ast.Name):
                return "litelm" in node.id
        ...
        try:
            ...
            for node in ast.walk(tree):
                ...
                if isinstance(node, ast.AnnAssign) and node.annotation:
                    if is_litelm_type_hint(node.annotation):
                        litelm_type_hints.append(
                ...
                # Check type hints in function arguments
                    ...
                    for arg in node.args.args:
                        if arg.annotation and is_litelm_type_hint(arg.annotation):
                            litelm_type_hints.append(
                                (file_path, arg.lineno, ast.unparse(arg.annotation))
                            )

                    # Check return type annotation
                    if node.returns and is_litelm_type_hint(node.returns):
                        litelm_type_hints.append(
...
def main():
    ...
    results = find_litelm_type_hints(directory)
