...
# Define the base directory for the litelm repository and documentation path
repo_base = "./litelm"  # Change this to your actual path

# Regular expressions to capture the keys used in os.getenv() and litelm.get_secret()
getenv_pattern = re.compile(r'os\.getenv\(\s*[\'"]([^\'"]+)[\'"]\s*(?:,\s*[^)]*)?\)')
get_secret_pattern = re.compile(
    r'litelm\.get_secret\(\s*[\'"]([^\'"]+)[\'"]\s*(?:,\s*[^)]*|,\s*default_value=[^)]*)?\)'
)
get_secret_str_pattern = re.compile(
    r'litelm\.get_secret_str\(\s*[\'"]([^\'"]+)[\'"]\s*(?:,\s*[^)]*|,\s*default_value=[^)]*)?\)'
)
...
# Directories to skip (dependencies, venvs, caches) - only scan litelm source
SKIP_DIRS = {
    ".venv",
    "venv",
    "__pycache__",
...
# Walk through all files in the litelm repo to find references of os.getenv() and litelm.get_secret()
