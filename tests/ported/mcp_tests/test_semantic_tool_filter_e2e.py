...
@pytest.mark.skipif(
    ...
    reason="semantic-router not installed. Install with: pip install 'litelm[semantic-router]'"
)
@pytest.mark.skipif(
...
async def test_e2e_semantic_filter():
    ...
    from litelm.proxy.hooks.mcp_semantic_filter import SemanticToolFilterHook
    from litelm.proxy._experimental.mcp_server.semantic_tool_filter import (
    ...
    router = Router(
        model_list=[{
            ...
            "litelm_params": {"model": "openai/text-embedding-3-small"},
        }]
    )
