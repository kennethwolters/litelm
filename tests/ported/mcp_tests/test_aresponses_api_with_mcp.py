...
import litelm
from litelm.responses.mcp.litelm_proxy_mcp_handler import LiteLLM_Proxy_MCP_Handler
from litelm.types.llms.openai import ResponsesAPIResponse, ResponsesAPIStreamingResponse, OpenAIMcpServerTool, ToolParam
...
async def test_mcp_helper_methods():
    ...
    # Test _should_use_litelm_mcp_gateway
    mcp_tools: List[Any] = [
        {
            "type": "mcp",
            "server_url": "litelm_proxy",
    ...
    # Should return True for MCP tools with litelm_proxy
    assert LiteLLM_Proxy_MCP_Handler._should_use_litelm_mcp_gateway(mcp_tools) == True
    
    # Should return False for other tools
    assert LiteLLM_Proxy_MCP_Handler._should_use_litelm_mcp_gateway(other_tools) == False
    
    # Should return False for None
    assert LiteLLM_Proxy_MCP_Handler._should_use_litelm_mcp_gateway(None) == False
...
async def test_aresponses_api_with_mcp_mock_integration():
    ...
    """
    # Define MCP tools with litelm_proxy server_url and require_approval="never"
    ...
    should_use_mcp = LiteLLM_Proxy_MCP_Handler._should_use_litelm_mcp_gateway(cast(Any, mcp_tools))
    assert should_use_mcp == True, "Should detect MCP tools with litelm_proxy server_url"
    ...
    assert mcp_parsed[0]["server_url"] == "litelm_proxy", "Should preserve server_url"
...
async def test_aresponses_api_with_mcp_passes_mcp_server_auth_headers_to_process_tools():
    ...
    """
    from litelm.responses.main import aresponses_api_with_mcp
    ...
    mcp_tools = [{"type": "mcp", "server_url": "litelm_proxy"}]
    secret_fields = {
    ...
    with patch.object(
        ...
        "litelm.responses.main.aresponses",
...
async def test_mcp_allowed_tools_filtering():
    ...
    filtered_tools = LiteLLM_Proxy_MCP_Handler._filter_mcp_tools_by_allowed_tools(
        ...
        mcp_tools_with_litelm_proxy=cast(List[ToolParam], mcp_tool_config_with_allowed_tools)
    )
    ...
    filtered_tools_all = LiteLLM_Proxy_MCP_Handler._filter_mcp_tools_by_allowed_tools(
        mcp_tools=mock_mcp_tools_from_server,
        mcp_tools_with_litelm_proxy=cast(List[ToolParam], mcp_tool_config_without_allowed_tools)
    )
    ...
    mock_mcp_tools_with_duplicates = [
        ...
        type('MCPTool', (), {
            'name': 'GitMCP-fetch_litelm_documentation',
            'description': 'Fetch entire documentation file from GitHub repository: BerriAI/litelm. Useful for general questions. Always call this tool first if asked about BerriAI/litelm.',
        ...
        # Other unique tools
        type('MCPTool', (), {
            'name': 'GitMCP-search_litelm_documentation',
            'description': 'Semantically search within the fetched documentation from GitHub repository: BerriAI/litelm. Useful for specific queries.',
    ...
    mcp_tool_config_with_duplicates = [
        {
            "type": "mcp",
            "server_label": "litelm",
            "server_url": "litelm_proxy/mcp",
            "require_approval": "never",
            "allowed_tools": ["GitMCP-fetch_litelm_documentation"]
        }
    ]
    ...
    filtered_tools_with_duplicates = LiteLLM_Proxy_MCP_Handler._filter_mcp_tools_by_allowed_tools(
        ...
        mcp_tools_with_litelm_proxy=cast(List[ToolParam], mcp_tool_config_with_duplicates)
    )
    ...
    assert filtered_tools_deduplicated[0].name == "GitMCP-fetch_litelm_documentation", \
        f"Expected GitMCP-fetch_litelm_documentation, got {filtered_tools_deduplicated[0].name}"
    ...
    # Should return 2 unique tools (GitMCP-fetch_litelm_documentation and GitMCP-search_litelm_documentation)
    ...
    expected_unique_names = ["GitMCP-fetch_litelm_documentation", "GitMCP-search_litelm_documentation"]
    ...
    filtered_tools_multiple = LiteLLM_Proxy_MCP_Handler._filter_mcp_tools_by_allowed_tools(
        mcp_tools=mock_mcp_tools_from_server,
        mcp_tools_with_litelm_proxy=cast(List[ToolParam], multiple_mcp_configs)
    )
    ...
    filtered_tools_empty = LiteLLM_Proxy_MCP_Handler._filter_mcp_tools_by_allowed_tools(
        mcp_tools=mock_mcp_tools_from_server,
        mcp_tools_with_litelm_proxy=cast(List[ToolParam], mcp_config_empty_allowed)
    )
...
async def test_streaming_mcp_events_validation():
    ...
    from litelm.types.llms.openai import ResponsesAPIStreamEvents
    ...
    mock_mcp_tools = [
        type(
            ...
            {
                "name": "search_repo",
                "description": "Search BerriAI/litelm repository for information",
                "inputSchema": {
                    "type": "object",
                    "properties": {
    ...
    with patch.object(
        ...
        mcp_tool_config = {
            "type": "mcp",
            "server_url": "litelm_proxy/mcp/test_server",
...
async def test_streaming_responses_api_with_mcp_tools(
    ...
    """
    Test the streaming responses API with MCP tools when using server_url="litelm_proxy"

    Under the hood the follow occurs

    - MCP: responses called litelm MCP manager.list_tools (MOCKED)
    ...
    with caplog.at_level(logging.ERROR):
        with patch.object(
            ...
            mock_get_tools.return_value = (mock_mcp_tools, ["litelm_proxy"])
                ...
                input=[
                    {
                        "role": "user",
                        "type": "message",
                        "content": "give me a TLDR of what BerriAI/litelm is about",
                    }
                ],
                stream=True,
            )
...
async def test_mcp_tool_execution_events_creation():
    ...
    mock_tool_calls = [
        ...
        },
        {
            "id": "call_def456", 
            "name": "get_repo_info",
            "arguments": '{"repo_name": "BerriAI/litelm"}',
            "type": "function_call"
        }
    ]
    ...
    mock_tool_results = [
        ...
        },
        {
            ...
            "result": "Repository: BerriAI/litelm - Python library for LLM integration"
        }
    ]
...
async def test_no_duplicate_mcp_tools_in_streaming_e2e():
    ...
    with patch.object(LiteLLM_Proxy_MCP_Handler, '_get_mcp_tools_from_manager', new_callable=AsyncMock) as mock_get_tools, \
         patch('litelm.aresponses', side_effect=capture_llm_tools) as mock_aresponses:
