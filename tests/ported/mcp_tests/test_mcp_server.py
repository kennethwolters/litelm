...
async def test_mcp_server_manager_https_server():
    ...
    with patch(
        "litelm.proxy._experimental.mcp_server.mcp_server_manager.MCPClient",
...
async def test_streamable_http_mcp_handler_mock():
    ...
    with patch(
        "litelm.proxy._experimental.mcp_server.server._SESSION_MANAGERS_INITIALIZED",
        True,
    ), patch(
        "litelm.proxy._experimental.mcp_server.server.session_manager",
        mock_session_manager,
    ), patch(
        "litelm.proxy._experimental.mcp_server.server.extract_mcp_auth_context",
...
async def test_sse_mcp_handler_mock():
    ...
    with patch(
        ...
        "litelm.proxy._experimental.mcp_server.server.set_auth_context",
    ):
        from litelm.proxy._experimental.mcp_server.server import handle_sse_mcp
...
async def test_list_tools_rest_api_server_not_found():
    ...
    from litelm.proxy._experimental.mcp_server.rest_endpoints import (
        list_tool_rest_api,
    ...
    with patch(
        "litelm.proxy._experimental.mcp_server.rest_endpoints.global_mcp_server_manager"
    ) as mock_manager:
...
async def test_list_tools_rest_api_success():
    ...
    with patch(
        ...
        with patch(
            "litelm.proxy._experimental.mcp_server.rest_endpoints._get_tools_for_single_server"
        ) as mock_get_tools:
...
async def test_get_tools_from_mcp_servers():
    ...
    try:
        ...
        with patch(
            ...
            with patch(
                "litelm.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp.MCPRequestHandler._get_mcp_servers_from_access_groups",
...
async def test_mcp_server_manager_access_groups_from_config():
    ...
    import litelm.proxy._experimental.mcp_server.mcp_server_manager as mcp_server_manager_mod
...
async def test_mcp_server_manager_config_integration_with_database():
    ...
    """
    import datetime
    from litelm.proxy._types import LiteLLM_MCPServerTable
...
def test_get_server_prefix_with_alias():
    ...
    """
    from litelm.proxy._experimental.mcp_server.utils import get_server_prefix
...
def test_normalize_server_name():
    ...
    """
    from litelm.proxy._experimental.mcp_server.utils import normalize_server_name
...
def test_add_server_prefix_to_name():
    ...
    from litelm.proxy._experimental.mcp_server.utils import add_server_prefix_to_name
...
async def test_list_tool_rest_api_with_server_specific_auth():
    ...
    from litelm.proxy._experimental.mcp_server.rest_endpoints import list_tool_rest_api
    from litelm.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp import (
        MCPRequestHandler,
    )
    from litelm.proxy._types import UserAPIKeyAuth
...
async def test_call_mcp_tool_uses_manager_permission_lookup():
    ...
    with patch.object(
        ...
        "litelm.proxy._experimental.mcp_server.server.global_mcp_tool_registry"
    ) as mock_tool_registry, patch(
        "litelm.proxy._experimental.mcp_server.server._handle_managed_mcp_tool",
        new_callable=AsyncMock,
    ) as mock_handle_managed, patch(
        "litelm.proxy._experimental.mcp_server.server.MCPRequestHandler.is_tool_allowed",
        return_value=True,
    ):
