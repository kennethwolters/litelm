...
litelm_provider were forwarded. Fields like cache_read_input_token_cost,
...
import litelm
from litelm.main import _build_custom_pricing_entry
...
def test_build_custom_pricing_entry_includes_all_kwargs_fields():
    ...
    assert entry["litelm_provider"] == "openai"
...
def test_register_model_receives_cache_pricing_fields():
    ...
    the cache pricing fields should be present in litelm.model_cost."""
    ...
    registered = litelm.model_cost.get(model_key)
    ...
    # Cleanup
    litelm.model_cost.pop(model_key, None)
