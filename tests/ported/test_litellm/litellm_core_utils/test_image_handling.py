...
from litelm.litelm_core_utils.prompt_templates.image_handling import (
...
def test_invalid_image_url_raises_bad_request(monkeypatch):
    monkeypatch.setattr(litelm, "module_level_client", DummyClient())
...
def test_image_exceeds_size_limit_with_content_length(monkeypatch):
    ...
    """
    monkeypatch.setattr(litelm, "module_level_client", LargeImageClient(size_mb=100))
...
def test_image_exceeds_size_limit_without_content_length(monkeypatch):
    ...
    """
    monkeypatch.setattr(
        litelm, "module_level_client", LargeImageClient(size_mb=100, include_content_length=False)
    )
...
def test_streaming_download_protects_against_huge_files(monkeypatch):
    ...
    monkeypatch.setattr(litelm, "module_level_client", client)
...
def test_image_within_size_limit(monkeypatch):
    ...
    """
    monkeypatch.setattr(litelm, "module_level_client", SmallImageClient())
...
def test_image_size_limit_disabled(monkeypatch):
    ...
    """
    import litelm.litelm_core_utils.prompt_templates.image_handling as image_handling
