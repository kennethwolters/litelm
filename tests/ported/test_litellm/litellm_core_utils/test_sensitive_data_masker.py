...
from litelm.litelm_core_utils.sensitive_data_masker import SensitiveDataMasker
...
def test_excluded_keys_exact_match():
    ...
    # With excluded_keys, litelm_credentials_name should NOT be masked (exact match)
    # This ensures that even if pattern matching logic changes, excluded keys won't be masked
    masked = masker.mask_dict(data, excluded_keys={"litelm_credentials_name"})
    assert masked["litelm_credentials_name"] == "my-credential-name"
    ...
    assert masked["litelm_credentials_name"] == "my-credential-name"  # Not masked because it doesn't match patterns anyway
...
def test_extra_headers_are_masked_recursively():
    ...
    masked = masker.mask_dict(data)
    extra_headers = masked["litelm_params"]["extra_headers"]
