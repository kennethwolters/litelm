"""
Tests for litelm.litelm_core_utils.llm_response_utils.response_metadata

Covers the callback_duration_ms timing metric that flows from the Logging object
through _hidden_params to the x-litelm-callback-duration-ms response header.
"""

import datetime
from unittest.mock import MagicMock

import litelm.litelm_core_utils.llm_response_utils.response_metadata as response_metadata_mod
import litelm.proxy.common_request_processing as common_request_processing_mod
...
class TestCallbackDurationMs:
    ...
    def test_update_response_metadata_includes_callback_duration(self):
        ...
        logging_obj.litelm_call_id = "test-call-id"
        ...
        assert hidden.get("litelm_overhead_time_ms") is not None
...
class TestCallbackDurationInCustomHeaders:
    ...
    def test_header_present_when_callback_duration_in_hidden_params(self):
        ...
        assert "x-litelm-callback-duration-ms" in headers
        assert headers["x-litelm-callback-duration-ms"] == "7.25"
    ...
    def test_header_absent_when_no_callback_duration(self):
        ...
        assert "x-litelm-callback-duration-ms" not in headers
...
class TestDetailedTiming:
    ...
    def test_detailed_timing_headers_in_custom_headers(self, monkeypatch):
        ...
        assert headers["x-litelm-timing-llm-api-ms"] == "500.0"
        assert headers["x-litelm-timing-pre-processing-ms"] == "20.0"
        assert headers["x-litelm-timing-post-processing-ms"] == "10.0"
        assert headers["x-litelm-timing-message-copy-ms"] == "2.5"
    ...
    def test_detailed_timing_headers_absent_when_disabled(self, monkeypatch):
        ...
        assert "x-litelm-timing-llm-api-ms" not in headers
        assert "x-litelm-timing-pre-processing-ms" not in headers
