...
from litelm.constants import SENTRY_DENYLIST, SENTRY_PII_DENYLIST
from litelm.litelm_core_utils.litelm_logging import Logging as LitellmLogging
from litelm.litelm_core_utils.litelm_logging import set_callbacks
from litelm.types.utils import ModelResponse, TextCompletionResponse
...
def test_use_custom_pricing_for_model():
    ...
    assert use_custom_pricing_for_model(litelm_params) == True
...
async def test_datadog_logger_not_shadowed_by_llm_obs(monkeypatch):
    ...
    from litelm.integrations.datadog.datadog import DataDogLogger
    from litelm.integrations.datadog.datadog_llm_obs import DataDogLLMObsLogger
    from litelm.litelm_core_utils import litelm_logging as logging_module
...
async def test_logfire_logger_accepts_env_vars_for_base_url(monkeypatch):
    ...
    from litelm.integrations.opentelemetry import OpenTelemetry  # logger class
    from litelm.litelm_core_utils import litelm_logging as logging_module
...
async def test_logging_result_for_bridge_calls(logging_obj):
    ...
    with patch.object(
        ...
        await litelm.anthropic_messages(
            max_tokens=100,
            messages=[{"role": "user", "content": "Hey"}],
...
async def test_logging_non_streaming_request():
    ...
    original_callbacks = getattr(litelm, "callbacks", [])

    try:
        mock_logging_obj = MockPrometheusLogger()

        litelm.callbacks = [mock_logging_obj]
...
def test_success_handler_skips_sync_callbacks_for_async_requests(
    ...
    logging_obj.model_call_details["litelm_params"] = {async_flag: True}
    logging_obj.litelm_params = logging_obj.model_call_details["litelm_params"]
...
def test_success_handler_runs_sync_callbacks_for_sync_requests(logging_obj, call_type):
    ...
    logging_obj.model_call_details["litelm_params"] = {}
    logging_obj.litelm_params = {}
...
def test_success_handler_skips_guardrail_logging_hook_when_disabled(logging_obj):
    ...
    from litelm.integrations.custom_logger import CustomLogger
    from litelm.types.guardrails import GuardrailEventHooks
...
def test_get_user_agent_tags():
    from litelm.litelm_core_utils.litelm_logging import StandardLoggingPayloadSetup

    tags = StandardLoggingPayloadSetup._get_user_agent_tags(
        proxy_server_request={
            "headers": {
                "user-agent": "litelm/0.1.0",
            }
        }
    )

    assert "User-Agent: litelm" in tags
    assert "User-Agent: litelm/0.1.0" in tags
...
def test_get_request_tags():
    ...
    tags = StandardLoggingPayloadSetup._get_request_tags(
        litelm_params={"metadata": {"tags": ["test-tag"]}},
        proxy_server_request={
            "headers": {
                "user-agent": "litelm/0.1.0",
            }
...
def test_get_request_tags_from_metadata_and_litelm_metadata():
    """
    Test that _get_request_tags correctly picks tags from both 'metadata' and 'litelm_metadata'.

    Scenarios tested:
    1. Tags in metadata only
    2. Tags in litelm_metadata only
    3. Tags in both (metadata should take priority)
    4. No tags in either
    5. None values for metadata/litelm_metadata
    """
    from litelm.litelm_core_utils.litelm_logging import StandardLoggingPayloadSetup

    # Test case 1: Tags in metadata only
    tags = StandardLoggingPayloadSetup._get_request_tags(
        litelm_params={"metadata": {"tags": ["metadata-tag-1", "metadata-tag-2"]}},
        proxy_server_request={},
    )
    ...
    # Test case 2: Tags in litelm_metadata only
    tags = StandardLoggingPayloadSetup._get_request_tags(
        litelm_params={
            "litelm_metadata": {
                "tags": ["litelm-metadata-tag-1", "litelm-metadata-tag-2"]
            }
        },
        proxy_server_request={},
    )
    assert "litelm-metadata-tag-1" in tags
    assert "litelm-metadata-tag-2" in tags
    assert len([t for t in tags if not t.startswith("User-Agent:")]) == 2
    ...
    tags = StandardLoggingPayloadSetup._get_request_tags(
        litelm_params={
            ...
            "litelm_metadata": {"tags": ["litelm-metadata-tag"]},
        },
        proxy_server_request={},
    )
    assert "metadata-tag" in tags
    assert "litelm-metadata-tag" not in tags
    assert len([t for t in tags if not t.startswith("User-Agent:")]) == 1

    # Test case 4: No tags in either
    tags = StandardLoggingPayloadSetup._get_request_tags(
        litelm_params={"metadata": {}, "litelm_metadata": {}},
        proxy_server_request={},
    )
    assert len([t for t in tags if not t.startswith("User-Agent:")]) == 0

    # Test case 5: None values for metadata/litelm_metadata
    tags = StandardLoggingPayloadSetup._get_request_tags(
        litelm_params={"metadata": None, "litelm_metadata": None},
        proxy_server_request={},
    )
    assert isinstance(tags, list)
    assert len([t for t in tags if not t.startswith("User-Agent:")]) == 0

    # Test case 6: Empty litelm_params
    tags = StandardLoggingPayloadSetup._get_request_tags(
        litelm_params={},
        proxy_server_request={},
    )
    ...
    tags = StandardLoggingPayloadSetup._get_request_tags(
        litelm_params={"metadata": {"tags": ["custom-tag"]}},
        proxy_server_request={
            "headers": {
                "user-agent": "litelm/1.0.0",
            }
        },
    )
    assert "custom-tag" in tags
    assert "User-Agent: litelm" in tags
    assert "User-Agent: litelm/1.0.0" in tags
...
def test_get_extra_header_tags():
    ...
    original_extra_headers = getattr(litelm, "extra_spend_tag_headers", None)

    try:
        # Test case 1: No extra headers configured
        litelm.extra_spend_tag_headers = None
        ...
        litelm.extra_spend_tag_headers = []
        result = StandardLoggingPayloadSetup._get_extra_header_tags(
            proxy_server_request={"headers": {"x-custom": "value"}}
        )
        assert result is None
        ...
        litelm.extra_spend_tag_headers = ["x-custom", "x-tenant"]
        result = StandardLoggingPayloadSetup._get_extra_header_tags(
        ...
        litelm.extra_spend_tag_headers = ["x-custom", "x-tenant", "x-missing"]
        result = StandardLoggingPayloadSetup._get_extra_header_tags(
            proxy_server_request={
                "headers": {
        ...
        litelm.extra_spend_tag_headers = ["x-custom", "x-empty"]
        result = StandardLoggingPayloadSetup._get_extra_header_tags(
        ...
        if original_extra_headers is not None:
            litelm.extra_spend_tag_headers = original_extra_headers
        else:
            # Remove the attribute if it didn't exist before
            if hasattr(litelm, "extra_spend_tag_headers"):
                delattr(litelm, "extra_spend_tag_headers")
...
def test_get_masked_values():
    from litelm.litelm_core_utils.litelm_logging import _get_masked_values
...
async def test_e2e_generate_cold_storage_object_key_successful():
    ...
    with patch("litelm.cold_storage_custom_logger", return_value="s3"), patch(
        "litelm.integrations.s3.get_s3_object_key"
    ) as mock_get_s3_key:
...
async def test_e2e_generate_cold_storage_object_key_with_custom_logger_s3_path():
    ...
    with patch("litelm.cold_storage_custom_logger", "s3_v2"), patch(
        "litelm.logging_callback_manager.get_active_custom_logger_for_callback_name"
...
async def test_e2e_generate_cold_storage_object_key_not_configured():
    ...
    with patch.object(litelm, "cold_storage_custom_logger", None):
        # Call the function
        result = StandardLoggingPayloadSetup._generate_cold_storage_object_key(
            start_time=start_time, response_id=response_id, team_alias=team_alias
        )
...
def test_get_usage_as_dict():
    ...
    """
    from litelm.litelm_core_utils.litelm_logging import StandardLoggingPayloadSetup
    from litelm.types.utils import Usage
...
async def test_async_success_handler_sets_standard_logging_object_for_pass_through_endpoints():
    ...
    from litelm.litelm_core_utils.litelm_logging import Logging as LiteLLMLoggingObj
    from litelm.types.utils import StandardPassThroughResponseObject
...
async def test_async_success_handler_prevents_reprocessing_for_pass_through_endpoints():
    ...
    logging_obj = LiteLLMLoggingObj(
        ...
        call_type="pass_through_endpoint",
        start_time=datetime.now(),
        litelm_call_id="test-call-id-reprocess",
...
async def test_async_success_handler_sets_standard_logging_object_for_streaming_pass_through():
    ...
    logging_obj = LiteLLMLoggingObj(
        ...
        call_type="pass_through_endpoint",
        start_time=datetime.now(),
        litelm_call_id="test-call-id-streaming",
...
def test_get_error_information_error_code_priority():
    ...
    litelm_exception = LiteLLMException(status_code=429, message="Rate limit exceeded")
    result = StandardLoggingPayloadSetup.get_error_information(litelm_exception)
...
async def test_async_success_handler_preserves_response_cost_for_pass_through_endpoints():
    ...
    from litelm.litelm_core_utils.litelm_logging import Logging as LiteLLMLoggingObj
    from litelm.types.utils import ModelResponse, Usage
...
def test_function_setup_litelm_metadata_populates_metadata():
    """
    Test that function_setup() properly handles litelm_metadata (used by /v1/messages,
    /batches, /responses, /files endpoints) and populates litelm_params["metadata"]
    ...
    # Simulate what happens for /v1/messages: metadata is in "litelm_metadata", not "metadata"
    kwargs = {
        "model": "claude-3-5-sonnet",
        "messages": [{"role": "user", "content": "hello"}],
        "litelm_call_id": "test-call-id-123",
        "litelm_metadata": {
    ...
    logging_obj, returned_kwargs = litelm.utils.function_setup(
        original_function="anthropic_messages",
        rules_obj=litelm.utils.Rules(),
        start_time=time.time(),
        **kwargs,
    )

    # litelm_params["metadata"] must contain the API key fields
    litelm_params = logging_obj.model_call_details.get("litelm_params", {})
    metadata = litelm_params.get("metadata")
    assert metadata is not None, "litelm_params['metadata'] should not be None"
    assert isinstance(metadata, dict), "litelm_params['metadata'] should be a dict"
    ...
    # litelm_metadata should also be preserved
    litelm_metadata = litelm_params.get("litelm_metadata")
    assert litelm_metadata is not None
    assert litelm_metadata.get("user_api_key_hash") == test_api_key_hash

    # metadata should be a COPY, not an alias — mutating one must not affect the other
    assert (
        metadata is not litelm_metadata
    ), "litelm_params['metadata'] should be a copy, not the same object"


def test_function_setup_metadata_takes_precedence_over_litelm_metadata():
    """
    Test that when BOTH metadata and litelm_metadata are present (e.g., user sets
    Anthropic API metadata AND proxy adds litelm_metadata), metadata is used as
    litelm_params["metadata"] and litelm_metadata is stored separately.
    """
    import litelm
    ...
    kwargs = {
        "model": "claude-3-5-sonnet",
        "messages": [{"role": "user", "content": "hello"}],
        "litelm_call_id": "test-call-id-456",
        "metadata": {
    ...
    logging_obj, _ = litelm.utils.function_setup(
        original_function="anthropic_messages",
        rules_obj=litelm.utils.Rules(),
        start_time=time.time(),
        **kwargs,
    ...
    # litelm_metadata should be preserved separately for merge_litelm_metadata()
    litelm_metadata = litelm_params.get("litelm_metadata")
    assert litelm_metadata is not None
    assert litelm_metadata.get("user_api_key_hash") == "sk-hashed-xyz"


def test_function_setup_empty_metadata_falls_back_to_litelm_metadata():
    """
    Test that when metadata is explicitly set to {} (empty dict), litelm_metadata
    is still used to populate litelm_params["metadata"] so API key fields are visible.
    """
    import litelm
    ...
    kwargs = {
        "model": "claude-3-5-sonnet",
        "messages": [{"role": "user", "content": "hello"}],
        "litelm_call_id": "test-call-id-789",
        "metadata": {},
        "litelm_metadata": {
