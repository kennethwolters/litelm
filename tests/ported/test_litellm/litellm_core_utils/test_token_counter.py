#### What this tests ####
#    This tests litelm.token_counter.token_counter() function
import os
import sys
import time
import traceback
...
import litelm
from litelm import create_pretrained_tokenizer, decode, encode, get_modified_max_tokens
from litelm import token_counter as token_counter_old
from litelm.litelm_core_utils.token_counter import token_counter as token_counter_new
from tests.large_text import text
from tests.test_litelm.litelm_core_utils.messages_with_counts import (
    MESSAGES_TEXT,
...
def test_get_modified_max_tokens(
    ...
    litelm.token_counter.side_effect = _mock_token_counter
...
def test_gpt_4o_token_counter():
    with patch.object(
        litelm.utils, "openai_token_counter", new=MagicMock()
    ) as mock_client:
        token_counter(
...
def test_img_url_token_counter(img_url):
    from litelm.litelm_core_utils.token_counter import get_image_dimensions
...
from litelm.utils import _select_tokenizer_helper, claude_json_str, encoding
...
class TestTokenizerSelection(unittest.TestCase):
    ...
    @patch("litelm.utils.Tokenizer.from_pretrained")
    ...
    def test_cohere_tokenizer_api_failure(self, mock_from_pretrained):
        ...
        litelm.cohere_models = ["command-r-v1"]
    ...
    @patch("litelm.utils.Tokenizer.from_str")
    def test_claude_tokenizer_api_failure(self, mock_from_str):
        ...
        litelm.anthropic_models = ["claude-2"]
    ...
    @patch("litelm.utils._return_huggingface_tokenizer")
    def test_disable_hf_tokenizer_download(self, mock_return_huggingface_tokenizer):
        monkeypatch = pytest.MonkeyPatch()
        monkeypatch.setattr(litelm, "disable_hf_tokenizer_download", True)
        try:
