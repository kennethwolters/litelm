...
from litelm.constants import LITTELM_INTERNAL_HEALTH_SERVICE_ACCOUNT_NAME
from litelm.litelm_core_utils.health_check_helpers import HealthCheckHelpers
from litelm.main import ahealth_check
...
def test_update_model_params_with_health_check_tracking_information():
    ...
    with patch(
        "litelm.proxy._types.UserAPIKeyAuth.get_litelm_internal_health_check_user_api_key_auth"
    ) as mock_get_auth:
        mock_auth = MagicMock()
        mock_get_auth.return_value = mock_auth
        
        with patch(
            "litelm.proxy.litelm_pre_call_utils.LiteLLMProxyRequestSetup.add_user_api_key_auth_to_request_metadata"
        ) as mock_add_auth:
            ...
            # Verify that litelm_metadata was added
            assert "litelm_metadata" in result
            assert result["litelm_metadata"]["tags"] == [LITTELM_INTERNAL_HEALTH_SERVICE_ACCOUNT_NAME]
            ...
            assert call_args[1]["_metadata_variable_name"] == "litelm_metadata"
...
def test_get_litelm_internal_health_check_user_api_key_auth():
    """Test get_litelm_internal_health_check_user_api_key_auth returns properly configured UserAPIKeyAuth object."""
    result = UserAPIKeyAuth.get_litelm_internal_health_check_user_api_key_auth()
