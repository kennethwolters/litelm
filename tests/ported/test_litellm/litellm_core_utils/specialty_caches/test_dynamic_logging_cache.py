...
import litelm
from litelm.litelm_core_utils.specialty_caches.dynamic_logging_cache import (
...
class TestLangfuseInMemoryCache:
    ...
    @patch("litelm.initialized_langfuse_clients", 5)
    def test_langfuse_client_count_decrements_on_eviction(self):
        ...
        with patch(
            "litelm.integrations.langfuse.langfuse.LangFuseLogger", MockLangFuseLogger
        ):
            ...
            initial_count = litelm.initialized_langfuse_clients

            # Trigger eviction
            self.cache.evict_cache()

            # Verify client count was decremented
            assert litelm.initialized_langfuse_clients == initial_count - 1

    @patch("litelm.initialized_langfuse_clients", 3)
