...
def test_gemini_context_window_error_mapping(error_message, should_raise_context_window):
    ...
    context window exceeded errors to litelm.ContextWindowExceededError.
    """
    ...
    if should_raise_context_window:
        with pytest.raises(litelm.ContextWindowExceededError) as excinfo:
            exception_type(
                model=model,
        ...
        assert isinstance(excinfo.value, litelm.ContextWindowExceededError)
    else:
        # For the negative case, we expect it to raise a generic APIConnectionError
        with pytest.raises(litelm.APIConnectionError):
            exception_type(
                model=model,
                original_exception=original_exception,
                custom_llm_provider=custom_llm_provider,
...
# As per https://github.com/BerriAI/litelm/issues/16189
...
def test_vertex_ai_rate_limit_error_mapping(error_message, should_raise_rate_limit):
    ...
    "Resource exhausted" error to a litelm.RateLimitError.
    """
    ...
    if should_raise_rate_limit:
        ...
        assert isinstance(excinfo.value, litelm.RateLimitError)
    else:
        # For the negative case, we expect it to raise a generic APIConnectionError
        with pytest.raises(litelm.APIConnectionError):
            exception_type(
...
class TestExtractAndRaiseLitellmException:
    """Tests for extract_and_raise_litelm_exception function"""
    ...
    def test_extract_and_raise_api_connection_error_without_response(self):
        ...
        This is a regression test for the bug where extract_and_raise_litelm_exception
        ...
        Relevant Issue: https://github.com/BerriAI/litelm/issues/XXXXX
        """
        error_str = "litelm.APIConnectionError: GeminiException - some error message"
        
        with pytest.raises(litelm.APIConnectionError) as excinfo:
            extract_and_raise_litelm_exception(
                response=None,
                error_str=error_str,
    ...
    def test_extract_and_raise_bad_request_error_with_response(self):
        ...
        """
        error_str = "litelm.BadRequestError: Invalid request format"
    ...
    def test_extract_and_raise_context_window_exceeded_error(self):
        ...
        """
        error_str = "litelm.ContextWindowExceededError: Token limit exceeded"
    ...
    def test_no_exception_raised_for_non_litelm_error(self):
        """
        Test that no exception is raised for non-litelm error strings.
        """
        error_str = "Some generic error that is not a litelm exception"
        
        # Should not raise any exception
        result = extract_and_raise_litelm_exception(
            response=None,
            error_str=error_str,
            model="gpt-4",
            custom_llm_provider="openai",
