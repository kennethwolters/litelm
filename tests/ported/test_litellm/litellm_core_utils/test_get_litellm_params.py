"""
Tests for get_litelm_params and related helpers.
...
from litelm.litelm_core_utils.get_litelm_params import (
    _OPTIONAL_KWARGS_KEYS,
    _get_base_model_from_litelm_call_metadata,
    get_litelm_params,
)
...
class TestGetBaseModelFromLitellmCallMetadata:
    def test_none_metadata_returns_none(self):
        assert _get_base_model_from_litelm_call_metadata(None) is None

    def test_empty_metadata_returns_none(self):
        assert _get_base_model_from_litelm_call_metadata({}) is None

    def test_missing_model_info_returns_none(self):
        assert _get_base_model_from_litelm_call_metadata({"foo": "bar"}) is None

    def test_model_info_none_returns_none(self):
        assert _get_base_model_from_litelm_call_metadata({"model_info": None}) is None

    def test_model_info_empty_dict_returns_none(self):
        assert _get_base_model_from_litelm_call_metadata({"model_info": {}}) is None

    def test_returns_base_model(self):
        result = _get_base_model_from_litelm_call_metadata(
...
class TestGetLitellmParamsKwargsExtraction:
    ...
    def test_all_optional_kwargs_extractable(self):
        ...
        result = get_litelm_params(**kwargs)
        for key in _OPTIONAL_KWARGS_KEYS:
...
class TestGetLitellmParamsExplicitFields:
    ...
    def test_no_log_from_kwargs(self):
        ...
        result = get_litelm_params(**{"no-log": True})
        assert result["no-log"] is True

    def test_no_log_from_explicit_param(self):
        result = get_litelm_params(no_log=True)
        assert result["no-log"] is True
