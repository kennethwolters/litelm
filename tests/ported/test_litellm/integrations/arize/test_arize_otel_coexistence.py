...
def _make_arize_logger(exporter: InMemorySpanExporter):
    ...
    """
    from litelm.integrations.arize.arize import ArizeLogger
...
class TestProxyLoggerNotOverwritten(unittest.TestCase):
    ...
    @patch("litelm.proxy.proxy_server.open_telemetry_logger", None)
...
class TestPhoenixAutoInitWithOtelOnly(unittest.TestCase):
    ...
    def test_auto_init_creates_phoenix_logger(self):
        from litelm.integrations.arize.arize_phoenix import ArizePhoenixLogger
        from litelm.litelm_core_utils.litelm_logging import _maybe_auto_initialize_arize_phoenix
