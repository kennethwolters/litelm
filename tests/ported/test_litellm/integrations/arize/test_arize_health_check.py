...
class TestArizeIntegrationWithProxy:
    ...
    async def test_arize_logging_with_completion(self):
        ...
        with patch.dict(os.environ, {
            ...
            original_callbacks = litelm.success_callback.copy() if litelm.success_callback else []
            
            try:
                # Add ArizeLogger to callbacks
                litelm.success_callback = [arize_logger]
                
                # Make completion request
                response = await litelm.acompletion(
                    model="openai/litelm-mock-response-model",
                ...
                litelm.success_callback = original_callbacks
