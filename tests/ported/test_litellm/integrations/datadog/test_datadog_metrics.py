...
from litelm.integrations.datadog.datadog_metrics import DatadogMetricsLogger
...
async def test_async_log_failure_event_extracts_status_code(clean_env):
    ...
    count_series = next(
        (s for s in logger.log_queue if s["metric"] == "litelm.llm_api.request_count"),
        None,
    )
...
async def test_async_send_batch(clean_env):
    ...
    logger.log_queue = [
        {
            "metric": "litelm.request.total_latency",
            "type": 3,
    ...
    assert payload["series"][0]["metric"] == "litelm.request.total_latency"
