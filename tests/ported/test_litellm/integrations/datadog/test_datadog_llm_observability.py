...
from litelm.types.integrations.datadog_llm_obs import (
...
class TestDataDogLLMObsLogger:
    ...
    def test_cost_and_trace_id_integration(self, mock_env_vars, mock_response_obj):
        ...
        with patch(
            "litelm.integrations.datadog.datadog_llm_obs.get_async_httpx_client"
...
async def test_dd_llms_obs_redaction(mock_env_vars):
    ...
    litelm.datadog_llm_observability_params = DatadogLLMObsInitParams(
        turn_off_message_logging=True
    )
    dd_llms_obs_logger = TestDataDogLLMObsLoggerForRedaction()
    test_s3_logger = TestS3Logger()
    litelm.callbacks = [dd_llms_obs_logger, test_s3_logger]
...
async def test_create_llm_obs_payload(mock_env_vars):
    ...
    assert payload["name"] == "litelm_llm_call"
...
def create_standard_logging_payload_with_latency_metrics() -> StandardLoggingPayload:
    ...
    hidden_params = StandardLoggingHiddenParams(
        ...
        api_base="https://api.openai.com",
        response_cost="0.05",
        litelm_overhead_time_ms=150.0,  # 150ms
        additional_headers=None,
    )
...
def test_latency_metrics_in_metadata(mock_env_vars):
    """Test that time to first token, litelm overhead, and guardrail overhead are included in metadata"""
    with patch(
        "litelm.integrations.datadog.datadog_llm_obs.get_async_httpx_client"
    ), patch("asyncio.create_task"):
        logger = DataDogLLMObsLogger()
        ...
        # Verify litelm overhead is included (150ms)
        assert "litelm_overhead_time_ms" in latency_metadata
        assert latency_metadata["litelm_overhead_time_ms"] == 150.0
        ...
        assert payload_metadata_latency["litelm_overhead_time_ms"] == 150.0
...
def test_latency_metrics_edge_cases(mock_env_vars):
    ...
    with patch(
        ...
        assert (
            "litelm_overhead_time_ms" not in metadata
...
class TestDataDogLLMObsLoggerToolCalls:
    ...
    def test_tool_call_payload_creation(self, mock_env_vars):
        ...
        with patch(
            ...
            assert payload.get("name") == "litelm_llm_call"
