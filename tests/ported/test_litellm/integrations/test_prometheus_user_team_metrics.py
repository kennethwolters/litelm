...
from litelm.integrations.prometheus import PrometheusLogger
...
class TestPrometheusUserTeamCountMetrics:
    ...
    def test_user_team_count_metrics_initialization(self, prometheus_logger):
        ...
        assert hasattr(prometheus_logger, "litelm_total_users_metric")
        assert hasattr(prometheus_logger, "litelm_teams_count_metric")

        # Verify the metrics are not None
        assert prometheus_logger.litelm_total_users_metric is not None
        assert prometheus_logger.litelm_teams_count_metric is not None

    def test_user_count_metric_has_no_labels(self, prometheus_logger):
        """Test that litelm_total_users metric has no labels (as specified)"""
        metric = prometheus_logger.litelm_total_users_metric
        ...
        try:
            ...
            pytest.fail(f"litelm_total_users_metric should not require labels: {e}")

    def test_teams_count_metric_has_no_labels(self, prometheus_logger):
        """Test that litelm_teams_count metric has no labels (as specified)"""
        metric = prometheus_logger.litelm_teams_count_metric
        ...
        try:
            ...
            pytest.fail(f"litelm_teams_count_metric should not require labels: {e}")
    ...
    def test_user_count_metric_accepts_various_values(self, prometheus_logger):
        ...
        for value in test_values:
            try:
                ...
                pytest.fail(
                    f"litelm_total_users_metric should accept value {value}: {e}"
                )
    ...
    def test_team_count_metric_accepts_various_values(self, prometheus_logger):
        ...
        for value in test_values:
            try:
                ...
                pytest.fail(
                    f"litelm_teams_count_metric should accept value {value}: {e}"
                )
    ...
    def test_user_count_metric_with_zero(self, prometheus_logger):
        ...
        try:
            ...
            pytest.fail(f"litelm_total_users_metric should handle zero: {e}")
    ...
    def test_team_count_metric_with_zero(self, prometheus_logger):
        ...
        try:
            ...
            pytest.fail(f"litelm_teams_count_metric should handle zero: {e}")
    ...
    def test_metrics_can_be_updated_multiple_times(self, prometheus_logger):
        ...
        user_metric = prometheus_logger.litelm_total_users_metric
        team_metric = prometheus_logger.litelm_teams_count_metric
    ...
    def test_metrics_can_be_collected_by_prometheus(self, prometheus_logger):
        ...
        # Set some values
        prometheus_logger.litelm_total_users_metric.set(100)
        prometheus_logger.litelm_teams_count_metric.set(20)
        ...
        assert "litelm_total_users" in metrics or "litelm_total_users_total" in metrics
        assert "litelm_teams_count" in metrics or "litelm_teams_count_total" in metrics
    ...
    def test_metrics_have_correct_type(self, prometheus_logger):
        ...
        assert hasattr(prometheus_logger.litelm_total_users_metric, "set")
        assert hasattr(prometheus_logger.litelm_teams_count_metric, "set")

        # Gauges have set() method, Counters only have inc()
        assert callable(prometheus_logger.litelm_total_users_metric.set)
        assert callable(prometheus_logger.litelm_teams_count_metric.set)
...
async def test_set_team_budget_metrics_after_api_request_no_inf_when_metadata_budget_none(
    ...
    set_call_args = (
        prometheus_logger.litelm_remaining_team_budget_metric.labels().set.call_args
    )
...
async def test_assemble_user_object_uses_db_max_budget_when_metadata_is_none(
    ...
    with patch("litelm.proxy.auth.auth_checks.get_user_object") as mock_get_user:
...
async def test_set_user_budget_metrics_after_api_request_no_inf_when_metadata_budget_none(
    ...
    prometheus_logger.litelm_user_max_budget_metric = MagicMock()
    prometheus_logger.litelm_user_budget_remaining_hours_metric = MagicMock()
    ...
    set_call_args = (
        prometheus_logger.litelm_remaining_user_budget_metric.labels().set.call_args
    )
