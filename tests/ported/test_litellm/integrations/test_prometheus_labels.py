...
def test_user_email_in_required_metrics():
    ...
    - litelm_proxy_total_requests_metric (already had it)
    - litelm_proxy_failed_requests_metric (added)
    - litelm_input_tokens_metric (added)
    - litelm_output_tokens_metric (added)
    - litelm_requests_metric (already had it)
    - litelm_spend_metric (added)
    """
...
def test_model_id_in_required_metrics():
    ...
    metrics_with_model_id = [
        ...
        "litelm_deployment_successful_fallbacks",
        "litelm_cache_hits_metric",
        "litelm_cache_misses_metric",
        "litelm_remaining_api_key_requests_for_model",
        "litelm_remaining_api_key_tokens_for_model",
        "litelm_llm_api_failed_requests_metric",
    ]
...
def test_model_id_in_required_metrics():
    ...
    - litelm_proxy_total_requests_metric
    - litelm_proxy_failed_requests_metric
    - litelm_request_total_latency_metric
    - litelm_llm_api_time_to_first_token_metric
    """
    model_id_label = UserAPIKeyLabelNames.MODEL_ID.value
...
def test_route_normalization_for_responses_api():
    ...
    Issue: https://github.com/BerriAI/litelm/issues/XXXX
    ...
    """
    from litelm.proxy.auth.auth_utils import normalize_request_route
    ...
    responses_routes = [
        ...
        ("/v1/responses/litelm_poll_xyz", "/v1/responses/{response_id}"),
    ]
...
def test_prometheus_metrics_use_normalized_routes():
    ...
    labels = prometheus_label_factory(
        supported_enum_labels=prometheus_logger.get_labels_for_metric(
            metric_name="litelm_proxy_total_requests_metric"
        ),
...
def test_prometheus_label_value_sanitization_unicode_paragraph_separator():
    ...
    from litelm.types.integrations.prometheus import _sanitize_prometheus_label_value
