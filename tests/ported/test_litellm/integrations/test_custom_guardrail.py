...
from litelm.proxy._types import CallTypes, UserAPIKeyAuth
from litelm.types.utils import GuardrailTracingDetail
...
class TestCustomGuardrailShouldRunGuardrail:
    ...
    def test_should_run_guardrail_with_litelm_metadata(self):
        """Test that should_run_guardrail works with litelm_metadata pattern"""
        ...
        # Test with guardrails in litelm_metadata
        data = {
            "model": "gpt-3.5-turbo",
            "litelm_metadata": {"guardrails": ["test_guardrail"]},
        }
    ...
    def test_should_run_guardrail_no_matching_guardrail(self):
        ...
        data = {
            "model": "gpt-3.5-turbo",
            "litelm_metadata": {"guardrails": ["different_guardrail"]},
        }
    ...
    def test_should_run_guardrail_with_disable_global_guardrail(self):
        ...
        # Test 3: Global guardrail is disabled when disable_global_guardrail=True in litelm_metadata
        data_with_disable_litelm = {
            "model": "gpt-3.5-turbo",
            "messages": [{"role": "user", "content": "test"}],
            "litelm_metadata": {"disable_global_guardrail": True},
        }
        result = custom_guardrail.should_run_guardrail(
            data=data_with_disable_litelm, event_type=GuardrailEventHooks.pre_call
        )
        assert (
            result is False
        ), "Global guardrail should be disabled when disable_global_guardrail=True in litelm_metadata"
...
class TestGuardrailLoggingAggregation:
    ...
    def test_appends_to_litelm_metadata(self):
        request_data = {
            "litelm_metadata": {
                "standard_logging_guardrail_information": [
                    {"guardrail_name": "litelm_existing"}
                ]
            }
        }
        ...
        info = request_data["litelm_metadata"][
            "standard_logging_guardrail_information"
        ]
        assert isinstance(info, list)
        assert len(info) == 2
...
class TestEventTypeLogging:
    ...
    async def test_log_guardrail_information_infers_event_type_from_async_pre_call_hook(
        ...
        """
        from litelm.integrations.custom_guardrail import log_guardrail_information
        from litelm.types.guardrails import GuardrailEventHooks
