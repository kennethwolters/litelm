...
async def test_construct_request_headers_project_id_from_env(monkeypatch):
    ...
    from litelm.integrations.gcs_pubsub.pub_sub import GcsPubSubLogger
    ...
    try:
        ...
        with patch(
            "litelm.vertex_chat_completion._ensure_access_token_async"
        ) as mock_ensure_token:
            mock_ensure_token.return_value = (mock_auth_header, test_project_id)

            with patch(
                "litelm.vertex_chat_completion._get_token_and_url"
            ) as mock_get_token:
