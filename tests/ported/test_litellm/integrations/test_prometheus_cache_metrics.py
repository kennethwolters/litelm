...
Run with: poetry run pytest tests/test_litelm/integrations/test_prometheus_cache_metrics.py -v
"""
import pytest
...
class TestPrometheusCacheMetrics:
    ...
    def test_cache_metrics_defined_in_types(self):
        ...
        from litelm.types.integrations.prometheus import DEFINED_PROMETHEUS_METRICS
        from typing import get_args

        defined_metrics = get_args(DEFINED_PROMETHEUS_METRICS)

        assert "litelm_cache_hits_metric" in defined_metrics
        assert "litelm_cache_misses_metric" in defined_metrics
        assert "litelm_cached_tokens_metric" in defined_metrics

    def test_cache_metric_labels_defined(self):
        """Test that cache metric labels are properly defined"""
        from litelm.types.integrations.prometheus import PrometheusMetricLabels

        # Verify labels are defined for each cache metric
        assert hasattr(PrometheusMetricLabels, "litelm_cache_hits_metric")
        assert hasattr(PrometheusMetricLabels, "litelm_cache_misses_metric")
        assert hasattr(PrometheusMetricLabels, "litelm_cached_tokens_metric")
        ...
        for label in expected_labels:
            assert label in PrometheusMetricLabels.litelm_cache_hits_metric
            assert label in PrometheusMetricLabels.litelm_cache_misses_metric
            assert label in PrometheusMetricLabels.litelm_cached_tokens_metric
    ...
    def test_increment_cache_metrics_on_cache_hit(self, sample_enum_values):
        ...
        # Create mock metrics
        mock_logger.litelm_cache_hits_metric = MagicMock()
        mock_logger.litelm_cache_misses_metric = MagicMock()
        mock_logger.litelm_cached_tokens_metric = MagicMock()
        ...
        mock_logger.litelm_cache_hits_metric.labels.assert_called()
        mock_logger.litelm_cache_hits_metric.labels().inc.assert_called_once()

        # Verify cached tokens metric was incremented with total_tokens
        mock_logger.litelm_cached_tokens_metric.labels.assert_called()
        mock_logger.litelm_cached_tokens_metric.labels().inc.assert_called_once_with(
            100
        )

        # Verify cache misses metric was NOT called
        mock_logger.litelm_cache_misses_metric.labels.assert_not_called()
    ...
    def test_increment_cache_metrics_on_cache_miss(self, sample_enum_values):
        ...
        mock_logger.litelm_cache_misses_metric.labels.assert_called()
        mock_logger.litelm_cache_misses_metric.labels().inc.assert_called_once()

        # Verify cache hits and cached tokens metrics were NOT called
        mock_logger.litelm_cache_hits_metric.labels.assert_not_called()
        mock_logger.litelm_cached_tokens_metric.labels.assert_not_called()
