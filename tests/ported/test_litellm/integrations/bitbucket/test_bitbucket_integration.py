...
import litelm
from litelm.integrations.bitbucket import BitBucketPromptManager


@patch("litelm.integrations.bitbucket.bitbucket_prompt_manager.BitBucketClient")
def test_bitbucket_prompt_integration_with_litelm(mock_client_class):
...
User: {{user_message}}"""
    ...
    litelm.set_global_bitbucket_config(bitbucket_config)

    # Test that the configuration was set
    assert litelm.global_bitbucket_config == bitbucket_config
...
User: {{user_message}}"""
    ...
    # Should have updated litelm_params
