"""Unit tests for LiteLLMAgentModelResolver - litelm_agent/ prefix model resolution."""

from unittest.mock import MagicMock

import pytest

from litelm.integrations.litelm_agent import LiteLLMAgentModelResolver


class TestLiteLLMAgentModelResolver:
    def test_get_chat_completion_prompt_strips_prefix(self):
        """Verify get_chat_completion_prompt strips litelm_agent/ prefix from model."""
        resolver = LiteLLMAgentModelResolver()
        messages = [{"role": "user", "content": "Hello"}]

        resolved_model, out_messages, out_params = resolver.get_chat_completion_prompt(
            model="litelm_agent/gpt-3.5-turbo",
            messages=messages,
            non_default_params={},
            prompt_id=None,
    ...
    def test_get_chat_completion_prompt_preserves_rest_of_model(self):
        ...
        resolved_model, _, _ = resolver.get_chat_completion_prompt(
            model="litelm_agent/openai/gpt-3.5-turbo",
            messages=messages,
            non_default_params={},
            prompt_id=None,
            prompt_variables=None,
    ...
    def test_get_chat_completion_prompt_respects_ignore_prompt_manager_model(self):
        ...
        assert resolved_model == "litelm_agent/gpt-3.5-turbo"
