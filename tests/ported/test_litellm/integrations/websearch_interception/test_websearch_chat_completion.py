...
litelm.acompletion() for transparent server-side web search execution.
"""
import os
...
from litelm.types.utils import LlmProviders, ModelResponse


@pytest.fixture
def mock_search_response():
    """Mock search response from litelm.asearch()"""
...
async def test_websearch_chat_completion_with_openai():
    ...
    1. Model calls litelm_web_search tool
...
async def test_websearch_chat_completion_hook_detection():
    ...
    # Mock response with litelm_web_search tool call
    ...
    should_run, tools_dict = (
        await websearch_logger.async_should_run_chat_completion_agentic_loop(
            ...
            tools=[
                {
                    "type": "function",
                    "function": {"name": "litelm_web_search"},
                }
            ],
            stream=False,
    ...
    assert tools_dict["tool_calls"][0]["name"] == "litelm_web_search"
...
async def test_websearch_json_serialization_fix():
    ...
    """
    from litelm.integrations.websearch_interception.transformation import (
