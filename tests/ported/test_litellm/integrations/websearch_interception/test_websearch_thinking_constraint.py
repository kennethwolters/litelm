...
Covers:
  ...
  - M2-I5/I8: litelm_logging_obj excluded from follow-up kwargs to prevent SpendLog dedup
...
from litelm.integrations.websearch_interception.handler import (
    ...
    return [
        {
            ...
            "type": "tool_use",
            "name": "web_search",
            "input": {"query": "litelm spend tracking"},
        }
    ]
...
class TestThinkingBudgetTokensConstraint:
    ...
    async def test_max_tokens_adjusted_when_less_than_budget(self):
        ...
        with patch(
            "litelm.integrations.websearch_interception.handler.anthropic_messages.acreate",
...
# ---------------------------------------------------------------------------
# M2-I5 / M2-I8: litelm_logging_obj excluded from follow-up kwargs
# ---------------------------------------------------------------------------

class TestLoggingObjExcludedFromFollowUp:
    """Verify litelm_logging_obj is NOT forwarded to the follow-up acreate() call.
    ...
    @pytest.mark.asyncio
    async def test_litelm_logging_obj_excluded_from_anthropic_followup(self):
        """The Anthropic messages follow-up must NOT receive litelm_logging_obj."""
        logger = WebSearchInterceptionLogger(enabled_providers=["bedrock"])
        captured_kwargs: Dict[str, Any] = {}
        ...
        with patch(
            ...
            await logger._execute_agentic_loop(
                ...
                stream=False,
                kwargs={
                    "litelm_logging_obj": fake_logging_obj,
                    "metadata": {"user_api_key": "test-key-hash"},
                    "temperature": 0.5,
                },
            )

        # litelm_logging_obj must be absent from the follow-up call
        assert "litelm_logging_obj" not in captured_kwargs
    ...
    async def test_websearch_flags_also_excluded(self):
        """Both _websearch_interception flags and litelm_logging_obj must be excluded."""
        logger = WebSearchInterceptionLogger(enabled_providers=["bedrock"])
        captured_kwargs: Dict[str, Any] = {}
...
class TestFollowUpErrorScenarios:
    ...
    async def test_metadata_preserved_after_logging_obj_exclusion(self):
        ...
        even after litelm_logging_obj is excluded — so the new logging_obj from
        ...
        with patch(
            ...
            await logger._execute_agentic_loop(
                ...
                kwargs={
                    ...
                    "litelm_call_id": "call-abc-123",
                },
            )

        # litelm_logging_obj excluded
        assert "litelm_logging_obj" not in captured_kwargs
        # But ALL proxy metadata must be preserved
        assert captured_kwargs.get("metadata") == proxy_metadata
        assert captured_kwargs.get("litelm_call_id") == "call-abc-123"
