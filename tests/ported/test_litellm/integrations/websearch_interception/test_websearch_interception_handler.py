...
def test_initialize_from_proxy_config():
    """Test initialization from proxy config with litelm_settings"""
    litelm_settings = {
...
async def test_async_pre_call_deployment_hook_provider_from_top_level_kwargs():
    ...
    Regression test for bug where the hook only checked kwargs["litelm_params"]["custom_llm_provider"]
    but the router places custom_llm_provider at the top level of kwargs.
    """
    logger = WebSearchInterceptionLogger(enabled_providers=["bedrock"])

    # Simulate kwargs as they arrive from the router path:
    # custom_llm_provider is at the TOP LEVEL (not nested under litelm_params)
    kwargs = {
    ...
    # The web_search tool should be converted to litelm_web_search (OpenAI format)
    assert any(
        t.get("type") == "function" and t.get("function", {}).get("name") == "litelm_web_search"
...
@pytest.mark.asyncio
async def test_async_pre_call_deployment_hook_nested_litelm_params_fallback():
    """Test that the hook still works when custom_llm_provider is in nested litelm_params.

    This is the Anthropic experimental pass-through path where litelm_params is
...
async def test_async_pre_call_deployment_hook_provider_derived_from_model_name():
    ...
    in kwargs at all — neither at top-level nor in litelm_params. The hook must derive
