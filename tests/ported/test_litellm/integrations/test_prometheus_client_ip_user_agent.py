...
async def test_async_post_call_failure_hook_includes_client_ip_user_agent():
    ...
    # Mocking
    with patch(
        "litelm.integrations.prometheus.PrometheusLogger.__init__", return_value=None
    ):
        logger = PrometheusLogger()
        # Initialize attributes manually as __init__ is mocked
        logger.litelm_proxy_failed_requests_metric = MagicMock()
        logger.litelm_proxy_total_requests_metric = MagicMock()
    ...
    with patch(
        "litelm.integrations.prometheus.prometheus_label_factory"
...
def test_set_llm_deployment_failure_metrics_includes_client_ip_user_agent():
    ...
    with patch(
        ...
        logger = PrometheusLogger()
        logger.litelm_deployment_failure_responses = MagicMock()
        logger.litelm_deployment_total_requests = MagicMock()
        logger.get_labels_for_metric = MagicMock(
            return_value=["client_ip", "user_agent"]
        )
