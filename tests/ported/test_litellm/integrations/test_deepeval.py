...
from litelm.integrations.deepeval.api import Endpoints, HttpMethods
from litelm.integrations.deepeval.deepeval import DeepEvalLogger
from litelm.integrations.deepeval.types import SpanApiType, TraceSpanApiStatus
...
class TestDeepEvalLogger(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.api_patcher = patch("litelm.integrations.deepeval.deepeval.Api")
    ...
    def _common_assertions(
        ...
        expected_name = (
            "litelm_success_callback"
            if expected_status == TraceSpanApiStatus.SUCCESS
            else "litelm_failure_callback"
        )
