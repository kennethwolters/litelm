...
from litelm.integrations.opentelemetry import OpenTelemetry, OpenTelemetryConfig
from litelm.litelm_core_utils.safe_json_dumps import safe_dumps


class TestOpenTelemetryGuardrails(unittest.TestCase):
    @patch("litelm.integrations.opentelemetry.datetime")
...
class TestOpenTelemetryCostBreakdown(unittest.TestCase):
    def test_cost_breakdown_emitted_to_otel_span(self):
        ...
        kwargs = {
            ...
            "optional_params": {},
            "litelm_params": {"custom_llm_provider": "openai"},
...
class TestOpenTelemetry(unittest.TestCase):
    ...
    def test_open_telemetry_config_manual_defaults(self):
        ...
        self.assertEqual(config.service_name, "litelm")
        self.assertEqual(config.deployment_environment, "production")
        self.assertEqual(config.model_id, "litelm")
    ...
    def test_get_tracer_with_dynamic_headers(self, mock_resource, mock_tracer_provider):
        ...
        with patch.object(otel, "_get_span_processor") as mock_get_span_processor:
            ...
            mock_provider_instance.get_tracer.assert_called_once_with("litelm")
    ...
    def test_get_litelm_resource_with_defaults(
        self, mock_detector_cls, mock_resource_create
    ):
        """Test _get_litelm_resource with default values when no environment variables are set."""
        ...
        result = OpenTelemetry._get_litelm_resource(config)
    ...
    @patch("opentelemetry.sdk.resources.Resource.create")
    @patch("opentelemetry.sdk.resources.OTELResourceDetector")
    def test_get_litelm_resource_with_litelm_env_vars(
        self, mock_detector_cls, mock_resource_create
    ):
        """Test _get_litelm_resource with LiteLLM-specific environment variables."""
        # Mock the Resource.create method
        mock_base_resource = MagicMock()
        mock_resource_create.return_value = mock_base_resource
    ...
    @patch("opentelemetry.sdk.resources.Resource.create")
    @patch("opentelemetry.sdk.resources.OTELResourceDetector")
    def test_get_litelm_resource_with_otel_resource_attributes(
        self, mock_detector_cls, mock_resource_create
    ):
        """Test _get_litelm_resource with OTEL_RESOURCE_ATTRIBUTES environment variable."""
    ...
    @patch.dict(os.environ, {}, clear=True)
    def test_get_litelm_resource_integration_with_real_resource(self):
        """Integration test to verify _get_litelm_resource works with actual OpenTelemetry Resource."""
        config = OpenTelemetryConfig()
        result = OpenTelemetry._get_litelm_resource(config)
        ...
        self.assertEqual(attributes.get("service.name"), "litelm")
        self.assertEqual(attributes.get("deployment.environment"), "production")
        self.assertEqual(attributes.get("model_id"), "litelm")
    ...
    def test_get_litelm_resource_real_otel_resource_attributes(self):
    ...
    @patch.dict(
        os.environ,
        {
            "OTEL_SERVICE_NAME": "litelm-service",
            "OTEL_RESOURCE_ATTRIBUTES": "service.name=otel-override,extra.attr=extra-value",
        },
        clear=True,
    )
    def test_get_litelm_resource_precedence(self):
        ...
        attributes = result.attributes
        self.assertEqual(attributes.get("service.name"), "litelm-service")
    ...
    def test_handle_success_spans_only(self):
        ...
        # must have the top‐level litelm_request span
        # self.assertIn(
        #     LITELLM_REQUEST_SPAN_NAME,
        #     [s.name for s in spans],
        #     "litelm_request span missing",
        # )
    ...
    def test_get_span_name_with_generation_name(self):
        ...
        otel = OpenTelemetry()
        kwargs = {"litelm_params": {"metadata": {"generation_name": "custom_span"}}}
    ...
    def test_get_span_name_without_generation_name(self):
        ...
        from litelm.integrations.opentelemetry import LITELLM_REQUEST_SPAN_NAME

        otel = OpenTelemetry()
        kwargs = {"litelm_params": {"metadata": {}}}
        result = otel._get_span_name(kwargs)
        self.assertEqual(result, LITELLM_REQUEST_SPAN_NAME)

    @patch("litelm.turn_off_message_logging", False)
    def test_maybe_log_raw_request_creates_span(self):
        """Test _maybe_log_raw_request creates span when logging enabled"""
        from litelm.integrations.opentelemetry import RAW_REQUEST_SPAN_NAME
    ...
    @patch("litelm.turn_off_message_logging", True)
...
class TestOpenTelemetryEndpointNormalization(unittest.TestCase):
    ...
    def test_normalize_endpoint_invalid_signal_type(self):
        ...
        with patch("litelm._logging.verbose_logger.warning") as mock_warning:
...
class TestOpenTelemetryExternalSpan(unittest.TestCase):
    ...
    def test_external_span_not_closed_with_use_otel_litelm_request_span_false(self):
        """
        ...
        - No litelm_request span is created
        ...
        # Should NOT have litelm_request spans (USE_OTEL_LITELLM_REQUEST_SPAN=false)
        litelm_spans = self._get_spans_by_name("litelm_request")
        self.assertEqual(
            len(litelm_spans),
            0,
            "Should NOT have litelm_request spans when USE_OTEL_LITELLM_REQUEST_SPAN=false",
        )
    ...
    def test_external_span_not_closed_with_use_otel_litelm_request_span_true(self):
        """
        Test that external spans are not closed when USE_OTEL_LITELLM_REQUEST_SPAN=true.

        Expected behavior:
        - External span remains open (is_recording = True)
        - litelm_request spans are created as children of external span
        - raw_gen_ai_request spans are children of litelm_request spans
        - Correct hierarchy: external_parent → litelm_request → raw_gen_ai_request
        """
        # Initialize OpenTelemetry
        otel = OpenTelemetry(tracer_provider=self.tracer_provider)
        ...
        # Should have litelm_request spans (USE_OTEL_LITELLM_REQUEST_SPAN=true)
        litelm_spans = self._get_spans_by_name("litelm_request")
        self.assertEqual(
            len(litelm_spans),
            2,
            "Should have 2 litelm_request spans when USE_OTEL_LITELLM_REQUEST_SPAN=true",
        )

        # Verify litelm_request spans are children of external span
        for litelm_span in litelm_spans:
            self.assertEqual(
                litelm_span.parent.span_id if litelm_span.parent else None,
                parent_span_id,
                "litelm_request should be child of external_parent_span",
            )

        # Verify raw_gen_ai_request spans (if present) are children of litelm_request
        raw_spans = self._get_spans_by_name("raw_gen_ai_request")
        if raw_spans:
            litelm_span_ids = {s.context.span_id for s in litelm_spans}
            for raw_span in raw_spans:
                self.assertIn(
                    raw_span.parent.span_id if raw_span.parent else None,
                    litelm_span_ids,
                    "raw_gen_ai_request should be child of litelm_request",
                )
...
class TestOpenTelemetrySemanticConventions138(unittest.TestCase):
    ...
    See: https://github.com/BerriAI/litelm/issues/17794
    """
    ...
    def test_handle_failure_langfuse_otel_nulls_parent_span(self):
        ...
        otel.tracer = tracer_provider.get_tracer("litelm")
        ...
        kwargs = {
            ...
            "litelm_params": {
                ...
                "metadata": {"litelm_parent_otel_span": other_span},
            },
            "standard_logging_object": {
                "id": "test-id",
                "call_type": "completion",
    ...
    def test_handle_failure_non_langfuse_preserves_parent_span(self):
        ...
        kwargs = {
            ...
            "litelm_params": {
                "custom_llm_provider": "openai",
                "metadata": {"litelm_parent_otel_span": parent_span},
            },
            "standard_logging_object": {
                "id": "test-id",
                "call_type": "completion",
    ...
    def test_handle_failure_hasattr_guard_on_parent_name(self):
        ...
        kwargs = {
            ...
            "litelm_params": {
                "custom_llm_provider": "openai",
                "metadata": {"litelm_parent_otel_span": parent_without_name},
            },
            "standard_logging_object": {
                "id": "test-id",
                "call_type": "completion",
...
class TestRawSpanAttributeIsolation(unittest.TestCase):
    ...
    def test_raw_span_does_not_duplicate_parent_attributes(self):
        ...
        kwargs = {
            "litelm_params": {"custom_llm_provider": "vertex_ai"},
...
class TestNoParentSpanDuplication(unittest.TestCase):
    """Issue #4: When litelm_request child span exists, the parent
    litelm_proxy_request span should NOT get set_attributes() called."""
    ...
    def test_parent_proxy_span_not_duplicated(self):
        ...
        from litelm.integrations.opentelemetry import LITELLM_PROXY_REQUEST_SPAN_NAME
        ...
        kwargs["litelm_params"]["metadata"]["litelm_parent_otel_span"] = parent_span
...
class TestGuardrailSpanParenting(unittest.TestCase):
    ...
    be children of the litelm_request span (or parent span)."""

    def test_guardrail_span_is_child_of_litelm_request(self):
        """When no parent proxy span exists, guardrail spans should be
        children of the litelm_request span, not orphaned root spans."""
        span_exporter = InMemorySpanExporter()
        tracer_provider = TracerProvider()
        tracer_provider.add_span_processor(SimpleSpanProcessor(span_exporter))
        ...
        kwargs = {
            ...
            "messages": [{"role": "user", "content": "Hello"}],
            "optional_params": {},
            "litelm_params": {"custom_llm_provider": "openai", "metadata": {}},
            "standard_logging_object": {
        ...
        litelm_spans = [s for s in spans if s.name == "litelm_request"]

        self.assertTrue(guardrail_spans, "Expected at least one guardrail span")
        self.assertTrue(litelm_spans, "Expected a litelm_request span")

        litelm_span = litelm_spans[0]
        for gs in guardrail_spans:
            # All spans should share the same trace_id (not orphaned)
            self.assertEqual(
                gs.context.trace_id,
                litelm_span.context.trace_id,
                "Guardrail span should share trace_id with litelm_request (not orphaned)",
            )
            # Guardrail should be a child of the litelm_request span
            self.assertIsNotNone(
                gs.parent,
            ...
            self.assertEqual(
                gs.parent.span_id,
                litelm_span.context.span_id,
                "Guardrail span should be a child of litelm_request",
            )
...
class TestResponseIdFallback(unittest.TestCase):
    ...
    def test_response_id_from_response_obj(self):
        ...
        # Should use provider response ID, not litelm call ID
        mock_span.set_attribute.assert_any_call(
    ...
    def test_response_id_fallback_for_embeddings(self):
        ...
        # Should fallback to litelm call ID
        mock_span.set_attribute.assert_any_call(
            "gen_ai.response.id", "litelm-embed-call-789"
        )
    ...
    def test_response_id_fallback_for_image_gen(self):
        ...
        # Should fallback to litelm call ID
        mock_span.set_attribute.assert_any_call(
            "gen_ai.response.id", "litelm-img-call-101"
        )
