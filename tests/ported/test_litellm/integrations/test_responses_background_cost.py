...
from litelm.types.llms.openai import ResponseAPIUsage, ResponsesAPIResponse
...
def _check_responses_cost_module_available():
    """Check if litelm_enterprise.proxy.common_utils.check_responses_cost module is available"""
    try:
        from litelm_enterprise.proxy.common_utils.check_responses_cost import (  # noqa: F401
            CheckResponsesCost,
        )
        return True
    except ImportError:
...
@pytest.mark.skipif(
    ...
    reason="litelm_enterprise.proxy.common_utils.check_responses_cost module not available (enterprise-only feature)"
)
class TestCheckResponsesCost:
    ...
    def mock_prisma_client(self):
        ...
        client = MagicMock()
        client.db = MagicMock()
        client.db.litelm_managedobjecttable = MagicMock()
        return client
    ...
    async def test_check_responses_cost_initialization(
        ...
        from litelm_enterprise.proxy.common_utils.check_responses_cost import (
            CheckResponsesCost,
        )
    ...
    async def test_check_responses_cost_no_jobs(
        ...
        mock_prisma_client.db.litelm_managedobjecttable.find_many = AsyncMock(
            return_value=[]
        )
        ...
        mock_prisma_client.db.litelm_managedobjecttable.find_many.assert_called_once_with(
            where={
    ...
    async def test_check_responses_cost_with_completed_job(
        ...
        # Mock update_many
        mock_prisma_client.db.litelm_managedobjecttable.update_many = AsyncMock()
        ...
        # Mock litelm.aget_responses to return completed response
        with patch("litelm.aget_responses", new_callable=AsyncMock) as mock_aget:
            mock_aget.return_value = completed_response

            await checker.check_responses_cost()

            # Verify update_many was called to mark job as completed
            mock_prisma_client.db.litelm_managedobjecttable.update_many.assert_called_once()
            call_args = (
                mock_prisma_client.db.litelm_managedobjecttable.update_many.call_args
            )
    ...
    async def test_check_responses_cost_with_in_progress_job(
        ...
        with patch("litelm.aget_responses", new_callable=AsyncMock) as mock_aget:
            ...
            mock_prisma_client.db.litelm_managedobjecttable.update_many.assert_not_called()
    ...
    async def test_check_responses_cost_error_handling(
        ...
        # Mock litelm.aget_responses to raise an exception
        with patch(
            "litelm.aget_responses",
