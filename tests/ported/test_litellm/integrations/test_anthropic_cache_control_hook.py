...
import litelm
from litelm.integrations.anthropic_cache_control_hook import AnthropicCacheControlHook
...
async def test_anthropic_cache_control_hook_system_message():
    ...
    with patch.dict(
        ...
        litelm.callbacks = [anthropic_cache_control_hook]
...
async def test_anthropic_cache_control_hook_out_of_bounds_logging():
    ...
    with patch.dict(
        ...
        with patch(
            "litelm.integrations.anthropic_cache_control_hook.verbose_logger"
        ) as mock_logger:
            with patch.object(client, "post", return_value=mock_response) as mock_post:
                messages = [
...
async def test_anthropic_cache_control_hook_multiple_content_items_last_only():
    ...
    This verifies the fix for https://github.com/BerriAI/litelm/issues/15696
    """
    with patch.dict(
        os.environ,
        {
...
def test_gemini_cache_control_injection_points_detected():
    ...
    from litelm.utils import is_cached_message
