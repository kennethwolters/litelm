...
async def test_asend_message_uses_cost_per_query():
    ...
    """
    from litelm.a2a_protocol import asend_message

    # Setup logger
    litelm.logging_callback_manager._reset_all_callbacks()
    cost_logger = CostLogger()
    litelm.callbacks = [cost_logger]

    # Mock A2A client
    mock_client = MagicMock()
    mock_client._litelm_agent_card = MagicMock()
    mock_client._litelm_agent_card.name = "test-agent"
...
async def test_asend_message_uses_input_output_cost_per_token():
    ...
    litelm.callbacks = [token_cost_logger]
...
async def test_asend_message_passes_agent_id_to_callback():
    ...
    litelm.callbacks = [agent_id_logger]
...
class MetadataLogger(CustomLogger):
    ...
    def __init__(self):
        ...
        self.litelm_params: Optional[dict] = None
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        self.litelm_params = kwargs.get("litelm_params", {})
        self.metadata = self.litelm_params.get("metadata", {})
...
async def test_asend_message_streaming_propagates_metadata():
    ...
    """
    from litelm.a2a_protocol import asend_message_streaming

    # Setup logger
    litelm.logging_callback_manager._reset_all_callbacks()
    metadata_logger = MetadataLogger()
    litelm.logging_callback_manager.add_litelm_async_success_callback(metadata_logger)
...
async def test_asend_message_streaming_triggers_callbacks():
    ...
    litelm.logging_callback_manager.add_litelm_async_success_callback(callback_logger)
    litelm.logging_callback_manager.add_litelm_success_callback(callback_logger)
