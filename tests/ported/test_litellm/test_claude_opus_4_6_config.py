...
def test_claude_4_6_australia_region_uses_au_prefix_not_apac():
    ...
    assert "au.anthropic.claude-opus-4-6-v1" in litelm.bedrock_converse_models, \
        "au.anthropic.claude-opus-4-6-v1 not registered in bedrock_converse_models"

    # Verify apac. is NOT registered for this model
    assert "apac.anthropic.claude-opus-4-6-v1" not in litelm.bedrock_converse_models, \
        "apac.anthropic.claude-opus-4-6-v1 should not be in bedrock_converse_models"

    # Verify the au. model is registered in bedrock_converse_models
    assert "au.anthropic.claude-sonnet-4-6" in litelm.bedrock_converse_models, \
        "au.anthropic.claude-sonnet-4-6 not registered in bedrock_converse_models"

    # Verify apac. is NOT registered for this model
    assert "apac.anthropic.claude-sonnet-4-6" not in litelm.bedrock_converse_models, \
...
def test_opus_4_6_model_pricing_and_capabilities():
    ...
    for model_name, config in expected_models.items():
        ...
        assert info["litelm_provider"] == config["provider"]
...
def test_opus_4_6_bedrock_regional_model_pricing():
    ...
    for model_name, expected in expected_models.items():
        assert model_name in model_data, f"Missing model entry: {model_name}"
        info = model_data[model_name]
        assert info["litelm_provider"] == "bedrock_converse"
...
def test_opus_4_6_bedrock_converse_registration():
    assert "anthropic.claude-opus-4-6-v1" in litelm.BEDROCK_CONVERSE_MODELS
    assert "global.anthropic.claude-opus-4-6-v1" in litelm.bedrock_converse_models
    assert "us.anthropic.claude-opus-4-6-v1" in litelm.bedrock_converse_models
    assert "eu.anthropic.claude-opus-4-6-v1" in litelm.bedrock_converse_models
    assert "au.anthropic.claude-opus-4-6-v1" in litelm.bedrock_converse_models
