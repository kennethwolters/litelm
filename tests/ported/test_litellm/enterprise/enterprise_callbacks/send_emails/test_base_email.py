...
from enterprise.litelm_enterprise.enterprise_callbacks.send_emails.base_email import (
    BaseEmailLogger,
)

sys.path.insert(0, os.path.abspath("../../.."))
from litelm_enterprise.types.enterprise_callbacks.send_emails import (
    EmailEvent,
    SendKeyCreatedEmailEvent,
    SendKeyRotatedEmailEvent,
)

from litelm.integrations.email_templates.email_footer import EMAIL_FOOTER
from litelm.proxy._types import CallInfo, Litellm_EntityType, WebhookEvent
from litelm.constants import EMAIL_BUDGET_ALERT_TTL
...
async def test_send_key_created_email(
    ...
    with mock.patch.dict(
        os.environ,
        {
            "EMAIL_LOGO_URL": "https://litelm-listing.s3.amazonaws.com/litelm_logo.png",
...
async def test_get_invitation_link(base_email_logger):
    ...
    mock_prisma.db.litelm_invitationlink.find_many = mock_find_many

    with mock.patch("litelm.proxy.proxy_server.prisma_client", mock_prisma):
        ...
        mock_prisma.db.litelm_invitationlink.find_many = mock_find_many_empty
        result = await base_email_logger._get_invitation_link(
            user_id="test-user", base_url="http://test.com"
        )
        assert result == "http://test.com"
...
async def test_get_invitation_link_uses_existing_when_available(base_email_logger):
    ...
    mock_prisma.db.litelm_invitationlink.find_many = mock_find_many_existing
...
async def test_get_invitation_link_creates_new_when_list_is_none(base_email_logger):
    ...
    mock_prisma.db.litelm_invitationlink.find_many = mock_find_many_none
...
async def test_get_email_params_user_invitation(
    ...
    with mock.patch.dict(
        ...
        with mock.patch.object(
            ...
            assert result.logo_url == "https://litelm-listing.s3.amazonaws.com/litelm_logo.png"
...
async def test_get_email_params_non_premium_user(mock_env_vars):
    ...
    with patch("litelm.proxy.proxy_server.premium_user", False):
        email_logger = BaseEmailLogger()
        ...
        assert email_params.logo_url == "https://litelm-listing.s3.amazonaws.com/litelm_logo.png"
