...
from enterprise.litelm_enterprise.enterprise_callbacks.callback_controls import (
    EnterpriseCallbackControls,
)
from litelm.constants import X_LITELLM_DISABLE_CALLBACKS
...
from litelm.integrations.s3_v2 import S3Logger
...
class TestEnterpriseCallbackControls:
    ...
    def mock_request_headers(self):
        ...
        with patch('enterprise.litelm_enterprise.enterprise_callbacks.callback_controls.get_proxy_server_request_headers') as mock_headers:
            yield mock_headers
    ...
    def test_callback_disabled_langfuse_string(self, mock_premium_user, mock_request_headers):
        ...
        litelm_params = {"proxy_server_request": {"url": "test"}}
        standard_callback_dynamic_params = StandardCallbackDynamicParams()
        
        result = EnterpriseCallbackControls.is_callback_disabled_dynamically("langfuse", litelm_params, standard_callback_dynamic_params)
    ...
    def test_callback_disabled_langfuse_customlogger(self, mock_premium_user, mock_request_headers):
        ...
        result = EnterpriseCallbackControls.is_callback_disabled_dynamically(langfuse_logger, litelm_params, standard_callback_dynamic_params)
        assert result is True
    ...
    def test_callback_disabled_s3_v2_string(self, mock_premium_user, mock_request_headers):
        ...
        result = EnterpriseCallbackControls.is_callback_disabled_dynamically("s3_v2", litelm_params, standard_callback_dynamic_params)
        assert result is True
    ...
    def test_callback_disabled_s3_v2_customlogger(self, mock_premium_user, mock_request_headers):
        ...
        with patch('litelm.integrations.s3_v2.S3Logger.__init__', return_value=None):
            s3_logger = S3Logger()
            result = EnterpriseCallbackControls.is_callback_disabled_dynamically(s3_logger, litelm_params, standard_callback_dynamic_params)
            assert result is True
    ...
    def test_callback_disabled_datadog_string(self, mock_premium_user, mock_request_headers):
        ...
        result = EnterpriseCallbackControls.is_callback_disabled_dynamically("datadog", litelm_params, standard_callback_dynamic_params)
        assert result is True
    ...
    def test_callback_disabled_datadog_customlogger(self, mock_premium_user, mock_request_headers):
        ...
        with patch('litelm.integrations.datadog.datadog.DataDogLogger.__init__', return_value=None):
            datadog_logger = DataDogLogger()
            result = EnterpriseCallbackControls.is_callback_disabled_dynamically(datadog_logger, litelm_params, standard_callback_dynamic_params)
            assert result is True
    ...
    def test_multiple_callbacks_disabled(self, mock_premium_user, mock_request_headers):
        ...
        assert EnterpriseCallbackControls.is_callback_disabled_dynamically("langfuse", litelm_params, standard_callback_dynamic_params) is True
        assert EnterpriseCallbackControls.is_callback_disabled_dynamically("datadog", litelm_params, standard_callback_dynamic_params) is True
        assert EnterpriseCallbackControls.is_callback_disabled_dynamically("s3_v2", litelm_params, standard_callback_dynamic_params) is True
        
        # Test non-disabled callback is not disabled
        assert EnterpriseCallbackControls.is_callback_disabled_dynamically("prometheus", litelm_params, standard_callback_dynamic_params) is False
    ...
    def test_custom_logger_not_in_registry(self, mock_premium_user, mock_request_headers):
        ...
        result = EnterpriseCallbackControls.is_callback_disabled_dynamically(unknown_logger, litelm_params, standard_callback_dynamic_params)
        assert result is False
    ...
    def test_callback_disabled_via_request_body_langfuse(self, mock_premium_user, mock_request_headers):
        """Test that callbacks can be disabled via request body litelm_disabled_callbacks"""
        mock_request_headers.return_value = {}  # No headers
        litelm_params = {"proxy_server_request": {"url": "test"}}
        standard_callback_dynamic_params = StandardCallbackDynamicParams(litelm_disabled_callbacks=["langfuse"])
    ...
    def test_callback_disabled_via_request_body_multiple(self, mock_premium_user, mock_request_headers):
        ...
        mock_request_headers.return_value = {}  # No headers
        litelm_params = {"proxy_server_request": {"url": "test"}}
        standard_callback_dynamic_params = StandardCallbackDynamicParams(litelm_disabled_callbacks=["langfuse", "datadog", "s3_v2"])
    ...
    def test_admin_can_disable_dynamic_callback_disabling(self, mock_request_headers):
        ...
        # Mock litelm.allow_dynamic_callback_disabling set to False
        with patch('litelm.allow_dynamic_callback_disabling', False):
    ...
    def test_admin_can_enable_dynamic_callback_disabling(self, mock_request_headers):
        ...
        # Mock litelm.allow_dynamic_callback_disabling set to True
        with patch('litelm.allow_dynamic_callback_disabling', True):
            with patch('litelm.proxy.proxy_server.premium_user', True):
                result = EnterpriseCallbackControls.is_callback_disabled_dynamically("langfuse", litelm_params, standard_callback_dynamic_params)
                assert result is True
    ...
    def test_default_admin_setting_allows_dynamic_callback_disabling(self, mock_request_headers):
        ...
        # litelm.allow_dynamic_callback_disabling should default to True
        with patch('litelm.proxy.proxy_server.premium_user', True):
            result = EnterpriseCallbackControls.is_callback_disabled_dynamically("langfuse", litelm_params, standard_callback_dynamic_params)
            assert result is True
