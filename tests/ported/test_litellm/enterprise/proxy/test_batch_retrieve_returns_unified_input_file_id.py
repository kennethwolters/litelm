...
def _mock_prisma(batch_json: str, managed_file_record=None):
    ...
    prisma.db.litelm_managedobjecttable.find_first = AsyncMock(
        return_value=batch_db_record
    )

    prisma.db.litelm_managedfiletable.find_first = AsyncMock(
...
async def test_should_resolve_raw_input_file_id_to_unified_id():
    ...
    prisma.db.litelm_managedfiletable.find_first.assert_called_once_with(
...
async def test_should_preserve_already_managed_input_file_id():
    ...
    unified_batch_id = "bGl0ZWxsbV9wcm94eTpiYXRjaF9pZA"
    decoded_unified = "litelm_proxy:application/octet-stream;unified_id,test-123"
    ...
    prisma.db.litelm_managedfiletable.find_first.assert_not_called()
