...
The fix (Option C) makes check_batch_cost call litelm.afile_content directly
...
def _make_unified_file_id() -> str:
    ...
    raw = "litelm_proxy:application/octet-stream;unified_id,test-123;target_model_names,azure-gpt-4"
...
def _make_managed_files_instance(file_created_by: str, unified_file_id: str):
    ...
    from litelm_enterprise.proxy.hooks.managed_files import (
    ...
    mock_prisma.db.litelm_managedfiletable.find_first = AsyncMock(
...
async def test_check_batch_cost_should_call_afile_content_directly_with_credentials():
    """
    check_batch_cost should call litelm.afile_content directly with deployment
    ...
    default_user_id.
    """
    from litelm_enterprise.proxy.common_utils.check_batch_cost import CheckBatchCost

    # Build a unified object ID in the expected format:
    # litelm_proxy;model_id:{};llm_batch_id:{};llm_output_file_id:{}
    unified_raw = "litelm_proxy;model_id:model-deploy-xyz;llm_batch_id:batch-123;llm_output_file_id:file-raw-output"
    ...
    # Mock prisma
    mock_prisma = MagicMock()
    mock_prisma.db.litelm_managedobjecttable.find_many = AsyncMock(
        return_value=[mock_job]
    )
    mock_prisma.db.litelm_managedobjecttable.update = AsyncMock()
    ...
    mock_deployment.litelm_params.custom_llm_provider = "azure"
    mock_deployment.litelm_params.model = "azure/gpt-4"
    ...
    with patch(
        "litelm.files.main.afile_content",
        ...
        mock_prisma.db.litelm_managedobjecttable.update.assert_called_once()
        update_call_kwargs = mock_prisma.db.litelm_managedobjecttable.update.call_args.kwargs
