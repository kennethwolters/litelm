...
import litelm
from litelm.router_utils.fallback_event_handlers import run_async_fallback
...
def test_update_kwargs_does_not_mutate_defaults_and_merges_metadata():
    ...
    router.default_litelm_params = {
        "foo": "bar",
        "metadata": {"baz": 123},
    }
    original = copy.deepcopy(router.default_litelm_params)
    kwargs: dict = {}

    # invoke the helper
    router._update_kwargs_with_default_litelm_params(
        kwargs=kwargs,
        metadata_variable_name="litelm_metadata",
    )

    # 1) router.defaults must be unchanged
    assert router.default_litelm_params == original

    # 2) non‑metadata keys get merged
    assert kwargs["foo"] == "bar"

    # 3) metadata lands under "metadata"
    assert kwargs["litelm_metadata"] == {"baz": 123}
...
async def test_async_router_acreate_file():
    ...
    router = litelm.Router(
        model_list=[
            ...
            {"model_name": "gpt-3.5-turbo", "litelm_params": {"model": "gpt-4o-mini"}},
        ],
    )

    with patch("litelm.acreate_file", return_value=MagicMock()) as mock_acreate_file:
...
async def test_async_router_acreate_file_with_jsonl():
    ...
    router = litelm.Router(
        model_list=[
            ...
            {
                "model_name": "gpt-3.5-turbo-router",
                "litelm_params": {"model": "gpt-4o-mini"},
            },
        ],
    )
...
async def test_arouter_async_get_healthy_deployments():
    ...
    assert result[0]["litelm_params"]["model"] == "gpt-3.5-turbo"


@pytest.mark.asyncio
@patch("litelm.amoderation")
async def test_arouter_amoderation_with_credential_name(mock_amoderation):
    """
    Test that router.amoderation passes litelm_credential_name to the underlying litelm.amoderation call
    """
        ...
        model_list=[
            {
                ...
                "litelm_params": {
                    ...
                    "litelm_credential_name": "my-custom-auth",
                },
            },
        ],
    )
    ...
    assert call_kwargs["litelm_credential_name"] == "my-custom-auth"
...
def test_arouter_ignore_invalid_deployments():
    ...
    """
    from litelm.types.router import Deployment

    router = litelm.Router(
        model_list=[
            {
                "model_name": "gpt-3.5-turbo",
                "litelm_params": {"model": "my-bad-model"},
            },
        ],
    ...
    router.upsert_deployment(
        Deployment(
            model_name="gpt-3.5-turbo",
            litelm_params={"model": "my-bad-model"},  # type: ignore
...
async def test_arouter_aretrieve_batch():
    ...
    with patch.object(
        litelm, "aretrieve_batch", return_value=AsyncMock()
...
async def test_arouter_aretrieve_file_content():
    ...
    with patch.object(
        litelm, "afile_content", return_value=AsyncMock()
...
async def test_arouter_filter_team_based_models():
    ...
    router.add_deployment(
        Deployment(
            model_name="gpt-3.5-turbo",
            litelm_params={"model": "gpt-3.5-turbo"},  # type: ignore
            model_info={"tpm": 1000, "rpm": 1000},
        )
    )
...
def test_arouter_responses_api_bridge():
    ...
    with patch.object(litelm, "responses", return_value=AsyncMock()) as mock_responses:
        ...
        model_list=[
            {
                ...
                "litelm_params": {
                    ...
                    "mock_response": "litelm.InternalServerError",
                },
            },
            {
...
def test_cached_get_model_group_info():
    ...
    router = litelm.Router(
        model_list=[
            {
                "model_name": "gpt-4",
                "litelm_params": {"model": "gpt-4", "api_key": "fake"},
    ...
    router.add_deployment(
        Deployment(
            model_name="gpt-4",
            litelm_params=LiteLLM_Params(model="gpt-4", api_key="fake2"),
...
def test_get_model_access_groups_caching():
    ...
    router.add_deployment(
        Deployment(
            model_name="gpt-3.5",
            litelm_params=LiteLLM_Params(model="gpt-3.5-turbo"),
...
def test_get_model_access_groups_cache_invalidation_set_model_list():
    ...
    router.set_model_list(
        [
            {
                "model_name": "claude-3",
                "litelm_params": {"model": "anthropic/claude-3-opus-20240229"},
...
def test_get_model_access_groups_cache_invalidation_upsert_deployment():
    ...
    router.upsert_deployment(
        Deployment(
            ...
            litelm_params=LiteLLM_Params(model="gpt-4-turbo"),
...
async def test_acompletion_streaming_iterator():
    ...
    from litelm.exceptions import MidStreamFallbackError
    ...
    router = litelm.Router(
        model_list=[
            {
                "model_name": "gpt-4",
                "litelm_params": {"model": "gpt-4", "api_key": "fake-key-1"},
            },
            {
                "model_name": "gpt-3.5-turbo",
                "litelm_params": {"model": "gpt-3.5-turbo", "api_key": "fake-key-2"},
            },
        ],
...
async def test_acompletion_streaming_iterator_edge_cases():
    ...
    router = litelm.Router(
        model_list=[
            {
                "model_name": "gpt-4",
                "litelm_params": {"model": "gpt-4", "api_key": "fake-key"},
            }
        ],
        set_verbose=True,
    )
...
async def test_acompletion_streaming_iterator_preserves_hidden_params():
    ...
    original CustomStreamWrapper so that x-litelm-overhead-duration-ms (and
    ...
    # update_response_metadata (litelm_overhead_time_ms, _response_ms, etc.)
    mock_response = MagicMock()
    mock_response.model = "gpt-4"
    mock_response.custom_llm_provider = "openai"
    mock_response.logging_obj = MagicMock()
    mock_response._hidden_params = {
        "litelm_overhead_time_ms": 12.34,
        "_response_ms": 500.0,
    ...
    assert result._hidden_params.get("litelm_overhead_time_ms") == 12.34, (
        "litelm_overhead_time_ms must be preserved — "
        "this is what drives x-litelm-overhead-duration-ms in streaming responses"
    )
    assert result._hidden_params.get("litelm_call_id") == "test-call-id"
...
def test_completion_streaming_iterator_preserves_hidden_params():
    ...
    mock_response.logging_obj = MagicMock()
    mock_response._hidden_params = {
        "litelm_overhead_time_ms": 42.0,
        "litelm_call_id": "test-sync-call",
    }
    ...
    assert result._hidden_params.get("litelm_overhead_time_ms") == 42.0
    assert result._hidden_params.get("litelm_call_id") == "test-sync-call"
...
def test_get_deployment_model_info_base_model_flow():
    ...
    with patch.object(
        litelm, "model_cost", {"test-custom-model": mock_custom_model_info}
    ):
        with patch.object(litelm, "get_model_info") as mock_get_model_info:
            # Configure mock returns
            mock_get_model_info.side_effect = lambda model: {
                "gpt-3.5-turbo": mock_base_model_info,
                "test-model": mock_litelm_model_name_info,
            }.get(model)
            ...
            # 2. The result of step 1 gets merged into litelm_model_name_info (custom+base override litelm)
            ...
            assert result["litelm_provider"] == "openai"  # From base model
            assert (
                result["mode"] == "chat"
            )  # From base model (overrides litelm "completion")

            # The key field comes from base model since both base and litelm have it
            # and base model info overrides litelm model name info in final merge
            assert (
                result["key"] == "gpt-3.5-turbo"
            )  # From base model (overrides litelm key)
    ...
    with patch.object(
        ...
        with patch.object(litelm, "get_model_info") as mock_get_model_info:
            ...
            assert result["max_tokens"] == 2048  # From litelm model name info
            assert result["custom_field"] == "custom_value"  # From custom model
            assert result["mode"] == "completion"  # From litelm model name info

    # Test Case 3: No custom model info, only litelm model name info
    with patch.object(litelm, "model_cost", {}):  # Empty model cost
        with patch.object(litelm, "get_model_info") as mock_get_model_info:
            mock_get_model_info.side_effect = lambda model: {
                "test-model": mock_litelm_model_name_info,
            }.get(model)
            ...
            # Result should be just the litelm model name info
            assert result is not None
            assert result == mock_litelm_model_name_info
        ...
        with patch.object(litelm, "get_model_info") as mock_get_model_info:
            ...
            def mock_get_model_info_side_effect(model):
                if model == "invalid-base-model":
                    ...
                    return mock_litelm_model_name_info
                return None
    ...
    with patch.object(litelm, "model_cost", {}):
        with patch.object(
            litelm, "get_model_info", side_effect=Exception("Not found")
        ):
            result = router.get_deployment_model_info(
...
def test_get_deployment_model_info_base_model_merge_priority():
    ...
    mock_litelm_model_name_info = {
        ...
        "mode": "completion",
        "litelm_only_field": "litelm_value",
    }

    with patch.object(
        litelm, "model_cost", {"custom-model-id": mock_custom_model_info}
    ):
        with patch.object(litelm, "get_model_info") as mock_get_model_info:
            mock_get_model_info.side_effect = lambda model: {
            ...
            # 3. Result from steps 1-2 overrides litelm_model_name_info
            ...
            assert (
                result["litelm_provider"] == "openai"
            )  # From base model (not in custom)
            assert (
                result["base_only_field"] == "base_value"
            )  # From base model (not in custom)

            # Fields that should come from litelm model name info (not overridden by custom+base)
            assert (
                result["mode"] == "completion"
            )  # From litelm model name info (not in custom or base)
            assert (
                result["litelm_only_field"] == "litelm_value"
            )  # From litelm model name info (not in custom or base)

            # Key comes from base model since both base and litelm have key fields
            # and the merged custom+base overrides litelm in the final merge
...
async def test_router_acompletion_with_unknown_model_and_default_fallback():
    ...
    router = litelm.Router(model_list=model_list, default_fallbacks=["gpt-4o"])
...
def test_get_deployment_credentials_with_provider_aws_bedrock_runtime_endpoint():
    ...
    aws_bedrock_runtime_endpoint from deployment litelm_params to credentials.
    """
    router = litelm.Router(
        model_list=[
            {
...
def test_get_deployment_credentials_with_provider_resolves_credential_name():
    ...
    litelm_credential_name to actual credential values (for UI-created models).
    """
    from litelm.types.utils import CredentialItem

    # Setup credential list with a test credential
    litelm.credential_list = [
        CredentialItem(
        ...
        model_list=[
            {
                ...
                "litelm_params": {
                    "model": "azure/gpt-4",
                    "litelm_credential_name": "test-azure-cred",
                },
            }
        ],
    )
    ...
    assert "litelm_credential_name" not in credentials
...
def test_get_available_guardrail_single_deployment():
    ...
    guardrail_config = {
        ...
        "litelm_params": {"guardrail": "custom", "mode": "pre_call"},
        "id": "guardrail-1",
    }
...
async def test_anthropic_messages_call_type_is_cached():
    ...
    from litelm.caching.dual_cache import DualCache
    from litelm.router_utils.pre_call_checks.prompt_caching_deployment_check import (
        PromptCachingDeploymentCheck,
    )
    from litelm.router_utils.prompt_caching_cache import PromptCachingCache
...
def test_update_kwargs_with_deployment_propagates_model_tags():
    """
    Test that deployment-level tags from litelm_params are merged into
    ...
    See: https://github.com/BerriAI/litelm/issues/XXXX
    """
    router = litelm.Router(
        model_list=[
            {
...
def test_update_kwargs_with_deployment_merges_tools():
    """
    Test that when both deployment litelm_params and request have tools,
...
def test_credential_name_injected_as_tag():
    """
    Test that litelm_credential_name from deployment litelm_params
...
def test_credential_name_not_injected_when_absent():
    """
    Test that when no litelm_credential_name is set, tags are unchanged.
    """
    router = litelm.Router(
        model_list=[
            {
...
def test_combine_fallback_usage():
    ...
    chunk = litelm.ModelResponseStream(
        id="test",
        model="gpt-4o",
        choices=[],
