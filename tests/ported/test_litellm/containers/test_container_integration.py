...
from litelm.containers.main import (
...
class TestContainerIntegration:
    ...
    @patch('litelm.llms.custom_httpx.llm_http_handler.HTTPHandler')
    ...
    @pytest.mark.asyncio
    @patch('litelm.llms.custom_httpx.llm_http_handler.AsyncHTTPHandler')
    async def test_async_container_create_full_flow(self, mock_async_http_handler):
        ...
        with patch('litelm.llms.custom_httpx.llm_http_handler.get_async_httpx_client') as mock_get_async_client:
    ...
    def test_container_workflow_simulation(self):
        ...
        with patch('litelm.containers.main.base_llm_http_handler') as mock_handler:
    ...
    def test_error_handling_integration(self):
        ...
        import importlib
        import litelm.containers.main as containers_main_module

        # Reload the module to ensure it has a fresh reference to base_llm_http_handler
        # after conftest reloads litelm
        importlib.reload(containers_main_module)

        # Re-import the function after reload
        from litelm.containers.main import create_container as create_container_fresh

        with patch('litelm.containers.main.base_llm_http_handler') as mock_handler:
            # Simulate an API error
            mock_handler.container_create_handler.side_effect = litelm.APIError(
                status_code=400,
    ...
    def test_provider_support(self, provider):
        ...
        # Reload the module to ensure it has a fresh reference to base_llm_http_handler
        # after conftest reloads litelm (same pattern as test_error_handling_integration)
        importlib.reload(containers_main_module)
