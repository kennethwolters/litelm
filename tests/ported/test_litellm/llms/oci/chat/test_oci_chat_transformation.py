...
from litelm.llms.oci.chat.transformation import OCIChatConfig, OCIRequestWrapper, version
...
class TestOCIChatConfig:
    def test_validate_environment_with_oci_region(self, supplied_params):
        ...
        assert result["user-agent"] == f"litelm/{version}"
    ...
    def test_transform_response_simple_text(self):
        ...
        assert isinstance(result.choices[0], litelm.Choices)
        ...
        assert isinstance(result.usage, litelm.Usage) # type: ignore
    ...
    def test_transform_response_with_tool_calls(self):
        ...
        model_response = ModelResponse(
            choices=[litelm.Choices(index=0, message=litelm.Message())]
        )
        ...
        assert isinstance(tool_call, litelm.utils.ChatCompletionMessageToolCall)
        ...
        assert isinstance(usage, litelm.Usage) # type: ignore
...
class TestOCISignerSupport:
    ...
    def test_sign_request_with_failing_oci_signer(self):
        ...
        from litelm.llms.oci.common_utils import OCIError
