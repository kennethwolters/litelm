...
from litelm.llms.openai.evals.transformation import OpenAIEvalsConfig
...
def test_validate_environment_sets_headers(config: OpenAIEvalsConfig):
    ...
    result = config.validate_environment(headers=headers, litelm_params=params)
...
def test_validate_environment_requires_api_key(config: OpenAIEvalsConfig, monkeypatch):
    ...
    with pytest.raises(ValueError, match="OPENAI_API_KEY is required"):
        config.validate_environment(headers=headers, litelm_params=params)
...
def test_transform_list_evals_request(config: OpenAIEvalsConfig):
    ...
    url, query_params = config.transform_list_evals_request(
        ...
        litelm_params=GenericLiteLLMParams(api_base="https://api.openai.com"),
        headers={},
    )
