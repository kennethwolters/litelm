...
import litelm
from litelm.llms.azure.responses.transformation import AzureOpenAIResponsesAPIConfig
...
class TestOpenAIResponsesAPIConfig:
    ...
    def test_validate_environment(self):
        ...
        litelm_params = GenericLiteLLMParams(api_key=api_key)
        result = self.config.validate_environment(
            headers=headers, model=self.model, litelm_params=litelm_params
        )
        ...
        with patch("litelm.api_key", "litelm_api_key"):
            ...
            assert "Authorization" in result
            assert result["Authorization"] == "Bearer litelm_api_key"

        # Test with existing headers
        headers = {"Content-Type": "application/json"}

        with patch("litelm.openai_key", "openai_key"):
            with patch("litelm.api_key", None):
                litelm_params = GenericLiteLLMParams()
                result = self.config.validate_environment(
                    headers=headers, model=self.model, litelm_params=litelm_params
                )
        ...
        with patch("litelm.api_key", None):
            with patch("litelm.openai_key", None):
                with patch(
                    "litelm.llms.openai.responses.transformation.get_secret_str",
    ...
    def test_get_complete_url(self):
        ...
        # Test with litelm.api_base
        with patch("litelm.api_base", "https://litelm-api-base.example.com/v1"):
            result = self.config.get_complete_url(
                api_base=None,
                litelm_params={},
            )

            assert result == "https://litelm-api-base.example.com/v1/responses"

        # Test with environment variable
        with patch("litelm.api_base", None):
            with patch(
                "litelm.llms.openai.responses.transformation.get_secret_str",
    ...
    def test_get_event_model_class_generic_event(self):
        ...
        from litelm.types.llms.openai import GenericEvent
    ...
    def test_transform_streaming_response_coalesces_null_error_code(self):
        ...
        """
        from litelm.types.llms.openai import ErrorEvent
    ...
    def test_transform_streaming_response_missing_required_fields_response_created(
        ...
        Reproduces https://github.com/BerriAI/litelm/issues/20570
        """
        from litelm.types.llms.openai import ResponseCreatedEvent
    ...
    def test_transform_streaming_response_missing_required_fields_output_text_delta(
        ...
        Reproduces https://github.com/BerriAI/litelm/issues/20570
        """
        from litelm.types.llms.openai import OutputTextDeltaEvent
    ...
    def test_transform_streaming_response_missing_required_fields_content_part_added(
        ...
        Reproduces https://github.com/BerriAI/litelm/issues/20570
        """
        from litelm.types.llms.openai import ContentPartAddedEvent
    ...
    def test_transform_streaming_response_missing_required_fields_output_item_added(
        ...
        Reproduces https://github.com/BerriAI/litelm/issues/20570
        """
        from litelm.types.llms.openai import OutputItemAddedEvent
...
class TestAzureResponsesAPIConfig:
    ...
    def test_azure_get_complete_url_with_version_types(self):
        ...
        base_url = "https://litelm8397336933.openai.azure.com"

        # Test with preview version - should use openai/v1/responses
        result_preview = self.config.get_complete_url(
            api_base=base_url,
            litelm_params={"api_version": "preview"},
        )
        assert (
            result_preview
            == "https://litelm8397336933.openai.azure.com/openai/v1/responses?api-version=preview"
        )

        # Test with latest version - should use openai/v1/responses
        result_latest = self.config.get_complete_url(
            api_base=base_url,
            litelm_params={"api_version": "latest"},
        )
        assert (
            result_latest
            == "https://litelm8397336933.openai.azure.com/openai/v1/responses?api-version=latest"
        )

        # Test with date-based version - should use openai/responses
        result_date = self.config.get_complete_url(
            api_base=base_url,
            litelm_params={"api_version": "2025-01-01"},
        )
        assert (
            result_date
            == "https://litelm8397336933.openai.azure.com/openai/responses?api-version=2025-01-01"
        )
...
class TestTransformListInputItemsRequest:
    ...
    def setup_method(self):
        ...
        self.litelm_params = GenericLiteLLMParams()
    ...
    def test_openai_transform_list_input_items_request_minimal(self):
        ...
        url, params = self.openai_config.transform_list_input_items_request(
            ...
            litelm_params=self.litelm_params,
    ...
    @patch("litelm.router.Router")
    def test_mock_litelm_router_with_transform_list_input_items_request(
        self, mock_router
    ):
        """Mock test using litelm.router for transform_list_input_items_request"""
        # Setup mock router
    ...
    @patch("litelm.list_input_items")
    def test_mock_litelm_list_input_items_integration(self, mock_list_input_items):
        """Test integration with litelm.list_input_items function"""
        # Setup mock response
        mock_response = {
            "object": "list",
            "data": [
...
class TestPhaseParameter:
    ...
    def _make_output_text(text: str):
        from litelm.types.responses.main import OutputText
    ...
    def test_generic_response_output_item_accepts_phase_commentary(self):
        from litelm.types.responses.main import GenericResponseOutputItem
    ...
    def test_output_function_tool_call_accepts_phase(self):
        from litelm.types.responses.main import OutputFunctionToolCall
