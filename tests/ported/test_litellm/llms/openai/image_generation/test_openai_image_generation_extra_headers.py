...
Verifies that extra_headers passed to litelm.image_generation() /
litelm.aimage_generation() are forwarded to the OpenAI API client as
...
class TestImageGenerationEntryPointHeaders:
    """Test that litelm.image_generation() passes headers through to the OpenAI provider."""

    @pytest.mark.asyncio
    async def test_extra_headers_reach_openai_provider(self):
        """End-to-end: extra_headers from litelm.aimage_generation() reach OpenAI images.generate()."""
        import litelm
        ...
        await litelm.aimage_generation(
            model="dall-e-3",
            prompt="A white cat",
