...
def test_openai_realtime_handler_url_construction(api_base):
    from litelm.llms.openai.realtime.handler import OpenAIRealtime
...
def test_openai_realtime_handler_url_with_extra_params():
    from litelm.llms.openai.realtime.handler import OpenAIRealtime
    from litelm.types.realtime import RealtimeQueryParams
...
async def test_async_realtime_success():
    ...
    with patch("websockets.connect", return_value=DummyAsyncContextManager(mock_backend_ws)) as mock_ws_connect, \
         patch("litelm.llms.openai.realtime.handler.RealTimeStreaming") as mock_realtime_streaming:
...
async def test_async_realtime_ws_url_has_no_ssl():
    ...
    This verifies the fix for: https://github.com/BerriAI/litelm/issues/19222
    """
    from litelm.llms.openai.realtime.handler import OpenAIRealtime
    from litelm.types.realtime import RealtimeQueryParams
