...
import litelm
from litelm.llms.custom_httpx.aiohttp_transport import LiteLLMAiohttpTransport
from litelm.llms.custom_httpx.http_handler import AsyncHTTPHandler, get_ssl_configuration


@pytest.mark.asyncio
async def test_ssl_security_level(monkeypatch):
    # Ensure aiohttp transport is enabled for this test
    original_disable = litelm.disable_aiohttp_transport
    litelm.disable_aiohttp_transport = False
    ...
    try:
        ...
        litelm.disable_aiohttp_transport = original_disable
...
async def test_ssl_verification_with_aiohttp_transport():
    ...
    We validate that the ssl settings for a litelm transport match what a ssl verify=False aiohttp client would have.
    ...
    try:
        # Create a test SSL context
        litelm_async_client = AsyncHTTPHandler(ssl_verify=False)

        transport = litelm_async_client.client._transport
        assert isinstance(transport, LiteLLMAiohttpTransport)
        ...
        # assert both litelm transport and aiohttp session have ssl_verify=False
...
async def test_aiohttp_transport_trust_env_setting(monkeypatch):
    ...
    # Default should be False (litelm.aiohttp_trust_env default)
    default_trust_env = getattr(litelm, 'aiohttp_trust_env', False)
    ...
    # Should respect the litelm.aiohttp_trust_env setting when env var is False
...
def test_get_ssl_configuration():
    ...
    from litelm.llms.custom_httpx.http_handler import _ssl_context_cache
...
async def test_get_async_httpx_client_with_shared_session():
    ...
    from litelm.llms.custom_httpx.http_handler import get_async_httpx_client, AsyncHTTPHandler as AsyncHTTPHandlerReload
...
def test_shared_session_parameter_in_acompletion():
    ...
    import inspect
    from litelm.main import acompletion
...
def test_shared_session_parameter_in_completion():
    ...
    import inspect
    from litelm.main import completion
...
@pytest.mark.parametrize(
    "env_curve,litelm_curve,expected_curve,should_call",
    [
        # env_curve: SSL_ECDH_CURVE env var | litelm_curve: litelm.ssl_ecdh_curve variable
        ...
        (None, "secp384r1", "secp384r1", True),     # litelm variable only
...
def test_ssl_ecdh_curve(env_curve, litelm_curve, expected_curve, should_call, monkeypatch):
    ...
    with patch.dict(os.environ, clear=True):
        ...
        original_value = litelm.ssl_ecdh_curve
        try:
            litelm.ssl_ecdh_curve = litelm_curve
            ...
            litelm.ssl_ecdh_curve = original_value


def test_default_user_agent_is_litelm_version(monkeypatch):
    from litelm._version import version
    from litelm.llms.custom_httpx.http_handler import get_default_headers

    monkeypatch.delenv("LITELLM_USER_AGENT", raising=False)

    assert get_default_headers()["User-Agent"] == f"litelm/{version}"
