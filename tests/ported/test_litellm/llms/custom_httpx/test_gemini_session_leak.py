...
async def test_aiohttp_handler_cleanup():
    ...
    from litelm.llms.custom_httpx.aiohttp_handler import BaseLLMAIOHTTPHandler
...
async def test_atexit_cleanup():
    ...
    from litelm.llms.custom_httpx.async_client_cleanup import (
    ...
    # Use the actual global base_llm_aiohttp_handler from litelm.main
    print("\nAccessing global base_llm_aiohttp_handler (like Gemini does)...")
    import litelm

    handler = litelm.base_llm_aiohttp_handler
    session = handler._get_async_client_session()
    ...
    print("\nCalling close_litelm_async_clients() (simulates atexit)...")
    await close_litelm_async_clients()
...
def test_new_event_loop_atexit():
    ...
    try:
        new_loop.run_until_complete(close_litelm_async_clients())
