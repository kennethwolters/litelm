...
from litelm.types.responses.main import DeleteResponseResult
...
class TestVolcengineResponsesAPITransformation:
    ...
    def test_validate_environment_uses_api_key(
        self, monkeypatch, litelm_params, expected_key
    ):
        """validate_environment should pull api key from params/env and attach headers."""
        config = VolcEngineResponsesAPIConfig()

        monkeypatch.setattr(litelm, "api_key", None)
        monkeypatch.delenv("ARK_API_KEY", raising=False)
        monkeypatch.delenv("VOLCENGINE_API_KEY", raising=False)

        headers = config.validate_environment(
            headers={}, model="volcengine/demo-model", litelm_params=litelm_params
        )
    ...
    def test_validate_environment_raises_without_key(self, monkeypatch):
        ...
        with pytest.raises(ValueError):
            config.validate_environment(
                headers={}, model="volcengine/demo", litelm_params={}
            )
    ...
    def test_error_class_returns_volcengine_error(self):
        ...
        # module reloading during parallel test execution (conftest reloads litelm)
