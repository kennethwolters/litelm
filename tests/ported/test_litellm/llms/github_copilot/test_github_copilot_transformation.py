...
import litelm.llms.github_copilot.chat.transformation
from litelm import Choices, Message, ModelResponse, Usage, acompletion, completion
from litelm.exceptions import AuthenticationError
from litelm.llms.github_copilot.authenticator import Authenticator
from litelm.llms.github_copilot.chat.transformation import GithubCopilotConfig
from litelm.llms.github_copilot.common_utils import (
    APIKeyExpiredError,
    GetAccessTokenError,
    GetAPIKeyError,
    GetDeviceCodeError,
...
@patch("litelm.llms.github_copilot.authenticator.Authenticator.get_api_key")
@patch("litelm.llms.openai.openai.OpenAIChatCompletion.completion")
...
def test_transform_messages_disable_copilot_system_to_assistant(monkeypatch):
    ...
    # Save original value
    original_flag = litelm.disable_copilot_system_to_assistant
    try:
        # Case 1: Flag is False (default, conversion happens)
        litelm.disable_copilot_system_to_assistant = False
        config = GithubCopilotConfig()
        messages = [
        ...
        litelm.disable_copilot_system_to_assistant = True
        out = config._transform_messages([m.copy() for m in messages], model="github_copilot/gpt-4")
        ...
        litelm.disable_copilot_system_to_assistant = original_flag
