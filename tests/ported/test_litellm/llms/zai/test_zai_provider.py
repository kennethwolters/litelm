...
def test_zai_in_provider_lists():
    ...
    assert "zai" in litelm.openai_compatible_providers
    assert "zai" in litelm.provider_list
...
def test_zai_models_in_model_cost():
    ...
    for model in zai_models:
        assert model in litelm.model_cost, f"Model {model} not found in model_cost"
        assert litelm.model_cost[model]["litelm_provider"] == "zai"
...
def test_glm47_supports_reasoning():
    ...
    key = "zai/glm-4.7"
    assert key in litelm.model_cost, f"Model {key} not found in model_cost"
