...
from litelm.llms.bedrock.chat.invoke_agent.transformation import (
    AmazonInvokeAgentConfig,
)
from litelm.types.llms.bedrock_invoke_agents import (
    InvokeAgentEvent,
    InvokeAgentEventHeaders,
    InvokeAgentUsage,
)
from litelm.types.utils import Message, ModelResponse, Usage
...
class TestAmazonInvokeAgentConfig:
    ...
    @patch(
        "litelm.llms.bedrock.chat.invoke_agent.transformation.convert_content_list_to_str"
    )
    def test_transform_request(self, mock_convert, config, sample_messages):
        ...
        result = config.transform_request(
            model, sample_messages, optional_params, litelm_params, headers
        )
    ...
    def test_get_complete_url(self, mock_region, mock_endpoint, mock_convert, config):
        ...
        result = config.get_complete_url(
            api_base, api_key, model, optional_params, litelm_params
        )
