...
import litelm
from litelm import ModelResponse, RateLimitError, completion
from litelm.llms.bedrock.chat.converse_transformation import AmazonConverseConfig
from litelm.types.llms.bedrock import ConverseTokenUsageBlock
...
def test_get_supported_openai_params_bedrock_converse():
    ...
    Note: This test is critical for routing, if we ever remove `litelm.BEDROCK_CONVERSE_MODELS`,
    please update this test to read `bedrock_converse` models from the model cost map.
    """
    for model in litelm.BEDROCK_CONVERSE_MODELS:
...
def test_transform_response_with_computer_use_tool():
    ...
    from litelm.llms.bedrock.chat.converse_transformation import AmazonConverseConfig
    from litelm.types.llms.bedrock import (
...
def test_empty_assistant_message_handling():
    ...
    """
    # Import the litelm module that factory.py uses to ensure we patch the correct reference
    import litelm.litelm_core_utils.prompt_templates.factory as factory_module
    from litelm.litelm_core_utils.prompt_templates.factory import (
        _bedrock_converse_messages_pt,
    )
    ...
    # Use patch to ensure we modify the litelm reference that factory.py actually uses
    # This avoids issues with module reloading during parallel test execution
    with patch.object(factory_module.litelm, "modify_params", True):
        result = _bedrock_converse_messages_pt(
            messages=messages,
            model="anthropic.claude-3-5-sonnet-20240620-v1:0",
            llm_provider="bedrock_converse"
...
def test_thinking_with_max_completion_tokens():
    ...
    from litelm.constants import DEFAULT_MAX_TOKENS
...
def test_drop_thinking_param_when_thinking_blocks_missing():
    ...
    Related issue: https://github.com/BerriAI/litelm/issues/14194
    """
    from litelm.utils import last_assistant_with_tool_calls_has_no_thinking_blocks

    # Save original modify_params setting
    original_modify_params = litelm.modify_params
    ...
    try:
        ...
        litelm.modify_params = original_modify_params
...
def test_transform_response_with_both_json_tool_call_and_real_tool():
    ...
    Fixes https://github.com/BerriAI/litelm/issues/18381
    """
    from litelm.llms.bedrock.chat.converse_transformation import AmazonConverseConfig
    from litelm.types.utils import ModelResponse
...
def test_streaming_filters_json_tool_call_with_real_tools():
    ...
    """
    from litelm.llms.bedrock.chat.invoke_handler import AWSEventStreamDecoder
    from litelm.types.llms.bedrock import (
