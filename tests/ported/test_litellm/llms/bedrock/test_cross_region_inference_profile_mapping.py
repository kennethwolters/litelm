...
from litelm.utils import _get_model_info_helper
from litelm.cost_calculator import completion_cost
from litelm.types.utils import ModelResponse, Usage, Choices, Message
...
def test_bedrock_cross_region_inference_profile_mapping():
    ...
    assert model_info["litelm_provider"] == "bedrock"
...
def test_proxy_cost_calculation_scenario():
    ...
    model = "litelm_proxy/bedrock/us.anthropic.claude-3-5-haiku-20241022-v1:0"

    # Test model info lookup works
    model_info = _get_model_info_helper(model=model, custom_llm_provider="litelm_proxy")
    assert model_info is not None
    ...
    cost = completion_cost(completion_response=response, model=model, custom_llm_provider="litelm_proxy")
