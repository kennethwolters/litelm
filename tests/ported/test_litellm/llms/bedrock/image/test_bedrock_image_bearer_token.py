...
class TestBedrockImageGeneration:
    def test_image_generation_with_api_key_bearer_token(self):
        ...
        with patch("litelm.llms.bedrock.image_generation.image_handler.BedrockImageGeneration.image_generation") as mock_bedrock_image_gen:
            # Setup mock response
            mock_image_response_obj = litelm.ImageResponse()
    ...
    def test_image_generation_with_env_variable_bearer_token(self, monkeypatch):
        ...
        with patch.dict(os.environ, {"AWS_BEARER_TOKEN_BEDROCK": test_api_key}), \
             patch("litelm.llms.bedrock.image_generation.image_handler.BedrockImageGeneration.image_generation") as mock_bedrock_image_gen:
    ...
    async def test_async_image_generation_with_bearer_token(self):
        ...
        with patch("litelm.llms.bedrock.image_generation.image_handler.BedrockImageGeneration.async_image_generation") as mock_async_bedrock_image_gen:
            mock_image_response_obj = litelm.ImageResponse()
            mock_image_response_obj.data = [{"url": "https://example.com/image.jpg"}]
