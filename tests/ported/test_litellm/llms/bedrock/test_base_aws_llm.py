...
import litelm
from litelm.llms.bedrock.base_aws_llm import (
    AwsAuthError,
    BaseAWSLLM,
...
BASE_AWS_LLM_PATH = os.path.join(
    os.path.dirname(__file__), "../../../../litelm/llms/bedrock/base_aws_llm.py"
)
...
def test_boto3_init_tracer_wrapping():
    ...
    Ensures observability of boto3 calls in litelm.
    """
...
def test_auth_functions_tracer_wrapping():
    ...
    Ensures observability of AWS authentication calls in litelm.
    """
    # Get the source code of base_aws_llm.py
    with open(BASE_AWS_LLM_PATH, "r") as f:
        content = f.read()
...
def test_get_aws_region_name_boto3_fallback():
    ...
    with patch("litelm.llms.bedrock.base_aws_llm.get_secret") as mock_get_secret:
...
def test_role_assumption_without_session_name():
    ...
    with patch("boto3.client", return_value=mock_sts_client):
        ...
        # Session name should be auto-generated with format "litelm-session-{timestamp}"
        assert call_args[1]["RoleSessionName"].startswith("litelm-session-")
...
def test_converse_handler_external_id_extraction():
    ...
    from litelm.llms.bedrock.chat.converse_handler import BedrockConverseLLM
    ...
    with patch.object(converse_llm, 'get_credentials', side_effect=mock_get_credentials):
        with patch.object(converse_llm, '_get_aws_region_name', return_value="us-west-2"):
            with patch.object(converse_llm, 'get_runtime_endpoint', return_value=("https://test", "https://test")):
                with patch('litelm.AmazonConverseConfig') as mock_config:
                    ...
                    with patch.object(converse_llm, 'get_request_headers') as mock_headers:
                        ...
                        with patch('litelm.llms.custom_httpx.http_handler._get_httpx_client') as mock_client:
