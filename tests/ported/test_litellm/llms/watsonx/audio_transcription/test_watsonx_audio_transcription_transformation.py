...
Validates that litelm.transcription transforms requests correctly for WatsonX.
"""
...
import litelm
from litelm.llms.watsonx.audio_transcription.transformation import (
    IBMWatsonXAudioTranscriptionConfig,
)
from litelm.types.utils import TranscriptionResponse


class TestWatsonXAudioTranscription:
    """Tests for WatsonX audio transcription via litelm.transcription."""

    @pytest.mark.asyncio
    async def test_watsonx_transcription_url_and_headers(self):
        """
        Test that litelm.transcription sends request to correct WatsonX URL with proper headers.
        """
        captured_request = {}
    ...
    async def test_watsonx_transcription_request_body(self):
        """
        Test that litelm.transcription sends correct request body for WatsonX.
