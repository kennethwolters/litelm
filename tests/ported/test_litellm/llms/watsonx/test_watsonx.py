    ...
    def _call(
        ...
        if patch_token_call:
            ...
            with patch.object(client, "post") as mock_post, patch.object(
                litelm.module_level_client, "post", return_value=mock_response
            ) as mock_get:
                try:
                    completion(
                        model=model,
...
def test_watsonx_gpt_oss_prompt_transformation(monkeypatch):
    ...
    This test calls litelm.completion (sync) and verifies what gets sent in the final POST request body.
    ...
    monkeypatch.setitem(litelm.known_tokenizer_config, hf_model, mock_tokenizer_config)
    ...
    with patch.object(client, "post") as mock_post, patch.object(
        litelm.module_level_client, "post", return_value=mock_token_response
    ):
        try:
            completion(
                model=model,
...
async def test_watsonx_gpt_oss_uses_async_http_handler():
    ...
    from litelm.litelm_core_utils.prompt_templates.huggingface_template_handler import (
    ...
    with patch(
        "litelm.litelm_core_utils.prompt_templates.huggingface_template_handler.get_async_httpx_client",
...
def test_watsonx_chat_completion_with_reasoning_effort(monkeypatch):
    ...
    # Call litelm.completion with the new parameter
    with patch.object(client, "post") as mock_post, patch.object(
        litelm.module_level_client, "post", return_value=mock_token_response
    ):
        try:
