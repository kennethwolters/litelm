...
See: https://github.com/BerriAI/litelm/issues/16353
"""
...
class TestOpenRouterNativeModelRouting:
    ...
    def test_double_prefixed_strips_once(self, input_model, expected_model):
        ...
        result_model, provider, _, _ = litelm.get_llm_provider(model=input_model)
    ...
    def test_bridge_double_call_preserves_native_model(self, input_model):
        ...
        model_first, provider, _, _ = litelm.get_llm_provider(model=input_model)
        assert provider == "openrouter"
        ...
        model_second, provider2, _, _ = litelm.get_llm_provider(
            model=model_first,
