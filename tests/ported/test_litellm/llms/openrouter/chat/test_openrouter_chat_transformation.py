...
from litelm.llms.openrouter.chat.transformation import (
...
def test_openrouter_extra_body_transformation():
    ...
    # https://github.com/BerriAI/litelm/issues/8425, validate its not contained in extra_body still
...
def test_openrouter_cost_tracking_non_streaming():
    ...
    assert "llm_provider-x-litelm-response-cost" in result._hidden_params["additional_headers"]
    assert result._hidden_params["additional_headers"]["llm_provider-x-litelm-response-cost"] == 0.00015
