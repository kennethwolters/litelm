...
from litelm.llms.vercel_ai_gateway.chat.transformation import VercelAIGatewayConfig
from litelm.cost_calculator import cost_per_token
import math
...
def test_vercel_ai_gateway_in_provider_lists():
    ...
    assert "vercel_ai_gateway" in litelm.openai_compatible_providers
    assert "vercel_ai_gateway" in litelm.provider_list
    assert "https://ai-gateway.vercel.sh/v1" in litelm.openai_compatible_endpoints
...
def test_vercel_ai_gateway_models_endpoint():
    ...
    with patch("litelm.module_level_client.get") as mock_get:
