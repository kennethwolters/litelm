...
Maps to: litelm/llms/vertex_ai/vertex_gemma_models/transformation.py
"""
...
def _reset_litelm_http_client_cache():
    """Ensure each test gets a fresh async HTTP client mock."""
    from litelm import in_memory_llm_clients_cache

    in_memory_llm_clients_cache.flush_cache()


class TestVertexGemmaCompletion:
    """Test completion flow for Vertex AI Gemma models using litelm.acompletion()"""

    @pytest.mark.asyncio
    async def test_acompletion_basic_request(self):
        """
        Test litelm.acompletion() with Vertex AI Gemma model
        ...
        with patch(
            "litelm.llms.custom_httpx.http_handler.AsyncHTTPHandler"
        ) as mock_http_handler, patch(
            "litelm.llms.vertex_ai.gemini.vertex_and_google_ai_studio_gemini.VertexLLM._ensure_access_token",
    ...
    async def test_acompletion_error_handling(self):
        """
        Test litelm.acompletion() error handling when Vertex returns invalid response
        
        Expected: Proper error handling when 'predictions' field is missing
        """
        from litelm.exceptions import APIConnectionError
    ...
    async def test_acompletion_fake_streaming(self):
        ...
        """
        from litelm.llms.base_llm.base_model_iterator import MockResponseIterator
        ...
        with patch(
            ...
            # Call litelm.acompletion() with stream=True
            response = await litelm.acompletion(
                model="vertex_ai/gemma/gemma-3-12b-it-1222199011122",
