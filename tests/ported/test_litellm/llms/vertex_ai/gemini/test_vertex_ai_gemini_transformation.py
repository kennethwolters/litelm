...
def test_extra_body_tags_not_forwarded_to_vertex_ai():
    ...
    It must NOT be forwarded to the Vertex AI request body.
    Documented in litelm_proxy.md: "Send tags by including them in the extra_body parameter"
    """
    messages = [{"role": "user", "content": "test"}]
    optional_params = {
        "extra_body": {
...
def test_metadata_to_labels_vertex_only():
    ...
    result = _transform_request_body(
        ...
        custom_llm_provider="gemini",
        litelm_params=litelm_params.copy(),
        cached_content=None,
    )
    assert "labels" not in result
...
def test_function_response_has_user_role():
    ...
    Fixes: https://github.com/BerriAI/litelm/issues/22003
    Fixes: https://github.com/BerriAI/litelm/issues/20690
    """
    messages = [
