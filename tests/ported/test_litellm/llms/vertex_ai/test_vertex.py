...
import litelm.litelm_core_utils.prompt_templates
import litelm.litelm_core_utils.prompt_templates.factory
...
import litelm
from litelm import get_optional_params
from litelm.llms.vertex_ai.gemini.transformation import _process_gemini_media
from litelm.types.llms.vertex_ai import BlobType
...
def test_logprobs_unit_test():
    from litelm import VertexGeminiConfig
...
def test_process_gemini_media():
    ...
    from litelm.llms.vertex_ai.gemini.transformation import _process_gemini_media
    from litelm.types.llms.vertex_ai import FileDataType
...
def test_vertex_embedding_url(model, expected_url):
    ...
    Relevant issue: https://github.com/BerriAI/litelm/issues/6482

    When a fine-tuned embedding model is used, the URL is different from the standard one.
    """
    from litelm.llms.vertex_ai.common_utils import _get_vertex_url
...
def mock_convert_url_to_base64():
    with patch(
        "litelm.litelm_core_utils.prompt_templates.factory.convert_url_to_base64",
    ) as mock:
...
def test_aaavertex_embeddings_distances(
    ...
    with patch.object(vertex_client, "post", return_value=mock_response), patch.object(
        litelm.main.vertex_multimodal_embedding,
    ...
    with patch.object(
        ...
        text_response = litelm.embedding(
            model="vertex_ai/multimodalembedding@001",
            input=[input_string],
            client=vertex_client,
        )
...
def test_vertex_parallel_tool_calls_false_multiple_tools_error():
    ...
    with pytest.raises(litelm.utils.UnsupportedParamsError) as excinfo:
        get_optional_params(
            model="gemini-1.5-pro",
            custom_llm_provider="vertex_ai",
            tools=tools,
...
from litelm.llms.vertex_ai.gemini.transformation import _transform_request_body
...
def test_system_prompt_only_adds_blank_user_message():
    ...
    Relevant Issue - https://github.com/BerriAI/litelm/issues/13769
    """
