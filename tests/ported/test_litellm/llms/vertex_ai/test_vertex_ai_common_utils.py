...
from litelm.constants import DEFAULT_MAX_RECURSE_DEPTH

sys.path.insert(
    0, os.path.abspath("../../..")
)  # Adds the parent directory to the system path

from litelm.llms.vertex_ai.common_utils import (
    _get_vertex_url,
...
async def test_get_supports_system_message():
    ...
    from litelm.llms.vertex_ai.common_utils import get_supports_system_message
...
def test_build_vertex_schema():
    ...
    from litelm.llms.vertex_ai.common_utils import _build_vertex_schema
...
def test_process_items_with_excessive_nesting():
    ...
    # generate a schema with excessive nesting
    from litelm.llms.vertex_ai.common_utils import process_items
...
def test_get_vertex_url_global_region(stream, expected_endpoint_suffix):
    ...
    # Mock litelm.VertexGeminiConfig.get_model_for_vertex_ai_url to return model as is
    # as we are not testing that part here, just the URL construction
    with patch(
        "litelm.VertexGeminiConfig.get_model_for_vertex_ai_url",
...
def test_vertex_filter_format_uri():
    ...
    from litelm.llms.vertex_ai.common_utils import filter_schema_fields
...
def test_convert_schema_types_type_array_conversion():
    ...
    Relevant issue: https://github.com/BerriAI/litelm/issues/14091
    """
    from litelm.llms.vertex_ai.common_utils import _convert_schema_types
...
def test_fix_enum_empty_strings():
    ...
    """
    from litelm.llms.vertex_ai.common_utils import _fix_enum_empty_strings
...
def test_get_vertex_model_id_from_url():
    ...
    from litelm.llms.vertex_ai.common_utils import get_vertex_model_id_from_url
...
def test_construct_target_url_with_version_prefix():
    ...
    from litelm.llms.vertex_ai.common_utils import construct_target_url
...
def test_fix_enum_types():
    ...
    """
    from litelm.llms.vertex_ai.common_utils import _fix_enum_types
...
async def test_vertex_ai_token_counter_routes_partner_models():
    ...
    from litelm.llms.vertex_ai.common_utils import VertexAITokenCounter
    ...
    with patch(
        "litelm.llms.vertex_ai.vertex_ai_partner_models.main.VertexAIPartnerModels.count_tokens"
...
async def test_vertex_ai_token_counter_routes_gemini_models():
    ...
    with patch(
        "litelm.llms.vertex_ai.count_tokens.handler.VertexAITokenCounter.acount_tokens"
...
async def test_vertex_ai_partner_model_detection():
    ...
    """
    from litelm.llms.vertex_ai.vertex_ai_partner_models.main import (
...
def test_add_object_type_schema_with_no_properties_and_no_type():
    ...
    """
    from litelm.llms.vertex_ai.common_utils import add_object_type
