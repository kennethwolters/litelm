...
from litelm.llms.vertex_ai.files.transformation import VertexAIFilesConfig
from litelm.types.llms.openai import OpenAIFileObject, HttpxBinaryResponseContent
...
class TestParseGcsUri:
    ...
    def test_should_parse_uri_with_nested_publisher_path(self, config):
        uri = "gs://litelm-local/litelm-vertex-files/publishers/google/models/gemini-2.0-flash-001/abc-123"
        bucket, encoded = config._parse_gcs_uri(uri)
        assert bucket == "litelm-local"
        expected_path = "litelm-vertex-files/publishers/google/models/gemini-2.0-flash-001/abc-123"
...
class TestTransformRetrieveFile:
    ...
    def test_should_build_correct_gcs_metadata_url(self, config):
        ...
        url, params = config.transform_retrieve_file_request(
            file_id=file_id, optional_params={}, litelm_params={}
        )
...
class TestTransformDeleteFile:
    ...
    def test_should_return_file_deleted_with_reconstructed_id(self, config):
        ...
        encoded_name = urllib.parse.quote(
            "litelm-vertex-files/publishers/google/models/gemini-2.0-flash-001/abc", safe=""
        )
        mock_request.url = (
        ...
        assert result.object == "file"
        assert result.id == "gs://my-bucket/litelm-vertex-files/publishers/google/models/gemini-2.0-flash-001/abc"
    ...
    def test_should_include_bucket_in_nested_object_path(self, config):
        ...
        encoded_object = urllib.parse.quote(
            "litelm-vertex-files/publishers/google/models/gemini-2.0-flash-001/abc-123",
            safe="",
        )
        mock_request.url = (
        ...
        assert result.id == (
            "gs://prod-bucket/litelm-vertex-files/publishers/google/"
