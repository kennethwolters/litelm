...
from litelm.llms.vertex_ai.rerank.transformation import VertexAIRerankConfig
...
class TestVertexAIRerankTransform:
    ...
    def test_get_complete_url(self):
        ...
        importlib.reload(litelm) in conftest.py.
        """
        ...
        # Test with litelm.vertex_project
        with patch.dict(os.environ, {}, clear=True):
            import litelm

            # Set vertex_project attribute if it doesn't exist
            if not hasattr(litelm, 'vertex_project'):
                litelm.vertex_project = None
            original_project = litelm.vertex_project
            litelm.vertex_project = "litelm-project-456"
            # Reset mock call count
            mock_ensure_access_token.reset_mock()
            mock_ensure_access_token.return_value = ("mock-token", "litelm-project-456")
            try:
                url = self.config.get_complete_url(api_base=None, model=self.model)
                expected_url = "https://discoveryengine.googleapis.com/v1/projects/litelm-project-456/locations/global/rankingConfigs/default_ranking_config:rank"
                assert url == expected_url
            finally:
                litelm.vertex_project = original_project
    ...
    def test_validate_environment_preserves_optional_params_for_get_complete_url(
        ...
        with patch('litelm.llms.vertex_ai.rerank.transformation.get_secret_str', return_value=None):
