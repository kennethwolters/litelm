...
def reload_huggingface_modules():
    """
    Reload modules to ensure fresh references after conftest reloads litelm.
    ...
    """
    import litelm.llms.custom_httpx.http_handler as http_handler_module
    import litelm.llms.huggingface.embedding.handler as hf_embedding_handler_module
...
def mock_embedding_async_http_handler(reload_huggingface_modules):
    ...
    with patch("litelm.llms.custom_httpx.http_handler.AsyncHTTPHandler.post", new_callable=AsyncMock) as mock_post:
        mock_response = MagicMock()
        mock_response.raise_for_status.return_value = None
        mock_response.status_code = 200
...
class TestHuggingFaceEmbedding:
    ...
    def setup(self, mock_embedding_http_handler, mock_embedding_async_http_handler):
        self.mock_get_task_patcher = patch("litelm.llms.huggingface.embedding.handler.get_hf_task_embedding_for_model")
