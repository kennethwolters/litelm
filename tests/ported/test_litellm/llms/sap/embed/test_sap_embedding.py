...
async def test_sap_chat(
    ...
    with patch(
        "litelm.llms.sap.embed.transformation.GenAIHubEmbeddingConfig.deployment_url",
        new_callable=PropertyMock,
        return_value=fake_deployment_url,
    ), patch(
        "litelm.llms.sap.embed.transformation.get_token_creator",
        ...
        if sync_mode:
            response = litelm.embedding(model=model, input=input)
        else:
            response = await litelm.aembedding(model=model, input=input)
