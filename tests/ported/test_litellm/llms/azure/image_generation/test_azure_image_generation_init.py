...
from litelm.llms.azure.image_generation import (
    AzureDallE3ImageGenerationConfig,
    get_azure_image_generation_config,
)
from litelm.utils import get_optional_params_image_gen
...
def test_azure_image_generation_flattens_extra_body():
    ...
    This test verifies the fix for: https://github.com/BerriAI/litelm/issues/16059
...
def test_azure_image_generation_creates_token_provider_from_credentials():
    ...
    azure_ad_token_provider from credentials in litelm_params if it's not already provided.
    """
    ...
    if azure_ad_token_provider is None:
        tenant_id = litelm_params_dict.get("tenant_id")
        client_id = litelm_params_dict.get("client_id")
        client_secret = litelm_params_dict.get("client_secret")
        azure_scope = litelm_params_dict.get("azure_scope") or "https://cognitiveservices.azure.com/.default"
...
def test_azure_image_generation_drop_params_response_format():
    ...
    """
    from litelm.llms.openai.image_generation.gpt_transformation import (
...
def test_azure_image_generation_drop_params_false_raises_error():
    ...
    is not enabled.
    """
    from litelm.exceptions import UnsupportedParamsError
    from litelm.llms.openai.image_generation.gpt_transformation import (
        GPTImageGenerationConfig,
    )
...
def test_azure_image_generation_base_model_vs_deployment_name():
    ...
    When base_model is specified in litelm_params, the request should:
