...
from litelm.llms.azure.responses.o_series_transformation import (
    AzureOpenAIOSeriesResponsesAPIConfig,
)
from litelm.llms.azure.responses.transformation import AzureOpenAIResponsesAPIConfig
from litelm.types.llms.openai import ResponsesAPIOptionalRequestParams
from litelm.types.router import GenericLiteLLMParams


@pytest.mark.serial
def test_validate_environment_api_key_within_litelm_params():
    azure_openai_responses_apiconfig = AzureOpenAIResponsesAPIConfig()
    litelm_params = GenericLiteLLMParams(api_key="test-api-key")

    result = azure_openai_responses_apiconfig.validate_environment(
        headers={}, model="", litelm_params=litelm_params
    )
...
@pytest.mark.serial
def test_validate_environment_api_key_within_litelm():
    azure_openai_responses_apiconfig = AzureOpenAIResponsesAPIConfig()

    with patch("litelm.api_key", "test-api-key"):
...
@pytest.mark.serial
def test_validate_environment_azure_key_within_litelm():
    azure_openai_responses_apiconfig = AzureOpenAIResponsesAPIConfig()

    with patch("litelm.azure_key", "test-azure-key"):
        litelm_params = GenericLiteLLMParams()
        result = azure_openai_responses_apiconfig.validate_environment(
            headers={}, model="", litelm_params=litelm_params
        )
...
def test_validate_environment_azure_key_within_headers():
    ...
    result = azure_openai_responses_apiconfig.validate_environment(
        headers=headers, model="", litelm_params=litelm_params
    )
...
def test_get_complete_url():
    ...
    """
    azure_openai_responses_apiconfig = AzureOpenAIResponsesAPIConfig()
    api_base = "https://litelm8397336933.openai.azure.com"
    litelm_params = {"api_version": "2024-05-01-preview"}

    result = azure_openai_responses_apiconfig.get_complete_url(
        api_base=api_base, litelm_params=litelm_params
    )

    expected = "https://litelm8397336933.openai.azure.com/openai/responses?api-version=2024-05-01-preview"
...
def test_provider_config_manager_o_series_selection():
    ...
    o_series_config = ProviderConfigManager.get_provider_responses_api_config(
        provider=litelm.LlmProviders.AZURE, model="o_series/gpt-o1"
    )
    ...
    regular_config = ProviderConfigManager.get_provider_responses_api_config(
        provider=litelm.LlmProviders.AZURE, model="gpt-4o"
    )
    ...
    default_config = ProviderConfigManager.get_provider_responses_api_config(
        provider=litelm.LlmProviders.AZURE, model=None
    )
...
class TestAzureResponsesAPIConfig:
    ...
    def test_azure_get_complete_url_with_default_api_version(self):
        ...
        from litelm.constants import AZURE_DEFAULT_RESPONSES_API_VERSION
        ...
        expected_url = f"https://litelm8397336933.openai.azure.com/openai/v1/responses?api-version={AZURE_DEFAULT_RESPONSES_API_VERSION}"
        assert result_no_version == expected_url

        # Test with empty litelm_params - should use default
        ...
        result_none_version = self.config.get_complete_url(
            api_base=base_url,
            litelm_params={"api_version": None},
        )
    ...
    def test_azure_cancel_response_api_request(self):
        ...
        litelm_params = GenericLiteLLMParams(api_version="2024-05-01-preview")
    ...
    def test_azure_cancel_response_api_response(self):
        ...
        from litelm.types.llms.openai import ResponsesAPIResponse
