...
from litelm.llms.custom_httpx.http_handler import get_shared_realtime_ssl_context
...
async def test_async_realtime_uses_max_size_parameter():
    ...
    This verifies the fix for: https://github.com/BerriAI/litelm/issues/15747
    """
    from litelm.constants import REALTIME_WEBSOCKET_MAX_MESSAGE_SIZE_BYTES
    from litelm.llms.azure.realtime.handler import AzureOpenAIRealtime
    ...
    with patch("websockets.connect", return_value=DummyAsyncContextManager(mock_backend_ws)) as mock_ws_connect, \
         patch("litelm.llms.azure.realtime.handler.RealTimeStreaming") as mock_realtime_streaming:
...
async def test_realtime_protocol_env_var_fallback():
    ...
    """
    from litelm.realtime_api.main import _arealtime
    ...
    with patch.dict(os.environ, {"LITELLM_AZURE_REALTIME_PROTOCOL": "v1"}):
        ...
        realtime_protocol = (
            ...
            or litelm_params.get("realtime_protocol")
...
@pytest.mark.asyncio
async def test_realtime_protocol_from_litelm_params():
    """
    Test that realtime_protocol is read from litelm_params (config.yaml extra field).
    Fixes #22127: realtime_protocol in litelm_params was not used.
    """
    from litelm.types.router import GenericLiteLLMParams

    # Simulate config.yaml with realtime_protocol as an extra field
    litelm_params = GenericLiteLLMParams(realtime_protocol="GA")
    assert litelm_params.get("realtime_protocol") == "GA"
