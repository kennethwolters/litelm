...
import litelm
from litelm.llms.azure.common_utils import BaseAzureLLM, get_azure_ad_token
from litelm.secret_managers.get_azure_ad_token_provider import (
    get_azure_ad_token_provider,
)
from litelm.types.router import GenericLiteLLMParams
from litelm.types.secret_managers.get_azure_ad_token_provider import (
    AzureCredentialType,
)
...
def setup_mocks(monkeypatch):
    ...
    with patch(
        "litelm.llms.azure.common_utils.get_azure_ad_token_from_entra_id"
    ) as mock_entra_token, patch(
        "litelm.llms.azure.common_utils.get_azure_ad_token_from_username_password"
    ) as mock_username_password_token, patch(
        "litelm.llms.azure.common_utils.get_azure_ad_token_from_oidc"
    ) as mock_oidc_token, patch(
        "litelm.llms.azure.common_utils.get_azure_ad_token_provider"
    ) as mock_token_provider, patch(
        "litelm.llms.azure.common_utils.litelm"
    ) as mock_litelm, patch(
        "litelm.llms.azure.common_utils.verbose_logger"
    ) as mock_logger, patch(
        "litelm.llms.azure.common_utils.select_azure_base_url_or_endpoint"
    ) as mock_select_url:
        # Configure mocks
        mock_litelm.AZURE_DEFAULT_API_VERSION = "2023-05-15"
        mock_litelm.enable_azure_ad_token_refresh = False
...
def test_initialize_with_oidc_token(setup_mocks, monkeypatch):
    ...
    result = BaseAzureLLM().initialize_azure_sdk_client(
        litelm_params={"azure_ad_token": "oidc/test-token"},
        api_key=None,
        api_base="https://test.openai.azure.com",
        model_name="gpt-4",
        api_version=None,
...
def test_initialize_with_enable_token_refresh(setup_mocks, monkeypatch):
    ...
    monkeypatch.delenv("AZURE_CLIENT_SECRET", raising=False)
    monkeypatch.delenv("AZURE_TENANT_ID", raising=False)
    setup_mocks["litelm"].enable_azure_ad_token_refresh = True
...
async def test_ensure_initialize_azure_sdk_client_always_used(call_type):
    ...
    patch_target = (
        "litelm.llms.azure.common_utils.BaseAzureLLM.initialize_azure_sdk_client"
    )
    if call_type == CallTypes.arerank:
        patch_target = (
            "litelm.rerank_api.main.azure_rerank.initialize_azure_sdk_client"
        )
    elif call_type == CallTypes.acreate_batch or call_type == CallTypes.aretrieve_batch or call_type == CallTypes.acancel_batch:
        patch_target = (
            "litelm.batches.main.azure_batches_instance.initialize_azure_sdk_client"
        )
    elif (
        ...
        patch_target = (
            "litelm.assistants.main.azure_assistants_api.initialize_azure_sdk_client"
        )
    elif call_type == CallTypes.acreate_file or call_type == CallTypes.afile_content:
        patch_target = (
            "litelm.files.main.azure_files_instance.initialize_azure_sdk_client"
        )
    elif (
    ...
    with patch(patch_target) as mock_init_azure:
        ...
        litelm_params = azure_calls[0].kwargs["litelm_params"]
        print("litelm_params", litelm_params)

        assert (
            "azure_ad_token" in litelm_params
...
async def test_ensure_initialize_azure_sdk_client_always_used_azure_text(call_type):
    ...
    patch_target = "litelm.main.azure_text_completions.initialize_azure_sdk_client"
...
async def test_azure_client_reuse(function_name, is_async, args):
    ...
    client_path = (
        "litelm.llms.azure.common_utils.AsyncAzureOpenAI"
        if is_async
        else "litelm.llms.azure.common_utils.AzureOpenAI"
    )
        ...
        for _ in range(10):
            try:
                ...
                if is_async:
                    ...
                    func = getattr(litelm, f"a{function_name}")
                    await func(**args)
                else:
                    func = getattr(litelm, function_name)
                    func(**args)
            except Exception:
...
async def test_azure_client_cache_separates_sync_and_async():
    ...
    """
    from litelm.llms.azure.common_utils import BaseAzureLLM

    # Clear the in-memory cache before test
    litelm.in_memory_llm_clients_cache._cache = {}
    ...
    with patch(
        "litelm.llms.azure.common_utils.AzureOpenAI"
...
def test_scope_always_string_in_initialize_azure_sdk_client(setup_mocks, monkeypatch):
    ...
    # Test case 1: scope is None in litelm_params
    result = base_llm.initialize_azure_sdk_client(
        litelm_params={"azure_scope": None},
        api_key="test-api-key",
        api_base="https://test.openai.azure.com",
        model_name="gpt-4",
        api_version="2023-06-01",
    ...
    result = base_llm.initialize_azure_sdk_client(
        litelm_params={"azure_scope": custom_scope},
        api_key="test-api-key",
        api_base="https://test.openai.azure.com",
        model_name="gpt-4",
        api_version="2023-06-01",
...
def test_with_existing_token_provider(setup_mocks):
    ...
    litelm_params = GenericLiteLLMParams(azure_ad_token_provider=token_provider)

    token = get_azure_ad_token(litelm_params)
...
def test_with_existing_azure_ad_token(setup_mocks):
    ...
    litelm_params = GenericLiteLLMParams(azure_ad_token="test-token")
...
def test_with_existing_azure_ad_token_from_env(setup_mocks):
    ...
    with patch("litelm.llms.azure.common_utils.get_secret_str") as mock_get_secret_str:
...
def test_token_provider_returns_non_string(setup_mocks):
    ...
    litelm_params = GenericLiteLLMParams(azure_ad_token_provider=non_string_provider)

    # Call the function and expect a TypeError
    with pytest.raises(TypeError) as excinfo:
        get_azure_ad_token(litelm_params)
...
def test_token_provider_raises_exception(setup_mocks):
    ...
    litelm_params = GenericLiteLLMParams(azure_ad_token_provider=error_provider)
