...
import litelm
from litelm.llms.azure.videos.transformation import AzureVideoConfig
from litelm.types.videos.main import VideoObject, VideoResponse, VideoCreateOptionalRequestParams
...
class TestAzureVideoConfig:
    ...
    @patch('litelm.llms.azure.common_utils.litelm')
    def test_validate_environment_with_api_key(self, mock_litelm):
        """Test environment validation with provided API key - should use api-key header for Azure."""
        # Since validate_environment passes litelm_params=None, it relies on litelm.api_key or litelm.azure_key
        mock_litelm.api_key = self.api_key
        mock_litelm.azure_key = None
    ...
    @patch('litelm.llms.azure.common_utils.get_secret_str')
    @patch('litelm.llms.azure.common_utils.litelm')
    def test_validate_environment_without_api_key(self, mock_litelm, mock_get_secret):
        """Test environment validation without provided API key - should fallback to secret manager."""
        mock_litelm.api_key = None
        mock_litelm.azure_key = None
    ...
    def test_url_construction_with_trailing_slash(self):
        ...
        litelm_params = {"api_base": api_base_with_slash}
    ...
    def test_url_construction_without_trailing_slash(self):
        ...
        litelm_params = {"api_base": api_base_without_slash}
    ...
    @patch('litelm.llms.azure.common_utils.litelm')
    def test_azure_specific_environment_validation(self, mock_litelm):
        """Test Azure-specific environment validation with different key sources."""
        # Test with azure_key
        mock_litelm.api_key = None
        mock_litelm.azure_key = "azure-test-key"
        mock_litelm.openai_key = None
