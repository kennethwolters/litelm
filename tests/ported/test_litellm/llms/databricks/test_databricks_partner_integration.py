...
from litelm.llms.databricks.common_utils import DatabricksBase, DatabricksException
...
class TestBuildUserAgent:
    ...
    def test_default_user_agent(self):
        """No custom user agent returns litelm/{version}."""
        ua = DatabricksBase._build_user_agent(None)
        assert ua.startswith("litelm/")
    ...
    def test_custom_user_agent_with_version(self):
        ...
        assert ua.startswith("mycompany_litelm/")
        # Verify the version is litelm's, not the custom one
        assert "/1.0.0" not in ua or "mycompany_litelm/1.0.0" not in ua
    ...
    def test_custom_user_agent_with_underscore(self):
        ...
        assert ua.startswith("my_company_litelm/")
    ...
    def test_custom_user_agent_with_hyphen(self):
        ...
        assert ua.startswith("my-company_litelm/")

    def test_custom_user_agent_ignores_custom_version(self):
        """Custom version is ignored, litelm version is used."""
        ua = DatabricksBase._build_user_agent("partner/99.99.99")
        parts = ua.split("/")
        assert parts[0] == "partner_litelm"
    ...
    def test_partner_with_numbers(self):
        ...
        assert ua.startswith("company123_litelm/")
...
class TestValidateEnvironmentUserAgent:
    ...
    def test_default_user_agent(self, monkeypatch):
        ...
        assert headers["User-Agent"].startswith("litelm/")
    ...
    def test_custom_user_agent_via_param(self, monkeypatch):
        ...
        assert headers["User-Agent"].startswith("mycompany_litelm/")
...
class TestSDKPartnerTelemetry:
    ...
    def test_sdk_partner_registered(self):
        ...
        with patch.dict(sys.modules, {
            ...
            mock_useragent.with_partner.assert_called_once_with("litelm")
...
class TestUserAgentFromEnvironment:
    ...
    def test_user_agent_from_databricks_env_var(self, monkeypatch):
        ...
        assert headers["User-Agent"].startswith("envpartner_litelm/")
    ...
    def test_custom_param_takes_precedence(self, monkeypatch):
        ...
        assert headers["User-Agent"].startswith("parampartner_litelm/")
...
class TestLiteLLMCompletionUserAgent:
    ...
    def test_completion_passes_user_agent_to_headers(self):
        """litelm.completion() correctly passes user_agent to request headers."""
        ...
        with patch.object(
            ...
            return_value=(
                "https://test.net/serving-endpoints/chat/completions",
                {
                    ...
                    "User-Agent": "testpartner_litelm/1.0.0",
                },
            ),
        ) as mock_validate:
...
class TestLiteLLMEmbeddingUserAgent:
    ...
    def test_embedding_passes_user_agent_to_headers(self):
        """litelm.embedding() correctly passes user_agent to request headers."""
        from litelm.llms.databricks.embed.handler import DatabricksEmbeddingHandler
        ...
        with patch.object(
            ...
            return_value=(
                ...
                {
                    "Authorization": "Bearer test",
                    "User-Agent": "embedpartner_litelm/1.0.0",
                },
            ),
        ) as mock_validate:
            with patch(
                "litelm.llms.openai_like.embedding.handler.OpenAILikeEmbeddingHandler.embedding"
            ):
                try:
                    handler.embedding(
                        model="databricks/test-model",
