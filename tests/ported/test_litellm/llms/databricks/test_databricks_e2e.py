...
LiteLLM Integration Tests:
    ...
    1. LiteLLM SDK Direct - Using litelm.completion() with user_agent parameter
    2. LangChain + LiteLLM - Using ChatLiteLLM wrapper (requires langchain-community)
    3. LiteLLM Async - Using litelm.acompletion() async API
    4. LiteLLM Streaming - Using litelm.completion() with stream=True
    5. LiteLLM Embedding - Using litelm.embedding() with user_agent parameter
...
Optional Dependencies (for LiteLLM integration tests):
    - pip install langchain-litelm  # For LangChain tests (recommended)

Setup:
    1. Copy the template to create your config file:
       cp databricks_config.template.txt ~/.databricks_litelm_config.txt
...
Run with:
    cd /path/to/litelm
    python tests/test_litelm/llms/databricks/test_databricks_e2e.py
...
# Add the litelm package to path
sys.path.insert(
    0, os.path.abspath(os.path.join(os.path.dirname(__file__), "../../../.."))
)

# Config file path - can be overridden with DATABRICKS_TEST_CONFIG env var
DEFAULT_CONFIG_PATH = os.path.expanduser("~/.databricks_litelm_config.txt")
...
def setup_environment(config: dict, auth_method: str):
    ...
    if auth_method == "oauth":
        ...
        # But we still need to pass api_base to litelm, so set it if provided
        if "DATABRICKS_API_BASE" in config:
            os.environ["DATABRICKS_API_BASE"] = config["DATABRICKS_API_BASE"]
...
def test_user_agent_building():
    ...
    from litelm.llms.databricks.common_utils import DatabricksBase

    # Test 1: Default
    ua = DatabricksBase._build_user_agent(None)
    print(f"  Default: {ua}")
    assert ua.startswith("litelm/"), f"Expected litelm/, got {ua}"
    print("  ✓ Default user agent works")

    # Test 2: With partner
    ua = DatabricksBase._build_user_agent("mycompany/1.0.0")
    print(f"  With partner: {ua}")
    assert ua.startswith("mycompany_litelm/"), f"Expected mycompany_litelm/, got {ua}"
    ...
    assert ua.startswith("acme_litelm/"), f"Expected acme_litelm/, got {ua}"
...
def test_chat_completion_default_user_agent(config: dict):
    ...
    saved_litelm_ua = os.environ.pop("LITELLM_USER_AGENT", None)

    try:
        from litelm._version import version
    except Exception:
        version = "unknown"
    ...
    print(f"  Model: {full_model}")
    print(f"  Expected User-Agent: litelm/{version}")
    ...
    try:
        ...
        print(
            f"  Note: Check Databricks Query History to verify User-Agent is 'litelm/{version}'"
        )
        return True
        ...
        if saved_litelm_ua:
            os.environ["LITELLM_USER_AGENT"] = saved_litelm_ua
...
def test_chat_completion_with_custom_user_agent(config: dict):
    ...
    print(f"  Expected User-Agent: testpartner_litelm/{version}")
    ...
    try:
        response = litelm.completion(
            ...
            max_tokens=10,
            user_agent="testpartner/2.0.0",  # This should result in testpartner_litelm/{version}
        )

        print(f"  Response: {response.choices[0].message.content}")
        print("  ✓ Custom user-agent test passed!")
        print(
            f"  Note: Check Databricks Query History to verify User-Agent is 'testpartner_litelm/{version}'"
        )
        return True
...
def test_chat_completion_with_env_user_agent(config: dict):
    ...
    print(f"  Expected User-Agent: {test_partner}_litelm/{version}")
    ...
    try:
        ...
        print(
            f"  Note: Check Databricks Query History to verify User-Agent is '{test_partner}_litelm/{version}'"
        )
        return True
...
def test_litelm_sdk_with_config_user_agent(config: dict):
    """
...
def test_langchain_litelm_with_user_agent(config: dict):
    """
    ...
    Requires: pip install langchain-litelm (recommended)
    ...
    # Try the new langchain-litelm package first, fall back to deprecated import
    ChatLiteLLM = None
    HumanMessage = None

    try:
        from langchain_litelm import ChatLiteLLM
        from langchain_core.messages import HumanMessage

        print("  Using: langchain-litelm package (recommended)")
    except ImportError:
        try:
            ...
            with warnings.catch_warnings():
                ...
                from langchain_core.messages import HumanMessage
            print(
                "  Using: langchain-community (deprecated, consider: pip install langchain-litelm)"
            )
        except ImportError:
            print("  Skipped: langchain-litelm not installed")
            print("  Install with: pip install langchain-litelm")
            return None
...
def test_litelm_async_completion(config: dict):
    """
...
def test_litelm_streaming_completion(config: dict):
    """
...
def test_litelm_embedding_with_user_agent(config: dict):
    """
...
def main():
    ...
    all_results.append(
        (
            ...
            test_litelm_sdk_with_config_user_agent(config),
        )
    )
    ...
    all_results.append(
        (
            ...
            test_langchain_litelm_with_user_agent(config),
        )
    )
    ...
    all_results.append(
        (
            ...
            test_litelm_async_completion(config),
        )
    )
    ...
    all_results.append(
        (
            ...
            test_litelm_streaming_completion(config),
        )
    )
    ...
    all_results.append(
        (
            ...
            test_litelm_embedding_with_user_agent(config),
        )
    )
