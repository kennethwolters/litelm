...
import litelm
from litelm.llms.bedrock_mantle.chat.transformation import BedrockMantleChatConfig
...
class TestBedrockMantleProviderRegistration:
    ...
    def test_provider_in_provider_list(self):
        assert "bedrock_mantle" in litelm.provider_list

    def test_models_loaded(self, monkeypatch):
        monkeypatch.setenv("LITELLM_LOCAL_MODEL_COST_MAP", "true")
        litelm.add_known_models()
        assert len(litelm.bedrock_mantle_models) > 0
        assert "bedrock_mantle/openai.gpt-oss-120b" in litelm.bedrock_mantle_models
        assert "bedrock_mantle/openai.gpt-oss-20b" in litelm.bedrock_mantle_models
        assert (
            "bedrock_mantle/openai.gpt-oss-safeguard-120b" in litelm.bedrock_mantle_models
        )
        assert (
            "bedrock_mantle/openai.gpt-oss-safeguard-20b" in litelm.bedrock_mantle_models
        )
...
class TestBedrockMantleProviderResolution:
    def test_get_llm_provider_resolves_correctly(self):
        model, provider, _, _ = litelm.get_llm_provider(
...
class TestBedrockMantlePricing:
    ...
    def test_gpt_oss_120b_pricing(self, monkeypatch):
        monkeypatch.setenv("LITELLM_LOCAL_MODEL_COST_MAP", "true")
        litelm.add_known_models()
        info = litelm.get_model_info("bedrock_mantle/openai.gpt-oss-120b")
    ...
    def test_gpt_oss_20b_pricing(self, monkeypatch):
        monkeypatch.setenv("LITELLM_LOCAL_MODEL_COST_MAP", "true")
        litelm.add_known_models()
        info = litelm.get_model_info("bedrock_mantle/openai.gpt-oss-20b")
    ...
    def test_pricing_significantly_cheaper_than_openai_native(self, monkeypatch):
        ...
        monkeypatch.setenv("LITELLM_LOCAL_MODEL_COST_MAP", "true")
        litelm.add_known_models()
        bedrock_info = litelm.get_model_info("bedrock_mantle/openai.gpt-oss-120b")
    ...
    def test_safeguard_models_have_larger_output_tokens(self, monkeypatch):
        monkeypatch.setenv("LITELLM_LOCAL_MODEL_COST_MAP", "true")
        litelm.add_known_models()
        info_120b = litelm.get_model_info("bedrock_mantle/openai.gpt-oss-120b")
        info_safeguard = litelm.get_model_info(
