...
This test file mirrors litelm/llms/pg_vector/vector_stores/transformation.py
...
from litelm.llms.pg_vector.vector_stores.transformation import PGVectorStoreConfig
...
class TestPGVectorStoreConfig:
    ...
    def test_validate_environment_with_api_key_in_params(self):
        """
        Test that validate_environment works when api_key is provided in litelm_params.
        
        This test validates that API key from params is correctly set in headers.
        """
        config = PGVectorStoreConfig()
        litelm_params = GenericLiteLLMParams(api_key="test_pg_vector_key_123")
        headers = {}
        
        result_headers = config.validate_environment(headers, litelm_params)
    ...
    def test_validate_environment_missing_api_key(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            config.validate_environment(headers, litelm_params)
    ...
    def test_get_complete_url_with_api_base(self):
        ...
        result_url = config.get_complete_url(api_base, litelm_params)
    ...
    def test_get_complete_url_missing_api_base(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            config.get_complete_url(None, litelm_params)
    ...
    def test_inheritance_from_openai_config(self):
        ...
        """
        from litelm.llms.openai.vector_stores.transformation import (
    ...
    def test_config_methods_with_mock_data(self):
        ...
        litelm_params = GenericLiteLLMParams(api_key="test_key")
        headers = config.validate_environment({}, litelm_params)
    ...
    def test_environment_variable_support(self):
        ...
        with patch.dict(os.environ, {'PG_VECTOR_API_KEY': 'env_api_key_123'}):
            litelm_params = GenericLiteLLMParams()  # No API key in params
        ...
        with patch.dict(os.environ, {'PG_VECTOR_API_KEY': 'env_key'}):
            litelm_params = GenericLiteLLMParams(api_key="param_key")
    ...
    @patch('litelm.llms.custom_httpx.http_handler.HTTPHandler.post')
    def test_pg_vector_search_request_construction(self, mock_post):
        ...
        try:
            ...
            # Call litelm vector store search
            exception_raised = None
            response = None
            try:
                response = litelm.vector_stores.search(
                    query=query,
