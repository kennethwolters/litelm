...
from litelm import supports_reasoning
...
def test_get_models_url_no_double_v1(api_base, expected_url_prefix):
    ...
    with (
        patch("litelm.module_level_client.get", return_value=mock_response) as mock_get,
        patch(
            "litelm.llms.fireworks_ai.chat.transformation.get_secret_str",
...
def test_transform_messages_helper_removes_provider_specific_fields():
    ...
    # Call helper
    out = config._transform_messages_helper(messages, model="fireworks/test", litelm_params={})
    for msg in out:
