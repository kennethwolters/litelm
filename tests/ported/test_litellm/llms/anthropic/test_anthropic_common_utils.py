...
class TestOptionallyHandleAnthropicOAuth:
    ...
    def test_oauth_token_in_authorization_header(self):
        ...
        from litelm.llms.anthropic.common_utils import (
...
class TestPassthroughOAuth:
    ...
    def test_passthrough_oauth_no_x_api_key(self):
        ...
        from litelm.llms.anthropic.experimental_pass_through.messages.transformation import (
...
class TestIsAnthropicOAuthKey:
    ...
    def test_oauth_token_raw(self):
        ...
        from litelm.llms.anthropic.common_utils import is_anthropic_oauth_key
...
class TestProxyOAuthHeaderForwarding:
    ...
    def test_clean_headers_preserves_oauth_authorization(self):
        ...
        from litelm.proxy.litelm_pre_call_utils import clean_headers
    ...
    def test_clean_headers_forwards_x_api_key_when_authenticated_with_litelm_key(self):
        """clean_headers should forward x-api-key when user authenticated with x-litelm-api-key and forward_llm_provider_auth_headers=True."""
        from starlette.datastructures import Headers
        ...
        raw_headers = Headers(
            raw=[
                (b"x-litelm-api-key", b"sk-litelm-proxy-key"),
        ...
        cleaned = clean_headers(
            ...
            authenticated_with_header="x-litelm-api-key",
        )

        # x-api-key should be forwarded (it's a provider key, not used for auth)
        assert "x-api-key" in cleaned
        assert cleaned["x-api-key"] == "sk-ant-api03-client-key"
        # x-litelm-api-key should be excluded (special header)
        assert "x-litelm-api-key" not in cleaned
        assert cleaned["content-type"] == "application/json"
    ...
    def test_clean_headers_excludes_x_api_key_when_used_for_auth(self):
        ...
        raw_headers = Headers(
            raw=[
                (b"x-api-key", b"sk-litelm-proxy-key"),
                (b"content-type", b"application/json"),
            ]
        )
    ...
    def test_clean_headers_forwards_x_api_key_when_authenticated_with_authorization(
        ...
        raw_headers = Headers(
            raw=[
                (b"authorization", b"Bearer sk-litelm-proxy-key"),
                (b"x-api-key", b"sk-ant-api03-client-key"),
                (b"content-type", b"application/json"),
            ]
        )
    ...
    def test_clean_headers_forwards_x_api_key_with_forward_flag_and_litelm_auth(
        self,
    ):
    ...
    def test_clean_headers_authorization_not_forwarded_when_used_for_litelm_auth(
        self,
    ):
        ...
        raw_headers = Headers(
            raw=[
                (b"authorization", b"Bearer sk-1234-litelm-proxy-key"),
    ...
    def test_clean_headers_oauth_authorization_forwarded_when_not_used_for_litelm_auth(
        self,
    ):
        """OAuth Authorization header IS forwarded when x-litelm-api-key was used for proxy auth."""
        ...
        # x-litelm-api-key was used for LiteLLM auth; Authorization carries the Anthropic OAuth token
        with patch(
            "litelm.llms.anthropic.common_utils.is_anthropic_oauth_key",
            return_value=True,
        ):
            cleaned = clean_headers(
                raw_headers,
