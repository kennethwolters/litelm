...
from litelm.types.utils import Delta, ModelResponse, StreamingChoices


def test_anthropic_experimental_pass_through_messages_handler():
    """
    Test that api key is passed to litelm.responses for OpenAI models.
    ...
    with patch("litelm.responses", return_value="test-response") as mock_responses:
        try:
            anthropic_messages_handler(
                max_tokens=100,
...
def test_anthropic_experimental_pass_through_messages_handler_dynamic_api_key_and_api_base_and_custom_values():
    """
    Test that api key, api base, and extra kwargs are forwarded to litelm.completion for Azure models.
    ...
    with patch("litelm.completion", return_value=MagicMock()) as mock_completion:
        try:
            anthropic_messages_handler(
                max_tokens=100,
                messages=[{"role": "user", "content": "Hello, how are you?"}],
...
def test_anthropic_experimental_pass_through_messages_handler_custom_llm_provider():
    """
    Test that litelm.completion is called when a custom LLM provider is given
    """
    from litelm.llms.anthropic.experimental_pass_through.messages.handler import (
        anthropic_messages_handler,
    )

    with patch("litelm.completion", return_value="test-response") as mock_completion:
        try:
            anthropic_messages_handler(
                max_tokens=100,
                messages=[{"role": "user", "content": "Hello, how are you?"}],
        ...
        # Assert that litelm.completion was called when using a custom LLM provider
        mock_completion.assert_called_once()
...
async def test_bedrock_converse_budget_tokens_preserved():
    ...
    """
    # Mock litelm.acompletion which is called internally by anthropic_messages_handler
...
def test_openai_model_with_thinking_converts_to_reasoning():
    ...
    litelm.responses() is called with `reasoning` properly set.
    """
    from litelm.llms.anthropic.experimental_pass_through.messages.handler import (
        anthropic_messages_handler,
    )
    ...
    with patch("litelm.responses", return_value="test-response") as mock_responses:
        ...
        assert "reasoning" in call_kwargs, "reasoning should be passed to litelm.responses"
        ...
        assert "thinking" not in call_kwargs, "thinking should NOT be passed directly to litelm.responses"
...
class TestThinkingParameterTransformation:
    ...
    def test_claude_model_preserves_thinking_with_budget_tokens(self):
        ...
        from litelm.llms.anthropic.experimental_pass_through.adapters.transformation import (
