...
from litelm.llms.anthropic.chat.guardrail_translation.handler import (
...
class TestAnthropicMessagesHandlerStreamingOutputProcessing:
    ...
    async def test_process_output_streaming_response_empty_model_response(self):
        ...
        with patch.object(
            ...
            "litelm.llms.anthropic.chat.guardrail_translation.handler.AnthropicPassthroughLoggingHandler._build_complete_streaming_response",
            return_value=None,
        ):
...
class TestAnthropicMessagesHandlerInputProcessing:
    """Test input processing preserves litelm_metadata for dynamic guardrails."""

    @pytest.mark.asyncio
    async def test_process_input_messages_preserves_litelm_metadata_guardrails(self):
        handler = AnthropicMessagesHandler()
        ...
        assert data.get("litelm_metadata", {}).get("guardrails")
