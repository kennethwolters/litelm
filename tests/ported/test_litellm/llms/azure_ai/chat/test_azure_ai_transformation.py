...
from litelm.llms.azure_ai.azure_model_router.transformation import (
    AzureModelRouterConfig,
)
from litelm.llms.azure_ai.chat.transformation import AzureAIStudioConfig
...
def test_azure_ai_validate_environment_with_azure_ad_token():
    ...
    Regression test for https://github.com/BerriAI/litelm/issues/20759
    """
    import litelm

    config = AzureAIStudioConfig()
    with patch(
        "litelm.llms.azure.common_utils.get_azure_ad_token",
        return_value="fake-azure-ad-token",
    ), patch(
        "litelm.llms.azure.common_utils.get_secret_str",
        return_value=None,
    ), patch.object(litelm, "api_key", None), patch.object(
        litelm, "azure_key", None
    ):
        headers = config.validate_environment(
            headers={},
            model="Kimi-K2.5",
...
def test_azure_model_router_response_shows_actual_model():
    ...
    from litelm.llms.base_llm.chat.transformation import LiteLLMLoggingObj
    ...
    result = config.transform_response(
        ...
        optional_params={},
        litelm_params={"model": "azure_ai/model-router"},  # Original request model
        encoding=None,
        api_key="test-key",
        json_mode=False,
    )
