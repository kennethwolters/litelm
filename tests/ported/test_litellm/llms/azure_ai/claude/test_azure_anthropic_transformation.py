...
from litelm.llms.azure_ai.anthropic.transformation import AzureAnthropicConfig
...
class TestAzureAnthropicConfig:
    ...
    def test_validate_environment_with_dict_litelm_params(self):
        """Test validate_environment with dict litelm_params"""
        config = AzureAnthropicConfig()
        headers = {}
        model = "claude-sonnet-4-5"
        messages = [{"role": "user", "content": "Hello"}]
        optional_params = {}
        litelm_params = {"api_key": "test-api-key"}
        api_key = "test-api-key"

        with patch(
            "litelm.llms.azure.common_utils.BaseAzureLLM._base_validate_azure_environment"
        ) as mock_validate:
            ...
            assert isinstance(call_args[1]["litelm_params"], GenericLiteLLMParams)
            assert call_args[1]["litelm_params"].api_key == "test-api-key"
            assert "anthropic-version" in result

    def test_validate_environment_with_generic_litelm_params(self):
    ...
    def test_validate_environment_sets_api_key_in_litelm_params(self):
        """Test that api_key parameter is set in litelm_params if provided"""
        config = AzureAnthropicConfig()
        headers = {}
        model = "claude-sonnet-4-5"
        messages = [{"role": "user", "content": "Hello"}]
        optional_params = {}
        litelm_params = {}  # Empty dict, no api_key
        ...
        with patch(
            ...
            # Verify that api_key was set in litelm_params
            call_args = mock_validate.call_args
            assert call_args[1]["litelm_params"].api_key == "provided-api-key"
