...
from litelm.llms.azure_ai.anthropic.count_tokens.transformation import (
...
class TestAzureAIAnthropicCountTokensConfig:
    ...
    def test_get_required_headers_with_litelm_params(self):
        """
        Test that get_required_headers works with litelm_params.
        """
        config = AzureAIAnthropicCountTokensConfig()
        api_key = "test-key"
        litelm_params = {"api_key": "param-key", "custom_field": "value"}

        headers = config.get_required_headers(
            api_key=api_key, litelm_params=litelm_params
        )

        # x-api-key should use the direct api_key parameter
        assert headers["x-api-key"] == api_key
        # Azure api-key should come from litelm_params
