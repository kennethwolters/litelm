...
import litelm
from litelm.cost_calculator import (
    completion_cost,
    handle_realtime_stream_cost_calculation,
    response_cost_calculator,
)
from litelm.types.llms.openai import OpenAIRealtimeStreamList
from litelm.types.utils import ModelResponse, PromptTokensDetailsWrapper, Usage
from litelm.utils import TranscriptionResponse
...
def test_cost_calculator_with_response_cost_in_additional_headers():
    class MockResponse(BaseModel):
        _hidden_params = {
            "additional_headers": {"llm_provider-x-litelm-response-cost": 1000}
        }
...
def test_cost_calculator_with_usage(monkeypatch):
    ...
    model_info = litelm.model_cost["gemini-2.0-flash-001"]
    ...
    # Invalidate caches after modifying litelm.model_cost
...
def test_transcription_cost_uses_token_pricing():
    from litelm import completion_cost
...
def test_handle_realtime_stream_cost_calculation():
    from litelm.cost_calculator import RealtimeAPITokenUsageProcessor
...
def test_azure_realtime_cost_calculator():
    ...
    cost = handle_realtime_stream_cost_calculation(
        ...
        litelm_model_name="my-custom-azure-deployment",
    )
...
def test_azure_audio_output_cost_calculation():
    ...
    Reproduces issue: https://github.com/BerriAI/litelm/issues/19764
    ...
    model_info = litelm.get_model_info("azure/gpt-audio-2025-08-28")
...
def test_default_image_cost_calculator(monkeypatch):
    from litelm.cost_calculator import default_image_cost_calculator

    temp_object = {
        "litelm_provider": "azure",
...
def test_cost_calculator_with_cache_creation():
    ...
    litelm_model_response = ModelResponse(
    ...
    assert litelm_model_response.usage.prompt_tokens_details.cached_tokens == 28491

    result = completion_cost(
        completion_response=litelm_model_response,
        model=model,
        custom_llm_provider="vertex_ai",
    )
...
def test_bedrock_cost_calculator_comparison_with_without_cache():
    ...
    from litelm import completion_cost
    from litelm.types.utils import Choices, Message, Usage
...
def test_gemini_25_explicit_caching_cost_direct_usage():
    ...
    a 75% discount.
    """
    from litelm.litelm_core_utils.llm_cost_calc.utils import generic_cost_per_token
    from litelm.types.utils import (
        CompletionTokensDetailsWrapper,
        PromptTokensDetailsWrapper,
        Usage,
...
def test_azure_ai_cache_cost_calculation():
    ...
    model_info = litelm.model_cost[test_model_id]
    expected_input_cost = (
...
def test_cost_discount_vertex_ai():
    ...
    original_discount_config = litelm.cost_discount_config.copy()
    ...
    litelm.cost_discount_config = {"vertex_ai": 0.05}
    ...
    litelm.cost_discount_config = original_discount_config
...
def test_cost_margin_percentage():
    ...
    # Save original config
    original_margin_config = litelm.cost_margin_config.copy()
    ...
    litelm.cost_margin_config = {"openai": 0.10}
    ...
    # Restore original config
    litelm.cost_margin_config = original_margin_config
...
def test_cost_margin_fixed_amount():
    ...
    litelm.cost_margin_config = {"openai": {"fixed_amount": 0.001}}
...
def test_cost_margin_global():
    ...
    litelm.cost_margin_config = {"global": 0.05}
...
def test_cost_margin_provider_overrides_global():
    ...
    litelm.cost_margin_config = {"global": 0.05, "openai": 0.10}
...
def test_cost_margin_with_discount():
    ...
    litelm.cost_discount_config = {"openai": 0.05}
    litelm.cost_margin_config = {"openai": 0.10}
...
def test_gemini_implicit_caching_cost_calculation():
    ...
    See: https://github.com/BerriAI/litelm/issues/16341
    """
    from litelm import completion_cost
    from litelm.llms.vertex_ai.gemini.vertex_and_google_ai_studio_gemini import (
        VertexGeminiConfig,
    )
    from litelm.types.utils import Choices, Message, ModelResponse
    ...
    model_info = litelm.get_model_info("gemini/gemini-2.0-flash")
...
def test_additional_costs_only_for_azure_ai():
    ...
    """
    from litelm.cost_calculator import _get_additional_costs
