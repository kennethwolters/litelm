...
def test_get_silent_experiment_kwargs():
    ...
    assert "litelm_call_id" not in result
...
async def test_router_silent_experiment_acompletion():
    ...
    mock_response = litelm.ModelResponse(choices=[{"message": {"content": "hello"}}])
    mock_acompletion = AsyncMock(return_value=mock_response)

    # Patch at the litelm.router module level where it's imported and used
    with patch.object(litelm, "acompletion", mock_acompletion):
        ...
        # Verify no silent_model in any call to litelm.acompletion
...
def test_router_silent_experiment_completion():
    ...
    # Patch at the litelm module level
    with patch.object(litelm, "acompletion", mock_acompletion_mock), patch.object(
        litelm, "completion", mock_completion_mock
    ):
