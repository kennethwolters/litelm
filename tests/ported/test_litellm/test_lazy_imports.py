...
import litelm
from litelm._lazy_imports import (
...
def _clear_names_from_globals(names: tuple):
    """Clear all names from litelm globals."""
    # Get the actual globals dict, not a copy
    litelm_globals = sys.modules["litelm"].__dict__
    for name in names:
        if name in litelm_globals:
            del litelm_globals[name]


def _clear_names_from_utils_globals(names: tuple):
    """Clear all names from litelm.utils globals."""
    # Get the actual globals dict, not a copy
    utils_globals = sys.modules["litelm.utils"].__dict__
    for name in names:
...
def _verify_only_requested_name_imported(name: str, all_names: tuple):
    ...
    for other_name in all_names:
        if other_name != name:
            assert other_name not in litelm_globals, f"{other_name} should not be imported when importing {name}"
...
def test_litelm_logging_lazy_imports():
    """Test that all litelm_logging items can be lazy imported."""
    # Get the actual globals dict, not a copy
    litelm_globals = sys.modules["litelm"].__dict__
    ...
    for name in LITELLM_LOGGING_NAMES:
        ...
        item = _lazy_import_litelm_logging(name)
...
def test_unknown_attribute_raises_error():
    ...
    with pytest.raises(AttributeError):
        _lazy_import_litelm_logging("unknown")
