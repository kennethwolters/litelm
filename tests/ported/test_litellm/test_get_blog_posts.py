...
import litelm
from litelm.litelm_core_utils.get_blog_posts import (
    BlogPost,
    BlogPostsResponse,
    GetBlogPosts,
    get_blog_posts,
    ...
    "posts": [
        {
            ...
            "date": "2026-01-01",
            "url": "https://www.litelm.ai/blog/test",
        }
    ]
}
...
def test_get_blog_posts_success():
    ...
    with patch("litelm.litelm_core_utils.get_blog_posts.httpx.get", return_value=mock_response):
        posts = get_blog_posts(url=litelm.blog_posts_url)
...
def test_get_blog_posts_network_error_falls_back_to_local():
    ...
    with patch(
        "litelm.litelm_core_utils.get_blog_posts.httpx.get",
...
def test_get_blog_posts_ttl_cache_not_refetched():
    ...
    with patch("litelm.litelm_core_utils.get_blog_posts.httpx.get", side_effect=mock_get):
        posts = get_blog_posts(url=litelm.blog_posts_url)
...
def test_get_blog_posts_ttl_expired_refetches():
    ...
    with patch(
        "litelm.litelm_core_utils.get_blog_posts.httpx.get", return_value=mock_response
    ) as mock_get:
        posts = get_blog_posts(url=litelm.blog_posts_url)
...
def test_get_blog_posts_local_env_var_skips_remote(monkeypatch):
    ...
    with patch("litelm.litelm_core_utils.get_blog_posts.httpx.get") as mock_get:
        posts = get_blog_posts(url=litelm.blog_posts_url)
