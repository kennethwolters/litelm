...
class TestExceptionMessageFormatting:
    ...
    def test_bad_request_error_message_format(self):
        """BadRequestError should format message with litelm prefix."""
        error = BadRequestError(
            message="test error",
            model="gpt-4",
            llm_provider="azure",
        ...
        assert "litelm.BadRequestError" in error.message
    ...
    def test_content_policy_violation_error_message_format(self):
        ...
        assert "litelm.ContentPolicyViolationError" in error.message
        assert "test error" in error.message
    ...
    def test_context_window_exceeded_error_message_format(self):
        ...
        assert "litelm.ContextWindowExceededError" in error.message
        assert "test error" in error.message
...
class TestExceptionAttributes:
    ...
    def test_bad_request_error_attributes(self):
        ...
        assert error.litelm_debug_info == "debug info"
    ...
    def test_midstream_fallback_error_status_code_propagation(self):
        ...
        assert midstream_error.message == "litelm.MidStreamFallbackError: stream broke"
        assert midstream_error.args == ("litelm.MidStreamFallbackError: stream broke",)
...
class TestProxyHeaderExtraction:
    ...
    def test_get_response_headers_adds_llm_provider_prefix(self):
        ...
        from litelm.litelm_core_utils.llm_response_utils.get_headers import (
