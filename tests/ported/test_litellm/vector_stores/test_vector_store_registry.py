...
from litelm.vector_stores.main import search
...
def test_get_credentials_for_vector_store():
    ...
    vector_store_1 = LiteLLM_ManagedVectorStore(
        ...
        litelm_credential_name="test_creds_1",
    ...
    vector_store_2 = LiteLLM_ManagedVectorStore(
        ...
        litelm_credential_name="test_creds_2",
        created_at=datetime.now(timezone.utc),
        updated_at=datetime.now(timezone.utc),
    )
    ...
    with patch(
        "litelm.litelm_core_utils.credential_accessor.CredentialAccessor.get_credential_values"
    ) as mock_get_creds:
...
def test_search_uses_registry_credentials():
    ...
    import litelm.vector_stores.main as vector_stores_main
    ...
    try:
        ...
        with patch.object(
            ...
            "litelm.vector_stores.main.ProviderConfigManager.get_provider_vector_stores_config",
            ...
            search(vector_store_id="vs1", query="test", litelm_logging_obj=logger)
            mock_get_creds.assert_called_once_with("vs1")
            called_params = mock_handler.call_args.kwargs["litelm_params"]
