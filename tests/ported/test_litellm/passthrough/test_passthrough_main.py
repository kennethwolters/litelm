...
import litelm
from litelm.passthrough.main import allm_passthrough_route, llm_passthrough_route
...
def test_bedrock_application_inference_profile_url_encoding():
    ...
    with patch(
        "litelm.utils.ProviderConfigManager.get_provider_passthrough_config",
        return_value=mock_provider_config,
    ), patch(
        "litelm.litelm_core_utils.get_litelm_params.get_litelm_params",
        return_value={},
    ), patch(
        "litelm.litelm_core_utils.get_llm_provider_logic.get_llm_provider",
...
async def test_pass_through_request_stream_param_override(
    ...
    with patch(
        ...
        "litelm.proxy.proxy_server.proxy_logging_obj.pre_call_hook",
