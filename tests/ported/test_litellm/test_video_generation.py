...
import litelm
from litelm.cost_calculator import default_video_cost_calculator
...
from litelm.llms.custom_httpx.llm_http_handler import BaseLLMHTTPHandler
from litelm.llms.gemini.videos.transformation import GeminiVideoConfig
from litelm.llms.openai.videos.transformation import OpenAIVideoConfig
from litelm.types.videos.main import VideoObject, VideoResponse
from litelm.videos import main as videos_main
from litelm.videos.main import (
    avideo_generation,
    avideo_status,
    video_generation,
    video_status,
...
class TestVideoGeneration:
    ...
    def test_video_generation_cost_with_custom_model_info(self):
        ...
        instead of looking up the global litelm.model_cost map.

        Related: https://github.com/BerriAI/litelm/issues/21907
        """
        model_info = {
    ...
    def test_video_generation_cost_custom_pricing_through_completion_cost(self):
        """Test that custom video pricing flows through completion_cost via litelm_logging_obj.

        This tests the full cost calculation path: completion_cost extracts model_info
        from litelm_logging_obj.litelm_params.metadata.model_info and passes it to
        the video cost calculator.

        Related: https://github.com/BerriAI/litelm/issues/21907
        """
        from litelm.cost_calculator import completion_cost
        ...
        # Create mock litelm_logging_obj with custom pricing
        mock_logging_obj = MagicMock()
        mock_logging_obj.litelm_params = {
            "metadata": {
                "model_info": {
                    "output_cost_per_video_per_second": 0.05,
                }
    ...
    def test_video_generation_uses_api_key_from_litelm_params(self):
        """Test that video generation handler uses api_key from litelm_params when function parameter is None."""
        ...
        with patch.object(config, 'validate_environment') as mock_validate:
            ...
            with patch.object(config, 'transform_video_create_request') as mock_transform:
                ...
                with patch.object(config, 'transform_video_create_response') as mock_transform_response:
                    ...
                    with patch(
                        "litelm.llms.custom_httpx.llm_http_handler._get_httpx_client",
                        ...
                        result = handler.video_generation_handler(
                            ...
                            custom_llm_provider="openai",
                            litelm_params={"api_key": "deployment-api-key", "api_base": "https://api.openai.com/v1"},
                            logging_obj=MagicMock(),
                            timeout=5.0,
                    ...
                    # Verify validate_environment was called with api_key from litelm_params
    ...
    def test_video_generation_unsupported_parameters(self):
        ...
        from litelm.videos.utils import VideoGenerationRequestUtils
...
class TestVideoLogging:
    ...
    async def test_video_generation_logging(self):
        ...
        litelm.logging_callback_manager._reset_all_callbacks()
        litelm.callbacks = [custom_logger]
        ...
        with patch.object(videos_main.base_llm_http_handler, 'async_video_generation_handler', side_effect=mock_async_handler):
            response = await litelm.avideo_generation(
...
def test_video_content_handler_passes_variant_to_url():
    ...
    with patch(
        ...
        result = handler.video_content_handler(
            ...
            custom_llm_provider="openai",
            litelm_params=GenericLiteLLMParams(
                api_base="https://api.openai.com/v1"
            ),
            logging_obj=MagicMock(),
            timeout=5.0,
...
def test_video_content_handler_uses_get_for_openai():
    ...
    with patch('litelm.llms.custom_httpx.llm_http_handler._get_httpx_client') as mock_get_client:
        ...
        result = handler.video_content_handler(
            ...
            video_content_provider_config=config,
            custom_llm_provider="openai",
            litelm_params=GenericLiteLLMParams(api_base="https://api.openai.com/v1"),
            logging_obj=MagicMock(),
            timeout=5.0,
            api_key="sk-test",
            client=mock_client,
...
def test_video_content_respects_api_base_and_api_key_from_kwargs():
    ...
    from litelm.videos.main import video_content

    # Mock the handler to capture litelm_params
    captured_litelm_params = None
    
    def capture_litelm_params(*args, **kwargs):
        nonlocal captured_litelm_params
        captured_litelm_params = kwargs.get("litelm_params")
        return b"mp4-bytes"
    
    with patch('litelm.videos.main.base_llm_http_handler') as mock_handler:
        mock_handler.video_content_handler = capture_litelm_params
    ...
    # Verify that api_base and api_key from kwargs were included in litelm_params
    assert captured_litelm_params is not None
    assert captured_litelm_params.get("api_base") == "https://test-resource.openai.azure.com/"
    assert captured_litelm_params.get("api_key") == "test-api-key-from-db"
    assert result == b"mp4-bytes"
...
def test_encode_video_id_with_provider_handles_azure_video_prefix():
    ...
    """
    from litelm.types.videos.utils import (
...
class TestVideoListTransformation:
    ...
    def test_transform_video_list_response_encodes_first_id_and_last_id(self):
        ...
        from litelm.types.videos.utils import decode_video_id_with_provider
    ...
    def test_transform_video_list_request_decodes_after_parameter(self):
        ...
        from litelm.types.videos.utils import encode_video_id_with_provider
...
class TestVideoEndpointsProxyLitellmParams:
    """Test that video proxy endpoints (status, content, remix) respect litelm_params from proxy config."""

    @pytest.fixture
    def client_with_vertex_config(self, monkeypatch):
        """Create a test client with a proxy config that includes Vertex AI model with litelm_params."""
        import asyncio
        import tempfile
        ...
        from litelm.proxy.video_endpoints.endpoints import router as video_router
    ...
    @pytest.mark.asyncio
    async def test_video_status_respects_litelm_params(
        self, client_with_vertex_config, mock_video_generation_response, mock_video_status_response
    ):
        """Test that video_status endpoint uses litelm_params from proxy config."""
        from unittest.mock import AsyncMock, MagicMock, patch
        ...
        with patch("litelm.proxy.proxy_server.llm_router", mock_router_instance):
            with patch("litelm.proxy.common_request_processing.route_request", side_effect=create_mock_coroutine) as mock_route_request:
    ...
    @pytest.mark.asyncio
    async def test_video_content_respects_litelm_params(
        self, client_with_vertex_config, mock_video_generation_response, mock_video_content_response
    ):
        """Test that video_content endpoint uses litelm_params from proxy config."""
        from unittest.mock import AsyncMock, MagicMock, patch
...
def test_video_remix_handler_uses_api_key_from_litelm_params():
    """Sync remix handler should fall back to litelm_params api_key when api_key param is None."""
    handler = BaseLLMHTTPHandler()
    config = OpenAIVideoConfig()
            ...
            with patch.object(config, "transform_video_remix_response") as mock_resp:
                ...
                with patch(
                    ...
                    handler.video_remix_handler(
                        ...
                        custom_llm_provider="openai",
                        litelm_params={"api_key": "deployment-key", "api_base": "https://api.openai.com/v1"},
                        logging_obj=MagicMock(),
                        timeout=5.0,
                        api_key=None,
                        _is_async=False,
...
@pytest.mark.asyncio
async def test_async_video_remix_handler_uses_api_key_from_litelm_params():
    """Async remix handler should fall back to litelm_params api_key when api_key param is None."""
    handler = BaseLLMHTTPHandler()
    config = OpenAIVideoConfig()
    ...
    with patch.object(config, "validate_environment") as mock_validate:
        ...
        with patch.object(config, "transform_video_remix_request") as mock_transform:
            ...
            with patch.object(config, "transform_video_remix_response") as mock_resp:
                ...
                with patch(
                    "litelm.llms.custom_httpx.llm_http_handler.get_async_httpx_client",
                    return_value=mock_client,
                ):
...
def test_video_remix_handler_prefers_explicit_api_key():
    """Sync remix handler should prefer explicit api_key over litelm_params."""
    handler = BaseLLMHTTPHandler()
    config = OpenAIVideoConfig()
