...
def test_transform_request_with_response_format():
    ...
    litelm_logging_obj = MockLoggingObj()
...
def test_transform_request_strips_internal_metadata_to_litelm_metadata():
    handler = LiteLLMResponsesTransformationHandler()
    messages = [{"role": "user", "content": "Hello"}]
    optional_params = {}
    litelm_params = {
        "metadata": {"user_api_key_auth": {"id": "abc"}},
        "litelm_metadata": {"trace_id": "trace-1"},
        "api_key": "sk-test",
    }
    ...
    assert result["litelm_metadata"]["user_api_key_auth"]["id"] == "abc"
    assert result["litelm_metadata"]["trace_id"] == "trace-1"
...
def test_transform_request_preserves_user_metadata():
    ...
    litelm_params = {"metadata": {"internal_key": "secret"}}
    ...
    assert result["litelm_metadata"]["internal_key"] == "secret"
