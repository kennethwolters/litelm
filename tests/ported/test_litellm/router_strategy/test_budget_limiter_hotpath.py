...
@pytest.mark.asyncio
async def test_get_llm_provider_for_deployment_dict_does_not_require_litelm_params_instantiation(
    ...
    monkeypatch.setattr(
        "litelm.router_strategy.budget_limiter.LiteLLM_Params",
        RaiseOnInit,
    )
    ...
    deployment = {"litelm_params": {"model": "openai/gpt-4o-mini"}}
...
async def test_get_llm_provider_for_deployment_dict_view_supports_mapping_and_attr_access(
    ...
    def _future_style_get_llm_provider(
        ...
        assert litelm_params is not None
        observed["model_attr"] = litelm_params.model
        observed["provider_get"] = litelm_params.get("custom_llm_provider")
        observed["api_base_item"] = litelm_params["api_base"]
        observed["has_api_key"] = "api_key" in litelm_params
        observed["model_dump"] = litelm_params.model_dump()
        return model, "openai", None, None

    monkeypatch.setattr(
        "litelm.router_strategy.budget_limiter.litelm.get_llm_provider",
    ...
    healthy_deployments = [
        {
            ...
            "litelm_params": {"model": "openai/gpt-4o-mini"},
...
async def test_async_filter_deployments_does_not_recompute_provider_when_resolved_none(
    ...
    healthy_deployments = [
        {
            "model_name": "gpt-4o-mini",
            "litelm_params": {"model": "unknown-provider/model"},
            "model_info": {"id": "deployment-1"},
        }
    ]
...
def _legacy_provider_resolution(deployment):
    ...
    """
    try:
        _litelm_params = LiteLLM_Params(**deployment.get("litelm_params", {"model": ""}))
        _, custom_llm_provider, _, _ = litelm.get_llm_provider(
            model=_litelm_params.model,
            litelm_params=_litelm_params,
        )
    except Exception:
        return None
...
@pytest.mark.parametrize(
    ...
    [
        {"litelm_params": {"model": "openai/gpt-4o-mini"}},
        {"litelm_params": {"model": "gpt-4o-mini", "custom_llm_provider": "openai"}},
        {"litelm_params": {"model": "unknown-provider/model"}},
    ],
)
@pytest.mark.asyncio
