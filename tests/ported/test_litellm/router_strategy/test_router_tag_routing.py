...
def test_tag_routing_with_list_of_tags():
    ...
    """
    from litelm.router_strategy.tag_based_routing import is_valid_deployment_tag
...
def test_get_tags_from_request_kwargs_none():
    from litelm.router_strategy.tag_based_routing import _get_tags_from_request_kwargs
...
def test_get_tags_from_request_kwargs_various_inputs():
    ...
    # Indirect via "litelm_params" - metadata inside
    assert (
        _get_tags_from_request_kwargs(
            {"litelm_params": {"metadata": {"tags": ["paid"]}}}
        )
        == ["paid"]
    )
    assert _get_tags_from_request_kwargs({"litelm_params": {"metadata": None}}) == []
    assert _get_tags_from_request_kwargs({"litelm_params": {}}) == []

    # Alternate metadata variable name: "litelm_metadata"
    assert (
        _get_tags_from_request_kwargs(
            {"litelm_metadata": {"tags": ["alt"]}},
    ...
    assert (
        _get_tags_from_request_kwargs(
            {"litelm_params": {"litelm_metadata": {"tags": ["nested-alt"]}}},
            metadata_variable_name="litelm_metadata",
        )
        == ["nested-alt"]
    )
