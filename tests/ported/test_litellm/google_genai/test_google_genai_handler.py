...
def test_non_stream_response_when_stream_requested_sync():
    ...
    """
    from litelm.types.utils import Choices
...
async def test_stream_response_when_stream_requested_async():
    ...
    with patch.object(
        ...
        with patch("litelm.acompletion", return_value=mock_stream):
            # Call the handler with stream=True
...
def test_stream_transformation_error_sync():
    ...
    with patch.object(
        ...
        # Patch litelm.completion directly to prevent real API calls
        with patch("litelm.completion", return_value=mock_stream):
...
async def test_stream_transformation_error_async():
    ...
    with patch.object(
        ...
        # Mock litelm.acompletion at the module level where it's imported
        # We need to patch it in the handler module, not in litelm itself
        with patch("litelm.google_genai.adapters.handler.litelm") as mock_litelm:
            # Use AsyncMock for async function
            mock_litelm.acompletion = AsyncMock(return_value=mock_stream)
            # Call the handler with stream=True and expect a ValueError
            with pytest.raises(ValueError, match="Failed to transform streaming response"):
