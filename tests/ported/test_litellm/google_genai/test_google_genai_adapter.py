...
from litelm.google_genai.main import agenerate_content
...
def test_adapter_import():
    ...
    from litelm.google_genai.adapters.handler import GenerateContentToCompletionHandler
    from litelm.google_genai.adapters.transformation import GoogleGenAIAdapter
...
def test_completion_to_generate_content_with_tool_calls():
    ...
    from litelm.types.utils import Choices, ModelResponse, Usage
...
def test_streaming_tool_calls_transformation():
    ...
    from litelm.google_genai.adapters.transformation import (
        GoogleGenAIAdapter,
...
def test_streaming_partial_tool_calls_accumulation():
    ...
    assert function_call["args"]["path"] == "/Users/ishaanjaffer/Github/litelm/README.md", f"Expected complete path, got {function_call['args']}"
...
def test_completion_to_generate_content_transformation():
    ...
    from litelm.google_genai.adapters.transformation import GoogleGenAIAdapter
    from litelm.types.llms.openai import ChatCompletionAssistantMessage
    from litelm.types.utils import Choices, ModelResponse, Usage
...
def test_api_base_and_api_key_passthrough(function_name, is_async, is_stream):
    """Test that api_base and api_key parameters are passed through to litelm.completion/acompletion when using generate_content"""
    import asyncio
    ...
    if function_name == "generate_content":
        from litelm.google_genai.main import generate_content as test_function
    elif function_name == "agenerate_content":
        from litelm.google_genai.main import agenerate_content as test_function
    elif function_name == "generate_content_stream":
        from litelm.google_genai.main import generate_content_stream as test_function
    elif function_name == "agenerate_content_stream":
        from litelm.google_genai.main import agenerate_content_stream as test_function
    ...
    # Mock the appropriate litelm function (completion vs acompletion)
    mock_target = 'litelm.acompletion' if is_async else 'litelm.completion'
    ...
    with unittest.mock.patch(mock_target) as mock_completion:
        ...
        # Verify that the appropriate litelm function was called
        mock_completion.assert_called_once()
        
        # Get the arguments passed to litelm.completion/acompletion
...
def test_shared_schema_normalization_utilities():
    ...
    from litelm.litelm_core_utils.json_validation_rule import (
...
async def test_google_generate_content_with_openai():
    ...
    with unittest.mock.patch("litelm.google_genai.main.litelm.acompletion", new_callable=unittest.mock.AsyncMock) as mock_completion:
        ...
        # Print the request args sent to litelm.completion
        call_args, call_kwargs = mock_completion.call_args
        print("Arguments sent to litelm.completion:")
        ...
        #########################################################
        # validate only expected fields were sent to litelm.completion
...
async def test_agenerate_content_x_goog_api_key_header():
    ...
    with unittest.mock.patch.dict(os.environ, {"GEMINI_API_KEY": test_api_key}, clear=False):
        ...
        with unittest.mock.patch("litelm.llms.custom_httpx.http_handler.AsyncHTTPHandler.post", new_callable=unittest.mock.AsyncMock) as mock_post:
