...
def test_eager_loading_enabled():
    ...
    modules_to_clear = [k for k in sys.modules.keys() if k.startswith("litelm")]
    for module in modules_to_clear:
        del sys.modules[module]
    
    # Import litelm - encoding should be loaded immediately
    import litelm
    
    # Check that encoding is available (not lazy loaded)
    assert hasattr(litelm, "encoding"), "Encoding should be available when eager loading is enabled"
    
    # Verify it's actually the encoding object
    encoding = litelm.encoding
...
def test_eager_loading_env_var_values():
    ...
    for value in values:
        ...
        import litelm
        assert hasattr(litelm, "encoding"), f"Encoding should be available for value: {value}"
        encoding = litelm.encoding
...
def test_lazy_loading_default():
    ...
    # Import litelm - encoding should NOT be loaded yet
    import litelm
    
    # Encoding should be accessible via __getattr__ (lazy loading)
    encoding = litelm.encoding  # This triggers lazy loading
