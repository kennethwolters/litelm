...
from litelm.proxy.health_endpoints.health_app_factory import build_health_app
from litelm.proxy.proxy_cli import ProxyInitializationHelpers
...
class TestProxyInitializationHelpers:
    ...
    @patch("click.echo")
    def test_echo_litelm_version(self, mock_echo, mock_version):
        # Setup
        mock_version.return_value = "1.0.0"

        # Execute
        ProxyInitializationHelpers._echo_litelm_version()

        # Assert
        mock_version.assert_called_once_with("litelm")
    ...
    def test_get_default_unvicorn_init_args(self):
        ...
        assert args["app"] == "litelm.proxy.proxy_server:app"
        ...
        with patch("litelm.json_logs", True):
            args = ProxyInitializationHelpers._get_default_unvicorn_init_args(
                "localhost", 8000
            )
    ...
    @patch("litelm.proxy.db.prisma_client.PrismaManager.setup_database")
    @patch("litelm.proxy.db.prisma_client.should_update_prisma_schema", return_value=False)
    def test_skip_server_startup(self, mock_should_update, mock_setup_db, mock_atexit_register, mock_uvicorn_run):
        from click.testing import CliRunner

        from litelm.proxy.proxy_cli import run_server
        ...
        with patch.dict(
            ...
            "sys.modules",
            {
                ...
                "litelm.proxy.proxy_server": mock_proxy_module,
            },
        ), patch(
            "litelm.proxy.proxy_cli.ProxyInitializationHelpers._get_default_unvicorn_init_args"
        ) as mock_get_args:
            mock_get_args.return_value = {
                "app": "litelm.proxy.proxy_server:app",
                "host": "localhost",
                "port": 8000,
            }
    ...
    def test_construct_database_url_from_env_vars(self):
        ...
        from litelm.proxy.utils import construct_database_url_from_env_vars
    ...
    def test_run_server_no_config_passed(self, mock_print, mock_uvicorn_run):
        ...
        async def mock_get_config(config_file_path=None):
            return {"general_settings": {}, "litelm_settings": {}}
        ...
        # litelm.proxy.proxy_server import at line 820 of proxy_cli.py has heavy
        ...
        with patch.dict(os.environ, env_overrides):
            ...
            with patch.dict(
                "sys.modules",
                {
                    ...
                    # Also mock litelm.proxy.proxy_server to prevent the real
                    # import at line 820 of proxy_cli.py which has heavy side
                    # effects (FastAPI app init, logging setup, etc.)
                    "litelm.proxy.proxy_server": mock_proxy_server_module,
                },
            ), patch(
                "litelm.proxy.proxy_cli.ProxyInitializationHelpers._get_default_unvicorn_init_args"
            ) as mock_get_args:
...
class TestHealthAppFactory:
    ...
    @patch("litelm.proxy.db.prisma_client.PrismaManager.setup_database")
    @patch("litelm.proxy.db.check_migration.check_prisma_schema_diff")
    @patch("litelm.proxy.db.prisma_client.should_update_prisma_schema")
...
class TestWorkerStartupHooks:
    ...
    async def test_should_run_worker_startup_hooks(self):
        ...
        env_overrides = {
            "LITELLM_WORKER_STARTUP_HOOKS": "tests.test_litelm.proxy.test_proxy_cli:_dummy_hook",
        }
    ...
    async def test_should_run_async_worker_startup_hook(self):
        ...
        env_overrides = {
            "LITELLM_WORKER_STARTUP_HOOKS": "tests.test_litelm.proxy.test_proxy_cli:_dummy_async_hook",
        }
        clean_env = {
            k: v for k, v in os.environ.items() if k not in ("DATABASE_URL", "DIRECT_URL")
        }
    ...
    async def test_should_raise_on_failing_worker_startup_hook(self):
        ...
        env_overrides = {
            "LITELLM_WORKER_STARTUP_HOOKS": "tests.test_litelm.proxy.test_proxy_cli:_failing_hook",
        }
        clean_env = {
            k: v for k, v in os.environ.items() if k not in ("DATABASE_URL", "DIRECT_URL")
        }
    ...
    async def test_should_run_multiple_hooks(self):
        ...
        hooks = (
            "tests.test_litelm.proxy.test_proxy_cli:_dummy_hook,"
            "tests.test_litelm.proxy.test_proxy_cli:_dummy_async_hook"
        )
        env_overrides = {
