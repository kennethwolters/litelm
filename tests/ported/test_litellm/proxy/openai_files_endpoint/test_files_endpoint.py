...
from litelm.proxy._types import LiteLLM_UserTableFiltered, UserAPIKeyAuth
from litelm.proxy.hooks import get_proxy_hook
from litelm.proxy.management_endpoints.internal_user_endpoints import ui_view_users
from litelm.proxy.proxy_server import app
from litelm.types.llms.openai import OpenAIFileObject
...
def setup_proxy_logging_object(monkeypatch, llm_router: Router) -> ProxyLogging:
    ...
    monkeypatch.setattr(
        "litelm.proxy.proxy_server.proxy_logging_obj", proxy_logging_object
    )
...
def test_mock_create_audio_file(mocker: MockerFixture, monkeypatch, llm_router: Router):
    ...
    monkeypatch.setattr("litelm.proxy.proxy_server.master_key", None)
    monkeypatch.setattr("litelm.proxy.proxy_server.prisma_client", None)

    # Mock create_file as an async function
    mock_create_file = mocker.patch("litelm.files.main.create_file", new=mocker.AsyncMock())
    ...
    from litelm.llms.base_llm.files.transformation import BaseFileEndpoints

    class DummyManagedFiles(BaseFileEndpoints):
        async def acreate_file(self, llm_router, create_file_request, target_model_names_list, litelm_parent_otel_span, user_api_key_dict):
            ...
            # Call the mocked litelm.files.main.create_file to ensure asserts work
            await litelm.files.main.create_file(
        ...
        async def afile_retrieve(self, file_id, litelm_parent_otel_span, llm_router):
            raise NotImplementedError("Not implemented for test")
        
        async def afile_list(self, purpose, litelm_parent_otel_span):
            raise NotImplementedError("Not implemented for test")
        
        async def afile_delete(self, file_id, litelm_parent_otel_span, llm_router, **data):
            raise NotImplementedError("Not implemented for test")
        
        async def afile_content(self, file_id, litelm_parent_otel_span, llm_router, **data):
            raise NotImplementedError("Not implemented for test")

    # Manually add the hook to the proxy_hook_mapping
    proxy_logging_obj.proxy_hook_mapping["managed_files"] = DummyManagedFiles()

    monkeypatch.setattr("litelm.proxy.proxy_server.llm_router", llm_router)
    monkeypatch.setattr(
        "litelm.proxy.proxy_server.proxy_logging_obj", proxy_logging_obj
    )
...
def test_target_storage_invokes_storage_backend(
    ...
    mocker.patch(
        "litelm.proxy.openai_files_endpoints.files_endpoints.StorageBackendFileService.upload_file_to_storage_backend",
        new=async_mock,
    )
...
@pytest.mark.skip(reason="function migrated to litelm/proxy/hooks/managed_files.py")
def test_create_file_for_each_model(
    ...
    from litelm import CreateFileRequest
    from litelm.proxy._types import LitellmUserRoles, UserAPIKeyAuth
    from litelm.proxy.openai_files_endpoints.files_endpoints import (
        create_file_for_each_model,
    )
    from litelm.proxy.utils import ProxyLogging
    from litelm.types.llms.openai import OpenAIFileObject, OpenAIFilesPurpose
...
def test_managed_files_with_loadbalancing(mocker: MockerFixture, monkeypatch, llm_router: Router):
    ...
    monkeypatch.setattr("litelm.enable_loadbalancing_on_batch_endpoints", True)
    ...
    try:
        ...
        app.dependency_overrides.pop(ps.user_api_key_auth, None)
    result = response.json()
    assert result["id"] == "litelm_managed_file_abc123"
...
def test_create_file_with_nested_litelm_metadata(
    mocker: MockerFixture, monkeypatch, llm_router: Router
):
    """
    Test that nested litelm_metadata is correctly parsed from form data in bracket notation.
    
    Regression test for: litelm_metadata[spend_logs_metadata][owner] format should be
    ...
    captured_litelm_metadata = {}
    
    class DummyManagedFiles(BaseFileEndpoints):
        async def acreate_file(self, llm_router, create_file_request, target_model_names_list, litelm_parent_otel_span, user_api_key_dict):
            # Capture litelm_metadata for verification
            if isinstance(create_file_request, dict):
                captured_litelm_metadata.update(
                    create_file_request.get("litelm_metadata", {})
                )
            else:
                captured_litelm_metadata.update(
                    getattr(create_file_request, "litelm_metadata", {})
                )
    ...
    # Test with nested litelm_metadata in bracket notation
    response = client.post(
        "/v1/files",
        files={"file": test_file},
        data={
            "purpose": "fine-tune",
            "target_model_names": "gpt-3.5-turbo",
            "litelm_metadata[spend_logs_metadata][owner]": "john_doe",
            "litelm_metadata[spend_logs_metadata][team]": "engineering",
            "litelm_metadata[tags]": "production",
            "litelm_metadata[environment]": "prod",
        },
        headers={"Authorization": "Bearer test-key"},
    )
    ...
    assert "spend_logs_metadata" in captured_litelm_metadata
    assert captured_litelm_metadata["spend_logs_metadata"]["owner"] == "john_doe"
    assert captured_litelm_metadata["spend_logs_metadata"]["team"] == "engineering"
    assert captured_litelm_metadata["tags"] == "production"
    assert captured_litelm_metadata["environment"] == "prod"


def test_create_file_with_deep_nested_litelm_metadata(
    mocker: MockerFixture, monkeypatch, llm_router: Router
):
    """
    Test that deeply nested litelm_metadata is correctly parsed from form data.
    
    Regression test for: litelm_metadata[a][b][c] format should be correctly parsed.
    """
    from litelm.llms.base_llm.files.transformation import BaseFileEndpoints
    from litelm.proxy._types import LitellmUserRoles
    import litelm.proxy.proxy_server as ps
    ...
    try:
        ...
        response = client.post(
            ...
            data={
                "purpose": "batch",
                "target_model_names": "gpt-3.5-turbo",
                "litelm_metadata[config][database][host]": "localhost",
                "litelm_metadata[config][database][port]": "5432",
                "litelm_metadata[config][cache][enabled]": "true",
            },
            headers={"Authorization": "Bearer test-key"},
        )
        ...
        assert "config" in captured_litelm_metadata
        assert "database" in captured_litelm_metadata["config"]
        assert captured_litelm_metadata["config"]["database"]["host"] == "localhost"
        assert captured_litelm_metadata["config"]["database"]["port"] == "5432"
        assert "cache" in captured_litelm_metadata["config"]
        assert captured_litelm_metadata["config"]["cache"]["enabled"] == "true"
    finally:
        app.dependency_overrides.pop(ps.user_api_key_auth, None)
...
def _make_capturing_managed_files():
    ...
    class CapturingManagedFiles(BaseFileEndpoints):
        ...
        async def afile_delete(
            self, file_id, litelm_parent_otel_span, llm_router, **data
        ):
            raise NotImplementedError
