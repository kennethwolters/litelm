...
from litelm.proxy.hooks.key_management_event_hooks import KeyManagementEventHooks
...
class TestKeyManagementEventHooksIndependentOperations:
    ...
    async def test_email_failure_does_not_block_secret_manager(self):
        ...
        with patch.object(
            ...
            "litelm.proxy.hooks.key_management_event_hooks.verbose_proxy_logger"
        ):
...
class TestRotateVirtualKeyInSecretManager:
    ...
    async def test_rotate_virtual_key_with_team_id(self):
        ...
        from litelm.types.secret_managers.main import KeyManagementSystem, KeyManagementSettings
        from litelm.secret_managers.base_secret_manager import BaseSecretManager
        import litelm
        ...
        litelm.secret_manager_client = mock_secret_manager
        litelm._key_management_system = KeyManagementSystem.HASHICORP_VAULT
        litelm._key_management_settings = KeyManagementSettings(
            store_virtual_keys=True,
            prefix_for_stored_virtual_keys="litelm/",
        )
        ...
        with patch.object(
            ...
            "litelm.proxy.hooks.key_management_event_hooks.isinstance",
            ...
            assert call_kwargs["current_secret_name"] == "litelm/virtual-key-old"
            assert call_kwargs["new_secret_name"] == "litelm/virtual-key-new"
    ...
    async def test_rotate_virtual_key_in_key_rotated_hook(self):
        ...
        from litelm.proxy._types import LiteLLM_VerificationToken, GenerateKeyResponse, RegenerateKeyRequest
        from litelm.types.secret_managers.main import KeyManagementSystem, KeyManagementSettings
        import litelm
