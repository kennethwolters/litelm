...
async def test_async_post_call_failure_hook():
    ...
    with patch(
        "litelm.proxy.db.db_spend_update_writer.DBSpendUpdateWriter.update_database",
        ...
        assert "litelm_params" in call_args["kwargs"]
        assert call_args["kwargs"]["litelm_params"]["proxy_server_request"] == {
            "request_id": "test_request_id"
        }
        metadata = call_args["kwargs"]["litelm_params"]["metadata"]
...
async def test_enrich_failure_metadata_with_team_alias():
    ...
    with patch(
        "litelm.proxy.hooks.proxy_track_cost_callback.get_team_object",
        new_callable=AsyncMock,
...
async def test_enrich_failure_metadata_with_full_key_lookup():
    ...
    with patch(
        "litelm.proxy.hooks.proxy_track_cost_callback.get_key_object",
        new_callable=AsyncMock,
