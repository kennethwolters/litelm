...
from litelm.proxy.utils import InternalUsageCache, ProxyLogging, hash_token
...
async def test_normal_router_call_tpm_v3(
    ...
    parallel_request_handler.batch_rate_limiter_script = mock_batch_rate_limiter
    monkeypatch.setattr(litelm, "callbacks", [parallel_request_handler])
...
async def test_async_increment_tokens_with_ttl_preservation():
    ...
    from litelm.types.caching import RedisPipelineIncrementOperation
...
async def test_async_log_success_event_with_dict_usage(
    ...
    from litelm.types.utils import BaseLiteLLMOpenAIResponseObject
...
async def test_agent_level_rate_limit_descriptors():
    ...
    with patch(
        "litelm.proxy.agent_endpoints.agent_registry.global_agent_registry.get_agent_by_id",
