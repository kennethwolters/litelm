...
from litelm.proxy.hooks.user_management_event_hooks import UserManagementEventHooks
from litelm.proxy.hooks.key_management_event_hooks import KeyManagementEventHooks
from litelm.proxy._types import NewUserRequest, NewUserResponse, GenerateKeyRequest, GenerateKeyResponse, UserAPIKeyAuth
import builtins
import sys
...
async def test_v1_user_creation_no_email_when_send_invite_email_false():
    ...
    with patch("litelm.logging_callback_manager.get_custom_loggers_for_type", return_value=[]):
        mock_proxy_server = SimpleNamespace(
            general_settings={"alerting": ["email"]},
            proxy_logging_obj=mock_proxy_logging_obj,
            litelm_proxy_admin_name="admin-user",
        )
        with patch.dict(sys.modules, {"litelm.proxy.proxy_server": mock_proxy_server}):
            data = NewUserRequest(
