...
import litelm
from litelm import DualCache, Router
from litelm.proxy._types import UserAPIKeyAuth
from litelm.proxy.hooks.dynamic_rate_limiter_v3 import (
...
async def test_priority_weight_allocation():
    ...
    litelm.priority_reservation = {"high": 0.9, "low": 0.1}
...
async def test_concurrent_pre_call_hooks_stress():
    ...
    litelm.priority_reservation = {"premium": 0.8, "standard": 0.2}
    ...
    async def mock_get_cache(key, litelm_parent_otel_span=None, local_only=False):
...
async def test_fake_calls_case_1_no_rate_limiting_at_capacity():
    ...
    # Set up priority reservations
    litelm.priority_reservation = {"key_a": 0.75, "key_b": 0.25}
...
async def test_fake_calls_case_3_spillover_capacity_default_keys():
    ...
    litelm.priority_reservation = {"key_a": 0.75}
    litelm.priority_reservation_settings.default_priority = 0.25
...
async def test_fake_calls_case_4_over_allocated_with_normalization():
    ...
    litelm.priority_reservation = {"key_a": 0.60, "key_b": 0.80}
...
async def test_fake_calls_case_5_default_value_priority_reservation():
    ...
    litelm.priority_reservation = {"key_a": 0.50, "key_b": 0.20, "key_c": 0.05}
    litelm.priority_reservation_settings.default_priority = 0.05
...
async def test_default_priority_shared_pool():
    ...
    litelm.priority_reservation = {"prod": 0.75}
    litelm.priority_reservation_settings.default_priority = 0.25
...
async def test_async_log_success_event_increments_by_actual_tokens():
    ...
    os.environ["LITELLM_LICENSE"] = "test-license-key"
    litelm.priority_reservation = {"dev": 0.1, "prod": 0.9}
    ...
    with patch(
        "litelm.proxy.common_utils.callback_utils.get_model_group_from_litelm_kwargs",
        return_value=model,
    ):
...
async def test_saturation_check_cache_ttl_configuration():
    ...
    original_ttl = litelm.priority_reservation_settings.saturation_check_cache_ttl
    litelm.priority_reservation_settings.saturation_check_cache_ttl = 5
    ...
    try:
        ...
        async def mock_get_cache(key, litelm_parent_otel_span=None, local_only=False, **kwargs):
        ...
        litelm.priority_reservation_settings.saturation_check_cache_ttl = 30
        ...
        litelm.priority_reservation_settings.saturation_check_cache_ttl = original_ttl
...
async def test_async_log_success_event_uses_team_priority_from_auth_metadata():
    ...
    os.environ["LITELLM_LICENSE"] = "test-license-key"
    litelm.priority_reservation = {"team_priority": 0.8, "default": 0.2}
