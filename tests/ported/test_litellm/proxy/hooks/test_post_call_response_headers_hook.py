...
async def test_response_headers_hook_returns_headers():
    ...
    with patch("litelm.callbacks", [injector]):
...
async def test_response_headers_hook_multiple_callbacks_merge():
    ...
    with patch("litelm.callbacks", [injector1, injector2]):
        from litelm.proxy.utils import ProxyLogging
        from litelm.caching.caching import DualCache
...
# --- Tests for litelm_call_info parameter ---


class CallInfoInspectorLogger(CustomLogger):
    """Logger that captures litelm_call_info for inspection."""
    ...
    async def async_post_call_response_headers_hook(
        ...
        response: Any,
        request_headers: Optional[Dict[str, str]] = None,
        litelm_call_info: Optional[Dict[str, Any]] = None,
    ) -> Optional[Dict[str, str]]:
        self.called = True
        self.received_call_info = litelm_call_info
        return None


@pytest.mark.asyncio
async def test_litelm_call_info_from_hidden_params():
    """Test that litelm_call_info is built from response._hidden_params."""
    ...
    with patch("litelm.callbacks", [inspector]):
        from litelm.proxy.utils import ProxyLogging
        from litelm.caching.caching import DualCache
...
@pytest.mark.asyncio
async def test_litelm_call_info_from_litelm_metadata():
    """Test that litelm_call_info finds model_info under litelm_metadata (responses API path)."""
    inspector = CallInfoInspectorLogger()
    ...
    with patch("litelm.callbacks", [inspector]):
        ...
        await proxy_logging.post_call_response_headers_hook(
            data={"model": "gpt-4", "litelm_metadata": {"model_info": {"id": "deploy-xyz"}}},
            user_api_key_dict=UserAPIKeyAuth(api_key="test-key"),
            response=MockResponse(),
        )
...
@pytest.mark.asyncio
async def test_litelm_call_info_with_none_response():
    """Test that litelm_call_info handles None response (failure path)."""
    inspector = CallInfoInspectorLogger()
...
@pytest.mark.asyncio
async def test_litelm_call_info_backwards_compatible():
    """Test that existing callbacks without litelm_call_info parameter still work."""
    # HeaderInjectorLogger doesn't accept litelm_call_info — must not crash
