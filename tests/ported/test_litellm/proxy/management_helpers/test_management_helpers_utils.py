...
from litelm.proxy.management_helpers.utils import add_new_member
...
async def test_add_new_member_uses_default_team_budget_id():
    ...
    mock_prisma_client.db.litelm_usertable.upsert = AsyncMock(
    ...
    mock_prisma_client.db.litelm_teammembership.create = AsyncMock(
    ...
    result_user, result_team_membership = await add_new_member(
        ...
        litelm_proxy_admin_name=test_admin_name,
    ...
    mock_prisma_client.db.litelm_usertable.upsert.assert_called_once()
    mock_prisma_client.db.litelm_teammembership.create.assert_called_once()

    # Verify that no budget table creation was called (since max_budget_in_team is None)
    assert (
        not hasattr(mock_prisma_client.db, "litelm_budgettable")
        or not mock_prisma_client.db.litelm_budgettable.create.called
    )

    # Verify the team membership was created with the correct budget_id
    team_membership_call_args = (
        mock_prisma_client.db.litelm_teammembership.create.call_args
    )
...
async def test_add_new_member_creates_new_budget_when_max_budget_provided():
    ...
    2. A new budget is created in the litelm_budgettable
    ...
    mock_prisma_client.db.litelm_budgettable.create = AsyncMock(
    ...
    mock_prisma_client.db.litelm_budgettable.create.assert_called_once()
    budget_call_args = mock_prisma_client.db.litelm_budgettable.create.call_args
...
async def test_attach_object_permission_to_dict_with_object_permission_id():
    ...
    """
    from litelm.proxy.management_helpers.object_permission_utils import attach_object_permission_to_dict
...
async def test_attach_object_permission_to_dict_without_object_permission_id():
    ...
    mock_prisma_client.db.litelm_objectpermissiontable.find_unique.assert_not_called()
