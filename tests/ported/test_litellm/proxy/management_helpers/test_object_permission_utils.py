...
from litelm.proxy.management_helpers.object_permission_utils import (
...
async def test_set_object_permission():
    ...
    mock_prisma_client.db.litelm_objectpermissiontable.create.assert_called_once()
    
    # Verify the data passed to create excludes None values and object_permission_id
    call_args = mock_prisma_client.db.litelm_objectpermissiontable.create.call_args
...
@pytest.mark.asyncio
@patch(
    "litelm.proxy.management_helpers.object_permission_utils._get_allow_all_keys_server_ids",
    return_value=set(),
)
@patch(
