...
from litelm.proxy.client.cli.commands.auth import (
    clear_token,
    get_stored_api_key,
    get_token_file_path,
    load_token,
...
class TestTokenUtilities:
    ...
    def test_get_token_file_path(self):
        ...
        with patch('pathlib.Path.home') as mock_home, \
            ...
            assert result == '/home/user/.litelm/token.json'
    ...
    def test_save_token(self):
        ...
        with patch('builtins.open', mock_open()) as mock_file, \
             patch('litelm.proxy.client.cli.commands.auth.get_token_file_path') as mock_path, \
    ...
    def test_load_token_file_not_exists(self):
        ...
        with patch('litelm.proxy.client.cli.commands.auth.get_token_file_path') as mock_path, \
    ...
    def test_get_stored_api_key_success(self):
        ...
        with patch('litelm.litelm_core_utils.cli_token_utils.load_cli_token', return_value=token_data):
    ...
    def test_get_stored_api_key_no_token(self):
        ...
        with patch('litelm.litelm_core_utils.cli_token_utils.load_cli_token', return_value=None):
            result = get_stored_api_key()
            assert result is None
...
class TestLoginCommand:
    ...
    def test_login_success(self):
        ...
        with patch('webbrowser.open') as mock_browser, \
             ...
             patch('litelm.proxy.client.cli.commands.auth.save_token') as mock_save, \
             patch('litelm.proxy.client.cli.interface.show_commands') as mock_show_commands, \
             patch('litelm._uuid.uuid.uuid4', return_value='test-uuid-123'):
...
class TestLogoutCommand:
    ...
    def test_logout_success(self):
        ...
        with patch('litelm.proxy.client.cli.commands.auth.clear_token') as mock_clear:
...
class TestWhoamiCommand:
    ...
    def test_whoami_authenticated(self):
        ...
        with patch('litelm.proxy.client.cli.commands.auth.load_token', return_value=token_data):
    ...
    def test_whoami_not_authenticated(self):
        ...
        with patch('litelm.proxy.client.cli.commands.auth.load_token', return_value=None):
            result = self.runner.invoke(whoami)
            
            assert result.exit_code == 0
            assert "❌ Not authenticated" in result.output
            assert "Run 'litelm-proxy login'" in result.output
...
class TestCLIKeyRegenerationFlow:
    ...
    def test_login_with_team_selection_flow(self):
        ...
        with patch('webbrowser.open') as mock_browser, \
             ...
             patch('litelm.proxy.client.cli.commands.auth.save_token') as mock_save, \
             patch('litelm.proxy.client.cli.interface.show_commands') as mock_show_commands, \
             patch('litelm._uuid.uuid.uuid4', return_value='session-uuid-456'), \
    ...
    def test_login_without_teams_flow(self):
        ...
        with patch('webbrowser.open') as mock_browser, \
             patch('requests.get', return_value=mock_response), \
             patch('litelm.proxy.client.cli.commands.auth.save_token') as mock_save, \
             patch('litelm.proxy.client.cli.interface.show_commands'), \
             patch('litelm._uuid.uuid.uuid4', return_value='session-uuid-solo'):
            ...
            call_args = mock_browser.call_args[0][0]
            assert "https://test.example.com/sso/key/generate" in call_args
            assert "source=litelm-cli" in call_args
