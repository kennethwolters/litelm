...
# local imports
from litelm.proxy.client.cli import cli
from litelm.proxy.client.cli.commands.models import (
...
def mock_client():
    ...
    with patch("litelm.proxy.client.cli.commands.models.Client") as MockClient:
        yield MockClient
...
def mock_models_info(mock_client):
    ...
    mock_client.return_value.models.info.return_value = [
        {
            "model_name": "gpt-4",
            "litelm_params": {"model": "gpt-4", "litelm_credential_name": "openai-1"},
            "model_info": {
                "id": "model-123",
...
def test_models_import_only_models_matching_regex(tmp_path, mock_client, cli_runner):
    ...
    yaml_content = {
        "model_list": [
            ...
            },
            {
                ...
                "litelm_params": {"model": "llama2"},
                "model_info": {"id": "id-3"},
            },
            {
                "model_name": "other-model",
                "litelm_params": {"model": "other"},
                "model_info": {"id": "id-4"},
            },
        ]
    }
    ...
    # Only match models containing 'gpt' in their litelm_params.model
