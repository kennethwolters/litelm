...
def _make_minimal_chat_completion_response(model: str) -> litelm.ModelResponse:
...
def _make_model_response_stream_chunk(model: str) -> litelm.ModelResponseStream:
    """
    ...
    return litelm.ModelResponseStream(**chunk_dict)
            ...
            "model_list": [
                {
                    ...
                    "litelm_params": {"model": internal_model},
                }
            ],
        },
        tmp_path=tmp_path,
...
async def test_proxy_streaming_chunks_use_client_requested_model_before_alias_mapping(monkeypatch):
    ...
    gen = proxy_server.async_data_generator(
        ...
        request_data={
            ...
            "_litelm_client_requested_model": client_model_alias,
        },
    )
...
async def test_proxy_streaming_azure_model_router_preserves_actual_model(monkeypatch):
    ...
    gen = proxy_server.async_data_generator(
        ...
        request_data={
            "model": router_model,
            "_litelm_client_requested_model": router_model,
        },
    )
