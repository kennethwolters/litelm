...
class TestResponsesAPIEndpoints(unittest.TestCase):
    ...
    @patch("litelm.proxy.proxy_server.user_api_key_auth")
    ...
    async def test_cursor_chat_completions_route(self, mock_auth, mock_router):
        ...
        from litelm.types.utils import ResponseOutputMessage, ResponseOutputText
    ...
    async def test_responses_api_key_spend_header_includes_response_cost(
        ...
        # Verify x-litelm-key-spend header includes current request cost
        assert "x-litelm-key-spend" in response.headers
        key_spend_value = float(response.headers["x-litelm-key-spend"])
        ...
        # Verify x-litelm-response-cost header is present
        assert "x-litelm-response-cost" in response.headers
        response_cost_value = float(response.headers["x-litelm-response-cost"])
