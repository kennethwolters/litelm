...
from litelm.proxy.discovery_endpoints.ui_discovery_endpoints import router
...
def test_ui_discovery_endpoints_with_defaults():
    ...
    with patch("litelm.proxy.utils.get_server_root_path", return_value="/"), \
         patch("litelm.proxy.utils.get_proxy_base_url", return_value=None), \
         patch("litelm.proxy.auth.auth_utils._has_user_setup_sso", return_value=False), \
         patch.dict(os.environ, {"DISABLE_ADMIN_UI": "false"}, clear=False):
        
        response = client.get("/.well-known/litelm-ui-config")
...
def test_ui_discovery_endpoints_with_custom_server_root_path():
    ...
    with patch("litelm.proxy.utils.get_server_root_path", return_value="/litelm"), \
         patch("litelm.proxy.utils.get_proxy_base_url", return_value=None), \
         patch("litelm.proxy.auth.auth_utils._has_user_setup_sso", return_value=False), \
         patch.dict(os.environ, {"DISABLE_ADMIN_UI": "false"}, clear=False):
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["server_root_path"] == "/litelm"
        assert data["proxy_base_url"] is None
        assert data["auto_redirect_to_sso"] is False
        assert data["sso_configured"] is False
...
def test_ui_discovery_endpoints_with_proxy_base_url_when_set():
    ...
    with patch("litelm.proxy.utils.get_server_root_path", return_value="/"), \
         patch("litelm.proxy.utils.get_proxy_base_url", return_value="https://proxy.example.com"), \
         patch("litelm.proxy.auth.auth_utils._has_user_setup_sso", return_value=False), \
         patch.dict(os.environ, {"DISABLE_ADMIN_UI": "false"}, clear=False):
        
        response = client.get("/litelm/.well-known/litelm-ui-config")
...
def test_ui_discovery_endpoints_with_sso_configured_and_auto_redirect_enabled():
    ...
    with patch("litelm.proxy.utils.get_server_root_path", return_value="/litelm"), \
         patch("litelm.proxy.utils.get_proxy_base_url", return_value="https://proxy.example.com"), \
         patch("litelm.proxy.auth.auth_utils._has_user_setup_sso", return_value=True), \
...
def test_ui_discovery_endpoints_both_routes_return_same_data():
    ...
    with patch("litelm.proxy.utils.get_server_root_path", return_value="/litelm"), \
        ...
        response1 = client.get("/.well-known/litelm-ui-config")
        response2 = client.get("/litelm/.well-known/litelm-ui-config")
...
def test_ui_discovery_endpoints_with_auto_redirect_via_general_settings():
    ...
    with patch("litelm.proxy.utils.get_server_root_path", return_value="/"), \
         patch("litelm.proxy.utils.get_proxy_base_url", return_value=None), \
         patch("litelm.proxy.auth.auth_utils._has_user_setup_sso", return_value=True), \
         patch("litelm.proxy.proxy_server.general_settings", {"auto_redirect_ui_login_to_sso": True}), \
         patch.dict(os.environ, {"DISABLE_ADMIN_UI": "false"}, clear=False):
        os.environ.pop("AUTO_REDIRECT_UI_LOGIN_TO_SSO", None)
...
def test_ui_discovery_endpoints_with_auto_redirect_env_var_overrides_general_settings():
    ...
    with patch("litelm.proxy.utils.get_server_root_path", return_value="/"), \
         patch("litelm.proxy.utils.get_proxy_base_url", return_value=None), \
         patch("litelm.proxy.auth.auth_utils._has_user_setup_sso", return_value=True), \
         patch("litelm.proxy.proxy_server.general_settings", {"auto_redirect_ui_login_to_sso": False}), \
         patch.dict(os.environ, {"AUTO_REDIRECT_UI_LOGIN_TO_SSO": "true", "DISABLE_ADMIN_UI": "false"}, clear=False):
