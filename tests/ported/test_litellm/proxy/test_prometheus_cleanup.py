"""
Tests for litelm.proxy.prometheus_cleanup.wipe_directory and
...
from litelm.proxy.prometheus_cleanup import mark_worker_exit, wipe_directory
...
class TestMaybeSetupPrometheusMultiprocDir:
    def test_respects_existing_env_var(self, tmp_path):
        ...
        litelm_settings = {"callbacks": ["prometheus"]}
    ...
    def test_noop_when_setup_not_needed(self, num_workers, litelm_settings):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("PROMETHEUS_MULTIPROC_DIR", None)
            os.environ.pop("prometheus_multiproc_dir", None)
    ...
    def test_auto_creates_dir_when_prometheus_configured(self, litelm_settings):
