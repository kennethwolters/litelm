...
def _make_db_model(model_name: str, model_id: str):
    ...
    record.litelm_params = {"model": model_name}
...
class TestUpdateLlmRouterResilience:
    ...
    async def test_models_loaded_when_get_config_times_out(self):
        ...
        with (
            ...
            patch("litelm.proxy.proxy_server.llm_model_list", []),
    ...
    async def test_normal_delete_still_works(self):
        ...
        with (
            patch.object(
                ...
                new_callable=AsyncMock,
                return_value={"model_list": [
                    {"model_name": "gpt-4", "litelm_params": {"model": "gpt-4"}, "model_info": {"id": "config-id-1"}}
                ]},
            ),
            patch("litelm.proxy.proxy_server.llm_router", mock_router),
            patch("litelm.proxy.proxy_server.premium_user", False),
