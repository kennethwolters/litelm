...
import litelm
from litelm.proxy._types import TeamCallbackMetadata, UserAPIKeyAuth
...
class TestGetMetadataVariableName:
    ...
    def test_returns_litelm_metadata_for_thread_routes(self):
        request = self._make_request("/v1/threads/thread_123/messages")
        assert _get_metadata_variable_name(request) == "litelm_metadata"

    def test_returns_litelm_metadata_for_assistant_routes(self):
        request = self._make_request("/v1/assistants/asst_123")
        assert _get_metadata_variable_name(request) == "litelm_metadata"

    def test_returns_litelm_metadata_for_batches_route(self):
        request = self._make_request("/v1/batches")
        assert _get_metadata_variable_name(request) == "litelm_metadata"

    def test_returns_litelm_metadata_for_messages_route(self):
        request = self._make_request("/v1/messages")
        assert _get_metadata_variable_name(request) == "litelm_metadata"

    def test_returns_litelm_metadata_for_files_route(self):
...
def test_get_enforced_params(
    ...
    from litelm.proxy.litelm_pre_call_utils import _get_enforced_params
...
@pytest.mark.asyncio
async def test_add_litelm_data_to_request_parses_string_metadata():
    ...
    # Call
    updated_data = await add_litelm_data_to_request(
        data=data,
    ...
    # Assert
    litelm_metadata = updated_data.get("metadata", {})
    assert isinstance(litelm_metadata, dict)
    assert updated_data["metadata"]["generation_name"] == "gen123"


@pytest.mark.asyncio
async def test_add_litelm_data_to_request_user_spend_and_budget():
    from litelm.proxy.litelm_pre_call_utils import add_litelm_data_to_request
...
@pytest.mark.asyncio
async def test_add_litelm_data_to_request_audio_transcription_multipart():
    from litelm.proxy.litelm_pre_call_utils import add_litelm_data_to_request
    ...
    litelm_metadata = updated_data.get("litelm_metadata", {})
...
@pytest.mark.asyncio
async def test_add_litelm_data_to_request_disabled_callbacks():
    """
    Test that litelm_disabled_callbacks from key metadata is properly added to the request data.
    """
    from litelm.proxy.litelm_pre_call_utils import add_litelm_data_to_request
    ...
    user_api_key_dict = UserAPIKeyAuth(
        ...
        user_id="test_user_id",
        org_id="test_org_id",
        metadata={"litelm_disabled_callbacks": ["langfuse", "langsmith", "datadog"]},
    )
    ...
    # Call add_litelm_data_to_request
    result = await add_litelm_data_to_request(
        data=data,
        request=request_mock,
        user_api_key_dict=user_api_key_dict,
    ...
    # Verify that litelm_disabled_callbacks was added to the request data
    assert "litelm_disabled_callbacks" in result
    assert result["litelm_disabled_callbacks"] == ["langfuse", "langsmith", "datadog"]
...
@pytest.mark.asyncio
async def test_add_litelm_data_to_request_disabled_callbacks_empty():
    """
    Test that litelm_disabled_callbacks is not added when it's empty.
    """
    from litelm.proxy.litelm_pre_call_utils import add_litelm_data_to_request
    ...
    user_api_key_dict = UserAPIKeyAuth(
        ...
        user_id="test_user_id",
        org_id="test_org_id",
        metadata={"litelm_disabled_callbacks": []},
    )
    ...
    # Verify that litelm_disabled_callbacks is not added when empty
    assert "litelm_disabled_callbacks" not in result
...
@pytest.mark.asyncio
async def test_add_litelm_data_to_request_disabled_callbacks_not_present():
    """
    Test that litelm_disabled_callbacks is not added when it's not present in metadata.
    """
    from litelm.proxy.litelm_pre_call_utils import add_litelm_data_to_request
    ...
    user_api_key_dict = UserAPIKeyAuth(
        ...
        user_id="test_user_id",
        org_id="test_org_id",
        metadata={},  # No litelm_disabled_callbacks
    )
    ...
    # Verify that litelm_disabled_callbacks is not added when not present
    assert "litelm_disabled_callbacks" not in result
...
@pytest.mark.asyncio
async def test_add_litelm_data_to_request_disabled_callbacks_invalid_type():
    """
    Test that litelm_disabled_callbacks is not added when it's not a list.
    """
    from litelm.proxy.litelm_pre_call_utils import add_litelm_data_to_request
    ...
    user_api_key_dict = UserAPIKeyAuth(
        ...
        user_id="test_user_id",
        org_id="test_org_id",
        metadata={"litelm_disabled_callbacks": "not_a_list"},  # Should be a list
    )
    ...
    # Verify that litelm_disabled_callbacks is not added when invalid type
    assert "litelm_disabled_callbacks" not in result
...
@pytest.mark.asyncio
async def test_add_litelm_data_to_request_disabled_callbacks_with_logging_settings():
    """
    Test that litelm_disabled_callbacks works correctly alongside logging settings.
    """
    from litelm.proxy.litelm_pre_call_utils import add_litelm_data_to_request
    ...
    user_api_key_dict = UserAPIKeyAuth(
        ...
        metadata={
            ...
            "litelm_disabled_callbacks": ["langsmith", "datadog"],
        },
    )
    ...
    assert "litelm_disabled_callbacks" in result
    assert result["litelm_disabled_callbacks"] == ["langsmith", "datadog"]
...
def test_get_num_retries_from_request():
    ...
    headers_with_retries = {"x-litelm-num-retries": "3"}
    ...
    headers_with_zero = {"x-litelm-num-retries": "0"}
    ...
    headers_with_large_number = {"x-litelm-num-retries": "100"}
    result = LiteLLMProxyRequestSetup._get_num_retries_from_request(
    ...
    headers_with_invalid = {"x-litelm-num-retries": "invalid"}
...
def test_add_user_api_key_auth_to_request_metadata():
    ...
    data = {
        "model": "gpt-3.5-turbo",
        "messages": [{"role": "user", "content": "Hello"}],
        "litelm_metadata": {},  # This will be the metadata variable name
    }
    ...
    metadata_variable_name = "litelm_metadata"
...
def test_add_headers_to_llm_call_by_model_group(
    ...
    original_model_group_settings = getattr(litelm, "model_group_settings", None)
    litelm.model_group_settings = model_group_settings
    ...
    try:
        ...
        litelm.model_group_settings = original_model_group_settings
...
def test_add_headers_to_llm_call_by_model_group_empty_headers_returned():
    ...
    original_model_group_settings = getattr(litelm, "model_group_settings", None)
    litelm.model_group_settings = mock_settings
...
from litelm.proxy.common_request_processing import ProxyBaseLLMRequestProcessing
from litelm.proxy.utils import ProxyLogging
from litelm.types.utils import StandardLoggingPayload
...
@pytest.mark.asyncio
async def test_add_litelm_metadata_from_request_headers():
    """
    Test that add_litelm_metadata_from_request_headers properly adds litelm metadata from request headers,
    ...
    Relevant issue: https://github.com/BerriAI/litelm/issues/14008
    """
    # Set up test logger
    litelm._turn_on_debug()
    test_logger = TestCustomLogger()
    original_callbacks = litelm.callbacks
    litelm.callbacks = [test_logger]

    try:
        # Prepare test data (ensure no streaming, add mock_response and api_key to route to litelm.acompletion)
        headers = {"x-litelm-spend-logs-metadata": '{"user_id": "12345", "project_id": "proj_abc", "request_type": "chat_completion", "timestamp": "2025-09-02T10:30:00Z"}'}
        ...
        assert SPEND_LOGS_METADATA == dict(json.loads(headers["x-litelm-spend-logs-metadata"])), "spend_logs_metadata should be the same as the headers"
    finally:
        litelm.callbacks = original_callbacks


def test_add_litelm_metadata_from_request_headers_x_litelm_trace_id_sets_chain_id():
    """x-litelm-trace-id sets both metadata and top-level litelm_session_id/litelm_trace_id for call chaining."""
    headers = {"x-litelm-trace-id": "foo"}
    data = {"metadata": {}}
    LiteLLMProxyRequestSetup.add_litelm_metadata_from_request_headers(
    ...
    assert data["litelm_session_id"] == "foo"
    assert data["litelm_trace_id"] == "foo"


def test_add_litelm_metadata_from_request_headers_x_litelm_session_id_sets_chain_id():
    """x-litelm-session-id sets both metadata and top-level litelm_session_id/litelm_trace_id for call chaining."""
    headers = {"x-litelm-session-id": "bar"}
    data = {"metadata": {}}
    LiteLLMProxyRequestSetup.add_litelm_metadata_from_request_headers(
        headers=headers, data=data, _metadata_variable_name="metadata"
    )
    ...
    assert data["litelm_session_id"] == "bar"
    assert data["litelm_trace_id"] == "bar"


def test_add_litelm_metadata_from_request_headers_both_headers_trace_id_precedence():
    """When both x-litelm-trace-id and x-litelm-session-id are present, trace-id takes precedence for chain_id."""
    headers = {
        "x-litelm-trace-id": "trace-value",
        "x-litelm-session-id": "session-value",
    }
    data = {"metadata": {}}
    LiteLLMProxyRequestSetup.add_litelm_metadata_from_request_headers(
        headers=headers, data=data, _metadata_variable_name="metadata"
    ...
    assert data["litelm_session_id"] == "trace-value"
    assert data["litelm_trace_id"] == "trace-value"


def test_get_internal_user_header_from_mapping_returns_expected_header():
    mappings = [
        {"header_name": "X-OpenWebUI-User-Id", "litelm_user_role": "internal_user"},
        {"header_name": "X-OpenWebUI-User-Email", "litelm_user_role": "customer"},
    ]
...
def test_get_internal_user_header_from_mapping_none_when_absent():
    ...
    single = {"header_name": "X-Only-Customer", "litelm_user_role": "customer"}
...
async def test_team_guardrails_append_to_key_guardrails():
    ...
    with patch("litelm.proxy.utils._premium_user_check"):
        updated_data = await add_litelm_data_to_request(
            data=data,
            request=request_mock,
            user_api_key_dict=user_api_key_dict,
...
async def test_request_guardrails_do_not_override_key_guardrails():
    ...
    with patch("litelm.proxy.utils._premium_user_check"):
        updated_data_empty = await add_litelm_data_to_request(
...
async def test_embedding_header_forwarding_with_model_group():
    ...
    import litelm.proxy.litelm_pre_call_utils as pre_call_utils_module

    # Reload the module to ensure it has a fresh reference to litelm
    # This is necessary because conftest.py reloads litelm at module scope,
    # which can cause the module's litelm reference to become stale
    ...
    # Use string-based patch to ensure we patch the current sys.modules['litelm']
    ...
    with patch("litelm.model_group_settings", mock_settings):
        # Call add_litelm_data_to_request which includes header forwarding logic
        updated_data = await add_litelm_data_to_request(
            data=data,
            request=request_mock,
            user_api_key_dict=user_api_key_dict,
...
async def test_add_guardrails_from_policy_engine():
    ...
    """
    from litelm.proxy.policy_engine.attachment_registry import get_attachment_registry
    from litelm.proxy.policy_engine.policy_registry import get_policy_registry
    from litelm.types.proxy.policy_engine import (
        Policy,
        PolicyAttachment,
        PolicyGuardrails,
    )
...
async def test_add_guardrails_from_policy_engine_policy_version_by_id():
    ...
    from litelm.proxy.policy_engine.attachment_registry import get_attachment_registry
    from litelm.proxy.policy_engine.policy_registry import get_policy_registry
    from litelm.types.proxy.policy_engine import Policy, PolicyGuardrails
...
async def test_bearer_token_not_in_debug_logs():
    ...
    Calls add_litelm_data_to_request with a Bearer token in the request
    ...
    appears in any log message — covering the exact paths the client reported:
      - "Request Headers: ..."
      - "receiving data: ..."
      - "[PROXY] returned data from litelm_pre_call_utils: ..."
    """
    import logging
    ...
    try:
        with patch("litelm.proxy.proxy_server.llm_router", None), \
             patch("litelm.proxy.proxy_server.premium_user", True):
            await add_litelm_data_to_request(
                data=data,
                request=mock_request,
                user_api_key_dict=user_api_key_dict,
