...
async def test_daily_spend_tracking_with_disabled_spend_logs():
    ...
    with patch("litelm.proxy.proxy_server.disable_spend_logs", True), patch(
        "litelm.proxy.proxy_server.prisma_client", MagicMock()
    ), patch("litelm.proxy.proxy_server.user_api_key_cache", MagicMock()), patch(
        "litelm.proxy.proxy_server.litelm_proxy_budget_name", "test-budget"
    ):
        # Test data
        test_data = {
            "token": "test-token",
...
async def test_update_daily_spend_with_null_entity_id():
    ...
    mock_batcher.litelm_dailyuserspend = mock_table
...
async def test_update_daily_spend_tag_with_request_id():
    ...
    mock_table = MagicMock()
    mock_prisma_client.db.batch_.return_value.__aenter__.return_value = mock_batcher
    mock_batcher.litelm_dailytagspend = mock_table
    ...
    await DBSpendUpdateWriter._update_daily_spend(
        ...
        entity_type="tag",
        entity_id_field="tag",
        table_name="litelm_dailytagspend",
...
async def test_update_tag_db_with_valid_tags():
    ...
    """
    from litelm.proxy._types import Litellm_EntityType, SpendUpdateQueueItem
...
async def test_update_agent_db_enqueues_agent_spend():
    ...
    """
    from litelm.proxy._types import Litellm_EntityType
...
async def test_commit_spend_updates_to_db_increments_agent_spend():
    """
    Test that _commit_spend_updates_to_db calls litelm_agentstable.update_many
    with spend increment when agent_list_transactions is present.
    """
    db_writer = DBSpendUpdateWriter()

    mock_batcher = MagicMock()
    mock_batcher.litelm_verificationtoken = MagicMock()
    mock_batcher.litelm_verificationtoken.update_many = MagicMock()
    mock_batcher.litelm_usertable = MagicMock()
    mock_batcher.litelm_usertable.update_many = MagicMock()
    mock_batcher.litelm_teamtable = MagicMock()
    mock_batcher.litelm_teamtable.update_many = MagicMock()
    mock_batcher.litelm_teammembership = MagicMock()
    mock_batcher.litelm_teammembership.update_many = MagicMock()
    mock_batcher.litelm_organizationtable = MagicMock()
    mock_batcher.litelm_organizationtable.update_many = MagicMock()
    mock_batcher.litelm_tagtable = MagicMock()
    mock_batcher.litelm_tagtable.update_many = MagicMock()
    mock_batcher.litelm_agentstable = MagicMock()
    mock_batcher.litelm_agentstable.update_many = MagicMock()
    ...
    with patch("litelm.proxy.utils._raise_failed_update_spend_exception"):
    ...
    mock_batcher.litelm_agentstable.update_many.assert_called_once()
    call_kwargs = mock_batcher.litelm_agentstable.update_many.call_args[1]
...
async def test_update_daily_spend_logs_detailed_error_on_batch_upsert_failure():
    ...
    with patch.object(verbose_proxy_logger, "exception") as mock_exception_logger:
        ...
        assert "Table: litelm_dailyuserspend" in call_args
        assert (
...
async def test_commit_key_spend_updates_includes_last_active():
    ...
    mock_batcher.litelm_verificationtoken.update_many.assert_called_once()
    call_kwargs = mock_batcher.litelm_verificationtoken.update_many.call_args[1]
...
async def test_update_database_creates_single_task():
    ...
    with patch("litelm.proxy.proxy_server.disable_spend_logs", False), patch(
        "litelm.proxy.proxy_server.prisma_client", MagicMock()
    ), patch("litelm.proxy.proxy_server.user_api_key_cache", MagicMock()), patch(
        "litelm.proxy.proxy_server.litelm_proxy_budget_name", "test-budget"
    ), patch(
        "litelm.proxy.db.db_spend_update_writer.asyncio.create_task"
    ) as mock_create_task:
...
async def test_batch_database_updates_isolation_on_failure():
    ...
    await db_writer._batch_database_updates(
        ...
        litelm_proxy_budget_name="budget",
...
async def test_daily_agent_receives_deepcopied_payload():
    ...
    with patch("litelm.proxy.proxy_server.disable_spend_logs", True), patch(
        ...
        "litelm.proxy.spend_tracking.spend_tracking_utils.get_logging_payload",
