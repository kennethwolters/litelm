...
that exposes litelm_tooltable.upsert / find_many / find_unique.
"""
...
from litelm.proxy.db.tool_registry_writer import (
    ToolPolicyRegistry,
    batch_upsert_tools,
    get_tool,
...
def _make_prisma(
    *,
    ...
    """Return a mock prisma_client with litelm_tooltable.upsert, find_many, find_unique."""
    prisma = MagicMock()
    prisma.db.litelm_tooltable = MagicMock()
    prisma.db.litelm_tooltable.upsert = AsyncMock(return_value=upsert_return)
    prisma.db.litelm_tooltable.find_many = AsyncMock(
        return_value=find_many_rows if find_many_rows is not None else []
    )
    prisma.db.litelm_tooltable.find_unique = AsyncMock(
...
async def test_batch_upsert_tools_calls_upsert():
    ...
    prisma.db.litelm_tooltable.upsert.assert_awaited_once()
    call_kw = prisma.db.litelm_tooltable.upsert.call_args.kwargs
...
async def test_batch_upsert_tools_empty_list():
    ...
    prisma.db.litelm_tooltable.upsert.assert_not_awaited()
...
async def test_batch_upsert_multiple_tools_calls_upsert_per_tool():
    ...
    await batch_upsert_tools(prisma, items)
    assert prisma.db.litelm_tooltable.upsert.await_count == 2
    calls = prisma.db.litelm_tooltable.upsert.call_args_list
...
async def test_list_tools_no_filter():
    ...
    prisma.db.litelm_tooltable.find_many.assert_awaited_once()
    call_kw = prisma.db.litelm_tooltable.find_many.call_args.kwargs
...
async def test_get_tool_found():
    ...
    prisma.db.litelm_tooltable.find_unique.assert_awaited_once_with(
...
async def test_update_tool_policy_calls_upsert_then_get_tool():
    ...
    prisma.db.litelm_tooltable.find_unique.assert_awaited_with(
        where={"tool_name": "my_tool"}
    )
...
async def test_get_tools_by_names_returns_policy_map():
    ...
    prisma.db.litelm_tooltable.find_many.assert_awaited_once_with(
...
async def test_get_tools_by_names_empty_list():
    ...
    assert result == {}
    prisma.db.litelm_tooltable.find_many.assert_not_awaited()
...
async def test_tool_policy_registry_sync_and_get_effective_policies():
    ...
    prisma.db.litelm_objectpermissiontable.find_many = AsyncMock(
        return_value=[
