...
def test_check_migration_out_of_sync(mocker):
    ...
    mock_logger = mocker.patch("litelm._logging.verbose_logger")

    # Import the function after mocking the logger
    from litelm.proxy.db.check_migration import check_prisma_schema_diff

    # Mock the helper function to simulate out-of-sync state
    mock_diff_helper = mocker.patch(
        "litelm.proxy.db.check_migration.check_prisma_schema_diff_helper",
