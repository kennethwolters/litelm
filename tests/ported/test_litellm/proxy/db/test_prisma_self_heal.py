...
async def test_attempt_db_reconnect_should_not_leak_lock_on_timeout_race(
    ...
    with patch("litelm.proxy.utils.asyncio.wait", side_effect=_fake_wait):
        result = await client.attempt_db_reconnect(
...
async def test_attempt_db_reconnect_should_set_cooldown_after_attempt(mock_proxy_logging):
    ...
    with patch(
        "litelm.proxy.utils.time.time", side_effect=lambda: float(next(fake_clock))
    ):
        result = await client.attempt_db_reconnect(
...
async def test_db_health_watchdog_should_trigger_reconnect_on_db_error(mock_proxy_logging):
    ...
    with patch(
        "litelm.proxy.utils.asyncio.sleep",
        AsyncMock(side_effect=[None, asyncio.CancelledError()]),
    ), patch(
        "litelm.proxy.db.exception_handler.PrismaDBExceptionHandler.is_database_connection_error",
        return_value=True,
    ):
...
async def test_db_health_watchdog_start_stop_lifecycle(mock_proxy_logging):
    ...
    with patch("litelm.proxy.utils.asyncio.create_task", side_effect=_fake_create_task):
