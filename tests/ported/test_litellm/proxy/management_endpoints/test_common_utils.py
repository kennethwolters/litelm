"""
Tests for litelm/proxy/management_endpoints/common_utils.py
...
class TestUpdateMetadataFieldsEmptyCollections:
    ...
    @patch("litelm.proxy.management_endpoints.common_utils._premium_user_check")
...
class TestTeamAdminCanInviteUser:
    ...
    async def test_team_admin_can_invite_user_parametrized(
        ...
        mock_prisma.db.litelm_teamtable.find_many = AsyncMock(
            return_value=teams
        )
...
class TestSetObjectMetadataField:
    ...
    def test_set_object_metadata_field_parametrized(
        ...
        with patch(
            "litelm.proxy.management_endpoints.common_utils._premium_user_check"
        ) as mock_premium:
