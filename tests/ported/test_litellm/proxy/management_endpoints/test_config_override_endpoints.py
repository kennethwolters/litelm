...
from litelm.proxy._types import KeyManagementSystem, LitellmUserRoles, UserAPIKeyAuth
from litelm.proxy.management_endpoints.config_override_endpoints import (
    HASHICORP_ENV_VAR_MAPPING,
    _build_field_schema,
    _set_env_vars,
)
from litelm.proxy.proxy_server import app
from litelm.types.proxy.management_endpoints.config_overrides import (
...
def _make_mock_db():
    ...
    prisma = MagicMock()
    prisma.db.litelm_configoverrides = mock
    return prisma, mock
...
async def test_hashicorp_vault_crud_lifecycle(client, monkeypatch):
    ...
    old_client, old_kms = litelm.secret_manager_client, litelm._key_management_system
    _set_admin()
    ...
    try:
        ...
        litelm.secret_manager_client = MagicMock()
        ...
        assert litelm.secret_manager_client is None
        ...
        litelm.secret_manager_client = old_client
        litelm._key_management_system = old_kms
        _cleanup()
...
async def test_hashicorp_vault_validation_errors_and_access_control(client, monkeypatch):
    ...
    try:
        ...
        aws = MagicMock()
        litelm.secret_manager_client = aws
        litelm._key_management_system = KeyManagementSystem.AWS_SECRET_MANAGER
        assert client.delete(VAULT_URL).status_code == 200
        assert litelm.secret_manager_client is aws
        assert litelm._key_management_system == KeyManagementSystem.AWS_SECRET_MANAGER
