...
from litelm.proxy.management_endpoints.scim.scim_v2 import patch_user
...
async def test_patch_user_updates_fields():
    ...
    mock_db.litelm_usertable.find_unique = AsyncMock(return_value=mock_user)
    mock_db.litelm_usertable.update = AsyncMock(side_effect=mock_update)
    mock_db.litelm_teamtable.find_unique = AsyncMock(return_value=None)
    ...
    with patch("litelm.proxy.proxy_server.prisma_client", mock_client), \
         patch("litelm.proxy.management_endpoints.scim.scim_v2.ScimTransformations.transform_litelm_user_to_scim_user", 
               AsyncMock(return_value=mock_scim_user)):
        result = await patch_user(user_id="user-1", patch_ops=patch_ops)

    mock_db.litelm_usertable.update.assert_called_once()
...
async def test_patch_user_manages_group_memberships():
    ...
    with patch("litelm.proxy.proxy_server.prisma_client", mock_client), \
         patch("litelm.proxy.management_endpoints.scim.scim_v2.team_member_add",
               AsyncMock(side_effect=mock_add)) as mock_add_fn, \
         patch("litelm.proxy.management_endpoints.scim.scim_v2.team_member_delete",
    ...
    call_args = mock_db.litelm_usertable.update.call_args
