...
from litelm.proxy._types import LiteLLM_TeamTable, LiteLLM_UserTable, Member
...
def mock_prisma_client():
    ...
    mock_db.litelm_teamtable.find_unique = mock_find_unique
...
class TestScimTransformations:
    @pytest.mark.asyncio
    async def test_transform_litelm_user_to_scim_user(
        ...
        with patch("litelm.proxy.proxy_server.prisma_client", mock_client):
            scim_user = await ScimTransformations.transform_litelm_user_to_scim_user(
                mock_user
            )
    ...
    @pytest.mark.asyncio
    async def test_transform_litelm_team_to_scim_group(
        self, mock_team, mock_prisma_client
    ):
        mock_client, _ = mock_prisma_client

        with patch("litelm.proxy.proxy_server.prisma_client", mock_client):
            scim_group = await ScimTransformations.transform_litelm_team_to_scim_group(
                mock_team
            )
