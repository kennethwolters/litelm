...
from litelm.proxy._types import LitellmUserRoles, NewUserRequest, NewUserResponse, ProxyException
from litelm.proxy.management_endpoints.scim.scim_v2 import (
...
from litelm.types.proxy.management_endpoints.scim_v2 import (
    SCIMGroup,
    SCIMMember,
    SCIMPatchOp,
    SCIMPatchOperation,
...
async def test_create_user_existing_user_conflict(mocker):
    ...
    mock_prisma_client.db.litelm_usertable = mocker.MagicMock()
    mock_prisma_client.db.litelm_usertable.find_unique = AsyncMock(return_value={"user_id": "existing-user"})

    # Mock the _get_prisma_client_or_raise_exception to return our mock
    mocker.patch(
        "litelm.proxy.management_endpoints.scim.scim_v2._get_prisma_client_or_raise_exception",
        AsyncMock(return_value=mock_prisma_client),
    )
    
    mocked_new_user = mocker.patch(
        "litelm.proxy.management_endpoints.scim.scim_v2.new_user",
        AsyncMock(),
    )
...
async def test_create_user_defaults_to_viewer(mocker, monkeypatch):
    ...
    mock_prisma_client.db.litelm_usertable.find_first = AsyncMock(return_value=None)

    monkeypatch.setattr(
        "litelm.default_internal_user_params", None, raising=False
    )
    ...
    mocker.patch(
        "litelm.proxy.management_endpoints.scim.scim_v2.ScimTransformations.transform_litelm_user_to_scim_user",
...
async def test_create_user_uses_default_internal_user_params_role(mocker, monkeypatch):
    ...
    monkeypatch.setattr(
        "litelm.default_internal_user_params", default_params, raising=False
    )
...
async def test_scim_create_user_respects_default_role_set_via_ui(mocker, monkeypatch):
    ...
    The UI saves the setting via _update_litelm_setting, which should update
    litelm.default_internal_user_params in memory. Then SCIM create_user
    ...
    """
    from litelm.proxy._types import DefaultInternalUserParams
    from litelm.proxy.ui_crud_endpoints.proxy_setting_endpoints import (
        _update_litelm_setting,
    )

    # Step 1: Start with no default params (fresh proxy state)
    monkeypatch.setattr("litelm.default_internal_user_params", None, raising=False)

    # Step 2: Simulate the UI saving "Internal User (Create/Delete/View)" as default role
    # Mock the proxy_config and store_model_in_db that _update_litelm_setting needs
    mock_proxy_config = mocker.MagicMock()
    mock_proxy_config.get_config = AsyncMock(return_value={"litelm_settings": {}})
    ...
    mocker.patch(
        "litelm.proxy.proxy_server.store_model_in_db",
        True,
    )
    ...
    await _update_litelm_setting(
        settings=settings,
        settings_key="default_internal_user_params",
        in_memory_var=litelm.default_internal_user_params,
        success_message="ok",
    )

    # Verify the in-memory variable was actually updated
    assert litelm.default_internal_user_params is not None, (
        "BUG: _update_litelm_setting did not update litelm.default_internal_user_params in memory. "
        "The local variable reassignment (in_memory_var = ...) doesn't propagate back."
    )
    assert litelm.default_internal_user_params.get("user_role") == LitellmUserRoles.INTERNAL_USER
    ...
    assert called_args.user_role == LitellmUserRoles.INTERNAL_USER, (
        ...
        f"in-memory variable was not updated by _update_litelm_setting."
    )
...
async def test_handle_existing_user_by_email_existing_user_updated(mocker):
    ...
    mock_prisma_client.db = mocker.MagicMock()
    mock_prisma_client.db.litelm_usertable = mocker.MagicMock()
    mock_prisma_client.db.litelm_usertable.find_first = AsyncMock(return_value=existing_user)
    mock_prisma_client.db.litelm_usertable.update = AsyncMock(return_value=updated_user)
    ...
    mock_prisma_client.db.litelm_usertable.update.assert_called_once_with(
...
async def test_handle_team_membership_changes_no_changes(mocker):
    ...
    mock_patch_team_membership = mocker.patch(
        "litelm.proxy.management_endpoints.scim.scim_v2.patch_team_membership",
        AsyncMock()
    )
...
async def test_update_user_success(mocker):
    ...
    mocker.patch(
        "litelm.proxy.management_endpoints.scim.scim_v2._check_user_exists",
        AsyncMock(return_value=existing_user)
    )
    mocker.patch(
        "litelm.proxy.management_endpoints.scim.scim_v2._handle_team_membership_changes",
        AsyncMock()
    )
    mocker.patch(
        "litelm.proxy.management_endpoints.scim.scim_v2.ScimTransformations.transform_litelm_user_to_scim_user",
    ...
    mock_prisma_client.db.litelm_usertable.update.assert_called_once()
    call_args = mock_prisma_client.db.litelm_usertable.update.call_args
...
async def test_update_group_metadata_serialization_issue(mocker):
    ...
    """
    from litelm.proxy.management_endpoints.scim.scim_v2 import update_group
    from litelm.types.proxy.management_endpoints.scim_v2 import SCIMGroup, SCIMMember
    ...
    mock_prisma_client = mocker.MagicMock()
    mock_prisma_client.db = mocker.MagicMock()
    mock_prisma_client.db.litelm_teamtable = mocker.MagicMock()
    mock_prisma_client.db.litelm_usertable = mocker.MagicMock()
    
    # Mock team operations
    mock_prisma_client.db.litelm_teamtable.find_unique = AsyncMock(return_value=mock_existing_team)
    mock_prisma_client.db.litelm_teamtable.update = AsyncMock(return_value=mock_updated_team)
    ...
    mock_prisma_client.db.litelm_usertable.find_unique = AsyncMock(return_value=mock_user)
    mock_prisma_client.db.litelm_usertable.update = AsyncMock(return_value=mock_user)
    ...
    mocker.patch(
        "litelm.proxy.management_endpoints.scim.scim_v2.ScimTransformations.transform_litelm_team_to_scim_group",
    ...
    mock_prisma_client.db.litelm_teamtable.update.assert_called_once()
...
async def test_team_membership_management(mocker):
    ...
    """
    from litelm.proxy._types import Member
    from litelm.proxy.management_endpoints.scim.scim_v2 import (
...
async def test_update_group_e2e(mocker):
    ...
    """
    from litelm.proxy._types import LiteLLM_TeamTable, Member
    from litelm.proxy.management_endpoints.scim.scim_transformations import (
        ScimTransformations,
    )
    ...
    mock_prisma_client.db.litelm_teamtable.find_unique = AsyncMock(return_value=existing_team)
    ...
    mock_prisma_client.db.litelm_teamtable.update = AsyncMock(return_value=updated_team)
    ...
    mocker.patch.object(
        ScimTransformations,
        "transform_litelm_team_to_scim_group",
    ...
    ScimTransformations.transform_litelm_team_to_scim_group.assert_called_once_with(updated_team)
...
async def test_create_group_with_nonexistent_users_rejects(mocker, monkeypatch):
    ...
    from litelm.proxy.proxy_server import proxy_config
    ...
    mock_prisma_client.db.litelm_teamtable.find_unique = AsyncMock(return_value=None)
    ...
    mock_prisma_client.db.litelm_usertable.find_unique = AsyncMock(side_effect=mock_user_lookup)
...
async def test_update_group_with_nonexistent_users_rejects(mocker, monkeypatch):
    ...
    mocker.patch(
        "litelm.proxy.management_endpoints.scim.scim_v2._check_team_exists",
...
async def test_create_group_with_nonexistent_users_creates_when_flag_true(mocker, monkeypatch):
    ...
    mock_create_user = mocker.patch(
        "litelm.proxy.management_endpoints.scim.scim_v2._create_user_if_not_exists",
    ...
    mock_new_team = mocker.patch(
        "litelm.proxy.management_endpoints.scim.scim_v2.new_team",
