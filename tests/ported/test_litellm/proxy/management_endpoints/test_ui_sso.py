...
import litelm
from litelm.proxy._types import LiteLLM_UserTable, NewUserResponse
from litelm.proxy.auth.handle_jwt import JWTHandler
from litelm.proxy.management_endpoints.sso import CustomMicrosoftSSO
from litelm.proxy.management_endpoints.types import CustomOpenID
from litelm.proxy.management_endpoints.ui_sso import (
    GoogleSSOHandler,
    MicrosoftSSOHandler,
...
from litelm.types.proxy.management_endpoints.ui_sso import (
...
def test_microsoft_sso_handler_openid_from_response_with_custom_attributes():
    ...
    # Act
    with patch(
        "litelm.constants.MICROSOFT_USER_EMAIL_ATTRIBUTE", "custom_email_field"
    ), patch(
        "litelm.constants.MICROSOFT_USER_DISPLAY_NAME_ATTRIBUTE", "custom_display_name"
    ), patch(
        "litelm.constants.MICROSOFT_USER_ID_ATTRIBUTE", "custom_id_field"
    ), patch(
        "litelm.constants.MICROSOFT_USER_FIRST_NAME_ATTRIBUTE", "custom_first_name"
    ), patch(
        "litelm.constants.MICROSOFT_USER_LAST_NAME_ATTRIBUTE", "custom_last_name"
    ), patch(
        "litelm.proxy.management_endpoints.ui_sso.MICROSOFT_USER_EMAIL_ATTRIBUTE",
        "custom_email_field",
    ), patch(
        "litelm.proxy.management_endpoints.ui_sso.MICROSOFT_USER_DISPLAY_NAME_ATTRIBUTE",
        "custom_display_name",
    ), patch(
        "litelm.proxy.management_endpoints.ui_sso.MICROSOFT_USER_ID_ATTRIBUTE",
        "custom_id_field",
    ), patch(
        "litelm.proxy.management_endpoints.ui_sso.MICROSOFT_USER_FIRST_NAME_ATTRIBUTE",
        "custom_first_name",
    ), patch(
        "litelm.proxy.management_endpoints.ui_sso.MICROSOFT_USER_LAST_NAME_ATTRIBUTE",
        "custom_last_name",
    ):
        result = MicrosoftSSOHandler.openid_from_response(
            response=mock_response, team_ids=expected_team_ids, user_role=None
...
async def test_get_user_groups_from_graph_api():
    ...
    with patch(
        "litelm.proxy.management_endpoints.ui_sso.get_async_httpx_client"
    ) as mock_client:
...
async def test_default_team_params(team_params):
    """
    When litelm.default_team_params is set, it should be used to create a new team
    """
    # Arrange
    litelm.default_team_params = team_params
    ...
    mock_prisma.db.litelm_teamtable.find_first = AsyncMock(return_value=None)
    mock_prisma.db.litelm_teamtable.create = AsyncMock()
    ...
    with patch("litelm.proxy.proxy_server.prisma_client", mock_prisma):
        # Act
        team_id = str(uuid.uuid4())
        await MicrosoftSSOHandler.create_litelm_teams_from_service_principal_team_ids(
        ...
        mock_prisma.db.litelm_teamtable.create.assert_called_once()
        create_call_args = mock_prisma.db.litelm_teamtable.create.call_args.kwargs[
            "data"
        ]
...
async def test_create_team_without_default_params():
    """
    Test team creation when litelm.default_team_params is None
    Should create team with just the basic required fields
    """
    # Arrange
    litelm.default_team_params = None
...
def test_apply_user_info_values_to_sso_user_defined_values():
    from litelm.proxy._types import LiteLLM_UserTable, SSOUserDefinedValues
    from litelm.proxy.management_endpoints.ui_sso import (
...
def test_build_sso_user_update_data_with_valid_role():
    ...
    from litelm.proxy.management_endpoints.types import CustomOpenID
    from litelm.proxy.management_endpoints.ui_sso import _build_sso_user_update_data
...
def test_generic_response_convertor_normalizes_email():
    ...
    """
    from litelm.proxy.management_endpoints.ui_sso import generic_response_convertor
...
async def test_upsert_sso_user_updates_role_for_existing_user():
    ...
    """
    from litelm.proxy._types import LiteLLM_UserTable, LitellmUserRoles
    from litelm.proxy.management_endpoints.types import CustomOpenID
    from litelm.proxy.management_endpoints.ui_sso import SSOAuthenticationHandler

    # Mock prisma client
    mock_prisma = MagicMock()
    mock_prisma.db.litelm_usertable.update_many = AsyncMock()
    ...
    mock_prisma.db.litelm_usertable.update_many.assert_called_once()
    call_args = mock_prisma.db.litelm_usertable.update_many.call_args
...
async def test_get_user_info_from_db_user_exists():
    ...
    """
    from litelm.proxy.management_endpoints.ui_sso import get_user_info_from_db
    ...
    with patch(
        "litelm.proxy.management_endpoints.ui_sso.get_user_object"
...
async def test_get_user_info_from_db_user_not_exists_creates_user():
    ...
    """
    from litelm.proxy._types import NewUserResponse, SSOUserDefinedValues
    from litelm.proxy.management_endpoints.ui_sso import get_user_info_from_db
    ...
    with patch(
        "litelm.proxy.management_endpoints.ui_sso.get_existing_user_info_from_db",
        return_value=None,  # User doesn't exist
    ) as mock_get_existing, patch(
        "litelm.proxy.management_endpoints.ui_sso.SSOAuthenticationHandler.upsert_sso_user",
        return_value=mock_new_user,
    ) as mock_upsert, patch(
        "litelm.proxy.management_endpoints.ui_sso.SSOAuthenticationHandler.add_user_to_teams_from_sso_response",
    ) as mock_add_teams:
        # Act
...
async def test_check_and_update_if_proxy_admin_id():
    ...
    # Mock Prisma client
    mock_prisma = MagicMock()
    mock_prisma.db.litelm_usertable.update = AsyncMock()
    ...
    with patch.dict(os.environ, {"PROXY_ADMIN_ID": test_user_id}):
        ...
        mock_prisma.db.litelm_usertable.update.assert_called_once_with(
...
async def test_check_and_update_if_proxy_admin_id_already_admin():
    ...
    with patch.dict(os.environ, {"PROXY_ADMIN_ID": test_user_id}):
        ...
        # Assert
        assert updated_role == LitellmUserRoles.PROXY_ADMIN.value
        mock_prisma.db.litelm_usertable.update.assert_not_called()
...
async def test_get_generic_sso_response_with_additional_headers():
    ...
    """
    from litelm.proxy.management_endpoints.ui_sso import get_generic_sso_response
...
class TestAuthCallbackRouting:
    ...
    def test_cli_state_detection_and_routing(self):
        ...
        from litelm.constants import LITELLM_CLI_SESSION_TOKEN_PREFIX
...
class TestGoogleLoginCLIIntegration:
    ...
    def test_google_login_cli_state_generation(self):
        ...
        assert cli_state.startswith("litelm-session-token:")
...
class TestSSOHandlerIntegration:
    ...
    def test_get_redirect_url_for_sso(self):
        ...
        mock_request.base_url = "https://test.litelm.ai/"
        ...
        assert redirect_url.startswith("https://test.litelm.ai")
...
class TestSSOStateHandling:
    ...
    def test_get_cli_state_valid(self):
        ...
        state = SSOAuthenticationHandler._get_cli_state(
            source="litelm-cli", key="sk-test123"
        )

        assert state is not None
        assert state.startswith("litelm-session-token:")
    ...
    def test_get_cli_state_no_key(self):
        ...
        state = SSOAuthenticationHandler._get_cli_state(source="litelm-cli", key=None)
    ...
    def test_get_cli_state_with_existing_key(self):
        ...
        assert state == "litelm-session-token:sk-new-key-123:sk-existing-key-456"
    ...
    def test_get_cli_state_without_existing_key(self):
        ...
        state = SSOAuthenticationHandler._get_cli_state(
            source="litelm-cli", key="sk-new-key-789", existing_key=None
        )
        ...
        assert state == "litelm-session-token:sk-new-key-789"
...
class TestHTMLIntegration:
    ...
    def test_html_render_utils_import(self):
        ...
        from litelm.proxy.common_utils.html_forms.cli_sso_success import (
...
class TestCustomUISSO:
    ...
    def test_enterprise_import_error_handling(self):
        ...
        with patch("litelm.proxy.proxy_server.premium_user", True):
            with patch(
                "litelm.proxy.proxy_server.user_custom_ui_sso_sign_in_handler",
                MagicMock(),
            ):
                with patch.dict(
                    "sys.modules",
                    {
                        "enterprise.litelm_enterprise.proxy.auth.custom_sso_handler": None
                    },
                ):
                    ...
                    async def mock_google_login():
                        ...
                        try:
                            from enterprise.litelm_enterprise.proxy.auth.custom_sso_handler import (  # noqa: F401
    ...
    async def test_handle_custom_ui_sso_sign_in_success(self):
        ...
        from enterprise.litelm_enterprise.proxy.auth.custom_sso_handler import (
            EnterpriseCustomSSOHandler,
        )
        from litelm.integrations.custom_sso_handler import CustomSSOLoginHandler

        # Mock request with custom headers
        mock_request = MagicMock(spec=Request)
        mock_request.headers = {
            "x-litelm-user-id": "test_user_123",
            "x-litelm-user-email": "test@example.com",
    ...
    async def test_custom_ui_sso_handler_execution_with_real_class(self):
        ...
        class TestCustomSSOHandler(CustomSSOLoginHandler):
            ...
            async def handle_custom_ui_sso_sign_in(self, request: Request) -> OpenID:
                ...
                return OpenID(
                    id=request_headers_dict.get("x-litelm-user-id", "default_user"),
                    email=request_headers_dict.get(
                        "x-litelm-user-email", "default@test.com"
                    ),
                    first_name="Custom",
                    last_name="Handler",
        ...
        mock_request = MagicMock(spec=Request)
        mock_request.headers = {
            "x-litelm-user-id": "custom_test_user_456",
            "x-litelm-user-email": "custom@example.com",
            "x-forwarded-for": "10.0.0.1",
        }
        mock_request.base_url = "https://custom.litelm.ai/"
...
class TestCLIKeyRegenerationFlow:
    ...
    async def test_cli_sso_callback_stores_session(self):
        ...
        with patch(
            "litelm.proxy.management_endpoints.ui_sso.get_user_info_from_db",
            return_value=mock_user_info,
        ), patch("litelm.proxy.proxy_server.prisma_client", MagicMock()), patch(
            "litelm.proxy.proxy_server.user_api_key_cache", mock_cache
        ), patch(
            "litelm.proxy.common_utils.html_forms.cli_sso_success.render_cli_sso_success_page",
    ...
    async def test_cli_poll_key_returns_teams_for_selection(self):
        ...
        with patch("litelm.proxy.proxy_server.user_api_key_cache", mock_cache):
    ...
    async def test_auth_callback_routes_to_cli_with_existing_key(self):
        ...
        with patch(
            "litelm.proxy.management_endpoints.ui_sso.cli_sso_callback"
        ) as mock_cli_callback, patch(
            "litelm.proxy.proxy_server.prisma_client", MagicMock()
        ), patch(
            "litelm.proxy.proxy_server.master_key", "test-master-key"
        ), patch(
            "litelm.proxy.proxy_server.general_settings", {}
        ), patch(
            "litelm.proxy.proxy_server.jwt_handler", MagicMock()
        ), patch(
            "litelm.proxy.proxy_server.user_api_key_cache", MagicMock()
        ), patch.dict(
            os.environ, {"GOOGLE_CLIENT_ID": "test-google-id"}, clear=True
        ), patch(
            "litelm.proxy.management_endpoints.ui_sso.GoogleSSOHandler.get_google_callback_response",
    ...
    def test_get_redirect_url_does_not_include_existing_key_in_url(self):
        ...
        with patch(
            "litelm.proxy.utils.get_custom_url", return_value="https://test.litelm.ai"
        ):
            ...
            assert "https://test.litelm.ai/sso/callback" == redirect_url
    ...
    async def test_cli_poll_key_generates_jwt_with_team(self):
        ...
        with patch("litelm.proxy.proxy_server.user_api_key_cache", mock_cache), patch(
            "litelm.proxy.proxy_server.prisma_client"
        ) as mock_prisma, patch(
            "litelm.proxy.auth.auth_checks.ExperimentalUIJWTToken.get_cli_jwt_auth_token",
            return_value=mock_jwt_token,
        ) as mock_get_jwt:
            # Mock the user lookup
            mock_prisma.db.litelm_usertable.find_unique = AsyncMock(
                return_value=mock_user_info
            )
...
async def test_get_ui_settings_includes_api_doc_base_url():
    ...
    from litelm.proxy.management_endpoints.ui_sso import get_ui_settings
...
class TestGetGenericSSORedirectParams:
    ...
    def test_state_priority_cli_state_provided(self):
        ...
        # Arrange
        cli_state = "litelm-session-token:sk-test123"
...
class TestPKCEFunctionality:
    ...
    async def test_prepare_token_exchange_parameters_with_pkce(self):
        ...
        with patch("litelm.proxy.proxy_server.redis_usage_cache", None), patch("litelm.proxy.proxy_server.user_api_key_cache", mock_cache), patch.dict(os.environ, {"GENERIC_CLIENT_USE_PKCE": "true"}):
            # Act
            token_params = (
    ...
    async def test_get_generic_sso_redirect_response_with_pkce(self):
        ...
        with patch.dict(os.environ, {"GENERIC_CLIENT_USE_PKCE": "true"}):
            with patch("litelm.proxy.proxy_server.redis_usage_cache", None), patch(
                "litelm.proxy.proxy_server.user_api_key_cache", mock_cache
            ):
                # Act
    ...
    async def test_pkce_redis_multi_pod_verifier_roundtrip(self):
        ...
        with patch.dict(os.environ, {"GENERIC_CLIENT_USE_PKCE": "true"}):
            with patch("litelm.proxy.proxy_server.redis_usage_cache", mock_redis):
                with patch(
                    "litelm.proxy.proxy_server.user_api_key_cache", mock_in_memory
                ):
    ...
    async def test_pkce_fallback_in_memory_roundtrip_when_redis_none(self):
        ...
        with patch.dict(os.environ, {"GENERIC_CLIENT_USE_PKCE": "true"}):
            with patch("litelm.proxy.proxy_server.redis_usage_cache", None):
                with patch(
                    "litelm.proxy.proxy_server.user_api_key_cache", mock_in_memory
                ):
    ...
    async def test_pkce_prepare_token_exchange_returns_nothing_when_no_state(self):
        ...
        with patch("litelm.proxy.proxy_server.redis_usage_cache", mock_redis), patch(
            "litelm.proxy.proxy_server.user_api_key_cache", mock_in_memory
    ...
    async def test_pkce_token_exchange_basic_auth(self):
        ...
        with patch("litelm.proxy.management_endpoints.ui_sso.httpx.AsyncClient") as mock_client_cls:
    ...
    async def test_delete_pkce_verifier_swallows_deletion_errors(self):
        ...
        with patch("litelm.proxy.proxy_server.redis_usage_cache", None), patch(
            "litelm.proxy.proxy_server.user_api_key_cache", failing_cache
        ):
    ...
    async def test_pkce_cache_miss_non_strict_logs_warning_and_continues(self, caplog):
        ...
        with caplog.at_level(logging.WARNING), patch(
            "litelm.proxy.proxy_server.redis_usage_cache", None
        ), patch(
            "litelm.proxy.proxy_server.user_api_key_cache", mock_cache
        ), patch.dict(
            os.environ,
...
class TestAddMissingTeamMember:
    ...
    async def test_add_missing_team_member_with_new_user_response_teams_none(self):
        ...
        """
        from litelm.proxy._types import NewUserResponse
        from litelm.proxy.management_endpoints.ui_sso import add_missing_team_member
        ...
        with patch(
            "litelm.proxy.management_endpoints.ui_sso.create_team_member_add_task"
        ) as mock_add_task:
    ...
    @pytest.mark.asyncio
    async def test_add_missing_team_member_with_litelm_user_table_empty_teams(self):
        """
    ...
    async def test_add_user_to_teams_from_sso_response_new_user(self):
        ...
        with patch(
            "litelm.proxy.management_endpoints.ui_sso.add_missing_team_member"
        ) as mock_add_member:
...
async def test_role_mappings_override_default_internal_user_params():
    ...
    """
    from litelm.proxy._types import NewUserResponse, SSOUserDefinedValues
    from litelm.proxy.management_endpoints.ui_sso import insert_sso_user

    # Save original default_internal_user_params
    original_default_params = getattr(litelm, "default_internal_user_params", None)

    try:
        # Set default_internal_user_params with a role that should be overridden
        litelm.default_internal_user_params = {
            "user_role": "internal_user",
            "max_budget": 100,
            "budget_duration": "30d",
        ...
        with patch(
            "litelm.proxy.management_endpoints.ui_sso.new_user",
        ...
        # the attribute is defined in litelm/__init__.py and delattr-ing it breaks parallel tests)
        litelm.default_internal_user_params = original_default_params
...
async def test_sso_role_preserved_without_role_mappings():
    ...
    try:
        ...
        # litelm.__getattr__ has no handler for this name)
        litelm.default_internal_user_params = original_default_params
...
async def test_setup_team_mappings():
    ...
    mock_prisma.db.litelm_ssoconfig.find_unique = AsyncMock(
        return_value=mock_sso_config
    )

    with patch(
        "litelm.proxy.utils.get_prisma_client_or_throw",
        ...
        mock_prisma.db.litelm_ssoconfig.find_unique.assert_called_once_with(
            where={"id": "sso_config"}
        )


# ============================================================================
# Tests for get_litelm_user_role with list inputs (Keycloak returns lists)
# ============================================================================


def test_get_litelm_user_role_with_string():
    """Test that get_litelm_user_role works with a plain string."""
    from litelm.proxy._types import LitellmUserRoles
    from litelm.proxy.management_endpoints.types import get_litelm_user_role

    result = get_litelm_user_role("proxy_admin")
    assert result == LitellmUserRoles.PROXY_ADMIN


def test_get_litelm_user_role_with_list():
    """
    Test that get_litelm_user_role handles list inputs.
...
def test_get_litelm_user_role_with_empty_list():
    """Test that get_litelm_user_role returns None for empty lists."""
    from litelm.proxy.management_endpoints.types import get_litelm_user_role

    result = get_litelm_user_role([])
    assert result is None


def test_get_litelm_user_role_with_invalid_role():
    """Test that get_litelm_user_role returns None for invalid roles."""
    from litelm.proxy.management_endpoints.types import get_litelm_user_role

    result = get_litelm_user_role("not_a_real_role")
    assert result is None


def test_get_litelm_user_role_with_list_multiple_roles():
    """Test that get_litelm_user_role takes the first element from a multi-element list."""
    from litelm.proxy._types import LitellmUserRoles
    from litelm.proxy.management_endpoints.types import get_litelm_user_role

    result = get_litelm_user_role(["proxy_admin", "internal_user"])
    assert result == LitellmUserRoles.PROXY_ADMIN
...
def test_process_sso_jwt_access_token_extracts_role_from_access_token():
    ...
    # Call with GENERIC_USER_ROLE_ATTRIBUTE pointing to litelm_role
    with patch.dict(os.environ, {"GENERIC_USER_ROLE_ATTRIBUTE": "litelm_role"}):
        process_sso_jwt_access_token(
...
def test_process_sso_jwt_access_token_with_role_mappings():
    ...
    from litelm.proxy._types import LitellmUserRoles
    from litelm.types.proxy.management_endpoints.ui_sso import RoleMappings
