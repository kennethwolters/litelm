...
from litelm.proxy.management_endpoints.usage_endpoints.ai_usage_chat import (
    TOOL_HANDLERS,
    TOOLS_ADMIN,
    TOOLS_BASE,
...
class TestStreamUsageAiChat:
    ...
    async def test_stream_emits_status_events(self):
        ...
        with patch(
            "litelm.proxy.management_endpoints.usage_endpoints.ai_usage_chat.litelm"
        ) as mock_litelm, patch(
            "litelm.proxy.management_endpoints.usage_endpoints.ai_usage_chat._fetch_usage_data",
            new_callable=AsyncMock,
        ) as mock_fetch:
            mock_litelm.acompletion = AsyncMock(
                side_effect=[
                    mock_first_response,
                    mock_stream(),
                ]
    ...
    async def test_stream_handles_team_tool(self):
        ...
        with patch(
            ...
            "litelm.proxy.management_endpoints.usage_endpoints.ai_usage_chat._fetch_team_usage_data",
            new_callable=AsyncMock,
        ) as mock_fetch:
            mock_litelm.acompletion = AsyncMock(
                side_effect=[
    ...
    async def test_stream_handles_error(self):
        with patch(
            ...
            mock_litelm.acompletion = AsyncMock(side_effect=Exception("LLM error"))
    ...
    async def test_non_admin_enforces_user_id(self):
        ...
        with patch(
            "litelm.proxy.management_endpoints.usage_endpoints.ai_usage_chat.litelm"
        ) as mock_litelm, patch.dict(
            "litelm.proxy.management_endpoints.usage_endpoints.ai_usage_chat.TOOL_HANDLERS",
            {
                "get_usage_data": {
                    "fetch": mock_fetch,
