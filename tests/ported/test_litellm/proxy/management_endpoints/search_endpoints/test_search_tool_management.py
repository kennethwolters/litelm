...
async def test_list_search_tools_db_only(monkeypatch):
    ...
    db_tools = [
        {
            ...
            "litelm_params": {"search_provider": "perplexity", "api_key": "sk-test"},
    ...
    with patch(
        "litelm.proxy.search_endpoints.search_tool_management.SEARCH_TOOL_REGISTRY",
        mock_registry,
    ):
        # Mock prisma_client
        ...
        with patch("litelm.proxy.proxy_server.prisma_client", mock_prisma):
            ...
            with patch("litelm.proxy.proxy_server.proxy_config", mock_proxy_config):
                # Mock auth
                ...
                try:
                    ...
                    assert tool["litelm_params"]["api_key"] != "sk-test"
                    assert "****" in tool["litelm_params"]["api_key"]
                    assert tool["litelm_params"]["search_provider"] == "perplexity"
                finally:
    ...
    config_tools = [
        {
            ...
            "litelm_params": {"search_provider": "tavily", "api_key": "tvly-secret-key"},
        ...
        with patch("litelm.proxy.proxy_server.prisma_client", mock_prisma):
            ...
            with patch("litelm.proxy.proxy_server.proxy_config", mock_proxy_config):
                ...
                try:
                    ...
                    # Verify masking
                    assert "tv****ey" in tool["litelm_params"]["api_key"]
                finally:
                    app.dependency_overrides.pop(user_api_key_auth, None)
    ...
    db_tools = [
        {
            ...
            "litelm_params": {"search_provider": "perplexity", "api_key": "sk-db"},
    ...
    config_tools = [
        {
            ...
            "litelm_params": {"search_provider": "tavily", "api_key": "tvly-config"},
            "search_tool_info": {"description": "Config tool - duplicate"},
        },
        {
            "search_tool_name": "unique-config-tool",  # Unique - should be included
            "litelm_params": {"search_provider": "tavily", "api_key": "tvly-unique"},
        ...
        with patch("litelm.proxy.proxy_server.prisma_client", mock_prisma):
            ...
            with patch("litelm.proxy.proxy_server.proxy_config", mock_proxy_config):
                ...
                try:
                    ...
                    assert db_tool["litelm_params"]["api_key"] != "sk-db"
                    assert "****" in db_tool["litelm_params"]["api_key"]
                    assert db_tool["litelm_params"]["search_provider"] == "perplexity"
...
async def test_list_search_tools_datetime_conversion(monkeypatch):
    ...
    db_tools = [
        ...
        },
        {
            ...
            "litelm_params": {"search_provider": "tavily", "api_key": "tvly-test"},
        ...
        with patch("litelm.proxy.proxy_server.prisma_client", mock_prisma):
            ...
            with patch("litelm.proxy.proxy_server.proxy_config", mock_proxy_config):
                ...
                try:
                    ...
                    # Verify masking of sensitive values
                    assert tool1["litelm_params"]["api_key"] != "sk-test"
                    assert "****" in tool1["litelm_params"]["api_key"]
                    ...
                    # Verify masking of sensitive values
                    assert tool2["litelm_params"]["api_key"] != "tvly-test"
                    assert "****" in tool2["litelm_params"]["api_key"]
                    ...
                    # Verify masking of sensitive values
                    assert tool3["litelm_params"]["api_key"] != "sk-test"
                    assert "****" in tool3["litelm_params"]["api_key"]
                finally:
                    app.dependency_overrides.pop(user_api_key_auth, None)
        ...
        with patch("litelm.proxy.proxy_server.prisma_client", mock_prisma):
            ...
            with patch("litelm.proxy.proxy_server.proxy_config", mock_proxy_config):
                ...
                try:
                    ...
                    # Verify masking of sensitive values
                    assert data["search_tools"][0]["litelm_params"]["api_key"] != "sk-test"
                    assert "****" in data["search_tools"][0]["litelm_params"]["api_key"]
                finally:
                    app.dependency_overrides.pop(user_api_key_auth, None)
        ...
        with patch("litelm.proxy.proxy_server.prisma_client", mock_prisma):
            ...
            with patch("litelm.proxy.proxy_server.proxy_config", mock_proxy_config):
                ...
                try:
                    ...
                    assert tool1 is not None
                    assert tool1["litelm_params"]["api_key"] != "pplx-sk-1234567890abcdef"
                    assert "****" in tool1["litelm_params"]["api_key"]
                    assert tool1["litelm_params"]["search_provider"] == "perplexity"
                    assert tool1["litelm_params"]["api_base"] == "https://api.perplexity.ai"
                    ...
                    assert tool2 is not None
                    assert tool2["litelm_params"]["api_key"] != "tvly-secret-key-12345"
                    assert "****" in tool2["litelm_params"]["api_key"]
                    assert tool2["litelm_params"]["search_provider"] == "tavily"
                    ...
                    assert tool3 is not None
                    assert tool3["litelm_params"]["access_token"] != "token-abcdefghijklmnop"
                    assert "****" in tool3["litelm_params"]["access_token"]
                    assert tool3["litelm_params"]["secret_key"] != "secret-xyz123"
                    assert "****" in tool3["litelm_params"]["secret_key"]
                    ...
                    assert tool4["litelm_params"]["max_results"] == 10
                    assert tool4["litelm_params"]["timeout"] == 30
                    assert tool4["litelm_params"]["search_provider"] == "custom"
                finally:
                    app.dependency_overrides.pop(user_api_key_auth, None)
