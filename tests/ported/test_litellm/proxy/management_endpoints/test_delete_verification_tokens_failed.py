...
Related PR: https://github.com/BerriAI/litelm/pull/12577
...
def _mock_prisma(keys, deleted_tokens):
    ...
    mock = AsyncMock()
    mock.db.litelm_verificationtoken.find_many = AsyncMock(return_value=keys)
    mock.delete_data = AsyncMock(return_value=deleted_tokens)
    mock.db.litelm_deletedverificationtoken.create_many = AsyncMock()
    return mock
...
async def test_delete_tokens_non_admin_all_succeed_returns_empty_failed_tokens(
    ...
    monkeypatch.setattr(
        "litelm.proxy.management_endpoints.key_management_endpoints.can_modify_verification_token",
...
async def test_delete_tokens_non_admin_token_not_in_db_returns_failed_tokens(
    ...
    mock_prisma.db.litelm_verificationtoken.find_many = AsyncMock(return_value=[key1])
    mock_prisma.delete_data = AsyncMock(return_value=["hashed-token-1"])
    mock_prisma.db.litelm_deletedverificationtoken.create_many = AsyncMock()
