...
from litelm.proxy.management_endpoints.customer_endpoints import router
...
def mock_user_api_key_auth():
    with patch("litelm.proxy.proxy_server.user_api_key_auth") as mock:
...
def test_update_customer_success(mock_prisma_client, mock_user_api_key_auth):
    ...
    mock_prisma_client.db.litelm_endusertable.find_first = AsyncMock(
        return_value=mock_end_user
    )

    # Mock the update response
    mock_prisma_client.db.litelm_endusertable.update = AsyncMock(
...
def test_update_customer_not_found(mock_prisma_client, mock_user_api_key_auth):
    ...
    mock_prisma_client.db.litelm_endusertable.find_first = AsyncMock(return_value=None)
...
def test_delete_customer_not_found(mock_prisma_client, mock_user_api_key_auth):
    ...
    mock_prisma_client.db.litelm_endusertable.find_many = AsyncMock(return_value=[])
...
def test_error_schema_consistency(mock_prisma_client, mock_user_api_key_auth):
    ...
    mock_prisma_client.db.litelm_endusertable.create = AsyncMock(
...
async def test_get_customer_daily_activity_admin_param_passing(monkeypatch):
    ...
    from litelm.proxy.management_endpoints import customer_endpoints
    from litelm.proxy.management_endpoints.customer_endpoints import (
        get_customer_daily_activity,
    )

    mock_prisma_client = AsyncMock()
    mock_prisma_client.db.litelm_endusertable.find_many = AsyncMock(
        return_value=[]
    )
    ...
    assert kwargs["table_name"] == "litelm_dailyenduserspend"
