...
async def test_create_duplicate_access_group_fails():
    ...
    from litelm.proxy.management_endpoints.model_access_group_management_endpoints import (
        create_model_group,
    )
    from litelm.types.proxy.management_endpoints.model_management_endpoints import (
    ...
    mock_prisma.db.litelm_proxymodeltable.find_many = AsyncMock(
        return_value=[
            MagicMock(
                model_id="1",
                model_name="gpt-4",
    ...
    with patch("litelm.proxy.proxy_server.llm_router", mock_router), \
         patch("litelm.proxy.proxy_server.prisma_client", mock_prisma):
...
async def test_create_access_group_with_model_ids_tags_only_specific_deployments():
    ...
    Fixes: https://github.com/BerriAI/litelm/issues/21544
    """
    from litelm.proxy._types import LitellmUserRoles, UserAPIKeyAuth
    from litelm.proxy.management_endpoints.model_access_group_management_endpoints import (
        create_model_group,
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.litelm_proxymodeltable.find_many = AsyncMock(return_value=[])
    mock_prisma.db.litelm_proxymodeltable.find_unique = AsyncMock(return_value=deploy_a)
    mock_prisma.db.litelm_proxymodeltable.update = AsyncMock()
    ...
    with patch("litelm.proxy.proxy_server.llm_router", MagicMock()), \
         patch("litelm.proxy.proxy_server.prisma_client", mock_prisma), \
         patch(
             "litelm.proxy.management_endpoints.model_access_group_management_endpoints.clear_cache",
    ...
    mock_prisma.db.litelm_proxymodeltable.find_unique.assert_called_once_with(
        where={"model_id": "deploy-A"}
    )
    assert mock_prisma.db.litelm_proxymodeltable.update.call_count == 1
    update_call = mock_prisma.db.litelm_proxymodeltable.update.call_args
...
async def test_create_access_group_with_model_names_tags_all_deployments():
    ...
    mock_router = Router(
        model_list=[{"model_name": "gpt-4o", "litelm_params": {"model": "gpt-4o", "api_key": "fake-key"}}]
    )
    ...
    assert mock_prisma.db.litelm_proxymodeltable.update.call_count == 3
...
async def test_create_access_group_requires_model_names_or_model_ids():
    ...
    with patch("litelm.proxy.proxy_server.llm_router", MagicMock()), \
         patch("litelm.proxy.proxy_server.prisma_client", MagicMock()):
        with pytest.raises(HTTPException) as exc_info:
            await create_model_group(data=request_data, user_api_key_dict=mock_user)
...
async def test_create_access_group_invalid_model_id_returns_400():
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.litelm_proxymodeltable.find_many = AsyncMock(return_value=[])
    mock_prisma.db.litelm_proxymodeltable.find_unique = AsyncMock(return_value=None)
