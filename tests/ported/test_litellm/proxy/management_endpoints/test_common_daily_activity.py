...
async def test_get_daily_activity_empty_entity_id_list():
    ...
    mock_prisma.db.litelm_verificationtoken = MagicMock()
    mock_prisma.db.litelm_verificationtoken.find_many = AsyncMock(return_value=[])

    # Set the table name dynamically
    mock_prisma.db.litelm_dailyspend = mock_table

    # Call the function with empty entity_id list
    result = await get_daily_activity(
        prisma_client=mock_prisma,
        table_name="litelm_dailyspend",
...
async def test_get_api_key_metadata_returns_active_key_metadata():
    ...
    mock_prisma.db.litelm_verificationtoken.find_many = AsyncMock(
...
async def test_get_api_key_metadata_falls_back_to_deleted_keys():
    ...
    mock_prisma.db.litelm_deletedverificationtoken.find_many = AsyncMock(
    ...
    mock_prisma.db.litelm_deletedverificationtoken.find_many.assert_called_once_with(
...
async def test_get_api_key_metadata_deleted_table_not_queried_when_all_keys_found():
    ...
    mock_prisma.db.litelm_deletedverificationtoken = MagicMock()
    mock_prisma.db.litelm_deletedverificationtoken.find_many = AsyncMock(
        return_value=[]
    )
    ...
    mock_prisma.db.litelm_deletedverificationtoken.find_many.assert_not_called()
...
async def test_tag_daily_activity_metadata_totals_not_zero():
    ...
    NULL.  Rows in litelm_dailytagspend are pre-aggregated and always have
    ...
    mock_prisma.db.litelm_dailytagspend = mock_table
    mock_prisma.db.litelm_verificationtoken = MagicMock()
    mock_prisma.db.litelm_verificationtoken.find_many = AsyncMock(return_value=[])
