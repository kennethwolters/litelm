...
class MockPrismaClient:
    def __init__(self):
        self.db = MagicMock()
        self.config_data = {
            "litelm_settings": {"success_callback": ["langfuse"]},
        ...
        self.db.litelm_config.find_first = AsyncMock(
    ...
    async def _mock_find_first(self, where=None):
        ...
        if where and "param_name" in where:
            ...
            if param_name == "litelm_settings":
                return MagicMock(
                    param_name="litelm_settings",
                    param_value=self.config_data["litelm_settings"]
                )
    ...
    def remove_callback_from_config(self, callback_name):
        ...
        if "success_callback" in self.config_data["litelm_settings"]:
            callbacks = self.config_data["litelm_settings"]["success_callback"]
