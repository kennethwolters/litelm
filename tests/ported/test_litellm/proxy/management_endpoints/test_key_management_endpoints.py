...
from litelm.proxy.auth.user_api_key_auth import UserAPIKeyAuth
...
async def test_list_keys():
    ...
    assert json.dumps({"team_id": {"not": "litelm-dashboard"}}) in json.dumps(
        where_condition
    )
...
async def test_list_keys_include_created_by_keys():
    ...
    mock_prisma_client.db.litelm_verificationtoken.find_many = mock_find_many
    mock_prisma_client.db.litelm_verificationtoken.count = mock_count
...
async def test_key_token_handling(monkeypatch):
    ...
    mock_insert_data = AsyncMock(
        return_value=MagicMock(
            token="hashed_token_123", litelm_budget_table=None, object_permission=None
        )
    )
    mock_prisma_client.insert_data = mock_insert_data
    mock_prisma_client.db = MagicMock()
    mock_prisma_client.db.litelm_verificationtoken = MagicMock()
    mock_prisma_client.db.litelm_verificationtoken.find_unique = AsyncMock(
        return_value=None
    )
    mock_prisma_client.db.litelm_verificationtoken.find_many = AsyncMock(
        return_value=[]
    )
    mock_prisma_client.db.litelm_verificationtoken.count = AsyncMock(return_value=0)
    mock_prisma_client.db.litelm_verificationtoken.update = AsyncMock(
        return_value=MagicMock(
            token="hashed_token_123", litelm_budget_table=None, object_permission=None
        )
    )

    from litelm.proxy._types import GenerateKeyRequest, LitellmUserRoles
    from litelm.proxy.auth.user_api_key_auth import UserAPIKeyAuth
    from litelm.proxy.management_endpoints.key_management_endpoints import (
        generate_key_fn,
    )
    from litelm.proxy.proxy_server import prisma_client
...
async def test_budget_reset_and_expires_at_first_of_month(monkeypatch):
    ...
    mock_prisma_client = AsyncMock()
    mock_insert_data = AsyncMock(
        return_value=MagicMock(token="hashed_token_123", litelm_budget_table=None)
    )
    mock_prisma_client.insert_data = mock_insert_data
    mock_prisma_client.db = MagicMock()
    mock_prisma_client.db.litelm_verificationtoken = MagicMock()
...
async def test_key_generation_with_object_permission(monkeypatch):
    ...
    # Mock the prisma_client.db.litelm_objectpermissiontable.create call
    mock_object_permission_create = AsyncMock(
        return_value=MagicMock(object_permission_id="objperm123")
    )
    mock_prisma_client.db = MagicMock()
    mock_prisma_client.db.litelm_objectpermissiontable = MagicMock()
    mock_prisma_client.db.litelm_objectpermissiontable.create = (
...
async def test_generate_key_helper_fn_with_access_group_ids(monkeypatch):
    ...
    mock_prisma_client.db = MagicMock()
    mock_prisma_client.db.litelm_objectpermissiontable = MagicMock()
    mock_prisma_client.db.litelm_objectpermissiontable.create = AsyncMock(
...
async def test_key_generation_with_mcp_tool_permissions(monkeypatch):
    ...
    mock_prisma_client.db = MagicMock()
    mock_prisma_client.db.litelm_objectpermissiontable = MagicMock()
    mock_prisma_client.db.litelm_objectpermissiontable.create = mock_create
    ...
    monkeypatch.setattr("litelm.proxy.proxy_server.prisma_client", mock_prisma_client)
    monkeypatch.setattr(
        "litelm.proxy.management_endpoints.key_management_endpoints.validate_key_mcp_servers_against_team",
        AsyncMock(),
    )
...
async def test_key_info_returns_object_permission(monkeypatch):
    ...
    from litelm.proxy._types import LiteLLM_VerificationToken
    from litelm.proxy.management_endpoints.key_management_endpoints import info_key_fn
    ...
    mock_key_info.litelm_budget_table = None
...
def test_get_new_token_with_valid_key():
    ...
    from litelm.proxy._types import RegenerateKeyRequest
    from litelm.proxy.management_endpoints.key_management_endpoints import (
        get_new_token,
    )
...
async def test_generate_service_account_works_with_team_id():
    ...
    ) as mock_router, patch("litelm.proxy.proxy_server.premium_user", False), patch(
        "litelm.proxy.management_endpoints.key_management_endpoints.generate_key_helper_fn"
...
async def test_prepare_key_update_data_duration_never_expires():
    ...
    from litelm.proxy._types import UpdateKeyRequest
    from litelm.proxy.management_endpoints.key_management_endpoints import (
        prepare_key_update_data,
    )
...
async def test_generate_service_account_key_endpoint_validation():
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma:
        # Mock team not found
        mock_find_unique = AsyncMock(return_value=None)
        mock_prisma.db.litelm_teamtable.find_unique = mock_find_unique
...
async def test_unblock_key_supports_both_sk_and_hashed_tokens(monkeypatch):
    ...
    from litelm.proxy._types import BlockKeyRequest
    from litelm.proxy.management_endpoints.key_management_endpoints import unblock_key
    ...
    monkeypatch.setattr("litelm.proxy.proxy_server.prisma_client", mock_prisma_client)
    monkeypatch.setattr(
        "litelm.proxy.proxy_server.user_api_key_cache", mock_user_api_key_cache
    )
    monkeypatch.setattr(
        "litelm.proxy.proxy_server.proxy_logging_obj", mock_proxy_logging_obj
    )
    monkeypatch.setattr("litelm.proxy.proxy_server.hash_token", mock_hash_token)
    monkeypatch.setattr(
        "litelm.store_audit_logs", False
    ...
    monkeypatch.setattr(
        "litelm.proxy.management_endpoints.key_management_endpoints.get_key_object",
        mock_get_key_object,
    )
    monkeypatch.setattr(
        "litelm.proxy.management_endpoints.key_management_endpoints._cache_key_object",
    ...
    mock_prisma_client.db.litelm_verificationtoken.update.assert_called_with(
    ...
    mock_prisma_client.db.litelm_verificationtoken.update.reset_mock()
...
async def test_unblock_key_invalid_key_format(monkeypatch):
    ...
    from litelm.proxy._types import BlockKeyRequest
    from litelm.proxy.management_endpoints.key_management_endpoints import unblock_key
    from litelm.proxy.utils import ProxyException
...
async def test_validate_key_team_change_with_member_permissions():
    ...
    from litelm.proxy._types import KeyManagementRoutes
    ...
    with patch(
        "litelm.proxy.management_endpoints.key_management_endpoints.can_team_access_model",
        new_callable=AsyncMock,
    ):
        with patch(
            "litelm.proxy.management_endpoints.key_management_endpoints._get_user_in_team"
        ) as mock_get_user:
            with patch(
                "litelm.proxy.management_endpoints.key_management_endpoints._is_user_team_admin"
            ) as mock_is_admin:
                with patch(
                    "litelm.proxy.management_endpoints.key_management_endpoints.TeamMemberPermissionChecks.does_team_member_have_permissions_for_endpoint"
                ) as mock_has_perms:
...
async def test_update_key_fn_auto_rotate_enable():
    ...
    from litelm.proxy._types import LiteLLM_VerificationToken, UpdateKeyRequest
    from litelm.proxy.management_endpoints.key_management_endpoints import (
        prepare_key_update_data,
    )
...
async def test_check_team_key_limits_no_existing_keys():
    ...
    mock_prisma_client.db.litelm_verificationtoken.find_many.assert_called_once_with(
...
async def test_generate_key_with_object_permission():
    ...
    mock_prisma_client.db.litelm_objectpermissiontable = MagicMock()
    mock_prisma_client.db.litelm_objectpermissiontable.create = mock_object_perm_create
    ...
    with patch(
        ...
        mock_prisma_client,
    ), patch("litelm.proxy.proxy_server.llm_router", None), patch(
...
async def test_check_org_key_limits_no_existing_keys():
    ...
    org_table = LiteLLM_OrganizationTable(
        ...
        created_by="admin",
        updated_by="admin",
        litelm_budget_table=LiteLLM_BudgetTable(
            budget_id="budget-123",
            tpm_limit=20000,
            rpm_limit=2000,
        ),
...
def test_transform_verification_tokens_to_deleted_records():
    ...
    assert "litelm_budget_table" not in record1
    assert "litelm_organization_table" not in record1
...
async def test_delete_verification_tokens_persists_deleted_keys(monkeypatch):
    ...
    monkeypatch.setattr(
        "litelm.proxy.management_endpoints.key_management_endpoints._hash_token_if_needed",
        mock_hash_token,
    )
    monkeypatch.setattr(
        "litelm.proxy.management_endpoints.key_management_endpoints.hash_token",
        mock_hash_token,
    )
    monkeypatch.setattr(
        "litelm.proxy.proxy_server.prisma_client",
...
async def test_delete_key_fn_persists_deleted_keys(monkeypatch):
    from litelm.proxy._types import KeyRequest
    from litelm.proxy.management_endpoints.key_management_endpoints import (
        delete_key_fn,
        delete_verification_tokens,
    )
    ...
    monkeypatch.setattr(
        "litelm.proxy.management_endpoints.key_management_endpoints.delete_verification_tokens",
    ...
    monkeypatch.setattr(
        "litelm.proxy.management_endpoints.key_management_endpoints.KeyManagementEventHooks.async_key_deleted_hook",
        AsyncMock(),
    )
...
async def test_can_delete_verification_token_proxy_admin_team_key(monkeypatch):
    ...
    monkeypatch.setattr(
        "litelm.proxy.management_endpoints.key_management_endpoints.get_team_object",
...
async def test_list_keys_with_expand_user():
    ...
    mock_prisma_client.db.litelm_verificationtoken.count = mock_count_keys
    mock_prisma_client.db.litelm_usertable.find_many = mock_find_many_users
    ...
    with patch(
        "litelm.proxy.management_endpoints.key_management_endpoints.attach_object_permission_to_dict",
...
async def test_list_keys_with_status_deleted():
    ...
    mock_prisma_client.db.litelm_deletedverificationtoken.find_many = mock_find_many_deleted
    mock_prisma_client.db.litelm_deletedverificationtoken.count = mock_count_deleted
    mock_prisma_client.db.litelm_verificationtoken.find_many = mock_find_many_regular
    mock_prisma_client.db.litelm_verificationtoken.count = mock_count_regular
...
async def test_list_keys_with_invalid_status():
    ...
    from litelm.proxy.management_endpoints.key_management_endpoints import list_keys
    from litelm.proxy.utils import ProxyException
...
async def test_list_keys_non_admin_user_id_auto_set():
    ...
    with patch("litelm.proxy.proxy_server.prisma_client", mock_prisma_client):
        with patch(
            "litelm.proxy.management_endpoints.key_management_endpoints.validate_key_list_check",
            return_value=mock_user_info,
        ):
            with patch(
                "litelm.proxy.management_endpoints.key_management_endpoints.get_admin_team_ids",
                return_value=[],
            ):
                with patch(
...
async def test_process_single_key_update():
    ...
    """
    from litelm.types.proxy.management_endpoints.key_management_endpoints import (
    ...
    with patch(
        "litelm.proxy.management_endpoints.key_management_endpoints.prepare_key_update_data"
    ) as mock_prepare:
        mock_prepare.return_value = {"max_budget": 100.0, "tags": ["production"]}
        
        # Mock TeamMemberPermissionChecks
        with patch(
            "litelm.proxy.management_endpoints.key_management_endpoints.TeamMemberPermissionChecks.can_team_member_execute_key_management_endpoint"
        ) as mock_permission_check:
            mock_permission_check.return_value = None
            
            # Mock _delete_cache_key_object
            with patch(
                "litelm.proxy.management_endpoints.key_management_endpoints._delete_cache_key_object"
            ) as mock_delete_cache:
                mock_delete_cache.return_value = None
                
                # Mock hash_token (imported from litelm.proxy._types)
                with patch(
                    "litelm.proxy._types.hash_token"
                ) as mock_hash:
                    mock_hash.return_value = "hashed-test-key-123"
                    
                    # Mock KeyManagementEventHooks
                    with patch(
                        "litelm.proxy.management_endpoints.key_management_endpoints.KeyManagementEventHooks.async_key_updated_hook"
                    ):
...
async def test_bulk_update_keys_success(monkeypatch):
    ...
    from litelm.proxy.proxy_server import (
        llm_router,
        prisma_client,
        proxy_logging_obj,
        user_api_key_cache,
    ...
    monkeypatch.setattr("litelm.proxy.proxy_server.llm_router", mock_llm_router)
...
async def test_reset_key_spend_success(monkeypatch):
    ...
    with patch(
        "litelm.proxy.proxy_server.hash_token"
    ) as mock_hash_token, patch(
        "litelm.proxy.management_endpoints.key_management_endpoints._check_proxy_or_team_admin_for_key"
        ...
        mock_prisma_client.db.litelm_verificationtoken.update.assert_called_once()
...
async def test_reset_key_spend_key_not_found(monkeypatch):
    ...
    with patch("litelm.proxy.proxy_server.hash_token") as mock_hash_token:
...
async def test_reset_key_spend_authorization_failure(monkeypatch):
    ...
    with patch("litelm.proxy.proxy_server.hash_token") as mock_hash_token, patch(
        "litelm.proxy.management_endpoints.key_management_endpoints._check_proxy_or_team_admin_for_key"
    ) as mock_check_admin:
        mock_hash_token.return_value = hashed_key
...
async def test_reset_key_spend_hashed_key(monkeypatch):
    ...
    with patch(
        ...
        assert response["spend"] == 50.0
        mock_prisma_client.db.litelm_verificationtoken.find_unique.assert_called_once_with(
            where={"token": hashed_key}, include={"litelm_budget_table": True}
        )
...
async def test_validate_key_list_check_team_admin_success():
    ...
    mock_prisma_client.db.litelm_usertable.find_unique = AsyncMock(
...
async def test_validate_key_list_check_key_hash_authorized():
    ...
    with patch(
        "litelm.proxy.management_endpoints.key_management_endpoints._can_user_query_key_info"
    ) as mock_can_query:
...
async def test_key_with_budget_id_does_not_store_budget_duration():
    ...
    with patch(
        "litelm.proxy.proxy_server.prisma_client", mock_prisma
    ), patch(
        "litelm.proxy.proxy_server.llm_router", None
    ), patch(
        "litelm.proxy.proxy_server.premium_user", False
    ), patch(
    ...
    mock_prisma.db.litelm_budgettable.find_unique.assert_not_called()
...
async def test_key_does_not_override_explicit_budget_duration():
    ...
    mock_prisma.db.litelm_budgettable.find_unique = AsyncMock(
...
@pytest.mark.asyncio
@patch(
    "litelm.proxy.management_endpoints.key_management_endpoints.rotate_mcp_server_credentials_master_key"
)
async def test_rotate_master_key_model_data_valid_for_prisma(
    ...
    litelm_params/model_info are JSON strings (create_many expects dicts).
    """
    from unittest.mock import AsyncMock, MagicMock
    ...
    mock_model.litelm_params = '{"model": "openai/gpt-4", "api_key": "sk-encrypted-old"}'
    mock_model.model_info = '{"id": "model-1"}'
    mock_model.created_by = "admin"
    mock_model.updated_by = "admin"
    mock_prisma_client.db.litelm_proxymodeltable.find_many = AsyncMock(
        return_value=[mock_model]
    )

    # Mock transaction context manager
    mock_tx = AsyncMock()
    mock_tx.litelm_proxymodeltable = MagicMock()
    mock_tx.litelm_proxymodeltable.delete_many = AsyncMock()
    mock_tx.litelm_proxymodeltable.create_many = AsyncMock()
    ...
    mock_prisma_client.db.litelm_config.find_many = AsyncMock(return_value=[])

    # Mock credentials table — no credentials
    mock_prisma_client.db.litelm_credentialstable.find_many = AsyncMock(
        return_value=[]
    )
    ...
    with patch(
        "litelm.proxy.proxy_server.proxy_config",
        mock_proxy_config,
    ):
    ...
    mock_tx.litelm_proxymodeltable.create_many.assert_called_once()

    # Get the data passed to create_many
    call_args = mock_tx.litelm_proxymodeltable.create_many.call_args
    ...
    # Verify litelm_params and model_info are prisma.Json wrappers, NOT JSON strings
    import prisma

    assert isinstance(model_data["litelm_params"], prisma.Json), (
        f"litelm_params should be prisma.Json for create_many(), got {type(model_data['litelm_params'])}"
    )
    ...
    mock_tx.litelm_proxymodeltable.delete_many.assert_called_once()
async def test_default_key_generate_params_duration(monkeypatch):
    ...
    original_value = litelm.default_key_generate_params
    litelm.default_key_generate_params = {"duration": "180d"}
    ...
    try:
        ...
        litelm.default_key_generate_params = original_value
...
async def test_get_member_team_ids():
    ...
    mock_prisma_client.db.litelm_teamtable.find_many = AsyncMock(
...
async def test_generate_key_with_agent_id():
    ...
    from litelm.proxy._types import GenerateKeyRequest
...
async def test_generate_key_helper_fn_agent_id():
    ...
    import litelm.proxy.management_endpoints.key_management_endpoints as km
...
class TestValidateKeyAliasFormat:
    ...
    def reset_key_alias_flag(self):
        litelm.enable_key_alias_format_validation = False
        yield
        litelm.enable_key_alias_format_validation = False
    ...
    def test_validation_skipped_when_flag_disabled(self):
        ...
        from litelm.proxy.management_endpoints.key_management_endpoints import _validate_key_alias_format
    ...
    def test_validate_key_alias_format_valid(self):
        ...
        litelm.enable_key_alias_format_validation = True
        # Valid cases
    ...
    def test_validate_key_alias_format_invalid(self):
        from litelm.proxy.management_endpoints.key_management_endpoints import _validate_key_alias_format
        from litelm.proxy._types import ProxyException
