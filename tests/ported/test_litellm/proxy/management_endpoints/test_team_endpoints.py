...
from litelm._uuid import uuid

sys.path.insert(
    0, os.path.abspath("../../../")
)  # Adds the parent directory to the system path
from litelm.proxy._types import UserAPIKeyAuth  # Import UserAPIKeyAuth
...
from litelm.proxy.management_endpoints.team_endpoints import (
...
from litelm.proxy.management_helpers.team_member_permission_checks import (
    TeamMemberPermissionChecks,
)
from litelm.proxy.proxy_server import app
from litelm.router import Router
from litelm.types.proxy.management_endpoints.team_endpoints import (
...
mock_prisma_client.db.litelm_teamtable = MagicMock()
mock_prisma_client.db.litelm_teamtable.update = AsyncMock()
...
def mock_db_client():
    with patch(
        "litelm.proxy.proxy_server.prisma_client", mock_prisma_client
...
async def test_validate_team_org_change_same_org_id():
    ...
    organization.models = []
    organization.litelm_budget_table = MagicMock()
    organization.litelm_budget_table.max_budget = (
        50.0  # This would normally fail validation
    )
    organization.litelm_budget_table.tpm_limit = (
        500  # This would normally fail validation
    )
    organization.litelm_budget_table.rpm_limit = (
    ...
    with patch(
        "litelm.proxy.management_endpoints.team_endpoints.can_org_access_model"
...
async def test_validate_team_org_change_members_in_org():
    ...
    organization.models = []
    organization.litelm_budget_table = None
...
async def test_get_team_permissions_list_success(mock_db_client, mock_admin_auth):
    ...
    with patch(
        "litelm.proxy.management_endpoints.team_endpoints.get_team_object",
...
async def test_update_team_permissions_success(mock_db_client, mock_admin_auth):
    ...
    with patch(
        ...
        mock_db_client.db.litelm_teamtable.update = AsyncMock(
        ...
        mock_db_client.db.litelm_teamtable.update.assert_awaited_once_with(
...
async def test_new_team_with_object_permission(mock_db_client, mock_admin_auth):
    ...
    1. Creating a record in litelm_objectpermissiontable
    ...
    mock_db_client.db.litelm_objectpermissiontable = MagicMock()
    mock_db_client.db.litelm_objectpermissiontable.create = mock_object_perm_create

    # Mock model table creation
    mock_db_client.db.litelm_modeltable = MagicMock()
    mock_db_client.db.litelm_modeltable.create = AsyncMock(
    ...
    mock_db_client.db.litelm_teamtable = MagicMock()
    mock_db_client.db.litelm_teamtable.create = mock_team_create
    mock_db_client.db.litelm_teamtable.count = mock_team_count
    mock_db_client.db.litelm_teamtable.update = AsyncMock(
        return_value=team_create_result
    )

    # Mock user table
    mock_db_client.db.litelm_usertable = MagicMock()
    mock_db_client.db.litelm_usertable.update = AsyncMock(return_value=MagicMock())

    from fastapi import Request

    from litelm.proxy._types import LiteLLM_ObjectPermissionBase, NewTeamRequest
    from litelm.proxy.management_endpoints.team_endpoints import new_team
...
async def test_new_team_with_mcp_tool_permissions(mock_db_client, mock_admin_auth):
    ...
    mock_db_client.db.litelm_objectpermissiontable = MagicMock()
    mock_db_client.db.litelm_objectpermissiontable.create = mock_obj_perm_create
    ...
    mock_db_client.db.litelm_teamtable = MagicMock()
    mock_db_client.db.litelm_teamtable.create = AsyncMock(return_value=team_create_result)
    mock_db_client.db.litelm_teamtable.count = AsyncMock(return_value=0)
    mock_db_client.db.litelm_teamtable.update = AsyncMock(return_value=team_create_result)
...
async def test_team_update_object_permissions_existing_permission(monkeypatch):
    ...
    from litelm.proxy._types import LiteLLM_ObjectPermissionBase, LiteLLM_TeamTable
    from litelm.proxy.management_endpoints.team_endpoints import (
    ...
    monkeypatch.setattr("litelm.proxy.proxy_server.prisma_client", mock_prisma_client)
    ...
    mock_prisma_client.db.litelm_objectpermissiontable.find_unique = AsyncMock(
    ...
    mock_prisma_client.db.litelm_objectpermissiontable.upsert = AsyncMock(
    ...
    mock_prisma_client.db.litelm_objectpermissiontable.find_unique.assert_called_once_with(
        where={"object_permission_id": "existing_perm_id_123"}
    )
    mock_prisma_client.db.litelm_objectpermissiontable.upsert.assert_called_once()
...
async def test_add_team_member_budget_table_success():
    ...
    """
    from litelm.proxy._types import TeamInfoResponseObjectTeamTable
    from litelm.proxy.management_endpoints.team_endpoints import (
    ...
    mock_prisma_client.db.litelm_budgettable.find_unique = AsyncMock(
    ...
    mock_prisma_client.db.litelm_budgettable.find_unique.assert_called_once_with(
...
async def test_add_team_member_budget_table_exception_handling():
    ...
    with patch(
        "litelm.proxy.management_endpoints.team_endpoints.verbose_proxy_logger"
    ) as mock_logger:
        # Call the function
        result = await _add_team_member_budget_table(
...
async def test_add_team_member_budget_table_budget_not_found():
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.litelm_budgettable.find_unique = AsyncMock(return_value=None)
...
def test_add_new_models_to_team():
    ...
    """
    from litelm.proxy._types import SpecialModelNames
    from litelm.proxy.management_endpoints.team_endpoints import add_new_models_to_team
...
async def test_validate_team_member_add_permissions_non_admin():
    ...
    with patch(
        "litelm.proxy.management_endpoints.team_endpoints._is_user_team_admin",
        return_value=False,
    ), patch(
        "litelm.proxy.management_endpoints.team_endpoints._is_available_team",
        return_value=False,
    ):
...
async def test_process_team_members_single_member():
    ...
    """
    from litelm.proxy._types import LiteLLM_TeamMembership, LiteLLM_UserTable
    from litelm.proxy.management_endpoints.team_endpoints import _process_team_members
    ...
    with patch(
        "litelm.proxy.management_endpoints.team_endpoints.add_new_member",
        new_callable=AsyncMock,
...
async def test_update_team_members_list_single_member():
    ...
    """
    from litelm.proxy._types import LiteLLM_UserTable
    from litelm.proxy.management_endpoints.team_endpoints import (
...
async def test_update_team_team_member_budget_not_passed_to_db():
    """
    Test that 'team_member_budget' is never passed to prisma_client.db.litelm_teamtable.update
    ...
    from litelm.proxy._types import LitellmUserRoles, UpdateTeamRequest, UserAPIKeyAuth
    from litelm.proxy.management_endpoints.team_endpoints import update_team
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma_client, patch(
        "litelm.proxy.proxy_server.llm_router"
    ) as mock_llm_router, patch(
        "litelm.proxy.proxy_server.user_api_key_cache"
    ) as mock_cache, patch(
        "litelm.proxy.proxy_server.proxy_logging_obj"
    ) as mock_logging, patch(
        "litelm.proxy.proxy_server.litelm_proxy_admin_name", "admin"
    ), patch(
        "litelm.proxy.auth.auth_checks._cache_team_object"
    ) as mock_cache_team, patch(
        "litelm.proxy.management_endpoints.team_endpoints.TeamMemberBudgetHandler.upsert_team_member_budget_table"
        ...
        mock_prisma_client.db.litelm_teamtable.find_unique = AsyncMock(
        ...
        assert mock_prisma_client.db.litelm_teamtable.update.called

        # Get the call arguments
        call_args = mock_prisma_client.db.litelm_teamtable.update.call_args
        ...
        mock_prisma_client.db.litelm_teamtable.update.reset_mock()
...
async def test_create_team_member_budget_table():
    ...
    from litelm.proxy._types import LitellmUserRoles, NewTeamRequest, UserAPIKeyAuth
    from litelm.proxy.management_endpoints.team_endpoints import (
        TeamMemberBudgetHandler,
    )
    ...
    with patch(
        "litelm.proxy.management_endpoints.budget_management_endpoints.new_budget",
        new_callable=AsyncMock
    ) as mock_new_budget:
...
async def test_upsert_team_member_budget_table_existing_budget():
    ...
    from litelm.proxy._types import LitellmUserRoles, LiteLLM_TeamTable, UserAPIKeyAuth
    from litelm.proxy.management_endpoints.team_endpoints import (
        TeamMemberBudgetHandler,
    )
    ...
    with patch(
        "litelm.proxy.management_endpoints.budget_management_endpoints.update_budget",
        new_callable=AsyncMock
...
async def test_update_team_with_team_member_budget_duration():
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma_client, patch(
        ...
        assert mock_prisma_client.db.litelm_teamtable.update.called
        update_call_args = mock_prisma_client.db.litelm_teamtable.update.call_args
...
async def test_bulk_team_member_add_success():
    ...
    from litelm.proxy.management_endpoints.team_endpoints import bulk_team_member_add
    ...
    with patch(
        "litelm.proxy.management_endpoints.team_endpoints.team_member_add",
        new_callable=AsyncMock,
...
async def test_bulk_team_member_add_all_users_flag():
    ...
    """
    from litelm.proxy._types import LiteLLM_UserTable, TeamAddMemberResponse
    from litelm.proxy.management_endpoints.team_endpoints import bulk_team_member_add
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        ...
        mock_prisma.db.litelm_usertable.find_many = AsyncMock(
        ...
        mock_prisma.db.litelm_usertable.find_many.assert_called_once_with(
...
async def test_bulk_team_member_add_no_db_connection():
    ...
    with patch("litelm.proxy.proxy_server.prisma_client", None):
        mock_auth = UserAPIKeyAuth(user_role=LitellmUserRoles.PROXY_ADMIN)
...
async def test_list_team_v2_security_check_non_admin_user():
    ...
    from litelm.proxy._types import LitellmUserRoles, UserAPIKeyAuth
    from litelm.proxy.management_endpoints.team_endpoints import list_team_v2
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma_client:
...
async def test_list_team_v2_security_check_non_admin_user_own_teams():
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma_client:
        ...
        mock_db.litelm_usertable.find_unique = AsyncMock(return_value=mock_user_object)
        ...
        mock_db.litelm_teamtable.find_many = AsyncMock(return_value=mock_teams)
        mock_db.litelm_teamtable.count = AsyncMock(return_value=2)
...
async def test_list_team_v2_with_status_deleted():
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma_client:
        ...
        mock_db.litelm_deletedteamtable.find_many = AsyncMock(return_value=[mock_deleted_team1, mock_deleted_team2])
        mock_db.litelm_deletedteamtable.count = AsyncMock(return_value=2)
        
        # Mock regular teams table (should NOT be called)
        mock_db.litelm_teamtable.find_many = AsyncMock(return_value=[])
        mock_db.litelm_teamtable.count = AsyncMock(return_value=0)
        ...
        mock_db.litelm_deletedteamtable.find_many.assert_called_once()
        mock_db.litelm_deletedteamtable.count.assert_called_once()
        
        # Verify that regular table was NOT queried
        mock_db.litelm_teamtable.find_many.assert_not_called()
        mock_db.litelm_teamtable.count.assert_not_called()
...
async def test_list_team_v2_with_invalid_status():
    ...
    with patch("litelm.proxy.proxy_server.prisma_client", mock_prisma_client):
...
async def test_team_member_delete_cleans_membership(mock_db_client, mock_admin_auth):
    ...
    """
    from litelm.proxy._types import TeamMemberDeleteRequest
    from litelm.proxy.management_endpoints.team_endpoints import team_member_delete
    ...
    mock_db_client.db.litelm_teamtable.find_unique = AsyncMock(return_value=mock_team_row)
    mock_db_client.db.litelm_teamtable.update = AsyncMock(return_value=mock_team_row)

    # User row to allow removal from user's teams list
    mock_user_row = MagicMock()
    mock_user_row.user_id = test_user_id
    mock_user_row.teams = [test_team_id]
    mock_db_client.db.litelm_usertable.find_many = AsyncMock(return_value=[mock_user_row])
    mock_db_client.db.litelm_usertable.update = AsyncMock(return_value=MagicMock())

    # Membership deletion should be called
    mock_db_client.db.litelm_teammembership = MagicMock()
    mock_db_client.db.litelm_teammembership.delete_many = AsyncMock(return_value=MagicMock())

    # Verification token deletion should be called
    mock_db_client.db.litelm_verificationtoken = MagicMock()
    mock_db_client.db.litelm_verificationtoken.find_many = AsyncMock(return_value=[])
    mock_db_client.db.litelm_verificationtoken.delete_many = AsyncMock(return_value=MagicMock())
    ...
    mock_db_client.db.litelm_teammembership.delete_many.assert_awaited_with(
...
async def test_team_member_delete_cleans_verification_tokens(mock_db_client, mock_admin_auth):
    ...
    mock_db_client.db.litelm_verificationtoken.delete_many.assert_awaited_once_with(
        where={
...
async def test_new_team_max_budget_exceeds_user_max_budget():
    ...
    from litelm.proxy._types import NewTeamRequest, ProxyException, UserAPIKeyAuth
    from litelm.proxy.management_endpoints.team_endpoints import new_team
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        "litelm.proxy.proxy_server._license_check"
    ) as mock_license, patch(
        "litelm.proxy.proxy_server.user_api_key_cache"
    ) as mock_cache, patch(
        "litelm.proxy.proxy_server.litelm_proxy_admin_name", "admin"
    ), patch(
        "litelm.proxy.proxy_server.create_audit_log_for_update", new=AsyncMock()
    ) as mock_audit:
        # Setup basic mocks
        mock_prisma.db.litelm_teamtable.count = AsyncMock(return_value=0)
...
async def test_new_team_max_budget_within_user_limit():
    ...
    from litelm.proxy._types import NewTeamRequest, UserAPIKeyAuth
    from litelm.proxy.management_endpoints.team_endpoints import new_team
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        ...
        mock_prisma.db.litelm_teamtable.create = AsyncMock(return_value=mock_created_team)
        mock_prisma.db.litelm_teamtable.update = AsyncMock(return_value=mock_created_team)
        
        # Mock model table
        mock_prisma.db.litelm_modeltable = MagicMock()
        mock_prisma.db.litelm_modeltable.create = AsyncMock(return_value=MagicMock(id="model123"))
        ...
        mock_prisma.db.litelm_usertable = MagicMock()
        mock_prisma.db.litelm_usertable.upsert = AsyncMock(return_value=mock_user)
        mock_prisma.db.litelm_usertable.update = AsyncMock(return_value=mock_user)
        ...
        mock_prisma.db.litelm_teammembership = MagicMock()
        mock_prisma.db.litelm_teammembership.create = AsyncMock(return_value=mock_membership)
...
async def test_new_team_org_scoped_budget_bypasses_user_limit():
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        ...
        "litelm.proxy.management_endpoints.team_endpoints.get_org_object"
    ) as mock_get_org:
        ...
        mock_org.litelm_budget_table = None  # No budget table for this test
...
async def test_new_team_org_scoped_models_bypasses_user_limit():
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        ...
        mock_org.max_budget = 100.0
        mock_org.models = ["gpt-4", "gpt-3.5-turbo", "claude-3-opus"]
        mock_org.litelm_budget_table = None
        mock_get_org.return_value = mock_org
...
async def test_new_team_org_scoped_budget_exceeds_org_limit():
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        ...
        mock_org.litelm_budget_table = mock_budget_table
        mock_get_org.return_value = mock_org
...
async def test_update_team_standalone_budget_exceeds_user_limit():
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        ...
        mock_prisma.db.litelm_teamtable.find_unique = AsyncMock(return_value=mock_existing_team)
...
async def test_update_team_standalone_models_exceeds_user_limit():
    ...
    from litelm.proxy._types import ProxyException, UpdateTeamRequest, UserAPIKeyAuth
    from litelm.proxy.management_endpoints.team_endpoints import update_team
...
async def test_update_team_org_scoped_budget_bypasses_user_limit():
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        ...
        mock_updated_team.litelm_model_table = None
        mock_updated_team.model_dump.return_value = {
            "team_id": "org-team-update-budget-123",
            "organization_id": "test-org-update-budget",
            "max_budget": 50.0,
        }
        mock_prisma.db.litelm_teamtable.update = AsyncMock(return_value=mock_updated_team)
...
async def test_new_team_org_scoped_tpm_rpm_bypasses_user_limit():
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        ...
        "litelm.proxy.management_endpoints.team_endpoints._add_team_members_to_team",
        new=AsyncMock()
    ):
        mock_license.is_team_count_over_limit.return_value = False
        mock_prisma.db.litelm_teamtable.count = AsyncMock(return_value=0)
...
async def test_update_team_guardrails_with_org_id():
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        ...
        "litelm.proxy.proxy_server.premium_user", True  # Required for guardrails feature
    ):
        ...
        mock_prisma.db.litelm_teamtable.find_unique = AsyncMock(
            return_value=mock_existing_team
        )
        mock_cache.async_set_cache = AsyncMock()
        ...
        mock_prisma.db.litelm_organizationtable.find_unique = AsyncMock(
        ...
        mock_prisma.db.litelm_teamtable.update = AsyncMock(
            return_value=mock_updated_team
        )
        mock_prisma.jsonify_team_object = MagicMock(side_effect=lambda db_data: db_data)
        ...
        with patch("litelm.proxy.proxy_server.llm_router", mock_router):
            ...
            assert mock_prisma.db.litelm_organizationtable.find_unique.call_count >= 1
            
            # Get the first call (from fetch_and_validate_organization)
            first_call_kwargs = mock_prisma.db.litelm_organizationtable.find_unique.call_args_list[0].kwargs
...
def test_transform_teams_to_deleted_records():
    ...
    assert all("litelm_changed_by" in record for record in records)
...
async def test_save_deleted_team_records():
    ...
    mock_prisma_client.db.litelm_deletedteamtable.create_many = mock_create_many
...
async def test_persist_deleted_team_records():
    ...
    assert records[0]["litelm_changed_by"] == "admin-user"


@pytest.mark.asyncio
async def test_delete_team_persists_deleted_teams(monkeypatch):
    from litelm.proxy._types import DeleteTeamRequest
    ...
    mock_prisma_client.db.litelm_teamtable.find_unique = mock_find_unique
    ...
    mock_prisma_client.db.litelm_deletedteamtable.create_many = mock_create_many_teams

    mock_create_many_keys = AsyncMock()
    mock_prisma_client.db.litelm_deletedverificationtoken.create_many = (
        mock_create_many_keys
    )

    mock_find_many_keys = AsyncMock(return_value=[])
    mock_prisma_client.db.litelm_verificationtoken.find_many = mock_find_many_keys

    monkeypatch.setattr(
        "litelm.proxy.proxy_server.prisma_client",
        mock_prisma_client,
    )
    monkeypatch.setattr(
        "litelm.proxy.proxy_server.create_audit_log_for_update",
        AsyncMock(),
    )
    monkeypatch.setattr(
        "litelm.proxy.proxy_server.litelm_proxy_admin_name",
        "admin",
    )
    monkeypatch.setattr(
        "litelm.proxy.management_endpoints.team_endpoints.team_member_delete",
...
async def test_team_member_delete_persists_deleted_keys(monkeypatch):
    ...
    mock_prisma_client.db.litelm_teamtable.find_unique = mock_find_unique_team
    ...
    mock_prisma_client.db.litelm_usertable.find_many = mock_find_many_user

    mock_update_team = AsyncMock()
    mock_prisma_client.db.litelm_teamtable.update = mock_update_team

    mock_update_user = AsyncMock()
    mock_prisma_client.db.litelm_usertable.update = mock_update_user

    mock_delete_membership = AsyncMock()
    mock_prisma_client.db.litelm_teammembership.delete_many = mock_delete_membership

    mock_find_many_keys = AsyncMock(return_value=[key1, key2])
    mock_prisma_client.db.litelm_verificationtoken.find_many = mock_find_many_keys

    mock_delete_keys = AsyncMock()
    mock_prisma_client.db.litelm_verificationtoken.delete_many = mock_delete_keys
...
async def test_new_team_negative_max_budget():
    ...
    """
    from litelm.proxy._types import NewTeamRequest
...
async def test_update_team_negative_max_budget():
    ...
    Validation is done at API level, not model level.
    """
    from litelm.proxy._types import UpdateTeamRequest
...
async def test_get_team_daily_activity_non_admin_filters_by_user_api_keys(
    ...
    # Setup mocks
    mock_db_client.db.litelm_teamtable.find_many = AsyncMock(
        return_value=[mock_team]
    )
    mock_db_client.db.litelm_verificationtoken.find_many = AsyncMock(
        return_value=[user_api_key_1, user_api_key_2]
    )

    # Mock get_user_object
    with patch(
        "litelm.proxy.management_endpoints.team_endpoints.get_user_object",
        new_callable=AsyncMock,
        ...
        with patch(
            "litelm.proxy.management_endpoints.team_endpoints.get_daily_activity",
            new_callable=AsyncMock,
            ...
            mock_db_client.db.litelm_verificationtoken.find_many.assert_called_once()
            api_key_call_kwargs = (
                mock_db_client.db.litelm_verificationtoken.find_many.call_args[1]
            )
...
async def test_get_team_daily_activity_team_admin_sees_all_spend(mock_db_client):
    ...
    with patch(
        ...
        with patch(
            ...
            if hasattr(
                mock_db_client.db.litelm_verificationtoken, "find_many"
            ) and mock_db_client.db.litelm_verificationtoken.find_many.called:
...
async def test_update_team_with_router_settings(mock_db_client, mock_admin_auth):
    ...
    mock_db_client.db.litelm_teamtable = MagicMock()
    mock_db_client.db.litelm_teamtable.find_unique = mock_team_find_unique
    mock_db_client.db.litelm_teamtable.update = mock_team_update
...
async def test_validate_and_populate_member_user_info_only_email_provided():
    ...
    mock_prisma_client.db.litelm_usertable.find_first = AsyncMock(
    ...
    mock_prisma_client.db.litelm_usertable.find_first.assert_called_once_with(
...
async def test_validate_and_populate_member_user_info_only_user_id_not_found():
    ...
    mock_prisma_client.db.litelm_usertable.find_unique = AsyncMock(return_value=None)
    ...
    mock_prisma_client.db.litelm_usertable.find_unique.assert_called_once_with(
...
async def test_list_available_teams_returns_empty_list_when_none_configured():
    ...
    with patch(
        ...
        original = litelm.default_internal_user_params
        litelm.default_internal_user_params = None
        ...
        litelm.default_internal_user_params = {"some_other_param": "value"}
        result = await list_available_teams(
            http_request=mock_request,
            user_api_key_dict=mock_user_key,
        )
        ...
        litelm.default_internal_user_params = original
...
async def test_list_team_v1_batches_key_queries():
    ...
    from litelm.proxy.management_endpoints.team_endpoints import list_team
    ...
    with patch(
        ...
        "litelm.proxy.management_endpoints.team_endpoints._authorize_and_filter_teams",
        new_callable=AsyncMock,
        return_value=[team1, team2],
    ), patch(
        "litelm.proxy.management_endpoints.team_endpoints.get_all_team_memberships",
        new_callable=AsyncMock,
        return_value=[],
    ):
        mock_find_many = AsyncMock(return_value=[key1, key2, key3])
        mock_prisma_client.db.litelm_verificationtoken.find_many = mock_find_many
