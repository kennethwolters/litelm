...
async def test_organization_update_object_permissions_existing_permission(monkeypatch):
    ...
    from litelm.proxy.management_endpoints.organization_endpoints import (
...
async def test_get_organization_daily_activity_admin_param_passing(monkeypatch):
    ...
    from litelm.proxy.management_endpoints import organization_endpoints
    from litelm.proxy.management_endpoints.organization_endpoints import (
    ...
    # Mock prisma client
    mock_prisma_client = AsyncMock()
    mock_prisma_client.db.litelm_organizationtable.find_many = AsyncMock(
        return_value=[]
    )
    monkeypatch.setattr("litelm.proxy.proxy_server.prisma_client", mock_prisma_client)
    ...
    monkeypatch.setattr(
        "litelm.proxy.management_endpoints.organization_endpoints._user_has_admin_view",
        lambda _: True,
    )
    ...
    assert kwargs["table_name"] == "litelm_dailyorganizationspend"
...
async def test_get_organization_daily_activity_non_admin_defaults_to_admin_orgs(monkeypatch):
    ...
    mock_prisma_client.db.litelm_organizationmembership.find_many = AsyncMock(
        return_value=[
            SimpleNamespace(
...
async def test_list_organization_filter_by_org_id(monkeypatch):
    ...
    mock_prisma_client.db.litelm_organizationtable.find_many.assert_called_once()
    call_args = mock_prisma_client.db.litelm_organizationtable.find_many.call_args
    assert call_args.kwargs["where"] == {"organization_id": "org-123"}
    assert call_args.kwargs["include"] == {
        "litelm_budget_table": True,
        "members": True,
        "teams": True,
    }
