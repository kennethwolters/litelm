...
async def test_ui_view_users_with_null_email(mocker, caplog):
    ...
    mock_prisma_client.db.litelm_usertable.find_many = mock_find_many

    # Flag OFF by default
    mocker.patch(
        "litelm.proxy.ui_crud_endpoints.proxy_setting_endpoints.get_ui_settings_cached",
        return_value={},
    )

    mocker.patch("litelm.proxy.proxy_server.prisma_client", mock_prisma_client)
...
async def test_ui_view_users_org_admin_filtered_by_org(mocker):
    ...
    """
    from litelm.proxy._types import LiteLLM_OrganizationMembershipTable
    ...
    mocker.patch("litelm.proxy.proxy_server.prisma_client", mock_prisma_client)
    mocker.patch("litelm.proxy.proxy_server.user_api_key_cache", mocker.MagicMock())
    mocker.patch("litelm.proxy.proxy_server.proxy_logging_obj", mocker.MagicMock())
    ...
    mocker.patch(
        "litelm.proxy.management_endpoints.internal_user_endpoints.get_user_object",
...
async def test_ui_view_users_flag_on_team_admin_org_team(mocker):
    ...
    mocker.patch(
        "litelm.proxy.management_endpoints.internal_user_endpoints.get_team_object",
...
def test_user_daily_activity_types():
    ...
    """
    from litelm.proxy.management_endpoints.common_daily_activity import (
        DailySpendMetadata,
        SpendMetrics,
    )
...
async def test_get_users_includes_timestamps(mocker):
    ...
    mock_prisma_client.db.litelm_usertable.find_many = mock_find_many
    mock_prisma_client.db.litelm_usertable.count = mock_count
    ...
    mocker.patch(
        "litelm.proxy.management_endpoints.internal_user_endpoints.get_user_key_counts",
...
def test_update_internal_user_params_email():
    ...
    Test that _update_internal_user_params correctly processes an email-only update
    """
    from litelm.proxy._types import UpdateUserRequest
    from litelm.proxy.management_endpoints.internal_user_endpoints import (
        _update_internal_user_params,
    )
...
def test_update_internal_user_params_reset_spend_and_max_budget():
    """
    Relevant Issue: https://github.com/BerriAI/litelm/issues/10495
    """
    from litelm.proxy._types import UpdateUserRequest
    from litelm.proxy.management_endpoints.internal_user_endpoints import (
        _update_internal_user_params,
...
async def test_new_user_license_over_limit(mocker):
    ...
    from litelm.proxy._types import NewUserRequest, UserAPIKeyAuth
    ...
    mocker.patch(
        "litelm.proxy.management_endpoints.internal_user_endpoints._check_duplicate_user_email",
        mock_check_duplicate_user_email,
    )
    mocker.patch(
        "litelm.proxy.management_endpoints.internal_user_endpoints._check_duplicate_user_id",
    ...
    mocker.patch("litelm.proxy.proxy_server.prisma_client", mock_prisma_client)
    mocker.patch("litelm.proxy.proxy_server._license_check", mock_license_check)
...
async def test_user_info_url_encoding_plus_character(mocker):
    ...
    from litelm.proxy._types import LiteLLM_UserTable, UserAPIKeyAuth
...
async def test_user_info_nonexistent_user(mocker):
    ...
    from litelm.proxy._types import ProxyException, UserAPIKeyAuth
    from litelm.proxy.management_endpoints.internal_user_endpoints import user_info
...
async def test_new_user_default_teams_flow(mocker):
    ...
    """
    import litelm
    from litelm.proxy._types import NewUserRequest, NewUserRequestTeam, UserAPIKeyAuth
    from litelm.proxy.management_endpoints.internal_user_endpoints import new_user
    ...
    try:
        ...
        mocker.patch("litelm.proxy.proxy_server._license_check", mock_license_check)
        mocker.patch(
            "litelm.proxy.management_endpoints.internal_user_endpoints.generate_key_helper_fn",
            mock_generate_key_helper_fn,
        )
        mocker.patch(
            "litelm.proxy.management_endpoints.internal_user_endpoints._add_user_to_team",
            mock_add_user_to_team,
        )
        mocker.patch(
            "litelm.proxy.management_endpoints.internal_user_endpoints.UserManagementEventHooks.async_user_created_hook",
        ...
        # is defined in litelm/__init__.py and delattr-ing it breaks parallel tests)
...
def test_update_internal_new_user_params_proxy_admin_role():
    ...
    """
    import litelm
    from litelm.proxy._types import LitellmUserRoles, NewUserRequest
    from litelm.proxy.management_endpoints.internal_user_endpoints import (
...
async def test_check_duplicate_user_email_case_insensitive(mocker):
    ...
    mock_prisma_client.db.litelm_usertable.find_first = mock_find_first_duplicate
    ...
    mock_prisma_client.db.litelm_usertable.find_first = mock_find_first_no_duplicate
...
def test_process_keys_for_user_info_filters_dashboard_keys(monkeypatch):
    """
    Test that _process_keys_for_user_info filters out keys with team_id='litelm-dashboard'
    
    UI session tokens (team_id='litelm-dashboard') should be excluded from user info responses
    to prevent confusion, as these are automatically created during dashboard login.
    """
    from unittest.mock import MagicMock

    from litelm.constants import UI_SESSION_TOKEN_TEAM_ID
    from litelm.proxy.management_endpoints.internal_user_endpoints import (
    ...
    # Mock general_settings and litelm_master_key_hash (they're imported from proxy_server)
    monkeypatch.setattr(
        "litelm.proxy.proxy_server.general_settings",
        {},
    )
    monkeypatch.setattr(
        "litelm.proxy.proxy_server.litelm_master_key_hash",
        "different-hash",
    )
...
def test_generate_request_base_validator():
    ...
    """
    from litelm.proxy._types import GenerateRequestBase
...
async def test_get_user_daily_activity_non_admin_cannot_view_other_users(monkeypatch):
    ...
    with patch(
        "litelm.proxy.management_endpoints.internal_user_endpoints.get_daily_activity",
...
async def test_get_user_daily_activity_aggregated_admin_global_view(monkeypatch):
    ...
    monkeypatch.setattr(
        "litelm.proxy.management_endpoints.internal_user_endpoints.get_daily_activity_aggregated",
        mock_get_daily_agg,
    )
    ...
    mock_get_daily_agg.assert_called_once_with(
        ...
        table_name="litelm_dailyuserspend",
...
async def test_delete_user_cleans_up_created_by_invitation_links(mocker):
    ...
    """
    from litelm.proxy._types import DeleteUserRequest, UserAPIKeyAuth
    from litelm.proxy.management_endpoints.internal_user_endpoints import delete_user
    ...
    mock_prisma_client.db.litelm_usertable.find_unique = mocker.AsyncMock(
        side_effect=mock_find_unique
    )

    # Mock find_many for teams (no teams)
    mock_prisma_client.db.litelm_teamtable.find_many = mocker.AsyncMock(
        return_value=[]
    )

    # Mock all delete_many calls
    mock_prisma_client.db.litelm_verificationtoken.delete_many = mocker.AsyncMock(
        return_value=0
    )
    mock_prisma_client.db.litelm_invitationlink.delete_many = mocker.AsyncMock(
        return_value=1
    )
    mock_prisma_client.db.litelm_organizationmembership.delete_many = mocker.AsyncMock(
        return_value=0
    )
    mock_prisma_client.db.litelm_teammembership.delete_many = mocker.AsyncMock(
        return_value=0
    )
    mock_prisma_client.db.litelm_usertable.delete_many = mocker.AsyncMock(
        return_value=1
    )
    ...
    mock_prisma_client.db.litelm_invitationlink.delete_many.assert_called_once()
    call_kwargs = mock_prisma_client.db.litelm_invitationlink.delete_many.call_args
