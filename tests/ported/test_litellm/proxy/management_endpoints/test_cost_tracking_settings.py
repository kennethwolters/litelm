...
import litelm
from litelm.proxy.management_endpoints.cost_tracking_settings import router
...
class TestCostTrackingSettings:
    ...
    async def test_update_cost_discount_config_success(self):
        ...
        with patch(
        ...
        ), patch.object(litelm, "cost_discount_config", {}):
            # Make request
            ...
            # Verify litelm.cost_discount_config was updated
            assert litelm.cost_discount_config == test_discount_config
...
class TestResolveModelForCostLookup:
    ...
    def test_resolves_base_model_for_azure_deployment(self):
        ...
        instead of the raw litelm_params.model (Azure deployment name).
        """
        from litelm.proxy.management_endpoints.cost_tracking_settings import (
    ...
    def test_falls_back_to_litelm_params_model_when_no_base_model(self):
        """
        When no base_model is set, should fall back to litelm_params.model.
        """
        from litelm.proxy.management_endpoints.cost_tracking_settings import (
            _resolve_model_for_cost_lookup,
        )
    ...
    def test_resolves_base_model_from_litelm_params(self):
        """
        When base_model is in litelm_params (not model_info),
