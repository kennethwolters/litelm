...
from litelm.types.tag_management import TagDeleteRequest, TagInfoRequest, TagNewRequest
...
async def test_create_and_get_tag():
    ...
    try:
        with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma, patch(
            ...
            "litelm.proxy.proxy_server.litelm_proxy_admin_name", "default_user_id"
        ), patch(
            "litelm.proxy.management_endpoints.tag_management_endpoints.get_deployments_by_model"
            ...
            mock_db.litelm_tagtable.find_unique = AsyncMock(return_value=None)
            
            # Mock find_many for model lookup
            mock_db.litelm_proxymodeltable.find_many = AsyncMock(return_value=[])
            ...
            mock_db.litelm_tagtable.create = AsyncMock(return_value=created_tag)
            ...
            retrieved_tag.litelm_budget_table = None
            mock_db.litelm_tagtable.find_many = AsyncMock(return_value=[retrieved_tag])
...
async def test_update_tag():
    ...
    try:
        with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma, patch(
            ...
            mock_db.litelm_tagtable.find_unique = AsyncMock(return_value=existing_tag)
            ...
            mock_db.litelm_tagtable.update = AsyncMock(return_value=updated_tag)
...
async def test_delete_tag():
    ...
    try:
        with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma:
            ...
            # Mock delete
            mock_db.litelm_tagtable.delete = AsyncMock(return_value=existing_tag)
            ...
            mock_db.litelm_tagtable.delete.assert_called_once()
    finally:
        # Clean up dependency overrides
        app.dependency_overrides.clear()
...
async def test_list_tags_with_dynamic_tags():
    ...
    try:
        with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma:
            ...
            stored_tag.litelm_budget_table = None
            mock_db.litelm_tagtable.find_many = AsyncMock(return_value=[stored_tag])

            # Setup dynamic tags via group_by — includes one that overlaps with stored
            mock_db.litelm_dailytagspend.group_by = AsyncMock(return_value=[
...
async def test_get_deployments_by_model_id():
    ...
    from litelm.proxy.management_endpoints.tag_management_endpoints import (
        get_deployments_by_model,
    )
    from litelm.types.router import Deployment, LiteLLM_Params, ModelInfo
...
async def test_get_deployments_by_model_name():
    ...
    mock_router.get_model_list.return_value = [
        {
            "model_name": "gpt-3.5-turbo",
            "litelm_params": {"model": "gpt-3.5-turbo", "api_key": "test-key"},
...
async def test_add_tag_to_deployment_preserves_encrypted_fields():
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma:
        ...
        mock_db.litelm_proxymodeltable.find_unique = AsyncMock(return_value=db_model)

        # Mock update
        mock_db.litelm_proxymodeltable.update = AsyncMock(return_value=db_model)
        ...
        mock_db.litelm_proxymodeltable.find_unique.assert_called_once_with(
            where={"model_id": "model-123"}
        )

        # Verify update was called with preserved encrypted fields
        update_call = mock_db.litelm_proxymodeltable.update.call_args
        assert update_call[1]["where"] == {"model_id": "model-123"}

        # Parse the updated litelm_params
        updated_params = json.loads(update_call[1]["data"]["litelm_params"])
...
async def test_add_tag_to_deployment_with_string_params():
    """
    Test that _add_tag_to_deployment handles string litelm_params correctly
    """
    from unittest.mock import AsyncMock, Mock
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma:
        ...
        # Mock the database model with litelm_params as string
        db_model = Mock()
        db_model.model_id = "model-456"
        db_model.litelm_params = json.dumps({
            "model": "claude-3",
        ...
        deployment = Deployment(
            model_name="claude-3",
            litelm_params=LiteLLM_Params(model="claude-3"),
...
async def test_add_tag_to_deployment_no_duplicate_tags():
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma:
        ...
        deployment = Deployment(
            model_name="gpt-4",
            litelm_params=LiteLLM_Params(model="gpt-4"),
...
async def test_add_tag_to_deployment_model_not_found():
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma:
        ...
        mock_db.litelm_proxymodeltable.find_unique = AsyncMock(return_value=None)

        # Create deployment
        deployment = Deployment(
            model_name="nonexistent-model",
            litelm_params=LiteLLM_Params(model="nonexistent-model"),
