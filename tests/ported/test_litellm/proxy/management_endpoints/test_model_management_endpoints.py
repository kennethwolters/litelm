...
from litelm.proxy.management_endpoints.model_management_endpoints import (
    ModelManagementAuthChecks,
    clear_cache,
)
from litelm.proxy.utils import PrismaClient
from litelm.types.router import Deployment, LiteLLM_Params, updateDeployment
...
class TestModelManagementAuthChecks:
    ...
    async def test_allow_team_model_action_success(self):
        ...
        model_params = Deployment(
            ...
            litelm_params=LiteLLM_Params(model="test_model", team_id="test_team"),
...
class MockPrismaWrapper:
    def __init__(self, model_aliases_list):
        self.litelm_modeltable = MockPrismaDB(model_aliases_list)
...
class TestDeleteTeamModelAlias:
    ...
    async def test_delete_team_model_alias_success(self):
        ...
        # Verify results
        mock_db = mock_prisma.db.litelm_modeltable
        assert (
...
class TestClearCache:
    ...
    async def test_clear_cache_success(self):
        ...
        with patch("litelm.proxy.proxy_server.llm_router", mock_router), patch(
            "litelm.proxy.proxy_server.proxy_config", mock_config
        ), patch("litelm.proxy.proxy_server.prisma_client", mock_prisma), patch(
            "litelm.proxy.proxy_server.proxy_logging_obj", mock_logging
        ), patch(
            "litelm.proxy.proxy_server.verbose_proxy_logger"
        ):
            await clear_cache()
    ...
    async def test_clear_cache_preserve_config_models(self):
        ...
        """
        from litelm.proxy.management_endpoints.model_management_endpoints import clear_cache
...
class TestTeamModelUpdate:
    ...
    async def test_patch_model_with_team_id_creates_proper_setup(self):
        ...
        db_model = Deployment(
            ...
            litelm_params=LiteLLM_Params(model="test_model"),
            model_info=ModelInfo(),
        )
        ...
        with patch(
            ...
            "litelm.proxy.management_endpoints.model_management_endpoints.update_team"
        ) as mock_update_team, patch(
            "litelm.proxy.management_endpoints.model_management_endpoints.team_model_add"
...
class TestModelInfoEndpoint:
    ...
    async def test_model_info_accessible_model_success(self):
        ...
        from litelm.proxy.proxy_server import model_info
        ...
        with patch("litelm.proxy.proxy_server.llm_router") as mock_router, \
             patch("litelm.proxy.proxy_server.get_key_models") as mock_get_key_models, \
             patch("litelm.proxy.proxy_server.get_team_models") as mock_get_team_models, \
             patch("litelm.proxy.proxy_server.get_complete_model_list") as mock_get_complete_models, \
             patch("litelm.get_llm_provider") as mock_get_provider:
