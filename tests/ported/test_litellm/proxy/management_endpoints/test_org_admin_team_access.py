...
def _patch_org_admin_deps(get_user_return):
    ...
    return (
        patch("litelm.proxy.auth.auth_checks.get_user_object", new_callable=AsyncMock, return_value=get_user_return),
        patch("litelm.proxy.proxy_server.prisma_client", MagicMock(), create=True),
        patch("litelm.proxy.proxy_server.proxy_logging_obj", MagicMock(), create=True),
        patch("litelm.proxy.proxy_server.user_api_key_cache", MagicMock(), create=True),
    )
...
class TestValidateMembership:
    ...
    async def test_proxy_admin_allowed(self):
        from litelm.proxy.management_endpoints.team_endpoints import validate_membership
