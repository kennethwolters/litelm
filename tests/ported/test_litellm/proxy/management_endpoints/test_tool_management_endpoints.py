...
Patches target the source modules (litelm.proxy.db.tool_registry_writer.*
and litelm.proxy.proxy_server.prisma_client) because the endpoint code
...
from litelm.proxy.management_endpoints.tool_management_endpoints import router
from litelm.types.tool_management import LiteLLM_ToolTableRow
...
class TestToolManagementEndpoints:
    ...
    @patch(
        "litelm.proxy.db.tool_registry_writer.list_tools",
        new_callable=AsyncMock,
    )
    @patch("litelm.proxy.proxy_server.prisma_client", _MOCK_PRISMA)
    ...
    @patch(
        "litelm.proxy.db.tool_registry_writer.get_tool",
        new_callable=AsyncMock,
    )
    @patch("litelm.proxy.proxy_server.prisma_client", _MOCK_PRISMA)
    ...
    @patch(
        "litelm.proxy.db.tool_registry_writer.update_tool_policy",
        new_callable=AsyncMock,
    )
    @patch("litelm.proxy.proxy_server.prisma_client", _MOCK_PRISMA)
