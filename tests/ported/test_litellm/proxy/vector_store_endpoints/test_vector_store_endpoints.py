...
import litelm
from litelm.integrations.vector_store_integrations.vector_store_pre_call_hook import (
    LiteLLM_ManagedVectorStore,
)
from litelm.proxy._types import UserAPIKeyAuth
from litelm.proxy.vector_store_endpoints.endpoints import (
    _update_request_data_with_litelm_managed_vector_store_registry,
)
from litelm.proxy.vector_store_endpoints.management_endpoints import (
...
from litelm.proxy.vector_store_endpoints.utils import (
...
async def test_router_avector_store_search_passes_correct_args():
    ...
    to downstream litelm.vector_stores.asearch() with custom_llm_provider and query.
    """
    # Create a router
    router = litelm.Router(model_list=[])
...
async def test_router_avector_store_file_list_passes_correct_args():
    with patch(
        "litelm.vector_store_files.main.alist",
...
def test_router_vector_store_file_delete_passes_correct_args():
    with patch(
        "litelm.vector_store_files.main.delete",
...
def test_update_request_data_with_litelm_managed_vector_store_registry():
    """
    Test that _update_request_data_with_litelm_managed_vector_store_registry
    ...
    mock_vector_store: LiteLLM_ManagedVectorStore = {
        "vector_store_id": "test_store_id",
        "custom_llm_provider": "bedrock",
        "litelm_credential_name": "test_credential",
        "litelm_params": {"api_key": "test_key", "aws_region_name": "us-east-1"},
    }

    mock_registry = MagicMock()
    mock_registry.get_litelm_managed_vector_store_from_registry.return_value = (
        mock_vector_store
    )

    # Test with vector store registry
    with patch.object(litelm, "vector_store_registry", mock_registry):
        result = _update_request_data_with_litelm_managed_vector_store_registry(
        ...
        assert result["litelm_credential_name"] == "test_credential"
        assert result["api_key"] == "test_key"
        assert result["aws_region_name"] == "us-east-1"

        # Verify registry was called correctly
        mock_registry.get_litelm_managed_vector_store_from_registry.assert_called_once_with(
            vector_store_id="test_store_id"
        )

    # Test with no vector store registry
    with patch.object(litelm, "vector_store_registry", None):
...
class TestIsAllowedToCallVectorStoreEndpoint:
    ...
    def test_read_permission_allowed(self):
        ...
        with patch(
            "litelm.proxy.vector_store_endpoints.utils.ProviderConfigManager.get_provider_vector_stores_config",
...
class TestIsAllowedToCallVectorStoreFilesEndpoint:
    ...
    def test_allows_access_with_permissions(self):
        ...
        with patch(
            "litelm.proxy.vector_store_endpoints.utils.ProviderConfigManager.get_provider_vector_store_files_config",
...
async def test_vector_store_synchronization_across_instances():
    ...
    from litelm.types.vector_stores import (
        LiteLLM_ManagedVectorStore,
        VectorStoreDeleteRequest,
    )
    from litelm.vector_stores.vector_store_registry import VectorStoreRegistry
    ...
    mock_prisma_client.db.litelm_managedvectorstorestable.find_unique = AsyncMock(
        side_effect=mock_find_unique
    )
    mock_prisma_client.db.litelm_managedvectorstorestable.find_many = AsyncMock(
        side_effect=mock_find_many
    )
    mock_prisma_client.db.litelm_managedvectorstorestable.create = AsyncMock(
        side_effect=mock_create
    )
    mock_prisma_client.db.litelm_managedvectorstorestable.delete = AsyncMock(
    ...
    await mock_prisma_client.db.litelm_managedvectorstorestable.create(
    ...
    assert instance_1_registry.get_litelm_managed_vector_store_from_registry(
        test_vector_store_id
    ) is not None, "Vector store should be in Instance 1's memory"
    
    # Verify it's in the database
    db_store = await mock_prisma_client.db.litelm_managedvectorstorestable.find_unique(
    ...
    found_store = await instance_2_registry.get_litelm_managed_vector_store_from_registry_or_db(
        vector_store_id=test_vector_store_id,
    ...
    assert instance_2_registry.get_litelm_managed_vector_store_from_registry(
        test_vector_store_id
    ...
    await mock_prisma_client.db.litelm_managedvectorstorestable.delete(
        where={"vector_store_id": test_vector_store_id}
    )
    ...
    db_store_after_delete = await mock_prisma_client.db.litelm_managedvectorstorestable.find_unique(
        where={"vector_store_id": test_vector_store_id}
    )
...
async def test_vector_store_update_and_list_synchronization():
    ...
    from litelm.types.vector_stores import LiteLLM_ManagedVectorStore
    from litelm.vector_stores.vector_store_registry import VectorStoreRegistry
    ...
    mock_prisma_client.db.litelm_managedvectorstorestable.update = AsyncMock(
    ...
    instance_1_vs = instance_1_registry.get_litelm_managed_vector_store_from_registry(
        test_vector_store_id
    )
    instance_2_vs = instance_2_registry.get_litelm_managed_vector_store_from_registry(
        test_vector_store_id
    )
    ...
    await mock_prisma_client.db.litelm_managedvectorstorestable.update(
        where={"vector_store_id": test_vector_store_id},
        data=update_data
    )
    ...
    instance_1_vs_after_update = instance_1_registry.get_litelm_managed_vector_store_from_registry(
        test_vector_store_id
    )
    assert instance_1_vs_after_update.get("vector_store_name") == updated_name
    
    # Verify Instance 2 still has stale data in cache
    instance_2_vs_before_list = instance_2_registry.get_litelm_managed_vector_store_from_registry(
        test_vector_store_id
    )
    ...
    instance_2_vs_after_list = instance_2_registry.get_litelm_managed_vector_store_from_registry(
        test_vector_store_id
    )
...
async def test_resolve_embedding_config_from_db():
    ...
    # Mock database model with litelm_params
    mock_db_model = MagicMock()
    mock_db_model.litelm_params = {
        "api_key": "test-api-key",
        "api_base": "https://api.openai.com",
        "api_version": "2024-01-01"
    }
    
    mock_prisma_client.db.litelm_proxymodeltable.find_first = AsyncMock(
        return_value=mock_db_model
    )
    
    with patch(
        "litelm.proxy.vector_store_endpoints.management_endpoints.decrypt_value_helper",
    ...
    mock_prisma_client.db.litelm_proxymodeltable.find_first.assert_called_once_with(
...
async def test_new_vector_store_auto_resolves_embedding_config():
    ...
    vector_store_data: LiteLLM_ManagedVectorStore = {
        ...
        "custom_llm_provider": "openai",
        "litelm_params": {
            "litelm_embedding_model": "text-embedding-ada-002",
            # Note: litelm_embedding_config is not provided
        }
    }
    ...
    async def mock_create(*args, **kwargs):
        ...
        mock_created_vector_store.model_dump.return_value = {
            "vector_store_id": "test-store-001",
            "custom_llm_provider": "openai",
            "litelm_params": kwargs.get("data", {}).get("litelm_params")
        }
    ...
    with patch(
        ...
        litelm, "vector_store_registry", mock_registry
    ):
    ...
    litelm_params_json = captured_create_data.get("litelm_params")
    assert litelm_params_json is not None
    litelm_params_dict = json.loads(litelm_params_json)
    assert "litelm_embedding_config" in litelm_params_dict
    assert litelm_params_dict["litelm_embedding_config"]["api_key"] == "resolved-api-key"
    assert litelm_params_dict["litelm_embedding_config"]["api_base"] == "https://api.openai.com"
    assert litelm_params_dict["litelm_embedding_config"]["api_version"] == "2024-01-01"
...
def test_resolve_embedding_config_from_router():
    ...
    # Create a mock deployment with litelm_params
    mock_litelm_params = MagicMock(spec=LiteLLM_Params)
    mock_litelm_params.api_key = "config-api-key"
    mock_litelm_params.api_base = "https://config-api-base.com"
    mock_litelm_params.api_version = "2024-02-01"
    
    mock_deployment = MagicMock(spec=Deployment)
    mock_deployment.litelm_params = mock_litelm_params
...
def test_resolve_embedding_config_from_router_with_provider_prefix():
    ...
    mock_litelm_params = MagicMock(spec=LiteLLM_Params)
    mock_litelm_params.api_key = "azure-api-key"
    mock_litelm_params.api_base = "https://azure-endpoint.openai.azure.com"
    mock_litelm_params.api_version = "2024-02-15"
...
def test_resolve_embedding_config_from_router_handles_os_environ():
    ...
    mock_litelm_params = MagicMock(spec=LiteLLM_Params)
    mock_litelm_params.api_key = "os.environ/OPENAI_API_KEY"
    mock_litelm_params.api_base = "https://direct-url.com"
    mock_litelm_params.api_version = None
    ...
    with patch(
        "litelm.proxy.vector_store_endpoints.management_endpoints.get_secret",
...
async def test_resolve_embedding_config_tries_router_then_db():
    ...
    mock_litelm_params = MagicMock(spec=LiteLLM_Params)
    mock_litelm_params.api_key = "router-api-key"
    mock_litelm_params.api_base = "https://router-api-base.com"
    mock_litelm_params.api_version = None
    ...
    mock_prisma_client.db.litelm_proxymodeltable.find_first.assert_not_called()
...
async def test_resolve_embedding_config_falls_back_to_db():
    ...
    mock_prisma_client.db.litelm_proxymodeltable.find_first.assert_called()
...
async def test_new_vector_store_auto_resolves_from_router():
    ...
    vector_store_data: LiteLLM_ManagedVectorStore = {
        ...
        "custom_llm_provider": "openai",
        "litelm_params": {
            "litelm_embedding_model": "config-embedding-model",
            # Note: litelm_embedding_config is not provided
        }
    }
    ...
    mock_router = MagicMock()
    mock_litelm_params = MagicMock(spec=LiteLLM_Params)
    mock_litelm_params.api_key = "router-resolved-api-key"
    mock_litelm_params.api_base = "https://router-resolved-base.com"
    mock_litelm_params.api_version = "2024-03-01"
    ...
    litelm_params_dict = json.loads(litelm_params_json)
    assert "litelm_embedding_config" in litelm_params_dict
    assert litelm_params_dict["litelm_embedding_config"]["api_key"] == "router-resolved-api-key"
    assert litelm_params_dict["litelm_embedding_config"]["api_base"] == "https://router-resolved-base.com"
    assert litelm_params_dict["litelm_embedding_config"]["api_version"] == "2024-03-01"
...
async def test_create_vector_store_in_db():
    ...
    litelm_params = {"api_key": "test-key"}
    team_id = "team-123"
    user_id = "user-456"
    ...
    mock_prisma_client.db.litelm_managedvectorstorestable.find_unique.assert_called_once_with(
        where={"vector_store_id": vector_store_id}
    )
    mock_prisma_client.db.litelm_managedvectorstorestable.create.assert_called_once()
    ...
    create_call_args = mock_prisma_client.db.litelm_managedvectorstorestable.create.call_args
...
async def test_create_vector_store_in_db_raises_when_exists():
    ...
    mock_prisma_client.db.litelm_managedvectorstorestable.create.assert_not_called()
