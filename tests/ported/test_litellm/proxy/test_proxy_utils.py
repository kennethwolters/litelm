...
from litelm.proxy.utils import get_custom_url, join_paths


def test_get_custom_url(monkeypatch):
    monkeypatch.setenv("SERVER_ROOT_PATH", "/litelm")
    custom_url = get_custom_url(request_base_url="http://0.0.0.0:4000", route="ui/")
    assert custom_url == "http://0.0.0.0:4000/litelm/ui/"
...
def _patch_today(monkeypatch, year, month, day):
    ...
    monkeypatch.setattr("litelm.proxy.utils.date", PatchedDate)


def test_get_projected_spend_over_limit_day_one(monkeypatch):
    from litelm.proxy.utils import _get_projected_spend_over_limit
