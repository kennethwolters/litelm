...
async def _invoke_agent(agent, request):
    ...
    with patch(
        ...
        "litelm.proxy.agent_endpoints.auth.agent_permission_handler.AgentRequestHandler.is_agent_allowed",
        ...
        "litelm.proxy.proxy_server.proxy_config", MagicMock()
    ), patch(
        ...
        "litelm.a2a_protocol.main.A2A_SDK_AVAILABLE", True
    ):
...
async def test_create_a2a_client_uses_fresh_httpx_client():
    ...
    from litelm.a2a_protocol.main import create_a2a_client
    ...
    class FakeA2AClient:
        def __init__(self, httpx_client, agent_card):
            ...
            self._litelm_agent_card = agent_card

    with patch("litelm.a2a_protocol.main.A2A_SDK_AVAILABLE", True), patch(
        "litelm.a2a_protocol.main.A2ACardResolver", FakeResolver
    ), patch("litelm.a2a_protocol.main._A2AClient", FakeA2AClient):
