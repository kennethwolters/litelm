...
from litelm.proxy.agent_endpoints import endpoints as agent_endpoints
from litelm.proxy.agent_endpoints.endpoints import (
    _check_agent_management_permission,
    get_agent_daily_activity,
    router,
    user_api_key_auth,
)
from litelm.types.agents import AgentResponse
...
def _sample_agent_config() -> dict:
    return {
        ...
        "litelm_params": {"make_public": False},
    }
...
def mock_prisma_client():
    with patch("litelm.proxy.proxy_server.prisma_client") as mock:
        yield mock


@pytest.fixture
def mock_user_api_key_auth():
    with patch("litelm.proxy.agent_endpoints.endpoints.user_api_key_auth") as mock:
...
def test_update_agent_success(mock_prisma_client, mock_user_api_key_auth, monkeypatch):
    ...
    mock_prisma_client.db.litelm_agentstable.find_unique = AsyncMock(
...
def test_update_agent_not_found(
    ...
    mock_prisma_client.db.litelm_agentstable.find_unique = AsyncMock(return_value=None)
...
async def test_get_agent_daily_activity_admin_param_passing(monkeypatch):
    ...
    mock_prisma.db.litelm_agentstable.find_many = AsyncMock(return_value=[])
    ...
    assert kwargs["table_name"] == "litelm_dailyagentspend"
...
async def test_get_agent_daily_activity_with_agent_names(monkeypatch):
    ...
    mock_prisma.db.litelm_agentstable.find_many = AsyncMock(
...
class TestAgentRBACInternalUser:
    ...
    def test_should_allow_internal_user_to_get_agent_by_id(self, monkeypatch):
        ...
        with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma:
            mock_prisma.db.litelm_agentstable.find_unique = AsyncMock(
                return_value=None
            )
            resp = self.internal_client.get(
...
class TestAgentRoutesIncludesAgentIdPattern:
    ...
    def test_should_include_agent_id_pattern(self):
        from litelm.proxy._types import LiteLLMRoutes
...
class TestAgentHealthCheck:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch):
        from litelm.proxy.agent_endpoints import agent_registry as ar_mod
...
class TestCheckAgentUrlHealth:
    ...
    async def test_should_return_healthy_when_no_url(self):
        from litelm.proxy.agent_endpoints.endpoints import _check_agent_url_health
    ...
    @pytest.mark.asyncio
    @patch("litelm.proxy.agent_endpoints.endpoints.get_async_httpx_client")
