...
Tests that invoke_agent_a2a properly integrates with add_litelm_data_to_request.
"""
...
@pytest.mark.asyncio
async def test_invoke_agent_a2a_adds_litelm_data():
    """
    Test that invoke_agent_a2a calls add_litelm_data_to_request
    and the resulting data includes proxy_server_request.
    """
    from litelm.proxy._types import UserAPIKeyAuth

    # Track the data passed to add_litelm_data_to_request
    captured_data = {}

    async def mock_add_litelm_data(data, **kwargs):
        # Simulate what add_litelm_data_to_request does
    ...
    # Note: add_litelm_data_to_request is called from common_request_processing,
    # so we need to patch it there, not at litelm_pre_call_utils
    with patch(
        "litelm.proxy.agent_endpoints.a2a_endpoints._get_agent",
        return_value=mock_agent,
    ), patch(
        "litelm.proxy.common_request_processing.add_litelm_data_to_request",
        side_effect=mock_add_litelm_data,
    ) as mock_add_data, patch(
        "litelm.a2a_protocol.create_a2a_client",
        new_callable=AsyncMock,
    ), patch(
        "litelm.a2a_protocol.asend_message",
        new_callable=AsyncMock,
        ...
        "litelm.proxy.proxy_server.version",
        "1.0.0",
    ), patch.dict(
        sys.modules,
        {"a2a": MagicMock(), "a2a.types": mock_a2a_types},
    ), patch(
        "litelm.a2a_protocol.main.A2A_SDK_AVAILABLE",
        True,
    ):
        from litelm.proxy.agent_endpoints.a2a_endpoints import invoke_agent_a2a
        ...
        # Verify add_litelm_data_to_request was called
