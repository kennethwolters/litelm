...
Maps to: litelm/proxy/agent_endpoints/model_list_helpers.py
"""
import os
import sys
...
from litelm.proxy.agent_endpoints.model_list_helpers import (
...
async def test_append_agents_to_model_group():
    ...
    with patch(
        "litelm.proxy.agent_endpoints.auth.agent_permission_handler.AgentRequestHandler.get_allowed_agents",
...
async def test_append_agents_to_model_info():
    ...
    with patch(
        ...
        with patch(
            ...
            assert result[0]["litelm_params"]["model"] == "a2a/test-agent"
            assert result[0]["litelm_params"]["custom_llm_provider"] == "a2a"
