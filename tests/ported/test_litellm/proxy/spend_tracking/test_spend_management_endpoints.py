...
def make_ui_spend_logs_mock_prisma(mock_spend_logs, filter_fn, team_lookup_fn=None):
    ...
    Args:
        ...
        team_lookup_fn: Optional async callable for team RBAC (find_unique).
                        If provided, adds litelm_teamtable to db.
    """
    filtered_holder = []
    ...
    class MockPrismaClient:
        def __init__(self):
            self.db = MockDB()
            self.db.litelm_spendlogs = self.db
            if team_lookup_fn is not None:
                self.db.litelm_teamtable = self
...
from litelm.proxy.hooks.proxy_track_cost_callback import _ProxyDBLogger
from litelm.proxy.proxy_server import app, prisma_client
from litelm.proxy.spend_tracking import spend_management_endpoints
from litelm.router import Router
from litelm.types.utils import BudgetConfig
...
async def test_can_team_member_view_log_none_team_id():
    ...
    class MockPrisma:
        class DB:
            ...
            def __init__(self):
                self.litelm_teamtable = self.TeamTable()
...
ignored_keys = [
    ...
    "metadata.litelm_overhead_time_ms",
...
def disable_budget_sync(monkeypatch):
    ...
    monkeypatch.setattr(
        "litelm.router_strategy.budget_limiter.RouterBudgetLimiting.periodic_sync_in_memory_spend_with_redis",
        noop,
    )
...
async def test_ui_view_spend_logs_sort_by_and_sort_order(
    ...
    class MockPrismaClient:
        def __init__(self):
            self.db = MagicMock()
            self.db.litelm_spendlogs = MagicMock()
            self.db.litelm_spendlogs.count = AsyncMock(side_effect=mock_count)
            self.db.query_raw = AsyncMock(side_effect=mock_query_raw)

    monkeypatch.setattr("litelm.proxy.proxy_server.prisma_client", MockPrismaClient())
    monkeypatch.setattr(
        "litelm.proxy.spend_tracking.spend_management_endpoints._is_admin_view_safe",
...
async def test_ui_view_spend_logs_sort_validation_errors(
    ...
    class MockPrismaClient:
        def __init__(self):
            self.db = MagicMock()
            self.db.litelm_spendlogs = MagicMock()
            self.db.litelm_spendlogs.find_many = AsyncMock(return_value=[])
            self.db.litelm_spendlogs.count = AsyncMock(side_effect=mock_count)
...
async def test_ui_view_spend_logs_with_model_id(client, monkeypatch):
    """Test that the model_id query param filters spend logs by litelm model deployment id."""
    mock_spend_logs = [
        {
            "id": "log1",
            "request_id": "req1",
...
class TestSpendLogsPayload:
    def setup_method(self):
        self._original_callbacks = litelm.callbacks[:]
        self._original_cache = litelm.cache
        litelm.cache = None

    def teardown_method(self):
        litelm.callbacks = self._original_callbacks
        litelm.cache = self._original_cache

    @pytest.mark.asyncio
    async def test_spend_logs_payload_e2e(self):
        litelm.callbacks = [_ProxyDBLogger(message_logging=False)]
        # litelm._turn_on_debug()

        with patch.object(
            litelm.proxy.db.db_spend_update_writer.DBSpendUpdateWriter,
            "_insert_spend_log_to_db",
        ) as mock_client, patch.object(litelm.proxy.proxy_server, "prisma_client"):
...
async def test_view_spend_tags(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "litelm.proxy.spend_tracking.spend_management_endpoints.get_spend_by_tags",
...
async def test_provider_budget_provider_budgets(disable_budget_sync):
    ...
    with patch("litelm.proxy.proxy_server.llm_router", router):
...
async def test_build_ui_spend_logs_response_dict_rows_session_counts():
    ...
    mock_prisma.db.litelm_spendlogs.group_by = AsyncMock(
        return_value=[
    ...
    mock_prisma.db.litelm_spendlogs.group_by.assert_called_once_with(
        by=["session_id"],
