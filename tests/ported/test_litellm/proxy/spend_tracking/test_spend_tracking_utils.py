...
import litelm
from litelm.constants import (
    LITELLM_TRUNCATED_PAYLOAD_FIELD,
    LITELLM_TRUNCATION_DB_SAFEGUARD_NOTE,
    REDACTED_BY_LITELM_STRING,
)
from litelm.litelm_core_utils.safe_json_dumps import safe_dumps
from litelm.proxy.spend_tracking.spend_tracking_utils import (
...
def test_sanitize_request_body_for_spend_logs_payload_long_string():
    from litelm.constants import MAX_STRING_LENGTH_PROMPT_IN_DB
...
@patch(
    "litelm.proxy.spend_tracking.spend_tracking_utils._should_store_prompts_and_responses_in_spend_logs"
)
...
def test_response_truncation_logs_info_message(mock_should_store):
    ...
    with patch(
        "litelm.proxy.spend_tracking.spend_tracking_utils.verbose_proxy_logger"
    ) as mock_logger:
...
def test_request_body_truncation_logs_info_message(mock_should_store):
    ...
    with patch(
        ...
        _get_proxy_server_request_for_spend_logs_payload(
            metadata={}, litelm_params=litelm_params, kwargs={}
        )
        mock_logger.info.assert_called_once()
        log_msg = mock_logger.info.call_args[0][0]
...
@patch("litelm.proxy.proxy_server.master_key", None)
...
@pytest.mark.asyncio
@patch("litelm.proxy.proxy_server.master_key", "sk-master-key")
@patch("litelm.proxy.proxy_server.general_settings", {})
...
def test_get_logging_payload_includes_overhead_in_spend_logs_metadata():
    """
    Test that get_logging_payload extracts litelm_overhead_time_ms from hidden_params
    ...
    standard_logging_payload = StandardLoggingPayload(
        ...
        hidden_params=StandardLoggingHiddenParams(
            ...
            api_base="https://api.openai.com",
            response_cost="0.001",
            litelm_overhead_time_ms=test_overhead_ms,
            additional_headers=None,
            batch_models=None,
            litelm_model_name=None,
            usage_object=None,
        ),
    )
    ...
    assert (
        metadata.get("litelm_overhead_time_ms") == test_overhead_ms
    ), f"Expected overhead '{test_overhead_ms}', got '{metadata.get('litelm_overhead_time_ms')}'"
...
def test_get_logging_payload_handles_missing_overhead_gracefully():
    ...
    standard_logging_payload = StandardLoggingPayload(
        ...
        hidden_params=StandardLoggingHiddenParams(
            ...
            api_base="https://api.openai.com",
            response_cost="0.001",
            litelm_overhead_time_ms=None,  # No overhead
            additional_headers=None,
            batch_models=None,
            litelm_model_name=None,
            usage_object=None,
    ...
    # When overhead is None, litelm_overhead_time_ms should be None or not present
    assert (
        metadata.get("litelm_overhead_time_ms") is None
    ), "litelm_overhead_time_ms should be None when overhead is not provided"
...
def test_spend_logs_redacts_request_and_response_when_turn_off_message_logging_enabled(
    ...
    request_result = _get_proxy_server_request_for_spend_logs_payload(
        metadata=metadata, litelm_params=litelm_params, kwargs=kwargs
    )

    parsed_request = json.loads(request_result)
    assert parsed_request["messages"] == [{"role": "user", "content": "redacted-by-litelm"}]
    ...
    assert parsed_response["choices"][0]["message"]["content"] == "redacted-by-litelm"
    assert parsed_response["choices"][0]["message"]["role"] == "assistant"


@patch("litelm.secret_managers.main.get_secret_bool")
def test_should_store_prompts_and_responses_in_spend_logs_case_insensitive_string(
    ...
    for true_value in ["true", "TRUE", "True", "TrUe"]:
        with patch("litelm.proxy.proxy_server.general_settings", {"store_prompts_in_spend_logs": true_value}):
    ...
    # Test boolean True
    with patch("litelm.proxy.proxy_server.general_settings", {"store_prompts_in_spend_logs": True}):
    ...
    for false_value in [False, None, "false", "FALSE", "False", "anything"]:
        with patch("litelm.proxy.proxy_server.general_settings", {"store_prompts_in_spend_logs": false_value}):
...
def test_get_logging_payload_guardrail_info_when_no_standard_logging_payload():
    ...
    with patch("litelm.proxy.proxy_server.master_key", "sk-master"):
        with patch("litelm.proxy.proxy_server.general_settings", {}):
            payload = get_logging_payload(
                kwargs=kwargs,
                response_obj={},
...
def test_get_logging_payload_includes_request_duration_ms():
    ...
    kwargs = {
        "model": "gpt-4",
        "litelm_params": {"api_base": "https://api.openai.com"},
        "standard_logging_object": None,
    }
    response_obj = {"usage": {"prompt_tokens": 10, "completion_tokens": 5, "total_tokens": 15}}

    with patch("litelm.proxy.proxy_server.master_key", None), \
         patch("litelm.proxy.proxy_server.general_settings", {}):
        payload = get_logging_payload(
            kwargs=kwargs,
            response_obj=response_obj,
