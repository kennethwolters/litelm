...
import litelm
from litelm.proxy.auth.user_api_key_auth import user_api_key_auth
from litelm.proxy.proxy_server import app, initialize
from litelm.utils import _invalidate_model_cost_lowercase_map
...
def mock_patch_aembedding():
    return mock.patch(
        "litelm.proxy.proxy_server.llm_router.aembedding",
...
def client_no_auth():
    # Assuming litelm.proxy.proxy_server is an object
    from litelm.proxy.proxy_server import cleanup_router_config_variables
...
def test_login_v2_returns_redirect_url_and_sets_cookie(monkeypatch):
    ...
    monkeypatch.setattr(
        "litelm.proxy.auth.login_utils.authenticate_user",
        mock_authenticate_user,
    )
    monkeypatch.setattr(
        "litelm.proxy.auth.login_utils.create_ui_token_object",
        mock_create_ui_token_object,
    )
    monkeypatch.setattr("jwt.encode", mock_jwt_encode)
    monkeypatch.setattr("litelm.proxy.proxy_server.master_key", "test-master-key")
    monkeypatch.setattr("litelm.proxy.proxy_server.general_settings", {})
    monkeypatch.setattr("litelm.proxy.proxy_server.premium_user", False)
    monkeypatch.setattr("litelm.proxy.proxy_server.prisma_client", mock_prisma_client)
    monkeypatch.setattr("litelm.proxy.utils.get_server_root_path", lambda: "")
    monkeypatch.setattr("litelm.proxy.utils.get_proxy_base_url", lambda: None)
...
def test_login_v2_returns_json_on_proxy_exception(monkeypatch):
    ...
    from litelm.proxy._types import ProxyErrorTypes, ProxyException
...
def test_sso_key_generate_shows_deprecation_banner(client_no_auth, monkeypatch):
    ...
    monkeypatch.setattr(
        "litelm.proxy.management_endpoints.ui_sso.show_missing_vars_in_env",
        lambda: None,
    )
    monkeypatch.setattr(
        "litelm.proxy.management_endpoints.ui_sso.SSOAuthenticationHandler.get_redirect_url_for_sso",
        lambda *args, **kwargs: "http://test/redirect",
    )
    monkeypatch.setattr(
        "litelm.proxy.management_endpoints.ui_sso.SSOAuthenticationHandler._get_cli_state",
        lambda *args, **kwargs: None,
    )
    monkeypatch.setattr(
        "litelm.proxy.management_endpoints.ui_sso.SSOAuthenticationHandler.should_use_sso_handler",
        lambda *args, **kwargs: False,
    )
    # Mock premium_user to bypass enterprise check (prevents 403 Forbidden)
    monkeypatch.setattr(
        "litelm.proxy.proxy_server.premium_user",
        True,
    )
...
def test_restructure_ui_html_files_handles_nested_routes(tmp_path):
    ...
    """
    from litelm.proxy import proxy_server
    ...
    write_file(ui_root / "litelm-asset-prefix" / "ignore.html", "asset")
    ...
    assert (
        (ui_root / "litelm-asset-prefix" / "ignore.html").read_text()
        == "asset"
    )
...
def test_restructure_always_happens(monkeypatch):
    ...
    In non-root Docker (is_non_root=True), restructuring happens in /var/lib/litelm/ui.
    """
    # Test Case 1: is_non_root is True - restructuring happens in /var/lib/litelm/ui
    monkeypatch.setenv("LITELLM_NON_ROOT", "true")
    
    runtime_ui_path = "/var/lib/litelm/ui"
...
async def test_initialize_scheduled_jobs_credentials(monkeypatch):
    ...
    from litelm.proxy.proxy_server import ProxyStartupEvent
    from litelm.proxy.utils import ProxyLogging
    ...
    with patch("litelm.proxy.proxy_server.proxy_config", mock_proxy_config), patch(
        "litelm.proxy.proxy_server.store_model_in_db", False
    ...
    with patch("litelm.proxy.proxy_server.proxy_config", mock_proxy_config), patch(
        "litelm.proxy.proxy_server.store_model_in_db", True
    ), patch("litelm.proxy.proxy_server.get_secret_bool", return_value=True):
        await ProxyStartupEvent.initialize_scheduled_background_jobs(
            general_settings={},
            prisma_client=mock_prisma_client,
            proxy_budget_rescheduler_min_time=1,
...
def test_update_config_fields_deep_merge_db_wins():
    from litelm.proxy.proxy_server import ProxyConfig
...
def test_get_config_custom_callback_api_env_vars(monkeypatch):
    ...
    """
    from litelm.proxy.proxy_server import app, proxy_config, user_api_key_auth

    # Mock config with custom_callback_api enabled and generic logger env vars present
    config_data = {
        "litelm_settings": {"success_callback": ["custom_callback_api"]},
        "general_settings": {},
    ...
    monkeypatch.setattr("litelm.proxy.proxy_server.llm_router", mock_router)
    monkeypatch.setattr(
...
@patch(
    "litelm.proxy.proxy_server.ProxyStartupEvent._setup_prisma_client",
...
async def test_aaaproxy_startup_master_key(mock_prisma, monkeypatch, tmp_path):
    ...
    from litelm.proxy.proxy_server import proxy_startup_event

    # Mock the Prisma import
    monkeypatch.setattr("litelm.proxy.proxy_server.PrismaClient", MockPrisma)
    ...
    async with proxy_startup_event(app):
        from litelm.proxy.proxy_server import master_key
...
def test_embedding_input_array_of_tokens(client_no_auth):
    ...
    Ref: https://github.com/BerriAI/litelm/issues/10113
    """
    from litelm.proxy import proxy_server
...
async def test_get_all_team_models():
    ...
    from litelm.proxy._types import LiteLLM_TeamTable
    from litelm.proxy.proxy_server import get_all_team_models
    ...
    mock_prisma_client = MagicMock()
    mock_db = MagicMock()
    mock_litelm_teamtable = MagicMock()

    mock_prisma_client.db = mock_db
    mock_db.litelm_teamtable = mock_litelm_teamtable

    # Make find_many async
    mock_litelm_teamtable.find_many = AsyncMock()
    ...
    mock_litelm_teamtable.find_many.return_value = [mock_team1, mock_team2]

    with patch("litelm.proxy.proxy_server.LiteLLM_TeamTable") as mock_team_table_class:
        ...
        mock_litelm_teamtable.find_many.assert_called_with()
    ...
    mock_litelm_teamtable.reset_mock()
    mock_router.reset_mock()
    mock_router.get_model_list.side_effect = mock_get_model_list

    # Only return team1 for specific team query
    mock_litelm_teamtable.find_many.return_value = [mock_team1]
    ...
    mock_litelm_teamtable.reset_mock()
    mock_router.reset_mock()
    mock_litelm_teamtable.find_many.return_value = []
    ...
    mock_litelm_teamtable.find_many.assert_called_with(where={"team_id": {"in": []}})
...
def test_add_team_models_to_all_models():
    ...
    """
    from litelm.proxy._types import LiteLLM_TeamTable
    from litelm.proxy.proxy_server import _add_team_models_to_all_models
...
async def test_delete_deployment_type_mismatch():
    ...
    with patch("litelm.proxy.proxy_server.llm_router", mock_llm_router), patch(
        "litelm.proxy.proxy_server.user_config_file_path", "test_config.yaml"
    ):
...
async def test_get_config_from_file(tmp_path, monkeypatch):
    ...
    test_config = {
        "model_list": [{"model_name": "gpt-4", "litelm_params": {"model": "gpt-4"}}],
        "general_settings": {"master_key": "sk-test"},
        "router_settings": {"enable_pre_call_checks": True},
        "litelm_settings": {"drop_params": True},
    }
    ...
    monkeypatch.setattr("litelm.proxy.proxy_server.user_config_file_path", None)

    result = await proxy_config._get_config_from_file(str(config_file))
    assert result == test_config

    # Verify that user_config_file_path was set
    from litelm.proxy.proxy_server import user_config_file_path
    ...
    monkeypatch.setattr(
        "litelm.proxy.proxy_server.user_config_file_path", str(config_file)
    )
...
def test_normalize_datetime_for_sorting():
    ...
    """
    from litelm.proxy.proxy_server import _normalize_datetime_for_sorting
...
async def test_add_proxy_budget_to_db_only_creates_user_no_keys():
    ...
    # Set up required litelm settings
    litelm.budget_duration = "30d"
    litelm.max_budget = 100.0

    litelm_proxy_budget_name = "litelm-proxy-budget"

    # Mock generate_key_helper_fn to capture its call arguments
    mock_generate_key_helper = AsyncMock(
        return_value={
            "user_id": litelm_proxy_budget_name,
            "max_budget": 100.0,
            "budget_duration": "30d",
            "spend": 0,
            "models": [],
    ...
    with patch(
        "litelm.proxy.proxy_server.generate_key_helper_fn", mock_generate_key_helper
    ):
        # Call the function under test
        ProxyStartupEvent._add_proxy_budget_to_db(litelm_proxy_budget_name)
        ...
        assert call_args.kwargs["user_id"] == litelm_proxy_budget_name
...
async def test_custom_ui_sso_sign_in_handler_config_loading():
    ...
    try:
        with patch(
            "litelm.proxy.proxy_server.get_instance_fn",
            ...
            from litelm.proxy.proxy_server import user_custom_ui_sso_sign_in_handler
...
async def test_load_environment_variables_direct_and_os_environ():
    ...
    with patch(
        "litelm.proxy.proxy_server.get_secret_str", return_value=mock_secret_value
    ) as mock_get_secret:
        with patch.dict(
...
@pytest.mark.asyncio
async def test_load_environment_variables_litelm_license_and_edge_cases():
    """
    ...
    with patch("litelm.proxy.proxy_server._license_check", mock_license_check):
    ...
    with patch("litelm.proxy.proxy_server.get_secret_str", return_value=None):
        with patch.dict(os.environ, {}, clear=False):
            # Call the method under test
...
async def test_write_config_to_file(monkeypatch):
    ...
    monkeypatch.setattr("litelm.proxy.proxy_server.store_model_in_db", True)
    ...
    monkeypatch.setattr(
        "litelm.proxy.proxy_server.general_settings", mock_general_settings
    )

    # Mock user_config_file_path
    test_config_path = "/tmp/test_config.yaml"
    monkeypatch.setattr(
        "litelm.proxy.proxy_server.user_config_file_path", test_config_path
    )
...
async def test_write_config_to_file_when_store_model_in_db_false(monkeypatch):
    ...
    # Set store_model_in_db to False
    monkeypatch.setattr("litelm.proxy.proxy_server.store_model_in_db", False)

    # Mock prisma_client to be None (so file path is taken)
    monkeypatch.setattr("litelm.proxy.proxy_server.prisma_client", None)
...
async def test_async_data_generator_midstream_error():
    ...
    from litelm.proxy.proxy_server import async_data_generator
    from litelm.proxy.utils import ProxyLogging
    ...
    with patch("litelm.proxy.proxy_server.proxy_logging_obj", mock_proxy_logging_obj):
...
async def test_chat_completion_result_no_nested_none_values():
    ...
    import litelm
    from litelm.proxy._types import UserAPIKeyAuth
    from litelm.proxy.proxy_server import chat_completion

    # Create a mock ModelResponse with nested None values
    mock_model_response = litelm.ModelResponse()
    ...
    setattr(
        mock_model_response,
        "usage",
        litelm.Usage(prompt_tokens=10, completion_tokens=5, total_tokens=15),
    )
    ...
    with patch(
        "litelm.proxy.proxy_server._read_request_body",
        return_value={"model": "gpt-3.5-turbo", "messages": []},
    ), patch(
        "litelm.proxy.proxy_server.ProxyBaseLLMRequestProcessing",
...
class TestPriceDataReloadAPI:
    ...
    def client_with_auth(self):
        ...
        from litelm.proxy._types import LitellmUserRoles
        from litelm.proxy.proxy_server import cleanup_router_config_variables
    ...
    def test_reload_model_cost_map_admin_access(self, client_with_auth):
        ...
        # (litelm.model_cost = new_model_cost_map) does not contaminate
        # subsequent tests running in the same worker process.
        original_model_cost = litelm.model_cost.copy()
        try:
            with patch(
                "litelm.litelm_core_utils.get_model_cost_map.get_model_cost_map"
            ) as mock_get_map:
                mock_get_map.return_value = {
                    "gpt-3.5-turbo": {"input_cost_per_token": 0.001}
                }
                # Mock the database connection
                with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma:
                    mock_prisma.db.litelm_config.find_unique = AsyncMock(
                        return_value=None
                    )
                    mock_prisma.db.litelm_config.upsert = AsyncMock(return_value=None)
            ...
            litelm.model_cost = original_model_cost
    ...
    def test_get_model_cost_map_public_access(self, client_no_auth):
        ...
        with patch(
            "litelm.model_cost", {"gpt-3.5-turbo": {"input_cost_per_token": 0.001}}
        ):
            response = client_no_auth.get("/public/litelm_model_cost_map")
    ...
    def test_cancel_model_cost_map_reload_admin_access(self, client_with_auth):
        ...
        with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma:
            ...
            mock_prisma.db.litelm_config.delete = AsyncMock(return_value=None)
    ...
    def test_get_model_cost_map_reload_status_admin_access(self, client_with_auth):
        ...
        with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma:
            ...
            with patch(
                "litelm.proxy.proxy_server.last_model_cost_map_reload",
                "2024-01-01T06:00:00",
            ):
                with patch("litelm.proxy.proxy_server.datetime") as mock_datetime:
    ...
    def test_get_model_cost_map_reload_status_no_config(self, client_with_auth):
        ...
        with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma:
            mock_prisma.db.litelm_config.find_unique = AsyncMock(return_value=None)
...
class TestPriceDataReloadIntegration:
        ...
        try:
            with patch(
                ...
                with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma:
                    ...
                    # Test get endpoint
                    response = client_with_auth.get("/public/litelm_model_cost_map")
                    assert response.status_code == 200
        finally:
            litelm.model_cost = original_model_cost
            _invalidate_model_cost_lowercase_map()
    ...
    def test_distributed_reload_check_function(self):
        ...
        mock_config = MagicMock()
        mock_config.param_value = {"interval_hours": 6, "force_reload": False}
        mock_prisma.db.litelm_config.find_unique = AsyncMock(return_value=mock_config)
        ...
        try:
            with patch(
                ...
                mock_prisma.db.litelm_config.upsert.assert_called()
                call_args = mock_prisma.db.litelm_config.upsert.call_args
        ...
        try:
            with patch(
                ...
                with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma:
                    ...
                    mock_prisma.db.litelm_config.find_unique = AsyncMock(return_value=mock_existing)
                    mock_prisma.db.litelm_config.upsert = AsyncMock(return_value=None)
    ...
    def test_anthropic_beta_headers_reload_preserves_interval_hours(self):
        ...
        with patch(
            "litelm.anthropic_beta_headers_manager.reload_beta_headers_config"
        ) as mock_reload:
...
"""
    ...
    def test_database_config_storage(self):
        ...
        asyncio.run(
            mock_prisma.db.litelm_config.upsert(
        ...
        mock_prisma.db.litelm_config.upsert.assert_called_once()
        call_args = mock_prisma.db.litelm_config.upsert.call_args
...
async def test_add_router_settings_from_db_config_merge_logic():
    ...
    # Mock prisma client
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.litelm_config.find_first = AsyncMock(
    ...
    mock_prisma_client.db.litelm_config.find_first.assert_called_once_with(
...
async def test_add_router_settings_from_db_config_edge_cases():
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.litelm_config.find_first = AsyncMock(return_value=None)
...
async def test_model_info_v1_oci_secrets_not_leaked():
    ...
    from litelm.proxy._types import UserAPIKeyAuth
    from litelm.proxy.proxy_server import model_info_v1
    ...
    with patch("litelm.proxy.proxy_server.llm_router", mock_router), \
         patch("litelm.proxy.proxy_server.llm_model_list", [mock_model_data]), \
         patch("litelm.proxy.proxy_server.general_settings", {"infer_model_from_keys": False}), \
         patch("litelm.proxy.proxy_server.user_model", None):
        
        # Call the model_info_v1 endpoint
        result = await model_info_v1(
            user_api_key_dict=mock_user_api_key_dict,
            litelm_model_id=None
        )
        ...
        litelm_params = model_info["litelm_params"]
        
        # Verify that sensitive OCI fields are masked
        assert "****" in litelm_params["oci_key"], "oci_key should be masked"
        assert "****" in litelm_params["oci_fingerprint"], "oci_fingerprint should be masked"
        assert "****" in litelm_params["oci_tenancy"], "oci_tenancy should be masked"
        assert "****" in litelm_params["oci_key_file"], "oci_key_file should be masked"
        
        # Verify that non-sensitive fields are NOT masked
        assert litelm_params["model"] == "oci/xai.grok-4", "model field should not be masked"
        assert litelm_params["oci_region"] == "us-phoenix-1", "oci_region should not be masked"
        assert litelm_params["drop_params"] is True, "drop_params should not be masked"
        
        # Verify the model field specifically is not masked (this was the original issue)
        assert "****" not in litelm_params["model"], "model field should never be masked"
        assert litelm_params["model"].startswith("oci/"), "model should retain its full value"
...
def test_add_callback_from_db_to_in_memory_litelm_callbacks():
    """
    Test that _add_callback_from_db_to_in_memory_litelm_callbacks correctly adds callbacks
    ...
    with patch("litelm.proxy.proxy_server.litelm") as mock_litelm:
        # Set up mock litelm attributes
        mock_litelm._known_custom_logger_compatible_callbacks = []
        mock_litelm.logging_callback_manager = mock_callback_manager
        
        # Test Case 1: Add success callback
        mock_success_callbacks = []
        proxy_config._add_callback_from_db_to_in_memory_litelm_callbacks(
            callback="prometheus",
            event_types=["success"],
            existing_callbacks=mock_success_callbacks,
        )
        mock_callback_manager.add_litelm_success_callback.assert_called_once_with("prometheus")
        ...
        mock_callback_manager.add_litelm_failure_callback.assert_called_once_with("langfuse")
        mock_callback_manager.reset_mock()
        ...
        mock_callback_manager.add_litelm_callback.assert_called_once_with("s3")
        mock_callback_manager.reset_mock()
        ...
        mock_callback_manager.add_litelm_success_callback.assert_not_called()
...
def test_should_load_db_object_with_supported_db_objects():
    ...
    with patch("litelm.proxy.proxy_server.general_settings", {}):
    ...
    with patch(
        "litelm.proxy.proxy_server.general_settings",
...
async def test_tag_cache_update_called():
    ...
    from litelm.proxy.proxy_server import user_api_key_cache

    cache = DualCache()

    setattr(
        litelm.proxy.proxy_server,
        "user_api_key_cache",
        cache,
    )
    ...
    with patch.object(cache, "async_get_cache", new=AsyncMock(return_value=mock_tag_obj)) as mock_get_cache:
        with patch.object(cache, "async_set_cache_pipeline", new=AsyncMock()) as mock_set_cache:
            await litelm.proxy.proxy_server.update_cache(
                token=None,
                user_id=None,
                end_user_id=None,
                team_id=None,
...
async def test_init_sso_settings_in_db():
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.litelm_ssoconfig.find_unique = AsyncMock(
    ...
    with patch.object(
        ...
        mock_prisma_client.db.litelm_ssoconfig.find_unique.assert_awaited_once_with(
            where={"id": "sso_config"}
        )
...
async def test_init_sso_settings_in_db_no_settings():
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.litelm_ssoconfig.find_unique = AsyncMock(return_value=None)
...
def test_get_prompt_spec_for_db_prompt_with_versions():
    ...
    mock_prompt_v1.model_dump.return_value = {
        ...
        "version": 1,
        "litelm_params": '{"prompt_id": "chat_prompt", "prompt_integration": "dotprompt", "model": "gpt-3.5-turbo", "messages": [{"role": "user", "content": "v1 content"}]}',
    ...
    mock_prompt_v2.model_dump.return_value = {
        ...
        "prompt_id": "chat_prompt",
        "version": 2,
        "litelm_params": '{"prompt_id": "chat_prompt", "prompt_integration": "dotprompt", "model": "gpt-4", "messages": [{"role": "user", "content": "v2 content"}]}',
        "prompt_info": '{"prompt_type": "db"}',
...
def test_root_redirect_when_docs_url_not_root_and_redirect_url_set(monkeypatch):
    ...
    from litelm.proxy.proxy_server import cleanup_router_config_variables
    from litelm.proxy.utils import _get_docs_url
...
async def test_get_image_non_root_uses_var_lib_assets_dir(monkeypatch):
    """
    Test that get_image uses /var/lib/litelm/assets when LITELLM_NON_ROOT is true.
    """
    from unittest.mock import patch

    from litelm.proxy.proxy_server import get_image
    ...
    def exists_side_effect(path):
        return False if path == "/var/lib/litelm/assets" else True

    with patch("litelm.proxy.proxy_server.os.makedirs") as mock_makedirs, \
         patch("litelm.proxy.proxy_server.os.path.exists", side_effect=exists_side_effect), \
         patch("litelm.proxy.proxy_server.os.access", return_value=True), \
         patch("litelm.proxy.proxy_server.os.getenv") as mock_getenv, \
         patch("litelm.proxy.proxy_server.FileResponse") as mock_file_response:
        ...
        # Verify makedirs was called with /var/lib/litelm/assets
        mock_makedirs.assert_called_once_with("/var/lib/litelm/assets", exist_ok=True)
...
async def test_get_image_non_root_fallback_to_default_logo(monkeypatch):
    ...
    in /var/lib/litelm/assets for non-root case.
    """
    from unittest.mock import patch
    ...
    # Track path.exists calls to verify it checks /var/lib/litelm/assets/logo.jpg
    exists_calls = []

    def exists_side_effect(path):
        exists_calls.append(path)
        # Return False for /var/lib/litelm/assets* so: makedirs is called, logo fallback
        # triggers, and we don't return early with cached file
        if "/var/lib/litelm/assets" in path:
            return False
        return True
    ...
    with patch("litelm.proxy.proxy_server.os.makedirs") as mock_makedirs, \
        ...
        # Verify that exists was called to check /var/lib/litelm/assets/logo.jpg
        assets_logo_path = "/var/lib/litelm/assets/logo.jpg"
...
async def test_get_image_root_case_uses_current_dir(monkeypatch):
    ...
    # Mock os.path operations
    with patch("litelm.proxy.proxy_server.os.makedirs") as mock_makedirs, \
         patch("litelm.proxy.proxy_server.os.path.exists", return_value=True), \
         patch("litelm.proxy.proxy_server.os.getenv") as mock_getenv, \
         patch("litelm.proxy.proxy_server.FileResponse") as mock_file_response:
        ...
        # Verify makedirs was NOT called with /var/lib/litelm/assets (should not create it for root case)
        var_lib_assets_calls = [
            call for call in mock_makedirs.call_args_list
            if "/var/lib/litelm/assets" in str(call)
        ]
        assert len(var_lib_assets_calls) == 0, "Should not create /var/lib/litelm/assets for root case"
...
async def test_get_image_custom_local_logo_bypasses_cache(monkeypatch):
    ...
    with patch("litelm.proxy.proxy_server.os.path.exists", return_value=True), \
         patch("litelm.proxy.proxy_server.os.access", return_value=True), \
         patch("litelm.proxy.proxy_server.FileResponse", side_effect=fake_file_response):
...
async def test_get_image_custom_logo_missing_falls_through_to_default(monkeypatch):
    ...
    with patch("litelm.proxy.proxy_server.os.path.exists", side_effect=exists_side_effect), \
         patch("litelm.proxy.proxy_server.os.access", return_value=True), \
         patch("litelm.proxy.proxy_server.FileResponse", side_effect=fake_file_response):
...
class TestInvitationEndpoints:
    ...
    def test_invitation_endpoints_proxy_admin_success(
        ...
        with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma:
            mock_prisma.db.litelm_invitationlink = MagicMock()
            if endpoint == "/invitation/new":
                mock_create = AsyncMock(return_value=mock_return)
                with patch(
                    "litelm.proxy.management_helpers.user_invitation.create_invitation_for_user",
                    mock_create,
                ):
                    response = client_with_auth.post(endpoint, json=payload)
            else:
                mock_prisma.db.litelm_invitationlink.find_unique = AsyncMock(
                    return_value={**mock_return, "created_by": "admin-user-id"}
                )
                mock_prisma.db.litelm_invitationlink.delete = AsyncMock(
    ...
    def test_invitation_endpoints_non_admin_denied(
        ...
        with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma:
            ...
            with patch(
                "litelm.proxy.proxy_server._user_has_admin_privileges",
...
def test_store_model_in_db_in_config_general_settings():
    ...
    """
    from litelm.proxy._types import ConfigGeneralSettings
...
async def test_update_general_settings_store_model_in_db_true():
    ...
    with patch(
        ...
        import litelm.proxy.proxy_server as ps
...
async def test_update_general_settings_store_model_in_db_false():
    ...
    with patch(
        "litelm.proxy.proxy_server.store_model_in_db", True
    ), patch("litelm.proxy.proxy_server.general_settings", {}):
        await proxy_config._update_general_settings(
...
async def test_store_model_in_db_db_override_when_config_false():
    ...
    with patch(
        "litelm.proxy.proxy_server.proxy_config", mock_proxy_config
    ), patch(
        "litelm.proxy.proxy_server.store_model_in_db", False
    ), patch(
        "litelm.proxy.proxy_server.get_secret_bool", return_value=False
    ):
        await ProxyStartupEvent.initialize_scheduled_background_jobs(
            general_settings={},
            prisma_client=mock_prisma_client,
...
async def test_store_model_in_db_db_check_skipped_when_already_true(monkeypatch):
    ...
    with patch(
        ...
        "litelm.proxy.proxy_server.get_secret_bool", return_value=True
    ):
        await ProxyStartupEvent.initialize_scheduled_background_jobs(
            general_settings={},
            prisma_client=mock_prisma_client,
