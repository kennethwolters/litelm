...
from litelm.proxy.image_endpoints import endpoints
...
async def test_image_generation_prompt_rerouting(monkeypatch):
    ...
    monkeypatch.setattr("litelm.proxy.proxy_server.llm_router", None)
    monkeypatch.setattr("litelm.proxy.proxy_server.proxy_config", {})
    monkeypatch.setattr("litelm.proxy.proxy_server.proxy_logging_obj", fake_proxy_logger)
    monkeypatch.setattr("litelm.proxy.proxy_server.user_model", None)
    monkeypatch.setattr("litelm.proxy.proxy_server.version", "test-version")
    monkeypatch.setattr(
        "litelm.proxy.common_request_processing.ProxyBaseLLMRequestProcessing.get_custom_headers",
        classmethod(lambda *args, **kwargs: {}),
    )
    monkeypatch.setattr(
        "litelm.proxy.image_endpoints.endpoints.route_request", fake_route_request
    )
