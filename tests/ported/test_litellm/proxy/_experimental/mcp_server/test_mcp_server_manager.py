...
# Add the parent directory to the path so we can import litelm
sys.path.insert(0, "../../../../../")
...
from litelm.proxy._types import LiteLLM_MCPServerTable, MCPTransport
from litelm.types.mcp import MCPAuth
from litelm.types.mcp_server.mcp_server_manager import MCPOAuthMetadata, MCPServer


def _reload_mcp_manager_module():
    utils_module = sys.modules["litelm.proxy._experimental.mcp_server.utils"]
    manager_module = sys.modules[
        "litelm.proxy._experimental.mcp_server.mcp_server_manager"
    ]
    ...
    server_module = sys.modules.get(
        "litelm.proxy._experimental.mcp_server.server"
    )
...
class TestMCPServerManager:
    ...
    async def test_create_mcp_client_stdio(self):
        ...
        from litelm.constants import MCP_NPM_CACHE_DIR
    ...
    async def test_fetch_oauth_metadata_from_resource_returns_servers_and_scopes(self):
        ...
        with patch(
            "litelm.proxy._experimental.mcp_server.mcp_server_manager.get_async_httpx_client",
    ...
    async def test_register_openapi_tools_includes_static_headers(self, tmp_path):
        ...
        with patch(
            "litelm.proxy._experimental.mcp_server.openapi_to_mcp_generator.create_tool_function",
            side_effect=fake_create_tool_function,
        ), patch(
            "litelm.proxy._experimental.mcp_server.openapi_to_mcp_generator.build_input_schema",
            return_value={"type": "object", "properties": {}, "required": []},
        ), patch(
            "litelm.proxy._experimental.mcp_server.tool_registry.global_mcp_tool_registry.register_tool",
            return_value=None,
        ):
    ...
    async def test_rest_endpoint_filters_by_allowed_tools(self):
        ...
        from litelm.proxy._experimental.mcp_server import rest_endpoints
    ...
    async def test_get_allowed_mcp_servers_with_user_api_key_auth(self):
        ...
        litelm_metadata from responses API.
        """
    ...
    def test_get_mcp_server_from_tool_name_uses_server_name_not_name(self):
        ...
        """
        from litelm.proxy._experimental.mcp_server.utils import (
