...
from litelm.proxy._experimental.mcp_server.mcp_debug import (
...
class TestMask:
    ...
    def test_litelm_key_masked(self):
...
class TestBuildDebugHeaders:
    ...
    def test_litelm_key_in_dedicated_header(self):
        headers = MCPDebug.build_debug_headers(
            inbound_headers={
                "x-litelm-api-key": "Bearer sk-1234567890abcdef",
                "host": "localhost",
            },
            oauth2_headers=None,
            litelm_api_key="Bearer sk-1234567890abcdef",
            auth_resolution="no-auth",
            server_url="https://mcp.example.com",
            server_auth_type="oauth2",
        )
        assert "x-litelm-api-key=" in headers["x-mcp-debug-inbound-auth"]
        assert headers["x-mcp-debug-oauth2-token"] == "(none)"

    def test_same_key_flagged(self):
        """When Authorization and x-litelm-api-key carry the same token."""
        headers = MCPDebug.build_debug_headers(
            inbound_headers={
    ...
    def test_different_tokens_not_flagged(self):
        ...
        headers = MCPDebug.build_debug_headers(
            inbound_headers={
                "x-litelm-api-key": "Bearer sk-litelm-key-here",
                "authorization": "Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.atlassian",
            },
            oauth2_headers={
                "Authorization": "Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.atlassian"
            },
            litelm_api_key="Bearer sk-litelm-key-here",
            auth_resolution="oauth2-passthrough",
    ...
    def test_m2m_resolution(self):
        headers = MCPDebug.build_debug_headers(
            inbound_headers={"x-litelm-api-key": "Bearer sk-key"},
            oauth2_headers=None,
            litelm_api_key="Bearer sk-key",
