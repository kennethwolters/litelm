...
def cleanup_mcp_global_state():
    ...
    """
    try:
        from litelm.proxy._experimental.mcp_server.mcp_server_manager import (
...
async def test_mcp_server_tool_call_body_contains_request_data():
    ...
    try:
        from litelm.proxy._experimental.mcp_server.server import (
    ...
    # Mock the add_litelm_data_to_request function to capture the data
    captured_data = {}

    async def mock_add_litelm_data_to_request(
    ...
    with patch(
        "litelm.proxy.litelm_pre_call_utils.add_litelm_data_to_request",
        mock_add_litelm_data_to_request,
    ):
        with patch(
            "litelm.proxy._experimental.mcp_server.server.call_mcp_tool",
            mock_call_mcp_tool,
        ):
            with patch(
...
async def test_get_prompts_from_mcp_servers_success():
    ...
    with patch(
        "litelm.proxy._experimental.mcp_server.server._get_allowed_mcp_servers",
        AsyncMock(return_value=[server_a, server_b]),
    ) as mock_allowed, patch(
        "litelm.proxy._experimental.mcp_server.server._prepare_mcp_server_headers",
        return_value=(None, None),
    ) as mock_headers, patch(
        "litelm.proxy._experimental.mcp_server.server.global_mcp_server_manager",
    ) as mock_manager:
...
async def test_mcp_get_prompt_success():
    try:
        from litelm.proxy._experimental.mcp_server.server import mcp_get_prompt
    except ImportError:
        pytest.skip("MCP server not available")
...
async def test_mcp_read_resource_success():
    try:
        from litelm.proxy._experimental.mcp_server.server import mcp_read_resource
    except ImportError:
        pytest.skip("MCP server not available")
...
async def test_get_tools_from_mcp_servers_continues_when_one_server_fails():
    ...
    with patch(
        ...
        with patch(
            "litelm.proxy._experimental.mcp_server.server.verbose_logger",
        ) as mock_logger:
...
async def test_concurrent_initialize_session_managers():
    ...
    import litelm.proxy._experimental.mcp_server.server as mcp_server
    ...
    try:
        ...
        with patch(
            "litelm.proxy._experimental.mcp_server.server.session_manager"
        ) as mock_session_manager, patch(
            "litelm.proxy._experimental.mcp_server.server.sse_session_manager"
...
async def test_streamable_http_session_manager_is_stateless():
    ...
    """
    try:
        from litelm.proxy._experimental.mcp_server.server import session_manager
    except ImportError:
        pytest.skip("MCP server not available")
    ...
    assert session_manager.stateless is True, (
        ...
        "See: https://github.com/BerriAI/litelm/issues/20242"
    )
...
async def test_mcp_routing_with_conflicting_alias_and_group_name():
    ...
    try:
        ...
        from litelm.proxy._types import MCPTransport
        from litelm.types.mcp_server.mcp_server_manager import MCPServer
    except ImportError:
        pytest.skip("MCP server not available")
    ...
    with patch(
        "litelm.proxy._experimental.mcp_server.server.global_mcp_server_manager.get_allowed_mcp_servers",
        mock_get_allowed,
    ), patch(
        "litelm.proxy._experimental.mcp_server.server.MCPRequestHandler._get_mcp_servers_from_access_groups",
        mock_db_lookup,
    ), patch(
        "litelm.proxy._experimental.mcp_server.server.global_mcp_server_manager._get_tools_from_server",
        mock_get_tools_spy,
    ):
...
async def test_mcp_manager_allows_public_servers_without_permissions():
    ...
    with patch(
        "litelm.proxy.management_endpoints.common_utils._user_has_admin_view",
        return_value=False,
    ), patch(
        "litelm.proxy._experimental.mcp_server.mcp_server_manager.MCPRequestHandler.get_allowed_mcp_servers",
...
async def test_call_mcp_tool_user_unauthorized_access():
    ...
    from litelm.proxy._experimental.mcp_server.server import call_mcp_tool
    ...
    with patch(
        "litelm.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp.MCPRequestHandler.get_allowed_mcp_servers",
        AsyncMock(return_value=["allowed_server", "another_server"]),
    ), patch(
        "litelm.proxy._experimental.mcp_server.server.global_mcp_server_manager.get_mcp_server_by_id",
...
async def test_list_tools_filters_by_key_team_permissions():
    ...
    try:
        ...
        from litelm.proxy._types import LiteLLM_ObjectPermissionTable, UserAPIKeyAuth
    except ImportError:
        pytest.skip("MCP server not available")
...
async def test_list_tools_with_team_tool_permissions_inheritance():
    ...
    with patch(
        ...
        with patch(
            "litelm.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp.MCPRequestHandler._get_team_object_permission",
...
async def test_list_tools_strips_prefix_when_matching_permissions():
    ...
    Tools from MCP servers are prefixed (e.g., "GITMCP-fetch_litelm_documentation"),
    but allowed tools in DB are stored without prefix (e.g., "fetch_litelm_documentation").
    ...
    object_permission = LiteLLM_ObjectPermissionTable(
        ...
        mcp_tool_permissions={
            "gitmcp_server": [
                "fetch_litelm_documentation",  # No prefix in DB
                "search_litelm_code",  # No prefix in DB
            ],
        },
    )
    ...
    async def mock_get_tools_from_server(
        ...
        tool1 = MagicMock()
        tool1.name = "GITMCP-fetch_litelm_documentation"  # Prefixed
        tool1.description = "Fetch docs"
        tool1.inputSchema = {}

        tool2 = MagicMock()
        tool2.name = (
            "GITMCP-search_litelm_documentation"  # Prefixed, not in allowed list
        )
        tool2.description = "Search docs"
        tool2.inputSchema = {}

        tool3 = MagicMock()
        tool3.name = "GITMCP-search_litelm_code"  # Prefixed
    ...
    assert tool_names == [
        "GITMCP-fetch_litelm_documentation",
        "GITMCP-search_litelm_code",
    ]
...
def test_filter_tools_by_allowed_tools():
    ...
    from litelm.types.mcp import MCPTransport
    from litelm.types.mcp_server.mcp_server_manager import MCPServer
...
def test_apply_tool_overrides():
    ...
    from litelm.proxy._experimental.mcp_server.server import apply_tool_overrides
    from litelm.types.mcp import MCPTransport
    from litelm.types.mcp_server.mcp_server_manager import MCPServer
...
class TestMCPServerManagerReload:
    ...
    async def test_reuses_existing_server_when_updated_at_matches(self):
        ...
        with patch(
            "litelm.proxy._experimental.mcp_server.db.get_all_mcp_servers",
...
async def test_call_mcp_tool_logs_failure_via_post_call_failure_hook():
    ...
    try:
        ...
        from litelm.proxy._types import MCPTransport, UserAPIKeyAuth
        from litelm.types.mcp_server.mcp_server_manager import MCPServer
    except ImportError:
        pytest.skip("MCP server not available")
    ...
    with patch.object(
        ...
        "litelm.proxy._experimental.mcp_server.server._get_allowed_mcp_servers_from_mcp_server_names",
        new_callable=AsyncMock,
        return_value=[mock_server],
    ), patch(
        "litelm.proxy._experimental.mcp_server.server.execute_mcp_tool",
        new_callable=AsyncMock,
...
async def test_get_tools_from_mcp_servers_logs_list_tools_to_spendlogs_when_enabled():
    ...
    with patch(
        ...
        "litelm.proxy._experimental.mcp_server.server.filter_tools_by_allowed_tools",
        side_effect=lambda tools, _server: tools,
    ), patch(
        "litelm.proxy._experimental.mcp_server.server.filter_tools_by_key_team_permissions",
        new=AsyncMock(side_effect=lambda tools, **_: tools),
    ), patch(
        "litelm.proxy._experimental.mcp_server.server.function_setup",
...
def test_tool_name_matches_case_insensitive():
    ...
    """
    try:
        from litelm.proxy._experimental.mcp_server.server import _tool_name_matches
    except ImportError:
        pytest.skip("MCP server not available")
...
def test_filter_tools_by_allowed_tools_case_insensitive():
    ...
    try:
        ...
        from litelm.types.mcp_server.tool_registry import MCPTool
    except ImportError:
        pytest.skip("MCP server not available")
...
def test_filter_tools_by_allowed_tools_no_filter():
    ...
    tools = [
        MCPTool(
            name="fusion_litelm_mcp-model_list",
            description="List models",
            input_schema={"type": "object"},
            handler=mock_handler,
        ),
        MCPTool(
            name="fusion_litelm_mcp-chat_completion",
...
async def test_get_tools_from_mcp_servers_injects_stored_oauth2_token():
    ...
    with patch(
        ...
        "litelm.proxy._experimental.mcp_server.server._prefetch_oauth_creds_for_user",
