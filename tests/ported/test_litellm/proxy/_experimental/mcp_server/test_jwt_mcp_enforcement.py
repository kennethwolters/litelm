...
from litelm.proxy._experimental.mcp_server.mcp_server_manager import MCPServerManager
...
async def test_reproduce_jwt_mcp_enforcement_issue(monkeypatch):
    ...
    # Setup mock router
    router = Router(model_list=[{"model_name": "gpt-4", "litelm_params": {"model": "gpt-4"}}])
    import sys
    import types
...
async def test_mcp_route_check_passes_for_team():
    ...
    from litelm.proxy.auth.auth_checks import allowed_routes_check
...
async def test_e2e_jwt_team_mcp_permissions_enforced(monkeypatch):
    ...
    monkeypatch.setattr(
        "litelm.proxy.auth.auth_checks.get_team_object", mock_get_team_object
    )
