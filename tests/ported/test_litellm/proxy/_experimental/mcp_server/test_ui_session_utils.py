...
from litelm.proxy._experimental.mcp_server.ui_session_utils import (
...
async def test_resolve_ui_session_team_ids_returns_unique_ids(monkeypatch):
    ...
    monkeypatch.setattr(
        "litelm.proxy.auth.auth_checks.get_user_object",
...
async def test_build_effective_auth_contexts_returns_cloned_contexts(monkeypatch):
    ...
    monkeypatch.setattr(
        "litelm.proxy._experimental.mcp_server.ui_session_utils.resolve_ui_session_team_ids",
        mock_resolve,
    )
