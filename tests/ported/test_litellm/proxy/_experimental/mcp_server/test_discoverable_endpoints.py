...
def mock_mcp_client_ip():
    ...
    """
    with patch(
        "litelm.proxy._experimental.mcp_server.discoverable_endpoints.IPAddressUtils.get_mcp_client_ip",
        return_value=None,
    ):
        yield
...
async def test_oauth_callback_redirects_with_state():
    ...
    with patch(
        "litelm.proxy._experimental.mcp_server.discoverable_endpoints.decode_state_hash"
    ) as mock_decode:
