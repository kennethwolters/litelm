...
from litelm.proxy._experimental.mcp_server.auth import (
    user_api_key_auth_mcp as auth_mcp,
)
from litelm.proxy._types import NewMCPServerRequest, UserAPIKeyAuth
...
class TestExecuteWithMcpClient:
    ...
    async def test_m2m_drops_incoming_oauth2_headers(self, monkeypatch):
        ...
        the litelm API key) must NOT be forwarded as extra_headers — otherwise
        ...
        incoming_oauth2 = {"Authorization": "Bearer sk-litelm-api-key"}
        result = await rest_endpoints._execute_with_mcp_client(
            payload,
            ok_operation,
...
class TestListToolsRestAPI:
    ...
    async def test_name_resolution_finds_server_by_uuid(self, monkeypatch):
        ...
        from litelm.proxy._experimental.mcp_server.server import MCPServer
...
class TestCallToolRestAPI:
    ...
    async def test_rejects_disallowed_server(self, monkeypatch):
        ...
        async def fake_add_litelm_data_to_request(**kwargs):
        ...
        monkeypatch.setattr(
            "litelm.proxy.proxy_server.add_litelm_data_to_request",
            fake_add_litelm_data_to_request,
            raising=False,
        )
