...
from litelm.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp import (
    MCPRequestHandler,
)
from litelm.proxy._types import SpecialHeaders, UserAPIKeyAuth
...
class TestMCPRequestHandler:
    ...
    async def test_permission_inheritance_edge_cases(self):
        ...
        mock_prisma_client.db.litelm_objectpermissiontable.find_unique.return_value = (
            None
        )
        ...
        mock_prisma_client.db.litelm_objectpermissiontable.find_unique.side_effect = (
            Exception("DB Error")
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            # Test case 1: x-litelm-api-key header present
            (
                [(b"x-litelm-api-key", b"test-api-key-123")],
                "test-api-key-123",
                None,
                {},
            ),
            ...
            (
                [
                    (b"x-litelm-api-key", b"primary-key"),
    ...
    async def test_process_mcp_request_with_server_auth_headers(
        ...
        with patch(
            "litelm.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp.user_api_key_auth",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                [
                    (b"x-litelm-api-key", b"test-api-key"),
            ...
            # Test case 6: Using Authorization header instead of x-litelm-api-key
            (
                [
    ...
    async def test_header_extraction(self, headers, expected_result):
        ...
        api_key = MCPRequestHandler.get_litelm_api_key_from_headers(extracted_headers)
...
class TestMCPOAuth2AuthFlow:
    ...
    async def test_explicit_litelm_key_with_oauth2_authorization(self):
        """
        When both x-litelm-api-key AND Authorization header are present,
        ...
        scope = {
            ...
            "path": "/mcp/atlassian_mcp",
            "headers": [
                (b"x-litelm-api-key", b"sk-litelm-valid-key"),
        ...
        with patch(
            ...
            assert call_args.kwargs["api_key"] == "sk-litelm-valid-key"
    ...
    async def test_litelm_key_in_authorization_backward_compat(self):
        """
        ...
        scope = {
            ...
            "headers": [
                (b"authorization", b"Bearer sk-litelm-valid-key"),
            ],
        }
        ...
        with patch(
            ...
            assert auth_result.api_key == "Bearer sk-litelm-valid-key"
            mock_auth.assert_called_once()
...
class TestMCPCustomHeaderName:
    ...
    def test_get_mcp_client_side_auth_header_name(
        ...
        with patch("litelm.secret_managers.main.get_secret_str") as mock_get_secret:
            with patch(
    ...
    def test_get_mcp_server_auth_headers_from_headers(self):
        ...
        headers = Headers(
            {"x-litelm-api-key": "test-key", "content-type": "application/json"}
        )
        ...
        headers = Headers(
            {"x-litelm-api-key": "test-key", "x-mcp-github-authorization": ""}
        )
        result = MCPRequestHandler._get_mcp_server_auth_headers_from_headers(headers)
        ...
        headers = Headers(
            {"x-litelm-api-key": "test-key", "x-mcp-github-authorization": long_token}
        )
        result = MCPRequestHandler._get_mcp_server_auth_headers_from_headers(headers)
...
def test_mcp_path_based_server_segregation(monkeypatch):
    ...
    from litelm.proxy._experimental.mcp_server.server import app, get_auth_context
    ...
    monkeypatch.setattr(
        "litelm.proxy._experimental.mcp_server.server.initialize_session_managers",
        AsyncMock(),
    )
    ...
    response = client.get(
        "/mcp/zapier,group1/tools", headers={"x-litelm-api-key": "test"}
    )
...
async def test_get_team_object_permission_with_already_loaded_permission():
    ...
    """
    from litelm.proxy._types import LiteLLM_ObjectPermissionTable, LiteLLM_TeamTable
    ...
    with patch(
        ...
        with patch("litelm.proxy.auth.auth_checks.get_team_object") as mock_get_team:
            with patch(
                "litelm.proxy.auth.auth_checks.get_object_permission"
            ) as mock_get_perm:
...
async def test_get_allowed_mcp_servers_for_key_guard_conditions(
    ...
    with patch(
        ...
        with patch("litelm.proxy.proxy_server.prisma_client", prisma_client_value):
...
async def test_get_allowed_mcp_servers_for_key_returns_empty_when_db_returns_none():
    ...
    with patch("litelm.proxy.proxy_server.prisma_client", mock_prisma), patch(
        "litelm.proxy.auth.auth_checks.get_object_permission",
        new_callable=AsyncMock,
    ) as mock_get_perm:
...
async def test_tool_permission_servers_included_in_allowed_servers():
    ...
    Regression test for https://github.com/BerriAI/litelm/issues/21954
    """
    perm = MagicMock()
    perm.mcp_servers = []
