...
from litelm.proxy._experimental.mcp_server.byok_oauth_endpoints import (
    _byok_auth_codes,
    _verify_pkce,
    router,
)
...
async def test_token_endpoint_success():
    ...
    with patch(
        "litelm.proxy._experimental.mcp_server.byok_oauth_endpoints.store_user_credential",
        mock_store,
    ), patch(
        "litelm.proxy._experimental.mcp_server.byok_oauth_endpoints.router",
    ):
        ...
        with patch(
            ...
            import litelm.proxy._experimental.mcp_server.byok_oauth_endpoints as mod
            ...
            ), patch("litelm.proxy.proxy_server.master_key", test_master_key):
...
async def test_token_endpoint_invalid_code():
    from litelm.proxy._experimental.mcp_server.byok_oauth_endpoints import byok_token

    mock_request = MagicMock()
    with pytest.raises(HTTPException) as exc_info:
        with patch("litelm.proxy.proxy_server.prisma_client", MagicMock()), patch(
            "litelm.proxy.proxy_server.master_key", "key"
        ):
            await byok_token(
                request=mock_request,
                grant_type="authorization_code",
...
async def test_check_byok_credential_not_byok():
    ...
    from litelm.proxy._experimental.mcp_server.server import _check_byok_credential
...
async def test_check_byok_credential_missing_credential():
    ...
    with patch(
        "litelm.proxy._experimental.mcp_server.db.get_user_credential",
