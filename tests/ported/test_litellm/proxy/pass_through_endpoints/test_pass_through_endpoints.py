...
from litelm.proxy.pass_through_endpoints.success_handler import (
...
async def test_pass_through_request_failure_handler():
    ...
    Critical Test: When a users pass through endpoint request fails, we must log the failure code, exception in litelm spend logs.
    """
    with patch("litelm.proxy.proxy_server.proxy_logging_obj") as mock_proxy_logging:
        with patch(
            "litelm.llms.custom_httpx.http_handler.get_async_httpx_client"
        ) as mock_get_client:
            with patch(
                "litelm.proxy.pass_through_endpoints.pass_through_endpoints.ProxyBaseLLMRequestProcessing"
            ) as mock_processing:
...
async def test_langfuse_passthrough_no_logging():
    ...
    from litelm.types.passthrough_endpoints.pass_through_endpoints import (
...
async def test_initialize_pass_through_endpoints_with_include_subpath():
    ...
    with patch(
        "litelm.proxy.pass_through_endpoints.pass_through_endpoints.InitPassThroughEndpointHelpers.add_exact_path_route"
    ) as mock_add_exact_route:
        with patch(
            "litelm.proxy.pass_through_endpoints.pass_through_endpoints.InitPassThroughEndpointHelpers.add_subpath_route"
            ...
            with patch(
                ...
                with patch(
                    "litelm.proxy.pass_through_endpoints.pass_through_endpoints.set_env_variables_in_header"
                ) as mock_set_env:
...
async def test_create_pass_through_route_with_cost_per_request():
    ...
    with patch(
        "litelm.proxy.pass_through_endpoints.pass_through_endpoints.pass_through_request"
    ) as mock_pass_through, patch(
        "litelm.proxy.pass_through_endpoints.pass_through_endpoints.InitPassThroughEndpointHelpers.is_registered_pass_through_route"
    ) as mock_is_registered, patch(
        "litelm.proxy.pass_through_endpoints.pass_through_endpoints.InitPassThroughEndpointHelpers.get_registered_pass_through_route"
...
async def test_pass_through_request_contains_proxy_server_request_in_kwargs():  # noqa: PLR0915
    ...
    with patch("litelm.proxy.proxy_server.proxy_logging_obj") as mock_proxy_logging:
        with patch(
            "litelm.proxy.pass_through_endpoints.pass_through_endpoints.HttpPassThroughEndpointHelpers.non_streaming_http_request_handler"
        ) as mock_http_handler:
            with patch(
                "litelm.proxy.pass_through_endpoints.pass_through_endpoints.ProxyBaseLLMRequestProcessing"
            ) as mock_processing:
                with patch(
                    "litelm.proxy.pass_through_endpoints.pass_through_endpoints.pass_through_endpoint_logging.pass_through_async_success_handler"
                ) as mock_success_handler:
                    with patch(
                        "litelm.proxy.pass_through_endpoints.pass_through_endpoints.get_response_body"
                        ...
                        # Verify that litelm_params exists in kwargs
                        assert "litelm_params" in call_kwargs
                        litelm_params = call_kwargs["litelm_params"]

                        # Verify that proxy_server_request exists in litelm_params
                        assert "proxy_server_request" in litelm_params
                        proxy_server_request = litelm_params["proxy_server_request"]
                        ...
                        assert "litelm_call_id" in call_kwargs
                        assert "passthrough_logging_payload" in call_kwargs

                        # Verify metadata contains user information
                        assert "metadata" in litelm_params
                        metadata = litelm_params["metadata"]
...
async def test_create_pass_through_endpoint():
    ...
    with patch(
        "litelm.proxy.proxy_server.get_config_general_settings"
    ) as mock_get_config:
        with patch(
            "litelm.proxy.proxy_server.update_config_general_settings"
...
async def test_delete_pass_through_endpoint_not_found():
    ...
    from litelm.proxy._types import ConfigFieldInfo, UserAPIKeyAuth
    from litelm.proxy.pass_through_endpoints.pass_through_endpoints import (
        delete_pass_through_endpoints,
    )
...
async def test_get_pass_through_endpoints_includes_config_and_db():
    ...
    with patch(
        ...
        with patch(
            "litelm.proxy.pass_through_endpoints.pass_through_endpoints._get_pass_through_endpoints_from_db",
            new_callable=AsyncMock,
        ) as mock_get_db:
            with patch(
                "litelm.proxy.pass_through_endpoints.pass_through_endpoints._get_pass_through_endpoints_from_config"
            ) as mock_get_config:
                db_objects = [
...
def test_get_pass_through_endpoints_from_config_skips_malformed():
    ...
    with patch(
        "litelm.proxy.proxy_server.config_passthrough_endpoints",
        config_passthrough_endpoints,
    ):
...
async def test_filter_endpoints_by_team_allowed_routes_with_filter():
    ...
    """
    from litelm.proxy._types import PassThroughGenericEndpoint
    from litelm.proxy.pass_through_endpoints.pass_through_endpoints import (
    ...
    mock_prisma_client.db.litelm_teamtable.find_unique.assert_called_once_with(
...
async def test_bedrock_router_passthrough_metadata_initialization():
    ...
    litelm_params.proxy_server_request were missing for /bedrock passthrough
...
@pytest.mark.asyncio
async def test_add_litelm_data_to_request_adds_headers_to_metadata():
    """
    Test that add_litelm_data_to_request adds headers to metadata for guardrails.
...
async def test_create_pass_through_route_custom_body_url_target():
    ...
    with patch(
        ...
        "litelm.proxy.pass_through_endpoints.pass_through_endpoints._parse_request_data_by_content_type"
...
def test_build_full_path_with_root_default():
    ...
    with patch("litelm.proxy.pass_through_endpoints.pass_through_endpoints.get_server_root_path") as mock_get_root:
...
def test_get_registered_pass_through_route_with_custom_root():
    ...
    with patch("litelm.proxy.pass_through_endpoints.pass_through_endpoints.get_server_root_path") as mock_get_root:
        # Test with custom root path /litelm
        mock_get_root.return_value = "/litelm"

        # Should return config when request route includes root path
        result = InitPassThroughEndpointHelpers.get_registered_pass_through_route("/litelm/chat/completions")
...
def test_mapped_pass_through_routes_with_server_root_path():
    ...
    Regression test for https://github.com/BerriAI/litelm/issues/22272
    """
    from litelm.proxy.pass_through_endpoints.pass_through_endpoints import (
        InitPassThroughEndpointHelpers,
    )
    ...
    with patch(
        "litelm.proxy.utils.get_server_root_path"
    ) as mock_get_root:
        mock_get_root.return_value = "/litelm"
        ...
        assert (
            InitPassThroughEndpointHelpers.is_registered_pass_through_route(
                "/litelm/vertex_ai/v1/projects/foo"
            )
            is True
        )
        assert (
            InitPassThroughEndpointHelpers.is_registered_pass_through_route(
                "/litelm/bedrock/model/invoke"
            )
            is True
        )
