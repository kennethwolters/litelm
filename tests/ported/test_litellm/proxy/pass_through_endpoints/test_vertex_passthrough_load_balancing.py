...
async def test_vertex_passthrough_load_balancing():
    ...
    with patch("litelm.llms.vertex_ai.common_utils.get_vertex_model_id_from_url", return_value="gemini-pro"), \
         patch("litelm.proxy.proxy_server.llm_router", mock_router), \
         patch("litelm.llms.vertex_ai.common_utils.get_vertex_project_id_from_url", return_value=None), \
         patch("litelm.llms.vertex_ai.common_utils.get_vertex_location_from_url", return_value=None), \
         patch("litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints.passthrough_endpoint_router") as mock_pt_router, \
         patch("litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints._prepare_vertex_auth_headers", new_callable=AsyncMock) as mock_prep_headers, \
         patch("litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints.create_pass_through_route") as mock_create_route, \
         patch("litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints.user_api_key_auth", new_callable=AsyncMock) as mock_auth:
...
def test_get_available_deployment_for_pass_through_filters_correctly():
    ...
    assert deployment["litelm_params"]["vertex_project"] == "project-1"
    assert deployment["litelm_params"]["use_in_pass_through"] is True
...
def test_get_available_deployment_for_pass_through_load_balancing():
    ...
    for _ in range(100):
        deployment = router.get_available_deployment_for_pass_through(model="gemini-pro")
        project = deployment["litelm_params"]["vertex_project"]
...
@pytest.mark.asyncio
async def test_vertex_passthrough_does_not_forward_litelm_auth_token():
    """
    ...
    The incoming request has:
      - authorization: Bearer <litelm_token>  (should NOT be forwarded)
    ...
    # Create a mock request with ONLY the litelm auth token (no other headers)
    mock_request = MagicMock()
    mock_request.headers = Headers({
        "authorization": "Bearer sk-litelm-secret-key",  # LiteLLM token - should NOT be forwarded
        "Authorization": "Bearer sk-litelm-secret-key-uppercase",  # Also try uppercase
    })
    ...
    with patch.object(
        ...
        assert headers.get("Authorization") != "Bearer sk-litelm-secret-key"
        assert headers.get("Authorization") != "Bearer sk-litelm-secret-key-uppercase"
...
def test_forward_headers_from_request_x_pass_prefix():
    ...
    """
    from litelm.passthrough.utils import BasePassthroughUtils
    ...
    request_headers = {
        ...
        "authorization": "Bearer sk-litelm-key",
        "x-litelm-api-key": "sk-1234",
        "content-type": "application/json",
    }
    ...
    assert "x-litelm-api-key" not in result
