...
from litelm.proxy.pass_through_endpoints.llm_provider_handlers.openai_passthrough_logging_handler import (
...
class TestOpenAIPassthroughLoggingHandler:
    ...
    @patch('litelm.completion_cost')
    @patch('litelm.litelm_core_utils.litelm_logging.get_standard_logging_object_payload')
    ...
    def test_openai_passthrough_handler_with_user_tracking(self, mock_get_standard_logging, mock_completion_cost):
        ...
        # Verify user information is included in litelm_params
        assert "litelm_params" in result["kwargs"]
        assert "proxy_server_request" in result["kwargs"]["litelm_params"]
        assert "body" in result["kwargs"]["litelm_params"]["proxy_server_request"]
        assert result["kwargs"]["litelm_params"]["proxy_server_request"]["body"]["user"] == "test_user_123"
    ...
    def test_build_complete_streaming_response(self):
        ...
        result = self.handler._build_complete_streaming_response(
            ...
            litelm_logging_obj=self._create_mock_logging_obj(),
            model="gpt-4o",
        )
    ...
    def test_azure_passthrough_tags_metadata_model_provider(self, mock_get_standard_logging, mock_completion_cost):
        ...
        # Set up kwargs with existing litelm_params containing metadata tags
        kwargs = {
            "passthrough_logging_payload": passthrough_payload,
            "model": "gpt-4o",
        ...
        # Verify metadata tags are preserved in litelm_params
        assert "litelm_params" in result["kwargs"]
        assert "metadata" in result["kwargs"]["litelm_params"]
        assert "tags" in result["kwargs"]["litelm_params"]["metadata"]
        assert result["kwargs"]["litelm_params"]["metadata"]["tags"] == ["production", "azure-deployment"]
        assert result["kwargs"]["litelm_params"]["metadata"]["user_id"] == "user_123"
    ...
    @patch('litelm.completion_cost')
    @patch('litelm.litelm_core_utils.litelm_logging.get_standard_logging_object_payload')
    @patch('litelm.proxy.pass_through_endpoints.llm_provider_handlers.openai_passthrough_logging_handler.OpenAIPassthroughLoggingHandler.get_provider_config')
...
class TestOpenAIPassthroughIntegration:
    ...
    @patch('litelm.proxy.pass_through_endpoints.llm_provider_handlers.openai_passthrough_logging_handler.OpenAIPassthroughLoggingHandler.openai_passthrough_handler')
    @pytest.mark.asyncio
    ...
    @patch('litelm.cost_calculator.default_image_cost_calculator')
