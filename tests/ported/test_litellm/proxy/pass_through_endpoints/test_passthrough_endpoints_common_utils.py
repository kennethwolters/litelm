...
from litelm.passthrough.utils import CommonUtils
...
from litelm.proxy.pass_through_endpoints.common_utils import get_litelm_virtual_key


@pytest.mark.asyncio
async def test_get_litelm_virtual_key():
    """
    Test that the get_litelm_virtual_key function correctly handles the API key authentication
    """
    # Test with x-litelm-api-key
    mock_request = Mock()
    mock_request.headers = {"x-litelm-api-key": "test-key-123"}
    result = get_litelm_virtual_key(mock_request)
    ...
    # Test with both headers (x-litelm-api-key should take precedence)
    mock_request.headers = {
        "x-litelm-api-key": "test-key-123",
