...
from litelm.types.passthrough_endpoints.vertex_ai import VertexPassThroughCredentials
...
class TestBaseOpenAIPassThroughHandler:
    def test_join_url_paths(self):
        ...
        result = BaseOpenAIPassThroughHandler._join_url_paths(
            base_url, path, litelm.LlmProviders.OPENAI.value
        )
    ...
    @patch(
        "litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints.create_pass_through_route"
    )
    async def test_base_openai_pass_through_handler(self, mock_create_pass_through):
        ...
        result = await BaseOpenAIPassThroughHandler._base_openai_pass_through_handler(
            ...
            api_key="test_api_key",
            custom_llm_provider=litelm.LlmProviders.OPENAI.value,
        )
...
class TestVertexAIPassThroughHandler:
    ...
    async def test_vertex_passthrough_with_credentials(self, monkeypatch):
        ...
        """
        from litelm.proxy.pass_through_endpoints.passthrough_endpoint_router import (
        ...
        monkeypatch.setattr(
            "litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints.passthrough_endpoint_router",
            pass_through_router,
        )
        ...
        with mock.patch(
            "litelm.llms.vertex_ai.vertex_llm_base.VertexBase.load_auth"
        ) as mock_load_auth, mock.patch(
            "litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints.create_pass_through_route"
        ) as mock_create_route, mock.patch(
            "litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints.get_litelm_virtual_key"
        ) as mock_get_virtual_key, mock.patch(
            "litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints.user_api_key_auth"
        ) as mock_user_auth, mock.patch(
            "litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints.get_vertex_pass_through_handler"
        ) as mock_get_handler:
    ...
    async def test_vertex_passthrough_with_no_default_credentials(self, monkeypatch):
        ...
        with mock.patch(
            "litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints.vertex_llm_base._ensure_access_token_async"
        ) as mock_ensure_token, mock.patch(
            "litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints.vertex_llm_base._get_token_and_url"
    ...
    async def test_async_vertex_proxy_route_api_key_auth(self):
        ...
        # Mock dependencies
        mock_request = Mock()
        mock_request.headers = {"x-litelm-api-key": "test-key-123"}
        mock_request.method = "POST"
        mock_response = Mock()
    ...
    def test_vertex_passthrough_handler_multimodal_embedding_response(self):
        ...
        from litelm.proxy.pass_through_endpoints.llm_provider_handlers.vertex_passthrough_logging_handler import (
        ...
        mock_logging_obj.litelm_call_id = "test-call-id-123"
        ...
        with patch(
            "litelm.llms.vertex_ai.multimodal_embeddings.transformation.VertexAIMultimodalEmbeddingConfig"
            ...
            from litelm.types.utils import Embedding, EmbeddingResponse, Usage
    ...
    def test_vertex_passthrough_handler_predict_cost_tracking(self):
        ...
        with patch("litelm.completion_cost") as mock_completion_cost:
...
class TestBedrockLLMProxyRoute:
    ...
    async def test_bedrock_llm_proxy_route_application_inference_profile(self):
        ...
        with patch(
            "litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints._read_request_body",
            return_value=mock_request_body,
        ), patch(
            "litelm.proxy.common_request_processing.ProxyBaseLLMRequestProcessing",
    ...
    async def test_bedrock_error_handling_returns_actual_error(self):
        ...
        with patch(
            "litelm.proxy.common_request_processing.ProxyBaseLLMRequestProcessing.base_passthrough_process_llm_request",
            new_callable=AsyncMock,
    ...
    async def test_bedrock_passthrough_uses_model_specific_credentials(self):
        ...
        from litelm.litelm_core_utils.get_litelm_params import get_litelm_params
        from litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints import (
            handle_bedrock_passthrough_router_model,
        )
        ...
        with patch.dict(
            ...
            # Test 1: Verify get_litelm_params extracts AWS credentials from kwargs
            kwargs_with_creds = {
            ...
            litelm_params = get_litelm_params(**kwargs_with_creds)

            # Verify credentials are extracted
            assert litelm_params.get("aws_access_key_id") == model_access_key
            assert litelm_params.get("aws_secret_access_key") == model_secret_key
            assert litelm_params.get("aws_region_name") == model_region
            assert litelm_params.get("aws_session_token") == model_session_token
            ...
            deployment_litelm_params = deployment.get("litelm_params", {})

            # Verify model-specific credentials are in the deployment
            assert deployment_litelm_params.get("aws_access_key_id") == model_access_key
            assert deployment_litelm_params.get("aws_secret_access_key") == model_secret_key
            assert deployment_litelm_params.get("aws_region_name") == model_region
            assert deployment_litelm_params.get("aws_session_token") == model_session_token

            # Verify environment variables are NOT in the deployment
            assert deployment_litelm_params.get("aws_access_key_id") != env_access_key
            assert deployment_litelm_params.get("aws_secret_access_key") != env_secret_key
            assert deployment_litelm_params.get("aws_region_name") != env_region
            ...
            with patch(
                "litelm.passthrough.main.llm_passthrough_route",
                new_callable=AsyncMock,
                ...
                # The key verification is that get_litelm_params extracts the credentials
...
class TestLLMPassthroughFactoryProxyRoute:
    ...
    async def test_llm_passthrough_factory_proxy_route_success(self):
        ...
        with patch(
            "litelm.utils.ProviderConfigManager.get_provider_model_info"
        ) as mock_get_provider, patch(
            "litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints.passthrough_endpoint_router.get_credentials"
            ...
            mock_get_provider.assert_called_once_with(
                provider=litelm.LlmProviders(LlmProviders.VLLM), model=None
            )
...
class TestVLLMProxyRoute:
    @pytest.mark.asyncio
    @patch(
        "litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints.get_request_body",
        return_value={"model": "router-model", "stream": False},
    )
    @patch(
        "litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints.is_passthrough_request_using_router_model",
        return_value=True,
    )
    @patch("litelm.proxy.proxy_server.llm_router")
    ...
    @patch(
        "litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints.llm_passthrough_factory_proxy_route"
    )
...
class TestForwardHeaders:
    ...
    async def test_pass_through_request_with_forward_headers_true(self):
        ...
        """
        from litelm.proxy.pass_through_endpoints.pass_through_endpoints import (
        ...
        with patch(
            "litelm.proxy.pass_through_endpoints.pass_through_endpoints._read_request_body",
            return_value=mock_request_body,
        ), patch(
            "litelm.proxy.pass_through_endpoints.pass_through_endpoints.get_async_httpx_client"
            ...
            assert sent_headers["x-litelm-header"] == "litelm-value"
    ...
    async def test_pass_through_request_with_forward_headers_false(self):
        ...
        custom_headers = {
            "x-litelm-header": "litelm-value",
            "authorization": "Bearer litelm-token",
        }
        ...
        with patch(
            ...
            assert sent_headers["x-litelm-header"] == "litelm-value"
            assert sent_headers["authorization"] == "Bearer litelm-token"
...
class TestMilvusProxyRoute:
    ...
    async def test_milvus_proxy_route_success(self):
        ...
        mock_index_object.litelm_params.vector_store_name = vector_store_name
        mock_index_object.litelm_params.vector_store_index = vector_store_index
        ...
        with patch(
            ...
            "litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints.ProviderConfigManager.get_provider_vector_stores_config"
        ) as mock_get_config, patch(
            "litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints.is_allowed_to_call_vector_store_endpoint"
        ) as mock_is_allowed, patch(
            "litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints._safe_set_request_parsed_body"
        ) as mock_safe_set, patch(
            "litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints.create_pass_through_route"
        ) as mock_create_route, patch.object(
            litelm, "vector_store_index_registry"
        ) as mock_index_registry, patch.object(
            litelm, "vector_store_registry"
            ...
            mock_vector_registry.get_litelm_managed_vector_store_from_registry_by_name.return_value = (
                mock_vector_store
            )
    ...
    async def test_milvus_proxy_route_no_index_registry(self):
        ...
        with patch(
            ...
            litelm, "vector_store_index_registry", None
        ):
            mock_get_config.return_value = MagicMock()
    ...
    async def test_milvus_proxy_route_not_managed_index(self):
        ...
        with patch(
            ...
            litelm, "vector_store_registry", MagicMock()
        ):
            mock_get_config.return_value = MagicMock()
            ...
            assert exc_info.value.status_code == 400
            assert (
                f"Collection {collection_name} is not a litelm managed vector store index"
                in str(exc_info.value.detail)
            )
    ...
    async def test_milvus_proxy_route_no_api_base(self):
        ...
        mock_vector_store = {"litelm_params": {}}  # No api_base
    ...
    async def test_milvus_proxy_route_endpoint_without_leading_slash(self):
        ...
        mock_vector_store = {"litelm_params": {"api_base": api_base}}
...
class TestCursorProxyRoute:
    ...
    async def test_cursor_proxy_route_raises_on_missing_api_key(self):
        ...
        with patch(
            ...
            "litelm.proxy.pass_through_endpoints.llm_passthrough_endpoints.litelm.credential_list",
            [],
        ):
            with pytest.raises(Exception) as exc_info:
    ...
    async def test_cursor_proxy_route_uses_ui_credential(self):
        """should use credentials added via UI (litelm.credential_list) when env var is not set"""
