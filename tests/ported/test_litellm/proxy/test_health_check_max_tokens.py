...
@pytest.mark.asyncio
async def test_update_litelm_params_max_tokens_default():
    """
...
@pytest.mark.asyncio
async def test_update_litelm_params_max_tokens_custom():
    """
...
@pytest.mark.asyncio
async def test_update_litelm_params_max_tokens_wildcard():
    """
...
async def test_ahealth_check_wildcard_models_respects_max_tokens():
    ...
    """
    with patch(
        "litelm.litelm_core_utils.llm_request_utils.pick_cheapest_chat_models_from_llm_provider",
        return_value=["gpt-4o-mini"],
    ), patch("litelm.acompletion", new_callable=AsyncMock):
