...
def test_google_generate_content_endpoint():
    ...
    try:
        ...
        from litelm.proxy.google_endpoints.endpoints import router as google_router
...
def test_google_generate_content_with_cost_tracking_metadata():
    ...
    with patch("litelm.proxy.proxy_server.llm_router") as mock_router, patch(
        "litelm.proxy.proxy_server.general_settings", {}
    ), patch("litelm.proxy.proxy_server.proxy_config") as mock_proxy_config, patch(
        "litelm.proxy.proxy_server.version", "1.0.0"
    ), patch(
        "litelm.proxy.litelm_pre_call_utils.add_litelm_data_to_request"
    ) as mock_add_data:
        mock_router.agenerate_content = AsyncMock(return_value={"test": "response"})

        # Mock add_litelm_data_to_request to return data with metadata
        async def mock_add_litelm_data(
            data, request, user_api_key_dict, proxy_config, general_settings, version
        ):
            # Simulate adding user metadata
            data["litelm_metadata"] = {
        ...
        mock_add_data.side_effect = mock_add_litelm_data
        ...
        # Verify that add_litelm_data_to_request was called
        ...
        # Verify that litelm_metadata exists and contains user information
        assert "litelm_metadata" in called_data
        assert called_data["litelm_metadata"]["user_api_key_user_id"] == "test-user-id"
        assert called_data["litelm_metadata"]["user_api_key_team_id"] == "test-team-id"
...
def test_google_generate_content_with_system_instruction():
    ...
    with patch("litelm.proxy.proxy_server.llm_router") as mock_router, patch(
        ...
        # Mock add_litelm_data_to_request to pass through data unchanged
        async def mock_add_litelm_data(
            data, request, user_api_key_dict, proxy_config, general_settings, version
        ):
            return data
...
def test_google_generate_content_metadata_and_trace_id_callbacks():
    """Test that google_generate_content sets litelm_call_id and logging_obj for callbacks (e.g. S3, Langfuse)"""
    try:
        from fastapi import FastAPI
        from fastapi.testclient import TestClient
    ...
    with patch("litelm.proxy.proxy_server.llm_router") as mock_router, patch(
        ...
        # Send a request to the endpoint with x-litelm-call-id header
        ...
        # Verify that the litelm_logging_obj got assigned in the final called_data to router
        assert "litelm_logging_obj" in called_data
        assert "litelm_call_id" in called_data
        assert called_data["litelm_call_id"] == test_call_id


def test_google_stream_generate_content_metadata_and_trace_id_callbacks():
    """Test that google_stream_generate_content sets litelm_call_id and logging_obj for callbacks"""
    try:
        from fastapi import FastAPI
        from fastapi.testclient import TestClient
