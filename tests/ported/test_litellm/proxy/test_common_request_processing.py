...
from litelm.integrations.opentelemetry import UserAPIKeyAuth
from litelm.proxy.common_request_processing import (
...
from litelm.proxy.dd_span_tagger import DDSpanTagger
from litelm.proxy.utils import ProxyLogging


class TestProxyBaseLLMRequestProcessing:
    @pytest.mark.asyncio
    async def test_common_processing_pre_call_logic_pre_call_hook_receives_litelm_call_id(
        self, monkeypatch
    ):
        ...
        async def mock_add_litelm_data_to_request(*args, **kwargs):
            return {}
        ...
        monkeypatch.setattr(
            litelm.proxy.common_request_processing,
            "add_litelm_data_to_request",
            mock_add_litelm_data_to_request,
        )
        ...
        assert "litelm_call_id" in data_passed
        try:
            uuid.UUID(data_passed["litelm_call_id"])
        except ValueError:
            pytest.fail("litelm_call_id is not a valid UUID")
        assert data_passed["litelm_call_id"] == returned_data["litelm_call_id"]

    def test_add_dd_apm_tags_for_litelm_call_id_uses_dd_tracing_helper(self, monkeypatch):
        mock_set_active_span_tag = MagicMock(return_value=True)
        import litelm.proxy.dd_span_tagger

        monkeypatch.setattr(
            litelm.proxy.dd_span_tagger,
            "set_active_span_tag",
    ...
    async def test_stream_timeout_header_processing(self):
        """
        Test that x-litelm-stream-timeout header gets processed and added to request data as stream_timeout.
        """
        from litelm.proxy.litelm_pre_call_utils import LiteLLMProxyRequestSetup

        # Test with stream timeout header
        headers_with_timeout = {"x-litelm-stream-timeout": "30.5"}
        ...
        headers_with_invalid = {"x-litelm-stream-timeout": "invalid"}
    ...
    @pytest.mark.asyncio
    async def test_add_litelm_data_to_request_with_stream_timeout_header(self):
        """
        Test that x-litelm-stream-timeout header gets processed and added to request data
        when calling add_litelm_data_to_request.
        """
        ...
        mock_request = MagicMock(spec=Request)
        mock_request.headers = {"x-litelm-stream-timeout": "45.0"}
        ...
        result_data = await add_litelm_data_to_request(
            data=test_data,
            request=mock_request,
    ...
    def test_get_custom_headers_with_discount_info(self):
        ...
        headers = ProxyBaseLLMRequestProcessing.get_custom_headers(
            ...
            litelm_logging_obj=logging_obj,
        )

        # Verify discount headers are present
        assert "x-litelm-response-cost" in headers
        assert float(headers["x-litelm-response-cost"]) == 0.000095

        assert "x-litelm-response-cost-original" in headers
        assert float(headers["x-litelm-response-cost-original"]) == 0.0001

        assert "x-litelm-response-cost-discount-amount" in headers
        assert float(headers["x-litelm-response-cost-discount-amount"]) == 0.000005
    ...
    def test_get_custom_headers_without_discount_info(self):
        ...
        assert "x-litelm-response-cost" in headers
        assert float(headers["x-litelm-response-cost"]) == 0.0001

        # Discount headers should not be in the final dict
        assert "x-litelm-response-cost-original" not in headers
        assert "x-litelm-response-cost-discount-amount" not in headers
    ...
    def test_get_custom_headers_with_margin_info(self):
        ...
        logging_obj = LiteLLMLoggingObj(
            ...
            call_type="completion",
            start_time=None,
            litelm_call_id="test-call-id-margin",
        ...
        assert "x-litelm-response-cost" in headers
        assert float(headers["x-litelm-response-cost"]) == 0.00011

        assert "x-litelm-response-cost-margin-amount" in headers
        assert float(headers["x-litelm-response-cost-margin-amount"]) == 0.00001

        assert "x-litelm-response-cost-margin-percent" in headers
        assert float(headers["x-litelm-response-cost-margin-percent"]) == 0.10
    ...
    def test_get_custom_headers_without_margin_info(self):
        ...
        logging_obj = LiteLLMLoggingObj(
            ...
            call_type="completion",
            start_time=None,
            litelm_call_id="test-call-id-no-margin",
            function_id="test-function",
        )
        logging_obj.set_cost_breakdown(
            input_cost=0.00005,
        ...
        assert "x-litelm-response-cost-margin-amount" not in headers
        assert "x-litelm-response-cost-margin-percent" not in headers
    ...
    def test_get_custom_headers_key_spend_includes_response_cost(self):
        """
        Test that x-litelm-key-spend header includes the current request's response_cost.
        ...
        assert "x-litelm-key-spend" in headers_1
        expected_spend_1 = 0.001 + 0.0005  # Initial spend + current request cost
        assert float(headers_1["x-litelm-key-spend"]) == pytest.approx(
            expected_spend_1, abs=1e-10
        )
        assert float(headers_1["x-litelm-response-cost"]) == response_cost_1
        ...
        assert "x-litelm-key-spend" in headers_2
        expected_spend_2 = 0.001 + 0.0003  # Initial spend + current request cost
        assert float(headers_2["x-litelm-key-spend"]) == pytest.approx(
        ...
        assert "x-litelm-key-spend" in headers_3
        assert (
            float(headers_3["x-litelm-key-spend"]) == 0.001
        ...
        assert "x-litelm-key-spend" in headers_4
        assert (
            float(headers_4["x-litelm-key-spend"]) == 0.001
        ...
        assert "x-litelm-key-spend" in headers_5
        assert float(headers_5["x-litelm-key-spend"]) == 0.0002  # 0.0 + 0.0002
        ...
        assert "x-litelm-key-spend" in headers_6
        assert (
            float(headers_6["x-litelm-key-spend"]) == 0.001
        )  # Should use original spend
        ...
        assert "x-litelm-key-spend" in headers_7
        assert (
            float(headers_7["x-litelm-key-spend"]) == 0.001
    ...
    async def test_queue_time_seconds_is_set_in_metadata(self, monkeypatch):
        ...
        after add_litelm_data_to_request populates arrival_time.

        This verifies the fix for the bug where queue_time_seconds was always None
        because arrival_time was read BEFORE add_litelm_data_to_request set it.
        """
        processing_obj = ProxyBaseLLMRequestProcessing(data={})
        mock_request = MagicMock(spec=Request)
        mock_request.headers = {}
        ...
        async def mock_add_litelm_data_to_request(*args, **kwargs):
            ...
            # Simulate what add_litelm_data_to_request does: set arrival_time
            import time
...
class TestCommonRequestProcessingHelpers:
    ...
    async def test_create_streaming_response_all_chunks_have_dd_trace(self):
        ...
        with patch("litelm.proxy.common_request_processing.tracer", mock_tracer):
            response = await create_response(mock_generator(), "text/event-stream", {})
...
class TestOverrideOpenAIResponseModel:
    ...
    def test_override_model_preserves_fallback_model_when_fallback_occurred_object(
        ...
        """
        Test that when a fallback occurred (x-litelm-attempted-fallbacks > 0),
        ...
        response_obj._hidden_params = {
            "additional_headers": {"x-litelm-attempted-fallbacks": 1}
        }
    ...
    def test_override_model_preserves_fallback_model_multiple_fallbacks(self):
        ...
        response_obj._hidden_params = {
            "additional_headers": {
                "x-litelm-attempted-fallbacks": 2  # Multiple fallbacks
            }
        }
    ...
    def test_override_model_overrides_when_attempted_fallbacks_is_zero(self):
        ...
        response_obj._hidden_params = {
            "additional_headers": {
                "x-litelm-attempted-fallbacks": 0  # Zero means no fallback occurred
            }
        }
    ...
    def test_override_model_overrides_when_attempted_fallbacks_is_none(self):
        ...
        response_obj.model = downstream_model
        response_obj._hidden_params = {
            "additional_headers": {"x-litelm-attempted-fallbacks": None}
        }
...
class TestStreamingOverheadHeader:
    """
    Tests that x-litelm-overhead-duration-ms is emitted in streaming responses.
    ...
    def test_get_custom_headers_includes_overhead_when_set(self):
        """
        get_custom_headers() returns x-litelm-overhead-duration-ms
        when litelm_overhead_time_ms is in hidden_params.
        """
        mock_user_api_key_dict = MagicMock(spec=UserAPIKeyAuth)
        mock_user_api_key_dict.tpm_limit = None
        mock_user_api_key_dict.rpm_limit = None
        ...
        assert "x-litelm-overhead-duration-ms" in headers
        assert headers["x-litelm-overhead-duration-ms"] == "42.5"

    def test_get_custom_headers_omits_overhead_when_none(self):
        """
        get_custom_headers() omits x-litelm-overhead-duration-ms
        when litelm_overhead_time_ms is not in hidden_params.
        """
        mock_user_api_key_dict = MagicMock(spec=UserAPIKeyAuth)
        mock_user_api_key_dict.tpm_limit = None
        mock_user_api_key_dict.rpm_limit = None
        ...
        assert "x-litelm-overhead-duration-ms" not in headers

    def test_update_response_metadata_sets_overhead_on_stream_wrapper(self):
        """
        update_response_metadata() sets litelm_overhead_time_ms on
        a streaming response's _hidden_params when llm_api_duration_ms is available.
        """
        from litelm.litelm_core_utils.llm_response_utils.response_metadata import (
        ...
        mock_logging_obj.litelm_call_id = "test-call-id"
        ...
        assert "litelm_overhead_time_ms" in stream_result._hidden_params
        overhead = stream_result._hidden_params["litelm_overhead_time_ms"]
    ...
    async def test_streaming_response_includes_overhead_header(self):
        ...
        x-litelm-overhead-duration-ms in its headers.
        """
        ...
        headers = {
            "x-litelm-overhead-duration-ms": "42.5",
            "x-litelm-call-id": "test-call-id",
            "x-litelm-model-id": "test-model-id",
        }
        ...
        assert response.headers.get("x-litelm-overhead-duration-ms") == "42.5"
    ...
    def test_streaming_overhead_header_in_custom_headers_from_stream_hidden_params(
        ...
        response's hidden_params (containing litelm_overhead_time_ms),
        the x-litelm-overhead-duration-ms header is correctly populated.
        ...
        hidden_params = {
            ...
            "api_base": "https://api.openai.com",
            "additional_headers": {},
            "litelm_overhead_time_ms": 55.3,  # set by update_response_metadata
            "_response_ms": 280.0,
            "litelm_call_id": "test-call-id",
            "response_cost": 0.002,
            "cache_key": None,
        ...
        assert "x-litelm-overhead-duration-ms" in custom_headers, (
            "x-litelm-overhead-duration-ms header must be emitted during streaming. "
            "It was missing — this is the streaming overhead header regression."
        )
        assert custom_headers["x-litelm-overhead-duration-ms"] == "55.3"
...
class TestDDSpanTaggerTagRequest:
    ...
    def test_tags_key_alias_and_model(self):
        ...
        with patch(
            "litelm.proxy.dd_span_tagger.set_active_span_tag"
        ) as mock_set_tag:
        ...
        mock_set_tag.assert_any_call("litelm.key_alias", "my-prod-key")
        mock_set_tag.assert_any_call("litelm.key_hash", "hashed123")
        mock_set_tag.assert_any_call("litelm.requested_model", "gpt-4o")
    ...
    def test_only_model_tagged_when_no_key_info(self):
        ...
        mock_set_tag.assert_called_once_with("litelm.requested_model", "claude-3-5-sonnet")
