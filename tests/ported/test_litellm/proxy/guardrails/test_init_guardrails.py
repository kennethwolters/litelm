...
from litelm.proxy.guardrails.guardrail_registry import InMemoryGuardrailHandler
...
def test_initialize_presidio_guardrail():
    ...
    assert (
        result["litelm_params"].guardrail
        == SupportedGuardrailIntegrations.PRESIDIO.value
    )
    assert result["litelm_params"].mode == "pre_call"
