...
from litelm.proxy.common_utils.callback_utils import get_logging_caching_headers
from litelm.proxy.guardrails.guardrail_hooks.pillar import (
    PillarGuardrail,
    PillarGuardrailAPIError,
    PillarGuardrailMissingSecrets,
)
from litelm.proxy.guardrails.guardrail_hooks.pillar.pillar import (
...
def setup_and_teardown():
    """
    Standard LiteLLM fixture that reloads litelm before every function
    ...
    # 1. litelm not in sys.modules (parallel worker removed it)
    # 2. litelm already imported (normal case)
    _module = importlib.import_module("litelm")
    litelm = importlib.reload(_module)
...
def test_pillar_guard_config_missing_api_key(pillar_guardrail_config, monkeypatch):
    ...
    pillar_guardrail_config["litelm_params"].pop("api_key", None)
...
@pytest.mark.asyncio
async def test_litelm_context_headers_automatically_added(
    sample_request_data,
...
@pytest.mark.asyncio
async def test_litelm_context_with_partial_fields(
    sample_request_data,
    dual_cache,
    pillar_clean_response,
):
...
def test_metadata_inclusion(pillar_guardrail_instance):
    ...
    assert payload["metadata"]["source"] == "litelm"
