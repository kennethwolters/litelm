...
from litelm.proxy.guardrails.guardrail_hooks.noma import NomaV2Guardrail
from litelm.proxy.guardrails.guardrail_hooks.noma.noma import NomaBlockedMessage
from litelm.types.proxy.guardrails.guardrail_hooks.noma import (
...
class TestNomaV2Configuration:
    ...
    def test_build_scan_payload_sends_raw_available_data(self, noma_v2_guardrail):
        ...
        request_data = {
            ...
            "litelm_metadata": {"user_api_key_alias": "litelm-alias"},
            "litelm_call_id": "call-id-1",
        }
    ...
    def test_build_scan_payload_passes_model_call_details_as_is(self, noma_v2_guardrail):
        class _LoggingObj:
            def __init__(self) -> None:
                self.model_call_details = {
                    ...
                    "litelm_call_id": "call-id-123",
                    "function_id": "fn-id-456",
                    "litelm_trace_id": "trace-id-789",
                    "api_key": "included-as-is",
                }

        request_data = {"litelm_logging_obj": "<Logging object>"}
        payload = noma_v2_guardrail._build_scan_payload(
            inputs={"texts": ["hello"]},
            request_data=request_data,
            input_type="request",
        ...
        assert payload["request_data"]["litelm_logging_obj"] == {
            "model": "gpt-4.1-mini",
            "messages": [{"role": "user", "content": "hello"}],
            "stream": False,
            "call_type": "acompletion",
        ...
        assert request_data["litelm_logging_obj"] == "<Logging object>"
    ...
    def test_sanitize_payload_for_transport_falls_back_to_safe_dumps(self, noma_v2_guardrail):
        with patch(
            "litelm.proxy.guardrails.guardrail_hooks.noma.noma_v2.json.dumps",
            side_effect=TypeError("cannot serialize"),
        ):
            with patch(
                "litelm.proxy.guardrails.guardrail_hooks.noma.noma_v2.safe_dumps",
    ...
    def test_sanitize_payload_for_transport_logs_warning_when_payload_becomes_empty(self, noma_v2_guardrail):
        with patch(
            "litelm.proxy.guardrails.guardrail_hooks.noma.noma_v2.safe_json_loads",
            return_value={},
        ):
            with patch(
                "litelm.proxy.guardrails.guardrail_hooks.noma.noma_v2.verbose_proxy_logger.warning"
            ) as mock_warning:
                sanitized = noma_v2_guardrail._sanitize_payload_for_transport({"inputs": {"texts": ["hello"]}})
...
class TestNomaV2ApplicationIdResolution:
    ...
    async def test_apply_guardrail_ignores_request_metadata_application_id(self, noma_v2_guardrail):
        ...
        request_data = {
            "metadata": {"headers": {"x-noma-application-id": "header-app"}},
            "litelm_metadata": {"user_api_key_alias": "alias-app"},
        }
        with patch.object(
            noma_v2_guardrail,
            "get_guardrail_dynamic_request_body_params",
