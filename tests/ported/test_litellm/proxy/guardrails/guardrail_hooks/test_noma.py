...
from litelm.proxy.guardrails.guardrail_hooks.noma import (
    NomaGuardrail,
    NomaV2Guardrail,
    initialize_guardrail,
)
import litelm.proxy.guardrails.guardrail_hooks.noma.noma as noma_legacy_module
from litelm.proxy.guardrails.guardrail_hooks.noma.noma import NomaBlockedMessage
from litelm.proxy.guardrails.init_guardrails import init_guardrails_v2
from litelm.types.llms.openai import AllMessageValues
...
class TestNomaGuardrailConfiguration:
    ...
    def test_initialize_guardrail_function(self):
        ...
        from litelm.types.guardrails import Guardrail, LitellmParams
        ...
        with patch("litelm.logging_callback_manager.add_litelm_callback") as mock_add:
            result = initialize_guardrail(litelm_params, guardrail)
...
class TestNomaApplicationIdResolution:
    ...
    @pytest.mark.asyncio
    async def test_application_id_defaults_to_litelm(
        self, noma_guardrail, mock_user_api_key_dict, mock_request_data
    ):
        request_data = self._clone_request_data(mock_request_data)
        user_auth = self._clone_user_auth(mock_user_api_key_dict)
...
class TestBackgroundProcessing:
    ...
    async def test_handle_verdict_background_blocked(self, monitor_mode_guardrail):
        ...
        with patch("litelm._logging.verbose_proxy_logger.warning") as mock_warning:
    ...
    async def test_handle_verdict_background_allowed(self, monitor_mode_guardrail):
        ...
        with patch("litelm._logging.verbose_proxy_logger.info") as mock_info:
            await monitor_mode_guardrail._handle_verdict_background(
...
class TestNomaImageProcessing:
    ...
    def test_extract_user_message_with_image_url(self):
        ...
        from litelm.completion_extras.litelm_responses_transformation.transformation import (
    ...
    async def test_image_content_blocked(
        ...
        from litelm.proxy.guardrails.guardrail_hooks.noma.noma import (
            NomaBlockedMessage,
        )
...
class TestIntegration:
    ...
    async def test_full_guardrail_flow(self):
        ...
        with patch.dict(
            ...
            custom_loggers = (
                litelm.logging_callback_manager.get_custom_loggers_for_type(
                    callback_type=litelm.integrations.custom_guardrail.CustomGuardrail
                )
            )
