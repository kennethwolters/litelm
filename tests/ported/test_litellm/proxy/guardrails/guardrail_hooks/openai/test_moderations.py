...
from litelm.types.llms.openai import OpenAIModerationResponse, OpenAIModerationResult
...
@pytest.mark.asyncio
async def test_openai_moderation_guardrail_adds_to_litelm_callbacks():
    """Test that OpenAI moderation guardrail adds itself to litelm callbacks during initialization"""
    import litelm
    from litelm.proxy.guardrails.guardrail_hooks.openai import (
    ...
    original_callbacks = litelm.callbacks.copy()
    litelm.logging_callback_manager._reset_all_callbacks()

    try:
        with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
            guardrail_litelm_params = LitellmParams(
            ...
            guardrail = openai_initialize_guardrail(
                litelm_params=guardrail_litelm_params,
                guardrail=Guardrail(
                    guardrail_name="test-openai-moderation",
                    litelm_params=guardrail_litelm_params,
                ),
            ...
            # Check that the guardrail was added to litelm callbacks
            assert guardrail in litelm.callbacks
            assert len(litelm.callbacks) == 1

            # Verify it's the correct guardrail
            callback = litelm.callbacks[0]
        ...
        for callback in original_callbacks:
            litelm.logging_callback_manager.add_litelm_callback(callback)
...
async def test_openai_moderation_guardrail_streaming_safe_content():
    ...
    from litelm.proxy.guardrails.guardrail_hooks.unified_guardrail.unified_guardrail import (
    ...
    with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
        ...
        with patch.object(guardrail, "async_make_request", return_value=mock_response), patch(
            "litelm.llms.openai.chat.guardrail_translation.handler.stream_chunk_builder",
...
async def test_openai_moderation_guardrail_streaming_harmful_content():
    ...
    with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
        ...
        # Mock for stream_chunk_builder - use real litelm types so isinstance checks pass
