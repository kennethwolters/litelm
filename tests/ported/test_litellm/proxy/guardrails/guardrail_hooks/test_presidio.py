...
from litelm.proxy.guardrails.guardrail_hooks.presidio import (
    _OPTIONAL_PresidioPIIMasking,
)
from litelm.exceptions import GuardrailRaisedException
from litelm.types.guardrails import LitellmParams, PiiAction, PiiEntityType
...
async def test_presidio_filter_scope_initializer(monkeypatch):
    ...
    class DummyGuardrail:
        ...
        def update_in_memory_litelm_params(self, litelm_params):
            pass
    ...
    class DummyManager:
        ...
        def add_litelm_callback(self, cb):
            self.added.append(cb)

    mgr = DummyManager()
    monkeypatch.setattr(litelm, "logging_callback_manager", mgr, raising=False)
    import litelm.proxy.guardrails.guardrail_hooks.presidio as presidio_mod
    import litelm.proxy.guardrails.guardrail_initializers as gi
    ...
    # input-only
    created.clear()
    from litelm.proxy.guardrails.guardrail_initializers import initialize_presidio
...
def test_update_in_memory_applies_score_thresholds():
    """
    update_in_memory_litelm_params should refresh score thresholds.
    """
    guardrail = _OPTIONAL_PresidioPIIMasking(mock_testing=True)
    ...
    guardrail.update_in_memory_litelm_params(params)
...
async def test_apply_to_output_streaming_bytes_only_logs_warning():
    ...
    collected = []
    with patch(
        "litelm.proxy.guardrails.guardrail_hooks.presidio.verbose_proxy_logger"
    ) as mock_logger:
        async for chunk in guardrail.async_post_call_streaming_iterator_hook(
            user_api_key_dict=mock_user_api_key,
            response=mock_stream(),
