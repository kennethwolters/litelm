...
from litelm.integrations.custom_guardrail import ModifyResponseException
from litelm.proxy.guardrails.guardrail_hooks.block_code_execution import (
    DEFAULT_EVENT_HOOKS,
    BlockCodeExecutionGuardrail,
    initialize_guardrail,
)
from litelm.proxy.guardrails.guardrail_hooks.block_code_execution.block_code_execution import (
...
```'''
    ...
    async def test_detection_includes_confidence_and_action_taken(self):
        ...
        meta = request_data.get("metadata") or request_data.get("litelm_metadata") or {}
    ...
    def test_initialize_guardrail_default_mode_is_both(self):
        ...
        litelm_params.guardrail = "block_code_execution"
        litelm_params.blocked_languages = ["python"]
        litelm_params.action = "block"
        litelm_params.confidence_threshold = 0.7
        litelm_params.default_on = False
        litelm_params.mode = None  # not set
        guardrail = {"guardrail_name": "block-code-test"}
        instance = initialize_guardrail(litelm_params, guardrail)
