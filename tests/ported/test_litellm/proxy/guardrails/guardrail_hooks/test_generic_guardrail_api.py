...
from litelm._version import version as litelm_version
from litelm.exceptions import GuardrailRaisedException, Timeout
from litelm.proxy._types import UserAPIKeyAuth
from litelm.proxy.guardrails.guardrail_hooks.generic_guardrail_api import (
    GenericGuardrailAPI,
)
from litelm.proxy.guardrails.guardrail_hooks.generic_guardrail_api.generic_guardrail_api import (
...
class TestGenericGuardrailAPIConfiguration:
    ...
    def test_init_with_config(self):
        ...
        assert (
            guardrail.api_base
            == "https://api.test.guardrail.com/beta/litelm_basic_guardrail_api"
        )
    ...
    def test_init_with_env_vars(self):
        ...
        with patch.dict(
            ...
            assert (
                guardrail.api_base
                == "https://env.api.guardrail.com/beta/litelm_basic_guardrail_api"
            )
    ...
    def test_api_base_appends_endpoint(self):
        ...
        assert (
            guardrail.api_base
            == "https://api.test.guardrail.com/v1/beta/litelm_basic_guardrail_api"
        )

    def test_api_base_not_duplicated(self):
        """Test that endpoint path is not duplicated if already present"""
        guardrail = GenericGuardrailAPI(
            api_base="https://api.test.guardrail.com/beta/litelm_basic_guardrail_api",
        )
        assert (
            guardrail.api_base
            == "https://api.test.guardrail.com/beta/litelm_basic_guardrail_api"
...
class TestMetadataExtraction:
    ...
    async def test_extract_metadata_from_output_response(
        ...
        """Test extracting metadata from output response (litelm_metadata field)"""
        ...
        for key, value in user_dict.items():
            if value is not None and not key.startswith("_"):
                if key.startswith("user_api_key_"):
                    litelm_metadata[key] = value
                else:
                    litelm_metadata[f"user_api_key_{key}"] = value

        request_data = {
            "response": mock_response,
            "litelm_metadata": litelm_metadata,
        }
        ...
        with patch.object(
            ...
            # Verify metadata was extracted from request_data["litelm_metadata"]
    ...
    @pytest.mark.asyncio
    async def test_inbound_headers_and_litelm_version_forwarded_and_sanitized(
        self, generic_guardrail, mock_request_data_input
    ):
        """
        ...
        with patch.object(
            ...
            assert json_payload["litelm_version"] == litelm_version
...
class TestErrorHandling:
    ...
    async def test_timeout_fail_open_allows_flow(self, mock_request_data_input):
        """Test litelm.Timeout allows flow when unreachable_fallback=fail_open"""
        guardrail = GenericGuardrailAPI(
            api_base="https://api.test.guardrail.com",
            headers={"Authorization": "Bearer test-key"},
            unreachable_fallback="fail_open",
        ...
        with patch.object(
            ...
            side_effect=Timeout(
                ...
                llm_provider="litelm-httpx-handler",
            ),
        ):
            result = await guardrail.apply_guardrail(
                inputs={"texts": ["test"]},
