...
from litelm.proxy.guardrails.guardrail_hooks.model_armor import ModelArmorGuardrail
...
async def test_model_armor_post_call_hook_sanitization():
    ...
    with patch.object(guardrail.async_handler, "post", AsyncMock(return_value=mock_response)):
        ...
        mock_llm_response = litelm.ModelResponse()
        ...
        async def mock_stream():
            chunks = [
                litelm.ModelResponseStream(
                    choices=[
                        litelm.types.utils.StreamingChoices(
                            delta=litelm.types.utils.Delta(content="Sensitive ")
                        )
                    ]
                ),
                litelm.ModelResponseStream(
                    choices=[
                        litelm.types.utils.StreamingChoices(
                            delta=litelm.types.utils.Delta(content="information")
                        )
                    ]
                ),
            ]
...
def test_model_armor_ui_friendly_name():
    ...
    from litelm.types.proxy.guardrails.guardrail_hooks.model_armor import (
...
async def test_model_armor_missing_sanitized_text():
    ...
    with patch.object(guardrail.async_handler, "post", AsyncMock(return_value=mock_response)):
        ...
        mock_llm_response.choices = [
            litelm.Choices(
                message=litelm.Message(content="Original content")
            )
        ]
...
async def test_async_moderation_hook_with_sanitization():
    ...
    with patch.object(guardrail.async_handler, "post", AsyncMock(return_value=mock_response)):
        ...
        from litelm.litelm_core_utils.prompt_templates.common_utils import get_last_user_message
