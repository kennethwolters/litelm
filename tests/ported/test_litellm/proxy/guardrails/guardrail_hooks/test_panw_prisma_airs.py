...
from litelm.proxy.guardrails.guardrail_hooks.panw_prisma_airs import (
    PanwPrismaAirsHandler,
    initialize_guardrail,
)
from litelm.types.guardrails import GuardrailEventHooks, LitellmParams
...
def mock_panw_client():
    ...
    with patch(
        "litelm.proxy.guardrails.guardrail_hooks.panw_prisma_airs.panw_prisma_airs.get_async_httpx_client"
    ) as mock_client:
...
class TestPanwAirsInitialization:
    ...
    def test_initialize_guardrail_function(self):
        ...
        with patch("litelm.logging_callback_manager.add_litelm_callback"):
            handler = initialize_guardrail(litelm_params, guardrail_config)
...
class TestPanwAirsPromptScanning:
    ...
    async def test_empty_prompt_handling(self, base_handler, user_api_key_dict):
        ...
        empty_data = {
            ...
            "messages": [],
            "user": "test_user",
            "litelm_call_id": "test-call-id-empty",
        }
...
class TestPanwAirsMaskingFunctionality:
    ...
    async def test_response_masking_on_block(self):
        ...
        user_api_key_dict = UserAPIKeyAuth()
        data = {"model": "gpt-3.5-turbo", "litelm_call_id": "test-call-id"}
        response = ModelResponse(
            id="test_id",
            choices=[
                Choices(
...
class TestPanwAirsAdvancedFeatures:
    ...
    async def test_streaming_hook_adds_guardrail_header(self):
        ...
        with patch.object(
            ...
            with patch(
                "litelm.proxy.guardrails.guardrail_hooks.panw_prisma_airs.panw_prisma_airs.add_guardrail_to_applied_guardrails_header"
            ) as mock_header:
                mock_api.return_value = mock_scan_result
...
class TestPanwAirsSessionTracking:
    """Test session tracking with litelm_trace_id."""

    @pytest.mark.asyncio
    async def test_tr_id_always_call_id_with_trace_in_metadata(self, mock_panw_client):
        """Test that tr_id is always call_id even when metadata has litelm_trace_id."""
        handler = make_handler()
        ...
        assert payload["metadata"]["litelm_trace_id"] == trace_id

    @pytest.mark.asyncio
    async def test_fallback_to_call_id_when_trace_id_missing(self, mock_panw_client):
        """Test fallback to call_id when litelm_trace_id is missing."""
        handler = make_handler()
    ...
    async def test_trace_id_extraction_from_request_data(self):
        """Test that litelm_trace_id is extracted from request data."""
        handler = make_handler()
        ...
        assert "litelm_trace_id" in metadata
        assert metadata["litelm_trace_id"] == trace_id

    def test_trace_id_extraction_from_nested_metadata(self):
        """Test litelm_trace_id extraction from data['metadata'] (proxy path).

        The proxy stores user-supplied litelm_trace_id inside
        data["metadata"]["litelm_trace_id"], NOT at data["litelm_trace_id"].
        ...
        data = {
            ...
            "metadata": {
                "litelm_trace_id": trace_id,
                "requester_metadata": {"litelm_trace_id": trace_id},
            },
        }
    ...
    def test_trace_id_extraction_from_requester_metadata(self):
        """Test litelm_trace_id extraction from requester_metadata fallback.

        For /v1/messages routes, user metadata is deep-copied into
        requester_metadata. If litelm_trace_id is only there, we must find it.
        """
        handler = make_handler()
    ...
    def test_trace_id_extraction_from_header_key(self):
        """Test litelm_trace_id extraction from x-litelm-trace-id header.

        litelm_pre_call_utils stores the x-litelm-trace-id header value
        as metadata["trace_id"] (not "litelm_trace_id"). We must find it.
        """
        handler = make_handler()
        ...
        data = {
            "model": "gpt-3.5-turbo",
            "metadata": {
                "trace_id": trace_id,  # as stored by litelm_pre_call_utils
            },
        }
...
class TestPanwAirsAppUserMetadata:
    ...
    async def test_app_user_priority_chain(self):
        """Test that app_user follows priority: app_user > user > litelm_user."""
        handler = make_handler()
        ...
        test_cases = [
            ...
            ({}, "litelm_user", "litelm_user is default"),
        ]
...
class TestPanwAirsDeduplicationMissingCallId:
    """Test _check_and_mark_scanned fallback behavior when litelm_call_id is missing."""

    def test_check_and_mark_scanned_synthesizes_call_id_when_missing(self):
        """Test that _check_and_mark_scanned synthesizes litelm_call_id when missing."""
        handler = make_handler()
        ...
        assert data["litelm_call_id"]
        assert (
            data["litelm_metadata"][f"_panw_pre_scanned_{data['litelm_call_id']}"]
            is True
        )
...
class TestPanwAirsApplyGuardrail:
    ...
    async def test_apply_guardrail_allow(self, handler):
        ...
        request_data = {"litelm_call_id": "test-call-id", "model": "gpt-4"}
    ...
    async def test_apply_guardrail_missing_call_id_synthesizes_fallback(self, handler):
        """Missing litelm_call_id is synthesized (not a hard fail)."""
        inputs: GenericGuardrailAPIInputs = {"texts": ["Test content"]}
        request_data = {"model": "gpt-4"}  # No litelm_call_id
        ...
        with patch.object(
            ...
            assert "litelm_call_id" in request_data
            assert len(request_data["litelm_call_id"]) == 36  # UUID4 format
            assert mock_api.call_count == 1
    ...
    async def test_apply_guardrail_synthesizes_call_id_for_direct_endpoint(
        ...
        with patch.object(
            ...
            assert mock_api.call_count == 1
            assert (
                mock_api.call_args.kwargs["call_id"] == request_data["litelm_call_id"]
            )

    @pytest.mark.asyncio
    async def test_apply_guardrail_call_id_from_logging_obj(self, handler):
        """Test litelm_call_id resolved from logging_obj when missing from request_data."""
        inputs: GenericGuardrailAPIInputs = {"texts": ["Hello world"]}
        request_data = {"model": "gpt-4"}  # No litelm_call_id

        logging_obj = MagicMock()
        logging_obj.litelm_call_id = "logging-call-id"
    ...
    async def test_apply_guardrail_response_side_missing_call_id(self, handler):
        """Response-side with no litelm_call_id synthesizes a UUID fallback."""
        response = ModelResponse(
            id="chatcmpl-test",
        ...
        request_data: dict = {"response": response}  # No litelm_call_id
        ...
        with patch.object(
            ...
            assert "litelm_call_id" in request_data
            assert len(request_data["litelm_call_id"]) == 36
...
class TestPanwAirsStreamingBytesScan:
    ...
    async def test_streaming_bytes_scan(self, action):
        ...
        request_data = {
            ...
            "litelm_call_id": "test-bytes-call-id",
        }
    ...
    async def test_bytes_streaming_success_adds_observability_header(self):
        ...
        request_data = {
            "messages": [{"role": "user", "content": "test"}],
            "model": "claude-3-5-sonnet",
            "litelm_call_id": "test-obs-bytes-id",
        }
...
class TestPanwAirsStreamingPydanticEventsScan:
    ...
    async def test_streaming_pydantic_events_scan(self, action):
        ...
        request_data = {
            "messages": [{"role": "user", "content": "test"}],
            "model": "gpt-4",
            "litelm_call_id": "test-pydantic-call-id",
        }
    ...
    async def test_pydantic_streaming_success_adds_observability_header(self):
        ...
        request_data = {
            "messages": [{"role": "user", "content": "test"}],
            "model": "gpt-4",
            "litelm_call_id": "test-obs-pydantic-id",
        }
...
class TestPanwAirsApplyGuardrailMetadataEnrichment:
    ...
    async def test_apply_guardrail_metadata_enrichment(self):
        ...
        request_data = {"response": mock_response, "litelm_call_id": "test-enrich-id"}

        # logging_obj carries the original metadata
        logging_obj = MagicMock()
        logging_obj.litelm_call_id = "test-enrich-id"
        logging_obj.model = "gpt-4"
...
class TestPanwAirsToolCallToolEvent:
    ...
    async def test_tool_event_includes_metadata_and_input(self, handler):
        ...
        with patch.object(
            ...
            await handler._scan_tool_calls_for_guardrail(
                ...
                metadata={"user": "test", "model": "gpt-4"},
                call_id="test-call-id",
                request_data={"litelm_call_id": "test-call-id"},
                start_time=datetime.now(),
            )
    ...
    async def test_tool_event_empty_args_omits_input(self, handler):
        ...
        with patch.object(
            ...
            assert_canonical_tool_event(
                te, ecosystem="openai", server_name="litelm", tool_invoked="list_items"
            )
    ...
    async def test_dict_tool_call_extracts_name(self, handler):
        ...
        with patch.object(
            ...
            te = call_kwargs["tool_event"]
            assert_canonical_tool_event(
                te, ecosystem="openai", server_name="litelm", tool_invoked="search"
            )
...
class TestPanwAirsMcpToolEventScan:
    ...
    async def test_mcp_tool_event_server_id_resolution(self, handler):
        ...
        with patch(
            "litelm.proxy._experimental.mcp_server.mcp_server_manager.global_mcp_server_manager"
...
class TestPanwAirsChatStreamingPostCall:
    ...
    async def test_model_response_stream(self, action):
        ...
        request_data = {
            "messages": [{"role": "user", "content": "test"}],
            "model": "gpt-4",
            "litelm_call_id": "test-stream-chat",
        }
...
class TestPanwAirsTrIdOverride:
    """Test tr_id override from explicit litelm_trace_id in metadata."""
    ...
    async def test_tr_id_header_only_no_override(self, mock_panw_client):
        ...
        # Simulate header-derived trace_id (stored as "trace_id" by litelm_pre_call_utils)
        data = {
            "model": "gpt-3.5-turbo",
            "metadata": {
        ...
        assert metadata["litelm_trace_id"] == header_trace
        ...
        payload = mock_panw_client.client.post.call_args.kwargs["json"]
        assert payload["tr_id"] == call_id
        assert payload["metadata"]["litelm_trace_id"] == header_trace

    @pytest.mark.asyncio
    async def test_tr_id_uses_call_id_with_requester_metadata_trace(
        self, mock_panw_client
    ):
        """requester_metadata.litelm_trace_id is correlation-only, tr_id is always call_id."""
        handler = PanwPrismaAirsHandler(
            guardrail_name="test_panw_airs",
            api_key="test_api_key",
            profile_name="test_profile",
        ...
        assert "_panw_tr_id_override" not in metadata
        # litelm_trace_id still extracted for correlation
        assert metadata["litelm_trace_id"] == trace_id
    ...
    @pytest.mark.asyncio
    async def test_top_level_litelm_trace_id_is_correlation_only(
        self, mock_panw_client
    ):
        """Top-level data['litelm_trace_id'] is correlation-only, NOT a tr_id override."""
        handler = PanwPrismaAirsHandler(
            guardrail_name="test_panw_airs",
            api_key="test_api_key",
            profile_name="test_profile",
        ...
        # Only top-level litelm_trace_id, NO metadata.litelm_trace_id
        data = {
            "model": "gpt-3.5-turbo",
            "litelm_trace_id": top_level_trace,
            "metadata": {},
        }
        ...
        assert metadata["litelm_trace_id"] == top_level_trace
        ...
        payload = mock_panw_client.client.post.call_args.kwargs["json"]
        assert payload["tr_id"] == call_id
        # litelm_trace_id still forwarded for correlation
        assert payload["metadata"]["litelm_trace_id"] == top_level_trace
...
class TestPanwAirsDictChunkStreaming:
    ...
    async def test_streaming_hook_dict_chunks_scanned(self):
        ...
        request_data = {
            "messages": [{"role": "user", "content": "test"}],
            "model": "gpt-4",
            "litelm_call_id": "test-dict-chunk-call-id",
        }
...
class TestPanwAirsRawStreamingMaskingWarning:
    ...
    async def test_raw_streaming_block_with_masking_logs_warning(self):
        ...
        request_data = {
            "messages": [{"role": "user", "content": "test"}],
            "model": "gpt-4",
            "litelm_call_id": "test-raw-mask-call-id",
        }
        ...
        with patch.object(
            ...
            with patch(
                "litelm.proxy.guardrails.guardrail_hooks.panw_prisma_airs.panw_prisma_airs.verbose_proxy_logger"
            ) as mock_logger:
                with pytest.raises(HTTPException) as exc_info:
...
class TestPanwAirsLatestRoleMessageOnly:
    ...
    async def test_flag_unset_anthropic_defaults_latest_only(
        ...
        dumped = litelm_params.model_dump(exclude_unset=True)
        handler = PanwPrismaAirsHandler(
            **{
                **dumped,
                "guardrail_name": "test_panw_airs",
                "event_hook": litelm_params.mode,
                "default_on": False,
            }
        )
...
class TestPanwAirsMcpToolCallWithoutCallId:
    ...
    litelm_call_id — the bug fix for _convert_mcp_to_llm_format synthetic data."""
    ...
    async def test_mcp_tool_call_request_without_call_id(self, handler):
        """MCP tool call with no litelm_call_id should NOT raise 500.

        This is the core regression test: _convert_mcp_to_llm_format produces
        synthetic request_data without litelm_call_id, and logging_obj is None.
        ...
        with patch.object(
            ...
            # Assert 4: litelm_call_id backfilled into request_data
            assert request_data.get("litelm_call_id") == call_kwargs["call_id"]
    ...
    async def test_mcp_tool_call_with_logging_obj_call_id_uses_parent_id(self, handler):
        """When logging_obj has litelm_call_id, the handler should use it as tr_id
        ...
        request_data = {
            ...
            # NO litelm_call_id in request_data
        }
        mock_logging_obj = MagicMock()
        mock_logging_obj.litelm_call_id = "parent-call-id-123"
        ...
        with patch.object(
            ...
            # call_id should be the parent's litelm_call_id
            call_kwargs = mock_api.call_args.kwargs
    ...
    async def test_direct_apply_guardrail_empty_request_data_synthesizes_plain_uuid(
        ...
        with patch.object(
            ...
            synth_id = request_data.get("litelm_call_id")
    ...
    async def test_non_mcp_request_without_call_id_synthesizes_uuid(self, handler):
        ...
        request_data = {
            ...
            "litelm_call_id": None,  # explicitly missing
        }
        ...
        with patch.object(
            ...
            # UUID was synthesized and injected
            assert request_data["litelm_call_id"] is not None
            assert len(request_data["litelm_call_id"]) == 36
    ...
    async def test_mcp_rest_name_fallback_synthesizes_tr_id(self, handler):
        ...
        request_data = {
            ...
            "name": "web_search_exa",
            "arguments": {"path": "/tmp"},
            # NO mcp_tool_name, NO litelm_call_id
        }
...
class TestPanwAirsStreamingFallbackFix:
    ...
    async def test_streaming_transient_returns_tuple_without_raising(self, handler):
        ...
        request_data = _simple_data(litelm_call_id="test-call-id")
