...
from litelm.proxy.guardrails.guardrail_hooks.onyx.onyx import OnyxGuardrail
from litelm.proxy.guardrails.init_guardrails import init_guardrails_v2
from litelm.types.utils import Choices, GenericGuardrailAPIInputs, Message
...
def test_onyx_guard_with_custom_timeout_from_kwargs():
    ...
    with patch(
        "litelm.proxy.guardrails.guardrail_hooks.onyx.onyx.get_async_httpx_client"
    ) as mock_get_client:
...
class TestOnyxGuardrail:
    ...
    async def test_apply_guardrail_request_no_violations(self):
        ...
        assert (
            call_args.args[0]
            == f"{guardrail.api_base}/guard/evaluate/v1/{guardrail.api_key}/litelm"
        )
        assert (
...
class TestOnyxIntegration:
    ...
    async def test_full_guardrail_flow(self):
        ...
        custom_loggers = litelm.logging_callback_manager.get_custom_loggers_for_type(
