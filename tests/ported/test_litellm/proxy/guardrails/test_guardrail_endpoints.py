...
from litelm.proxy.guardrails.guardrail_endpoints import (
...
from litelm.proxy.guardrails.guardrail_registry import (
    IN_MEMORY_GUARDRAIL_HANDLER,
    InMemoryGuardrailHandler,
)
from litelm.types.guardrails import (
...
MOCK_GUARDRAIL = Guardrail(
    ...
    litelm_params=LitellmParams(**MOCK_CONFIG_GUARDRAIL["litelm_params"]),
...
MOCK_PATCH_REQUEST = PatchGuardrailRequest(
    ...
    litelm_params={"guardrail": "updated.guardrail", "mode": "post_call"},
...
def mock_prisma_client(mocker):
    ...
    mock_client.db.litelm_guardrailstable = mocker.Mock()
    mock_client.db.litelm_guardrailstable.find_many = AsyncMock(
        return_value=[MOCK_DB_GUARDRAIL]
    )
    mock_client.db.litelm_guardrailstable.find_unique = AsyncMock(
        return_value=MOCK_DB_GUARDRAIL
    )
    return mock_client
...
async def test_list_guardrails_v2_with_db_and_config(
    ...
    mocker.patch(
        "litelm.proxy.guardrails.guardrail_registry.IN_MEMORY_GUARDRAIL_HANDLER",
    ...
    assert isinstance(db_guardrail.litelm_params, BaseLitellmParams)
    ...
    assert isinstance(config_guardrail.litelm_params, BaseLitellmParams)


@pytest.mark.asyncio
async def test_list_guardrails_v2_masks_sensitive_data_in_db_guardrails(mocker):
    """Test that sensitive litelm_params are masked for DB guardrails in list response"""
    ...
    mock_prisma_client.db.litelm_guardrailstable = mocker.Mock()
    mock_prisma_client.db.litelm_guardrailstable.find_many = AsyncMock(
    ...
    litelm_params = guardrail.litelm_params
    if isinstance(litelm_params, dict):
        params = litelm_params
    else:
        params = litelm_params.model_dump() if hasattr(litelm_params, "model_dump") else dict(litelm_params)
...
async def test_list_guardrails_v2_masks_sensitive_data_in_config_guardrails(mocker):
    """Test that sensitive litelm_params are masked for in-memory/config guardrails in list response"""
...
async def test_get_guardrail_info_from_db(mocker, mock_prisma_client):
    ...
    assert isinstance(response.litelm_params, BaseLitellmParams)
...
async def test_get_guardrail_info_from_config(
    ...
    mock_prisma_client.db.litelm_guardrailstable.find_unique = AsyncMock(
        return_value=None
    )
...
def test_get_provider_specific_params():
    ...
    from litelm.proxy.guardrails.guardrail_endpoints import _get_fields_from_model
    from litelm.proxy.guardrails.guardrail_hooks.azure import (
...
def test_optional_params_not_returned_when_not_overridden():
    ...
    from litelm.proxy.guardrails.guardrail_endpoints import _get_fields_from_model
    from litelm.types.proxy.guardrails.guardrail_hooks.base import GuardrailConfigModel
...
async def test_bedrock_guardrail_prepare_request_with_api_key():
    ...
    from litelm.proxy.guardrails.guardrail_hooks.bedrock_guardrails import (
        BedrockGuardrail,
    )
...
async def test_bedrock_guardrail_prepare_request_without_api_key():
    ...
    with patch("litelm.proxy.guardrails.guardrail_hooks.bedrock_guardrails.get_secret_str") as mock_get_secret, \
...
async def test_create_guardrail_endpoint(
    ...
    if scenario == "success_with_sync":
        mock_prisma_client = mocker.Mock()
        mocker.patch("litelm.proxy.proxy_server.prisma_client", mock_prisma_client)
        mocker.patch("litelm.proxy.guardrails.guardrail_endpoints.GUARDRAIL_REGISTRY", mock_guardrail_registry)
        mocker.patch("litelm.proxy.guardrails.guardrail_registry.IN_MEMORY_GUARDRAIL_HANDLER", mock_in_memory_handler)
        
    elif scenario == "success_sync_fails":
        mock_prisma_client = mocker.Mock()
        mock_in_memory_handler.initialize_guardrail.side_effect = Exception("Sync failed")
        mock_logger = mocker.patch("litelm.proxy.guardrails.guardrail_endpoints.verbose_proxy_logger")
        ...
        mocker.patch("litelm.proxy.proxy_server.prisma_client", None)
...
async def test_apply_guardrail_not_found(mocker):
    ...
    mocker.patch("litelm.proxy.guardrails.guardrail_endpoints.GUARDRAIL_REGISTRY", mock_registry)
...
async def test_get_guardrail_info_endpoint_config_guardrail(mocker):
    ...
    """
    from litelm.proxy.guardrails.guardrail_endpoints import get_guardrail_info

    # Mock prisma_client to not be None (patch at the source where it's imported from)
    mock_prisma = mocker.Mock()
    mocker.patch("litelm.proxy.proxy_server.prisma_client", mock_prisma)
    ...
    mocker.patch(
        "litelm.litelm_core_utils.litelm_logging._get_masked_values",
...
class TestBuildFieldDict:
    ...
    def test_build_field_dict_with_string_ui_type(self):
        ...
        from litelm.proxy.guardrails.guardrail_endpoints import _build_field_dict
    ...
    def test_build_field_dict_with_enum_ui_type(self):
        ...
        from litelm.proxy.guardrails.guardrail_endpoints import _build_field_dict
        from litelm.types.guardrails import GuardrailParamUITypes
...
async def test_register_guardrail_success(mocker):
    ...
    mock_prisma = mocker.Mock()
    mock_prisma.db.litelm_guardrailstable.find_unique = AsyncMock(return_value=None)
    ...
    mock_prisma.db.litelm_guardrailstable.create = AsyncMock(return_value=created_row)
    mocker.patch("litelm.proxy.proxy_server.prisma_client", mock_prisma)
    ...
    mock_prisma.db.litelm_guardrailstable.create.assert_called_once()
    call_data = mock_prisma.db.litelm_guardrailstable.create.call_args[1]["data"]
...
async def test_register_guardrail_rejects_non_generic_api(mocker):
    """Register returns 400 when litelm_params.guardrail is not generic_guardrail_api."""
    mocker.patch("litelm.proxy.proxy_server.prisma_client", mocker.Mock())
    req = RegisterGuardrailRequest(
        guardrail_name="other-guard",
        litelm_params={"guardrail": "bedrock", "mode": "pre_call", "api_base": "https://x.com"},
    )
...
async def test_register_guardrail_duplicate_name(mocker):
    ...
    mock_prisma = mocker.Mock()
    mock_prisma.db.litelm_guardrailstable.find_unique = AsyncMock(
...
async def test_list_guardrail_submissions_success(mocker):
    ...
    row = mocker.Mock(
        ...
        status="pending_review",
        team_id="t1",
        litelm_params={"guardrail": "generic_guardrail_api", "api_base": "https://x.com"},
        guardrail_info={
            "description": "A guard",
    ...
    mock_prisma.db.litelm_guardrailstable.find_many = AsyncMock(return_value=[row])
    mocker.patch("litelm.proxy.proxy_server.prisma_client", mock_prisma)
...
async def test_list_guardrail_submissions_returns_only_team_guardrails(mocker):
    ...
    mock_prisma.db.litelm_guardrailstable.find_many = find_many
    mocker.patch("litelm.proxy.proxy_server.prisma_client", mock_prisma)
    user = UserAPIKeyAuth(user_role=LitellmUserRoles.PROXY_ADMIN)
...
async def test_approve_guardrail_submission_success(mocker):
    ...
    row = mocker.Mock(
        ...
        status="pending_review",
        litelm_params={"guardrail": "generic_guardrail_api", "mode": "pre_call", "api_base": "https://g.com"},
        guardrail_info={},
    )
    mock_prisma.db.litelm_guardrailstable.find_unique = AsyncMock(return_value=row)
    mock_prisma.db.litelm_guardrailstable.update = AsyncMock()
    mocker.patch("litelm.proxy.proxy_server.prisma_client", mock_prisma)
    ...
    mock_prisma.db.litelm_guardrailstable.update.assert_called_once()
    call_data = mock_prisma.db.litelm_guardrailstable.update.call_args[1]["data"]
...
async def test_list_submissions_summary_counts_unaffected_by_filters(mocker):
    ...
    pending_row = mocker.Mock(
        ...
        team_id="t1", litelm_params={}, guardrail_info={},
    ...
    mock_prisma.db.litelm_guardrailstable.find_many = AsyncMock(return_value=all_rows)
    mocker.patch("litelm.proxy.proxy_server.prisma_client", mock_prisma)
    user = UserAPIKeyAuth(user_role=LitellmUserRoles.PROXY_ADMIN)
