...
import litelm
from litelm.proxy._types import SpecialHeaders
from litelm.proxy.middleware.prometheus_auth_middleware import PrometheusAuthMiddleware
...
def test_valid_auth_metrics(app_with_middleware, monkeypatch):
    ...
    litelm.require_auth_for_metrics_endpoint = True
    # Patch the auth function to simulate a valid authentication.
    monkeypatch.setattr(
        "litelm.proxy.middleware.prometheus_auth_middleware.user_api_key_auth",
        fake_valid_auth,
    )
...
def test_no_auth_metrics_when_disabled(app_with_middleware, monkeypatch):
    ...
    """
    litelm.require_auth_for_metrics_endpoint = False
