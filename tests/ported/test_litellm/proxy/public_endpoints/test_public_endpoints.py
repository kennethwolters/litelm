...
from litelm.proxy.public_endpoints import router
...
def test_get_provider_create_fields():
    ...
    assert "litelm_provider" in first_provider
...
def test_get_litelm_model_cost_map_returns_cost_map():
    app = FastAPI()
    app.include_router(router)
    client = TestClient(app)

    response = client.get("/public/litelm_model_cost_map")
...
def test_public_model_hub_with_healthy_model():
    ...
    with patch("litelm.public_model_groups", ["gpt-3.5-turbo"]), \
         patch("litelm.proxy.proxy_server._get_model_group_info") as mock_get_info, \
         patch("litelm.proxy.proxy_server.llm_router", mock_llm_router), \
         patch("litelm.proxy.proxy_server.prisma_client", mock_prisma), \
         patch("litelm.proxy.health_endpoints._health_endpoints._convert_health_check_to_dict") as mock_convert:
...
def test_public_model_hub_with_unhealthy_model():
    ...
    with patch("litelm.public_model_groups", ["gpt-4"]), \
         patch("litelm.proxy.proxy_server._get_model_group_info") as mock_get_info, \
         patch("litelm.proxy.proxy_server.llm_router", mock_llm_router), \
         patch("litelm.proxy.proxy_server.prisma_client", mock_prisma), \
         patch("litelm.proxy.health_endpoints._health_endpoints._convert_health_check_to_dict") as mock_convert:
...
def test_public_model_hub_without_health_check():
    ...
    with patch("litelm.public_model_groups", ["claude-3"]), \
         patch("litelm.proxy.proxy_server._get_model_group_info") as mock_get_info, \
         patch("litelm.proxy.proxy_server.llm_router", mock_llm_router), \
         patch("litelm.proxy.proxy_server.prisma_client", mock_prisma):
...
def test_public_model_hub_mixed_health_statuses():
    ...
    with patch("litelm.public_model_groups", ["gpt-3.5-turbo", "gpt-4", "claude-3"]), \
         patch("litelm.proxy.proxy_server._get_model_group_info") as mock_get_info, \
         patch("litelm.proxy.proxy_server.llm_router", mock_llm_router), \
         patch("litelm.proxy.proxy_server.prisma_client", mock_prisma), \
         patch("litelm.proxy.health_endpoints._health_endpoints._convert_health_check_to_dict") as mock_convert:
...
import litelm.proxy.public_endpoints.public_endpoints as _pe_module
from litelm.proxy.public_endpoints.public_endpoints import _build_endpoints, _clean_display_name
...
def test_get_supported_endpoints_is_cached(reset_endpoints_cache):
    ...
    client = _make_client()
    with patch(
        "litelm.proxy.public_endpoints.public_endpoints._load_endpoints",
