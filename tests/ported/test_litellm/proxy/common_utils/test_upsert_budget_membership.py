# tests/litelm/proxy/common_utils/test_upsert_budget_membership.py
import types
from unittest.mock import AsyncMock, MagicMock

import pytest

from litelm.proxy.management_endpoints.common_utils import (
...
def mock_tx():
    ...
    tx = MagicMock()
    tx.litelm_teammembership = membership
    tx.litelm_budgettable = budget
    return tx
...
async def test_upsert_disconnect(mock_tx, fake_user):
    ...
    mock_tx.litelm_teammembership.update.assert_awaited_once_with(
        where={"user_id_team_id": {"user_id": "user-1", "team_id": "team-1"}},
        data={"litelm_budget_table": {"disconnect": True}},
    )
    mock_tx.litelm_budgettable.update.assert_not_called()
    mock_tx.litelm_budgettable.create.assert_not_called()
    mock_tx.litelm_teammembership.upsert.assert_not_called()
...
async def test_upsert_with_existing_budget_id_creates_new(mock_tx, fake_user):
    ...
    mock_tx.litelm_budgettable.create.assert_awaited_once_with(
        data={
            "max_budget": 42.0,
    ...
    new_budget_id = mock_tx.litelm_budgettable.create.return_value.budget_id
    mock_tx.litelm_teammembership.upsert.assert_awaited_once_with(
        where={"user_id_team_id": {"user_id": "user-2", "team_id": "team-2"}},
        data={
            "create": {
                "user_id": "user-2",
                "team_id": "team-2",
                "litelm_budget_table": {"connect": {"budget_id": new_budget_id}},
            },
            "update": {
                "litelm_budget_table": {"connect": {"budget_id": new_budget_id}},
            },
    ...
    mock_tx.litelm_budgettable.update.assert_not_called()
    mock_tx.litelm_teammembership.update.assert_not_called()
...
async def test_upsert_create_and_link(mock_tx, fake_user):
    ...
    bid = mock_tx.litelm_budgettable.create.return_value.budget_id
    ...
    mock_tx.litelm_teammembership.upsert.assert_awaited_once_with(
        ...
        data={
            "create": {
                "user_id": "user-3",
                "team_id": "team-3",
                "litelm_budget_table": {"connect": {"budget_id": bid}},
            },
            "update": {
                "litelm_budget_table": {"connect": {"budget_id": bid}},
            },
...
async def test_upsert_create_then_create_another(mock_tx, fake_user):
    ...
    created_bid = mock_tx.litelm_budgettable.create.return_value.budget_id

    # sanity: we really did the create + upsert path
    mock_tx.litelm_budgettable.create.assert_awaited_once()
    mock_tx.litelm_teammembership.upsert.assert_awaited_once()

    # SECOND CALL – reset call history and create another budget
    mock_tx.litelm_budgettable.create.reset_mock()
    mock_tx.litelm_teammembership.upsert.reset_mock()
    mock_tx.litelm_budgettable.update.reset_mock()

    # Set up a new budget ID for the second create call
    mock_tx.litelm_budgettable.create.return_value = types.SimpleNamespace(budget_id="new-budget-456")
