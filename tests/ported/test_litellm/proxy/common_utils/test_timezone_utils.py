...
import litelm
from litelm.proxy.common_utils.timezone_utils import (
...
def test_get_budget_reset_timezone_reads_litelm_attr():
    """
    Test that get_budget_reset_timezone reads from litelm.timezone attribute.
    """
    original = getattr(litelm, "timezone", None)
    try:
        litelm.timezone = "Asia/Tokyo"
        assert get_budget_reset_timezone() == "Asia/Tokyo"
    finally:
        if original is None:
            if hasattr(litelm, "timezone"):
                delattr(litelm, "timezone")
        else:
            litelm.timezone = original


def test_get_budget_reset_timezone_fallback_utc():
    """
    Test that get_budget_reset_timezone falls back to UTC when litelm.timezone is not set.
    """
    original = getattr(litelm, "timezone", None)
    try:
        if hasattr(litelm, "timezone"):
...
def test_get_budget_reset_timezone_fallback_on_none():
    """
    Test that get_budget_reset_timezone falls back to UTC when litelm.timezone is None.
    """
    original = getattr(litelm, "timezone", None)
    try:
        litelm.timezone = None
        assert get_budget_reset_timezone() == "UTC"
    finally:
        if original is None:
            if hasattr(litelm, "timezone"):
