...
from litelm.proxy.common_utils.key_rotation_manager import KeyRotationManager
...
class TestKeyRotationManager:
    ...
    async def test_find_keys_needing_rotation(self):
        ...
        mock_prisma_client.db.litelm_verificationtoken.find_many.return_value = (
            mock_keys
        )
        ...
        with patch(
            "litelm.proxy.common_utils.key_rotation_manager.datetime"
        ) as mock_datetime:
    ...
    async def test_rotate_key_updates_database(self):
        ...
        with patch(
            "litelm.proxy.common_utils.key_rotation_manager.regenerate_key_fn",
            return_value=mock_response,
        ):
            with patch(
                "litelm.proxy.common_utils.key_rotation_manager.KeyManagementEventHooks.async_key_rotated_hook"
            ):
                # Execute
        ...
        call_args = mock_prisma_client.db.litelm_verificationtoken.update.call_args
    ...
    async def test_cleanup_expired_deprecated_keys(self):
        ...
        """
        mock_prisma_client = AsyncMock()
        mock_prisma_client.db.litelm_deprecatedverificationtoken.delete_many.return_value = (
            3
        )
        manager = KeyRotationManager(mock_prisma_client)
        ...
        mock_prisma_client.db.litelm_deprecatedverificationtoken.delete_many.assert_called_once()
        call_args = (
            mock_prisma_client.db.litelm_deprecatedverificationtoken.delete_many.call_args
        )
    ...
    async def test_rotate_key_passes_grace_period(self):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "litelm.proxy.common_utils.key_rotation_manager.LITELLM_KEY_ROTATION_GRACE_PERIOD",
                    "48h",
                ):
                    await manager._rotate_key(key_to_rotate)
