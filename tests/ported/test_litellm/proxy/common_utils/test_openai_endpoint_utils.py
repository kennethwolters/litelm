...
from litelm.proxy.common_utils.openai_endpoint_utils import remove_sensitive_info_from_deployment


@pytest.mark.parametrize(
    "model_config, expected_config",
    [
        # Test case 1: Empty litelm_params
        (
                {
...
def test_remove_sensitive_info_from_deployment_with_excluded_keys():
    ...
    model_config = {
        ...
        "litelm_params": {
            "model": "openai/gpt-4",
            "api_key": "sk-sensitive-key-123",
            "litelm_credentials_name": "my-credential-name",
    ...
    sanitized_config = remove_sensitive_info_from_deployment(model_config)
    assert sanitized_config["litelm_params"]["access_token"] != "token-12345"
    assert "*" in sanitized_config["litelm_params"]["access_token"]
    
    # With excluded_keys, litelm_credentials_name should NOT be masked (even if it would match patterns)
    sanitized_config = remove_sensitive_info_from_deployment(
        model_config, excluded_keys={"litelm_credentials_name"}
    )
    assert sanitized_config["litelm_params"]["litelm_credentials_name"] == "my-credential-name"
    ...
    assert "api_key" not in sanitized_config["litelm_params"]
