...
from litelm.proxy.common_utils.callback_utils import (
    get_remaining_tokens_and_requests_from_request_data,
    normalize_callback_names,
)

from unittest.mock import patch
from litelm.proxy.common_utils.callback_utils import process_callback
...
def test_get_remaining_tokens_and_requests_from_request_data():
    ...
    casedata = {
        "metadata": {
            ...
            f"litelm-key-remaining-requests-{model_group}": 100,
            f"litelm-key-remaining-tokens-{model_group}": 200,
        }
    }
    ...
    assert headers == {
        f"x-litelm-key-remaining-requests-{expected_name}": 100,
        f"x-litelm-key-remaining-tokens-{expected_name}": 200,
    }


@patch(
    "litelm.proxy.common_utils.callback_utils.CustomLogger.get_callback_env_vars",
