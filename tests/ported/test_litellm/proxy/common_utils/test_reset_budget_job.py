...
class MockLiteLLMTeamMembership:
    async def update_many(
        ...
        # Mock the update_many method for litelm_teammembership
        return {"count": 1}
...
class MockDB:
    def __init__(self):
        self.litelm_teammembership = MockLiteLLMTeamMembership()
        self.litelm_verificationtoken = MockLiteLLMVerificationToken()
    ...
    async def get_data(self, table_name, query_type, **kwargs):
        ...
        if (
            ...
            return [
                item
                for item in data
                if hasattr(item, "litelm_budget_table")
                and hasattr(item.litelm_budget_table, "budget_id")
                and item.litelm_budget_table.budget_id in budget_id_list
            ]
...
def test_reset_budget_for_key(reset_budget_job, mock_prisma_client):
    ...
    # Run the test
    asyncio.run(reset_budget_job.reset_budget_for_litelm_keys())
...
def test_reset_budget_for_user(reset_budget_job, mock_prisma_client):
    ...
    # Run the test
    asyncio.run(reset_budget_job.reset_budget_for_litelm_users())
...
def test_reset_budget_for_team(reset_budget_job, mock_prisma_client):
    ...
    # Run the test
    asyncio.run(reset_budget_job.reset_budget_for_litelm_teams())
...
def test_reset_budget_for_enduser(reset_budget_job, mock_prisma_client):
    ...
    test_enduser = type(
        ...
        {
            "spend": 20.0,
            "litelm_budget_table": test_budget,
    ...
    # Run the test
    asyncio.run(reset_budget_job.reset_budget_for_litelm_budget_table())
...
def test_reset_budget_for_keys_linked_to_budgets(reset_budget_job, mock_prisma_client):
    ...
    """
    from litelm.proxy._types import LiteLLM_BudgetTableFull
    ...
    # Verify that update_many was called on litelm_verificationtoken
    calls = mock_prisma_client.db.litelm_verificationtoken.update_many_calls
...
def test_reset_budget_for_keys_linked_to_budgets_excludes_keys_with_own_budget_duration(
    ...
    schedule and are handled only by reset_budget_for_litelm_keys(). The
    ...
    # Those keys are reset only by reset_budget_for_litelm_keys() - no double-reset.
    assert call["where"]["budget_duration"] is None
    assert call["where"]["budget_id"] == {"in": ["7d-budget-tier"]}
...
def test_budget_table_reset_also_resets_linked_keys(
    ...
    """
    Integration-style test: when reset_budget_for_litelm_budget_table runs,
    ...
    calls = mock_prisma_client.db.litelm_verificationtoken.update_many_calls
    assert len(calls) == 1, (
        "Expected reset_budget_for_litelm_budget_table to also reset keys "
