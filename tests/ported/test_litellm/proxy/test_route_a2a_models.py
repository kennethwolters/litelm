...
Maps to: litelm/proxy/agent_endpoints/a2a_routing.py
"""
import os
import sys
...
from litelm.proxy.agent_endpoints.a2a_routing import route_a2a_agent_request
from litelm.proxy.route_llm_request import route_request


@pytest.mark.asyncio
async def test_route_a2a_model_bypasses_router():
    """Test that a2a/ prefixed models bypass router and go directly to litelm with api_base"""
    ...
    # Mock litelm.acompletion to verify it's called
    mock_acompletion = AsyncMock(return_value={"id": "test-response"})

    with patch("litelm.acompletion", mock_acompletion):
        with patch(
            "litelm.proxy.agent_endpoints.agent_registry.global_agent_registry",
            mock_registry,
        ):
            ...
            # Verify litelm.acompletion was called with api_base injected
...
async def test_route_non_a2a_model_raises_error_if_not_in_router():
    ...
    from litelm.proxy.route_llm_request import ProxyModelNotFoundError
