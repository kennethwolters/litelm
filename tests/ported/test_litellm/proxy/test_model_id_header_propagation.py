"""
Test that x-litelm-model-id header is propagated correctly on error responses.
...
def test_maybe_get_model_id_from_litelm_params():
    """
    Test extraction of model_id from logging_obj.litelm_params (used by /v1/chat/completions).
    """
    # Create a ProxyBaseLLMRequestProcessing instance
    processor = ProxyBaseLLMRequestProcessing(data={})

    # Create a mock logging object with model_info in litelm_params
    mock_logging_obj = MagicMock()
    mock_logging_obj.litelm_params = {
        "model_info": {
            "id": "test-model-id-from-litelm-params"
        }
    }
    ...
    assert model_id == "test-model-id-from-litelm-params"


def test_maybe_get_model_id_from_litelm_params_nested():
    """
    Test extraction of model_id from nested metadata in logging_obj.litelm_params.
    """
    processor = ProxyBaseLLMRequestProcessing(data={})
...
def test_maybe_get_model_id_from_kwargs():
    ...
    mock_logging_obj = MagicMock()
    mock_logging_obj.litelm_params = None
...
def test_maybe_get_model_id_priority_litelm_params_over_data():
    """
    Test that model_id from logging_obj.litelm_params takes priority over self.data.
    """
    ...
    mock_logging_obj.litelm_params = {
        "model_info": {
            "id": "model-id-from-litelm-params"
        }
    }

    # Test extraction - should prefer litelm_params
    model_id = processor.maybe_get_model_id(mock_logging_obj)

    assert model_id == "model-id-from-litelm-params"


def test_get_custom_headers_includes_model_id():
    """
    Test that get_custom_headers includes x-litelm-model-id when model_id is provided.
    """
    ...
    assert "x-litelm-model-id" in headers
    assert headers["x-litelm-model-id"] == "test-model-123"
...
def test_get_custom_headers_without_model_id():
    ...
    # x-litelm-model-id should not be in headers (or should be empty/None)
    if "x-litelm-model-id" in headers:
        assert headers["x-litelm-model-id"] in [None, ""]
...
def test_get_custom_headers_with_empty_string_model_id():
    ...
    # x-litelm-model-id should not be in headers (or should be empty)
    if "x-litelm-model-id" in headers:
        assert headers["x-litelm-model-id"] == ""
