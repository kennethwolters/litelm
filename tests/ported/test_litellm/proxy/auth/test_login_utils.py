...
from litelm.proxy.auth.login_utils import (
    LoginResult,
    authenticate_user,
    get_ui_credentials,
)
...
async def test_authenticate_user_admin_login_with_ui_credentials():
    ...
    with patch.dict(
        ...
        with patch(
            "litelm.proxy.auth.login_utils.generate_key_helper_fn",
            ...
            with patch(
                "litelm.proxy.auth.login_utils.user_update",
                new_callable=AsyncMock,
                return_value=None,
            ) as mock_user_update:
                with patch(
                    "litelm.proxy.auth.login_utils.get_secret_bool",
                    return_value=False,
                ):
...
async def test_authenticate_user_email_case_insensitive_login():
    ...
    calls = mock_prisma_client.db.litelm_usertable.find_first.await_args_list
    assert len(calls) == 2
