...
from litelm.proxy._types import LiteLLM_JWTAuth, UserAPIKeyAuth
from litelm.proxy.auth.handle_jwt import JWTHandler
from litelm.proxy.auth.route_checks import RouteChecks
from litelm.proxy.auth.user_api_key_auth import get_api_key, user_api_key_auth
...
@pytest.mark.parametrize(
    "custom_litelm_key_header, api_key, passed_in_key",
    [
...
def test_get_api_key_with_custom_litelm_key_header(
    custom_litelm_key_header, api_key, passed_in_key
):
    assert get_api_key(
        custom_litelm_key_header=custom_litelm_key_header,
        api_key=None,
        azure_api_key_header=None,
        anthropic_api_key_header=None,
        google_ai_studio_api_key_header=None,
...
def test_team_metadata_with_tags_flows_through_jwt_auth():
    ...
    team_metadata, causing team-level tags to be missing in litelm_pre_call_utils.py
    """
    from litelm.proxy._types import LiteLLM_TeamTable, UserAPIKeyAuth
...
async def test_proxy_admin_expired_key_from_cache():
    ...
    from litelm.proxy.auth.user_api_key_auth import _user_api_key_auth_builder
    ...
    with patch(
        "litelm.proxy.auth.user_api_key_auth.get_key_object",
        new_callable=AsyncMock,
    ) as mock_get_key_object, \
         patch("litelm.proxy.auth.user_api_key_auth._delete_cache_key_object", new_callable=AsyncMock) as mock_delete_cache:
        ...
        import litelm.proxy.proxy_server as _proxy_server_mod
...
async def test_return_user_api_key_auth_obj_user_spend_and_budget():
    ...
    with patch(
        "litelm.proxy.auth.user_api_key_auth.user_api_key_service_logger_obj",
...
def test_proxy_admin_jwt_auth_includes_identity_fields():
    ...
    """
    from litelm.proxy._types import LiteLLM_TeamTable, LitellmUserRoles, UserAPIKeyAuth
...
class TestJWTOAuth2Coexistence:
    ...
    async def test_both_enabled_opaque_token_uses_oauth2(self):
        ...
        with patch("litelm.proxy.proxy_server.general_settings", general_settings), \
             patch("litelm.proxy.proxy_server.premium_user", True), \
             patch("litelm.proxy.proxy_server.master_key", "sk-master"), \
             patch("litelm.proxy.proxy_server.prisma_client", None), \
             patch("litelm.proxy.auth.user_api_key_auth.Oauth2Handler.check_oauth2_token", new_callable=AsyncMock, return_value=mock_oauth2_response) as mock_oauth2, \
             patch("litelm.proxy.auth.user_api_key_auth.JWTAuthManager.auth_builder", new_callable=AsyncMock) as mock_jwt_auth:
    ...
    async def test_both_enabled_jwt_token_skips_oauth2(self):
        ...
        with patch("litelm.proxy.proxy_server.general_settings", general_settings), \
             ...
             patch("litelm.proxy.proxy_server.master_key", "sk-master"), \
             patch("litelm.proxy.proxy_server.prisma_client", None), \
             patch("litelm.proxy.auth.user_api_key_auth.Oauth2Handler.check_oauth2_token", new_callable=AsyncMock) as mock_oauth2, \
             patch("litelm.proxy.auth.user_api_key_auth.JWTAuthManager.auth_builder", new_callable=AsyncMock, return_value=mock_jwt_result) as mock_jwt_auth:
