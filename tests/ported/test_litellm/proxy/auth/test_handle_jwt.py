...
from litelm.proxy.auth.handle_jwt import JWTAuthManager, JWTHandler
...
async def test_map_user_to_teams_handles_already_in_team_exception():
    ...
    with patch(
        ...
        with patch("litelm.proxy.auth.handle_jwt.verbose_proxy_logger") as mock_logger:
...
async def test_auth_builder_proxy_admin_user_role():
    ...
    jwt_handler.litelm_jwtauth = LiteLLM_JWTAuth()
...
async def test_sync_user_role_and_teams():
    ...
    jwt_handler.update_environment(
        ...
        litelm_jwtauth=LiteLLM_JWTAuth(
            jwt_litelm_role_map=[
                JWTLiteLLMRoleMap(jwt_role="ADMIN", litelm_role=LitellmUserRoles.PROXY_ADMIN)
            ],
    ...
    prisma = AsyncMock()
    prisma.db.litelm_usertable.update = AsyncMock()
    ...
    prisma.db.litelm_usertable.update.assert_called_once()
...
@pytest.mark.asyncio
async def test_map_jwt_role_to_litelm_role():
    ...
    jwt_handler.update_environment(
        ...
        litelm_jwtauth=LiteLLM_JWTAuth(
            jwt_litelm_role_map=[
                ...
                JWTLiteLLMRoleMap(jwt_role="ADMIN", litelm_role=LitellmUserRoles.PROXY_ADMIN),
                # Wildcard patterns
                JWTLiteLLMRoleMap(jwt_role="user_*", litelm_role=LitellmUserRoles.INTERNAL_USER),
                JWTLiteLLMRoleMap(jwt_role="team_?", litelm_role=LitellmUserRoles.TEAM),
                JWTLiteLLMRoleMap(jwt_role="dev_[123]", litelm_role=LitellmUserRoles.INTERNAL_USER),
            ],
            roles_jwt_field="roles"
        ),
    )
    ...
    # Test exact match
    token = {"roles": ["ADMIN"]}
    result = jwt_handler.map_jwt_role_to_litelm_role(token)
    ...
    jwt_handler.litelm_jwtauth.jwt_litelm_role_map = None
    token = {"roles": ["ADMIN"]}
    result = jwt_handler.map_jwt_role_to_litelm_role(token)
    assert result is None

    # Test empty role mappings
    jwt_handler.litelm_jwtauth.jwt_litelm_role_map = []
    token = {"roles": ["ADMIN"]}
    result = jwt_handler.map_jwt_role_to_litelm_role(token)
    assert result is None
...
async def test_nested_jwt_field_access():
    ...
    jwt_handler.litelm_jwtauth = LiteLLM_JWTAuth(user_email_jwt_field="email")
    assert jwt_handler.get_user_email(flat_token, None) == "user@example.com"

    # Test 3: team_ids_jwt_field with nested access
    jwt_handler.litelm_jwtauth = LiteLLM_JWTAuth(team_ids_jwt_field="groups")
    ...
    jwt_handler.litelm_jwtauth = LiteLLM_JWTAuth(end_user_id_jwt_field="customer.end_user_id")
    ...
    jwt_handler.litelm_jwtauth = LiteLLM_JWTAuth(end_user_id_jwt_field="end_user_id")
    assert jwt_handler.get_end_user_id(flat_token, None) == "customer123"

    # Test 7: team_id_jwt_field with nested access
    jwt_handler.litelm_jwtauth = LiteLLM_JWTAuth(team_id_jwt_field="tenant.team_id")
    ...
    jwt_handler.litelm_jwtauth = LiteLLM_JWTAuth(team_id_jwt_field="team_id")
    ...
    jwt_handler.litelm_jwtauth = LiteLLM_JWTAuth(roles_jwt_field="resource_access.my-client.roles")
...
async def test_metadata_prefix_handling_in_nested_fields():
    ...
    jwt_handler.litelm_jwtauth = LiteLLM_JWTAuth(user_email_jwt_field="metadata.user.email")
...
async def test_find_team_with_model_access_model_group(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "litelm.proxy.proxy_server", proxy_server_module)
    ...
    monkeypatch.setattr(
        "litelm.proxy.auth.handle_jwt.get_team_object", mock_get_team_object
    )
...
async def test_auth_builder_returns_team_membership_object():
    ...
    # Create mock objects
    from litelm.proxy._types import LiteLLM_BudgetTable, LiteLLM_TeamMembership
...
def test_get_team_id_from_header():
    ...
    result = JWTAuthManager.get_team_id_from_header(
        request_headers={"x-litelm-team-id": "team-1"},
    ...
    with pytest.raises(HTTPException) as exc_info:
        JWTAuthManager.get_team_id_from_header(
            request_headers={"x-litelm-team-id": "invalid-team"},
            allowed_team_ids={"team-1", "team-2"},
        )
...
async def test_auth_builder_uses_team_from_header_e2e():
    """Test auth_builder e2e flow: selects team from x-litelm-team-id header."""
    from litelm.caching import DualCache
    from litelm.proxy.utils import ProxyLogging
    ...
    with patch.object(jwt_handler, "auth_jwt", new_callable=AsyncMock) as mock_auth_jwt, \
         ...
         patch("litelm.proxy.auth.handle_jwt.get_team_object", new_callable=AsyncMock) as mock_get_team, \
        ...
        result = await JWTAuthManager.auth_builder(
            ...
            parent_otel_span=None,
            proxy_logging_obj=ProxyLogging(user_api_key_cache=user_api_key_cache),
            request_headers={"x-litelm-team-id": "team-2"},
        )
...
async def test_find_and_validate_specific_team_id_with_team_alias():
    ...
    from litelm.caching import DualCache
    from litelm.proxy._types import LiteLLM_JWTAuth, LiteLLM_TeamTable
    from litelm.proxy.auth.handle_jwt import JWTAuthManager, JWTHandler
    from litelm.proxy.utils import ProxyLogging
    ...
    with patch(
        "litelm.proxy.auth.handle_jwt.get_team_object_by_alias",
        new_callable=AsyncMock
...
async def test_find_and_validate_team_id_takes_precedence_over_name():
    ...
    with patch(
        "litelm.proxy.auth.handle_jwt.get_team_object",
        new_callable=AsyncMock
...
async def test_get_org_alias_with_nested_fields():
    ...
    # Test nested access
    jwt_handler.litelm_jwtauth = LiteLLM_JWTAuth(org_alias_jwt_field="company.organization.name")
    assert jwt_handler.get_org_alias(nested_token, None) == "acme-corp"
    
    # Test flat access
    jwt_handler.litelm_jwtauth = LiteLLM_JWTAuth(org_alias_jwt_field="org_name")
    assert jwt_handler.get_org_alias(nested_token, None) == "flat-org"
    
    # Test missing field returns default
    jwt_handler.litelm_jwtauth = LiteLLM_JWTAuth(org_alias_jwt_field="nonexistent.field")
...
async def test_get_objects_resolves_org_by_name():
    ...
    """
    from litelm.caching import DualCache
    from litelm.proxy._types import LiteLLM_JWTAuth, LiteLLM_OrganizationTable
    from litelm.proxy.auth.handle_jwt import JWTAuthManager, JWTHandler
    from litelm.proxy.utils import ProxyLogging
    ...
    with patch(
        "litelm.proxy.auth.handle_jwt.get_org_object_by_alias",
        new_callable=AsyncMock
    ) as mock_get_by_alias:
...
def _make_jwt_handler(team_id_jwt_field: str) -> JWTHandler:
    ...
    handler.update_environment(
        prisma_client=None,
        user_api_key_cache=DualCache(),
        litelm_jwtauth=LiteLLM_JWTAuth(team_id_jwt_field=team_id_jwt_field),
    )
    return handler
