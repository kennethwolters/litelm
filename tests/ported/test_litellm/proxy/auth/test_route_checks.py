...
from litelm.proxy._types import LiteLLM_UserTable, LitellmUserRoles, UserAPIKeyAuth
...
def test_virtual_key_allowed_routes_with_litelm_routes_member_name_allowed():
...
def test_virtual_key_allowed_routes_with_litelm_routes_member_name_denied():
...
def test_virtual_key_llm_api_route_includes_passthrough_prefix(route):
    ...
    Relevant issue: https://github.com/BerriAI/litelm/issues/14017
    """
...
def test_virtual_key_allowed_routes_with_multiple_litelm_routes_member_names():
...
def test_virtual_key_llm_api_routes_allows_registered_pass_through_endpoints():
    ...
    with (
        patch(
            "litelm.proxy.pass_through_endpoints.pass_through_endpoints._registered_pass_through_routes",
            mock_registered_routes,
        ),
        patch(
            "litelm.proxy.pass_through_endpoints.pass_through_endpoints.get_server_root_path",
            return_value="/",
        ),
    ):
...
def test_videos_route_accessible_to_internal_users():
    ...
    https://github.com/BerriAI/litelm/issues/16470
...
class TestModelsRouteExemptFromDisableLLMEndpoints:
    ...
    Relevant issue: https://github.com/BerriAI/litelm/issues/new (UI breaks with DISABLE_LLM_ENDPOINTS)
    """
    ...
    def _get_enterprise_route_checks(self):
        ...
        local_file = os.path.join(
            ...
            "..", "..", "..", "..", "enterprise",
            "litelm_enterprise", "proxy", "auth", "route_checks.py",
        )
...
def test_route_in_additional_public_routes_wildcard_match():
    ...
    """
    from litelm.proxy.auth.auth_utils import route_in_additonal_public_routes

    with (
        patch(
            "litelm.proxy.proxy_server.general_settings", {"public_routes": ["/api/*"]}
        ),
        patch("litelm.proxy.proxy_server.premium_user", True),
    ):
...
from litelm.proxy.auth.auth_checks_organization import _user_is_org_admin
