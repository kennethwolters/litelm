...
import litelm
from litelm.proxy._types import InvitationClaim
...
def _make_invite(*, is_accepted: bool, expired: bool = False) -> MagicMock:
    now = litelm.utils.get_utc_datetime()
    invite = MagicMock()
...
def _make_prisma(invite: MagicMock, user: MagicMock | None = None) -> MagicMock:
    prisma = MagicMock()
    prisma.db.litelm_invitationlink.find_unique = AsyncMock(return_value=invite)
    prisma.db.litelm_invitationlink.update = AsyncMock()
    prisma.db.litelm_usertable.find_unique = AsyncMock(return_value=user)
    prisma.db.litelm_usertable.update = AsyncMock(return_value=user)
    return prisma
...
async def test_get_token_rejects_already_used_link():
    ...
    """
    from litelm.proxy.proxy_server import onboarding

    invite = _make_invite(is_accepted=True)
    prisma = _make_prisma(invite)
    request = MagicMock()

    with patch("litelm.proxy.proxy_server.prisma_client", prisma), \
         patch("litelm.proxy.proxy_server.master_key", "sk-test"):
    ...
    prisma.db.litelm_usertable.find_unique.assert_not_called()
...
async def test_get_token_does_not_set_is_accepted():
    ...
    with patch("litelm.proxy.proxy_server.prisma_client", prisma), \
         ...
         patch("litelm.proxy.proxy_server.premium_user", False), \
         patch(
             "litelm.proxy.proxy_server.generate_key_helper_fn",
             new_callable=AsyncMock,
             return_value=mock_token_response,
         ), \
         patch("litelm.proxy.proxy_server.get_custom_url", return_value="http://localhost:4000/"), \
         patch("litelm.proxy.proxy_server.get_disabled_non_admin_personal_key_creation", return_value=False), \
         patch("litelm.proxy.proxy_server.get_server_root_path", return_value=""):
    ...
    prisma.db.litelm_invitationlink.update.assert_not_called()
...
async def test_claim_token_rejects_already_used_link():
    ...
    """
    from litelm.proxy.proxy_server import claim_onboarding_link
    ...
    prisma.db.litelm_usertable.update.assert_not_called()
...
async def test_claim_token_sets_is_accepted_after_password_written():
    ...
    call_kwargs = prisma.db.litelm_usertable.update.call_args
    ...
    prisma.db.litelm_invitationlink.update.assert_called_once()
    link_update_data = prisma.db.litelm_invitationlink.update.call_args.kwargs["data"]
