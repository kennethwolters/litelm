...
from litelm.proxy.common_utils.encrypt_decrypt_utils import decrypt_value_helper
from litelm.utils import get_utc_datetime
...
def reset_constants_module():
    ...
    from litelm import constants
    from litelm.proxy.auth import auth_checks
...
def test_get_experimental_ui_login_jwt_auth_token_valid(valid_sso_user_defined_values):
    ...
    assert token_data["max_budget"] == litelm.max_ui_session_budget
...
def test_experimental_ui_token_ignores_litelm_ui_session_duration(
    valid_sso_user_defined_values,
):
...
def test_get_key_object_from_ui_hash_key_valid(
    ...
    assert key_object.max_budget == litelm.max_ui_session_budget
...
async def test_default_internal_user_params_with_get_user_object(monkeypatch):
    ...
    monkeypatch.setattr(litelm, "default_internal_user_params", default_params)
    ...
    mock_prisma_client.db.litelm_usertable.create = AsyncMock(return_value=mock_user)
    ...
    mock_prisma_client.db.litelm_usertable.create.assert_called_once()
    creation_args = mock_prisma_client.db.litelm_usertable.create.call_args[1]["data"]
...
def test_log_budget_lookup_failure_dry_run():
    ...
    with patch("litelm.proxy.auth.auth_checks.verbose_proxy_logger") as mock_logger:
...
@pytest.mark.asyncio
@patch("litelm.proxy.management_endpoints.team_endpoints.new_team", new_callable=AsyncMock)
async def test_get_team_db_check_calls_new_team_on_upsert(mock_new_team, monkeypatch):
    ...
    mock_db = AsyncMock()
    mock_prisma_client.db = mock_db
    mock_prisma_client.db.litelm_teamtable.find_unique.return_value = None
...
async def test_get_team_db_check_does_not_call_new_team_if_exists(mock_new_team, monkeypatch):
    ...
    mock_db = AsyncMock()
    mock_prisma_client.db = mock_db
    mock_prisma_client.db.litelm_teamtable.find_unique.return_value = MagicMock()
...
async def test_vector_store_access_check_early_returns(
    ...
    with patch("litelm.proxy.proxy_server.prisma_client", prisma_client), patch(
        "litelm.vector_store_registry", vector_store_registry
    ):
...
async def test_vector_store_access_check_with_permissions():
    ...
    with patch("litelm.proxy.proxy_server.prisma_client", mock_prisma_client), patch(
        "litelm.vector_store_registry", mock_vector_store_registry
    ):
        result = await vector_store_access_check(
            request_body=request_body,
            team_object=None,
...
def test_can_object_call_model_with_alias():
    ...
    from litelm.proxy.auth.auth_checks import _can_object_call_model
...
async def test_get_tag_objects_batch():
    ...
    """
    from litelm.proxy._types import LiteLLM_TagTable
    from litelm.proxy.auth.auth_checks import get_tag_objects_batch
    ...
    uncached_tag_1.litelm_budget_table = None
    ...
    uncached_tag_2.litelm_budget_table = None
    ...
    uncached_tag_3.litelm_budget_table = None
    ...
    mock_prisma.db.litelm_tagtable.find_many = AsyncMock(
    ...
    mock_prisma.db.litelm_tagtable.find_many.assert_called_once()
    call_args = mock_prisma.db.litelm_tagtable.find_many.call_args
...
async def test_reject_clientside_metadata_tags_enabled_with_tags():
    ...
    from litelm.proxy.auth.auth_checks import common_checks
...
async def test_get_fuzzy_user_object_case_insensitive_email():
    ...
    mock_prisma.db.litelm_usertable.find_first.assert_called_once()
    call_args = mock_prisma.db.litelm_usertable.find_first.call_args
...
async def test_custom_auth_common_checks_opt_in():
    ...
    """
    from litelm.proxy.auth.user_api_key_auth import _run_post_custom_auth_checks
    ...
    with patch(
        "litelm.proxy.auth.user_api_key_auth.common_checks",
