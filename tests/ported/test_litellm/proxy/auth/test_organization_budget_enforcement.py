...
async def test_organization_budget_exceeded_blocks_request():
    ...
    with patch("litelm.proxy.proxy_server.prisma_client") as mock_prisma:
        with patch("litelm.proxy.proxy_server.user_api_key_cache") as mock_cache:
            with patch("litelm.proxy.auth.auth_checks.get_org_object", new_callable=AsyncMock) as mock_get_org:
