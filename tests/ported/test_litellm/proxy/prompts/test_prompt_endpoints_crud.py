...
async def test_delete_prompt_success():
    ...
    """
    from litelm.proxy.prompts.prompt_endpoints import delete_prompt
    ...
    mock_prisma_client.db.litelm_prompttable.delete_many = AsyncMock(return_value=None)

    # Mock In-Memory Registry
    with patch(
        "litelm.proxy.prompts.prompt_registry.IN_MEMORY_PROMPT_REGISTRY"
    ) as mock_registry:
        ...
        with patch("litelm.proxy.proxy_server.prisma_client", mock_prisma_client):
            ...
            mock_prisma_client.db.litelm_prompttable.delete_many.assert_called_once_with(
...
async def test_get_prompt_info_by_base_id():
    ...
    """
    from litelm.proxy.prompts.prompt_endpoints import get_prompt_info
