...
import litelm.proxy.health_endpoints._health_endpoints as _health_endpoints_module

from litelm.proxy.health_endpoints._health_endpoints import (
...
from tests.test_litelm.proxy.conftest import create_proxy_test_client
...
async def test_health_services_endpoint_sqs(status, error_message):
    ...
    """
    with patch("litelm.integrations.sqs.SQSLogger") as MockSQSLogger:
...
async def test_health_license_endpoint_with_active_license():
    ...
    with patch(
        ...
        "litelm.proxy.proxy_server.premium_user_data",
        license_data,
    ):
...
async def test_test_model_connection_loads_config_from_router():
    ...
    # Mock _update_litelm_params_for_health_check
    def mock_update_params(model_info, litelm_params):
        # Just return params with messages added
        params = litelm_params.copy()
    ...
    with patch(
        ...
        "litelm.proxy.management_endpoints.model_management_endpoints.ModelManagementAuthChecks.can_user_make_model_call",
        mock_can_user_make_model_call,
    ), patch(
        "litelm.proxy.health_endpoints._health_endpoints.litelm.ahealth_check",
        mock_ahealth_check,
    ), patch(
        "litelm.proxy.health_endpoints._health_endpoints.run_with_timeout",
        mock_run_with_timeout,
    ), patch(
        "litelm.proxy.health_endpoints._health_endpoints._update_litelm_params_for_health_check",
        mock_update_params,
    ), patch(
        "litelm.proxy.health_endpoints._health_endpoints._resolve_os_environ_variables",
...
async def test_health_services_endpoint_datadog_llm_observability():
    ...
    Regression test for: https://github.com/BerriAI/litelm/issues/XXXX
    ...
    with patch("litelm.success_callback", ["datadog_llm_observability"]):
...
def test_health_readiness(proxy_client):
    ...
    assert "litelm_version" in response_data, "Response should contain 'litelm_version' field"
    ...
    print(f"LiteLLM Version: {response_data.get('litelm_version', 'unknown')}")
...
def test_get_callback_identifier_string_and_object_with_callback_name():
    ...
    """
    from litelm.proxy.health_endpoints._health_endpoints import get_callback_identifier
...
def test_get_callback_identifier_custom_logger_registry_and_fallback():
    ...
    """
    from litelm.proxy.health_endpoints._health_endpoints import get_callback_identifier
    from litelm.litelm_core_utils.custom_logger_registry import CustomLoggerRegistry
