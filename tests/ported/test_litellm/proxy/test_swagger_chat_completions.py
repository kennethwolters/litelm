...
from litelm.proxy.common_utils.custom_openapi_spec import CustomOpenAPISpec
...
class TestSwaggerChatCompletions:
    ...
    def test_openapi_schema_includes_chat_completions_request_body(self, client):
        ...
        for field in expected_litelm_fields:
    ...
    def test_chat_completions_endpoints_have_expanded_request_body(self, client):
        ...
        litelm_fields = ["guardrails", "caching", "fallbacks", "num_retries"]
        for field in litelm_fields:
    ...
    @patch('litelm.proxy.common_utils.custom_openapi_spec.CustomOpenAPISpec.add_chat_completion_request_schema')
    ...
    def test_proxy_chat_completion_request_pydantic_model_works(self):
        ...
        """
        from litelm.proxy._types import ProxyChatCompletionRequest
        ...
        litelm_fields = ["guardrails", "caching", "num_retries", "context_window_fallback_dict", "fallbacks"]
        for field in litelm_fields:
    ...
    def test_openapi_schema_servers_url_with_root_path(self):
        ...
        from litelm.proxy.proxy_server import app, custom_openapi, get_openapi_schema

        # Test cases: (server_root_path, expected_servers_url)
        # Note: empty string is falsy in Python, so servers won't be set
        test_cases = [
            ("/litelm", "/litelm"),
            ("/litelm/", "/litelm"),  # trailing slash should be removed
            ("litelm", "/litelm"),  # missing leading slash should be added
            ("/api/v1", "/api/v1"),
        ]
        ...
        for root_path, expected_url in test_cases:
            ...
            with patch("litelm.proxy.proxy_server.server_root_path", root_path):
