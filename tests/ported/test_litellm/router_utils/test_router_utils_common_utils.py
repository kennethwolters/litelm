...
from litelm.router_utils.common_utils import (
...
class TestFilterTeamBasedModels:
    ...
    def test_filter_team_based_models_team_match_litelm_metadata(
        self, sample_deployments_with_teams
    ):
        """Test filtering when team_id is in litelm_metadata"""
        request_kwargs = {"litelm_metadata": {"user_api_key_team_id": "team-b"}}
        result = filter_team_based_models(sample_deployments_with_teams, request_kwargs)
    ...
    def test_filter_team_based_models_priority_metadata_over_litelm(
        self, sample_deployments_with_teams
    ):
        """Test that metadata.user_api_key_team_id takes priority over litelm_metadata.user_api_key_team_id"""
        request_kwargs = {
            "metadata": {
                "user_api_key_team_id": "team-a",  # This should take priority
                "litelm_metadata": {"user_api_key_team_id": "team-b"},
            }
        }
        result = filter_team_based_models(sample_deployments_with_teams, request_kwargs)

        # Should filter based on team-a (from metadata, not litelm_metadata)
        expected_ids = ["deployment-1", "deployment-3", "deployment-4"]
        result_ids = [d.get("model_info", {}).get("id") for d in result]
        assert sorted(result_ids) == sorted(expected_ids)
...
class TestDeploymentSupportsWebSearch:
    ...
    def test_no_config_defaults_to_true(self):
        ...
        deployment = {"litelm_params": {"model": "gpt-4"}, "model_info": {"id": "123"}}
        assert _deployment_supports_web_search(deployment) is True
    ...
    def test_missing_model_info_defaults_to_true(self):
        ...
        deployment = {"litelm_params": {"model": "gpt-4"}}
        assert _deployment_supports_web_search(deployment) is True
