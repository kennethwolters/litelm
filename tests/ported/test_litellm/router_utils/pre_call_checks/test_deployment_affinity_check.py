...
from litelm.router_utils.pre_call_checks.deployment_affinity_check import (
...
async def test_async_user_key_affinity_routes_to_same_deployment():
    ...
    with patch(
        ...
        first_response = await router.aresponses(
            ...
            truncation="auto",
            litelm_metadata={"user_api_key_hash": user_api_key_hash},
        )
    ...
    healthy_deployments = [
        {
            ...
            "litelm_params": {"model": f"vertex_ai/{stable_model_map_key}"},
            "model_info": {"id": "deployment-1"},
        },
        {
            "model_name": stable_model_map_key,
            "litelm_params": {"model": f"bedrock/global.anthropic.{stable_model_map_key}-v1:0"},
