...
def _load_backup_json() -> dict:
    ...
    backup_path = os.path.join(
        os.path.dirname(litelm.__file__),
...
class TestDeepSeekModelCostEntries:
    ...
    def test_main_json_deepseek_chat_supports_response_schema(self):
        main_path = os.path.join(
            os.path.dirname(os.path.dirname(litelm.__file__)),
...
class TestBareModelFallback:
    ...
    def test_fallback_uses_bare_entry(self):
        ...
        original = litelm.model_cost.get(key, {}).get("supports_response_schema")
        try:
            # Simulate the pre-fix state: field missing from prefixed entry
            if key in litelm.model_cost:
                litelm.model_cost[key].pop("supports_response_schema", None)
            ...
            # Restore
            if key in litelm.model_cost and original is not None:
                litelm.model_cost[key]["supports_response_schema"] = original
    ...
    def test_no_fallback_when_explicitly_false(self):
        ...
        original = litelm.model_cost.get(key, {}).get("supports_function_calling")
        try:
            if key in litelm.model_cost:
                litelm.model_cost[key]["supports_function_calling"] = False
            result = _supports_factory(
            ...
            if key in litelm.model_cost and original is not None:
                litelm.model_cost[key]["supports_function_calling"] = original
