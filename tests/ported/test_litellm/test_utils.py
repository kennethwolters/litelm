...
import litelm
from litelm.proxy.utils import is_valid_api_key
...
def test_check_provider_match_azure_ai_allows_openai_and_azure():
    ...
    assert _check_provider_match(
        model_info={"litelm_provider": "openai"},
        custom_llm_provider="azure_ai"
    ) is True

    # azure_ai should match azure models
    assert _check_provider_match(
        model_info={"litelm_provider": "azure"},
        custom_llm_provider="azure_ai"
    ) is True

    # azure_ai should NOT match other providers
    assert _check_provider_match(
        model_info={"litelm_provider": "anthropic"},
        custom_llm_provider="azure_ai"
    ) is False
...
def test_check_provider_match_github_allows_upstream_provider_metadata():
    ...
    assert _check_provider_match(
        model_info={"litelm_provider": "github"},
        custom_llm_provider="github",
    ) is True
...
def test_supports_function_calling_github_openai_alias():
    assert litelm.utils.supports_function_calling(model="github/gpt-4o-mini") is True
    assert (
        litelm.utils.supports_function_calling(
...
def test_supports_function_calling_deepinfra_llama():
    ...
    Regression test for https://github.com/BerriAI/litelm/issues/22619
    """
    assert (
        litelm.utils.supports_function_calling(
...
def test_get_optional_params_image_gen():
    from litelm.llms.azure.image_generation import AzureGPTImageGenerationConfig
...
def test_all_model_configs():
    from litelm.llms.vertex_ai.vertex_ai_partner_models.ai21.transformation import (
        VertexAIAi21Config,
    )
    from litelm.llms.vertex_ai.vertex_ai_partner_models.llama3.transformation import (
    ...
    from litelm.llms.fireworks_ai.chat.transformation import FireworksAIConfig
    ...
    from litelm.llms.nvidia_nim.chat.transformation import NvidiaNimConfig
    ...
    from litelm.llms.ollama.chat.transformation import OllamaChatConfig
    ...
    from litelm.llms.predibase.chat.transformation import PredibaseConfig
    ...
    from litelm.llms.codestral.completion.transformation import (
    ...
    from litelm.llms.volcengine.chat.transformation import (
    ...
    from litelm.llms.ai21.chat.transformation import AI21ChatConfig
    ...
    from litelm.llms.azure.chat.gpt_transformation import AzureOpenAIConfig
    ...
    from litelm.llms.bedrock.chat.converse_transformation import AmazonConverseConfig
    ...
    from litelm import AmazonAnthropicClaudeConfig, AmazonAnthropicConfig
    ...
    from litelm.llms.databricks.chat.transformation import DatabricksConfig
    ...
    from litelm.llms.vertex_ai.vertex_ai_partner_models.anthropic.transformation import (
    ...
    from litelm.llms.gemini.chat.transformation import GoogleAIStudioGeminiConfig
    from litelm.llms.vertex_ai.gemini.vertex_and_google_ai_studio_gemini import (
        VertexGeminiConfig,
    )
...
def test_anthropic_web_search_in_model_info():
    ...
    litelm.model_cost = litelm.get_model_cost_map(url="")
    ...
    for model in supported_models:
        from litelm.utils import get_model_info
...
def test_cohere_embedding_optional_params():
    from litelm import get_optional_params_embeddings
...
def test_openai_models_in_model_info():
    ...
    for model, info in model_map.items():
        if (
            info.get("litelm_provider") == "openai"
...
def test_supports_tool_choice_simple_tests():
    ...
    """
    assert litelm.utils.supports_tool_choice(model="gpt-4o") == True
    assert (
        litelm.utils.supports_tool_choice(
    ...
    assert (
        litelm.utils.supports_tool_choice(model="us.amazon.nova-micro-v1:0") is False
    )
    assert (
        litelm.utils.supports_tool_choice(model="bedrock/us.amazon.nova-micro-v1:0")
        is False
    )
    assert (
        litelm.utils.supports_tool_choice(
    ...
    assert litelm.utils.supports_tool_choice(model="perplexity/sonar") is False
...
def test_check_provider_match():
    ...
    model_info = {"litelm_provider": "bedrock"}
    assert litelm.utils._check_provider_match(model_info, "bedrock") is True
    assert litelm.utils._check_provider_match(model_info, "bedrock_converse") is True

    # Test bedrock_converse provider
    model_info = {"litelm_provider": "bedrock_converse"}
    assert litelm.utils._check_provider_match(model_info, "bedrock") is True
    assert litelm.utils._check_provider_match(model_info, "bedrock_converse") is True

    # Test non-matching provider
    model_info = {"litelm_provider": "bedrock"}
    assert litelm.utils._check_provider_match(model_info, "openai") is False
...
def test_get_provider_rerank_config():
    ...
    """
    from litelm import HostedVLLMRerankConfig
    from litelm.utils import LlmProviders, ProviderConfigManager
...
def test_supports_computer_use_utility():
    """
    Tests the litelm.utils.supports_computer_use utility function.
    """
    from litelm.utils import supports_computer_use

    # Ensure LITELLM_LOCAL_MODEL_COST_MAP is set for consistent test behavior,
    # as supports_computer_use relies on get_model_info.
    # This also requires litelm.model_cost to be populated.
    original_env_var = os.getenv("LITELLM_LOCAL_MODEL_COST_MAP")
    original_model_cost = getattr(litelm, "model_cost", None)

    os.environ["LITELLM_LOCAL_MODEL_COST_MAP"] = "True"
    litelm.model_cost = litelm.get_model_cost_map(url="")  # Load with local/backup
    ...
    try:
        ...
        elif hasattr(litelm, "model_cost"):
            delattr(litelm, "model_cost")
...
def test_get_model_info_shows_supports_computer_use():
    ...
    """
    os.environ["LITELLM_LOCAL_MODEL_COST_MAP"] = "True"
    # Ensure litelm.model_cost is loaded, relying on the backup mechanism if primary fails
    ...
    info = litelm.get_model_info(model_known_to_support_computer_use)
    ...
    info_gpt = litelm.get_model_info(model_known_not_to_support_computer_use)
...
def test_pre_process_non_default_params(model, custom_llm_provider):
    ...
    from litelm.utils import ProviderConfigManager, pre_process_non_default_params
...
from litelm.utils import supports_function_calling
...
class TestProxyFunctionCalling:
    ...
    def reset_mock_cache(self):
        ...
        from litelm.utils import _model_cache
    ...
    @pytest.mark.parametrize(
        ...
        [
            # OpenAI models
            ("gpt-3.5-turbo", "litelm_proxy/gpt-3.5-turbo", True),
            ("gpt-4", "litelm_proxy/gpt-4", True),
            ("gpt-4o", "litelm_proxy/gpt-4o", True),
            ("gpt-4o-mini", "litelm_proxy/gpt-4o-mini", True),
            ("gpt-4-turbo", "litelm_proxy/gpt-4-turbo", True),
            ("gpt-4-1106-preview", "litelm_proxy/gpt-4-1106-preview", True),
            # Azure OpenAI models
            ("azure/gpt-4", "litelm_proxy/azure/gpt-4", True),
            ("azure/gpt-3.5-turbo", "litelm_proxy/azure/gpt-3.5-turbo", True),
            (
                "azure/gpt-4-1106-preview",
                "litelm_proxy/azure/gpt-4-1106-preview",
                True,
            ),
            # Anthropic models (Claude supports function calling)
            (
                "claude-sonnet-4-6",
                "litelm_proxy/claude-sonnet-4-6",
                True,
            ),
            # Google models
            ("gemini-2.5-pro", "litelm_proxy/gemini-2.5-pro", True),
            ("gemini/gemini-2.5-pro", "litelm_proxy/gemini/gemini-2.5-pro", True),
            ("gemini/gemini-2.5-flash", "litelm_proxy/gemini/gemini-2.5-flash", True),
            # Groq models (mixed support)
            ("groq/gemma-7b-it", "litelm_proxy/groq/gemma-7b-it", True),
            (
                "groq/llama-3.3-70b-versatile",
                "litelm_proxy/groq/llama-3.3-70b-versatile",
                True,
            ),
            # Cohere models (generally don't support function calling)
            ("command-nightly", "litelm_proxy/command-nightly", False),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "litelm_proxy/bedrock-claude-3-haiku",
                "bedrock/anthropic.claude-3-haiku-20240307-v1:0",
                False,
            ),
            (
                "litelm_proxy/bedrock-claude-3-sonnet",
                "bedrock/anthropic.claude-3-sonnet-20240229-v1:0",
                False,
            ),
            (
                "litelm_proxy/bedrock-claude-3-opus",
                "bedrock/anthropic.claude-3-opus-20240229-v1:0",
                False,
            ),
            (
                "litelm_proxy/bedrock-claude-instant",
                "bedrock/anthropic.claude-instant-v1",
                False,
            ),
            (
                "litelm_proxy/bedrock-titan-text",
                "bedrock/amazon.titan-text-express-v1",
                False,
            ),
            # Azure with custom deployment names (cannot be resolved)
            ("litelm_proxy/my-gpt4-deployment", "azure/gpt-4", False),
            ("litelm_proxy/production-gpt35", "azure/gpt-3.5-turbo", False),
            ("litelm_proxy/dev-gpt4o", "azure/gpt-4o", False),
            # Custom OpenAI deployments (cannot be resolved)
            ("litelm_proxy/company-gpt4", "gpt-4", False),
            ("litelm_proxy/internal-gpt35", "gpt-3.5-turbo", False),
            # Vertex AI with custom names (cannot be resolved)
            ("litelm_proxy/vertex-gemini-pro", "vertex_ai/gemini-1.5-pro", False),
            ("litelm_proxy/vertex-gemini-flash", "vertex_ai/gemini-1.5-flash", False),
            # Anthropic with custom names (cannot be resolved)
            ("litelm_proxy/claude-prod", "anthropic/claude-3-sonnet-20240229", False),
            ("litelm_proxy/claude-dev", "anthropic/claude-3-haiku-20240307", False),
            # Groq with custom names (cannot be resolved)
            ("litelm_proxy/fast-llama", "groq/llama-3.1-8b-instant", False),
            ("litelm_proxy/groq-gemma", "groq/gemma-7b-it", False),
            # Cohere with custom names (cannot be resolved)
            ("litelm_proxy/cohere-command", "cohere/command-r", False),
            ("litelm_proxy/cohere-command-plus", "cohere/command-r-plus", False),
            # Together AI with custom names (cannot be resolved)
            (
                "litelm_proxy/together-llama",
                "together_ai/meta-llama/Llama-2-70b-chat-hf",
                False,
            ),
            (
                "litelm_proxy/together-mistral",
                "together_ai/mistralai/Mistral-7B-Instruct-v0.1",
                False,
            ),
            # Ollama with custom names (cannot be resolved)
            ("litelm_proxy/local-llama", "ollama/llama2", False),
            ("litelm_proxy/local-mistral", "ollama/mistral", False),
        ],
    )
    ...
    def test_proxy_model_resolution_with_custom_names_documentation(self):
        ...
        custom_model = "litelm_proxy/my-custom-claude"
        ...
        resolvable_model = "litelm_proxy/claude-sonnet-4-5-20250929"
        ...
        ✅ WORKS (with current fallback logic):
           - litelm_proxy/gpt-4
           - litelm_proxy/claude-sonnet-4-5-20250929
           - litelm_proxy/anthropic/claude-3-haiku-20240307
           
        ❌ DOESN'T WORK (requires proxy server config):
           - litelm_proxy/my-custom-gpt4
           - litelm_proxy/bedrock-claude-3-haiku
           - litelm_proxy/production-model
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("litelm_proxy/gpt-4-with-functions", True),  # Hints at GPT-4
            ("litelm_proxy/claude-3-haiku-prod", True),  # Hints at Claude 3 Haiku
            (
                "litelm_proxy/bedrock-anthropic-claude-3-sonnet",
                True,
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("litelm_proxy/gpt-3.5-turbo", True),
            ("litelm_proxy/gpt-4", True),
            ("litelm_proxy/gpt-4o", True),
            ("litelm_proxy/claude-sonnet-4-6", True),
            ("litelm_proxy/gemini/gemini-2.5-pro", True),
            # Test proxy models that should not support function calling
            ("litelm_proxy/command-nightly", False),
            ("litelm_proxy/anthropic.claude-instant-v1", False),
        ],
    )
    ...
    def test_litelm_utils_supports_function_calling_import(self):
        """Test that supports_function_calling can be imported from litelm.utils."""
        try:
            from litelm.utils import supports_function_calling
    ...
    def test_litelm_supports_function_calling_import(self):
        """Test that supports_function_calling can be imported from litelm directly."""
        try:
            import litelm

            assert hasattr(litelm, "supports_function_calling")
            assert callable(litelm.supports_function_calling)
        except Exception as e:
            pytest.fail(f"Failed to access litelm.supports_function_calling: {e}")

    @pytest.mark.parametrize(
        "model_name",
        [
            "litelm_proxy/gpt-3.5-turbo",
            "litelm_proxy/gpt-4",
            "litelm_proxy/claude-sonnet-4-6",
            "litelm_proxy/gemini/gemini-2.5-pro",
        ],
    )
    ...
    def test_edge_cases_and_malformed_proxy_models(self):
        ...
        test_cases = [
            ("litelm_proxy/", False),  # Empty model name after proxy prefix
            ("litelm_proxy", False),  # Just the proxy prefix without slash
            ("litelm_proxy//gpt-3.5-turbo", False),  # Double slash
            ("litelm_proxy/nonexistent-model", False),  # Non-existent model
        ]
    ...
    def test_proxy_model_resolution_demonstration(self):
        ...
        proxy_model = "litelm_proxy/gpt-3.5-turbo"
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "litelm_proxy/bedrock-claude-3-5-sonnet",
            ...
            (
                "litelm_proxy/bedrock-claude-v2",
                "bedrock/anthropic.claude-v2",
                False,
                "Bedrock Claude v2 Legacy API",
            ),
            (
                "litelm_proxy/bedrock-claude-v2-1",
            ...
            (
                "litelm_proxy/prod-claude-haiku",
                "bedrock/converse/anthropic.claude-3-haiku-20240307-v1:0",
                False,
                "Production Claude Haiku",
            ),
            (
                "litelm_proxy/dev-claude-sonnet",
                "bedrock/converse/anthropic.claude-3-sonnet-20240229-v1:0",
                False,
                "Development Claude Sonnet",
            ),
            (
                "litelm_proxy/staging-claude-opus",
                "bedrock/converse/anthropic.claude-3-opus-20240229-v1:0",
                False,
                "Staging Claude Opus",
            ),
            (
                "litelm_proxy/cost-optimized-claude",
                "bedrock/converse/anthropic.claude-3-haiku-20240307-v1:0",
                False,
                "Cost-optimized Claude deployment",
            ),
            (
                "litelm_proxy/high-performance-claude",
                "bedrock/converse/anthropic.claude-3-opus-20240229-v1:0",
                False,
            ...
            (
                "litelm_proxy/us-east-claude",
                "bedrock/converse/anthropic.claude-3-sonnet-20240229-v1:0",
                False,
                "US East Claude deployment",
            ),
            (
                "litelm_proxy/eu-west-claude",
                "bedrock/converse/anthropic.claude-3-haiku-20240307-v1:0",
                False,
                "EU West Claude deployment",
            ),
            (
                "litelm_proxy/ap-south-llama",
                "bedrock/converse/meta.llama3-70b-instruct-v1:0",
                False,
...
def test_register_model_with_scientific_notation():
    ...
    litelm.register_model(model_cost_dict)

    registered_model = litelm.model_cost[test_model_name]
    print(registered_model)
    assert registered_model["input_cost_per_token"] == 3e-07
    assert registered_model["output_cost_per_token"] == 6e-07
    assert registered_model["litelm_provider"] == "openai"
    assert registered_model["mode"] == "chat"
    
    # Clean up after test
    if test_model_name in litelm.model_cost:
        del litelm.model_cost[test_model_name]
    _invalidate_model_cost_lowercase_map()
...
def test_register_model_openrouter_without_slash():
    ...
    Fixes https://github.com/BerriAI/litelm/issues/18936
    ...
    litelm.openrouter_models.discard("my-custom-alias")
    litelm.openrouter_models.discard("gpt-4")
    litelm.openrouter_models.discard("openai/gpt-4")
    ...
    litelm.register_model(
        {
            "my-custom-alias": {
                ...
                "litelm_provider": "openrouter",
                "mode": "chat",
            },
        }
    )
    assert "my-custom-alias" in litelm.openrouter_models
    ...
    assert "gpt-4" in litelm.openrouter_models
    ...
    assert "openai/gpt-4-turbo" in litelm.openrouter_models
...
def test_anthropic_claude_4_invoke_chat_provider_config():
    ...
    from litelm.llms.bedrock.chat.invoke_transformations.anthropic_claude3_transformation import (
...
def test_bedrock_application_inference_profile():
    ...
    from litelm.utils import supports_tool_choice
...
def test_image_response_utils():
    ...
    from litelm.utils import ImageResponse
...
def test_block_key_hashing_logic():
    ...
    from litelm.proxy.utils import hash_token
...
def test_generate_gcp_iam_access_token():
    ...
    with patch.dict(
        ...
        from litelm._redis import _generate_gcp_iam_access_token
...
def test_model_info_for_vertex_ai_deepseek_model():
    model_info = litelm.get_model_info(
        model="vertex_ai/deepseek-ai/deepseek-r1-0528-maas"
    )
    assert model_info is not None
    assert model_info["litelm_provider"] == "vertex_ai-deepseek_models"
...
def test_model_info_for_openrouter_kimi_k2_5():
    ...
    assert model_info["litelm_provider"] == "openrouter"
    assert model_info["mode"] == "chat"
...
def test_model_info_for_fireworks_short_form_models():
    ...
    for key in [
        ...
        assert info["litelm_provider"] == "fireworks_ai"
...
class TestGetValidModelsWithCLI:
    ...
    def test_get_valid_models_with_cli_pattern(self):
        """Test get_valid_models with litelm_proxy provider and CLI token pattern"""
        ...
        mock_response.json.return_value = {
            "data": [
                ...
                {"id": "litelm_proxy/gemini/gemini-2.5-flash", "object": "model"},
                {"id": "claude-3-sonnet", "object": "model"},
            ]
        }

        with patch.object(
            litelm.module_level_client, "get", return_value=mock_response
        ) as mock_get:
            # Test the exact pattern used in cli_token_usage.py
            result = litelm.get_valid_models(
                check_provider_endpoint=True,
                custom_llm_provider="litelm_proxy",
            ...
            assert len(result) == 4
            # All models get prefixed with "litelm_proxy/" by the get_models method
            assert "litelm_proxy/gpt-3.5-turbo" in result
            assert "litelm_proxy/gpt-4" in result
            # Note: This model already had the prefix, so it gets double-prefixed
            assert "litelm_proxy/litelm_proxy/gemini/gemini-2.5-flash" in result
            assert "litelm_proxy/claude-3-sonnet" in result
...
class TestProxyLoggingBudgetAlerts:
    ...
    async def test_budget_alerts_soft_budget_with_alert_emails_bypasses_alerting_none(self):
        ...
        """
        from litelm.caching.caching import DualCache
        from litelm.proxy._types import CallInfo, Litellm_EntityType
        from litelm.proxy.utils import ProxyLogging
...
def test_azure_ai_claude_provider_config():
    ...
    from litelm import AzureAIStudioConfig, AzureAnthropicConfig
    from litelm.utils import ProviderConfigManager
...
# Related to issue: https://github.com/BerriAI/litelm/issues/18926
...
def test_any_assistant_message_has_thinking_blocks_with_thinking():
    ...
    from litelm.utils import any_assistant_message_has_thinking_blocks
...
class TestAdditionalDropParamsForNonOpenAIProviders:
    ...
    Fixes https://github.com/BerriAI/litelm/issues/19225
    ...
    def test_additional_drop_params_filters_for_bedrock(self):
        ...
        """
        from litelm.utils import add_provider_specific_params_to_optional_params
...
class TestDropParamsWithPromptCacheKey:
    ...
    def test_prompt_cache_key_in_default_params(self):
        ...
        from litelm.constants import DEFAULT_CHAT_COMPLETION_PARAM_VALUES
    ...
    def test_drop_params_removes_prompt_cache_key_for_bedrock(self):
        ...
        """
        from litelm.utils import get_optional_params
...
class TestCallbackAsyncSyncSeparation:
    ...
    def setup_method(self):
        ...
        litelm.input_callback = []
        litelm.success_callback = []
        litelm.failure_callback = []
        litelm._async_input_callback = []
        litelm._async_success_callback = []
        litelm._async_failure_callback = []

    def test_async_success_callback_routed_to_async_list(self):
        async def my_async_cb(*args, **kwargs):
            pass

        litelm.logging_callback_manager.add_litelm_success_callback(my_async_cb)
        assert my_async_cb in litelm._async_success_callback
        assert my_async_cb not in litelm.success_callback

    def test_sync_success_callback_stays_in_sync_list(self):
        def my_sync_cb(*args, **kwargs):
            pass

        litelm.logging_callback_manager.add_litelm_success_callback(my_sync_cb)
        assert my_sync_cb in litelm.success_callback
        assert my_sync_cb not in litelm._async_success_callback

    def test_string_callback_stays_in_sync_list(self):
        litelm.logging_callback_manager.add_litelm_success_callback("langfuse")
        assert "langfuse" in litelm.success_callback
        assert "langfuse" not in litelm._async_success_callback

    def test_async_failure_callback_routed_to_async_list(self):
        async def my_async_cb(*args, **kwargs):
            pass

        litelm.logging_callback_manager.add_litelm_failure_callback(my_async_cb)
        assert my_async_cb in litelm._async_failure_callback
        assert my_async_cb not in litelm.failure_callback

    def test_sync_failure_callback_stays_in_sync_list(self):
        def my_sync_cb(*args, **kwargs):
            pass

        litelm.logging_callback_manager.add_litelm_failure_callback(my_sync_cb)
        assert my_sync_cb in litelm.failure_callback
        assert my_sync_cb not in litelm._async_failure_callback

    def test_dynamodb_routed_to_async_success(self):
        litelm.logging_callback_manager.add_litelm_success_callback("dynamodb")
        assert "dynamodb" in litelm._async_success_callback
        assert "dynamodb" not in litelm.success_callback

    def test_openmeter_routed_to_async_success(self):
        litelm.logging_callback_manager.add_litelm_success_callback("openmeter")
        assert "openmeter" in litelm._async_success_callback
        assert "openmeter" not in litelm.success_callback

    def test_async_input_callback_routed_to_async_list(self):
        async def my_async_cb(*args, **kwargs):
            pass

        litelm.logging_callback_manager.add_litelm_input_callback(my_async_cb)
        assert my_async_cb in litelm._async_input_callback
        assert my_async_cb not in litelm.input_callback

    def test_sync_input_callback_stays_in_sync_list(self):
        def my_sync_cb(*args, **kwargs):
            pass

        litelm.logging_callback_manager.add_litelm_input_callback(my_sync_cb)
        assert my_sync_cb in litelm.input_callback
        assert my_sync_cb not in litelm._async_input_callback
...
class TestMetadataNoneHandling:
    ...
    def test_metadata_none_model_group_check(self):
        ...
        kwargs = {"metadata": None}
        _is_litelm_router_call = "model_group" in (
            kwargs.get("metadata") or {}
        )
        assert _is_litelm_router_call is False
    ...
    def test_metadata_present_with_values(self):
        ...
        assert _is_litelm_router_call is True
    ...
    def test_litelm_params_metadata_none(self):
        """litelm_params.get("metadata") or {} should handle None value."""
        litelm_params = {"metadata": None}
        metadata = litelm_params.get("metadata") or {}
        assert metadata == {}
...
class TestValidateAndFixThinkingParam:
    ...
    def test_none_returns_none(self):
        from litelm.utils import validate_and_fix_thinking_param
