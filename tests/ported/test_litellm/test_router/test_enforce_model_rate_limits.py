...
from litelm.router_utils.pre_call_checks.model_rate_limit_check import (
...
class TestModelRateLimitingCheck:
    ...
    def test_get_deployment_limits_from_litelm_params(self):
        """Test extracting limits from litelm_params."""
        check = ModelRateLimitingCheck(dual_cache=MagicMock())

        deployment = {
            "litelm_params": {"model": "gpt-4", "tpm": 2000, "rpm": 20},
            "model_info": {"id": "test-id"},
        }
    ...
    def test_pre_call_check_raises_rate_limit_error_when_over_rpm(self):
        ...
        with pytest.raises(litelm.RateLimitError) as exc_info:
    ...
    def test_log_success_event_increments_cache(self):
        ...
        kwargs = {
            "standard_logging_object": {
                "model_id": "test-id",
                "total_tokens": 50,
                "hidden_params": {"litelm_model_name": "gpt-4"},
            }
        }
...
class TestRouterWithEnforceModelRateLimits:
    ...
    def test_router_initializes_with_enforce_model_rate_limits(self):
        ...
        model_list = [
            {
                "model_name": "gpt-4",
                "litelm_params": {"model": "gpt-4", "api_key": "test"},
                "rpm": 10,
            }
        ]
    ...
    def test_router_optional_callbacks_contains_model_rate_limiting(self):
        ...
        # Find the ModelRateLimitingCheck in litelm.callbacks
        found = False
        ...
        assert found, "ModelRateLimitingCheck should be in litelm.callbacks"
...
class TestModelRateLimitConcurrency:
    ...
    async def test_concurrent_requests_respect_rpm_limit(self):
        ...
        failures = [r for r in results if isinstance(r, litelm.RateLimitError)]
