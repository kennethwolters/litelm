"""
Unit tests for per-deployment num_retries in litelm_params
GitHub Issue: #18968 - Per-deployment max_retries/num_retries in litelm_params is not used in retry logic
"""
...
class TestPerDeploymentNumRetries:
    """Test that per-deployment num_retries in litelm_params is correctly used."""

    def test_set_deployment_num_retries_on_exception(self):
        """
        Test that _set_deployment_num_retries_on_exception sets num_retries
        on the exception from the deployment's litelm_params.
        """
        router = Router(
            model_list=[
                {
