...
import litelm
from litelm.responses.litelm_completion_transformation import session_handler
from litelm.responses.litelm_completion_transformation.session_handler import (
...
async def test_get_chat_completion_message_history_for_previous_response_id():
    ...
    with patch.object(
        ...
        assert "litelm_session_id" in result

        # Verify session_id is extracted correctly
        assert result["litelm_session_id"] == "a96757c4-c6dc-4c76-b37e-e7dfa526b701"
...
async def test_get_chat_completion_message_history_empty_spend_logs():
    ...
    with patch.object(
        ...
        assert result.get("litelm_session_id") is None
...
async def test_e2e_cold_storage_successful_retrieval():
    ...
    mock_spend_logs = [
        {
            ...
            "proxy_server_request": '{"litelm_truncated": true}',  # Truncated payload
            "response": {
    ...
    patch("litelm.cold_storage_custom_logger", return_value="s3"):
        ...
        assert result.get("litelm_session_id") == "session-456"
...
async def test_e2e_cold_storage_fallback_to_truncated_payload():
    ...
    patch.object(session_handler, "COLD_STORAGE_HANDLER") as mock_cold_storage:
        ...
        # Verify result structure
        assert result.get("litelm_session_id") == "session-999"
        assert len(result.get("messages", [])) >= 1  # At least the assistant response
...
async def test_should_check_cold_storage_for_full_payload():
    ...
    proxy_request_with_truncated_pdf = {
        ...
        "stream": True,
        "litelm_trace_id": "16b86861-c120-4ecb-865b-4d2238bfd8f0"
    }
    ...
    with patch("litelm.cold_storage_custom_logger", return_value="s3"):
...
async def test_get_chat_completion_message_history_empty_response_dict():
    ...
    with patch.object(ResponsesSessionHandler, "get_all_spend_logs_for_previous_response_id") as mock_get_spend_logs:
        ...
        assert result["litelm_session_id"] == "test-session"
