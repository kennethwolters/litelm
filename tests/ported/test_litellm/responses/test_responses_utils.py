...
import litelm
from litelm.llms.base_llm.responses.transformation import BaseResponsesAPIConfig
from litelm.llms.openai.responses.transformation import OpenAIResponsesAPIConfig
from litelm.responses.utils import ResponseAPILoggingUtils, ResponsesAPIRequestUtils
...
class TestResponsesAPIRequestUtils:
    ...
    def test_get_optional_params_responses_api_unsupported_param(self):
        ...
        # Execute and Assert
        with pytest.raises(litelm.UnsupportedParamsError) as excinfo:
    ...
    def test_update_responses_api_response_id_with_model_id_handles_dict(self):
        ...
        litelm_metadata = {"model_info": {"id": "gpt-4o"}}
...
def test_responses_extra_body_forwarded_to_completion_transformation_handler():
    ...
    not passed to litelm_completion_transformation_handler.response_api_handler(),
    so it was silently dropped.
    """
    with patch(
        "litelm.responses.main.ProviderConfigManager.get_provider_responses_api_config",
        return_value=None,
    ), patch(
        "litelm.responses.main.litelm_completion_transformation_handler.response_api_handler",
    ) as mock_handler:
...
def test_responses_maps_reasoning_effort_from_litelm_params_to_reasoning():
    """
    Test that when reasoning_effort is passed in kwargs (e.g. from proxy litelm_params)
