...
from litelm.responses.mcp import chat_completions_handler
from litelm.responses.mcp.chat_completions_handler import (
    acompletion_with_mcp,
)
from litelm.responses.mcp.litelm_proxy_mcp_handler import (
    LiteLLM_Proxy_MCP_Handler,
)
from litelm.responses.utils import ResponsesAPIRequestUtils
...
async def test_acompletion_with_mcp_returns_normal_completion_without_tools(monkeypatch):
    ...
    with patch("litelm.acompletion", mock_acompletion):
...
async def test_acompletion_with_mcp_without_auto_execution_calls_model(monkeypatch):
    ...
    monkeypatch.setattr(
        LiteLLM_Proxy_MCP_Handler,
        "_should_use_litelm_mcp_gateway",
...
async def test_acompletion_with_mcp_passes_mcp_server_auth_headers_to_process_tools(
    ...
    """
    tools = [{"type": "mcp", "server_url": "litelm_proxy"}]
    mock_acompletion = AsyncMock(return_value="ok")
...
async def test_acompletion_with_mcp_auto_exec_performs_follow_up(monkeypatch):
    ...
    from litelm.types.utils import ModelResponseStream, StreamingChoices, Delta, ChatCompletionDeltaToolCall, Function
    ...
    # Patch litelm.acompletion at module level to catch function-level imports
    with patch("litelm.acompletion", mock_acompletion_func), \
         patch.object(chat_completions_handler, "litelm_acompletion", mock_acompletion_func, create=True):
        result = await acompletion_with_mcp(
            model="gpt-4o-mini",
...
async def test_acompletion_with_mcp_adds_metadata_to_streaming(monkeypatch):
    ...
    """
    from litelm.utils import CustomStreamWrapper
    from litelm.types.utils import ModelResponseStream, StreamingChoices, Delta
    from litelm.litelm_core_utils.litelm_logging import Logging

    tools = [{"type": "mcp", "server_url": "litelm_proxy/mcp/local"}]
...
async def test_acompletion_with_mcp_streaming_initial_call_is_streaming(monkeypatch):
    ...
    # Patch litelm.acompletion at module level to catch function-level imports
    with patch("litelm.acompletion", mock_acompletion), \
         patch.object(chat_completions_handler, "litelm_acompletion", mock_acompletion, create=True):
        result = await acompletion_with_mcp(
            model="gpt-4o-mini",
            messages=[{"role": "user", "content": "hello"}],
            tools=tools,
...
async def test_acompletion_with_mcp_streaming_metadata_in_correct_chunks(monkeypatch):
    ...
    # Patch litelm.acompletion at module level to catch function-level imports
    with patch("litelm.acompletion", mock_acompletion_func), \
         patch.object(chat_completions_handler, "litelm_acompletion", side_effect=mock_acompletion, create=True):
        result = await acompletion_with_mcp(
            model="gpt-4o-mini",
            messages=[{"role": "user", "content": "hello"}],
            tools=tools,
...
async def test_execute_tool_calls_sets_proxy_server_request_arguments(monkeypatch):
    ...
    # NOTE: avoid monkeypatch string path here because `litelm.responses` is also
    # exported as a function on the top-level `litelm` package, which can confuse
    # pytest's dotted-path resolver.
    mcp_handler_module = importlib.import_module(
        "litelm.responses.mcp.litelm_proxy_mcp_handler"
    )
    monkeypatch.setattr(mcp_handler_module, "function_setup", mock_function_setup)
    monkeypatch.setattr(
        "litelm.proxy._experimental.mcp_server.mcp_server_manager.global_mcp_server_manager.call_tool",
        mock_call_tool,
    )
