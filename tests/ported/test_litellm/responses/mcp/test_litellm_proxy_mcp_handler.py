...
def _setup_mcp_call_environment(monkeypatch: pytest.MonkeyPatch) -> AsyncMock:
    ...
    monkeypatch.setitem(sys.modules, "litelm.proxy.proxy_server", proxy_module)
...
def test_transform_mcp_tools_to_openai_uses_chat_format(monkeypatch):
    ...
    monkeypatch.setattr(
        "litelm.experimental_mcp_client.tools.transform_mcp_tool_to_openai_tool",
        fake_transform_chat,
    )
    monkeypatch.setattr(
        "litelm.experimental_mcp_client.tools.transform_mcp_tool_to_openai_responses_api_tool",
...
@pytest.mark.asyncio
async def test_execute_tool_calls_passes_litelm_call_id_and_trace_id_to_function_setup(
    monkeypatch,
):
    """
    Regression test for ae4d92ad...:
    Ensure litelm_call_id / litelm_trace_id are forwarded into function_setup kwargs.
    """
    ...
    # NOTE: Don't patch via dotted string path here because `litelm.responses`
    # is a function attribute on the `litelm` package (shadowing the submodule),
    ...
    assert captured.get("litelm_call_id") == "cid"
    assert captured.get("litelm_trace_id") == "tid"
...
async def test_get_mcp_tools_from_manager_enables_list_tools_logging(monkeypatch):
    ...
    monkeypatch.setattr(
        "litelm.proxy._experimental.mcp_server.server._get_tools_from_mcp_servers",
        mock_get_tools,
    )
    ...
    tools, _server_names = await LiteLLM_Proxy_MCP_Handler._get_mcp_tools_from_manager(
        user_api_key_auth=user_auth,
        mcp_tools_with_litelm_proxy=[{"type": "mcp", "server_url": "litelm_proxy/mcp/deepwiki"}],
    )
