...
responses_api_bridge. The bridge converts metadata to litelm_metadata. This test
verifies metadata is preserved for custom callbacks via kwargs['litelm_params']['metadata'].
"""
...
async def test_metadata_passed_to_custom_callback_codex_models():
    ...
    which passes litelm_metadata. The fix ensures this is preserved as
    litelm_params.metadata for callback compatibility.
    """
    ...
    litelm_params = callback.captured_kwargs.get("litelm_params", {})
...
@pytest.mark.asyncio
async def test_metadata_passed_via_litelm_metadata_responses_api():
    """
    Test that when calling responses() directly with litelm_metadata,
