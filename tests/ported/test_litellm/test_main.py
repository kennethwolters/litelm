...
import litelm
from litelm import main as litelm_main
...
def clear_client_cache():
    ...
    """
    cache = getattr(litelm, "in_memory_llm_clients_cache", None)
...
async def test_url_with_format_param(model, sync_mode, monkeypatch):
    from litelm import acompletion, completion
...
def test_strip_input_examples_for_non_anthropic_providers():
    ...
    assert not litelm_main._should_allow_input_examples(
        custom_llm_provider="openai", model="gpt-4o-mini"
    )

    cleaned = litelm_main._drop_input_examples_from_tools(tools=tools)
...
async def test_extra_body_with_fallback(
    ...
    """
    test regression for https://github.com/BerriAI/litelm/issues/8425.
    ...
    original_disable_aiohttp = litelm.disable_aiohttp_transport
    ...
    try:
        ...
        litelm.disable_aiohttp_transport = original_disable_aiohttp
        litelm.in_memory_llm_clients_cache.flush_cache()
...
async def test_openai_env_base(
    ...
    try:
        response = await litelm.acompletion(model=model, messages=messages)
...
def test_responses_api_bridge_check_strips_responses_prefix():
    ...
    from litelm.main import responses_api_bridge_check

    with patch("litelm.main._get_model_info_helper") as mock_get_model_info:
...
def test_responses_api_bridge_check_gpt_5_4_pro():
    ...
    Regression test for https://github.com/BerriAI/litelm/issues/23014
...
def test_stream_chunk_builder_thinking_blocks():
    from litelm import stream_chunk_builder
    from litelm.types.utils import Delta, ModelResponseStream, StreamingChoices
...
async def test_retrying() -> None:
    ...
    with (
        ...
        pytest.raises(litelm.InternalServerError, match="LiteLLM Retried: 10 times"),
    ):
...
def test_anthropic_disable_url_suffix_env_var():
    ...
    with patch.dict(os.environ, {"ANTHROPIC_API_BASE": "https://api.example.com"}):
        ...
        with patch("litelm.main.anthropic_chat_completions") as mock_anthropic:
...
def test_anthropic_text_disable_url_suffix_env_var():
    ...
    with patch.dict(os.environ, {"ANTHROPIC_API_BASE": "https://api.example.com"}):
        ...
        with patch("litelm.main.base_llm_http_handler") as mock_handler:
...
def test_image_edit_merges_headers_and_extra_headers():
    from litelm.images.main import base_llm_http_handler
...
class TestCallTypesOCR:
    ...
    Fixes https://github.com/BerriAI/litelm/issues/17381
    """
