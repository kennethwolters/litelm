...
model key in litelm.model_cost.
...
def test_should_not_pollute_shared_key_with_zero_cost_pricing():
    ...
    builtin_info = litelm.get_model_info(model=backend_model)
...
def test_should_store_full_pricing_under_deployment_model_id():
    ...
    retrievable via the unique model_id key in litelm.model_cost.
    """
    backend_model = "vertex_ai/gemini-2.5-flash"
    ...
    entry = litelm.model_cost.get("deployment-zero-check")
