...
if "DATABASE_URL" not in os.environ:
    os.environ["DATABASE_URL"] = "postgresql://llmproxy:dbpassword9090@localhost:5432/litelm"


def use_local_litelm() -> bool:
    return os.environ.get("USE_LOCAL_LITELLM", "false").lower() == "true"


def use_remote_litelm() -> bool:
    return not use_local_litelm()
...
def get_local_litelm_base_url() -> str:
    return LOCAL_LITELLM_BASE_URL


def get_remote_litelm_base_url() -> str:
    return os.environ.get("LITELLM_BASE_URL", "").rstrip("/")


def get_litelm_base_url() -> str:
    if use_local_litelm():
        return get_local_litelm_base_url()
    return get_remote_litelm_base_url()


def get_litelm_api_key() -> str:
    if use_local_litelm():
        return "sk-1234"
...
class BaseLiteLLMIntegrationTest(ABC):
    ...
    @staticmethod
    def get_api_key() -> str:
        return get_litelm_api_key()

    @staticmethod
    def get_base_url() -> str:
        return get_litelm_base_url()
    ...
    def setup_class(cls):
        ...
        if use_remote_litelm() and isinstance(verify_setting, str):
    ...
    def extract_model_params(model_data: Dict[str, Any]) -> Tuple[str, str, str, str]:
        ...
        provider = model_info.get("litelm_provider", "unknown")
        litelm_params = model_data.get("litelm_params", {})

        if provider == "azure":
            api_base = litelm_params.get("api_base", "unknown")
            if api_base != "unknown" and "//" in api_base:
                domain_name = api_base.split("//")[1]
                deployment = domain_name.split(".")[0]
            else:
                deployment = "unknown"
            api_version = litelm_params.get("api_version", "unknown")
        elif provider in ["bedrock", "bedrock_converse"]:
            deployment = litelm_params.get("aws_region_name", "unknown")
            api_version = "unknown"
        else:
            deployment = "unknown"
            api_version = "unknown"
    ...
    @classmethod
    def _fetch_all_models_from_litelm(cls) -> List[Dict[str, Any]]:
    ...
    def _fetch_all_approved_models(cls) -> List[Dict[str, Any]]:
        return cls._fetch_all_models_from_litelm()
...
class UserKeyTestMixin:
    ...
    def setup_admin_client(cls):
        cls._base_url = get_litelm_base_url()
        cls._master_api_key = get_litelm_api_key()
        verify_setting = (
            False
            if use_local_litelm()
