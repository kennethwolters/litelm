...
pytestmark = pytest.mark.usefixtures("mock_azure_server", "litelm_proxy_server")
...
def test_litelm_proxy_health(litelm_proxy_server):
    """Verify LiteLLM proxy is running and healthy."""
    response = httpx.get(f"{litelm_proxy_server}/health", timeout=5.0)
    assert response.status_code == 200
    print(f"✓ LiteLLM proxy is healthy at {litelm_proxy_server}")


def test_litelm_proxy_model_list(litelm_proxy_server):
    """Verify LiteLLM proxy can list models."""
    response = httpx.get(
        f"{litelm_proxy_server}/v1/models",
