...
Maintainer attestation, 2026-05-14: upstream litellm was manually reviewed through `649eb2d`. The weekly automated upstream watch tracks structural API drift and was green on 2026-09-07. Local scoped tests on 2026-09-11: `228 passed, 54 skipped`; all 44 available-provider live tests and all 10 DSPy smoke tests also passed with the current dependency lock.

This attests litelm's declared routing/formatting/DSPy surface only, not full litellm compatibility.

## Status

**Alpha.** 228 own tests passing. The last fully classified ported LiteLLM baseline had 56 passing tests; a current-baseline reclassification is tracked in [#14](https://github.com/kennethwolters/litelm/issues/14).
...
```bash
uv run --extra all pytest tests/ -x --ignore=tests/ported --timeout=10  # 228 non-live tests
